"""nb3161 -- Range-clip nb3080 predictions to training-range quantile band.

NEW PARADIGM:
    Simpler clipping. Bound nb3080 predictions tightly to the in-sample range
    of fold-train truth y. Per-fold (q05, q95) of fold-train y defines the
    clip band; fold-val predictions are np.clip()-ped to that band. No model
    fit, no per-row blending -- just a 1-parameter-per-fold floor/ceiling.

    Hypothesis: nb3080 deep-30 blend (mean 0.4475) over-predicts the high tail
    and under-predicts the low tail (variance compression by training-range
    extrapolation). Tighter clipping to fold-train (q05, q95) tames the tails
    -> potential -0.001-0.003 RAE gain similar to rank-stretch family.
...
PROTOCOL (per kf_seed, 5-fold scaffold split):
    ...
    Repeat for 15 FRESH kf_seeds {1141..1155}.

GATE (on 15-seed mean):
    mean < 0.4475 -> "BETTER"
        (improves over nb3080 wide-seed 15-mean 0.4475)
    else        -> "FAIL"
        (range clipping does not help this anchor)
...
Outputs:
    data/processed/nb3161_summary.json
    data/processed/nb3161_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3161.npy         (513,) float32 -- deploy te
    submissions/nb3161_range_clip_nb3080.csv  (only on BETTER verdict)
"""
...
def main() -> dict:
    ...
    print("=" * 78)
    print(
        f"{TAG} -- RANGE-CLIP per-fold (q{Q_LOW:.2f}, q{Q_HIGH:.2f}) on "
    ...
    print(
        f"          gate: mean < {GATE_BETTER:.4f} -> BETTER, else FAIL"
    )
    print("=" * 78)
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3161 15-seed mean {mean_rae:.4f} beats "
            f"nb3080 parent {REF_NB3080:.4f} ({mean_rae - REF_NB3080:+.4f}). "
            f"Per-fold (q{Q_LOW:.2f}, q{Q_HIGH:.2f}) y-range clip yields real "
        ...
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3161 15-seed mean {mean_rae:.4f} does NOT beat "
            f"nb3080 parent {REF_NB3080:.4f} ({mean_rae - REF_NB3080:+.4f}). "
            f"Per-fold y-range clip at (q{Q_LOW:.2f}, q{Q_HIGH:.2f}) is "
            f"NEUTRAL or HURTS on this anchor (nb3080 preds already lie "
            f"mostly inside training range). Keep nb3080 / prior PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_range_clip_nb3080.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": (
            "per_fold_y_range_clip_nb3080_pred_oof_q05_q95"
        ),
