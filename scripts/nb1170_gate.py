"""nb1170_gate — honest ensemble gate for the EXTENDED sister-NR pooled aux head vs the DEPLOYED
sisterNR 3-head GNN (best 0.4367). Mirrors nb1169_gate; control = sn_oof (deployed), treatment = sne_oof
(deployed sisterNR head extended with FXR/LXR/RORg/RXRa/ERa agonist EC50).
Deploy: sne < deployed best - 0.001 AND sne < control on the matched 4-GBM deployed config.
"""
import os, sys, json
import numpy as np
...
def run_config(archs, label, Xtr, ytr, se, n_tr, chem, tab):
    ...
    arms = {"control": "sn_oof", "sne": "sne_oof"}
    res = {k: [] for k in arms}
...
def main():
    ...
    out = {"5arch": {"means": m5, "std": s5, "sne_minus_control": round(d5, 4)},
           "4gbm_deployed": {"means": m4, "std": s4, "sne_minus_control": round(d4, 4)}}
    json.dump(out, open(f"{P}/nb1170_summary.json", "w"), indent=2)

    prev = json.load(open(BEST))["rae"] if os.path.exists(BEST) else 0.4367
    sne4, ctl4 = m4["sne"], m4["control"]
    if d4 < -0.001 and sne4 < ctl4:
        print(f"\nDEPLOY-WORTHY: 4-GBM matched sne-control {d4:+.4f} (<-0.001). Build deploy submission next.")
    else:
