"""nb1951 -- Deploy nb1941 pooled 50-bag (LGBM MSE / regression) to 513-row CSV.

PROTOCOL:
    Same 117-col 5-way K-tuned feature stack as nb1941, but deploy on full 513.
    1. For 10 outer x 5 inner = 50 deploy fits:
         LGBM(objective='regression' (MSE), max_depth=4, num_leaves=15,
              n_est=300, lr=0.03, min_child_samples=5, reg_lambda=2).
         Outer seeds = {0,1,2,7,42,99,137,250,500,750}
         Inner seed  = outer*1000 + s  for s in {0,1,7,42,137}.
    2. Each fit on ALL 253 unblind compounds with target
         residual_unb = y_unb - chemprop_aux te[unb_idx].
         Predict residual on full 513 -> residual_513_k.
    3. Stack 50 residual_513 vectors. Row-level mean -> mean_residual_513.
    4. te_nb1951 = te_chemprop_aux + mean_residual_513.
    5. Save submissions/nb1951_deploy_nb1941.csv (513 rows).

ANCHORS:
    nb1941 pooled 50-bag honest cross-fit RAE = 0.4990 (predicted LB 0.502).
"""
...
INNER_PER_OUTER = len(INNER_OFFSETS)
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- mirrors nb1941/nb1861 exactly; all knobs fixed."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    n_fits_total = len(OUTER_SEEDS) * INNER_PER_OUTER
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1941 pooled 50-bag (LGBM MSE) to 513-row CSV")
    print(f"          outer seeds = {OUTER_SEEDS}")
    print(f"          inner offsets = {INNER_OFFSETS}  inner/outer = "
          f"{INNER_PER_OUTER}  total fits = {n_fits_total}")
    print(f"          anchor = {ANCHOR}  (PRE-unblind te slice)")
    print(f"          nb1941 honest cross-fit ref = {NB1941_HONEST_REF:.4f}  "
          f"(predicted LB {PREDICTED_LB:.3f})")
    print("=" * 78)
    ...
    if X_unb.shape[1] != feat_dim:
        raise ValueError(
            f"X_unb feat dim {X_unb.shape[1]} != X_te feat dim {feat_dim}"
        )
    print(f"\n   COMBINED 5-WAY K-TUNED matrix: X_te={X_te.shape}  "
          f"X_unb={X_unb.shape}")

    # ---- DEPLOY: 50 fits on full 253 unblind, predict 513 ----
    print("\n" + "-" * 78)
    print(f"DEPLOY: {n_fits_total} LGBM(MSE) fits on all {n_unb} unblind, "
          f"predict {n_test}")
    print("-" * 78)
    all_resid_513 = np.zeros(
        (n_fits_total, n_test), dtype=np.float64
    )
    fit_records = []
    k = 0
    for o_seed in OUTER_SEEDS:
        for s in INNER_OFFSETS:
            i_seed = o_seed * 1000 + s
            t_in = time.time()
            mdl = lgb.LGBMRegressor(**_lgbm_params(i_seed))
            mdl.fit(X_unb, residual_unb)
            resid_513 = mdl.predict(X_te)
            ...
            print(f"   fit {k+1:2d}/{n_fits_total}  outer={o_seed:4d}  "
                  f"inner={i_seed:6d}  "
    ...
    te_nb1951 = te_anchor_513 + mean_resid_513

    # ---- In-sample check on unb_idx ----
    in_pred_unb = te_nb1951[unb_idx]
    rae_in_unb = float(rae(y_unb, in_pred_unb))
    print("\n" + "-" * 78)
    print(f"in-sample RAE on unb_idx = {rae_in_unb:.4f}")
    print(f"anchor in_RAE             = {rae_anchor:.4f}")
    print(f"nb1941 pooled 50-bag      = {NB1941_HONEST_REF:.4f} "
          f"(honest cross-fit)")
    print("-" * 78)

    # ---- Stats ----
    te_mean = float(te_nb1951.mean())
    te_std = float(te_nb1951.std())
    te_min = float(te_nb1951.min())
    te_max = float(te_nb1951.max())
    print(f"\nte_nb1951 stats: mean={te_mean:.4f}  std={te_std:.4f}  "
    ...
    df_sub = pd.DataFrame({
        ...
        "pEC50": te_nb1951.astype(np.float32),
    })
    if len(df_sub) != 513:
        raise ValueError(f"submission rows {len(df_sub)} != 513")
    sub_path = SUBMISSIONS_DIR / f"{TAG}_deploy_nb1941.csv"
    df_sub.to_csv(sub_path, index=False)
    print(f"\n[save] submission CSV: {sub_path}  ({len(df_sub)} rows)")

    # ---- Save te artifact ----
    te_path = DATA_PROCESSED / f"te_{TAG}.npy"
    np.save(te_path, te_nb1951.astype(np.float32))
    ...
    summary = {
        ...
        "te_nb1951_mean": te_mean,
        "te_nb1951_std": te_std,
        "te_nb1951_min": te_min,
        "te_nb1951_max": te_max,
        "in_RAE_unb_idx": rae_in_unb,
        "nb1941_pooled_50bag_honest": NB1941_HONEST_REF,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_test", "n_unb", "feat_dim", "n_fits",
        "rae_anchor_chemprop_aux_unb",
        "te_nb1951_mean", "te_nb1951_std",
        "te_nb1951_min", "te_nb1951_max",
        "in_RAE_unb_idx",
        "nb1941_pooled_50bag_honest", "predicted_LB",
        "submission_csv",
    ):
        print(f"  {k}: {res.get(k)}")
