"""nb2053 -- LGBM(MSE) on 117-col 5-way K-tuned with n_estimators=500.

HYPOTHESIS:
    nb1852 used LGBM(MSE, max_depth=4, num_leaves=15, n_estimators=300, lr=0.03,
    min_child_samples=5, reg_lambda=2.0) and reported rae_mean_bag ~0.5100.
    nb1861 outer-bag-of-bags reproduced at BoB MEAN ~0.5013.

    Does increasing n_estimators 300 -> 500 (more boosting rounds, same lr/depth)
    improve, hurt, or leave unchanged the 5-seed cross-fit mean-bag RAE?

PROTOCOL:
    1. Features = 117-col 5-way K-tuned matrix (AtomPair + MACCS + Mordred +
       ChempropEmbed + Avalon + chembl_pec50_knn + mean_sim).
    2. Anchor = chemprop_aux te[unb_idx] (PRE-unblind, in_RAE 0.6216).
    3. LGBM(objective='regression' i.e. MSE, max_depth=4, num_leaves=15,
       n_estimators=500, learning_rate=0.03, min_child_samples=5,
       reg_lambda=2.0).
    4. 5 seeds = [0, 1, 7, 42, 137].
       For each seed: 5-fold KFold cross-fit (shuffle=True, random_state=seed)
       on residual=y_unb-anchor; corrected = anchor + resid_oof.
       Per-seed RAE = rae(y_unb, corrected).
       Mean-bag RAE  = rae(y_unb, mean(corrected over 5 seeds)).
    5. Verdict vs nb1861 BoB MEAN 0.5013.

Outputs:
    scripts/nb2053_lgbm_n500.py
    data/processed/nb2053_summary.json
    data/processed/nb2053_per_seed_oof.npy   (5, 253) float32
    data/processed/nb2053_mean_bag_oof.npy   (253,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) with n_estimators=500 (vs nb1852's 300); all else identical."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE, n_est={N_ESTIMATORS}) on 117-col 5-way K-tuned; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1861 BoB MEAN ({NB1861_REF_FALLBACK:.4f})")
    print("=" * 78)

    # ---- nb1852 / nb1861 references ----
    ...
    # ---- 5-SEED BAG LOOP ----
    print("\n" + "-" * 78)
    print(f"5-SEED BAG: {len(SEEDS)} seeds x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim}, n_est={N_ESTIMATORS})")
    print("-" * 78)
    ...
    for s_i, seed in enumerate(SEEDS):
        ...
        print(f"   seed={seed:4d}  rae={rae_s:.4f}  "
    ...
    print(f"   per-seed MED   RAE  = {per_seed_median:.4f}")
    print(f"   per-seed std/min/mx = {per_seed_std:.4f} / "
          f"{per_seed_min:.4f} / {per_seed_max:.4f}")
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "N500_BEATS_NB1861"
    elif delta_vs_nb1861 > DECISION_MARGIN:
        verdict = "N500_LOSES_TO_NB1861"
    else:
        verdict = "N500_TIES_NB1861"
