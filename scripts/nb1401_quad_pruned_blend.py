"""nb1401 -- Quad SHAP-pruned residual-learner blend.

Combine 4 SHAP-pruned residual learners from different fingerprint /
descriptor spaces:

    A nb1373  AtomPair-30   mean_bag RAE 0.5095   (current best)
    B nb1352  MACCS-20      mean_bag RAE 0.5323
    C nb1364  Mordred-30    mean_bag RAE 0.5242
    D nb1392  Avalon-30     mean_bag RAE 0.5391

HYPOTHESIS:  4 orthogonal feature spaces -- a 4-way convex blend may
extract additional gain via diversification, even though three of the
four candidates are individually weaker than nb1373.

PROTOCOL:
    1. Load the 4 *_mean_bag_oof.npy arrays (n_unb,).
    2. Load truth y_unb (audit_unblind_y.npy) + sanity-check shapes.
    3. Pairwise pred Pearson + residual Pearson (vs truth).
    4. 4D simplex GRID search at step 0.10  ->  C(13, 3) = 286 grid pts.
    5. Honest 5-fold cross-fit SLSQP convex blend (re-fit weights per
       outer fold; report pooled cross-fit RAE).
    6. Naive 1/4 mean blend.
    7. Best 1-anchor stretch: keep one component (nb1373) at high weight
       and sweep small perturbations (0.7-0.95) -- this is the
       "stay close to PRIMARY" probe.
    8. Verdict at 0.003 RAE margin vs nb1373 mean_bag (0.5095).

Outputs:
    scripts/nb1401_quad_pruned_blend.py            (this file)
    data/processed/nb1401_summary.json
    data/processed/nb1401_best_oof.npy              (n_unb,) float32
"""
...
CANDIDATES = [
    ("nb1373", "AtomPair-30",  0.5095),  # current PRIMARY-1 candidate
    ("nb1352", "MACCS-20",     0.5323),
    ("nb1364", "Mordred-30",   0.5242),
    ("nb1392", "Avalon-30",    0.5391),
]

REF_RAE = 0.5095        # nb1373 mean_bag
...
def _simplex_grid(n_components: int, step: float):
    ...
    # generate all non-negative integer compositions of k into n_components parts
...
def _slsqp_blend(y: np.ndarray, P: np.ndarray) -> np.ndarray:
    """In-sample SLSQP convex blend (returns weight vector)."""
    n_c = P.shape[1]
    ...
    if not res.success:
        # fall back to naive mean if optimizer fails
        return w0
    return res.x
...
def _slsqp_crossfit(y: np.ndarray, P: np.ndarray,
    ...
    """5-fold cross-fit SLSQP: refit weights on train fold, predict on val."""
    n = len(y)
...
def _anchor_stretch(y: np.ndarray, P: np.ndarray, anchor_col: int) -> list[dict]:
    """Hold anchor at weight w, distribute (1-w) equally across the other 3."""
    rows = []
    others = [j for j in range(P.shape[1]) if j != anchor_col]
    for w_anchor in np.arange(0.55, 1.001, 0.05):
        rem = (1.0 - w_anchor) / len(others)
        w = np.zeros(P.shape[1])
        w[anchor_col] = w_anchor
        for j in others:
            w[j] = rem
        pred = P @ w
        r = float(rae(y, pred))
        rows.append({"w_anchor": float(round(w_anchor, 4)),
                     "w_others_each": float(round(rem, 4)),
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 4-way SHAP-pruned residual-learner blend")
    print("           anchors: nb1373 (AtomPair) + nb1352 (MACCS) + "
          "nb1364 (Mordred) + nb1392 (Avalon)")
    print(f"           ref_rae = {REF_RAE:.4f}  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- 4D simplex grid (step 0.10) ----
    print("\n" + "-" * 78)
    print(f"4D SIMPLEX GRID  (step = {GRID_STEP}, expect 286 tuples)")
    print("-" * 78)
    grid_rows = _grid_search(y_unb, P, GRID_STEP)
    n_grid = len(grid_rows)
    print(f"   grid pts evaluated = {n_grid}")
    print(f"   top 5 tuples (w order = nb1373, nb1352, nb1364, nb1392):")
    for r in grid_rows[:5]:
        w = r["w"]
        print(f"     w=({w[0]:.2f}, {w[1]:.2f}, {w[2]:.2f}, {w[3]:.2f})  "
              f"RAE={r['rae']:.4f}")
    ...
    print(f"\n   naive 1/4 mean RAE = {naive_rae:.4f}")
    ...
    fold_w_arr = np.stack(fold_w, axis=0)  # (n_folds, n_c)
    ...
    # ---- Anchor stretch around nb1373 (col 0) ----
    print("\n" + "-" * 78)
    print("ANCHOR-STRETCH AROUND nb1373 (col 0)")
    print("-" * 78)
    stretch_rows = _anchor_stretch(y_unb, P, anchor_col=0)
    print(f"   top 5 stretch settings:")
    for r in stretch_rows[:5]:
        print(f"     w_anchor={r['w_anchor']:.2f}  "
              f"w_others_each={r['w_others_each']:.4f}  "
              f"RAE={r['rae']:.4f}")

    # ---- Pick BEST_OOF for export ----
    candidates_rae = {
        "grid_best": (best_grid_rae, best_grid_oof, best_grid_w.tolist()),
        "naive_quarter": (naive_rae, naive_oof, naive_w.tolist()),
    ...
    # But if grid_best is meaningfully better honest-wise, note it.
    # We export crossfit_oof because cross-fit RAE is LB-faithful.

    beats_nb1373 = pick_rae < REF_RAE - DECISION_MARGIN
    flat_vs_nb1373 = abs(pick_rae - REF_RAE) < DECISION_MARGIN

    if beats_nb1373:
        verdict = "QUAD_BLEND_BEATS_NB1373"
    elif flat_vs_nb1373:
        verdict = "QUAD_BLEND_FLAT_VS_NB1373"
    else:
        verdict = "QUAD_BLEND_WORSE_THAN_NB1373"

    print("\n" + "=" * 78)
    print(f"FINAL  pick={pick_label}  RAE={pick_rae:.4f}  "
          f"d_vs_nb1373={pick_rae - REF_RAE:+.4f}")
    ...
    summary = {
        ...
        "naive_quarter_w": naive_w.tolist(),
        "naive_quarter_rae": naive_rae,
        ...
        "anchor_stretch_top5": stretch_rows[:5],
        "anchor_stretch_all": stretch_rows,
        "pick_label": pick_label,
        "pick_rae": pick_rae,
        "pick_w": pick_w,
        "delta_pick_vs_nb1373": pick_rae - REF_RAE,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_unb", "n_components", "ref_rae_nb1373",
        "n_grid_pts",
