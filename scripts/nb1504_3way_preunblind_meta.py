"""nb1504 -- 3-way PRE-unblind meta-blend (different mechanisms).

Blends three PRE-unblind 253-vector OOFs:
    A  nb1484_best_oof          (SHAP-pruned 4-family chemprop_aux residual, SLSQP 5fold)
    B  nb1493_best_s_oof        (rank-stretch on nb1472 mean, in-sample best s)
    C  nb1483_mean_bag_oof      (CatBoost-MAE residual mean bag over 5 seeds)

All three are PRE-unblind LB-clean (anchored to chemprop_aux te file or to
nb1472 mean OOF which is itself PRE-unblind).

PROTOCOL:
    1. Load the three OOFs + unblind truth.
    2. Compute pairwise Pearson correlations (decorrelation diagnostic).
    3. 3D simplex grid (step 0.05; sum-to-1; non-negative) brute force.
    4. 5-fold cross-fit SLSQP convex blend.
    5. Verdict at 0.003 margin vs nb1484 best (0.5231).

Outputs:
    scripts/nb1504_3way_preunblind_meta.py        (this file)
    data/processed/nb1504_summary.json
    data/processed/nb1504_best_oof.npy             (253,) float32
"""
from __future__ import annotations

import json
import os
import sys
import time
import warnings
from itertools import product
from pathlib import Path

os.environ["PYTHONIOENCODING"] = "utf-8"
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "src"))
warnings.filterwarnings("ignore")

import numpy as np
from scipy.optimize import minimize
from sklearn.model_selection import KFold

from pxr.eval import rae
from pxr.paths import DATA_PROCESSED

TAG = "nb1504"
NB1484_REF = 0.5230936811989908  # nb1484 best SLSQP 5fold
DECISION_MARGIN = 0.003
GRID_STEP = 0.05
SLSQP_FOLDS = 5
SLSQP_SEED = 42

INPUTS = [
    ("A_nb1484", DATA_PROCESSED / "nb1484_best_oof.npy"),
    ("B_nb1493", DATA_PROCESSED / "nb1493_best_s_oof.npy"),
    ("C_nb1483", DATA_PROCESSED / "nb1483_mean_bag_oof.npy"),
]


def _pearson(a: np.ndarray, b: np.ndarray) -> float:
    a = np.asarray(a, dtype=np.float64).ravel()
    b = np.asarray(b, dtype=np.float64).ravel()
    a = a - a.mean()
    b = b - b.mean()
    den = float(np.sqrt((a * a).sum() * (b * b).sum()))
    if den < 1e-12:
        return float("nan")
    return float((a * b).sum() / den)


def _enumerate_simplex(step: float):
    """Yield (w1,w2,w3) tuples on 3-simplex with given step."""
    n_steps = int(round(1.0 / step))
    out = []
    for i in range(n_steps + 1):
        for j in range(n_steps + 1 - i):
            k = n_steps - i - j
            w = (i / n_steps, j / n_steps, k / n_steps)
            out.append(w)
    return out


def _slsqp_one_fold(preds_tr: np.ndarray, y_tr: np.ndarray) -> np.ndarray:
    """SLSQP: minimize RAE on training fold; weights sum-to-1 nonnegative."""
    K = preds_tr.shape[1]

    def loss(w):
        p = preds_tr @ w
        return float(rae(y_tr, p))

    w0 = np.full(K, 1.0 / K, dtype=np.float64)
    cons = [{"type": "eq", "fun": lambda w: float(w.sum() - 1.0)}]
    bounds = [(0.0, 1.0)] * K
    res = minimize(loss, w0, method="SLSQP", bounds=bounds,
                   constraints=cons, options={"maxiter": 200, "ftol": 1e-10})
    w = np.clip(res.x, 0.0, 1.0)
    s = w.sum()
    if s < 1e-12:
        w = np.full(K, 1.0 / K)
    else:
        w = w / s
    return w


def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 3-way PRE-unblind meta-blend")
    print(f"   inputs: {[name for name, _ in INPUTS]}")
    print(f"   ref nb1484 = {NB1484_REF:.4f}   margin = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- Load truth ----
    y_unb = np.load(DATA_PROCESSED / "_audit_unblind_y.npy").astype(np.float64)
    n_unb = len(y_unb)
    print(f"[load] n_unb = {n_unb}")

    # ---- Load three OOFs ----
    arrays = []
    raes_solo = {}
    for name, path in INPUTS:
        if not path.exists():
            raise FileNotFoundError(f"missing input: {path}")
        a = np.load(path).astype(np.float64).ravel()
        if a.shape[0] != n_unb:
            raise ValueError(f"{name} shape {a.shape} != {n_unb}")
        arrays.append(a)
        r = float(rae(y_unb, a))
        raes_solo[name] = r
        print(f"[load] {name:<10s} from {path.name:<35s}  RAE = {r:.4f}")

    P = np.stack(arrays, axis=1)  # (253, 3)
    names = [name for name, _ in INPUTS]

    # ---- Pairwise Pearson ----
    print("\n" + "-" * 78)
    print("PAIRWISE PEARSON")
    print("-" * 78)
    K = P.shape[1]
    pearson_mat = np.zeros((K, K), dtype=np.float64)
    for i in range(K):
        for j in range(K):
            pearson_mat[i, j] = _pearson(P[:, i], P[:, j])
    pearson_pairs = {}
    for i in range(K):
        for j in range(i + 1, K):
            key = f"{names[i]}__vs__{names[j]}"
            pearson_pairs[key] = float(pearson_mat[i, j])
            print(f"   {names[i]:<10s}  <->  {names[j]:<10s}  r = "
                  f"{pearson_mat[i, j]:+.4f}")

    # ---- 3D simplex grid ----
    print("\n" + "-" * 78)
    print(f"3D SIMPLEX GRID  step = {GRID_STEP}")
    print("-" * 78)
    grid = _enumerate_simplex(GRID_STEP)
    grid_records = []
    for w in grid:
        w_arr = np.array(w, dtype=np.float64)
        pred = P @ w_arr
        r = float(rae(y_unb, pred))
        grid_records.append({"w": [float(x) for x in w], "rae": r})
    grid_records.sort(key=lambda d: d["rae"])
    print(f"   total grid points: {len(grid_records)}")
    print("   top-5:")
    for rec in grid_records[:5]:
        w = rec["w"]
        print(f"     w = ({w[0]:.2f}, {w[1]:.2f}, {w[2]:.2f})  "
              f"RAE = {rec['rae']:.4f}")
    grid_best = grid_records[0]
    w_grid_best = np.array(grid_best["w"], dtype=np.float64)
    pred_grid_best = P @ w_grid_best
    rae_grid_best = grid_best["rae"]

    # ---- 5-fold cross-fit SLSQP ----
    print("\n" + "-" * 78)
    print(f"5-FOLD CROSS-FIT SLSQP  seed = {SLSQP_SEED}")
    print("-" * 78)
    kf = KFold(n_splits=SLSQP_FOLDS, shuffle=True, random_state=SLSQP_SEED)
    cf_oof = np.full(n_unb, np.nan, dtype=np.float64)
    fold_records = []
    for fi, (tr_loc, va_loc) in enumerate(kf.split(np.arange(n_unb))):
        w_star = _slsqp_one_fold(P[tr_loc], y_unb[tr_loc])
        pred_va = P[va_loc] @ w_star
        cf_oof[va_loc] = pred_va
        rae_tr = float(rae(y_unb[tr_loc], P[tr_loc] @ w_star))
        rae_va = float(rae(y_unb[va_loc], pred_va))
        fold_records.append({
            "fold": int(fi),
            "n_tr": int(len(tr_loc)),
            "n_va": int(len(va_loc)),
            "w": [float(x) for x in w_star],
            "rae_tr": rae_tr,
            "rae_va": rae_va,
        })
        print(f"   fold {fi}: w = ({w_star[0]:.3f}, {w_star[1]:.3f}, "
              f"{w_star[2]:.3f})  rae_tr = {rae_tr:.4f}  rae_va = {rae_va:.4f}")
    rae_slsqp_cf = float(rae(y_unb, cf_oof))
    w_mean = np.mean([fr["w"] for fr in fold_records], axis=0).tolist()
    w_std = np.std([fr["w"] for fr in fold_records], axis=0).tolist()
    print(f"   SLSQP cross-fit RAE = {rae_slsqp_cf:.4f}")
    print(f"   w_mean = ({w_mean[0]:.3f}, {w_mean[1]:.3f}, {w_mean[2]:.3f})")
    print(f"   w_std  = ({w_std[0]:.3f}, {w_std[1]:.3f}, {w_std[2]:.3f})")

    # ---- Pick best variant ----
    candidates = {
        "grid_best": (rae_grid_best, pred_grid_best),
        "slsqp_5fold": (rae_slsqp_cf, cf_oof),
    }
    best_tag = min(candidates, key=lambda k: candidates[k][0])
    rae_best, oof_best = candidates[best_tag]

    # ---- Verdict ----
    delta_grid_vs_nb1484 = rae_grid_best - NB1484_REF
    delta_slsqp_vs_nb1484 = rae_slsqp_cf - NB1484_REF
    delta_best_vs_nb1484 = rae_best - NB1484_REF

    beats_nb1484_grid = rae_grid_best < NB1484_REF - DECISION_MARGIN
    beats_nb1484_slsqp = rae_slsqp_cf < NB1484_REF - DECISION_MARGIN
    beats_nb1484_best = rae_best < NB1484_REF - DECISION_MARGIN
    flat_vs_nb1484_best = abs(rae_best - NB1484_REF) < DECISION_MARGIN

    if beats_nb1484_best:
        verdict = f"NB1504_3WAY_META_BEATS_NB1484_NEW_PRIMARY_via_{best_tag}"
    elif flat_vs_nb1484_best:
        verdict = f"NB1504_3WAY_META_FLAT_VS_NB1484  ({best_tag} RAE {rae_best:.4f})"
    else:
        verdict = f"NB1504_3WAY_META_HURTS_NB1484  ({best_tag} RAE {rae_best:.4f})"

    # PRE-unblind sanity: all 3 inputs are PRE-unblind anchored
    pre_unblind_clean = True

    print("\n" + "=" * 78)
    print("VERDICT")
    print("=" * 78)
    print(f"   solo RAE: {raes_solo}")
    print(f"   grid_best   RAE = {rae_grid_best:.4f}  delta vs nb1484 = "
          f"{delta_grid_vs_nb1484:+.4f}  beats = {beats_nb1484_grid}")
    print(f"   slsqp_5fold RAE = {rae_slsqp_cf:.4f}  delta vs nb1484 = "
          f"{delta_slsqp_vs_nb1484:+.4f}  beats = {beats_nb1484_slsqp}")
    print(f"   BEST = {best_tag}   RAE = {rae_best:.4f}")
    print(f"   verdict = {verdict}")
    print(f"   pre_unblind_clean = {pre_unblind_clean}")

    # ---- Save ----
    np.save(DATA_PROCESSED / f"{TAG}_best_oof.npy",
            oof_best.astype(np.float32))
    print(f"\n[save] {DATA_PROCESSED / f'{TAG}_best_oof.npy'}")

    summary = {
        "tag": TAG,
        "inputs": [
            {"name": name, "path": str(path), "rae_solo": raes_solo[name]}
            for name, path in INPUTS
        ],
        "n_unb": int(n_unb),
        "pearson_pairs": pearson_pairs,
        "pearson_matrix": [[float(x) for x in row] for row in pearson_mat],
        "grid_step": GRID_STEP,
        "grid_n_points": len(grid_records),
        "grid_top5": grid_records[:5],
        "grid_best_w": [float(x) for x in w_grid_best.tolist()],
        "grid_best_rae": float(rae_grid_best),
        "slsqp_folds": SLSQP_FOLDS,
        "slsqp_seed": SLSQP_SEED,
        "slsqp_fold_records": fold_records,
        "slsqp_w_mean": w_mean,
        "slsqp_w_std": w_std,
        "slsqp_crossfit_rae": float(rae_slsqp_cf),
        "candidate_rae": {k: float(v[0]) for k, v in candidates.items()},
        "best_variant": best_tag,
        "rae_best": float(rae_best),
        "nb1484_ref": NB1484_REF,
        "decision_margin": DECISION_MARGIN,
        "delta_grid_vs_nb1484": float(delta_grid_vs_nb1484),
        "delta_slsqp_vs_nb1484": float(delta_slsqp_vs_nb1484),
        "delta_best_vs_nb1484": float(delta_best_vs_nb1484),
        "beats_nb1484_grid": bool(beats_nb1484_grid),
        "beats_nb1484_slsqp": bool(beats_nb1484_slsqp),
        "beats_nb1484_best": bool(beats_nb1484_best),
        "flat_vs_nb1484_best": bool(flat_vs_nb1484_best),
        "verdict": verdict,
        "pre_unblind_clean": pre_unblind_clean,
        "wall_sec": round(time.time() - t0, 2),
    }
    out_path = DATA_PROCESSED / f"{TAG}_summary.json"
    with open(out_path, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"[save] {out_path}")
    print(f"[done] wall = {time.time() - t0:.1f}s")
    print("=" * 78)
    return summary


if __name__ == "__main__":
    res = main()
    print("\n==== SUMMARY ====")
    for k in (
        "n_unb",
        "pearson_pairs",
        "grid_best_w",
        "grid_best_rae",
        "slsqp_crossfit_rae",
        "best_variant",
        "rae_best",
        "delta_best_vs_nb1484",
        "beats_nb1484_best",
        "flat_vs_nb1484_best",
        "verdict",
        "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
