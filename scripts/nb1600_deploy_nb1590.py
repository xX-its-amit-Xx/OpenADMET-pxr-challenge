...
# 40 outer seeds.  If runtime is too long we will surface a 10-outer fallback.
OUTER_SEEDS = list(range(40))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1590 (40-outer CatBoost+LGBM blend) -> 513")
    ...
    print("\n" + "=" * 78)
    print("BoB MEAN OVER 40 OUTER SEEDS")
    print("=" * 78)
    ...
    csv_path = SUBMISSIONS / f"{TAG}_deploy_nb1590.csv"
    df = pd.DataFrame({
        "SMILES": test_smiles,
