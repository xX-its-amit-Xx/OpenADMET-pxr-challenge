"""nb2133 -- LR grid {0.02, 0.04, 0.06} at fixed K=28, L=15.

HYPOTHESIS:
    nb2103 found K=28 mean-bag RAE = 0.4737 as the new SHAP-pruning optimum
    on the 117-col 5-way K-tuned feature matrix (LGBM(MSE), max_depth=4,
    num_leaves=15, n_estimators=300, lr=0.03, min_child=5, reg_lambda=2.0,
    5-seed bag, 5-fold cross-fit).  This notebook holds K=28 / L=15 fixed
    and sweeps learning_rate in {0.02, 0.04, 0.06} to see whether the
    canonical lr=0.03 is locally optimal.

PROTOCOL:
    1. Reuse top-28 SHAP indices on the 117-col 5-way K-tuned matrix (same
       full-rank ordering from nb2063 SHAP importance, top-28 slice).
    2. For each lr in {0.02, 0.04, 0.06}: LGBM(MSE, max_depth=4,
       num_leaves=15, n_estimators=300, lr=X, min_child=5, reg_lambda=2),
       5-seed bag (seeds 0, 1, 7, 42, 137), 5-fold KFold cross-fit per seed.
    3. Verdict vs nb2103 K=28 lr=0.03 (0.4737) at decision_margin=0.003.

Outputs:
    scripts/nb2133_lr_grid_K28.py
    data/processed/nb2133_summary.json
    data/processed/nb2133_mean_bag_oof_lr{LR}.npy
"""
...
NB2103_K28_LR03_REF = 0.4737   # nb2103 K=28 lr=0.03 mean-bag RAE -- baseline
...
def _lgbm_params(seed: int, lr: float) -> dict:
    """LGBM(MSE) -- same as nb2103 except lr is swept."""
    return dict(
        objective="regression",
        max_depth=4,
        num_leaves=NUM_LEAVES_FIXED,
        n_estimators=300,
        learning_rate=lr,
        min_child_samples=5,
        reg_lambda=2.0,
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, lr: float) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = lgb.LGBMRegressor(**_lgbm_params(seed, lr))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LR grid {LR_GRID} at K={K_FIXED} L={NUM_LEAVES_FIXED} "
          f"on 117-col 5-way K-tuned matrix")
    print(f"          anchor={ANCHOR}  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          ref: nb2103 K=28 lr=0.03 mean-bag RAE = "
          f"{NB2103_K28_LR03_REF:.4f}  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    nb2063_k50_mean_bag = float(nb2063_sum.get("rae_mean_bag", 0.4933))
    print(f"[ref] nb2063.K=50 mean_bag_rae   = {nb2063_k50_mean_bag:.4f}")

    # ---- nb2103 K=28 lr=0.03 reference ----
    nb2103_k28_mean_bag = NB2103_K28_LR03_REF
    nb2103_k28_median_bag = NB2103_K28_LR03_REF
    if NB2103_SUMMARY.exists():
        ...
        for r in nb2103_sum.get("per_K_records", []):
            if int(r.get("K", -1)) == K_FIXED:
                ...
                break
    print(f"[ref] nb2103.K={K_FIXED} lr=0.03 mean_bag_rae   = "
          f"{nb2103_k28_mean_bag:.4f}")
    print(f"[ref] nb2103.K={K_FIXED} lr=0.03 median_bag_rae = "
    ...
    print(f"[ref] top-{K_FIXED} SHAP indices in 117-col matrix: "
          f"{topK_idx.tolist()}")
    ...
    # ---- LR-grid sweep ----
    print("\n" + "-" * 78)
    print(f"LR-GRID SWEEP at K={K_FIXED} L={NUM_LEAVES_FIXED}: {LR_GRID}")
    print("-" * 78)
    per_lr_results: list[dict] = []
    for lr in LR_GRID:
        print(f"\n--- lr={lr} ---")
        ...
        for i, s in enumerate(RESID_SEEDS):
            ts = time.time()
            resid_oof_s = _residual_cross_fit_one_seed(X_topK, residual, s, lr)
            ...
            print(f"   lr={lr} seed={s:3d}:  rae_corr = {rae_s:.4f}  "
        ...
        print(f"   lr={lr} per-seed RAE   = "
              f"[{', '.join(f'{r:.4f}' for r in per_seed_rae)}]")
        print(f"   lr={lr} per-seed mean  = {rae_per_seed_mean:.4f}  "
              f"std = {rae_per_seed_std:.4f}")
        print(f"   lr={lr} pooled mean    = {rae_mean_bag:.4f}  "
              f"(d_vs_anchor = {rae_mean_bag - rae_anchor:+.4f}  "
              f"d_vs_nb2103_K28_lr03 = {delta_vs_nb2103_k28:+.4f})")
        print(f"   lr={lr} pooled median  = {rae_median_bag:.4f}")
        if beats_nb2103_k28:
            verdict_lr = "BEATS_NB2103_K28_LR03"
        elif flat_vs_nb2103_k28:
            verdict_lr = "FLAT_VS_NB2103_K28_LR03"
        elif beats_anchor:
            verdict_lr = "BEATS_ANCHOR_BUT_WORSE_THAN_NB2103_K28_LR03"
        elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
            verdict_lr = "FLAT_VS_ANCHOR"
        else:
            verdict_lr = "HURTS_ANCHOR"
        print(f"   lr={lr} verdict        = {verdict_lr}")

        # Save mean-bag OOF per lr
        lr_str = f"{lr:.2f}".replace(".", "_")
        out_p = DATA_PROCESSED / f"{TAG}_mean_bag_oof_lr{lr_str}.npy"
        np.save(out_p, mean_bag_oof.astype(np.float32))
        print(f"   [save] {out_p}")

        per_lr_results.append({
            "lr": float(lr),
            "K": int(K_FIXED),
            "num_leaves": int(NUM_LEAVES_FIXED),
            ...
            "delta_mean_bag_vs_nb2103_K28_lr03": delta_vs_nb2103_k28,
            "beats_chemprop_aux": bool(beats_anchor),
            "beats_nb2103_K28_lr03": bool(beats_nb2103_k28),
            "flat_vs_nb2103_K28_lr03": bool(flat_vs_nb2103_k28),
            "verdict": verdict_lr,
        })

    # ---- Summary table ----
    print("\n" + "=" * 78)
    print(f"LR-GRID SUMMARY TABLE (K={K_FIXED} L={NUM_LEAVES_FIXED})")
    print("=" * 78)
    print(f"   {'lr':>6s}  {'mean_bag':>10s}  {'median_bag':>10s}  "
          f"{'per_seed_mean':>13s}  {'per_seed_std':>12s}  "
          f"{'d_vs_anchor':>11s}  {'d_vs_lr03':>11s}  verdict")
    print(f"   {0.03:>6.3f}  {nb2103_k28_mean_bag:>10.4f}  "
          f"{nb2103_k28_median_bag:>10.4f}  {'N/A':>13s}  {'N/A':>12s}  "
          f"{nb2103_k28_mean_bag - rae_anchor:>+11.4f}  {0.0:>+11.4f}  "
          f"BASELINE(nb2103_K28_lr03)")
    for r in per_lr_results:
        print(f"   {r['lr']:>6.3f}  {r['rae_mean_bag']:>10.4f}  "
              f"{r['rae_median_bag']:>10.4f}  {r['rae_per_seed_mean']:>13.4f}  "
              f"{r['rae_per_seed_std']:>12.4f}  "
              f"{r['delta_mean_bag_vs_chemprop_aux']:>+11.4f}  "
              f"{r['delta_mean_bag_vs_nb2103_K28_lr03']:>+11.4f}  "
              f"{r['verdict']}")

    # best lr by mean-bag RAE among the SWEEP (excluding lr=0.03 ref)
    sweep_rae = [r["rae_mean_bag"] for r in per_lr_results]
    best_sweep_i = int(np.argmin(sweep_rae))
    best_lr_sweep = float(per_lr_results[best_sweep_i]["lr"])
    best_lr_sweep_rae = float(per_lr_results[best_sweep_i]["rae_mean_bag"])

    # best lr overall (including lr=0.03 nb2103 ref)
    all_lr = [0.03] + [r["lr"] for r in per_lr_results]
    all_rae = [nb2103_k28_mean_bag] + sweep_rae
    best_overall_i = int(np.argmin(all_rae))
    best_lr_overall = float(all_lr[best_overall_i])
    best_lr_overall_rae = float(all_rae[best_overall_i])

    print(f"\n   best lr in sweep   = {best_lr_sweep}  "
          f"(mean_bag RAE {best_lr_sweep_rae:.4f})")
    print(f"   best lr overall    = {best_lr_overall}  "
          f"(mean_bag RAE {best_lr_overall_rae:.4f})")
    print(f"   delta best vs nb2103 K=28 lr=0.03 = "
          f"{best_lr_overall_rae - nb2103_k28_mean_bag:+.4f}")

    # ---- Overall verdict ----
    if best_lr_overall == 0.03:
        global_verdict = "NB2103_K28_LR03_REMAINS_OPTIMAL_LR_GRID_FLAT_OR_WORSE"
    elif best_lr_overall_rae < nb2103_k28_mean_bag - DECISION_MARGIN:
        global_verdict = (
            f"LR_GRID_BEATS_NB2103_K28_LR03_AT_LR={best_lr_overall}"
        )
    elif abs(best_lr_overall_rae - nb2103_k28_mean_bag) < DECISION_MARGIN:
        global_verdict = (
            f"LR_GRID_FLAT_VS_NB2103_K28_LR03_BEST_LR={best_lr_overall}"
        )
    else:
        global_verdict = "LR_GRID_DOES_NOT_BEAT_NB2103_K28_LR03"
    ...
    summary = {
        "tag": TAG,
        "method": (
            f"lgbm_mse_lr_grid_K{K_FIXED}_L{NUM_LEAVES_FIXED}_lr"
            f"_0_02_0_04_0_06_on_117col"
        ),
        "anchor": ANCHOR,
        ...
        "lgbm_max_depth": 4,
        "lgbm_num_leaves": NUM_LEAVES_FIXED,
        "lgbm_n_estimators": 300,
        "lgbm_lr_grid": LR_GRID,
        ...
        "nb2103_K28_lr03_mean_bag_ref": nb2103_k28_mean_bag,
        "nb2103_K28_lr03_median_bag_ref": nb2103_k28_median_bag,
        "per_lr_records": per_lr_results,
        "best_lr_sweep": best_lr_sweep,
        "best_lr_sweep_rae_mean_bag": best_lr_sweep_rae,
        "best_lr_overall": best_lr_overall,
        "best_lr_overall_rae_mean_bag": best_lr_overall_rae,
        "delta_best_lr_overall_vs_nb2103_K28_lr03": (
            best_lr_overall_rae - nb2103_k28_mean_bag
        ),
        "verdict": global_verdict,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb2103_K28_lr03_ref": NB2103_K28_LR03_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb2103_K28_lr03_mean_bag_ref",
        "best_lr_sweep", "best_lr_sweep_rae_mean_bag",
        "best_lr_overall", "best_lr_overall_rae_mean_bag",
        "delta_best_lr_overall_vs_nb2103_K28_lr03",
        "verdict", "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
    print("\n==== PER-LR TABLE ====")
    for r in res["per_lr_records"]:
        print(f"  lr={r['lr']:>5.3f}  mean_bag={r['rae_mean_bag']:.4f}  "
              f"median_bag={r['rae_median_bag']:.4f}  "
              f"per_seed_mean={r['rae_per_seed_mean']:.4f}  "
              f"std={r['rae_per_seed_std']:.4f}  "
              f"d_vs_lr03={r['delta_mean_bag_vs_nb2103_K28_lr03']:+.4f}  "
              f"{r['verdict']}")
