...
C4 = code("""COMP_DIM = 128; MACCS_DIM = 64
...
def make_features(fp4_a, fp4_q, fp6_a, fp6_q, ap_a, ap_q, tt_a, tt_q, rdfp_a, rdfp_q,
    ...
    def cd(fa, fb):
        return compress(np.minimum(fa,fb).astype(np.float32), COMP_DIM), \
               compress(np.abs(fa-fb).astype(np.float32), COMP_DIM)
...
C5 = code("""def train_3tier_models(idx_subset, tanimoto_sub):
...
with open('notebooks/123_nested_3tier_rdkit.ipynb', 'w') as f:
    json.dump(nb, f, indent=1)
print("Written notebooks/123_nested_3tier_rdkit.ipynb")
