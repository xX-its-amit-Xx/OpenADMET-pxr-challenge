"""nb2092 -- 25-bag POOLED SHAP top-30 (tighter estimate at the new optimum K).

PROTOCOL:
    1. Reuse top-30 SHAP feature indices from nb2081_summary.json
       (per_K_records[K=30]['top_K_idx_in_117']; best_K_sweep / best_K_overall).
...
Outputs:
    scripts/nb2092_pooled_25bag_shap30.py
    data/processed/nb2092_summary.json
    data/processed/nb2092_pooled25_oof.npy  (n_unb,) float32
"""
...
NB2082_SUMMARY = DATA_PROCESSED / "nb2082_summary.json"
...
def main() -> dict:
    ...
    # ---- Load nb2081 top-30 SHAP indices ----
    if not NB2081_SUMMARY.exists():
        raise FileNotFoundError(f"missing {NB2081_SUMMARY}")
    with open(NB2081_SUMMARY) as f:
        nb2081_sum = json.load(f)
    rec30 = _extract_K_record(nb2081_sum, "per_K_records", K=TOP_K_SHAP)
    top30_idx = np.array(rec30["top_K_idx_in_117"], dtype=np.int32)
    if top30_idx.shape[0] != TOP_K_SHAP:
        raise ValueError(
            f"nb2081 K=30 top_K_idx_in_117 has {top30_idx.shape[0]} entries, "
            f"expected {TOP_K_SHAP}"
        )
    nb2081_K30_mean_bag = float(rec30.get("rae_mean_bag", float("nan")))
    nb2081_best_K_sweep = int(nb2081_sum.get("best_K_sweep", -1))
    nb2081_best_K_overall = int(nb2081_sum.get("best_K_overall", -1))
    print(f"[reuse] nb2081 top-{TOP_K_SHAP} indices: "
          f"{top30_idx[:10].tolist()}...")
    print(f"[reuse] nb2081 K=30 rae_mean_bag (5-bag) = "
          f"{nb2081_K30_mean_bag:.6f}")
    print(f"[reuse] nb2081 best_K_sweep   = {nb2081_best_K_sweep}")
    print(f"[reuse] nb2081 best_K_overall = {nb2081_best_K_overall}")

    nb2063_mean_bag_actual = float("nan")
    if NB2063_SUMMARY.exists():
        with open(NB2063_SUMMARY) as f:
            nb2063_sum = json.load(f)
        nb2063_mean_bag_actual = float(
            nb2063_sum.get("rae_mean_bag", float("nan"))
        )
        print(f"[reuse] nb2063 rae_mean_bag (5-bag, K=50) = "
    ...
    if NB2071_SUMMARY.exists():
        ...
        print(f"[reuse] nb2071 BoB MEAN (5x5, K=50) = {nb2071_bob_mean:.6f}")

    nb2082_pooled25 = None
    if NB2082_SUMMARY.exists():
        with open(NB2082_SUMMARY) as f:
            nb2082_sum = json.load(f)
        nb2082_pooled25 = float(nb2082_sum.get("pooled25_rae", float("nan")))
        print(f"[reuse] nb2082 POOLED-25 (K=50)      = {nb2082_pooled25:.6f}")
    ...
    # ---- Restrict to top-30 (reused from nb2081) ----
    if int(top30_idx.max()) >= feat_dim:
        raise ValueError(
            f"nb2081 top30_idx.max()={int(top30_idx.max())} >= feat_dim "
            f"{feat_dim}")
    X_top30 = X_unb[:, top30_idx].astype(np.float32)
    print(f"   X_top30 shape = {X_top30.shape}")
    ...
    for o_i, o in enumerate(OUTER_SEEDS):
        ...
        for i_s, s in enumerate(inner_seeds):
            resid_oof_s = _residual_cross_fit_one_seed(X_top30, residual, s)
    ...
    delta_vs_nb2081_K30 = (pooled25_rae - nb2081_K30_mean_bag) \
        if not np.isnan(nb2081_K30_mean_bag) else None
    delta_vs_nb2082_K50 = (pooled25_rae - nb2082_pooled25) \
        if (nb2082_pooled25 is not None
            and not np.isnan(nb2082_pooled25)) else None
    delta_vs_nb2063_K50 = (pooled25_rae - nb2063_mean_bag_actual) \
        if not np.isnan(nb2063_mean_bag_actual) else None
    delta_vs_nb2071_K50 = (pooled25_rae - nb2071_bob_mean) \
        if (nb2071_bob_mean is not None
            and not np.isnan(nb2071_bob_mean)) else None
    if delta_vs_nb2081_K30 is not None:
        print(f"\n   POOLED-25 vs nb2081 K30 5-bag      : "
              f"d = {delta_vs_nb2081_K30:+.4f}")
    if delta_vs_nb2082_K50 is not None:
        print(f"   POOLED-25 vs nb2082 K50 POOLED-25  : "
              f"d = {delta_vs_nb2082_K50:+.4f}")
    if delta_vs_nb2063_K50 is not None:
        print(f"   POOLED-25 vs nb2063 K50 5-bag      : "
              f"d = {delta_vs_nb2063_K50:+.4f}")
    if delta_vs_nb2071_K50 is not None:
        print(f"   POOLED-25 vs nb2071 K50 BoB MEAN   : "
              f"d = {delta_vs_nb2071_K50:+.4f}")
    ...
    summary = {
        "tag": TAG,
        "method": "pooled_25_bag_shap_top30_lgbm_mse",
        "anchor": ANCHOR,
        ...
        "top30_idx_in_117_from_nb2081": top30_idx.tolist(),
        "feat_dim_full": int(feat_dim),
        "feat_dim_top30": int(X_top30.shape[1]),
        ...
        "nb2081_best_K_sweep": nb2081_best_K_sweep,
        "nb2081_best_K_overall": nb2081_best_K_overall,
        "nb2063_K50_mean_bag_ref": nb2063_mean_bag_actual,
        "nb2071_K50_bob_mean_ref": nb2071_bob_mean,
        "nb2082_K50_pooled25_ref": nb2082_pooled25,
        "delta_pooled25_vs_nb2081_K30_5bag": delta_vs_nb2081_K30,
        "delta_pooled25_vs_nb2082_K50_pooled25": delta_vs_nb2082_K50,
        "delta_pooled25_vs_nb2063_K50_5bag": delta_vs_nb2063_K50,
        "delta_pooled25_vs_nb2071_K50_bob_mean": delta_vs_nb2071_K50,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb2063_K50_mean_bag_ref", "nb2071_K50_bob_mean_ref",
        "nb2082_K50_pooled25_ref",
        "delta_pooled25_vs_nb2081_K30_5bag",
        "delta_pooled25_vs_nb2082_K50_pooled25",
        "delta_pooled25_vs_nb2063_K50_5bag",
        "delta_pooled25_vs_nb2071_K50_bob_mean",
        "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
