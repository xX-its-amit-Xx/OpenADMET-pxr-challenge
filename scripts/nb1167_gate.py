"""nb1167_gate — honest ensemble gate for CAR 3-head GNN vs the DEPLOYED octant-main-head GNN.

Mirrors nb1166_gate but the control is now the DEPLOYED model (octant_oof) and the treatment adds the CAR aux head:
  control  = octant_oof (deployed: PXR main+octant, ext-EC50 aux)
  car      = car_oof   (deployed + CAR aux head)
GBM/CheMeleon/TabPFN OOFs identical between arms. Reports both the 5-arch(topK incl mlp) and the
deployed-matched 4-GBM-no-mlp config (= best_ensemble.json members). Deploy: car < deployed best - 0.001 AND < control.
"""
import os, sys, json
import numpy as np
...
def run_config(archs, label, Xtr, ytr, se, n_tr, chem, tab):
    ...
    arms = {"control": "octant_oof", "car": "car_oof"}
    res = {k: [] for k in arms}
...
def main():
    ...
    out = {"5arch": {"means": m5, "std": s5, "car_minus_control": round(d5, 4)},
           "4gbm_deployed": {"means": m4, "std": s4, "car_minus_control": round(d4, 4)}}
    json.dump(out, open(f"{P}/nb1167_summary.json", "w"), indent=2)

    prev = json.load(open(BEST))["rae"] if os.path.exists(BEST) else 0.4386
    # deployed-matched (4-GBM) is the honest signal
    car4, ctl4 = m4["car"], m4["control"]
    if d4 < -0.001 and car4 < ctl4:
        print(f"\nDEPLOY-WORTHY: 4-GBM matched car-control {d4:+.4f} (<-0.001). Build deploy submission next.")
    else:
