"""nb1171_gate — honest ensemble gate for the PPARa/d-pooled sister-NR aux head vs the DEPLOYED
sisterNR 3-head GNN (best 0.4367). Mirrors nb1168_gate; control = sn_oof (deployed), treatment =
ppar_oof (deployed sisterNR head pooled with PPARalpha/PPARdelta agonist EC50).
Deploy: ppar < deployed best - 0.001 AND ppar < control on the matched 4-GBM deployed config.
"""
import os, sys, json
import numpy as np
...
def run_config(archs, label, Xtr, ytr, se, n_tr, chem, tab):
    ...
    arms = {"control": "sn_oof", "ppar": "ppar_oof"}
    res = {k: [] for k in arms}
...
def main():
    ...
    out = {"5arch": {"means": m5, "std": s5, "ppar_minus_control": round(d5, 4)},
           "4gbm_deployed": {"means": m4, "std": s4, "ppar_minus_control": round(d4, 4)}}
    json.dump(out, open(f"{P}/nb1171_summary.json", "w"), indent=2)

    prev = json.load(open(BEST))["rae"] if os.path.exists(BEST) else 0.4367
    pp4, ctl4 = m4["ppar"], m4["control"]
    if d4 < -0.001 and pp4 < prev:
        print(f"\nDEPLOY-WORTHY: 4-GBM matched ppar-control {d4:+.4f} (<-0.001), ppar {pp4:.4f} < best {prev}. Build deploy submission next.")
    else:
        print(f"\nno deploy: 4-GBM matched delta {d4:+.4f} (need <-0.001); ppar {pp4:.4f} vs best {prev}; best stays {prev}")
