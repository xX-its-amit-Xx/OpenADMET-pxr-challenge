"""nb1061 -- Per-quantile-bin rank-stretch on nb1014, 10 bins (finer than nb1053).

nb1053 used 5 quantile bins (~50/bin in train fold) on top of te_nb1014 and
fit per-bin stretch s_b. This variant doubles the bin count to 10
(~25/bin in train fold) to test whether finer-grained local shape correction
helps or whether the additional 5 free parameters overfit at small per-bin n.

Hypothesis: finer bins capture more local shape but more risk of overfit
at 25/bin.

If pooled cross-fit RAE < 0.578 (nb1053), 10 bins helps; if higher, 5 was
optimal.

Procedure identical to nb1053, only N_BINS changed.

Outputs:
  data/processed/te_nb1061.npy
  data/processed/nb1061_summary.json
  submissions/nb1061_finer_bins.csv
"""
...
NB1014_SEED42_RAE = 0.5994
NB1053_HONEST_RAE = 0.578  # 5-bin variant; gate for "finer helps".
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-quantile-bin stretch on te_{ANCHOR}  (N_BINS={N_BINS})")
    print("=" * 78)
    ...
    # =================================================================
    # Per-bin diagnostic (in-sample on 253)
    # =================================================================
    ...
    plain = SUBMISSIONS / f"{TAG}_finer_bins.csv"
    pd.DataFrame({
    ...
    delta_vs_nb1053 = pooled - NB1053_HONEST_RAE
    if delta_vs_nb1053 <= -0.001:
        verdict = "FINER_HELPS"
    elif abs(delta_vs_nb1053) < 0.001:
        verdict = "TIES_NB1053"
    else:
        verdict = "5_BINS_OPTIMAL"
    print(f"\n[verdict] pooled CV vs nb1053 (5 bins {NB1053_HONEST_RAE}): "
          f"delta={delta_vs_nb1053:+.4f}  -> {verdict}")
    ...
    print(f"   nb1053 (5 bins)            = {NB1053_HONEST_RAE:.4f}")
...
if __name__ == "__main__":
    ...
    for k in ("in_rae_anchor_on_253", "pooled_cross_fit_rae",
              "in_rae_deploy_on_253", "delta_vs_nb1053", "verdict",
