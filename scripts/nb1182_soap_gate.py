"""nb1182 — dscribe SOAP geometric descriptors on the DEPLOYED ensemble config.

The QUEUED other half of the strain row. Mirrors nb1181 (strain) exactly, but the
control is the CURRENT deployed best (0.4268) which already includes AIMNet2 QM +
MMFF strain; treatment adds the SOAP-PCA geometric block. SOAP encodes the local
3D atomic environment power spectrum — a distinct geometric observable from the
electronic (AIMNet2) and conformer-energy (strain) blocks.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain
  treatment = control + SOAP-PCA (24 comps)
Deploy only if matched delta < -0.001 AND treatment < deployed best (0.4268).
"""
import os, sys, json
...
SOAP = "C:/pxr_work/soap/soap_pca.csv"
...
def main():
    ...
    soap_cols = [c for c in pd.read_csv(SOAP, nrows=1).columns if c.startswith("soap_")]
    Xsp, sp_nan = aligned_block(SOAP, soap_cols, tr["name"])
    print(f"AIMNet2 imputed={qm_nan}  strain imputed={st_nan}  SOAP imputed={sp_nan}  soap_dim={len(soap_cols)}")
    ...
    for seed in range(N_SEEDS):
        ...
        scp = StandardScaler().fit(Xsp[trn]); Xsp_std = scp.transform(Xsp)
        Xctrl = np.hstack([Xqm_std, Xst_std])              # deployed = combined + AIMNet2 + strain
        Xtreat = np.hstack([Xqm_std, Xst_std, Xsp_std])    # + SOAP
    ...
    print(f"treatment(deployed + SOAP)                RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           "c_raes": c_raes, "t_raes": t_raes, "soap_imputed": sp_nan, "soap_dim": len(soap_cols),
           "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1182_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1182_summary.json -> DEPLOY={deploy}")
