"""nb1296 — Politzer/GIPF molecular-surface ESP statistics on the DEPLOYED
ensemble config (AIMNet2 + MMFF strain + DFT-D4 + DBSTEP, best 0.4242).

12 surface-ESP scalars (Vmax/Vmin/Vrange/Vavg, Vavg_pos/neg, sigma2_pos/neg/tot,
balance nu, local-polarity Pi, frac_pos) from Gasteiger charges projected onto
the vdW surface. SPATIAL surface-electrostatic field is a genuinely-untested
observable: AIMNet2 = per-atom charges (magnitude), this = the POTENTIAL field a
binding pocket reads + its statistical balance (electrostatic complementarity).

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + D4 + DBSTEP
  treatment = control + 12 surface-ESP scalars
...
DBCOLS = ["vbur_r25", "vbur_r35", "vbur_r45", "vbur_r55", "vbur_r65",
          ...
          "radgyr", "inertial_sf"]
ESP = "C:/pxr_work/surface_esp/surface_esp_features.csv"
ECOLS = ["esp_vmax", "esp_vmin", "esp_vrange", "esp_vavg",
...
def main():
    ...
    Xes, es_nan = aligned_block(ESP, ECOLS, tr["name"])
    print(f"AIMNet2 imp={qm_nan} strain imp={st_nan} d4 imp={d4_nan} dbstep imp={db_nan} esp imp={es_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        sce = StandardScaler().fit(Xes[trn]); Xes_std = sce.transform(Xes)
        Xctrl = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std])              # deployed 0.4242
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xes_std])   # + surface-ESP
        ...
        err = ytr[ho] - ctrl
        esmean = Xes_std[ho].mean(1)
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(esmean))[0, 1]))
    ...
    print(f"treatment(deployed + surface-ESP)            RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           ...
           "esp_imputed": es_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1296_surface_esp_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1296_surface_esp_summary.json -> DEPLOY={deploy}")
