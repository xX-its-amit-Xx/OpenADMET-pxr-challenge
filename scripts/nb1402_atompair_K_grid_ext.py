"""nb1402 -- Extended AtomPair K-grid (gap-fill between nb1384 grid).

Hypothesis:
    nb1373 used K=30 (mean-bag RAE 0.5095).  nb1384 swept
    {10, 20, 50, 75, 100, 150} and found best K=20 (0.5111).
    The grid had gaps between 20-50 and 50-150 -- the *true* optimum
    may sit between 25-45 (just above nb1373's K=30) or in the
    untested 60-80 band.  Fill the gap with K in
    {25, 35, 40, 45, 60, 80, 100}.  100 is included as overlap with
    nb1384 for sanity (should reproduce ~0.5330).

Protocol:
    1.  Reuse SHAP ranking on AtomPair-2048 -- compute via one global
        LGBM Huber (seed=0) on full 2050-col feature matrix on the 253
        unblind rows, exactly as nb1373/nb1384.  (The ranking is
        deterministic at seed=0 given identical data, so the top-K
        slices nest the previous grids.)
    2.  For each K in {25, 35, 40, 45, 60, 80, 100}:
    ...
    5.  Verdict at 0.003 margin vs nb1373 K=30 (0.5095) and
        nb1384 best K=20 (0.5111).

Outputs:
    scripts/nb1402_atompair_K_grid_ext.py        (this file)
    data/processed/nb1402_summary.json
    data/processed/nb1402_best_K_oof.npy         mean-bag OOF for best K (253,)
    data/processed/nb1402_per_K_mean_bag_oof.npy (7, 253) all K mean-bags
"""
...
NB1384_REF_BEST_RAE = 0.5111   # best in old grid
NB1384_REF_K100 = 0.5330       # overlap sanity check anchor
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- EXTENDED SHAP K-grid AtomPair (gap-fill); "
          f"anchor={ANCHOR}")
    ...
    # ---- ChEMBL pool ----
    print("\n" + "-" * 78)
    print("CHEMBL PXR POOL (local cache; same union as nb1373/nb1384)")
    print("-" * 78)
    ...
    # ---- Sanity check K=100 vs nb1384 ----
    k100_idx = K_GRID.index(100) if 100 in K_GRID else None
    k100_rae = (per_K_records[k100_idx]["rae_mean_bag"]
                if k100_idx is not None else None)
    k100_consistent = (k100_rae is not None and
                       abs(k100_rae - NB1384_REF_K100) < 0.01)

    # ---- Report table ----
    print("\n" + "=" * 78)
    print("K-GRID RAE TABLE (extended grid)")
    print("=" * 78)
    print(f"  {'K':>5}  {'feat_dim':>9}  {'mean_bag':>10}  {'median_bag':>11}  "
          f"{'per_seed_std':>13}  {'d_vs_nb1373':>12}  {'d_vs_nb1384':>12}")
    print("  " + "-" * 84)
    ...
    print(f"  nb1384 K=100  reference  = {NB1384_REF_K100:.4f}  "
          f"(overlap sanity)")
    ...
    if k100_rae is not None:
        print(f"  K=100 overlap check: this={k100_rae:.4f}  "
              f"nb1384={NB1384_REF_K100:.4f}  "
              f"|delta|={abs(k100_rae - NB1384_REF_K100):.4f}  "
              f"consistent={k100_consistent}")

    if beats_nb1373 and beats_nb1384:
        verdict = (f"EXT_K_GRID_BEST_K{best_K}_BEATS_BOTH_NB1373_AND_NB1384"
                   f"_NEW_PRIMARY_CANDIDATE")
    elif beats_nb1373:
        verdict = (f"EXT_K_GRID_BEST_K{best_K}_BEATS_NB1373_NEW_PRIMARY_CANDIDATE")
    elif ties_nb1373:
        verdict = f"EXT_K_GRID_BEST_K{best_K}_FLAT_VS_NB1373"
    elif beats_nb1384:
        verdict = (f"EXT_K_GRID_BEST_K{best_K}_BEATS_NB1384"
                   f"_BUT_WORSE_THAN_NB1373")
    elif ties_nb1384:
        verdict = f"EXT_K_GRID_BEST_K{best_K}_FLAT_VS_NB1384_WORSE_NB1373"
    else:
        verdict = f"EXT_K_GRID_BEST_K{best_K}_WORSE_THAN_BOTH"
    ...
    summary = {
        ...
        "k100_overlap_rae": k100_rae,
        "k100_overlap_nb1384_ref": NB1384_REF_K100,
        "k100_overlap_consistent": bool(k100_consistent),
        "verdict": verdict,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_best_vs_nb1373", "delta_best_vs_nb1384_bestK20",
        "beats_nb1373", "ties_nb1373",
        "beats_nb1384", "ties_nb1384",
        "k100_overlap_rae", "k100_overlap_consistent",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
