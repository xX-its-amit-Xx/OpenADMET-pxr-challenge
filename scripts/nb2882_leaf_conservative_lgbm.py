"""nb2882 -- LGBM with leaf-conservative depth + smoothing.

NEW PARADIGM:
    Three orthogonal LGBM regularization knobs combined for the first
    time on this anchor:

      1. max_depth=3 (vs typical 4-6) -- aggressively shallow trees so
         each split is forced to capture a high-level interaction; deep
         leaves cannot specialize on n=253 noise. num_leaves=7 enforces
         the depth-3 budget exactly (2^3 - 1 = 7).

      2. path_smooth=2.0 (vs the more common 0.1-0.5) -- HEAVY leaf
         smoothing toward parent-node prediction. Each leaf is pulled
         strongly toward its hierarchical ancestor, acting as a strong
         Bayesian prior with parent as the prior mean. Combined with
         shallow trees, this means every leaf prediction is essentially
         a smoothed average over depth-2 ancestors.

      3. n_estimators=600 (vs the typical 200-300) with lr=0.02 -- many
         weak shallow learners compensating for the heavy per-tree
         shrinkage. Total effective capacity is comparable to a deeper
         shorter ensemble but distributed across many smoothed weak
         learners (functional gradient descent with smaller steps).

    The combination is orthogonal to nb2842 (path_smooth=0.5,
    boost_from_average=False) by going much heavier on smoothing,
    much shallower on depth, and much longer on boosting.
...
PROTOCOL:
    ...
    - LGBM(max_depth=3, num_leaves=7, n_estimators=600, lr=0.02,
           reg_lambda=2, path_smooth=2.0).
...
OUTPUTS:
    scripts/nb2882_leaf_conservative_lgbm.py
    data/processed/nb2882_summary.json
    data/processed/nb2882_pred_oof.npy   (253,) float32 mean-bag CORRECTED
    data/processed/te_nb2882.npy         (513,) float32 deploy refit
    submissions/nb2882_leaf_conservative_lgbm.csv
"""
...
# LGBM hyperparams per protocol -- leaf-conservative
LGBM_PARAMS = dict(
    objective="regression",
    metric="rmse",
    max_depth=3,                 # NEW: shallower than typical
    num_leaves=7,                # 2^3 - 1, matches max_depth=3 budget
    n_estimators=600,            # NEW: many weak learners
    learning_rate=0.02,          # NEW: small step compensating for n_est
    lambda_l2=2.0,               # NEW: reg_lambda=2 strong L2
    path_smooth=2.0,             # NEW: heavy leaf smoothing
    min_data_in_leaf=10,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM (leaf-conservative: depth=3 + path_smooth=2 + "
          f"n_est=600) on {ANCHOR} residual")
    print(f"        max_depth=3  num_leaves=7  n_est=600  lr=0.02  "
          f"reg_lambda=2  path_smooth=2.0")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_leaf_conservative_lgbm.csv"
    pd.DataFrame({
        "SMILES": test_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "LGBM_leaf_conservative_depth3_pathsmooth2_chemprop_aux_residual",
        "rationale": (
            "Three orthogonal LGBM regularization knobs combined for the "
            "first time on this anchor: max_depth=3 (aggressively shallow "
            "trees, num_leaves=7 enforces exact budget), path_smooth=2.0 "
            "(heavy hierarchical Bayesian shrinkage of leaves toward parent "
            "node), and n_estimators=600 with lr=0.02 (many weak learners "
            "with small steps compensating for the heavy per-tree shrinkage)."
        ),
        "anchor": ANCHOR,
