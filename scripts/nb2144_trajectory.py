"""nb2144 -- TRAJECTORY over SHAP top-28 of the 117-col 5-way K-tuned matrix,
LGBM num_leaves locked to L=12 (nb2123 best-mean-bag K=28 setting).

PROTOCOL:
    chemprop_aux 0.6216 -> K=28 0.4698-0.4737. 27 wins, predicted LB
    0.473-0.477, 119 cycles, ~760 methods.
...
DESIGN:
    ...
    num_leaves grid (L in {12, 14, 16, 18}) reports:
        L=12 -> rae_mean_bag = 0.4726  (best across the grid)
        L=14 -> rae_mean_bag = 0.4757  (locked by nb2134)
        L=16 -> rae_mean_bag = 0.4742
        L=18 -> rae_mean_bag = 0.4742
    L=12 is therefore the locked leaf count for this trajectory -- the
    nb2123 "best_L_overall" selection, complementing nb2134's L=14 lock
    on median-bag.

    nb2144 runs a 119-step trajectory: each step is a fresh 5-seed
    LGBM(MSE) bag x 5-fold cross-fit on the SAME K=28 substrate, with
    num_leaves pinned to L=12 and the remaining hyperparameters
    ...
    n_estimators).  We track:
        ...
        * best cycle index, mean RAE, std RAE, fraction of cycles at/below
          ...
          nb2123 K=28 L=12 mean_bag 0.4726 (LOOSE).
    Floor target = 0.4698 (matches nb2103 K=28 median_bag, since at L=12
    the nb2123 grid median_bag landed at 0.4719); wins target = 27.
...
PRE-UNBLIND:
    ...
    L=12 was selected by nb2123 (also K=28 substrate, in-sample on 253
...
LINEAGE:
    ...
    -> nb2144 (119, SHAP top-28 L=12 LOCKED floor 0.4698).

Outputs:
    scripts/nb2144_trajectory.py
    data/processed/nb2144_summary.json
    data/processed/nb2144_trajectory.csv         # 119 rows
    data/processed/nb2144_best_oof.npy           # (253,) float32
    data/processed/nb2144_per_cycle_rae.npy      # (119,) float32
    data/processed/nb2144_pooled_mean_oof.npy    # (253,) float32
    data/processed/nb2144_pooled_median_oof.npy  # (253,) float32
"""
...
FLOOR_TARGET = 0.4698
# nb2123 K=28 L=12 mean_bag -- LOOSE upper-band reference
FLOOR_LOOSE = 0.4726
DECISION_MARGIN = 0.0003       # strict improvement threshold for a "win"

# Per-cycle bag config
CYCLE_FOLDS = 5
CYCLE_SEEDS_PER_BAG = 5        # 5-seed bag per cycle (mirrors nb2063/nb2103/nb2124/nb2134)
CYCLE_BASE_SEEDS = [0, 1, 7, 42, 137]

# LOCKED num_leaves (nb2123 K=28 best-mean-bag L)
L_LOCKED = 12

# Substrate paths (same 117-col 5-way K-tuned matrix as nb2063/nb2103/nb2124/nb2134)
...
NB2123_K28_L12_MEAN_BAG_REF = 0.4726
NB2123_K28_L12_MEDIAN_BAG_REF = 0.4719
K_SHAP = 28
...
def _cycle_params(cycle: int, seed: int) -> dict:
    ...
    num_leaves is LOCKED to L_LOCKED=12 (nb2123 K=28 best mean_bag).
    All other knobs perturbed in a small neighborhood of
    nb1852/nb1861/nb2103/nb2124/nb2134 LGBM(MSE). K=28 substrate ->
    tighter colsample floor (0.65) to avoid losing core signal.

    Cycle index is offset by +4000 relative to nb2134 (which used +3000)
    so the perturbation streams are disjoint -- nb2144 explores a
    different deterministic sequence of hyperparameter tuples than
    nb2134 at the same locked-L assumption, giving a clean ablation
    of leaf count holding everything else equal.
    """
    rng = np.random.RandomState(4000 + cycle)
...
def main() -> dict:
    ...
    # ---- Load K-tuned indices (same 117-col stack as nb2103/nb2124/nb2134) ----
    ...
    nb2123_k28_l12_mean_bag_ref = NB2123_K28_L12_MEAN_BAG_REF
    nb2123_k28_l12_median_bag_ref = NB2123_K28_L12_MEDIAN_BAG_REF
    if NB2123_SUMMARY.exists():
        ...
        for r in sum_2123.get("per_L_records", []):
            if int(r.get("num_leaves", -1)) == L_LOCKED:
                nb2123_k28_l12_mean_bag_ref = float(r["rae_mean_bag"])
                nb2123_k28_l12_median_bag_ref = float(r["rae_median_bag"])
                break
    print(f"[ref] nb2123 K=28 L=12 mean_bag   = "
          f"{nb2123_k28_l12_mean_bag_ref:.4f}  (LOOSE)")
    print(f"[ref] nb2123 K=28 L=12 median_bag = "
          f"{nb2123_k28_l12_median_bag_ref:.4f}")
    ...
    # ---- Cross-cycle pooled OOF (mean / median across 119 cycles) ----
    ...
    print(f"   pooled MEAN-119   RAE   = {rae_pooled_mean:.4f}")
    print(f"   pooled MEDIAN-119 RAE   = {rae_pooled_median:.4f}")
    ...
    nb2123_l12_oof_path = DATA_PROCESSED / "nb2123_mean_bag_oof_L12.npy"
    if nb2123_l12_oof_path.exists():
        nb2123_l12_oof = np.load(nb2123_l12_oof_path).astype(np.float64)
        pearson_vs_nb2123_l12 = float(
            np.corrcoef(best_oof, nb2123_l12_oof)[0, 1]
        )
    else:
        pearson_vs_nb2123_l12 = None
    ...
    summary = {
        ...
        "rae_pooled_mean_119": rae_pooled_mean,
        "rae_pooled_median_119": rae_pooled_median,
        ...
        "pearson_best_vs_nb2123_L12": pearson_vs_nb2123_l12,
        "verdict": verdict,
        ...
        "nb2123_K28_L12_mean_bag_ref": nb2123_k28_l12_mean_bag_ref,
        "nb2123_K28_L12_median_bag_ref": nb2123_k28_l12_median_bag_ref,
        "predicted_lb_band_low": nb2123_k28_l12_median_bag_ref,
        "predicted_lb_band_high": nb2123_k28_l12_mean_bag_ref,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_pooled_mean_119", "rae_pooled_median_119",
