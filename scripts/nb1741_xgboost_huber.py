"""nb1741 -- Final residual learner: XGBoost(pseudo-Huber) on 5-way K-tuned 117-col PRE-unblind.

HYPOTHESIS:
    Triangulate a third GBM family (XGBoost with pseudo-Huber loss) on the
    identical 117-col 5-way K-tuned feature substrate used by nb1554
    (CatBoost-MAE, mean_bag 0.5163).  nb1531 already triangulated XGBoost
    pseudo-Huber on the 4-way 112-col substrate; nb1741 extends it to the
    5-way 117-col K-tuned substrate (adds Avalon top-30 via nb1392).

    The 117-col feature stack matches nb1554 exactly:
        top-25 AtomPair   (nb1524 best_K=25, sliced from nb1484 SHAP)
        ...
        pred_chembl_pec50 (ChEMBL PXR kNN-5 Tanimoto over Morgan union pool)
        mean_sim
...
PROTOCOL:
    ...
    2.  Build 117-col feature stack from cached top-K rankings (mirrors nb1554).
    3.  5-seed bag XGBRegressor(objective='reg:pseudohubererror',
        max_depth=4, n_estimators=200, learning_rate=0.05, reg_lambda=5,
        huber_slope=1.0).  KFold(n=5, shuffle=True) cross-fit per seed.
    4.  Pool mean_bag and median_bag corrected anchors.
    5.  Verdict at 0.003 margin vs nb1554 (CatBoost-MAE 5-way mean_bag 0.5163).
        Fallback refs: nb1543 (4-way K-tuned CatBoost 0.5237), nb1501
        (4-way CatBoost 0.5223), nb1484 (4-way LGBM-Huber 0.5231),
        chemprop_aux standalone (0.6216).

Outputs:
    scripts/nb1741_xgboost_huber.py
    data/processed/nb1741_summary.json
    data/processed/nb1741_mean_bag_oof.npy            (253,) float32
"""
...
NB1501_REF = 0.5223                   # CatBoost-MAE 4-way mean-bag
NB1543_REF = 0.5237                   # CatBoost-MAE 4-way K-tuned mean-bag
NB1554_REF = 0.5163                   # CatBoost-MAE 5-way K-tuned mean-bag (PRIMARY ref)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- XGBoost(pseudo-Huber, d4, n200, lr0.05, lam=5, slope=1.0) "
    ...
    if not XGB_OK:
        raise RuntimeError(f"xgboost import failed: {_XGB_IMPORT_ERR}")
    ...
    # Avalon = nb1392 top-30 SHAP ranking (K=30 default; matches nb1554)
    ...
    # ---- Per-seed XGBoost residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED XGBOOST RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    pearson_vs_nb1531 = _pearson_vs(DATA_PROCESSED / "nb1531_mean_bag_oof.npy")
    ...
    if pearson_vs_nb1531 is not None:
        print(f"   Pearson(mean_bag, nb1531_mean)   = {pearson_vs_nb1531:.4f}")
    if pearson_vs_nb1484 is not None:
        print(f"   Pearson(mean_bag, nb1484_best)   = {pearson_vs_nb1484:.4f}")

    # ---- Verdict (PRIMARY anchor = nb1554 at 0.5163) ----
    ...
    if beats_nb1554:
        verdict = "XGB_HUBER_5WAY_K_TUNED_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "XGB_HUBER_5WAY_K_TUNED_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "XGB_HUBER_5WAY_K_TUNED_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif flat_vs_nb1501:
        verdict = "XGB_HUBER_5WAY_K_TUNED_FLAT_VS_NB1501"
    elif beats_nb1543:
        verdict = "XGB_HUBER_5WAY_K_TUNED_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif flat_vs_nb1543:
        verdict = "XGB_HUBER_5WAY_K_TUNED_FLAT_VS_NB1543"
    elif beats_anchor:
        verdict = "XGB_HUBER_5WAY_K_TUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "XGB_HUBER_5WAY_K_TUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "XGB_HUBER_5WAY_K_TUNED_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "xgb_huber_slope": XGB_HUBER_SLOPE,
        "K_AP_best": K_AP_best,
        ...
        "pearson_vs_nb1531_mean_bag": pearson_vs_nb1531,
