"""nb2060 -- nb1162 variant WITHOUT nb730_honest (POST-unblind anchor removed).

Per cycle-157 nb2053 ablation, nb730_honest carries ~88.7% deploy weight in
nb1162. Without it, the dropped-nb730 ablation gave RAE=0.4707 using
nb2103 + nb562. nb2060 expands the anchor set to a 6-element PRE/POST mix
that EXCLUDES nb730_honest entirely, in order to isolate signal that is
not contaminated by the POST-unblind anchor.

Anchors (6):
    0. chemprop_aux  data/processed/nb1133_chemprop_aux_pred_oof.npy   (PRE)
    1. nb1150        reconstructed from cached SLSQP4 full-pool weights
    2. nb1158        nb1158_mean_bag_oof_K32.npy
    3. nb2112_K28    nb2103_mean_bag_oof_K28.npy
    4. nb503         nb503_pred_oof.npy
    5. nb562         nb562_pred_oof.npy

Deploy te files (matched in order; same paths as nb1191/nb1162):
    chemprop_aux -> te_chemprop_aux.npy
    nb1150       -> te_nb1150.npy
    nb1158       -> te_nb1158.npy
    nb2112_K28   -> te_nb2112.npy
    nb503        -> te_nb503.npy
    nb562        -> te_nb562.npy

Stage 2 SLSQP: convex blend per scaffold-fold.
Stage 3 rank-stretch grid (per-fold): {1.000, 1.025, 1.050, 1.075, 1.100,
    1.125, 1.150}.
Scaffold 5-fold CV with kf_seeds {1001..1005}; pool RAE across seeds.

Comparisons:
    nb1191 honest cross-fit pooled = 0.4703-0.4718 (memo range)
    nb2053 dropped-nb730 ablation  = 0.4707  (4-anchor LOO on nb1162 base)

LB-band per memo: 0.51 * OOF + 0.49 * te[unb_idx].

GATE: pooled OOF (mean of seeds) <= 0.4700.
    If passes -> write submissions/nb2060_deploy_no_nb730.csv as
    PRIMARY-1-LB-CLEAN.

Always saves data/processed/nb2060_summary.json and
data/processed/te_nb2060.npy.
"""
...
# NOTE: nb730_honest intentionally EXCLUDED.
ANCHORS = [
...
# nb1150 reconstructed OOF — same approach as nb1191
NB1150_SLSQP4_OOFS = [
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- nb1162 variant WITHOUT nb730_honest")
    print("=" * 78)
    ...
    # ---- Anchors ----
    print("\n[anchors]  (nb730_honest INTENTIONALLY EXCLUDED)")
    ...
    nb1191_low, nb1191_high = 0.4703, 0.4718
    nb2053_drop_nb730 = 0.4707
    print(f"\n[compare] nb1191 cross-fit pooled RAE   = "
          f"[{nb1191_low:.4f}, {nb1191_high:.4f}]")
    print(f"[compare] nb2053 dropped-nb730 ablation = {nb2053_drop_nb730:.4f}")
    delta_vs_nb1191_mid = pooled_rae_mean_seeds - 0.5 * (
        nb1191_low + nb1191_high
    )
    delta_vs_drop_abl = pooled_rae_mean_seeds - nb2053_drop_nb730
    print(f"[compare] nb2060 - nb1191_mid           = "
          f"{delta_vs_nb1191_mid:+.4f}")
    print(f"[compare] nb2060 - dropped_ablation     = "
          f"{delta_vs_drop_abl:+.4f}")

    # =================================================================
    # Gate evaluation
    # =================================================================
    gate_pass = pooled_rae_mean_seeds <= GATE_OOF
    print("\n" + "-" * 78)
    print("GATE EVALUATION")
    print("-" * 78)
    print(f"   gate: OOF (mean of seeds) {pooled_rae_mean_seeds:.4f} "
          f"<= {GATE_OOF:.4f}  -> {'PASS' if gate_pass else 'FAIL'}")
    ...
    sub_csv_path = SUBMISSIONS / f"{TAG}_deploy_no_nb730.csv"
    if gate_pass:
        pd.DataFrame({
            "SMILES": te_smiles,
        ...
        print(f"[save] {sub_csv_path}  (gate PASSED -- PRIMARY-1-LB-CLEAN)")
    else:
    ...
    summary = {
        "tag": TAG,
        "method": "no_nb730_pyramid_SLSQP_then_rank_stretch_seedavg",
        ...
        "nb1191_cross_fit_low_memo": nb1191_low,
        "nb1191_cross_fit_high_memo": nb1191_high,
        "nb2053_dropped_nb730_ablation_rae": nb2053_drop_nb730,
        "delta_vs_nb1191_mid": delta_vs_nb1191_mid,
        "delta_vs_dropped_ablation": delta_vs_drop_abl,
    ...
    print(f"   gate (OOF <= {GATE_OOF})                  = {gate_pass}")
