"""nb1200 — TorchANI / ANI-2x AEV (fixed Behler-Parrinello symmetry-function) +
NNP energy on the DEPLOYED ensemble config (AIMNet2 + MMFF strain + DFT-D4,
best 0.4252).

26 ANI scalars: NNP energy (ani_energy), energy-per-atom (ani_epa), and 24
PCA comps of the mean-pooled 1008-d ANI-2x AEV. ANI's AEV is a FIXED radial+
angular symmetry-function basis (NOT a learned message-passing embedding) ->
argued lowest sink-absorption risk of the MLIP family; emits a 3D
local-geometry-symmetry-function axis. Honest deployed-config gate: does it add
over the saturated AIMNet2(electronic)+strain(conformer)+D4(dispersion) physics
stack?

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + D4
  treatment = control + 26 ANI scalars
...
DBCOLS = ["vbur_r25", "vbur_r35", "vbur_r45", "vbur_r55", "vbur_r65",
          ...
          "radgyr", "inertial_sf"]
ANI = "C:/pxr_work/ani/ani_features.csv"
NCOLS = ["ani_energy", "ani_epa"] + [f"ani_{i}" for i in range(24)]
...
def main():
    ...
    Xan, an_nan = aligned_block(ANI, NCOLS, tr["name"])
    print(f"AIMNet2 imp={qm_nan} strain imp={st_nan} d4 imp={d4_nan} dbstep imp={db_nan} ani imp={an_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        sca = StandardScaler().fit(Xan[trn]); Xan_std = sca.transform(Xan)
        Xctrl = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std])             # deployed = AIMNet2+strain+D4+DBSTEP
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xan_std])   # + ANI
        ...
        err = ytr[ho] - ctrl
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(Xan_std[ho].mean(1)))[0, 1]))
    ...
    print(f"treatment(deployed+ANI-2x AEV+energy)        RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           ...
           "ani_imputed": an_nan, "dbstep_imputed": db_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1200_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1200_summary.json -> DEPLOY={deploy}")
