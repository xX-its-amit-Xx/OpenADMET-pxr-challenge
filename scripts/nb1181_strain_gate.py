"""nb1181 — MMFF conformational STRAIN scalars on the DEPLOYED ensemble config.

Mirrors nb1177 (AIMNet2) exactly, swapping the 9 electronic QM scalars for the 8
conformational-strain scalars (strain_features.py). Strain is a DISTINCT physical
observable from AIMNet2's single-geometry electronic block (memory: StrainRelief
not covered by deployed AIMNet2). Gate on the EXACT deployed config, matched on
the same MTL ho_idx holdouts as nb1168/nb1177.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof              [+ AIMNet2 QM]
  treatment = control + 8 strain scalars
Deploy only if matched delta < -0.001 AND treatment < deployed best.

NOTE: the deployed best (0.4297) already includes the AIMNet2 QM block, so the
control here ALSO includes AIMNet2 (load from aimnet_features.csv) to test strain
as MARGINAL-over-the-current-deployed (not over a weaker anchor).
"""
import os, sys, json
...
def main():
    ...
    Xqm, qm_nan = aligned_block(AIM, ACOLS, tr["name"])      # deployed AIMNet2 block (in control)
    Xst, st_nan = aligned_block(STR, SCOLS, tr["name"])      # new strain block (treatment only)
    print(f"AIMNet2 imputed={qm_nan}  strain imputed={st_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        Xctrl = Xqm_std                          # control = combined + AIMNet2
        Xtreat = np.hstack([Xqm_std, Xst_std])   # treatment = control + strain
    ...
    print(f"treatment(deployed+AIMNet2+strain)  RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           "c_raes": c_raes, "t_raes": t_raes, "strain_imputed": st_nan,
           "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1181_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1181_summary.json -> DEPLOY={deploy}")
