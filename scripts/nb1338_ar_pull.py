"""nb1338 — pull on-target AR (NR3C4) agonist potency for cross-NR read-across.

Primary: ChEMBL CHEMBL1871 (human Androgen Receptor) EC50/AC50/Potency.
Augment: Tox21 AR-bla agonist actives (PubChem AID 743040).
Normalize all to p-scale (pX = 9 - log10(value_nM)); dedup by InChIKey.
Output rows compatible with nb1336-style gate: (id, std_smiles, inchikey, outcome, p_potency)
"""
...
OUT = "C:/pxr_work/ar_tox21"; os.makedirs(OUT, exist_ok=True)
...
def pull_chembl(stype):
    ...
    recs, off = [], 0
    while True:
        url = (f"{CHEMBL}/activity.json?target_chembl_id=CHEMBL1871&standard_type={stype}"
...
print("pulling ChEMBL CHEMBL1871 (AR) ...", flush=True)
...
try:
    ...
    print(f"Tox21 AR actives w/ SMILES: {len(tox_smis)}", flush=True)
...
pickle.dump(rows, open(f"{OUT}/ar_std.pkl", "wb"))
print("saved", f"{OUT}/ar_std.pkl", flush=True)
