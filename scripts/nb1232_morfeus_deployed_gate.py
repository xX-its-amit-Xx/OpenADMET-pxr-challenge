"""nb1232 -- morfeus SASA + surface-Dispersion descriptors on the DEPLOYED
ensemble config (AIMNet2 + MMFF strain + DFT-D4 + DBSTEP, best 0.4242).

7 morfeus scalars: SASA (area/volume = solvent-exposed surface geometry) +
Dispersion surface model (area/volume/p_int/p_min/p_max = surface dispersion
potential distribution). Distinct surface observables: DBSTEP encodes occluded
buried-volume + shape; D4 encodes dispersion ENERGY + atomic C6/alpha; morfeus
gives the SOLVENT-EXPOSED surface + the surface DISTRIBUTION of dispersion --
neither captured by the deployed blocks. Low-but-nonzero orthogonality prior.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + D4 + DBSTEP
  treatment = control + 7 morfeus SASA/Dispersion scalars
...
def main():
    ...
    print(f"imp AIMNet2={qm_nan} strain={st_nan} d4={d4_nan} dbstep={db_nan} morfeus={mf_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xmf_std])    # + morfeus
        ...
        err = ytr[ho] - ctrl
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(Xmf_std[ho].mean(1)))[0, 1]))
    ...
    print(f"treatment(deployed + morfeus SASA/Dispersion) RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    json.dump(out, open(f"{P}/nb1232_morfeus_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1232_morfeus_summary.json -> DEPLOY={deploy}")
