"""nb3144 -- Isotonic calibration on nb3080 PRIMARY-1 deploy.

NEW PARADIGM: apply iso on the new PRIMARY-1 nb3080 (vs cycle 254 nb3021
which did iso on nb3002).

PROTOCOL:
    Anchor: nb3080 = wide-seed quantile-conditional hard-split blend
                     {K18, K19} deep-30 (q_cut=0.4)
        - nb3080_pred_oof.npy : (253,) single-seed OOF blend
        - te_nb3080.npy       : (513,) deploy te
    Outer CV: 5-fold scaffold split, 15 fresh kf_seeds {1141..1155}
    Per fold:
...
References:
    nb3080 wide-seed 15 deep-30 OOF mean   = 0.4475 (current PRIMARY-1)
    nb3002 K18+K19 deep-30 SLSQP outer-val = 0.4479 (15 seeds)
    nb3021 iso-on-nb3002 outer-val         = depends on per-seed
...
Outputs:
    data/processed/nb3144_summary.json
    data/processed/nb3144_pred_oof.npy   (253,) float32 -- per-fold iso OOF (first seed)
    data/processed/te_nb3144.npy         (513,) float32 -- deploy te (full-fit iso)
    submissions/nb3144_iso_on_nb3080.csv  (only if verdict != "FAIL")
"""
...
# -- Inputs --------------------------------------------------------------------
OOF_PATH = DATA_PROCESSED / "nb3080_pred_oof.npy"
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- isotonic calibration on nb3080 PRIMARY-1 deploy prediction")
    ...
    print(f"          gate: <{GATE_BETTER}  BETTER")
    print("=" * 78)
    ...
    # -- Load nb3080 anchor --------------------------------------------------
    print("\n" + "-" * 78)
    print("STEP 1: load nb3080 anchor (pred_oof on 253, te on 513)")
    print("-" * 78)
    nb3080_oof = np.load(OOF_PATH).astype(np.float64)
    nb3080_te = np.load(TE_PATH).astype(np.float64)
    if nb3080_oof.shape != (n_unb,):
        raise ValueError(f"nb3080 OOF shape {nb3080_oof.shape} != ({n_unb},)")
    if nb3080_te.shape != (n_test,):
        raise ValueError(f"nb3080 te shape {nb3080_te.shape} != ({n_test},)")
    nb3080_full_rae = float(rae(y_unb, nb3080_oof))
    nb3080_te_unb_in_rae = float(rae(y_unb, nb3080_te[unb_idx]))
    mu_oof = float(nb3080_oof.mean())
    mu_y = float(y_unb.mean())
    print(f"   nb3080 OOF      mean={mu_oof:.4f}  std={nb3080_oof.std():.4f}  RAE={nb3080_full_rae:.4f}")
    print(f"   nb3080 te(unb)  in-sample RAE = {nb3080_te_unb_in_rae:.4f}")
    ...
    for seed in KF_SEEDS:
        p_rae, fold_recs, oof_sh = _run_one_seed(
            seed, nb3080_oof, y_unb, unb_scaffolds,
        )
    ...
    print("-" * 78)
    iso_full = _fit_iso(nb3080_oof, y_unb)
    full_train_pred = iso_full.transform(nb3080_oof)
    r_full = float(rae(y_unb, full_train_pred))
    print(f"   full-OOF in-sample iso RAE = {r_full:.4f}")

    te_pred = iso_full.transform(nb3080_te).astype(np.float32)
    ...
    if mean_pooled < GATE_BETTER:
        ...
        verdict = "FAIL"
    delta_vs_nb3080_nom = mean_pooled - REF_NB3080_NOM
    delta_vs_nb3080_oof = mean_pooled - nb3080_full_rae
    delta_vs_nb3002 = mean_pooled - REF_NB3002_NOM
    delta_vs_nb2171 = mean_pooled - REF_NB2171
    print(f"   mean_pooled_rae              = {mean_pooled:.4f} (std {std_pooled:.4f})")
    print(f"   delta vs nb3080 ref 0.4475   = {delta_vs_nb3080_nom:+.4f}")
    print(f"   delta vs nb3080 full OOF     = {delta_vs_nb3080_oof:+.4f}")
    print(f"   delta vs nb3002 (0.4479)     = {delta_vs_nb3002:+.4f}")
    ...
    print(f"   [save] {te_out_path}   (deploy = iso_full(nb3080_te))")

    sub_csv = SUBMISSIONS / f"{TAG}_iso_on_nb3080.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_isotonic_calibration_on_nb3080",
        "paradigm": "isotonic_post_hoc_calibration",
        "anchor_pre_unblind": True,
        "anchor_pool": ["nb3080"],
        "anchor_full_oof_rae": round(nb3080_full_rae, 5),
        "anchor_te_unb_in_sample_rae": round(nb3080_te_unb_in_rae, 5),
        ...
        "delta_vs_nb3080_nom": delta_vs_nb3080_nom,
        "delta_vs_nb3080_full_oof": delta_vs_nb3080_oof,
    ...
    print("\n" + "=" * 78)
    print(f"=== {TAG} SUMMARY ===")
    print(f"   nb3080 anchor full OOF RAE = {nb3080_full_rae:.4f}")
