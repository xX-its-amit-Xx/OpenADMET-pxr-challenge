"""nb1298 — MACE-StrainRelief: DFT-accurate multi-conformer conformational strain
on the DEPLOYED ensemble config (AIMNet2 + MMFF strain + DFT-D4 + DBSTEP, best 0.4242).

MACE-OFF23 medium single-point energies across 4 conformers (1 seed + 3 extra ETKDG+MMFF),
giving 3 new physics scalars:
  mace_strain       E(seed) - min(E_conformers)  [eV; 0 if seed is global min]
  mace_conf_espread std(E_conformers / n_heavy)  [eV/atom; conformational flexibility]
  mace_conf_erange  (max-min E_conformers) / n_heavy  [eV/atom; energy spread]

Distinct from existing mace_fmax/frms (single-conformer force norms, already absorbed by AIMNet2):
this is a CONFORMATIONAL SAMPLING observable under the DFT-level MACE PES, analogous to the
MMFF strain that already won (-0.0023). DFT-accurate should be more precise.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + MMFF_strain + D4 + DBSTEP
  treatment = control + 3 MACE strain scalars
...
def main():
    import pandas as pd  # explicit local import to avoid shadowing from nb1126 import chain
    # Check MACE strain cache is ready
    if not os.path.exists(MS):
        print(f"ERROR: MACE strain cache not found: {MS}")
        print("Run scripts/mace_strain_local.py first (takes ~1.9h on CPU)")
        sys.exit(1)
    ...
    if n_done < 4000:
        print(f"WARNING: Only {n_done}/4652 done. Gate may have high imputation rate.")
    ...
    print(f"AIMNet2 imp={qm_nan} mmff_strain imp={st_nan} d4 imp={d4_nan} dbstep imp={db_nan} mace_strain imp={ms_nan}")

    # Check correlation of MACE strain with existing features and target
    tr2 = load_train().dropna(subset=["pec50"]).reset_index(drop=True)
    y = tr2["pec50"].values
    for j, col in enumerate(MSCOLS):
        x = Xms[:, j]
        corr_y = float(np.corrcoef(x, y)[0, 1]) if np.std(x) > 0 else 0
        corr_strain = float(np.corrcoef(x, Xst[:, 0])[0, 1]) if np.std(x) > 0 else 0
        corr_maceoff = float(np.corrcoef(x, Xqm[:, 0])[0, 1]) if np.std(x) > 0 else 0
        print(f"  {col}: corr_y={corr_y:.3f}  corr_MMFF_strain={corr_strain:.3f}  corr_AIMNet2E={corr_maceoff:.3f}")
    ...
    for seed in range(N_SEEDS):
        ...
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xms_std])
        ...
        err = ytr[ho] - ctrl
        msmean = Xms_std[ho].mean(1)
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(msmean))[0, 1]))
    ...
    print(f"\ncontrol  (deployed AIMNet2+MMFF_strain+D4+DBSTEP) RAE = {cm:.4f} +/- {np.std(c_raes):.4f}")
    print(f"treatment(deployed + 3 MACE strain scalars)      RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           ...
           "resid_corr_mean": float(np.mean(resid_corrs)),
           "ms_imputed": ms_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1298_mace_strain_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1298_mace_strain_summary.json -> DEPLOY={deploy}")
    if deploy:
        print("GATE PASSED: update best_ensemble.json and build 513 submission")
    else:
