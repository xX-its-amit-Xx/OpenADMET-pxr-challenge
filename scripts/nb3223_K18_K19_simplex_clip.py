"""nb3223 -- Per-fold SLSQP simplex on {K18, K19} deep-30 THEN per-fold
            learned clip on the simplex output.

NEW PARADIGM:
    Apply the per-fold learned-clip primitive to a SIMPLER base (raw SLSQP
    simplex on {K18, K19} deep-30) rather than the nb3070 quantile-conditional
    hard-split blend. The hypothesis is that the clip operator extracts
    decompression gain on top of any reasonable variance-compressed base, and
    a clean SLSQP simplex over 2 PRE-unblind anchors gives the cleanest
    possible substrate -- no per-row gating, no hard-split heuristic, just
    convex blend + tail clip.

    Composition:
        STEP 1 (per fold): SLSQP simplex w on fold-TRAIN over (K18, K19)
    ...
    Deploy:
        - Refit SLSQP simplex on FULL 253 -> single global (w_K18, w_K19).
        - Re-pick (q_low, q_high) on FULL 253 simplex output by inner grid.
        - Apply both to (513, 2) anchor te arrays -> te_nb3223.

GATE (on per-fold-mean across 15 seeds):
    mean < 0.4424 -> "BETTER"   (matches user gate)
    else          -> "FAIL"
...
References (all PRE-unblind anchor chain):
    ...
    nb3070 quantile-conditional 15-seed pooled    = 0.4509 (parent of nb3063)
...
Outputs:
    data/processed/nb3223_summary.json
    data/processed/nb3223_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3223.npy         (513,) float32 -- deploy te
    submissions/nb3223_K18_K19_simplex_clip.csv  (only on BETTER verdict)
"""
...
TAG = "nb3223"
PARENT_TAG = "nb3002_simplex+clip"
...
# -- Per-fold clip grid (matches nb3201 / nb3173 family) -----------------------
Q_LOW_GRID = [0.01, 0.05, 0.10]
Q_HIGH_GRID = [0.90, 0.95, 0.98]

# -- Gates (on PER-FOLD-MEAN over 15 seeds) ------------------------------------
GATE_BETTER = 0.4424  # mean < this -> BETTER (user-supplied gate)
...
def main() -> dict:
    ...
    print(f"          paradigm: SIMPLER base than nb3070 quantile-conditional")
    ...
    print(
        f"   ref K19 deep-30 OOF         = {REF_K19:.4f}"
    )
    print(
        f"   ref nb3002 SLSQP simplex    = {REF_NB3002:.4f}  "
        f"<- raw simplex base (no clip)"
    )
    print(
        f"   delta vs nb3002 (raw)       = "
        f"{pf_mean - REF_NB3002:+.4f}"
    )
    print(
        f"   ref nb3070 quantile-cond    = {REF_NB3070:.4f}  "
        f"<- comparable parent (other base)"
    )
    print(
        f"   delta vs nb3070             = "
        f"{pf_mean - REF_NB3070:+.4f}"
    )
    print(
    ...
    if pf_mean < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3223 15-seed per-fold-mean {pf_mean:.4f} "
            f"clears BETTER gate {GATE_BETTER:.4f} "
            f"({pf_mean - GATE_BETTER:+.4f}). Applying learned clip to the "
            f"SIMPLER SLSQP simplex base on {K_LABELS} deep-30 beats prior "
            f"post-hoc-ceiling cluster. Mean-of-folds weights = "
            f"{{{K_LABELS[0]}={w_mean_of_folds[0]:.2f}, "
            f"{K_LABELS[1]}={w_mean_of_folds[1]:.2f}}}, "
            f"modal clip = (q{ql_mode:.2f}, q{qh_mode:.2f}). "
            f"Confirms the clip primitive composes well with a clean convex "
            f"blend base. Re-verify with deep-30 before any PRIMARY-1 swap."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3223 15-seed per-fold-mean {pf_mean:.4f} fails "
            f"BETTER gate {GATE_BETTER:.4f} ({pf_mean - GATE_BETTER:+.4f}). "
            f"Applying learned clip to the simpler SLSQP simplex base does "
            f"NOT beat clip on the nb3070/nb3080 wide-bag children. The "
            f"raw simplex base preserves rank structure that the clip "
            f"operator can extract less from than the smoother bag. Keep "
            f"current clip-winner ladder (nb3173 best single 0.4422, "
            f"nb3214/nb3210 ensemble candidates). Mean-of-folds weights = "
            f"{{{K_LABELS[0]}={w_mean_of_folds[0]:.2f}, "
            f"{K_LABELS[1]}={w_mean_of_folds[1]:.2f}}}, "
            f"modal clip = (q{ql_mode:.2f}, q{qh_mode:.2f})."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_K18_K19_simplex_clip.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_deep30_then_per_fold_learned_clip",
        "paradigm": "clip_on_simpler_simplex_base_vs_nb3070_quantile",
    ...
    print(f"   delta vs nb3002 raw   = {pf_mean - REF_NB3002:+.4f}")
    print(f"   delta vs nb3070 cond  = {pf_mean - REF_NB3070:+.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_nb3002_raw_simplex", "delta_vs_nb3070_quantile_cond",
