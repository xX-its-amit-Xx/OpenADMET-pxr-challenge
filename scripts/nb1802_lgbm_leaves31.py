"""nb1802 -- LightGBM (max_depth=5, num_leaves=31, n_est=200, lr=0.05) on
5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1771 used max_depth=4, num_leaves=15, n_est=300, lr=0.03 and hit
    rae_mean_bag 0.5100.  This script pushes leaf capacity slightly upward
    (num_leaves 15->31, max_depth 4->5) with a larger learning rate (0.05)
    and shorter trees (n_est 200), holding min_child_samples=5 and
    reg_lambda=2 fixed.  Tests whether the small-leaf regime in nb1771 was
    overconstrained and a moderate-depth/num_leaves config extracts more
    fine-grain residual signal on the same 117-col K-tuned matrix:
        objective='huber', alpha=1.0
        max_depth=5, num_leaves=31
        n_estimators=200, lr=0.05
        min_child_samples=5
        reg_lambda=2.0
    Same 5-way K-tuned 117-col feature stack as nb1771/nb1554:
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(huber, max_depth=5, num_leaves=31, n_est=200,
       lr=0.05, min_child_samples=5, reg_lambda=2).  KFold(n=5, shuffle=True)
       cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1771 (0.5100) and nb1780 BoB (0.5032).

Outputs:
    scripts/nb1802_lgbm_leaves31.py
    data/processed/nb1802_summary.json
    data/processed/nb1802_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM small leaves (d4/lv15/n300/lr0.03/min5/l2=2)
NB1780_BOB_REF = 0.5032  # nb1780 best-of-bag aggregation over nb1771 seeds
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1492/nb1543/nb1554/nb1771."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=5, num_leaves=31, "
          f"n_est=200, lr=0.05, min_child=5, reg_lambda=2) on 5-way K-tuned "
    ...
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1771 ({NB1771_REF:.4f}), nb1780_BoB ({NB1780_BOB_REF:.4f})  "
    ...
    # ---- Per-seed LGBM (depth=5/leaves=31/lr=0.05) residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (d=5, leaves=31, n=200, lr=0.05, min_child=5) "
    ...
    pearson_vs_nb1780 = _pearson_vs(DATA_PROCESSED / "nb1780_bob_mean_oof.npy")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1780_BoB = {rae_mean_bag - NB1780_BOB_REF:+.4f})")
    ...
    if pearson_vs_nb1780 is not None:
        print(f"   Pearson(mean_bag, nb1780_BoB)    = {pearson_vs_nb1780:.4f}")
    ...
    # ---- Verdict (vs nb1771 and nb1780 BoB) ----
    ...
    beats_nb1780_bob = rae_mean_bag < NB1780_BOB_REF - DECISION_MARGIN
    flat_vs_nb1771 = abs(rae_mean_bag - NB1771_REF) < DECISION_MARGIN
    flat_vs_nb1780_bob = abs(rae_mean_bag - NB1780_BOB_REF) < DECISION_MARGIN
    flat_vs_nb1554 = abs(rae_mean_bag - NB1554_REF) < DECISION_MARGIN

    if beats_nb1780_bob:
        verdict = "LGBM_LEAVES31_BEATS_NB1780_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1780_bob:
        verdict = "LGBM_LEAVES31_FLAT_VS_NB1780_BOB"
    elif beats_nb1771:
        verdict = "LGBM_LEAVES31_BEATS_NB1771_BUT_WORSE_THAN_NB1780_BOB"
    elif flat_vs_nb1771:
        verdict = "LGBM_LEAVES31_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_LEAVES31_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_LEAVES31_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_LEAVES31_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_LEAVES31_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_LEAVES31_HURTS_ANCHOR"
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1780_bob",
        "beats_chemprop_aux",
        "beats_nb1554", "flat_vs_nb1554",
        "beats_nb1771", "flat_vs_nb1771",
        "beats_nb1780_bob", "flat_vs_nb1780_bob",
        "pearson_vs_anchor",
