"""nb1752 -- CatBoost(MAE) Ordered boosting on K-tuned 5-way 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE d4 n200 lr0.05 l2=5, boosting_type default='Plain')
    yielded mean_bag RAE = 0.5163 on the 117-col K-tuned matrix.  CatBoost's
    flagship feature is *ordered boosting* (Prokhorenkova et al., NeurIPS
    2018) -- a permutation-driven scheme that fits each tree on the residual
    of a fold-out model rather than the in-sample residual, eliminating the
    prediction-shift bias that uniformly afflicts GBDT on small n.  With
    n_unb=253 inside each 5-fold cross-fit (effective train n ~= 200),
    prediction-shift is non-negligible; Ordered should produce slightly
    better-calibrated tree splits, but pays an O(K) memory + ~3-5x runtime
    cost.  This is the exact axis nb1554 left untouched.

PROTOCOL:
    1. Features: identical 117-col K-tuned stack as nb1554
       (top-25 AP + top-20 MACCS + top-20 Mord + top-20 ChempropEmbed
        + top-30 Avalon + pred_chembl_pec50 + mean_sim).
    2. Anchor: chemprop_aux te[unb_idx] (PRE-unblind, in_RAE 0.6216).
    3. CatBoost(loss=MAE, depth=4, n_est=200, lr=0.05, l2=5,
                boosting_type='Ordered').  5-seed bag, 5-fold cross-fit.
    4. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1752_catboost_ordered.py
    data/processed/nb1752_summary.json
    data/processed/nb1752_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # direct sibling: Plain boosting on identical 117-col stack
...
def _cat_params(seed: int, boosting_type: str = "Ordered") -> dict:
    # Ordered boosting is the differentiating axis vs nb1554.  Loss/depth/
    # iterations/lr/l2 are pinned to nb1554 exactly so the only moving part
    # is the boosting scheme.
    return dict(
        loss_function="MAE",
        depth=4,
        iterations=200,
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, boosting_type: str = "Ordered"
                                 ) -> tuple[np.ndarray, str]:
    """Cross-fit residuals. Falls back to Plain if Ordered OOMs (mirrors
    nb932's resilience pattern -- Ordered's memory cost scales with
    permutations and can spike on small machines)."""
    n = len(residual)
    ...
    for tr_loc, va_loc in kf.split(np.arange(n)):
        try:
            mdl = CatBoostRegressor(**_cat_params(seed, boosting_type=used))
            mdl.fit(X[tr_loc], residual[tr_loc])
        except (MemoryError, RuntimeError) as e:
            if used == "Ordered":
                print(f"      Ordered failed ({e}); falling back to Plain "
                      "for the remainder of this seed.")
                used = "Plain"
                mdl = CatBoostRegressor(**_cat_params(seed, boosting_type=used))
                mdl.fit(X[tr_loc], residual[tr_loc])
            else:
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5, "
          f"boosting_type='Ordered') on 5-way K-tuned 117-col matrix; "
    ...
    # AtomPair = nb1524 best_K, sliced from nb1484 SHAP ranking
    full_ap_ranked = _extract_atompair_top_idx_from_nb1484(sum_1484)
    K_AP_best = int(sum_1524["best_K"])
    top_ap_bit_idx = full_ap_ranked[:K_AP_best]

    # ChempropEmbed = nb1541 best_K, sliced from nb1541 top_dim_order
    ...
    # ---- Per-seed CatBoost(Ordered) residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST(ORDERED) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    per_seed_corrected = np.zeros((len(RESID_SEEDS), n_unb), dtype=np.float64)
    per_seed_rae: list[float] = []
    per_seed_records = []
    per_seed_boosting_used: list[str] = []
    any_fallback = False
    for i, s in enumerate(RESID_SEEDS):
        ts = time.time()
        resid_oof_s, used_s = _residual_cross_fit_one_seed(
            X_unb, residual, s, boosting_type="Ordered"
        )
        per_seed_boosting_used.append(used_s)
        if used_s != "Ordered":
            any_fallback = True
        ...
        print(f"   seed {s:3d}:  rae_corr = {rae_s:.4f}  "
              ...
              f"d_vs_nb1554 = {rae_s - NB1554_REF:+.4f})  "
              f"boost = {used_s}  "
    ...
    print(f"   boosting_used per seed = {per_seed_boosting_used}")

    # ---- Verdict (margin vs nb1554) ----
    ...
    if beats_nb1554:
        verdict = "CATBOOST_ORDERED_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_ORDERED_FLAT_VS_NB1554_ORDERED_AXIS_INERT"
    elif beats_anchor:
        verdict = "CATBOOST_ORDERED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_ORDERED_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_ORDERED_HURTS_ANCHOR"

    pre_unblind_clean = True
    print(f"   verdict                = {verdict}")
    print(f"   any_ordered_fallback   = {any_fallback}")
    ...
    summary = {
        ...
        "boosting_used_per_seed": per_seed_boosting_used,
        "any_ordered_fallback": bool(any_fallback),
        "K_AP_best": K_AP_best,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "catboost_boosting_type", "boosting_used_per_seed",
        "any_ordered_fallback",
