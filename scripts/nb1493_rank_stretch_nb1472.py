"""nb1493 -- RANK STRETCH grid on nb1472 (PRE-unblind anchor path).

Context:
  nb1330 stretch on nb1290 hurt. nb1411 stretch (truncated) likely flat.
  nb1472 PRE-unblind has different anchor -- may have different std shape.

Per memory: s in {1.05..1.10} yields -0.003 to -0.005 RAE on every
variance-compressed predictor.

Recipe: stretched[i] = mu + s * (pred[i] - mu), with mu = mean(pred).

Protocol:
  1. Load nb1472_mean_oof.npy (0.5330).
  2. Compute pred_std vs truth_std.
  3. In-sample grid: s in {0.95, 1.00, 1.02, 1.05, 1.08, 1.10, 1.12, 1.15,
     1.20, 1.25, 1.30}. Pool RAE per s.
  4. 5-fold cross-fit s grid.
  5. Verdict at 0.003 margin vs nb1472 (0.5330).

Outputs:
  data/processed/nb1493_summary.json
  data/processed/nb1493_best_s_oof.npy   -- in-sample stretched OOF (253,)
  data/processed/nb1493_cf_oof.npy       -- cross-fit stretched OOF (253,)
"""
...
SEED = 0
N_FOLDS = 5
STRETCH_GRID = [0.95, 1.00, 1.02, 1.05, 1.08, 1.10, 1.12, 1.15, 1.20, 1.25, 1.30]
NB1472_REF = 0.5330
MARGIN = 0.003
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- RANK STRETCH grid on nb1472 (PRE-unblind anchor)")
    print(f"          grid={STRETCH_GRID}")
    print(f"          margin={MARGIN} vs nb1472 (ref {NB1472_REF:.4f})")
    print("=" * 78)
    ...
    # ---- Load nb1472 OOF (mean blend across 3 families) ----
    p_path = DATA_PROCESSED / "nb1472_mean_oof.npy"
    ...
    rae_nb1472 = float(rae(y_unb, p_oof))
    print(f"\n[diag] nb1472 OOF pooled RAE = {rae_nb1472:.4f}  "
          f"(ref {NB1472_REF:.4f})")
    ...
    # Verify reference
    if abs(rae_nb1472 - NB1472_REF) > 5e-4:
        print(f"  WARN: rae_nb1472 ({rae_nb1472:.4f}) "
              f"differs from reference {NB1472_REF:.4f} by "
              f">5e-4 ({rae_nb1472 - NB1472_REF:+.4f}). Proceeding with actual.")
    ...
    # =================================================================
    candidates = {
        "nb1472_passthrough_s1.00": rae_nb1472,
    ...
    delta_cf_vs_nb1472 = rae_cf - rae_nb1472
    delta_is_vs_nb1472 = best_rae_is - rae_nb1472

    beats_nb1472_cf = rae_cf < rae_nb1472 - MARGIN
    flat_nb1472_cf = abs(rae_cf - rae_nb1472) < MARGIN
    beats_nb1472_is = best_rae_is < rae_nb1472 - MARGIN

    if beats_nb1472_cf:
        verdict = (f"STRETCH_NB1472_BEATS_NB1472_CROSSFIT  "
                   f"(cf RAE {rae_cf:.4f}, delta {delta_cf_vs_nb1472:+.4f})")
    elif flat_nb1472_cf:
        verdict = (f"STRETCH_NB1472_FLAT_VS_NB1472_CROSSFIT  "
                   f"(cf RAE {rae_cf:.4f}, delta {delta_cf_vs_nb1472:+.4f})")
    else:
        verdict = (f"STRETCH_NB1472_HURTS_VS_NB1472_CROSSFIT  "
                   f"(cf RAE {rae_cf:.4f}, delta {delta_cf_vs_nb1472:+.4f})")

    print("\n" + "=" * 78)
    print("VERDICT")
    print("=" * 78)
    print(f"   nb1472 standalone (s=1.00) : {rae_nb1472:.4f}  "
          f"(ref {NB1472_REF:.4f})")
    print(f"   best in-sample s={best_s_is:.2f} : {best_rae_is:.4f}  "
          f"(delta {delta_is_vs_nb1472:+.4f})")
    print(f"   5-fold cross-fit           : {rae_cf:.4f}  "
          f"(delta {delta_cf_vs_nb1472:+.4f})")
    print(f"   margin                     : {MARGIN}")
    print(f"   beats nb1472 cross-fit     : {beats_nb1472_cf}")
    print(f"   beats nb1472 in-sample     : {beats_nb1472_is}")
    ...
    summary = {
        ...
        "delta_cf_vs_nb1472": float(delta_cf_vs_nb1472),
        "delta_in_sample_vs_nb1472": float(delta_is_vs_nb1472),
        "beats_nb1472_cross_fit": bool(beats_nb1472_cf),
        "beats_nb1472_in_sample": bool(beats_nb1472_is),
        "flat_vs_nb1472_cross_fit": bool(flat_nb1472_cf),
        "verdict": verdict,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_cf_vs_nb1472", "delta_in_sample_vs_nb1472",
        "beats_nb1472_cross_fit", "beats_nb1472_in_sample", "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
