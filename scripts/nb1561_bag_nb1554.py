"""nb1561 -- Outer-bag VALIDATION of nb1554 (CatBoost 5-way K-tuned).

HYPOTHESIS:
    nb1554 reported a pooled mean_bag RAE = 0.5163 on the 253 PRE-unblind
    set using a 5-inner-seed CatBoost(MAE, d4, n200, lr0.05, l2=5) bag on
    a 117-col 5-way K-tuned feature stack
        ...
        mean_sim
    anchored on chemprop_aux te[unb_idx] (in_RAE 0.6216).

    The original nb1554 used fixed inner seeds [0, 1, 7, 42, 137].  This
    script wraps that recipe in an OUTER bag of 5 outer-seeds.  For each
    outer seed o, the inner-seed list is  inner_seeds = [o*1000 + s for
    s in [0, 1, 7, 42, 137]] -- entirely disjoint from the original 5
    inner seeds whenever o > 0 (and shifted away from them at o = 0).
    This probes seed sensitivity of the per-outer pooled mean-bag RAE.
...
PROTOCOL
    1. For each outer seed o in {0, 1, 7, 42, 137}:
         ...
         c. nb1554_o = mean of 5 inner pred_corr_{o,s'} (per-outer "mean_bag").
         d. Per-outer pooled RAE = rae(y_unb, nb1554_o).
    2. Row-level BoB MEAN + MEDIAN across the 5 nb1554_o vectors.
    3. Verdict NB1554_REPRODUCES iff
           |per_outer_mean - nb1554_ref| < 0.003
       where nb1554_ref = 0.5163 (the original nb1554 mean_bag RAE).

Outputs:
    scripts/nb1561_bag_nb1554.py                  (this file)
    data/processed/nb1561_summary.json
    data/processed/nb1561_bob_mean_oof.npy        (253,) float32
    data/processed/nb1561_bob_median_oof.npy      (253,) float32
"""
...
# Original nb1554 inner seeds were [0, 1, 7, 42, 137].
OUTER_SEEDS = [0, 1, 7, 42, 137]
INNER_OFFSETS = [0, 1, 7, 42, 137]
RESID_FOLDS = 5

# Reference: original nb1554 pooled mean_bag RAE on 253 PRE-unblind.
NB1554_REF = 0.5163
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1554/nb1551 (no leakage on PRE-unblind path)."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG VALIDATION of nb1554 CatBoost 5-way K-tuned")
    ...
    print(f"         nb1554_ref       = {NB1554_REF:.4f}  "
    ...
    # ---- Load all K-grid winners + SHAP rankings (same as nb1554) ----
    ...
    for oi, o in enumerate(OUTER_SEEDS):
        ...
        nb1554_o = inner_corrected.mean(axis=0)
        outer_nb1554[oi] = nb1554_o
        rae_nb1554_o = float(rae(y_unb, nb1554_o))
        rae_nb1554_o_median = float(
        ...
        per_outer_records.append({
            ...
            "rae_nb1554_o_mean_bag": rae_nb1554_o,
            "rae_nb1554_o_median_bag": rae_nb1554_o_median,
            "wall_sec": round(time.time() - t_outer, 2),
        })
        print(f"     [nb1554_o = mean over 5 inner seeds] pooled RAE = "
              f"{rae_nb1554_o:.4f}  (median_bag = {rae_nb1554_o_median:.4f})")
        print(f"   outer {o:3d}  wall = {time.time() - t_outer:.1f}s")

    # ---- Per-outer summary ----
    per_outer_rae = [rec["rae_nb1554_o_mean_bag"] for rec in per_outer_records]
    ...
    # ---- BoB row-level aggregations across 5 nb1554_o vectors ----
    bob_mean_oof = outer_nb1554.mean(axis=0)
    bob_median_oof = np.median(outer_nb1554, axis=0)
    ...
    print("=" * 78)
    print(f"   per-outer nb1554_o RAE list = "
          f"[{', '.join(f'{r:.4f}' for r in per_outer_rae)}]")
    print(f"   per-outer nb1554_o mean    = {per_outer_mean:.4f}")
    print(f"   per-outer nb1554_o std     = {per_outer_std:.4f}")
    print(f"   per-outer nb1554_o min     = {per_outer_min:.4f}")
    print(f"   per-outer nb1554_o max     = {per_outer_max:.4f}")
    print(f"   per-outer nb1554_o median  = {per_outer_median:.4f}")
    print(f"   BoB MEAN   pooled RAE = {rae_bob_mean:.4f}  "
          f"(d vs nb1554_ref = {rae_bob_mean - NB1554_REF:+.4f})")
    print(f"   BoB MEDIAN pooled RAE = {rae_bob_median:.4f}  "
          f"(d vs nb1554_ref = {rae_bob_median - NB1554_REF:+.4f})")

    # ---- Pearson sanity vs nb1554 mean_bag and anchor ----
    ...
    pearson_bobmean_vs_nb1551_mean = _pearson_vs(
        DATA_PROCESSED / "nb1551_bob_mean_oof.npy", bob_mean_oof
    )
    ...
    if pearson_bobmean_vs_nb1551_mean is not None:
        print(f"   Pearson(bob_mean, nb1551_bob_mean) = "
              f"{pearson_bobmean_vs_nb1551_mean:.4f}")
    print(f"   Pearson(bob_mean, anchor)          = "
          f"{pearson_bobmean_vs_anchor:.4f}")

    # ---- Verdict ----
    delta_per_outer = per_outer_mean - NB1554_REF
    reproduces = abs(delta_per_outer) < REPRODUCE_MARGIN
    if reproduces:
        verdict = "NB1554_REPRODUCES"
    elif per_outer_mean < NB1554_REF - REPRODUCE_MARGIN:
        verdict = "NB1554_OPTIMISTIC_OUTER_BAG_BETTER"
    else:
        verdict = "NB1554_LUCKY_SEED_OUTER_BAG_WORSE"

    print("\n" + "-" * 78)
    print(f"VERDICT (per-outer nb1554_o mean within {REPRODUCE_MARGIN} of "
          f"nb1554_ref = {NB1554_REF:.4f}):")
    ...
    summary = {
        ...
        "per_outer_rae_nb1554": per_outer_rae,
        ...
        "delta_per_outer_mean_vs_nb1554_ref": delta_per_outer,
        "delta_bob_mean_vs_nb1554_ref": rae_bob_mean - NB1554_REF,
        "delta_bob_median_vs_nb1554_ref": rae_bob_median - NB1554_REF,
        "reproduces": bool(reproduces),
        "pearson_bobmean_vs_nb1554_mean_bag": pearson_bobmean_vs_nb1554,
        "pearson_bobmean_vs_nb1551_bob_mean": pearson_bobmean_vs_nb1551_mean,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb1554_ref",
        "per_outer_rae_nb1554",
        "per_outer_mean", "per_outer_std",
        "per_outer_min", "per_outer_max", "per_outer_median",
        "rae_bob_mean", "rae_bob_median",
        "delta_per_outer_mean_vs_nb1554_ref",
        "delta_bob_mean_vs_nb1554_ref",
        "delta_bob_median_vs_nb1554_ref",
        "reproduces",
        "pearson_bobmean_vs_nb1554_mean_bag",
        "pearson_bobmean_vs_nb1551_bob_mean",
