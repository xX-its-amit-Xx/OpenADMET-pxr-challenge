"""nb1723 -- ExtraTreesRegressor on K-tuned 5-way 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE on the same 117-col 5-way K-tuned stack) anchors
    the residual-learner regime at mean_bag RAE = 0.5163.  ExtraTrees is a
    qualitatively different learner: fully randomized splits, no boosting,
    no implicit regularization beyond bagging.  If the residual surface is
    smooth + axis-aligned (the way Boost+MAE assumes), ET should lose; if
    the surface has high-variance ridges that benefit from un-tuned random
    cuts, ET could match or even outperform.  This script is the residual
    learner family probe: completely different inductive bias relative to
    every prior nb15xx/nb17xx residual head.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Reuse the exact 117-col feature stack from nb1554
       (top-25 AP + top-20 MACCS + top-20 Mord + top-20 ChempropEmbed
        + top-30 Avalon + pred_chembl_pec50 + mean_sim).
    3. 5-seed bag ExtraTreesRegressor(n_est=200, max_depth=8,
       max_features=0.6, min_samples_leaf=10). KFold(n=5, shuffle=True)
       cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1723_extra_trees.py
    data/processed/nb1723_summary.json
    data/processed/nb1723_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # CatBoost on same 117-col stack -- primary comparator
DECISION_MARGIN = 0.003

# ExtraTrees hyperparameters per protocol
ET_PARAMS_BASE = dict(
    n_estimators=200,
    max_depth=8,
    max_features=0.6,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- ExtraTrees(n200, d8, mf0.6, msl10) on 5-way K-tuned "
    ...
    # ---- Per-seed ExtraTrees residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED EXTRATREES RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1554:
        verdict = "EXTRATREES_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "EXTRATREES_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "EXTRATREES_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif beats_anchor:
        verdict = "EXTRATREES_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "EXTRATREES_FLAT_VS_ANCHOR"
    else:
        verdict = "EXTRATREES_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "et_n_estimators": ET_PARAMS_BASE["n_estimators"],
        "et_max_depth": ET_PARAMS_BASE["max_depth"],
        "et_max_features": ET_PARAMS_BASE["max_features"],
        "et_min_samples_leaf": ET_PARAMS_BASE["min_samples_leaf"],
        "K_AP_best": K_AP_best,
