"""nb2021 -- CatBoost(RMSE) with binary-bit columns marked as cat_features.

HYPOTHESIS:
    Combine nb1912's CatBoost(RMSE) residual learner with nb1981's binary-bit
    -> categorical marking trick on the SAME 117-col 5-way K-tuned matrix and
    SAME chemprop_aux PRE-unblind anchor. CatBoost's native cat_features path
    uses target-statistic encoding with ordered-boosting bias reduction, which
    may extract a different residual signal from sparse Bernoulli bits than
    either plain LGBM-MSE (nb1861=0.5013) or plain CatBoost-RMSE (nb1912)
    on those same binary columns treated as continuous.

PROTOCOL:
    1. Features: 117-col 5-way K-tuned (AtomPair K=25 + MACCS K=20 +
       Mordred K=20 + ChempropEmbed K=20 + Avalon K=30 + pred_chembl_pec50
       + mean_sim) -- identical stack to nb1861/nb1912/nb1981.
    2. Anchor: chemprop_aux te[unb_idx]  -> residual = y_unb - anchor.
    3. Binary bit indices (75 total): AP [0..25) U MACCS [25..45) U
       Avalon [85..115).  Marked via CatBoost cat_features at .fit() time.
    ...
    5. 5-seed bag: seeds = [0, 1, 7, 42, 137]
       Each seed: 5-fold KFold(shuffle=True, random_state=seed) cross-fit on
       residual, predictions added back to anchor -> per-seed corrected OOF.
    6. mean_bag = mean of 5 corrected OOFs -> rae_mean_bag.
    7. Verdict at 0.003 margin vs nb1861 (rae 0.5013):
         delta < -0.003  -> BEATS_NB1861
         |delta| <= 0.003 -> TIES_NB1861
         delta > +0.003  -> LOSES_vs_NB1861

Outputs:
    scripts/nb2021_catboost_cat_rmse.py
    data/processed/nb2021_summary.json
    data/processed/nb2021_per_seed_oof.npy   (5, 253) float32
    data/processed/nb2021_mean_bag_oof.npy   (253,)   float32
"""
...
NB1912_SUMMARY = DATA_PROCESSED / "nb1912_summary.json"
NB1981_SUMMARY = DATA_PROCESSED / "nb1981_summary.json"
...
NB1861_REF = 0.5013  # user-stated comparator
...
def _catboost_params(seed: int) -> dict:
    """CatBoostRegressor(RMSE) -- user-specified knobs (matches nb1912)."""
    return dict(
        loss_function="RMSE",
        depth=4,
        n_estimators=200,
...
def _residual_cross_fit_one_seed_with_cat(
    X: np.ndarray, residual: np.ndarray, seed: int,
    cat_indices: list[int],
) -> np.ndarray:
    """5-fold cross-fit CatBoost(RMSE) with cat_features=cat_indices.

    CatBoost cat_features expects integer-typed columns. We cast the binary
    bits to int32 (already 0/1) and pass the column index list directly.
    """
    n, p = X.shape
    # CatBoost requires categorical columns to be integer or string dtype.
    # Build a DataFrame column-by-column so each column has the correct dtype
    # (float for continuous, int32 for binary). Direct assignment after
    # DataFrame construction does not always preserve per-column dtypes when
    # the source is a float ndarray.
    cat_set = set(int(ci) for ci in cat_indices)
    feat_names = [f"f{i}" for i in range(p)]
    col_dict = {}
    for ci in range(p):
        col = X[:, ci]
        if ci in cat_set:
            col_dict[feat_names[ci]] = col.astype(np.int32)
        else:
            col_dict[feat_names[ci]] = col.astype(np.float32)
    df_all = pd.DataFrame(col_dict)

    # Sanity check: every cat_feature column must be integer-typed
    for ci in cat_indices:
        if df_all.dtypes.iloc[ci].kind not in ("i", "u"):
            raise TypeError(
                f"cat_feature col {ci} has non-integer dtype "
                f"{df_all.dtypes.iloc[ci]}"
            )

    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        df_tr = df_all.iloc[tr_loc].reset_index(drop=True)
        df_va = df_all.iloc[va_loc].reset_index(drop=True)
        train_pool = Pool(
            data=df_tr,
            label=residual[tr_loc],
        ...
        mdl.fit(train_pool)
        oof[va_loc] = mdl.predict(val_pool)
    return oof
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(RMSE) 5-seed bag x 5-fold cross-fit on 117-col "
          f"5-way K-tuned matrix with BINARY BITS as cat_features; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          CatBoost: loss=RMSE depth=4 n_est=200 lr=0.05 "
          f"l2_leaf_reg=5 + cat_features=binary_bit_indices")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1861 ({NB1861_REF:.4f})")
    print(f"          decision_margin = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- nb1861 reference if present ----
    if NB1861_SUMMARY.exists():
        with open(NB1861_SUMMARY) as f:
            nb1861_sum = json.load(f)
        nb1861_bob = float(nb1861_sum.get("bob_mean_rae", NB1861_REF))
        print(f"[ref] nb1861_summary.bob_mean_rae = {nb1861_bob:.4f}  "
              f"(user-stated comparator = {NB1861_REF:.4f})")
    else:
        nb1861_bob = NB1861_REF
        print(f"[ref] nb1861_summary.json missing -- using user-stated "
              f"comparator {nb1861_bob:.4f}")
    ...
    # Build the 117-col stack -- order matches nb1861/nb1912/nb1981.
    ...
    # ---- Build the binary indices list (same definition as nb1981) ----
    # AP bits occupy [0, n_top_ap)
    # MACCS bits occupy [n_top_ap, n_top_ap + n_top_maccs)
    # Mordred occupies [maccs_start+n_top_maccs, +n_top_mord)
    # Embed   occupies [mord_start+n_top_mord, +n_top_embed)
    # Avalon  occupies [embed_start+n_top_embed, +n_top_avalon)
    # extras  occupies [avalon_start+n_top_avalon, +2)
    ap_start = 0
    ...
    # ---- 5-seed bag x 5-fold cross-fit ----
    print("\n" + "-" * 78)
    print(f"CATBOOST(RMSE) + CAT_FEATURES on binary bits  "
          f"5-seed bag x {RESID_FOLDS}-fold cross-fit  (dim={feat_dim})  "
          f"cat_features -> {n_binary} bits")
    print("-" * 78)
    ...
    for s_i, seed in enumerate(SEEDS):
        t_s = time.time()
        resid_oof_s = _residual_cross_fit_one_seed_with_cat(
            X_unb, residual, seed, cat_indices=binary_indices_list,
        )
        pred_corr_s = anchor + resid_oof_s
        per_seed_oof[s_i] = pred_corr_s
        ...
        print(f"   seed={seed:4d}  rae={r_s:.4f}  "
    ...
    print(f"   nb1861 comparator   = {NB1861_REF:.4f}")
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        ...
        verdict = "LOSES_vs_NB1861"

    print(f"   verdict             = {verdict}  (margin {DECISION_MARGIN})")
    ...
    summary = {
        ...
        "cat_depth": 4,
        "cat_n_estimators": 200,
        "cat_learning_rate": 0.05,
        "cat_l2_leaf_reg": 5,
        "cat_features_used": "binary_bit_indices (AP+MACCS+Avalon)",
        "seeds": SEEDS,
        ...
        "per_seed_mean_rae": seed_mean,
        "per_seed_median_rae": seed_median,
        "per_seed_std_rae": seed_std,
        "per_seed_min_rae": seed_min,
        "per_seed_max_rae": seed_max,
        ...
        "nb1861_bob_mean_observed": nb1861_bob,
