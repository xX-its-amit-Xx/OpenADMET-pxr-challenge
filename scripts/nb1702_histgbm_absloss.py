"""nb1702 -- HistGradientBoostingRegressor (absolute_error) on PRE-unblind
            5-way K-tuned 117-col matrix.

HYPOTHESIS:
    nb1554 (CatBoost MAE, d4, n200, lr0.05, l2=5) on the 5-way K-tuned 117-col
    matrix yielded mean_bag RAE = 0.5163.  sklearn's HistGradientBoosting with
    'absolute_error' loss is a Huber/MAE-style fast tree learner with
    histogram-binned splits -- it differs from CatBoost in split criterion
    (ordered boosting vs L1 leaf optimization) and from LightGBM-MAE in
    histogram quantization.  This gives a third independent absolute-loss
    flavor on the same K-tuned matrix.  Goal: drive mean_bag RAE below
    nb1554's 0.5163 at margin 0.003.

PROTOCOL:
    1. Features: same 117-col 5-way K-tuned matrix as nb1554
    ...
    3. 5-seed bag HistGradientBoostingRegressor(
...
Outputs:
    scripts/nb1702_histgbm_absloss.py
    data/processed/nb1702_summary.json
    data/processed/nb1702_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # CatBoost MAE on same 117-col 5-way K-tuned matrix
...
def _hgbr_params(seed: int) -> dict:
    return dict(
        loss="absolute_error",
        max_iter=200,
        learning_rate=0.05,
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
    ...
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = HistGradientBoostingRegressor(**_hgbr_params(seed))
...
def main() -> dict:
    ...
    # ---- Per-seed HGBR residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED HGBR(absolute_error) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    # ---- Verdict (primary comparator: nb1554 at margin 0.003) ----
    ...
    if beats_nb1554:
        verdict = "HGBR_ABSLOSS_5WAY_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "HGBR_ABSLOSS_5WAY_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "HGBR_ABSLOSS_5WAY_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif beats_nb1543:
        verdict = "HGBR_ABSLOSS_5WAY_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif beats_anchor:
        verdict = "HGBR_ABSLOSS_5WAY_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "HGBR_ABSLOSS_5WAY_FLAT_VS_ANCHOR"
    else:
        verdict = "HGBR_ABSLOSS_5WAY_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "hgbr_loss": "absolute_error",
        "hgbr_max_iter": 200,
        "hgbr_learning_rate": 0.05,
        "hgbr_max_depth": 4,
        "hgbr_l2_regularization": 1.0,
        "K_AP_best": K_AP_best,
