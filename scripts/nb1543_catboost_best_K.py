"""nb1543 -- CatBoost on PRE-unblind 4-way features using best K from K-grids.

HYPOTHESIS:
    nb1523 (Mordred K-grid) found best_K=20 (rae_mean_bag 0.5354), which beats
    the K=30 default used by nb1484/nb1501.
    nb1524 (AtomPair K-grid) found best_K=25 (rae_mean_bag 0.5520), which
    beats the K=30 default used by nb1484/nb1501.
    Combine these two K-grid winners with the standard top-20 MACCS and top-30
    chemprop embed, then re-fit the nb1501 CatBoost(MAE, d4, n200, lr0.05,
    l2=5) recipe on the concatenated K_AP=25 + MACCS=20 + K_Mord=20 + embed=30
    matrix.  Goal: stack the K-grid wins into one combined CatBoost candidate
    that beats nb1501 (0.5223 mean_bag) and nb1484 (0.5231 SLSQP) on the
    PRE-unblind path.
...
PROTOCOL:
    ...
    2. Build features:
            top-25 AtomPair  (nb1524 best_K winning indices, top-K of
                              nb1373 SHAP ranking via nb1484 family entry)
            top-20 MACCS     (nb1352)
            top-20 Mordred   (nb1523 best_K=20 top_col_idx list)
            top-30 chemprop embed (nb1484 family entry)
            pred_chembl_pec50 (ChEMBL PXR kNN-5)
            mean_sim
       Total dim = 25 + 20 + 20 + 30 + 2 = 97.
    ...
    5. Verdict at 0.003 margin vs:
            ...
            nb1484 4-way LGBM-Huber SLSQP best            0.5231
            nb1501 CatBoost 4-way K=30 mean_bag           0.5223  (KEY REF)

Outputs:
    scripts/nb1543_catboost_best_K.py
    data/processed/nb1543_summary.json
    data/processed/nb1543_mean_bag_oof.npy            (253,) float32
"""
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1441 / nb1460 / nb1483 / nb1484 / nb1501."""
    frames = []
...
def main() -> dict:
    ...
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on 4-way pruned "
          f"features using K-grid winners; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1484 ({NB1484_REF:.4f}), nb1501 ({NB1501_REF:.4f})  "
    ...
    for p in (NB1352_SUMMARY, NB1484_SUMMARY, NB1523_SUMMARY, NB1524_SUMMARY):
        if not p.exists():
    ...
    # MACCS = standard top-20 from nb1352 (matches nb1484/nb1501)
    top_maccs_bit_idx = np.array(
        sum_1352["top_maccs_bit_indices_ranked"], dtype=int
    )

    # Mordred = nb1523 best_K winner (K=20) -- top_col_idx from per_K_records
    rec_mord = _extract_best_K_record(sum_1523, "per_K_records",
                                       best_K_key="best_K")
    K_Mord_best = int(rec_mord["K"])
    top_mord_col_idx = np.array(rec_mord["top_col_idx"], dtype=int)
    # Sanity: K_Mord_best should equal nb1523 best_K
    assert K_Mord_best == int(sum_1523["best_K"]), \
        f"K_Mord mismatch: rec={K_Mord_best} vs best={sum_1523['best_K']}"

    # AtomPair = top-K_AP_best from nb1484 AtomPair SHAP ranking
    # nb1524 only stored best_K (=25), not the corresponding top_idx list.
    # The nb1524 K-grid uses the SAME SHAP ranking on AtomPair bits as
    # nb1484 (both fits use nb1373/nb1484 family pruning).  So we take
    # the top-K_AP_best of the nb1484-AtomPair top_idx_ranked.
    full_ap_ranked = _extract_atompair_top_idx_from_nb1484(sum_1484)
    K_AP_best = int(sum_1524["best_K"])
    top_ap_bit_idx = full_ap_ranked[:K_AP_best]

    # Chemprop embed = standard top-30 from nb1484 family
    ...
    print(f"[reuse] top-{n_top_mord} Mordred cols (nb1523 best K={K_Mord_best})")
    print(f"[reuse] top-{n_top_ap} AtomPair bits (nb1524 best K={K_AP_best}, "
          f"sliced from nb1484 SHAP rank)")
    print(f"[reuse] top-{n_top_embed} Chemprop embed (from nb1484)")
    ...
    # ---- Build COMBINED-K feature matrix ----
    ...
    print(f"\n   COMBINED-K feature matrix: {X_unb.shape}  "
          f"(top-{n_top_ap} AP + top-{n_top_maccs} MACCS + "
          f"top-{n_top_mord} Mord + top-{n_top_embed} Embed + 2)")
    ...
    if beats_nb1501:
        verdict = ("COMBINED_K_BEATS_NB1501_NEW_PRE_UNBLIND_PRIMARY")
    elif flat_vs_nb1501:
        verdict = "COMBINED_K_FLAT_VS_NB1501"
    elif beats_nb1484:
        verdict = "COMBINED_K_BEATS_NB1484_BUT_WORSE_THAN_NB1501"
    elif flat_vs_nb1484:
        verdict = "COMBINED_K_FLAT_VS_NB1484"
    elif beats_anchor:
        verdict = "COMBINED_K_BEATS_ANCHOR_BUT_WORSE_THAN_NB1484"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "COMBINED_K_FLAT_VS_ANCHOR"
    else:
        verdict = "COMBINED_K_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "K_AP_source": "nb1524 best_K",
        "K_Mord_source": "nb1523 best_K",
        "K_MACCS_fixed": n_top_maccs,
        "K_embed_fixed": n_top_embed,
