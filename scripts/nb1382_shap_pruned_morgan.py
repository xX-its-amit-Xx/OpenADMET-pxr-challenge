"""nb1382 -- SHAP-pruned Morgan-2048 top-30 + ChEMBL residual learner.

Hypothesis:
    Morgan-2048 (ECFP4) was the OG fingerprint family used throughout this
    project (combined features in nb02_baseline, kNN in nb05, etc.).  nb1352
    showed that SHAP-pruning MACCS 167 -> 20 + (pred_chembl + sim) cut residual
    capacity at n=253 and improved RAE to 0.5323.  nb1373 then showed the same
    recipe on AtomPair-2048 -> 30 dropped further to 0.5095 (mean-bag).
    By analogy, SHAP-pruning Morgan-2048 -> 30 bits may yield a similar or
    better mean-bag RAE than AtomPair (the original Morgan family has been
    workhorse for PXR scaffold neighbourhoods).

Protocol:
    1.  Load Morgan-2048 features for the 253 unblind: rebuild via
        morgan_fp_batch(radius=2, n_bits=2048) over standardized test SMILES
        and slice to unblind indices.  (No persistent te_morgan.npy cache
        exists; rebuild is ~1-2s.)
    2.  Anchor = nb1070_pred_oof on 253; residual = y_unb - anchor.
    3.  Build ChEMBL PXR pool (same union as nb1242 / nb1352 / nb1373),
        compute kNN-5 Tanimoto on cached Morgan-2048 over std test SMILES;
        pull pred_chembl_pec50 + mean_sim and slice to unblind.
    4.  Build FULL 2050-col feature matrix on 253 (Morgan-2048 + pred_chembl
        + mean_sim).  Train ONE seed-0 shallow LGBM Huber on residual.
        Compute SHAP importance via shap.TreeExplainer; fall back to LGBM
        gain on failure.  Take mean(|SHAP|) per feature, slice Morgan-only,
        pick top-30 bit indices.
    5.  Build PRUNED 32-col feature matrix = top-30 Morgan + pred_chembl
        + mean_sim.
    ...
    7.  Verdict at 0.003 margin vs:
            ...
            nb1352  (SHAP-pruned MACCS top-20, 0.5323 mean-bag)
            nb1070  (anchor, ~0.5771 pooled)

Outputs:
    scripts/nb1382_shap_pruned_morgan.py        (this file)
    data/processed/nb1382_summary.json
    data/processed/nb1382_mean_bag_oof.npy        (253,) float32
    data/processed/nb1382_median_bag_oof.npy      (253,) float32
    data/processed/nb1382_per_seed_corrected_oof.npy (5, 253) float32
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP-pruned Morgan-{MORGAN_NBITS} top-{TOP_K_MORGAN} + "
          f"ChEMBL residual; anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          baselines: nb1352 ({NB1352_REF:.4f}), "
          f"nb1373 ({NB1373_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- ChEMBL pool ----
    print("\n" + "-" * 78)
    print("CHEMBL PXR POOL (local cache; same union as nb1352 / nb1373)")
    print("-" * 78)
    ...
    # ---- Morgan FPs for ChEMBL pool (for kNN) ----
    fp_pool = morgan_fp_batch(
        pool["std_smiles"].tolist(),
    ...
    # ---- Morgan FPs for test set (rebuild — no persistent cache) ----
    std_test_smiles = []
    for m in test_mols:
        if m is None:
    ...
    print(f"   morgan FP test: shape={fp_test.shape}  "
          f"density={fp_test.mean():.4f}  (wall {time.time() - t_fp0:.1f}s)")

    # ---- kNN k=5 Tanimoto on Morgan ----
    ...
    # ---- Morgan-2048 (unblind slice) ----
    X_morgan_te = fp_test.astype(np.float32)
    n_morgan = int(X_morgan_te.shape[1])
    print(f"   Morgan cache shape = {X_morgan_te.shape}  (n_bits={n_morgan})")
    X_morgan_unb = X_morgan_te[unb_idx]
    print(f"   bit density (unb) = {X_morgan_unb.mean():.4f}  "
          f"const cols = {int((X_morgan_unb.var(axis=0) == 0).sum())}/{n_morgan}")

    # ---- Build FULL feature matrix (Morgan + pred_chembl + sim) ----
    ...
    print(f"   FULL feature matrix: {X_unb_full.shape}  "
          f"(Morgan-{n_morgan} + pred_chembl + sim)")
    ...
    print(f"   pred_chembl importance = {imp_full[n_morgan]:.4f}")
    print(f"   sim importance         = {imp_full[n_morgan + 1]:.4f}")

    # Morgan-only importance slice
    m_imp = imp_full[:n_morgan]
    top_k = min(TOP_K_MORGAN, n_morgan)
    top_bit_order = np.argsort(-m_imp)
    top_bit_idx = top_bit_order[:top_k].astype(int)
    top_bit_idx_sorted = np.sort(top_bit_idx)
    top_bit_imp = m_imp[top_bit_idx]
    print(f"   top-{top_k} Morgan bit indices (ranked by importance):")
    ...
    n_nonzero_imp = int((m_imp > 0).sum())
    print(f"   Morgan bits with nonzero importance: {n_nonzero_imp}/{n_morgan}")

    # ---- Build PRUNED 32-col feature matrix ----
    X_morgan_unb_pruned = X_morgan_unb[:, top_bit_idx]
    ...
    print(f"\n   PRUNED feature matrix: {X_unb_pruned.shape}  "
          f"(top-{top_k} Morgan + pred_chembl + sim)")
    ...
    print(f"   nb1352 ref            = {NB1352_REF:.4f}  (SHAP-pruned MACCS top-20)")
    ...
    if beats_nb1373:
        verdict = "SHAP_PRUNED_MORGAN_BEATS_NB1373_NEW_PRIMARY_CANDIDATE"
    elif abs(rae_mean_bag - NB1373_REF) < DECISION_MARGIN:
        verdict = "SHAP_PRUNED_MORGAN_FLAT_VS_NB1373"
    elif beats_nb1352:
        verdict = "SHAP_PRUNED_MORGAN_BEATS_NB1352_BUT_WORSE_THAN_NB1373"
    elif beats_nb1070:
        verdict = "SHAP_PRUNED_MORGAN_HELPS_NB1070_BUT_WORSE_THAN_NB1352"
    else:
        verdict = "SHAP_PRUNED_MORGAN_HURTS_NB1070"
    ...
    summary = {
        ...
        "n_morgan_bits": n_morgan,
        "morgan_bit_density_unb": float(X_morgan_unb.mean()),
        "morgan_const_cols": int((X_morgan_unb.var(axis=0) == 0).sum()),
        "morgan_nonzero_imp_bits": n_nonzero_imp,
        "shap_importance_source": imp_src,
        "top_k_morgan": int(top_k),
        "top_morgan_bit_indices_ranked": [int(b) for b in top_bit_idx.tolist()],
        "top_morgan_bit_importance_ranked": [float(v) for v in top_bit_imp.tolist()],
        "top_morgan_bit_indices_sorted_asc": [int(b) for b in top_bit_idx_sorted.tolist()],
        "pred_chembl_importance": float(imp_full[n_morgan]),
        "sim_importance": float(imp_full[n_morgan + 1]),
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_chembl_pool", "n_morgan_bits", "shap_importance_source",
        "top_k_morgan",
        "top_morgan_bit_indices_ranked",
        "top_morgan_bit_importance_ranked",
        "pred_chembl_importance", "sim_importance",
        "feat_dim_full", "feat_dim_pruned",
        "morgan_nonzero_imp_bits",
        ...
        "beats_nb1070", "beats_nb1352", "beats_nb1373",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
