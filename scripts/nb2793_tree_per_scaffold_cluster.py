"""nb2793 -- Decision tree per scaffold cluster fallback on chemprop_aux residual.

NEW PARADIGM (vs single global LGBM K=20):
    Cluster the 253 unblind compounds into K=10 scaffold-shape clusters via
    K-means on the Bemis-Murcko-scaffold Morgan fingerprint (1024-bit), then
    fit a SHALLOW DecisionTreeRegressor (max_depth=3, min_samples_leaf=5) on
    chemprop_aux RESIDUAL inside each cluster. At inference, route each test
    row to the cluster whose centroid (in Morgan-FP space) has the highest
    Tanimoto similarity to the test compound's Bemis-Murcko scaffold FP.

    Hypothesis: residual on chemprop_aux is not stationary across scaffold
    families -- different scaffolds use different physchem axes. A single
    deep tree blurs them; per-cluster shallow trees (depth=3, 8 leaves max)
    fit a locally constant residual correction without overfitting the
    n_cluster=~25 effective per-leaf sample size.

    This is a SUBSTRATE-change move (cycle-134 sense): we keep the
    chemprop_aux anchor and K=20 features, but swap the *model class*
    from single global LGBM to a per-cluster routed-DT ensemble.
...
PROTOCOL:
    ...
    4. Per scaffold-CV fold (5-fold, kf_seeds 1001..1005):
       ...
       b. For each cluster c: fit DT(max_depth=3, min_samples_leaf=5) on
          chemprop_aux residual using K=20 features for compounds in c.
       c. For each val row: route via highest-Tanimoto centroid, predict
          residual using that cluster's DT.
    5. Mean-bag across 5 kf_seeds at OOF (residual) level.
    6. Deploy: refit K-means(K=10) + per-cluster DTs on all 253; predict
...
OUTPUTS:
    scripts/nb2793_tree_per_scaffold_cluster.py
    data/processed/nb2793_summary.json
    data/processed/nb2793_pred_oof.npy   (253,) float32 mean-bag CORRECTED
    data/processed/te_nb2793.npy         (513,) float32 deploy refit
    submissions/nb2793_tree_per_scaffold_cluster.csv
"""
...
def _scaffold_fp_for_smiles(smiles_list: list[str]) -> np.ndarray:
    ...
    # First try: FP from scaffold SMILES
    scaf_fp = morgan_fp_batch(scaffolds, radius=SCAF_FP_RADIUS, n_bits=SCAF_FP_BITS)
    # Fallback rows: any scaffold that is None / "" / produced all-zero FP
...
def _tanimoto_to_centroids(
    ...
    Centroids are continuous KMeans means; we threshold at 0.5 to get a
    boolean centroid representation, then compute Tanimoto on bit-sets.
    Returns (K,) similarities."""
...
def _fit_per_cluster_dt(
    ...
    """Fit a shallow DecisionTreeRegressor per cluster on chemprop_aux residual.

    For empty clusters (no training rows assigned), fall back to a
    constant-zero predictor (no residual correction)."""
    cluster_models: dict[int, object] = {}
    for c in range(n_clusters):
        mask = train_labels == c
        n_c = int(mask.sum())
        if n_c < DT_MIN_SAMPLES_LEAF * 2:
            # Too small to fit -- fall back to the cluster mean residual
            # (or 0 if truly empty)
        ...
        dt.fit(X_train[mask], resid_train[mask])
        cluster_models[c] = ("dt", dt)
...
def _scaffold_cv_one_seed(
    ...
    """One scaffold-CV pass: per-fold K-means + per-cluster DT on residual."""
    ...
    for fold_i, (tr_loc, va_loc) in enumerate(splits):
        # Cluster on TRAIN-fold scaffold FPs only
        km, tr_labels, centroids_bool = _kmeans_and_centroids_bool(
            fp_scaf[tr_loc], N_CLUSTERS, kf_seed,
        )
        # Fit per-cluster DTs on training residuals
        cluster_models = _fit_per_cluster_dt(
            X[tr_loc], residual[tr_loc], tr_labels, N_CLUSTERS, kf_seed,
        )
        # Route validation rows by Tanimoto to centroids
...
def _deploy_te_one_seed(
    ...
    """Fit K-means + per-cluster DTs on all 253; predict 513 routed residuals."""
    km, tr_labels, centroids_bool = _kmeans_and_centroids_bool(
...
def main() -> dict:
    ...
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DT per scaffold-cluster (K={N_CLUSTERS}) on "
          f"{ANCHOR} residual")
    print(f"        K-means on Bemis-Murcko scaffold Morgan FP "
          f"(radius={SCAF_FP_RADIUS}, n_bits={SCAF_FP_BITS})")
    print(f"        DT(max_depth={DT_MAX_DEPTH}, "
          f"min_samples_leaf={DT_MIN_SAMPLES_LEAF}) per cluster")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_tree_per_scaffold_cluster.csv"
    pd.DataFrame({
        "SMILES": test_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "DT_per_scaffold_cluster_K10_chemprop_aux_residual",
        "rationale": (
            "K-means(K=10) on Bemis-Murcko-scaffold Morgan FP partitions "
            "the 253 unblind; shallow DT(depth=3) per cluster on K=20 "
            "features regresses chemprop_aux residual; test rows routed "
            "by Tanimoto-to-centroid"
        ),
        "anchor": ANCHOR,
