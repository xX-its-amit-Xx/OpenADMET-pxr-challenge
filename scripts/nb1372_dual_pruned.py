"""nb1372 -- Dual-pruned: top-20 MACCS + top-30 Mordred + ChEMBL residual learner.

Hypothesis:
    nb1352 (MACCS-pruned 20 + ChEMBL, 22 cols) -> mean_bag 0.5323.
    nb1364 (Mordred-pruned 30 + ChEMBL, 32 cols) -> mean_bag 0.5242.
    Substructure (MACCS bits) and descriptor (Mordred numerics) signals
    may be complementary.  Concatenating BOTH SHAP-pruned subsets into a
    single 52-col residual learner could capture orthogonal signal while
    keeping feature count manageable for n=253.

Protocol:
    1. Reuse top-20 MACCS bit indices from nb1352_summary.json
       (top_maccs_bit_indices_ranked).
    2. Reuse top-30 Mordred col indices from nb1364_summary.json
       (top_mordred_col_indices_ranked).
    3. Rebuild ChEMBL kNN feature pipeline -- same local-parquet union
       + InChIKey leak guard + k=5 weighted Tanimoto -- extract
       pred_chembl_pec50 + sim on 513 -> slice to 253 unblind.
    4. Features = top-20 MACCS bits + top-30 Mordred cols
    ...
    6. Verdict at 0.003 margin vs nb1352 (0.5323 mean).
    7. Report Pearson(mean_bag_oof, nb1352_mean_bag_oof) for orthogonality.

Outputs:
    scripts/nb1372_dual_pruned.py                        (this file)
    data/processed/nb1372_summary.json
    data/processed/nb1372_mean_bag_oof.npy               (253,) float32
    data/processed/nb1372_per_seed_corrected_oof.npy     (5, 253) float32
    data/processed/nb1372_median_bag_oof.npy             (253,) float32
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DUAL-PRUNED top-20 MACCS + top-30 Mordred + ChEMBL; "
          f"anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          baseline refs: nb1352 (0.5323), nb1364 (0.5242)  "
    ...
    # ---- Reuse SHAP picks from nb1352 (MACCS) and nb1364 (Mordred) ----
    if not NB1352_SUMMARY.exists():
        raise FileNotFoundError(f"missing {NB1352_SUMMARY} -- run nb1352 first")
    if not NB1364_SUMMARY.exists():
        raise FileNotFoundError(f"missing {NB1364_SUMMARY} -- run nb1364 first")
    ...
    print("\n" + "-" * 78)
    print("CHEMBL PXR POOL (same union as nb1352 / nb1364)")
    print("-" * 78)
    ...
    # ---- Build DUAL-PRUNED 52-col feature matrix on 253 ----
    ...
    expected_dim = n_top_maccs + n_top_mord + 2
    if feat_dim != expected_dim:
        raise ValueError(f"feat_dim {feat_dim} != expected {expected_dim}")
    print(f"\n   DUAL-PRUNED feature matrix: {X_unb.shape}  "
          f"(top-{n_top_maccs} MACCS + top-{n_top_mord} Mordred + "
          f"pred_chembl + sim)")

    # ---- Per-seed residual cross-fit on DUAL-PRUNED features ----
    print("\n" + "-" * 78)
    ...
    # ---- Pearson correlation vs nb1352 mean-bag OOF (orthogonality check) ----
    nb1352_path = DATA_PROCESSED / "nb1352_mean_bag_oof.npy"
    pearson_vs_nb1352 = None
    if nb1352_path.exists():
        oof_1352 = np.load(nb1352_path).astype(np.float64)
        if oof_1352.shape[0] == n_unb:
            pearson_vs_nb1352 = float(
                np.corrcoef(mean_bag_oof, oof_1352)[0, 1]
            )

    nb1364_path = DATA_PROCESSED / "nb1364_mean_bag_oof.npy"
    pearson_vs_nb1364 = None
    if nb1364_path.exists():
        oof_1364 = np.load(nb1364_path).astype(np.float64)
        if oof_1364.shape[0] == n_unb:
            pearson_vs_nb1364 = float(
                np.corrcoef(mean_bag_oof, oof_1364)[0, 1]
            )
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1364 = {rae_mean_bag - NB1364_REF:+.4f})")
    ...
    if beats_nb1364:
        verdict = "DUAL_PRUNED_BEATS_NB1364_NEW_PRIMARY_CANDIDATE"
    elif abs(rae_mean_bag - NB1364_REF) < DECISION_MARGIN:
        verdict = "DUAL_PRUNED_FLAT_VS_NB1364"
    elif beats_nb1352:
        verdict = "DUAL_PRUNED_BEATS_NB1352_BUT_NOT_NB1364"
    elif abs(rae_mean_bag - NB1352_REF) < DECISION_MARGIN:
        verdict = "DUAL_PRUNED_FLAT_VS_NB1352"
    elif beats_nb1070:
        verdict = "DUAL_PRUNED_HELPS_NB1070_BUT_WORSE_THAN_NB1352"
    else:
        verdict = "DUAL_PRUNED_HURTS_NB1070"
    ...
    summary = {
        "tag": TAG,
        "anchor": ANCHOR,
        "data_source": "MACCS-cache + Mordred-cached_nb1030 + local_chembl_caches_union",
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_chembl_pool", "n_top_maccs", "n_top_mordred",
