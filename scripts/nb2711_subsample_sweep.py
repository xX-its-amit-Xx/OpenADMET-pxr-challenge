"""nb2711 -- LightGBM subsample (row) sweep on K=20 chemprop_aux residual.

NEW PARADIGM:
    Row subsampling per tree as regularization (vs colsample which is per
    feature).  `subsample` (a.k.a. `bagging_fraction`) controls the fraction
    of TRAINING ROWS randomly sampled (without replacement) for each tree
    when `subsample_freq > 0`.

    Note: at n_train ~ 202 (4/5 of 253), subsample=0.5 -> ~101 rows/tree,
    0.7 -> ~141, 1.0 -> all rows.  Row bagging decorrelates trees via row
    diversity, whereas colsample (nb2702) decorrelates via feature diversity.
    The two regularizers have different bias-variance trade-offs and are
    NOT redundant on the K=20 (small-feature, small-sample) regime.

    CRITICAL: `subsample_freq` must be > 0 for `subsample` to take effect
    in LightGBM.  We fix `subsample_freq=5` (resample every 5 trees) per
    the user spec.
...
PROTOCOL:
    ...
    2. Reuse K=20 RFE-surviving features (nb2231 snapshot K=20) over the
       ...
       / Avalon + ChEMBL kNN).
    3. For each subsample in {0.5, 0.6, 0.7, 0.8, 0.9, 1.0}:
       - 5-fold scaffold CV on 253 unblind, 5 kf_seeds {1001..1005}, fitting
         LGBM(MSE) with `subsample=X, subsample_freq=5` on the residual.
         Mean-bag over 5 internal seeds within each fold to denoise the row
         bagging draw.
       - Mean across kf_seeds.
    4. Save mean-bag pred_oof and te for the BEST subsample (lowest mean_rae).
...
OUTPUTS:
    scripts/nb2711_subsample_sweep.py
    data/processed/nb2711_summary.json
    data/processed/nb2711_pred_oof.npy      (253,) float32  -- best subsample
    data/processed/te_nb2711.npy            (513,) float32  -- best subsample
"""
...
# -------- subsample (row) sweep --------
SUBSAMPLE_GRID = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
SUBSAMPLE_FREQ = 5  # resample every 5 trees -- required for subsample to take effect
...
# -------- reference --------
NB2240_REF_OOF_K20 = 0.4682  # nb2240 K=20 LGBM(MSE) mean-bag context (subsample=1.0)

# -------- feature build paths (copied from nb2702/nb2240/nb2571) --------
...
# =============================================================================
# LGBM parameters with row subsampling
# =============================================================================

def _lgbm_params(seed: int, subsample: float) -> dict:
    """K=20 baseline (max_depth=4, num_leaves=15, n_est=300, lr=0.03) plus
    row-fraction bagging per tree.

    Notes
    -----
    * LightGBM sklearn API: `subsample` is an alias for `bagging_fraction`.
      Both pick a random subset of TRAINING ROWS for each tree.
    * `subsample_freq` (alias `bagging_freq`) MUST be > 0 for `subsample` to
      take effect; otherwise LightGBM ignores it.  We fix freq=5 (resample
      every 5 trees) per the user spec.
    * `bagging_seed` is derived from the model's `random_state`, so bagging
      across `RESID_SEEDS` re-shuffles the per-tree row draws, decorrelating
      the bag.
    """
    return dict(
...
# =============================================================================
# Helpers (copied from nb2702/nb2571 -- feature build + ChEMBL pool)
# =============================================================================
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM subsample (row) sweep on K=20 chemprop_aux residual")
    print(f"          subsample grid   = {SUBSAMPLE_GRID}")
    print(f"          subsample_freq   = {SUBSAMPLE_FREQ}")
    ...
    # --- rebuild 117-col 5-way feature matrix exactly as nb2240/nb2702/nb2571 ---
    ...
    # =========================================================================
    # subsample sweep: for each subsample, 5-fold scaffold CV across 5 kf_seeds
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"SUBSAMPLE SWEEP  kf_seeds={KF_SEEDS}  bag_seeds={RESID_SEEDS}  "
          f"subsample_freq={SUBSAMPLE_FREQ}")
    print(f"target: residual = y_unb - chemprop_aux  (LGBM MSE, K=20)")
    print("-" * 78)

    per_sub_records = []
    per_sub_oof = {}  # subsample -> mean-of-seeds OOF (corrected predictions)
    best_sub = None
    best_mean_rae = float("inf")

    for sub in SUBSAMPLE_GRID:
        ts = time.time()
        # n_train_per_fold ~ 0.8 * 253 = 202; approx rows/tree under subsample
        approx_rows = max(1, int(round(0.8 * n_unb * sub)))
        print(f"\n[subsample = {sub}  (~{approx_rows} train rows/tree at n_train~{int(0.8 * n_unb)})]")
        per_seed = []
        all_oofs = []
        for kf_seed in KF_SEEDS:
            ...
            for tr_loc, va_loc in splits:
                ...
                for bs in RESID_SEEDS:
                    mdl = lgb.LGBMRegressor(**_lgbm_params(bs, sub))
        ...
        rec = {
            "subsample": float(sub),
            "subsample_freq": int(SUBSAMPLE_FREQ),
            "approx_rows_per_tree": int(approx_rows),
        ...
        per_sub_records.append(rec)
        print(f"   subsample={sub}  mean_rae={mean_rae:.4f} (+/- {std_rae:.4f})  "
    ...
    print("\n" + "=" * 78)
    print(f"BEST subsample = {best_sub}  mean_rae = {best_mean_rae:.4f}")
    print("=" * 78)
    ...
    # =========================================================================
    # Deploy for BEST subsample: refit on all 253, mean-bag predict on 513 test
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"DEPLOY (best subsample = {best_sub})  refit on 253, mean-bag predict 513")
    print("-" * 78)
    te_bag_resid = np.zeros(n_test, dtype=np.float64)
    for bs in RESID_SEEDS:
        mdl = lgb.LGBMRegressor(**_lgbm_params(bs, best_sub))
    ...
    # --- save artifacts (mean-bag pred_oof for best subsample + deploy te) ---
    pred_oof_best = per_sub_oof[best_sub].astype(np.float32)
    ...
    summary = {
        "tag": TAG,
        "method": (
            "subsample_row_sweep_LGBM_MSE_K20_chemprop_aux_residual_"
            "scaffold_CV_5seed_mean_bag"
        ),
        "paradigm": "row_subsampling_per_tree_regularization",
        ...
        "per_subsample_records": per_sub_records,
    ...
    print(f"   subsample grid       = {SUBSAMPLE_GRID}")
    print(f"   subsample_freq       = {SUBSAMPLE_FREQ}")
    for rec in per_sub_records:
        print(f"     sub={rec['subsample']:.1f}  "
              f"mean_rae={rec['mean_rae']:.4f}  (+/- {rec['std_rae']:.4f})")
    print(f"   BEST subsample       = {best_sub}")
