"""nb2121 -- LGBM(MSE) reg_lambda grid {1, 2, 3, 4, 5} at fixed K=28 (top-28 SHAP).

HYPOTHESIS:
    nb2103 K-fine sweep at reg_lambda=2 found K=28 to be the optimum
    SHAP-pruning level (mean-bag RAE = 0.4737). This notebook holds K=28
    fixed and sweeps the L2 regularization term reg_lambda over
    {1, 2, 3, 4, 5} to ask whether a different lambda extracts a further
    RAE drop on top of the top-28 SHAP feature slice.

    Same LGBM(MSE) hyperparams as nb2103 (max_depth=4, num_leaves=15,
    n_estimators=300, lr=0.03, min_child_samples=5), 5-seed bag
    (seeds 0, 1, 7, 42, 137), 5-fold cross-fit per seed. Only reg_lambda
    varies.

PROTOCOL:
    1. Reuse SHAP importance ranking from nb2063
       (data/processed/nb2063_shap_importance_full117.npy) and slice top-28.
    2. Rebuild the same 117-col 5-way K-tuned feature matrix as
       nb2063/nb2103, using the same K-grid winners
       (nb1352/1392/1484/1523/1524/1541) and ChEMBL kNN.
    3. For each reg_lambda in {1, 2, 3, 4, 5}: slice X_unb to top-28 cols,
       run 5-seed bag (seeds 0, 1, 7, 42, 137) of LGBM(MSE) with
       KFold(n=5, shuffle=True) cross-fit per seed.
    4. Report per-lambda: per-seed RAE list, per-seed mean/median/std,
       mean-bag RAE, median-bag RAE.
    5. Verdict vs nb2103 K=28 / reg_lambda=2 (0.4737) at
       decision_margin = 0.003.

Outputs:
    scripts/nb2121_lambda_grid_K28.py
    data/processed/nb2121_summary.json
    data/processed/nb2121_mean_bag_oof_lam{lam}.npy  (253,) float32 per lambda
"""
...
NB2103_K28_LAM2_REF = 0.4737   # nb2103 mean-bag RAE at K=28, reg_lambda=2
DECISION_MARGIN = 0.003
...
def _lgbm_params(seed: int, reg_lambda: float) -> dict:
    """LGBM(MSE) -- identical to nb2103 except reg_lambda is parameterized."""
    return dict(
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, reg_lambda: float) -> np.ndarray:
    n = len(residual)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) reg_lambda grid {LAMBDA_GRID} at fixed K={K_FIXED}")
    print(f"          anchor={ANCHOR}  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          ref: nb2103 K={K_FIXED} lam=2 mean-bag RAE = "
          f"{NB2103_K28_LAM2_REF:.4f}  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- nb2103 K=28 / lam=2 reference (BASELINE) ----
    nb2103_k28_mean_bag = NB2103_K28_LAM2_REF
    nb2103_k28_median_bag = NB2103_K28_LAM2_REF
    if NB2103_SUMMARY.exists():
        ...
        for r in nb2103_sum.get("per_K_records", []):
            if int(r.get("K", -1)) == K_FIXED:
                ...
                break
    print(f"[ref] nb2103.K={K_FIXED} lam=2 mean_bag_rae   = "
          f"{nb2103_k28_mean_bag:.4f}")
    print(f"[ref] nb2103.K={K_FIXED} lam=2 median_bag_rae = "
    ...
    if not np.array_equal(full_rank_order[:50], nb2063_top50_idx_cached):
        print("   [warn] full-rank top-50 != cached nb2063 top-50; "
              "will use full re-rank for top-K slice")
    else:
    ...
    # ---- Lambda-grid sweep ----
    print("\n" + "-" * 78)
    print(f"LAMBDA-GRID SWEEP at fixed K={K_FIXED}: {LAMBDA_GRID}")
    print("-" * 78)
    per_lam_results: list[dict] = []
    for lam in LAMBDA_GRID:
        ...
        for i, s in enumerate(RESID_SEEDS):
            ...
            print(f"   lam={lam} seed={s:3d}:  rae_corr = {rae_s:.4f}  "
        ...
        print(f"   lam={lam} per-seed RAE   = "
              f"[{', '.join(f'{r:.4f}' for r in per_seed_rae)}]")
        print(f"   lam={lam} per-seed mean  = {rae_per_seed_mean:.4f}  "
              f"std = {rae_per_seed_std:.4f}")
        print(f"   lam={lam} pooled mean    = {rae_mean_bag:.4f}  "
              f"(d_vs_anchor = {rae_mean_bag - rae_anchor:+.4f}  "
              f"d_vs_nb2103_lam2 = {delta_vs_nb2103:+.4f})")
        print(f"   lam={lam} pooled median  = {rae_median_bag:.4f}")
        if beats_nb2103:
            verdict_lam = "BEATS_NB2103_K28_LAM2"
        elif flat_vs_nb2103:
            verdict_lam = "FLAT_VS_NB2103_K28_LAM2"
        elif beats_anchor:
            verdict_lam = "BEATS_ANCHOR_BUT_WORSE_THAN_NB2103_K28_LAM2"
        elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
            verdict_lam = "FLAT_VS_ANCHOR"
        else:
            verdict_lam = "HURTS_ANCHOR"
        print(f"   lam={lam} verdict        = {verdict_lam}")

        # Save mean-bag OOF per lambda
        out_p = DATA_PROCESSED / f"{TAG}_mean_bag_oof_lam{lam}.npy"
        np.save(out_p, mean_bag_oof.astype(np.float32))
        print(f"   [save] {out_p}")

        per_lam_results.append({
            ...
            "delta_mean_bag_vs_nb2103_K28_lam2": delta_vs_nb2103,
            "beats_chemprop_aux": bool(beats_anchor),
            "beats_nb2103_K28_lam2": bool(beats_nb2103),
            "flat_vs_nb2103_K28_lam2": bool(flat_vs_nb2103),
            "verdict": verdict_lam,
        })

    # ---- Select best lambda ----
    print("\n" + "=" * 78)
    print(f"LAMBDA-GRID SUMMARY TABLE at K={K_FIXED}")
    print("=" * 78)
    print(f"   {'lam':>4s}  {'mean_bag':>10s}  {'median_bag':>10s}  "
          f"{'per_seed_mean':>13s}  {'per_seed_std':>12s}  "
          f"{'d_vs_anchor':>11s}  {'d_vs_nb2103':>12s}  verdict")
    # nb2103 K=28 lam=2 baseline row (asterisk marks the reference fit)
    print(f"   {2:>4d}* {nb2103_k28_mean_bag:>10.4f}  "
          f"{nb2103_k28_median_bag:>10.4f}  {'N/A':>13s}  {'N/A':>12s}  "
          f"{nb2103_k28_mean_bag - rae_anchor:>+11.4f}  {0.0:>+12.4f}  "
          f"BASELINE(nb2103_K28_lam2)")
    for r in per_lam_results:
        print(f"   {int(r['reg_lambda']):>4d}  {r['rae_mean_bag']:>10.4f}  "
              f"{r['rae_median_bag']:>10.4f}  {r['rae_per_seed_mean']:>13.4f}  "
              f"{r['rae_per_seed_std']:>12.4f}  "
              f"{r['delta_mean_bag_vs_chemprop_aux']:>+11.4f}  "
              f"{r['delta_mean_bag_vs_nb2103_K28_lam2']:>+12.4f}  "
              f"{r['verdict']}")

    # best lambda by mean-bag RAE among the SWEEP
    sweep_rae = [r["rae_mean_bag"] for r in per_lam_results]
    best_sweep_i = int(np.argmin(sweep_rae))
    best_lam_sweep = float(per_lam_results[best_sweep_i]["reg_lambda"])
    best_lam_sweep_rae = float(per_lam_results[best_sweep_i]["rae_mean_bag"])

    print(f"\n   best lambda in sweep  = {best_lam_sweep:g}  "
          f"(mean_bag RAE {best_lam_sweep_rae:.4f})")
    print(f"   delta best vs nb2103 K=28 lam=2 = "
          f"{best_lam_sweep_rae - nb2103_k28_mean_bag:+.4f}")

    # ---- Overall verdict ----
    if best_lam_sweep_rae < nb2103_k28_mean_bag - DECISION_MARGIN:
        global_verdict = (
            f"LAMBDA_GRID_BEATS_NB2103_K28_LAM2_AT_LAM={best_lam_sweep:g}"
        )
    elif abs(best_lam_sweep_rae - nb2103_k28_mean_bag) < DECISION_MARGIN:
        global_verdict = (
            f"LAMBDA_GRID_FLAT_VS_NB2103_K28_LAM2_BEST_LAM={best_lam_sweep:g}"
        )
    else:
        global_verdict = "LAMBDA_GRID_DOES_NOT_BEAT_NB2103_K28_LAM2"
    ...
    summary = {
        "tag": TAG,
        "method": (f"lgbm_mse_reg_lambda_grid_1_2_3_4_5_at_K={K_FIXED}"
                   "_on_117col_top28_SHAP"),
        "anchor": ANCHOR,
        ...
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        ...
                        "Avalon + ChEMBL kNN); top-28 SHAP slice"),
        ...
        "nb2103_K28_lam2_mean_bag_ref": nb2103_k28_mean_bag,
        "nb2103_K28_lam2_median_bag_ref": nb2103_k28_median_bag,
        "per_lambda_records": per_lam_results,
        "best_lam_sweep": best_lam_sweep,
        "best_lam_sweep_rae_mean_bag": best_lam_sweep_rae,
        "delta_best_lam_vs_nb2103_K28_lam2": (
            best_lam_sweep_rae - nb2103_k28_mean_bag
        ),
        "verdict": global_verdict,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb2103_K28_lam2_ref": NB2103_K28_LAM2_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb2103_K28_lam2_mean_bag_ref",
        "best_lam_sweep", "best_lam_sweep_rae_mean_bag",
        "delta_best_lam_vs_nb2103_K28_lam2",
    ...
    for r in res["per_lambda_records"]:
        print(f"  lam={int(r['reg_lambda']):>3d}  "
              ...
              f"d_vs_nb2103={r['delta_mean_bag_vs_nb2103_K28_lam2']:+.4f}  "
              f"{r['verdict']}")
