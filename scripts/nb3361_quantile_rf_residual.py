"""nb3361 -- Quantile RF (median) on (y - nb3200) residual, 117-col K=20 slice.

NEW PARADIGM (cycle-221 quantile-forest, numpy 2.x compatible):
    `quantile-forest`'s RandomForestQuantileRegressor implements Meinshausen
    2006: fit ONE forest under squared loss, cache each leaf's training-row
    labels, and at predict time recover the conditional residual|x distribution
    by aggregating training labels from the leaves a test row lands in.  Reading
    off the 0.5 quantile (conditional MEDIAN) makes the residual estimate ROBUST
    TO OUTLIERS -- the median of the leaf-label distribution is insensitive to
    the heavy two-sided tails (novel-scaffold over/under-prediction) that an
    L2-mean residual learner chases.

    Contrast with nb3342 (ExtraTrees, randomized-split MEAN on same K=20 residual,
    FAIL) and nb3262 (LGBM, greedy-split L2 MEAN, FAIL): both fit the conditional
    MEAN of the residual.  nb3361 holds SUBSTRATE fixed (same 117-col K=20 slice,
    same nb3200 anchor) and swaps the ESTIMAND from conditional-mean to
    conditional-median.  This probes whether the residual-on-residual failures
    were driven by the mean-estimator's outlier sensitivity rather than the
    feature capacity.
...
PROTOCOL (per kf_seed, 5-fold scaffold split on 253 unblind):
    ...
    For each fold:
        mdl = RandomForestQuantileRegressor(n_estimators=300, max_depth=6,
                                            min_samples_leaf=2, random_state=kf_seed)
        mdl.fit(X_unb_K20[tr_loc], residual[tr_loc])
        resid_hat = mdl.predict(X_unb_K20[va_loc], quantiles=0.5)   # MEDIAN
        oof[va_loc] = clip(nb3200_oof[va_loc] + resid_hat, CLIP_LO, CLIP_HI)
    pooled = rae(y_unb, oof);  per_fold_mean = mean(per-fold val RAE)
    Repeat for 15 FRESH kf_seeds {1216..1230}.

DEPLOY:
    Refit QF on (X_unb_K20, residual) once per kf_seed; mean-bag MEDIAN
    prediction on X_te_K20; te_final = clip(te_nb3200 + mean-bag median resid,
    CLIP_LO, CLIP_HI).
...
GATE (on 15-seed per-fold-mean):
    ...
    If `import quantile_forest` fails, save INSTALL_FAILED summary and exit
    clean (no NPY artefacts written).
...
Outputs:
    data/processed/nb3361_summary.json
    data/processed/nb3361_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3361.npy         (513,) float32 -- deploy te
    submissions/nb3361_quantile_rf_residual.csv  (only on BETTER)
"""
...
# quantile-forest import is wrapped so a missing/uninstallable package exits
# clean with verdict INSTALL_FAILED rather than crashing the cron.
QF_IMPORT_OK = True
QF_IMPORT_ERR = None
...
# -- 117-col feature recipe (identical to nb3342 / nb3262 / nb3163 / nb2960) --
...
# -- Quantile RF residual learner params (per task spec) ---------------------
QF_N_ESTIMATORS = 300
QF_MAX_DEPTH = 6
QF_MIN_SAMPLES_LEAF = 2
QF_QUANTILE = 0.5  # conditional MEDIAN (robust to outliers)

# -- Output range clip (matches nb3342 / nb3070 / nb3263 / nb3314 deploy) -----
CLIP_LO = 3.0
CLIP_HI = 9.0
...
REF_NB3262 = 0.4424     # LGBM(K=20) on same 117col residual (FAIL, mean estimand)
REF_NB3313 = 0.4424     # LGBM(Avalon-512) on residual (FAIL, orthogonal subst.)
REF_NB3342 = 0.4424     # ExtraTrees(K=20) on residual (FAIL, randomized MEAN)
...
# ============================================================================
# helpers (lifted verbatim from nb3342 117-col feature builder)
# ============================================================================
...
def _new_qf(seed: int):
    """Quantile RF residual learner -- per task spec.

    n_estimators=300, max_depth=6, min_samples_leaf=2. ONE forest fit under L2;
    the 0.5 quantile of the cached leaf-label distribution gives a conditional
    MEDIAN residual that is robust to the heavy two-sided outlier tails an L2
    MEAN learner (nb3262 LGBM, nb3342 ExtraTrees) chases.
    """
...
def _qf_predict_median(mdl, X):
    """Predict the 0.5 quantile (median) of the conditional residual|x dist.

    `RandomForestQuantileRegressor.predict(X, quantiles=0.5)` returns a 1-D
    array of medians (one per row). Coerce to float64 and flatten defensively
    in case a future minor-release returns (N, 1).
    """
...
def _save_install_failed_summary(reason: str, t0: float) -> dict:
    ...
    summary = {
        ...
        "method": (
            "quantile_rf_median_on_y_minus_nb3200_residual_chemprop_aux_117col_"
            "K20_features_plus_clip_honest_5fold_scaffold_cv_15_fresh_seeds"
        ),
        "paradigm": (
            "quantile_forest_conditional_MEDIAN_robust_to_outliers_vs_"
            "L2_mean_residual_nb3262_nb3342"
        ),
        "install_failed": True,
...
def build_117col_feature_matrix(te_smiles, n_test):
    """117-col matrix identical to nb3342 / nb3262 / nb3163 / nb2604 / nb2960."""
...
# ============================================================================
# core honest CV: Quantile RF(K=20) MEDIAN on (y - nb3200) residual per kf_seed
# ============================================================================

def _run_one_seed(
    X_unb_K20, anchor_oof, residual, y_unb, unb_scaffolds, kf_seed, n_folds,
):
    """One kf_seed honest n-fold scaffold CV; QF(seed=kf_seed) MEDIAN on resid."""
    ...
    for fi, (tr_loc, va_loc) in enumerate(splits):
        mdl = _new_qf(seed=kf_seed)
        mdl.fit(X_unb_K20[tr_loc], residual[tr_loc])
        resid_pred_va = _qf_predict_median(mdl, X_unb_K20[va_loc])
        oof[va_loc] = np.clip(
            anchor_oof[va_loc] + resid_pred_va, CLIP_LO, CLIP_HI
        )
        per_fold_val_rae.append(float(rae(y_unb[va_loc], oof[va_loc])))
        resid_pred_tr = _qf_predict_median(mdl, X_unb_K20[tr_loc])
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Quantile RF (MEDIAN) on (y - nb3200) residual, 117col K=20")
    print(f"          parent     : {PARENT_TAG} (deep-30 mean {REF_NB3200:.4f})")
    print(f"          features   : 117-col 5-way -> K=20 (nb2280_RFE; same as nb3342)")
    print(f"          learner    : RandomForestQuantileRegressor(n_est="
          f"{QF_N_ESTIMATORS}, max_depth={QF_MAX_DEPTH}, "
          f"min_leaf={QF_MIN_SAMPLES_LEAF}, q={QF_QUANTILE})")
    print(f"          paradigm   : conditional MEDIAN (robust to outliers vs "
          f"L2 mean nb3262/nb3342)")
    ...
    if not QF_IMPORT_OK:
        ...
        print("[FATAL] `pip install quantile-forest` did not yield an importable "
              "module on this python/numpy combo. Exiting clean INSTALL_FAILED.")
    ...
    # -- Deploy: refit per-kf_seed on ALL 253, mean-bag MEDIAN predict on te --
    print("\n" + "-" * 78)
    print(f"STEP 3: deploy refit on all 253 unblind, mean-bag {len(KF_SEEDS)}"
          f"-seed QF MEDIAN, te = clip(nb3200_te + resid_median_te)")
    print("-" * 78)
    sum_te_resid = np.zeros(n_test, dtype=np.float64)
    for s in KF_SEEDS:
        mdl = _new_qf(seed=s)
        mdl.fit(X_unb_K20, residual)
        sum_te_resid += _qf_predict_median(mdl, X_te_K20)
    ...
    if mean_pf < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3361 15-seed per-fold-mean {mean_pf:.4f} "
            f"beats BETTER gate {GATE_BETTER:.4f} ({mean_pf - GATE_BETTER:+.4f}) "
            f"and nb3200 deep-30 {REF_NB3200:.4f} ({mean_pf - REF_NB3200:+.4f}). "
            f"Quantile-RF conditional-MEDIAN residual on the SAME 117-col K=20 "
            f"slice that L2-MEAN learners (LGBM nb3262, ExtraTrees nb3342) failed "
            f"on extracts residual structure the outlier-sensitive mean missed -- "
            f"the residual-on-residual failure was the ESTIMAND (mean vs median), "
            f"not the feature capacity. PRE-clean anchor chain (nb3090 -> nb3200). "
        ...
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3361 15-seed per-fold-mean {mean_pf:.4f} fails BETTER "
            f"gate {GATE_BETTER:.4f} ({mean_pf - GATE_BETTER:+.4f}). Delta vs "
            f"nb3200 (deep-30 mean {REF_NB3200:.4f}) = {mean_pf - REF_NB3200:+.4f}. "
            f"Quantile-RF conditional-MEDIAN on the 117-col K=20 residual finds no "
            f"new structure on top of the learned-clip chemprop_aux stack -- "
            f"swapping the ESTIMAND from L2-mean (nb3262 LGBM, nb3342 ExtraTrees) "
            f"to robust median does NOT help, confirming the bottleneck is the "
            f"anchor's n=253 capacity on already-absorbed K=20 columns, NOT "
            f"outlier sensitivity of the residual estimator. Residual-on-residual "
            f"on the nb3200 anchor remains closed across LGBM-mean (nb3262), "
            f"ExtraTrees-mean (nb3342), and QuantileRF-median (nb3361). Substrate "
            f"change must come from a DIFFERENT ANCHOR axis."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_quantile_rf_residual.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        "tag": TAG,
        "parent_tag": PARENT_TAG,
        "method": ("quantile_rf_median_on_y_minus_nb3200_residual_with_chemprop_aux_"
                   "117col_K20_features_plus_clip_honest_5fold_scaffold_cv_15_fresh_seeds"),
        "paradigm": ("quantile_forest_conditional_MEDIAN_robust_to_outliers_vs_"
                     "L2_mean_residual_nb3262_lgbm_nb3342_extratrees"),
