"""nb3342 -- ExtraTrees on (y - nb3200) residual using the 117-col K=20 slice.

NEW PARADIGM (different inductive bias, NOT a substrate change):
    nb3262 trained an LGBM(K=20) on the (y - nb3200) residual using the
    117-col 5-way chemprop_aux feature matrix sliced to the K=20 RFE survivors
    (FAIL: the gradient-boosted axis-aligned splits found nothing the
    chemprop_aux -> clip stack hadn't already absorbed).  nb3313 swapped the
    SUBSTRATE to Avalon-512 bits with another LGBM (also FAIL).

    nb3342 holds the SUBSTRATE fixed (same 117-col K=20 slice as nb3262) but
    swaps the LEARNER class: ExtraTreesRegressor.  ExtraTrees draws split
    thresholds at RANDOM (extremely randomized trees) rather than greedily
    optimizing impurity gain like LGBM/RandomForest.  The hypothesis: the
    randomized-split bias smooths the residual fit differently than LGBM's
    greedy gain-driven boundaries, so it may capture residual structure on the
    same K=20 features that LGBM's exact-split inductive bias keeps missing.
    This probes whether the nb3262 failure was the FEATURE capacity (n=253 on
    K=20 already-absorbed columns) or the LEARNER's split geometry.
...
PROTOCOL (per kf_seed, 5-fold scaffold split on 253 unblind):
    ...
    For each fold:
        mdl = ExtraTreesRegressor(n_estimators=500, max_depth=6,
                                  min_samples_leaf=3, random_state=kf_seed)
...
DEPLOY:
    Refit ExtraTrees on (X_unb_K20, residual) once per kf_seed; mean-bag
    prediction on X_te_K20; te_final = clip(te_nb3200 + mean-bag resid,
    CLIP_LO, CLIP_HI).
...
Outputs:
    data/processed/nb3342_summary.json
    data/processed/nb3342_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3342.npy         (513,) float32 -- deploy te
    submissions/nb3342_extra_tree_residual.csv  (only on BETTER)
"""
...
# -- ExtraTrees residual learner params (per task spec) ----------------------
ET_N_ESTIMATORS = 500
ET_MAX_DEPTH = 6
ET_MIN_SAMPLES_LEAF = 3

# -- Output range clip (matches nb3070 / nb3263 / nb3314 deploy stage) -------
CLIP_LO = 3.0
CLIP_HI = 9.0
...
REF_NB3262 = 0.4424     # LGBM(K=20) on same 117col residual (FAIL, redundant)
...
def _et_params(seed):
    """ExtraTrees residual learner -- per task spec.

    n_estimators=500, max_depth=6, min_samples_leaf=3.  Extremely randomized
    splits (random thresholds) give a DIFFERENT inductive bias than the
    greedy gain-driven splits LGBM (nb3262) used on the SAME K=20 features.
    """
    return dict(
...
# ============================================================================
# core honest CV: ExtraTrees(K=20) on (y - nb3200) residual per kf_seed
# ============================================================================

def _run_one_seed(
    X_unb_K20, anchor_oof, residual, y_unb, unb_scaffolds, kf_seed, n_folds,
):
    """One kf_seed honest n-fold scaffold CV; ExtraTrees(seed=kf_seed) on resid."""
    ...
    for fi, (tr_loc, va_loc) in enumerate(splits):
        mdl = ExtraTreesRegressor(**_et_params(kf_seed))
        mdl.fit(X_unb_K20[tr_loc], residual[tr_loc])
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- ExtraTrees on (y - nb3200) residual, 117-col K=20 slice")
    print(f"          parent     : {PARENT_TAG} (deep-30 mean {REF_NB3200:.4f})")
    print(f"          features   : 117-col 5-way -> K=20 (nb2280_RFE; same as nb3262)")
    print(f"          learner    : ExtraTrees(n_est={ET_N_ESTIMATORS}, "
          f"max_depth={ET_MAX_DEPTH}, min_leaf={ET_MIN_SAMPLES_LEAF})")
    print(f"          paradigm   : randomized splits (DIFFERENT bias than nb3262 LGBM)")
    ...
    print(f"   ref nb3313 (LGBM Avalon)  = {REF_NB3313:.4f}")
    ...
    print(f"STEP 3: deploy refit on all 253 unblind, mean-bag {len(KF_SEEDS)}"
          f"-seed ExtraTrees, te = clip(nb3200_te + resid_pred_te)")
    print("-" * 78)
    sum_te_resid = np.zeros(n_test, dtype=np.float64)
    for s in KF_SEEDS:
        mdl = ExtraTreesRegressor(**_et_params(s))
    ...
    if mean_pf < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3342 15-seed per-fold-mean "
            ...
            f"ExtraTrees randomized-split bias on (y - nb3200) residual using "
            f"the SAME 117-col K=20 slice that LGBM (nb3262) failed on extracts "
            f"residual structure the greedy gain-driven splits missed -- the "
            f"nb3262 failure was the LEARNER's split geometry, not the feature "
            f"capacity. PRE-clean anchor chain (nb3090 -> nb3200). Re-verify "
            f"with deep-30 (kf_seeds 30+) before any PRIMARY-1 swap; same "
            f"under-dispersion-risk root as cycle-160."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3342 15-seed per-fold-mean {mean_pf:.4f} fails "
            f"BETTER gate {GATE_BETTER:.4f} ({mean_pf - GATE_BETTER:+.4f}). "
            f"Delta vs nb3200 (deep-30 mean {REF_NB3200:.4f}) = "
            f"{mean_pf - REF_NB3200:+.4f}. ExtraTrees randomized-split bias on "
            f"(y - nb3200) residual using the 117-col K=20 slice finds no new "
            f"structure on top of the learned-clip chemprop_aux stack -- "
            f"swapping the LEARNER class (greedy LGBM nb3262 -> randomized "
            f"ExtraTrees) does NOT help, confirming the bottleneck is the "
            f"anchor's n=253 capacity on already-absorbed K=20 columns, NOT the "
            f"split geometry. Residual-on-residual on the nb3200 anchor remains "
            f"closed across LGBM (nb3262), Avalon-LGBM (nb3313), and ExtraTrees "
            f"(nb3342). Substrate change must come from a DIFFERENT ANCHOR axis."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_extra_tree_residual.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": ("extratrees_on_y_minus_nb3200_residual_with_chemprop_aux_117col_"
                   "K20_features_plus_clip_honest_5fold_scaffold_cv_15_fresh_seeds"),
        "paradigm": "extratrees_randomized_splits_different_bias_than_lgbm_nb3262",
