"""nb1632 -- Outer-bag VALIDATION of nb1622 (best grid w=0.55 blend).

PROTOCOL
    1. For each outer seed o in {0, 1, 7, 42, 137}: rebuild
    ...
    3. Row-level Bag-of-Bags MEAN + MEDIAN across the 5 blend_o vectors.
    4. Verdict NB1622_REPRODUCES iff per-outer mean within 0.003 of the
       nb1622 cross-fit RAE (0.5136).

OUTPUTS
    scripts/nb1632_bag_nb1622.py                  (this file)
    data/processed/nb1632_summary.json
    data/processed/nb1632_bob_mean_oof.npy        (253,) float32
    data/processed/nb1632_bob_median_oof.npy      (253,) float32
"""
...
# nb1622 cross-fit reference RAE
NB1622_REF = 0.5136
REPRODUCE_MARGIN = 0.003

# Outer bag definition (matches nb1561 / nb1623)
...
# ---- Feature caches (same as nb1561 / nb1612 / nb1623) ----
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG VALIDATION of nb1622 (best grid w=0.55 blend)")
    ...
    print(f"         nb1622_ref       = {NB1622_REF:.4f}  "
    ...
    for oi, o in enumerate(OUTER_SEEDS):
        ...
        delta_blend_vs_ref = rae_blend_o - NB1622_REF
        print(f"   [blend_o]   {W_NB1561:.2f}*nb1561_o + {W_NB1612:.2f}*nb1612_o  "
        ...
        per_outer_records.append({
            ...
            "delta_blend_vs_nb1622_ref": delta_blend_vs_ref,
    ...
    # ---- BoB MEAN + MEDIAN over the 5 blend_o vectors ----
    ...
    # ---- Verdict ----
    delta_per_outer = per_outer_mean - NB1622_REF
    delta_bob_mean = rae_bob_mean - NB1622_REF
    delta_bob_median = rae_bob_median - NB1622_REF
    reproduces = abs(delta_per_outer) < REPRODUCE_MARGIN
    if reproduces:
        verdict = "NB1622_REPRODUCES"
    elif per_outer_mean < NB1622_REF - REPRODUCE_MARGIN:
        verdict = "NB1622_OPTIMISTIC_OUTER_BAG_BETTER"
    else:
        verdict = "NB1622_LUCKY_SEED_OUTER_BAG_WORSE"
    ...
    summary = {
        ...
        "nb1622_ref": NB1622_REF,
        "reproduce_margin": REPRODUCE_MARGIN,
        "delta_per_outer_mean_vs_nb1622_ref": delta_per_outer,
        "delta_bob_mean_vs_nb1622_ref": delta_bob_mean,
        "delta_bob_median_vs_nb1622_ref": delta_bob_median,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_per_outer_mean_vs_nb1622_ref",
        "delta_bob_mean_vs_nb1622_ref",
        "delta_bob_median_vs_nb1622_ref",
        "reproduces",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
