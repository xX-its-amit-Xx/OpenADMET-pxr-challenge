"""nb1570 -- DEPLOY nb1561 outer-bag CatBoost 5-way K-tuned 117-col -> 513.

HANDOFF FROM nb1561 (BoB MEAN 0.5155 / MEDIAN 0.5156 on 253 PRE-unblind):
    For each of 5 outer seeds o in {0,1,7,42,137}:
        inner_seeds = [o*1000 + s for s in [0,1,7,42,137]]
        For each inner seed s' in inner_seeds:
            CatBoost(MAE, depth=4, n_est=200, lr=0.05, l2=5, random_seed=s')
            Fit on ALL 253 unblind, predict residual on 513.
        per-outer deploy = mean of 5 inner deploys -> (513,)
    Stack 5 per-outer deploys -> (5, 513).
    Row-level BoB MEAN + MEDIAN across outer seeds.
    te_nb1570_mean   = te_chemprop_aux + BoB_mean_residual_513
    te_nb1570_median = te_chemprop_aux + BoB_median_residual_513

Features: 117-col 5-way K-tuned
...
Honest LB anchors: 0.5155 mean / 0.5156 median (PRE-unblind, predicted LB ~0.519).

Outputs:
    data/processed/te_nb1570_mean.npy             (513,) float32
    data/processed/te_nb1570_median.npy           (513,) float32
    data/processed/nb1570_summary.json
    submissions/nb1570_deploy_nb1561_mean.csv     SMILES, Molecule Name, pEC50
    submissions/nb1570_deploy_nb1561_median.csv   SMILES, Molecule Name, pEC50
"""
...
HONEST_LB_ANCHOR_MEAN = 0.5155
HONEST_LB_ANCHOR_MEDIAN = 0.5156
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1561 outer-bag CatBoost 5-way K-tuned -> 513")
    ...
    # ---- Family top-K indices (same as nb1561 / nb1554) ----
    ...
    # ---- Outer-bag deploy ----
    print("\n" + "=" * 78)
    print("OUTER-BAG DEPLOY x 5 inner-seed CatBoost(MAE,d4,n200,lr0.05,l2=5)")
    print("=" * 78)
    ...
    # ---- BoB row-level aggregations across 5 per-outer 513 vectors ----
    bob_mean_resid_513 = outer_resid_513.mean(axis=0)
    bob_median_resid_513 = np.median(outer_resid_513, axis=0)

    te_nb1570_mean = te_anchor_513 + bob_mean_resid_513
    te_nb1570_median = te_anchor_513 + bob_median_resid_513

    in_rae_mean = float(rae(y_unb, te_nb1570_mean[unb_idx]))
    in_rae_median = float(rae(y_unb, te_nb1570_median[unb_idx]))

    print("\n" + "=" * 78)
    print("BoB DEPLOY SUMMARY")
    print("=" * 78)
    print(f"   te_nb1570_mean    mean={te_nb1570_mean.mean():.4f}  "
          f"std={te_nb1570_mean.std():.4f}  "
          f"min={te_nb1570_mean.min():.4f}  "
          f"max={te_nb1570_mean.max():.4f}")
    print(f"   te_nb1570_median  mean={te_nb1570_median.mean():.4f}  "
          f"std={te_nb1570_median.std():.4f}  "
          f"min={te_nb1570_median.min():.4f}  "
          f"max={te_nb1570_median.max():.4f}")
    ...
    np.save(te_mean_path, te_nb1570_mean.astype(np.float32))
    np.save(te_median_path, te_nb1570_median.astype(np.float32))
    print(f"\n[save] {te_mean_path}")
    print(f"[save] {te_median_path}")

    # ---- Save CSVs ----
    SUBMISSIONS.mkdir(parents=True, exist_ok=True)
    csv_mean_path = SUBMISSIONS / f"{TAG}_deploy_nb1561_mean.csv"
    csv_median_path = SUBMISSIONS / f"{TAG}_deploy_nb1561_median.csv"

    df_mean = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1570_mean.astype(np.float64),
    })
    df_median = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1570_median.astype(np.float64),
    })
    ...
    summary = {
        ...
        "te_nb1570_mean_stats": {
            "mean": float(te_nb1570_mean.mean()),
            "std": float(te_nb1570_mean.std()),
            "min": float(te_nb1570_mean.min()),
            "max": float(te_nb1570_mean.max()),
        },
        "te_nb1570_median_stats": {
            "mean": float(te_nb1570_median.mean()),
            "std": float(te_nb1570_median.std()),
            "min": float(te_nb1570_median.min()),
            "max": float(te_nb1570_median.max()),
        },
        ...
        "honest_lb_anchor_mean_nb1561": HONEST_LB_ANCHOR_MEAN,
        "honest_lb_anchor_median_nb1561": HONEST_LB_ANCHOR_MEDIAN,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "te_nb1570_mean_stats", "te_nb1570_median_stats",
        "in_rae_unb_mean", "in_rae_unb_median",
        "delta_mean_vs_anchor", "delta_median_vs_anchor",
        "honest_lb_anchor_mean_nb1561",
        "honest_lb_anchor_median_nb1561",
