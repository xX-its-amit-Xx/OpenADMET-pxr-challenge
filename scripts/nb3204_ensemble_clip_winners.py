"""nb3204 -- Equal-weight ensemble of 3 verified clip winners (nb3190, nb3173, nb3174).

NEW PARADIGM:
    Variance reduction by averaging 3 independent clip-winner predictions.
    No fitted parameters -- pure equal-weight (1/3) mean across pred_oof's.

CONTEXT:
    nb3173 15-seed mean = 0.4422 +/- 0.0010 (BETTER, learned-clip on parent A)
    nb3174 (uses nb3181 equivalent) 15-seed = 0.4433 (clip on alternates)
    nb3190 15-seed mean = 0.4426 +/- 0.0022 (MARGINAL, clip on nb3090)

    Pairwise correlations: > 0.999 across all three (very tight),
    so variance-reduction gain is bounded by sqrt(3) but tempered by
    near-perfect rank-correlation. Expected mean if no gain: ~0.4425;
    expected mean if VR works as on uncorrelated estimators: lower.

PROTOCOL:
    Load nb3190_pred_oof, nb3173_pred_oof, nb3174_pred_oof
    Equal-weight (1/3) mean -> single pred_oof_ens
    For each kf_seed in {1186..1200} (15 FRESH seeds):
        scaffold_kfold_indices(n_splits=5, seed=kf_seed)
        For each fold:
            pooled per-fold-val RAE for ensemble (no fit -- predictions fixed)
        Pool 5 folds -> pooled_rae (invariant to split, equals overall RAE)
...
References:
    nb3190 = 0.4426    nb3173 = 0.4422    nb3174 = 0.4433
...
Outputs:
    data/processed/nb3204_summary.json
    data/processed/nb3204_pred_oof.npy   (253,) float32 -- equal-weight mean
    data/processed/te_nb3204.npy         (513,) float32 -- equal-weight te mean
    submissions/nb3204_ensemble_clip_winners.csv  (only on BETTER)
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- EQUAL-WEIGHT ENSEMBLE of 3 clip winners ({', '.join(COMPONENTS)})")
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"BETTER. nb3204 equal-weight ensemble of (nb3190, nb3173, nb3174) "
            f"15-seed mean {mean_rae:.4f} +/- {std_rae:.4f} clears strict gate "
            f"{GATE_BETTER:.4f}. Variance-reduction over near-correlated "
            f"(>0.999) clip winners delivers delta {delta_vs_best_comp:+.4f} "
            f"vs best component and {delta_vs_mean_comp:+.4f} vs mean. "
            f"PROMOTE nb3204 to PRIMARY-1 candidate (displaces nb2171 0.4682 "
            f"by -{REF_NB2171 - mean_rae:.4f} RAE). Deep-30 verification "
            f"recommended before LB fire per cycle-160 rule (15-seed std "
            f"likely under-dispersed at this near-correlated ensemble level). "
            f"Watch dispersion against deep-30 in followup."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"FAIL. nb3204 equal-weight ensemble mean {mean_rae:.4f} fails "
            f"strict gate {GATE_BETTER:.4f}. Components correlate >0.999, so "
            f"variance-reduction gain bounded near zero -- delta vs best "
            f"component = {delta_vs_best_comp:+.4f}. Equal-weight averaging "
            f"of near-identical predictions cannot break the 0.4422 ceiling "
            f"of the best individual clip winner (nb3173). Keep nb2171 "
            f"PRIMARY-1; do not promote nb3204. Consider weighted blend "
            f"(SLSQP simplex) or adding a decorrelated anchor instead."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_ensemble_clip_winners.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "equal_weight_mean_of_3_clip_winners",
