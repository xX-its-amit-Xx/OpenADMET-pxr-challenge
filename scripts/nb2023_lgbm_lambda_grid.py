"""nb2023 -- LightGBM (MSE) reg_lambda grid search on 5-way K-tuned 117-col features.

HYPOTHESIS:
    Sweep reg_lambda in {0.5, 1, 3, 4, 5} (nb1852/nb1861 used reg_lambda=2.0) on
    the same 117-col 5-way K-tuned residual-LGBM stack to identify whether
    stronger or weaker L2 regularisation on leaf weights improves bag RAE.

    Configuration (mirrors nb1852/nb1861 except reg_lambda):
        objective='regression'        (MSE)
        max_depth=4, num_leaves=15
        n_estimators=300, lr=0.03
        min_child_samples=5
        reg_lambda in {0.5, 1.0, 3.0, 4.0, 5.0}

    Same 5-way K-tuned 117-col feature stack as nb1852/nb1861/nb2013:
        top-K AtomPair      (nb1524 best_K)
        top-K MACCS         (nb1352 standard)
        top-K Mordred       (nb1523 best_K)
        top-K ChempropEmbed (nb1541 best_K)
        top-K Avalon        (nb1392 SHAP K=30)
        pred_chembl_pec50
        mean_sim
...
PROTOCOL:
    ...
    2. Build 117-col feature stack (identical to nb2013).
    3. For each reg_lambda in grid:
       5-seed bag {0,1,2,3,4} LightGBM(MSE, depth=4, leaves=15, 300/0.03,
       min_child=5, lambda=X). KFold(n=5, shuffle=True, random_state=seed)
       cross-fit per seed.
       Mean-bag RAE per lambda.
    4. Report per-lambda RAE.

Outputs:
    scripts/nb2023_lgbm_lambda_grid.py
    data/processed/nb2023_summary.json
    data/processed/nb2023_mean_bag_oof_lambda<X>.npy   per lambda (253,) float32
"""
...
SEEDS = [0, 1, 2, 3, 4]
RESID_FOLDS = 5
LAMBDA_GRID = [0.5, 1.0, 3.0, 4.0, 5.0]
...
NB1852_REF = 0.5100   # reg_lambda=2.0 reference
NB1861_REF = 0.5013   # 5-outer-seed BoB of nb1852 MSE
...
def _lgbm_params(seed: int, reg_lambda: float) -> dict:
    """LGBM(MSE) -- same as nb1852/nb1861 with explicit reg_lambda."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) reg_lambda grid {LAMBDA_GRID} on 5-way "
          f"K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    ...
    # ---- Lambda grid loop ----
    print("\n" + "=" * 78)
    print(f"LAMBDA GRID SEARCH: {LAMBDA_GRID}")
    print(f"   5-seed bag {SEEDS}, {RESID_FOLDS}-fold cross-fit per seed "
          f"(dim={feat_dim})")
    print("=" * 78)
    ...
    for lam in LAMBDA_GRID:
        ...
        print(f"   per-seed RAE       = "
              f"[{', '.join(f'{r:.4f}' for r in per_seed_rae_lam)}]")
        print(f"   seed mean/std      = {seed_mean:.4f} / {seed_std:.4f}")
        ...
        print(f"   wall lambda={lam}  = {time.time() - t_lam:.1f}s")

        out_path = (DATA_PROCESSED /
                    f"{TAG}_mean_bag_oof_lambda{lam}.npy")
        np.save(out_path, mean_bag_oof.astype(np.float32))
        print(f"   [save] {out_path}")

        lambda_records.append({
            "reg_lambda": float(lam),
            "per_seed_rae": per_seed_rae_lam,
            "per_seed_records": per_seed_records_lam,
            ...
            "mean_bag_oof_path": str(out_path),
            "wall_sec": round(time.time() - t_lam, 2),
        })

    # ---- Summary table ----
    print("\n" + "=" * 78)
    print("LAMBDA GRID SUMMARY")
    print("=" * 78)
    print(f"   anchor (chemprop_aux) RAE = {rae_anchor:.4f}")
    print(f"   nb1852 ref (lambda=2.0)   = {NB1852_REF:.4f}")
    print(f"   nb1861 ref (5-out BoB)    = {NB1861_REF:.4f}")
    print()
    print(f"   {'lambda':>8} | {'mean_bag':>9} | {'median_bag':>11} | "
          f"{'seed_mean':>10} | {'seed_std':>9} | {'d_anchor':>9} | "
          f"{'d_nb1861':>9}")
    print(f"   {'-'*8}-+-{'-'*9}-+-{'-'*11}-+-{'-'*10}-+-{'-'*9}-+-"
          f"{'-'*9}-+-{'-'*9}")
    for r in lambda_records:
        print(f"   {r['reg_lambda']:>8.2f} | "
              f"{r['rae_mean_bag']:>9.4f} | "
              f"{r['rae_median_bag']:>11.4f} | "
              f"{r['seed_mean_rae']:>10.4f} | "
              f"{r['seed_std_rae']:>9.4f} | "
              f"{r['delta_mean_bag_vs_anchor']:>+9.4f} | "
              f"{r['delta_mean_bag_vs_nb1861']:>+9.4f}")

    # ---- Best lambda ----
    best_rec = min(lambda_records, key=lambda r: r["rae_mean_bag"])
    best_lambda = best_rec["reg_lambda"]
    best_rae = best_rec["rae_mean_bag"]
    print(f"\n   BEST lambda  = {best_lambda:.2f}  (mean_bag RAE = {best_rae:.4f})")
    delta_vs_nb1861 = best_rae - NB1861_REF
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "LAMBDA_GRID_IMPROVES_OVER_NB1861"
    elif delta_vs_nb1861 > DECISION_MARGIN:
        verdict = "LAMBDA_GRID_HURTS_VS_NB1861"
    else:
        verdict = "LAMBDA_GRID_NEUTRAL_VS_NB1861"
    ...
    summary = {
        ...
        "best_delta_vs_nb1861": delta_vs_nb1861,
        "verdict": verdict,
...
if __name__ == "__main__":
    ...
    print(f"  nb1852 ref         : {res['nb1852_ref']:.4f}")
    print(f"  nb1861 ref         : {res['nb1861_ref']:.4f}")
    print(f"  best_lambda        : {res['best_lambda']}")
    print(f"  best_rae_mean_bag  : {res['best_rae_mean_bag']:.4f}")
    print(f"  delta vs nb1861    : {res['best_delta_vs_nb1861']:+.4f}")
    print(f"  verdict            : {res['verdict']}")
    print(f"  per-lambda RAE:")
    for r in res["lambda_records"]:
        print(f"    lambda={r['reg_lambda']:>5.2f}  "
              f"mean_bag={r['rae_mean_bag']:.4f}  "
              f"median_bag={r['rae_median_bag']:.4f}  "
              f"seed_mean={r['seed_mean_rae']:.4f} +/- {r['seed_std_rae']:.4f}")
