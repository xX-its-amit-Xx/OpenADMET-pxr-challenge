"""nb1643 -- HistGradientBoostingRegressor on K-tuned 5-way 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE d=4 n=200 lr=0.05 l2=5) on the 5-way K-tuned 117-col
    feature stack achieved mean_bag RAE = 0.5163 on the 253 unblind set against
    chemprop_aux anchor (in_RAE 0.6216).  Try a third residual learner --
    sklearn HistGradientBoostingRegressor (HistGBM, absolute_error loss,
    max_iter=200, lr=0.05, max_depth=4, l2_regularization=1.0) -- which has
    different histogram binning + tree-growth heuristics than CatBoost/LightGBM
    and may give either a marginal win or a useful orthogonal blend partner.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack from cached top-K rankings (identical to
       nb1554): top-25 AP + top-20 MACCS + top-20 Mord + top-20 chemprop embed
       + top-30 Avalon + pred_chembl_pec50 + mean_sim.
    3. 5-seed bag HistGradientBoostingRegressor(loss='absolute_error',
       max_iter=200, learning_rate=0.05, max_depth=4, l2_regularization=1.0).
       KFold(n=5, shuffle=True) cross-fit per seed.
    4. Pool mean_bag corrected anchor; pooled RAE.
    5. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1643_histgbm_5way_K.py
    data/processed/nb1643_summary.json
    data/processed/nb1643_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # CatBoost MAE 5-way K-tuned mean_bag
...
def _hgbm_params(seed: int) -> dict:
    return dict(
        loss="absolute_error",
        max_iter=200,
        learning_rate=0.05,
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
    ...
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = HistGradientBoostingRegressor(**_hgbm_params(seed))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- HistGradientBoostingRegressor(absolute_error, max_iter=200, "
          f"lr=0.05, max_depth=4, l2=1.0) on 5-way K-tuned 117-col matrix; "
    ...
    # ---- Per-seed HistGBM residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED HIST-GBM RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1554:
        verdict = "HISTGBM_5WAY_K_TUNED_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "HISTGBM_5WAY_K_TUNED_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "HISTGBM_5WAY_K_TUNED_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif beats_nb1543:
        verdict = "HISTGBM_5WAY_K_TUNED_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif beats_anchor:
        verdict = "HISTGBM_5WAY_K_TUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "HISTGBM_5WAY_K_TUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "HISTGBM_5WAY_K_TUNED_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "hgbm_loss": "absolute_error",
        "hgbm_max_iter": 200,
        "hgbm_learning_rate": 0.05,
        "hgbm_max_depth": 4,
        "hgbm_l2_regularization": 1.0,
        "K_AP_best": K_AP_best,
