"""nb1540 -- CatBoost with shallower trees (depth=3) on PRE-unblind 4-way 112-col features.

HYPOTHESIS:
    nb1501 ran CatBoost(MAE, depth=4, n_est=200, lr=0.05, l2=5) on the same
    4-way 112-col PRE-unblind feature matrix as nb1484 (top-30 AP + top-20
    MACCS + top-30 Mordred + top-30 chemprop-embed + pred_chembl + sim) and
    landed at honest cross-fit RAE ~0.527.  Trees may be too deep relative
    to the residual signal on 253 samples; aggressive shallowing (d=3) and
    longer schedule (n=300 at lr=0.04) with stronger L2 (l2=8) may regulate
    the residual learner toward smoother fits.

PROTOCOL:
    1.  Same setup as nb1501: anchor = chemprop_aux te[unb_idx] (PRE-unblind),
        residual = y_unb - anchor, 4-way pruned 112-col feature matrix.
    2.  5-seed bag CatBoost(loss=MAE, depth=3, n_est=300, lr=0.04, l2=8).
    ...
    4.  Verdict at 0.003 margin vs:
            ...
            nb1501 CatBoost d4 4-way          ~0.527

Outputs:
    scripts/nb1540_catboost_depth3_preunblind.py
    data/processed/nb1540_summary.json
    data/processed/nb1540_mean_bag_oof.npy            (253,) float32
"""
...
# CatBoost hyperparameters: SHALLOWER + LONGER + STRONGER L2
CAT_DEPTH = 3
CAT_N_EST = 300
CAT_LR = 0.04
CAT_L2 = 8.0
...
NB1484_REF = 0.5231               # nb1484 4-way LGBM-Huber best  (KEY REF)
NB1501_REF = None                 # loaded from nb1501_summary.json
...
def main() -> dict:
    ...
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d={CAT_DEPTH}, n={CAT_N_EST}, "
          f"lr={CAT_LR}, l2={CAT_L2}) on 4-way pruned 112-col features; "
    ...
    # Try to load nb1501 ref number from its summary
    if NB1501_SUMMARY.exists():
        try:
            with open(NB1501_SUMMARY) as f:
                NB1501_REF = float(json.load(f).get("rae_mean_bag", 0.0))
            print(f"[ref ] nb1501 rae_mean_bag = {NB1501_REF:.4f}")
        except Exception as e:
            print(f"[ref ] nb1501 summary unreadable ({e}); NB1501_REF=None")
    ...
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST(d={CAT_DEPTH}, n={CAT_N_EST}, lr={CAT_LR}, "
          f"l2={CAT_L2}) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    beats_nb1501 = (NB1501_REF is not None) and (
        rae_mean_bag < NB1501_REF - DECISION_MARGIN
    )
    flat_vs_nb1484 = abs(rae_mean_bag - NB1484_REF) < DECISION_MARGIN
    flat_vs_nb1501 = (NB1501_REF is not None) and (
        abs(rae_mean_bag - NB1501_REF) < DECISION_MARGIN
    )

    if beats_nb1484:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_BEATS_NB1484_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1484:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_FLAT_VS_NB1484"
    elif beats_nb1501:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_BEATS_NB1501_BUT_WORSE_THAN_NB1484"
    elif flat_vs_nb1501:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_FLAT_VS_NB1501"
    elif beats_nb1472:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_BEATS_NB1472_BUT_WORSE_THAN_NB1501"
    elif beats_anchor:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_D3_4WAY_PRE_UNBLIND_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "catboost_iterations": CAT_N_EST,
        "catboost_learning_rate": CAT_LR,
        "catboost_l2_leaf_reg": CAT_L2,
        ...
        "delta_mean_bag_vs_nb1501": (
            None if NB1501_REF is None else rae_mean_bag - NB1501_REF
        ),
