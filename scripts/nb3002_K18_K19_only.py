"""nb3002 -- Per-fold SLSQP simplex on 2-anchor pool {K18, K19} (both deep-30).

NEW PARADIGM: drop K=20 (which was zeroed in nb2992 deploy) and test
whether the minimal 2-anchor {K18, K19} pool -- both deep-30 -- is
sufficient to reach (or beat) the nb2992 pooled outer-val RAE = 0.4479.
K=19 here is the nb3000 deep-30 rebuild, NOT the nb2631 5-seed cached
version used in nb2992. This is a cleaner head-to-head than nb2992
because both anchors live at the same depth tier.
...
PROTOCOL:
    ...
    Deploy:
        ...
        - Apply to (513, 2) stacked te arrays -> te_nb3002

GATE:
    mean_pooled_rae < 0.4518  -> "VERIFIED_NEW_PRIMARY1"
    mean_pooled_rae < 0.4570  -> "PROMOTE"
    else                      -> "FAIL"
...
References:
    ...
    nb2992 per-fold simplex K18,K19(5sd),K20 deploy  = 0.4479  (K20 zeroed)
...
Outputs:
    data/processed/nb3002_summary.json
    data/processed/nb3002_pred_oof.npy  (253,) float32 -- per-fold simplex OOF
                                         (taken from the first seed = 1036)
    data/processed/te_nb3002.npy        (513,) float32 -- deploy te
    submissions/nb3002_per_fold_simplex_K18_K19_deep30.csv  (only if verdict != "FAIL")
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 2 deep-30 K-anchors {K_LABELS}")
    print(f"          parents: nb2960 (K18 deep-30) + nb3000 (K19 deep-30)")
    print(f"          motivation: drop K=20 (zeroed in nb2992 deploy);")
    print(f"                      test if minimal 2-anchor pool {{K18,K19}} suffices")
    ...
    print(f"          gate: <{GATE_VERIFIED_PRIMARY1} VERIFIED_NEW_PRIMARY1 / "
    ...
    all_w_arr = np.asarray(all_weights, dtype=np.float64)  # (75, 2)
    ...
    print("-" * 78)
    if mean_pooled < GATE_VERIFIED_PRIMARY1:
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_simplex_K18_K19_deep30.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_deep30_15seed",
        "paradigm": "2_anchor_drop_K20_from_nb2992",
        ...
        "gate_verified_primary1": GATE_VERIFIED_PRIMARY1,
