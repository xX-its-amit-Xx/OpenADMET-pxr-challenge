"""nb2731 -- Extremely Randomized Trees (ExtraTrees) on K=20 chemprop_aux residual.

NEW PARADIGM:
    ExtraTreesRegressor (Geurts et al. 2006) differs from RandomForest in TWO
    ways that matter for the (anchor=chemprop_aux, K=20, n=253) substrate:
      1. Split THRESHOLD is drawn at RANDOM (one uniform draw per candidate
         feature), not chosen to minimize SSE as in classical RF/CART.
      2. Each tree is fit on the FULL training set (no bootstrap by default).
    Net effect: more variance reduction at the cost of slightly higher bias.
    Because the K=20 substrate is already SHAP-K28-truncated (low signal-noise
    along the dropped axes), the bias inflation should be modest while the
    extra variance reduction can plausibly beat plain RF on a 253-row residual
    target.

    Distinction from the boosting / quantile-RF / RF axes already swept:
      - GBDT/LGBM (nb2103/nb2112/...) pick optimal split values via gradient.
      - Quantile-RF (nb2721) picks best split values, recovers conditional CDF.
      - ExtraTrees here: ONE random threshold per candidate feature -> the
        only fully-stochastic-split tree ensemble in our zoo on this anchor.
...
PROTOCOL:
    ...
    3. Model: sklearn.ensemble.ExtraTreesRegressor(
...
Outputs:
    scripts/nb2731_extratrees_k20.py
    data/processed/nb2731_summary.json
    data/processed/nb2731_pred_oof.npy   (253,) float32
    data/processed/te_nb2731.npy         (513,) float32
"""
...
# --------------------------------------------------------------------------
# ExtraTrees hyperparameters (per spec)
# --------------------------------------------------------------------------
ET_N_ESTIMATORS = 500
ET_MAX_DEPTH = 10
...
def _new_et(seed: int = ET_RANDOM_STATE) -> ExtraTreesRegressor:
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Extremely Randomized Trees on K=20 chemprop_aux residual")
    print("=" * 78)
    ...
    print(f"SCAFFOLD 5-FOLD CV  seeds={KF_SEEDS}\n"
          f"ExtraTrees: n_estimators={ET_N_ESTIMATORS}  max_depth={ET_MAX_DEPTH}  "
          f"min_samples_split={ET_MIN_SAMPLES_SPLIT}  "
          f"min_samples_leaf={ET_MIN_SAMPLES_LEAF}  seed={ET_RANDOM_STATE}")
    print("-" * 78)
    ...
    for kf_seed in KF_SEEDS:
        ...
        for fi, (tr_loc, va_loc) in enumerate(splits):
            mdl = _new_et(seed=ET_RANDOM_STATE + kf_seed + fi)
    ...
    print("-" * 78)
    deploy_mdl = _new_et(seed=ET_RANDOM_STATE)
    ...
    summary = {
        "tag": TAG,
        "method": (
            "Extremely Randomized Trees (Geurts et al. 2006) via "
            "sklearn.ensemble.ExtraTreesRegressor; random split thresholds "
            "(one uniform draw per candidate feature) and no bootstrap, "
            "fit on chemprop_aux residual over first-K=20 cols of X_117."
        ),
        "paradigm": (
            "fully_stochastic_split_tree_ensemble_extra_trees_geurts_2006_"
            "more_variance_reduction_more_bias_vs_random_forest"
        ),
        ...
        "et_random_state": ET_RANDOM_STATE,
        "et_n_jobs": ET_N_JOBS,
        "n_folds": N_FOLDS,
        "kf_seeds": KF_SEEDS,
        "n_unb": int(n_unb),
