"""nb2571 -- Pseudo-Huber loss LGBM with delta sweep on K=20 residual.

NEW PARADIGM:
    Pseudo-Huber loss interpolates between MSE (small residuals: quadratic) and
    MAE (large residuals: linear).  delta controls the transition point.  Sweep
    delta in {0.3, 0.5, 1.0, 2.0, 5.0} on the chemprop_aux residual.

    Custom LGBM objective (NOT the built-in `huber` which is the regular Huber
    not pseudo-Huber):
        L(x)    = delta^2 * (sqrt(1 + (x/delta)^2) - 1)
        grad(x) = x / sqrt(1 + (x/delta)^2)              [= dL/dy_pred since x=p-t]
        hess(x) = 1 / (1 + (x/delta)^2)^(3/2)
...
PROTOCOL:
    ...
    2. Reuse K=20 RFE-surviving features (nb2231 snapshot K=20) over the
       ...
       / Avalon + ChEMBL kNN).
    3. For each delta in {0.3, 0.5, 1.0, 2.0, 5.0}:
       - 5-fold scaffold CV on 253 unblind, 5 kf_seeds {1001..1005}, fitting
         LGBM with the custom pseudo-Huber objective on the residual.
       - Mean across kf_seeds.
    4. Save mean-bag pred_oof and te for the BEST delta (lowest mean_rae).

GATE:
    best delta mean_rae < 0.4570 -> "PROMOTE"
                       < 0.4601 -> "MARGINAL_BEAT"
                       else     -> "FAIL"

OUTPUTS:
    scripts/nb2571_huber_delta_sweep.py
    data/processed/nb2571_summary.json
    data/processed/nb2571_pred_oof.npy      (253,) float32  -- best delta
    data/processed/te_nb2571.npy            (513,) float32  -- best delta
"""
...
# -------- delta sweep --------
DELTA_GRID = [0.3, 0.5, 1.0, 2.0, 5.0]
...
# -------- reference --------
NB2240_REF_OOF_K20 = 0.4682  # nb2240 K=20 LGBM(MSE) mean-bag context

# -------- feature build paths (copied from nb2502/nb2240) --------
...
# =============================================================================
# Pseudo-Huber custom objective
# =============================================================================

def make_pseudo_huber_obj(delta: float):
    """Return an LGBM custom objective for pseudo-Huber loss.

    LGBMRegressor sklearn API signature: objective(y_true, y_pred) -> (grad, hess).
    Loss is computed on x = y_pred - y_true:

        L(x)    = delta^2 * (sqrt(1 + (x/delta)^2) - 1)
        grad(x) = dL/dy_pred = x / sqrt(1 + (x/delta)^2)
        hess(x) = d2L/dy_pred^2 = 1 / (1 + (x/delta)^2)^(3/2)
    """
    d2 = float(delta) ** 2
    ...
    def _obj(y_true, y_pred):
        ...
        x = y_pred - y_true
        ratio_sq = (x * x) / d2  # (x/delta)^2
        s = np.sqrt(1.0 + ratio_sq)
        grad = x / s
        hess = 1.0 / (s * s * s)  # (1 + (x/delta)^2)^(-3/2)
        return grad, hess

    return _obj


def _lgbm_params(seed: int, delta: float) -> dict:
    """Same shape as nb2502/nb2240 (K=20 LGBM) but custom pseudo-Huber objective."""
    return dict(
        objective=make_pseudo_huber_obj(delta),
        max_depth=4,
        num_leaves=15,
        n_estimators=300,
        learning_rate=0.03,
...
# =============================================================================
# Helpers (copied from nb2502 -- feature build + ChEMBL pool)
# =============================================================================
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Pseudo-Huber LGBM (delta sweep) on K=20 chemprop_aux residual")
    ...
    # --- rebuild 117-col 5-way feature matrix exactly as nb2240/nb2502 ---
    ...
    # =========================================================================
    # delta sweep: for each delta, 5-fold scaffold CV across 5 kf_seeds
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"DELTA SWEEP  kf_seeds={KF_SEEDS}  bag_seeds={RESID_SEEDS}")
    print(f"target: residual = y_unb - chemprop_aux  (pseudo-Huber objective)")
    print("-" * 78)

    per_delta_records = []
    per_delta_oof = {}  # delta -> mean-of-seeds OOF (corrected predictions)
    best_delta = None
    ...
    for delta in DELTA_GRID:
        ...
        for kf_seed in KF_SEEDS:
            ...
            for tr_loc, va_loc in splits:
                ...
                for bs in RESID_SEEDS:
                    mdl = lgb.LGBMRegressor(**_lgbm_params(bs, delta))
        ...
        per_delta_records.append(rec)
        print(f"   delta={delta}  mean_rae={mean_rae:.4f} (+/- {std_rae:.4f})  "
    ...
    print("\n" + "=" * 78)
    print(f"BEST delta = {best_delta}  mean_rae = {best_mean_rae:.4f}")
    print("=" * 78)
    ...
    # =========================================================================
    # Deploy for BEST delta: refit on all 253, mean-bag predict on 513 test
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"DEPLOY (best delta = {best_delta})  refit on 253, mean-bag predict 513")
    print("-" * 78)
    te_bag_resid = np.zeros(n_test, dtype=np.float64)
    for bs in RESID_SEEDS:
        mdl = lgb.LGBMRegressor(**_lgbm_params(bs, best_delta))
    ...
    # --- save artifacts (mean-bag pred_oof for best delta + deploy te) ---
    pred_oof_best = per_delta_oof[best_delta].astype(np.float32)
    ...
    summary = {
        "tag": TAG,
        "method": (
            "pseudo_huber_LGBM_delta_sweep_K20_chemprop_aux_residual_"
        ...
        "per_delta_records": per_delta_records,
    ...
    print(f"=== {TAG} SUMMARY ===")
    print(f"   delta grid           = {DELTA_GRID}")
    for rec in per_delta_records:
        print(f"     delta={rec['delta']:>4}  mean_rae={rec['mean_rae']:.4f}  "
              f"(+/- {rec['std_rae']:.4f})")
