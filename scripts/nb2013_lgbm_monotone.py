"""nb2013 -- LightGBM (MSE) with monotone constraints on 5-way K-tuned 117-col features.

HYPOTHESIS:
    Final exploration of the residual-LGBM family: enforce a positive monotone
    constraint on the pred_chembl_pec50 feature (the ChEMBL kNN pEC50 estimate),
    which is the single feature with the strongest a-priori monotonic
    relationship to the residual y_unb - chemprop_aux_unb.  All other columns
    (atompair / maccs / mordred / chempropembed / avalon bits + mean_sim) get
    unconstrained (0) entries in the monotone_constraints vector.  Compared
    against nb1861 (5-outer-seed BoB of nb1852 MSE, 0.5013) to test whether
    monotone regularisation moves the bagged RAE.

    Configuration (mirrors nb1852/nb1861 exactly except monotone constraints):
        objective='regression'        (MSE)
        max_depth=4, num_leaves=15
        n_estimators=300, lr=0.03
        min_child_samples=5, reg_lambda=2.0
        monotone_constraints=[0,...,0, +1, 0]   (pred_chembl_pec50=+1)
        monotone_constraints_method='basic'

    Same 5-way K-tuned 117-col feature stack as nb1852/nb1861:
        ...
        pred_chembl_pec50      <-- monotone +1
        mean_sim               <-- monotone  0

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1861).
    3. 5-seed bag {0,1,2,3,4} LightGBM(MSE, depth=4, leaves=15, 300/0.03,
       min_child=5, lambda=2, monotone[pred_chembl_pec50]=+1).
       KFold(n=5, shuffle=True, random_state=seed) cross-fit per seed.
    4. Per-seed corrected anchor + mean-bag corrected anchor.
    5. Verdict vs nb1861 (0.5013) at 0.003 margin.

Outputs:
    scripts/nb2013_lgbm_monotone.py
    data/processed/nb2013_summary.json
    data/processed/nb2013_mean_bag_oof.npy        (253,) float32
"""
...
NB1852_SUMMARY = DATA_PROCESSED / "nb1852_summary.json"
...
def _lgbm_params(seed: int, monotone_constraints: list[int]) -> dict:
    """LGBM(MSE) -- same as nb1852/nb1861 plus monotone_constraints."""
    return dict(
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int,
                                 monotone_constraints: list[int]) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = lgb.LGBMRegressor(**_lgbm_params(seed, monotone_constraints))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) + monotone(+1 on pred_chembl_pec50) on 5-way "
    ...
    # column layout: [ap...][maccs...][mord...][embed...][avalon...][pred_chembl_pec50][mean_sim]
    n_unconstrained_prefix = (n_top_ap + n_top_maccs + n_top_mord
                              + n_top_embed + n_top_avalon)
    monotone_constraints = (
        [0] * n_unconstrained_prefix      # ap + maccs + mord + embed + avalon
        + [1]                              # pred_chembl_pec50  (positive)
        + [0]                              # mean_sim
    )
    assert len(monotone_constraints) == feat_dim, (
        f"monotone vec len {len(monotone_constraints)} != feat_dim {feat_dim}"
    )
    n_pos = sum(1 for c in monotone_constraints if c == 1)
    n_zero = sum(1 for c in monotone_constraints if c == 0)
    print(f"[mono] monotone_constraints: +1={n_pos}, 0={n_zero}  "
          f"(positive on column {n_unconstrained_prefix} = pred_chembl_pec50)")

    # ---- Seed loop ----
    print("\n" + "-" * 78)
    print(f"5-SEED BAG: seeds={SEEDS}, {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    for s_i, seed in enumerate(SEEDS):
        ...
        resid_oof = _residual_cross_fit_one_seed(
            X_unb, residual, seed, monotone_constraints
        )
    ...
    print(f"   seed min/max        = {seed_min:.4f} / {seed_max:.4f}")
    print(f"   mean_bag   RAE      = {rae_mean_bag:.4f}")
    print(f"   median_bag RAE      = {rae_median_bag:.4f}")
    print(f"   anchor     RAE      = {rae_anchor:.4f}")
    print(f"   nb1852 ref          = {NB1852_REF:.4f}")
    print(f"   nb1861 ref          = {NB1861_REF:.4f}")
    print(f"   d(mean_bag, anchor) = {rae_mean_bag - rae_anchor:+.4f}")
    print(f"   d(mean_bag, nb1852) = {rae_mean_bag - NB1852_REF:+.4f}")
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "MONOTONE_IMPROVES_OVER_NB1861"
    elif delta_vs_nb1861 > DECISION_MARGIN:
        verdict = "MONOTONE_HURTS_VS_NB1861"
    else:
        verdict = "MONOTONE_NEUTRAL_VS_NB1861"
    ...
    summary = {
        ...
        "lgbm_reg_lambda": 2.0,
        "lgbm_monotone_constraints_method": "basic",
        "monotone_pos_idx": n_unconstrained_prefix,
        "monotone_pos_feature_name": "pred_chembl_pec50",
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "monotone_pos_idx", "monotone_pos_feature_name",
        "n_monotone_pos", "n_monotone_zero",
        "rae_anchor_chemprop_aux",
        "per_seed_rae",
        "seed_mean_rae", "seed_std_rae",
        "seed_min_rae", "seed_max_rae",
        "rae_mean_bag", "rae_median_bag",
        "delta_mean_bag_vs_anchor",
        "delta_mean_bag_vs_nb1852",
        "delta_mean_bag_vs_nb1861",
        "nb1852_ref", "nb1861_ref",
