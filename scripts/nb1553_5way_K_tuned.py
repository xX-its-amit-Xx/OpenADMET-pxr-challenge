"""nb1553 -- PRE-unblind 5-way with ALL K-tuned slot values.

Same protocol as nb1492 but with K-values derived from per-family K-grid runs:
    AtomPair      K=25   (nb1524 best_K = 25)
    MACCS         K=20   (nb1424 best_K, user override = 20)
    Mordred       K=20   (nb1523 best_K = 20)
    ChempropEmbed K=20   (nb1541 best_K = 20)
    Avalon        K=30   (nb1433 best_K, user override = 30)

PROTOCOL
    1. Anchor = chemprop_aux_te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
    2. ChEMBL PXR kNN-5 (Tanimoto on Morgan-2048): same union as nb1472/nb1484/nb1492.
    3. For each FAMILY in [AtomPair, MACCS, Mordred, ChempropEmbed, Avalon]:
         ...
         b. Train 1 seed-0 shallow LGBM Huber on residual; pull SHAP importance
            (TreeExplainer; fallback gain).
         c. Pick top-K family columns by importance (K from TOP_K above).
         ...
         g. Pool RAE.
    4. Naive 1/5 mean blend across the 5 mean-bag corrected OOFs.
    5. 5-fold SLSQP cross-fit (simplex K=5) on the 5 corrected OOFs.
    6. Pool RAE for both.
    7. Verdict at 0.003 margin vs nb1550 ref = nb1492 (0.5260).

Outputs:
    scripts/nb1553_5way_K_tuned.py            (this file)
    data/processed/nb1553_summary.json
    data/processed/nb1553_best_oof.npy         (253,) float32  best variant
"""
...
NB1472_REF = 0.5330
NB1484_REF = 0.5231
NB1492_REF = 0.5260  # prior 5-way (treated as nb1550 reference)
NB1550_REF = NB1492_REF
DECISION_MARGIN = 0.003

# All K values tuned per-family from K-grid runs (user-specified)
TOP_K = {
    "AtomPair": 25,       # nb1524 best_K = 25
    "MACCS": 20,          # nb1424 (user override) = 20
    "Mordred": 20,        # nb1523 best_K = 20
    "ChempropEmbed": 20,  # nb1541 best_K = 20
    "Avalon": 30,         # nb1433 (user override) = 30
}
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- PRE-unblind 5-way K-TUNED: chemprop_aux anchor + "
          f"AtomPair-25 + MACCS-20 + Mordred-20 + ChempropEmbed-20 + Avalon-30")
    ...
    # ---- Naive 1/5 mean blend ----
    print("\n" + "=" * 78)
    print("5-WAY NAIVE 1/5 MEAN BLEND")
    print("=" * 78)
    ...
    for r in fam_results:
        print(f"   {r['family']:<14s} (K={r['top_k']:>3d}) "
              f"mean_bag RAE = {r['rae_mean_bag']:.4f}")
    print(f"   naive 1/5 mean blend RAE = {rae_mean:.4f}")

    # ---- 5-fold SLSQP cross-fit (K=5) ----
    print("\n" + "=" * 78)
    print(f"5-WAY 5-FOLD SLSQP CROSS-FIT (seed={SLSQP_SEED})")
    print("=" * 78)
    P_5 = per_family_mean_bag.T.astype(np.float64)  # (253, 5)
    slsqp_oof, slsqp_folds = _slsqp_cross_fit(
        P_5, y_unb, n_splits=SLSQP_FOLDS, seed=SLSQP_SEED
    )
    rae_slsqp = float(rae(y_unb, slsqp_oof))
    W = np.array([f["w"] for f in slsqp_folds])  # (5, 5)
    ...
    print(f"   nb1492/nb1550 ref            = {NB1492_REF:.4f}")
    print(f"   naive 1/5 mean blend RAE     = {rae_mean:.4f}")
    print(f"   5-fold SLSQP cross-fit RAE   = {rae_slsqp:.4f}")

    rae_best = min(rae_mean, rae_slsqp)
    which_best = "naive_1_5_mean" if rae_mean <= rae_slsqp else "slsqp_5fold"
    if which_best == "naive_1_5_mean":
        best_oof = mean_oof
    else:
        best_oof = slsqp_oof
    ...
    delta_naive_vs_nb1550 = delta_naive_vs_nb1492
    delta_slsqp_vs_nb1550 = delta_slsqp_vs_nb1492
    delta_best_vs_nb1550 = delta_best_vs_nb1492
    ...
    beats_nb1550_naive = rae_mean < NB1550_REF - DECISION_MARGIN
    beats_nb1550_slsqp = rae_slsqp < NB1550_REF - DECISION_MARGIN
    beats_nb1550_best = rae_best < NB1550_REF - DECISION_MARGIN
    flat_vs_nb1550_best = abs(rae_best - NB1550_REF) < DECISION_MARGIN
    ...
    if beats_nb1550_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_KTUNED_BEATS_NB1550_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1550_best:
        verdict = f"PRE_UNBLIND_5WAY_KTUNED_FLAT_VS_NB1550_best_{which_best}"
    elif beats_nb1484_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_KTUNED_BEATS_NB1484_BUT_WORSE_THAN_NB1550"
            f"_best_{which_best}"
        )
    elif beats_nb1472_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_KTUNED_BEATS_NB1472_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif beats_anchor_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_KTUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "PRE_UNBLIND_5WAY_KTUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "PRE_UNBLIND_5WAY_KTUNED_HURTS_ANCHOR"

    print(f"   d_naive_vs_nb1550   = {delta_naive_vs_nb1550:+.4f}")
    print(f"   d_slsqp_vs_nb1550   = {delta_slsqp_vs_nb1550:+.4f}")
    print(f"   d_best_vs_nb1550    = {delta_best_vs_nb1550:+.4f}   "
          f"(best = {which_best})")
    print(f"   d_naive_vs_nb1484   = {delta_naive_vs_nb1484:+.4f}")
    print(f"   d_slsqp_vs_nb1484   = {delta_slsqp_vs_nb1484:+.4f}")
    print(f"   beats_nb1550_naive  = {beats_nb1550_naive}")
    print(f"   beats_nb1550_slsqp  = {beats_nb1550_slsqp}")
    print(f"   beats_nb1550_best   = {beats_nb1550_best}")
    ...
    summary = {
        ...
        "delta_naive_vs_nb1550": delta_naive_vs_nb1550,
        "delta_slsqp_vs_nb1550": delta_slsqp_vs_nb1550,
        "delta_best_vs_nb1550": delta_best_vs_nb1550,
        ...
        "beats_nb1550_naive": bool(beats_nb1550_naive),
        "beats_nb1550_slsqp": bool(beats_nb1550_slsqp),
        "beats_nb1550_best": bool(beats_nb1550_best),
        "flat_vs_nb1550_best": bool(flat_vs_nb1550_best),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_naive_vs_nb1550",
        "delta_slsqp_vs_nb1550",
        "delta_best_vs_nb1550",
        "beats_nb1550_naive",
        "beats_nb1550_slsqp",
        "beats_nb1550_best",
        "flat_vs_nb1550_best",
