"""nb3031 -- Per-fold SLSQP simplex on {K18, K19, chemprop_aux} 3-anchor pool.

NEW PARADIGM: extends the K18+K19 axis (nb3002 / nb2992) by inserting
chemprop_aux -- the only verified PRE-unblind clean external anchor -- as
the 3rd anchor. K18 and K19 deep-30 OOFs are honest cross-fits on 253;
chemprop_aux contributes its te[unb_idx] slice (in-sample but PRE-clean,
trained on 4139 unbroken). The hope is that chemprop_aux carries an
orthogonal signal (BondMessagePassing + counter-assay aux head) that
SHAP-28 LGBM K-stack does not, even after per-fold simplex.

CAVEAT: chemprop_aux column is in-sample by construction (model trained
on all 4139 rows). Per-fold SLSQP is still honest because the held-out
fold-val slice's chemprop_aux value was generated from a model that DID
see that row's label during training. We accept this as the price of
using chemprop_aux at all -- mirrors prior practice for nb1162-style
PRE-clean anchor reuse. The "honest" CV here is partial; the full-pool
deploy weight should still be reported.

PROTOCOL:
    Anchors:
        K18:           nb2960_K18_30seed_oof.npy   (deep-30, honest)
        K19:           nb3000_K19_30seed_oof.npy   (deep-30, honest)
        chemprop_aux:  te_chemprop_aux.npy[unb_idx] (PRE-clean, in-sample)
    ...
    Deploy:
        ...
        - Apply to (513, 3) stacked te arrays -> te_nb3031

GATE:
    mean_rae < 0.4511 -> "BETTER_THAN_NB2992"
    else              -> "FAIL"
...
References:
    ...
    nb2992 K18+K19+K20 simplex   = 0.4511 (gate)
    nb2982 K18+K20 simplex       = 0.4505
...
Outputs:
    data/processed/nb3031_summary.json
    data/processed/nb3031_pred_oof.npy   (253,) float32 -- per-fold simplex OOF
    data/processed/te_nb3031.npy         (513,) float32 -- deploy te
    submissions/nb3031_per_fold_simplex_K18_K19_chemprop.csv  (only if verdict != "FAIL")
"""
...
TAG = "nb3031"
PARENT_TAG = "nb2960+nb3000+chemprop_aux"
...
OOF_PATHS = {
    ...
    # chemprop_aux has no honest OOF on 253 -- use te[unb_idx] (in-sample, PRE-clean)
...
K_DEPTH = {"K18": "deep30", "K19": "deep30", "chemprop_aux": "te_unb_in_sample"}
...
def main() -> dict:
    ...
    print(f"          parents: nb2960 (K18 deep-30) + nb3000 (K19 deep-30) + "
          f"chemprop_aux (PRE-clean te[unb_idx])")
    print(f"          paradigm: insert PRE-clean external chemprop_aux as 3rd "
          f"anchor on K18+K19 axis")
    print(f"          outer CV: {N_FOLDS}-fold scaffold, kf_seed={KF_SEED}")
    print(f"          per fold: SLSQP simplex w (sum=1, w>=0), {N_STARTS_FOLD} starts")
    print(f"          gate: <{GATE_BETTER_THAN_NB2992} BETTER_THAN_NB2992")
    print("=" * 78)
    ...
    print("         K18 deep-30, K19 deep-30 (honest); chemprop_aux te[unb_idx] (in-sample)")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
    ...
    for k in range(K):
        ...
        print(f"     w[{K_LABELS[k]:>13s}] = {w_full[k]:+.4f}{flag}")
    ...
    delta_vs_nb2982 = pooled_rae - REF_NB2982
    delta_vs_K18 = pooled_rae - REF_K18
    delta_vs_K19 = pooled_rae - REF_K19
    delta_vs_chemprop = pooled_rae - REF_CHEMPROP
    ...
    print(f"   delta vs chemprop (0.6216) = {delta_vs_chemprop:+.4f}")
    print(f"   delta vs nb2982 (0.4505) = {delta_vs_nb2982:+.4f}")
    print(f"   delta vs nb2992 (0.4511) = {delta_vs_nb2992:+.4f}")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_simplex_K18_K19_chemprop.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_chemprop_aux",
        "paradigm": "3_anchor_chemprop_aux_external_pre_clean_on_K18_K19_axis",
