"""nb2580 -- Geometric mean of anchor predictions (log-mean ensembling).

NEW PARADIGM:
    All ladder-decision blends so far (nb1150 SLSQP simplex, nb2171 anchor-swap
    pyramid, nb2572 per-anchor iso + SLSQP, nb562 / nb2200 rank-stretch) are
    ARITHMETIC convex combinations:  yhat = sum_k w_k * a_k.
    HERE we test the GEOMETRIC mean dual:  yhat = exp( sum_k w_k * log(a_k) )
    with weights on the simplex (w >= 0, sum = 1).

HYPOTHESIS:
    Geometric mean dampens extreme predictions because log() is concave.
    Truth pEC50 has a fat tail of low-activity (pEC50 < 4) compounds where
    arithmetic blending can be pulled up by an over-predicting anchor.
    If high-pEC50 errors and low-pEC50 errors live on different multiplicative
    scales, log-space blending may reduce variance asymmetrically vs arithmetic.

    Equivalently, geometric blending is arithmetic blending in log-pEC50
    space.  Since RAE is reported in pEC50 space, the geometric blend will
    only beat arithmetic if (truth - log-blend) has smaller absolute deviation
    than (truth - arithmetic-blend) on this 253-row scaffold-CV target.

PROTOCOL:
    - 4 PRE-clean anchors (when available):
        ...
        4. 100-seed bag  -> nb2560_mean_bag_oof_K20_100seed.npy /
                            te_nb2560_K20_100seed.npy
      If the nb2560 .npy outputs are missing on disk (its summary exists but
      the OOF/te arrays were not materialised in cycle-160 deep-30 run), we
      gracefully fall back to the 3 verified PRE-clean anchors.  The
      `anchor_pool` field in the summary records exactly which anchors
      participated.
    - For each fold/seed: clip per-anchor predictions at EPSILON=1.0
      (truth_min = 1.745, all anchor mins observed > 1.4, so clip floor 1.0
      preserves all rows).  Compute log_anchor_i.
    - Linear SLSQP simplex weights on LOG-SPACE residuals:
        minimize  || sum_k w_k * log(a_k[tr]) - log(y[tr]) ||^2
        s.t.      w_k >= 0,  sum w_k = 1
      Apply  log_pred_val = sum_k w_k * log(a_k[va])  ->  pred_val =
      exp(log_pred_val), clipped to [Y_MIN, Y_MAX] = [3.0, 8.0] for safety
      (matches nb2572 isotonic clip range).
...
Outputs:
    data/processed/nb2580_summary.json
    data/processed/nb2580_pred_oof.npy   (253,) float32
    data/processed/te_nb2580.npy         (513,) float32
"""
...
# ---- PRE-clean anchor pool (4 specified; nb2560 optional if npys missing) ----
ANCHOR_SPEC = [
    ("nb2240_K20",
    ...
    ("nb2560_100seed",
     DATA_PROCESSED / "nb2560_mean_bag_oof_K20_100seed.npy",
     DATA_PROCESSED / "te_nb2560_K20_100seed.npy",
     "100-seed K=20 deep bag pyramid (cycle-160 deep-30 saturation test)"),
]
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- geometric-mean blend (log-space SLSQP simplex)")
    print("=" * 78)
    ...
    # ---- load anchors (skip missing nb2560 gracefully) ----
    anchor_names = []
    anchor_prov = {}
    skipped = []
    oof_cols, te_cols = [], []
    for name, oof_path, te_path, prov in ANCHOR_SPEC:
        if not oof_path.exists() or not te_path.exists():
            skipped.append({
                ...
                "te_exists": te_path.exists(),
                "oof_path": str(oof_path),
                "te_path": str(te_path),
            })
            print(f"[skip] {name}: oof={oof_path.exists()} te={te_path.exists()}")
            continue
    ...
    K = len(anchor_names)
    if K < 2:
        raise RuntimeError(f"Need >= 2 anchors for blend; got K={K}")
    ...
    # ---- in-sample equal-weight geometric mean baseline (diagnostic) ----
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_geometric_mean_blend.csv"
    pd.DataFrame({
        "SMILES": te_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "geometric_mean_blend_log_space_slsqp_simplex",
        "paradigm": "log_mean_ensembling_vs_arithmetic_convex_blend",
    ...
    if skipped:
        print(f"   anchors skipped          = {[s['name'] for s in skipped]}")
