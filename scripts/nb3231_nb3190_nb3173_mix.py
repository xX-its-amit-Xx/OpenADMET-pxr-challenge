"""nb3231 -- per-fold SLSQP simplex on 2 best clip winners {nb3190, nb3173}.

NEW PARADIGM: weighted blend of the two best clip winners (per-fold).

Drop nb3174 from the 3-anchor cluster (nb3214) to test whether the weaker
member was diluting the SLSQP signal -- pure 2-anchor simplex over the two
strongest clip operators.

ANCHORS (both PRE-unblind anchor chain, both in the ~0.4422-0.4426 ceiling):
    1. nb3190  -- learned per-fold clip on nb3090 anchor
    2. nb3173  -- learned per-fold clip on nb3080 anchor

PROTOCOL (per kf_seed, 5-fold scaffold split on 253 unblind):
    P = stack([nb3190_oof, nb3173_oof], axis=1)  # (253, 2)
    Per outer fold:
...
Outputs:
    data/processed/nb3231_summary.json
    data/processed/nb3231_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3231.npy         (513,) float32 -- deploy te
    submissions/nb3231_nb3190_nb3173_mix.csv  (only on BETTER)
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 2 best clip winners")
    ...
    if mean_pf < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3231 15-seed per-fold-mean "
            f"{mean_pf:.4f} beats BETTER gate {GATE_BETTER:.4f} "
            f"({mean_pf - GATE_BETTER:+.4f}) and the best single clip-winner "
            f"nb3173 {REF_NB3173:.4f} ({mean_pf - REF_NB3173:+.4f}). "
            f"Pure 2-anchor SLSQP simplex over {ANCHOR_NAMES} extracts "
            f"residual orthogonality. Mean-of-fold weights = "
        ...
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3231 15-seed per-fold-mean {mean_pf:.4f} fails "
            f"BETTER gate {GATE_BETTER:.4f} ({mean_pf - GATE_BETTER:+.4f}). "
            f"The 2 clip winners are too correlated for SLSQP to extract "
            f"residual orthogonality; learned blend collapses to (or below) "
            f"the best single member. Delta vs nb3173 {REF_NB3173:.4f} = "
            f"{mean_pf - REF_NB3173:+.4f}, vs equal-weight {eq_oof_rae:.4f} "
            f"= {mean_pf - eq_oof_rae:+.4f}. Keep nb3173 on the ladder; "
            f"close the 2-anchor SLSQP-on-clip-winners axis."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_nb3190_nb3173_mix.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "per_fold_slsqp_simplex_on_2_clip_winners",
