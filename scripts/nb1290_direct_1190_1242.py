"""nb1290 -- DIRECT 2-way blend nb1190 BoB + nb1242 ChEMBL (never tested as pair).

Hypothesis:
    nb1190 (BoB-of-Bags mean, 0.5499) is an internal variance-reduction
    blend; nb1242 (ChEMBL-kNN feature-augmented residual, 0.5431) draws
    on external chemistry signal. Direct pairing was never tested -- nb1251
    paired nb1242 with nb1211 (BoB-of-BoBs) instead. May find a different
    minimum that beats nb1251 (0.5394).

Protocol:
  1. Load nb1190_bob_mean_oof.npy (0.5499) and nb1242_mean_bag_oof.npy (0.5431).
  2. Compute pred-pred Pearson and residual Pearson.
  3. Naive 0.5/0.5 mean RAE.
  4. 5-fold cross-fit SLSQP.
  5. Grid search w in {0.30..0.70 step 0.05} for w*nb1190 + (1-w)*nb1242.
     Pool RAE per w.
  6. Best fixed-w & RAE.
  7. Verdict at 0.003 margin vs nb1251 (0.5394).

NO deploy (513) refit -- 253-only honest cross-fit diagnostic.

Outputs:
  data/processed/nb1290_slsqp_oof.npy   (253,) float32 -- SLSQP cross-fit blend
  data/processed/nb1290_mean_oof.npy    (253,) float32 -- naive 0.5/0.5 mean
  data/processed/nb1290_bestw_oof.npy   (253,) float32 -- best fixed-w blend
  data/processed/nb1290_summary.json
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DIRECT 2-way blend nb1190 BoB (0.5499) + nb1242 ChEMBL (0.5431)")
    print(f"          5-fold SLSQP cross-fit + naive mean + fixed-w grid")
    print(f"          verdict margin {MARGIN} vs nb1251 (0.5394)")
    print("=" * 78)
    ...
    p1 = preds["nb1190"]  # weighted by w
    p2 = preds["nb1242"]  # weighted by (1-w)
    ...
    print(f"   nb1190 BoB-of-Bags mean      : {standalone_rae['nb1190']:.4f}  "
          f"(ref {NB1190_REF:.4f})")
    ...
    print(f"\n[diag] pred-pred  Pearson (nb1190, nb1242) = {pred_corr:.4f}")
    print(f"[diag] residual  Pearson (nb1190, nb1242) = {resid_corr:.4f}")
    print(f"[diag] residual std nb1190 = {r1.std():.4f}")
    print(f"[diag] residual std nb1242 = {r2.std():.4f}")
    ...
    print(f"   per-fold weights (nb1190, nb1242):")
    for rec in fold_records:
        w = rec["weights"]
        print(f"     fold {rec['fold']}: w[nb1190]={w[0]:.4f}  w[nb1242]={w[1]:.4f}")
    print(f"   mean weights across folds:  w[nb1190]={mean_w[0]:.4f}  "
    ...
    print(f"   in-sample SLSQP weights: w[nb1190]={w_full[0]:.4f}  "
          f"w[nb1242]={w_full[1]:.4f}   RAE = {rae_full:.4f}")
    ...
    # ---- Fixed-w grid (per spec: 0.30..0.70 step 0.05; w*nb1190 + (1-w)*nb1242) ----
    print("\n" + "-" * 78)
    print("  BLOCK: fixed-w grid (w on nb1190 in 0.30..0.70 step 0.05)")
    print("-" * 78)
    grid_w = [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]
    grid_results = []
    best_w = None
    best_rae = float("inf")
    best_oof = None
    for w in grid_w:
        ...
        grid_results.append({"w_nb1190": float(round(w, 4)),
                             "w_nb1242": float(round(1.0 - w, 4)),
                             "rae": r})
        marker = ""
        if r < best_rae:
            best_rae = r
            ...
            best_oof = blend
            marker = "  <-- best so far"
        print(f"   w[nb1190]={w:.2f}  w[nb1242]={1.0 - w:.2f}  RAE={r:.4f}{marker}")
    print(f"   best fixed-w blend: w[nb1190]={best_w:.4f}  "
          f"w[nb1242]={1.0 - best_w:.4f}  RAE={best_rae:.4f}")
    ...
    nb1190_rae = standalone_rae["nb1190"]
    ...
    if beats_nb1251:
        verdict = (f"DIRECT_1190_1242_BEATS_NB1251 "
                   f"({best_blend_tag} @ {best_blend_rae:.4f})")
    elif flat_nb1251:
        verdict = (f"DIRECT_1190_1242_FLAT_VS_NB1251 "
                   f"({best_blend_tag} @ {best_blend_rae:.4f})")
    else:
        verdict = (f"DIRECT_1190_1242_HURTS_VS_NB1251 "
                   f"({best_blend_tag} @ {best_blend_rae:.4f})")

    print("\n" + "=" * 78)
    print("VERDICT")
    print("=" * 78)
    print(f"   nb1190 standalone : {nb1190_rae:.4f}  (ref {NB1190_REF:.4f})")
    ...
    summary = {
        ...
        "residual_std_nb1190": float(r1.std()),
        "residual_std_nb1242": float(r2.std()),
        ...
        "best_fixed_w_nb1190": best_w,
        "best_fixed_w_nb1242": float(1.0 - best_w),
        ...
        "delta_best_vs_nb1190": best_blend_rae - nb1190_rae,
...
if __name__ == "__main__":
    ...
    for k in ("standalone_rae",
              ...
              "rae_naive_mean",
              "best_fixed_w_nb1190", "rae_best_fixed_w",
              "candidate_rae_table",
