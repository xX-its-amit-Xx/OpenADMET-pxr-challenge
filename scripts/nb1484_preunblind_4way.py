"""nb1484 -- PRE-unblind 4-way SHAP-pruned residual blend.

Extends nb1472's 3-way (AtomPair-30, MACCS-20, Mordred-30) with a fourth
chemprop embed (300-d) residual learner pruned to top-30, all anchored to
chemprop_aux (PRE-unblind te file on 513).
...
PROTOCOL
    ...
    3. For each FAMILY in [AtomPair-2048, MACCS-167, Mordred-1533,
         ...
         c. Pick top-K (30 / 20 / 30 / 30) family columns by importance.
         ...
         g. Pool RAE.
    4. Naive 1/4 mean blend across the 4 mean-bag corrected OOFs.
    5. 5-fold SLSQP cross-fit (simplex K=4) on the 4 corrected OOFs.
    6. Pool RAE for both.
    7. Verdict at 0.003 margin vs nb1472 (0.5330).

Outputs:
    scripts/nb1484_preunblind_4way.py            (this file)
    data/processed/nb1484_summary.json
    data/processed/nb1484_best_oof.npy            (253,) float32  best variant
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- PRE-unblind 4-way: chemprop_aux anchor + "
    ...
    flat_vs_nb1472_best = abs(rae_best - NB1472_REF) < DECISION_MARGIN
    beats_anchor_best = rae_best < rae_anchor - DECISION_MARGIN

    if beats_nb1472_best:
        verdict = (
            f"PRE_UNBLIND_4WAY_BEATS_NB1472_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1472_best:
        verdict = f"PRE_UNBLIND_4WAY_FLAT_VS_NB1472_best_{which_best}"
    elif beats_anchor_best:
        verdict = (
            f"PRE_UNBLIND_4WAY_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "PRE_UNBLIND_4WAY_FLAT_VS_ANCHOR"
    else:
        verdict = "PRE_UNBLIND_4WAY_HURTS_ANCHOR"

    print(f"   d_naive_vs_nb1472   = {delta_naive_vs_nb1472:+.4f}")
    print(f"   d_slsqp_vs_nb1472   = {delta_slsqp_vs_nb1472:+.4f}")
    print(f"   d_best_vs_nb1472    = {delta_best_vs_nb1472:+.4f}   "
          f"(best = {which_best})")
    print(f"   beats_nb1472_naive  = {beats_nb1472_naive}")
    print(f"   beats_nb1472_slsqp  = {beats_nb1472_slsqp}")
    ...
    summary = {
        ...
        "flat_vs_nb1472_best": bool(flat_vs_nb1472_best),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_naive_vs_nb1472",
        "delta_slsqp_vs_nb1472",
        "delta_best_vs_nb1472",
        "beats_nb1472_naive",
        "beats_nb1472_slsqp",
        "beats_nb1472_best",
        "flat_vs_nb1472_best",
