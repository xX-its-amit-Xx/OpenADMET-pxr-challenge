"""nb1852 -- LightGBM (regression / MSE, small leaves) on 5-way K-tuned 117-col features.

HYPOTHESIS:
    The user asked for an xentropy-style probe but instructed "standard MSE for
    comparison to huber" -- so this script swaps nb1771's huber objective for
    plain 'regression' (L2 / MSE) while holding every other knob fixed.  The
    comparison is anchored against nb1771 (huber, rae_mean_bag 0.5100) to
    isolate the loss-function effect on residuals atop chemprop_aux.

    Configuration (same shape as nb1771 except objective):
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(regression/MSE, max_depth=4, num_leaves=15,
    ...
    5. Verdict vs nb1771 (0.5100) at 0.003 margin.

Outputs:
    scripts/nb1852_lgbm_mse.py
    data/processed/nb1852_summary.json
    data/processed/nb1852_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM huber alpha=1.0 small-leaves on same 117-col stack
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- swap from nb1771's huber; all other knobs fixed."""
    return dict(
        objective="regression",     # standard MSE / L2 (for comparison to huber)
        max_depth=4,
        num_leaves=15,
        n_estimators=300,
        learning_rate=0.03,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(regression/MSE, max_depth=4, num_leaves=15, "
    ...
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1771 huber ({NB1771_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- Per-seed LGBM MSE residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (regression/MSE, min_child=5) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    # ---- Verdict (vs nb1771 huber baseline) ----
    ...
    if beats_nb1771:
        verdict = "LGBM_MSE_BEATS_NB1771_HUBER_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1771:
        verdict = "LGBM_MSE_FLAT_VS_NB1771_HUBER"
    elif beats_nb1554:
        verdict = "LGBM_MSE_BEATS_NB1554_BUT_WORSE_THAN_NB1771_HUBER"
    elif flat_vs_nb1554:
        verdict = "LGBM_MSE_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_MSE_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_MSE_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_MSE_HURTS_ANCHOR"
