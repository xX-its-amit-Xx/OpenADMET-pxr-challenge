"""nb1871 -- Deploy nb1861 pooled 25-bag (LGBM MSE / regression) to 513-row CSV.

PROTOCOL:
    Same 117-col feature stack as nb1861, but deploy on full 513 test set.
    1. For 5 outer x 5 inner = 25 deploy fits:
         LGBM(objective='regression' (MSE), max_depth=4, num_leaves=15,
              n_est=300, lr=0.03, min_child_samples=5, reg_lambda=2).
         Inner seed = outer*1000 + j  for j in 0..4.
    ...
    3. Stack 25 residual_513 vectors. Row-level mean -> mean_residual_513.
    4. te_nb1871 = te_chemprop_aux + mean_residual_513.
    5. Save submissions/nb1871_deploy_nb1861.csv (513 rows).

ANCHORS:
    nb1861 pooled 25-bag honest cross-fit RAE = 0.5013 (predicted LB 0.504).
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- mirrors nb1861 exactly; all knobs fixed."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1861 pooled 25-bag (LGBM MSE) to 513-row CSV")
    print(f"          outer seeds = {OUTER_SEEDS}  inner/outer = "
          f"{INNER_PER_OUTER}  total fits = "
          f"{len(OUTER_SEEDS) * INNER_PER_OUTER}")
    ...
    print("\n" + "-" * 78)
    print(f"DEPLOY: 25 LGBM(MSE) fits on all {n_unb} unblind, predict {n_test}")
    print("-" * 78)
    all_resid_513 = np.zeros(
        (len(OUTER_SEEDS) * INNER_PER_OUTER, n_test), dtype=np.float64
    )
    fit_records = []
    k = 0
    for o_seed in OUTER_SEEDS:
        for j in range(INNER_PER_OUTER):
            i_seed = o_seed * 1000 + j
            t_in = time.time()
            ...
            print(f"   fit {k+1:2d}/25  outer={o_seed:4d}  inner={i_seed:6d}  "
    ...
    te_nb1871 = te_anchor_513 + mean_resid_513

    # ---- In-sample check on unb_idx ----
    in_pred_unb = te_nb1871[unb_idx]
    rae_in_unb = float(rae(y_unb, in_pred_unb))
    print("\n" + "-" * 78)
    print(f"in-sample RAE on unb_idx = {rae_in_unb:.4f}")
    print(f"anchor in_RAE             = {rae_anchor:.4f}")
    print(f"nb1861 pooled 25-bag      = 0.5013 (honest cross-fit)")
    print("-" * 78)

    # ---- Stats ----
    te_mean = float(te_nb1871.mean())
    te_std = float(te_nb1871.std())
    te_min = float(te_nb1871.min())
    te_max = float(te_nb1871.max())
    print(f"\nte_nb1871 stats: mean={te_mean:.4f}  std={te_std:.4f}  "
    ...
    df_sub = pd.DataFrame({
        ...
        "pEC50": te_nb1871.astype(np.float32),
    })
    if len(df_sub) != 513:
        raise ValueError(f"submission rows {len(df_sub)} != 513")
    sub_path = SUBMISSIONS_DIR / f"{TAG}_deploy_nb1861.csv"
    df_sub.to_csv(sub_path, index=False)
    print(f"\n[save] submission CSV: {sub_path}  ({len(df_sub)} rows)")

    # ---- Save te artifact ----
    te_path = DATA_PROCESSED / f"te_{TAG}.npy"
    np.save(te_path, te_nb1871.astype(np.float32))
    ...
    summary = {
        ...
        "n_fits": int(len(OUTER_SEEDS) * INNER_PER_OUTER),
        "n_test": int(n_test),
        "n_unb": int(n_unb),
        ...
        "te_nb1871_mean": te_mean,
        "te_nb1871_std": te_std,
        "te_nb1871_min": te_min,
        "te_nb1871_max": te_max,
        "in_RAE_unb_idx": rae_in_unb,
        "nb1861_pooled_25bag_honest": 0.5013,
        "predicted_LB": 0.504,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "te_nb1871_mean", "te_nb1871_std",
        "te_nb1871_min", "te_nb1871_max",
        "in_RAE_unb_idx",
        "nb1861_pooled_25bag_honest", "predicted_LB",
        "submission_csv",
    ):
        print(f"  {k}: {res.get(k)}")
