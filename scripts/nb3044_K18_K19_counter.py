"""nb3044 -- 3-anchor per-fold SLSQP simplex on {K18, K19, counter_clean}.

NEW PARADIGM: counter_clean (nb2490 joint counter-residual anchor) as the
3rd axis -- biologically orthogonal to the K18/K19 SHAP-tuned LGBM pair.
nb2490 builds counter-assay-clean OOFs (no nb730 POST contamination) and
fits a residual LGBM on the joint anchor. cross-fit RAE 0.5382 on 253
unblind; PRE-clean (counter-assay axis not trained on unblind labels).

K18 and K19 are deep-30 SHAP-tuned chemprop_aux-anchored residuals on
the pEC50 axis. Adding counter_clean tests whether a different BIOLOGICAL
axis (counter-assay residual) breaks the post-hoc-blend ceiling that
cycle-167 nb2171 (0.4682) and cycle-169 (closed post-hoc-blend axes) and
nb3030 (verified 0.4509 K18+K19 simplex) all converge near.

PROTOCOL:
    Anchors:
        K18:           nb2960_K18_30seed_oof.npy   (deep-30, honest)   RAE 0.4536
        K19:           nb3000_K19_30seed_oof.npy   (deep-30, honest)   RAE 0.4607
        counter_clean: nb2490_pred_oof.npy         (PRE-clean joint)   RAE 0.5382
    Outer CV: 5-fold scaffold split on 253 unblind
    15 fresh kf_seeds {1066..1080} (post nb3030 1051-1065)
    Per fold:
    ...
    Deploy:
        ...
        - Apply to (513, 3) stacked te arrays -> te_nb3044
...
References:
    ...
    nb2490 counter_clean joint    = 0.5382
    nb3030 K18+K19 deep-30 simplex = 0.4509  (current verified PRIMARY-1)
    nb2992 K18+K19+K20 simplex     = 0.4479  (POST-unblind contamination risk)
...
Inputs:
    ...
    data/processed/nb2490_pred_oof.npy
    data/processed/te_nb2490.npy

Outputs:
    data/processed/nb3044_summary.json
    data/processed/nb3044_pred_oof.npy  (253,) float32 -- per-fold simplex OOF
                                         (first-seed slice; representative)
    data/processed/te_nb3044.npy        (513,) float32 -- deploy te
    submissions/nb3044_K18_K19_counter.csv  (only if verdict != "FAIL")
"""
...
TAG = "nb3044"
PARENT_TAG = "nb2960+nb3000+nb2490"

# -- Inputs --------------------------------------------------------------------
K_LABELS = ["K18", "K19", "counter_clean"]
OOF_PATHS = {
...
K_DEPTH = {"K18": "deep30", "K19": "deep30", "counter_clean": "joint_anchor_pre_clean"}
K_PRE_CLEAN = {"K18": True, "K19": True, "counter_clean": True}
...
def _run_one_seed(kf_seed: int, P_unb: np.ndarray, y_unb: np.ndarray,
    ...
    """Per-fold SLSQP simplex with one kf_seed. Returns (pooled_rae, fold_records, oof_blend, fold_weights)."""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 3 anchors {K_LABELS}")
    print(f"          parents: nb2960 (K18 deep-30) + nb3000 (K19 deep-30)")
    print(f"                   + nb2490 (counter_clean joint, PRE-clean)")
    print(f"          paradigm: counter_clean as 3rd axis (biological orthogonality)")
    ...
    print("\n" + "-" * 78)
    print("STEP 1: load anchor OOFs and te arrays")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
        ...
        print(f"   {k:>14s} ({K_DEPTH[k]:>22s}): oof_RAE = {r:.4f}  "
    ...
    # Leak sanity -- counter_clean te has +0.41 in-sample optimism (expected
    # deploy-refit), but we only use the OOF column for fold-level optimisation
    # so per-fold SLSQP is honest on the K=3 stack.
    leak_flags = {}
    ...
    print(f"        {'  '.join([f'{k:>14s}' for k in K_LABELS])}")
    for i, ki in enumerate(K_LABELS):
        row = "  ".join([f"{corr_mat[i, j]:14.4f}" for j in range(K)])
        print(f"   {ki:>14s}  {row}")

    # Highlight orthogonality vs K18/K19 pair
    corr_K18_K19 = float(corr_mat[0, 1])
    corr_K18_cc = float(corr_mat[0, 2])
    corr_K19_cc = float(corr_mat[1, 2])
    print(f"\n   corr(K18, K19)            = {corr_K18_K19:.4f}")
    print(f"   corr(K18, counter_clean)  = {corr_K18_cc:.4f}")
    print(f"   corr(K19, counter_clean)  = {corr_K19_cc:.4f}")
    print(f"   counter_clean orthogonality (lower=better axis difference)")
    ...
    # -- Per-seed per-fold SLSQP simplex (15 seeds) --------------------------
    print("\n" + "-" * 78)
    ...
    for seed in KF_SEEDS:
        ...
        # mean per-fold weights (just for log readability)
        wmat = np.asarray(fold_ws)
        wmean = wmat.mean(axis=0)
        wstr = " ".join([f"{K_LABELS[k]}={wmean[k]:.2f}" for k in range(K)])
    ...
    sem_pooled = std_pooled / np.sqrt(len(KF_SEEDS)) if len(KF_SEEDS) > 1 else 0.0
    ...
    # -- Aggregate per-seed weights across folds ------------------------------
    all_weights = []
    for seed_key, wlist in per_seed_fold_weights.items():
        for fw in wlist:
            all_weights.append(fw)
    all_w_arr = np.asarray(all_weights, dtype=np.float64)  # (75, 3)
    mean_w_across_all = all_w_arr.mean(axis=0)
    mean_w_across_all = mean_w_across_all / mean_w_across_all.sum()
    print(f"\n   mean weights across {len(all_weights)} (seed,fold) cells:")
    for k in range(K):
        print(f"     w[{K_LABELS[k]:>14s}] = {mean_w_across_all[k]:+.4f}")
    ...
    for k in range(K):
        ...
        print(f"     w[{K_LABELS[k]:>14s}] = {w_full[k]:+.4f}{flag}")
    ...
    print("-" * 78)
    verdict = "BETTER_THAN_NB3030" if mean_pooled < GATE_BETTER_THAN_NB3030 else "FAIL"
    delta_vs_K18 = mean_pooled - REF_K18
    delta_vs_K19 = mean_pooled - REF_K19
    delta_vs_counter = mean_pooled - REF_COUNTER_CLEAN
    ...
    print(f"   delta vs counter_clean (0.5382) = {delta_vs_counter:+.4f}")
    ...
    print(f"   gate                          = mean_pooled < {GATE_BETTER_THAN_NB3030}")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_K18_K19_counter.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_counter_clean_15seed",
        "paradigm": "3_anchor_counter_clean_biological_axis",
        ...
        "corr_K18_K19": round(corr_K18_K19, 4),
        "corr_K18_counter_clean": round(corr_K18_cc, 4),
        "corr_K19_counter_clean": round(corr_K19_cc, 4),
        "n_folds": N_FOLDS,
        "kf_seeds": KF_SEEDS,
