"""nb3101 -- Finer w-pair sweep at q_cut=0.35 for K18/K19 quantile-conditional blend.

NEW PARADIGM:
    nb3090 finer-grid sweep PICKED (q_cut=0.35, w_low=0.95, w_high=0.40) at 15-seed
    mean 0.4472 +/- 0.0008. nb3100 deep-verify on FRESH seeds {1141..1155} reported
    mean 0.4476 (VERIFIED_MARGINAL: beats nb3070 0.4477 by -0.0001 but does NOT beat
    nb3080 0.4475 wide-seed ceiling). The (0.95, 0.40) pair was a corner of the
    nb3090 grid - we did not probe finer w_low/w_high spacing AT q_cut=0.35
    specifically. nb3101 fixes q_cut=0.35 and probes a 4x5=20 combo w-pair grid:

      w_K18_low  in {0.85, 0.90, 0.95, 1.00}   (corner-extended below+up to 1.00)
      w_K18_high in {0.30, 0.35, 0.40, 0.45, 0.50}  (0.05 granularity)

    Each combo evaluated on the SAME 15 fresh kf_seeds {1141..1155} as nb3100, so
    selection-bias comparison is paired (every combo sees the identical seed batch).

PROTOCOL (per combo, per kf_seed, 5-fold scaffold split):
    1. Compute fold-train K18 q_cut=0.35 quantile threshold q (per fold).
    2. For fold-val rows where pred_K18 <= q  -> blend(w_low K18 + (1-w_low) K19).
       For fold-val rows where pred_K18 >  q  -> blend(w_high K18 + (1-w_high) K19).
    3. Stitch into oof_blend (253,); pooled_rae across the 5 outer folds.
    Repeat for 15 kf_seeds; report mean per combo. Best combo by 15-seed mean.

GATES (on best combo's 15-seed mean):
    best_mean < 0.4472 -> "BETTER_THAN_NB3090" (NEW PRIMARY-1 candidate)
    best_mean < 0.4475 -> "PROMOTE"             (beats nb3080 wide-seed ceiling)
    else               -> "FAIL"

References:
    nb3090 finer-grid 15-seed best   = 0.4472 +/- 0.0008 (PARENT pick; under-dispersed)
    nb3100 fresh-seed verify nb3090  = 0.4476 (VERIFIED_MARGINAL; (0.95, 0.40) at q=0.35)
    nb3080 wide-seed verify nb3073   = 0.4475   <- PROMOTE gate
    nb3070 5-seed grid               = 0.4477
...
Outputs:
    data/processed/nb3101_summary.json
    data/processed/nb3101_pred_oof.npy  (253,) float32 -- best combo median-seed OOF
    data/processed/te_nb3101.npy        (513,) float32 -- best combo deploy te
    submissions/nb3101_w_pair_at_q035.csv (only on PROMOTE/BETTER_THAN_NB3090 verdicts)
"""
...
# -- CV protocol ---------------------------------------------------------------
N_FOLDS = 5
KF_SEEDS = list(range(1141, 1156))  # 15 fresh seeds {1141..1155} (matches nb3100)

# -- Sweep grid: fix q_cut=0.35, sweep w-pair --------------------------------
Q_CUT_FIXED = 0.35
W_LOW_GRID = [0.85, 0.90, 0.95, 1.00]
W_HIGH_GRID = [0.30, 0.35, 0.40, 0.45, 0.50]
...
def main() -> dict:
    t0 = time.time()
    n_combos = len(W_LOW_GRID) * len(W_HIGH_GRID)
    print("=" * 78)
    print(f"{TAG} -- finer w-pair sweep at q_cut={Q_CUT_FIXED} (K18, K19 deep-30)")
    print(f"          q_cut    = {Q_CUT_FIXED} (fixed)")
    print(f"          w_low    = {W_LOW_GRID}")
    print(f"          w_high   = {W_HIGH_GRID}")
    print(f"          combos   = {n_combos}")
    print(f"          kf_seeds = {KF_SEEDS}")
    print(f"          gates: best_mean < {GATE_BETTER_THAN_NB3090} -> BETTER_THAN_NB3090")
    print(f"                 best_mean < {GATE_PROMOTE} -> PROMOTE")
    print("=" * 78)
    ...
    for w_low in W_LOW_GRID:
        for w_high in W_HIGH_GRID:
            ...
            ts = time.time()
            res = _eval_combo(
                P_unb, y_unb, unb_scaffolds, Q_CUT_FIXED, w_low, w_high,
            )
            record = {
                "combo_idx": combo_i,
                "q_cut": Q_CUT_FIXED,
            ...
            print(f"   [{combo_i:>2d}/{n_combos}] q={Q_CUT_FIXED} "
    ...
    print(f"   ref nb3090 = {REF_NB3090:.4f}  (parent pick mean)")
    print(f"   ref nb3100 = {REF_NB3100:.4f}  (fresh-seed verify of (0.95, 0.40))")
    print(f"   ref nb3080 = {REF_NB3080:.4f}  (wide-seed ceiling)")
    print(f"   delta vs nb3090 = {best_combo['mean_rae'] - REF_NB3090:+.4f}")
    print(f"   delta vs nb3080 = {best_combo['mean_rae'] - REF_NB3080:+.4f}")
    ...
    print("GATE")
    print("-" * 78)
    if best_mean < GATE_BETTER_THAN_NB3090:
        verdict = "BETTER_THAN_NB3090"
        ladder_action = (
            f"PROMOTE candidate as NEW PRIMARY-1. nb3101 finer w-pair sweep at "
            f"q_cut={Q_CUT_FIXED} best combo "
            f"(w_low={best_combo['w_K18_low']}, w_high={best_combo['w_K18_high']}) "
            f"15-seed mean {best_mean:.4f} beats nb3090 parent ceiling "
            f"{REF_NB3090:.4f} ({best_mean - REF_NB3090:+.4f}). "
            "Note: 20-combo selection on identical 15-seed batch as nb3100 -- "
            "deep-verify on fresh seeds recommended before final promote (per "
            "cycle-149 nb1191 / cycle-160 deep-30 rule)."
        )
    elif best_mean < GATE_PROMOTE:
        verdict = "PROMOTE"
        ladder_action = (
            f"PROMOTE candidate. nb3101 finer w-pair sweep at q_cut={Q_CUT_FIXED} "
            f"best combo (w_low={best_combo['w_K18_low']}, "
            f"w_high={best_combo['w_K18_high']}) "
            f"15-seed mean {best_mean:.4f} beats nb3080 wide-seed ceiling "
            f"{REF_NB3080:.4f} ({best_mean - REF_NB3080:+.4f}) but does NOT "
            f"beat nb3090 parent {REF_NB3090:.4f}. Inside PROMOTE band; "
            "deep-verify on fresh seeds before deploy."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3101 finer w-pair sweep at q_cut={Q_CUT_FIXED} best combo "
            f"(w_low={best_combo['w_K18_low']}, w_high={best_combo['w_K18_high']}) "
            f"15-seed mean {best_mean:.4f} does not beat nb3080 ceiling "
            f"{REF_NB3080:.4f} ({best_mean - REF_NB3080:+.4f}). "
            "Finer w-pair spacing at q_cut=0.35 does not break the post-hoc-blend "
            "ceiling on this anchor pair. Keep nb3090 / prior PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_w_pair_at_q035.csv"
    if verdict in ("BETTER_THAN_NB3090", "PROMOTE"):
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "finer_w_pair_at_q035_K18_K19_deep30",
        ...
        "q_cut_fixed": Q_CUT_FIXED,
        "w_low_grid": W_LOW_GRID,
        "w_high_grid": W_HIGH_GRID,
        "n_combos": n_combos,
        "n_folds": N_FOLDS,
        "kf_seeds": KF_SEEDS,
        ...
        "delta_vs_nb3090": round(best_mean - REF_NB3090, 4),
        ...
        "submission_csv": str(sub_csv) if verdict in ("BETTER_THAN_NB3090", "PROMOTE") else None,
    ...
    print(f"   delta vs nb3090               = {best_mean - REF_NB3090:+.4f}")
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "best_mean_rae", "delta_vs_nb3090", "delta_vs_nb3080",
