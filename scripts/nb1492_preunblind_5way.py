...
PROTOCOL
    ...
    3. For each FAMILY in [AtomPair-2048, MACCS-167, Mordred-1533,
         ...
         c. Pick top-K (30 / 20 / 30 / 30 / 30) family columns by importance.
    ...
    6. Pool RAE for both.
    7. Verdict at 0.003 margin vs nb1484 (0.5231; fallback nb1472 0.5330).

Outputs:
    scripts/nb1492_preunblind_5way.py            (this file)
    data/processed/nb1492_summary.json
    data/processed/nb1492_best_oof.npy            (253,) float32  best variant
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- PRE-unblind 5-way: chemprop_aux anchor + "
          f"AtomPair-30 + MACCS-20 + Mordred-30 + ChempropEmbed-30 + Avalon-30")
    ...
    if beats_nb1484_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_BEATS_NB1484_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1484_best:
        verdict = f"PRE_UNBLIND_5WAY_FLAT_VS_NB1484_best_{which_best}"
    elif beats_nb1472_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_BEATS_NB1472_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif beats_anchor_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "PRE_UNBLIND_5WAY_FLAT_VS_ANCHOR"
    else:
        verdict = "PRE_UNBLIND_5WAY_HURTS_ANCHOR"
    ...
    print(f"   d_best_vs_nb1484    = {delta_best_vs_nb1484:+.4f}   "
          f"(best = {which_best})")
    print(f"   d_naive_vs_nb1472   = {delta_naive_vs_nb1472:+.4f}")
    print(f"   d_slsqp_vs_nb1472   = {delta_slsqp_vs_nb1472:+.4f}")
