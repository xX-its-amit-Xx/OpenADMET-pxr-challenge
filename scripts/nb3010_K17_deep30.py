"""nb3010 -- DEEP-30 fresh-seed K=17 RFE pyramid.

CONTEXT (cycle 247+):
    nb3000 deep-30 K=19 rebuild added value over the 5-seed cache
    (cycle 246 K-pyramid {K18, K19, K20} simplex).  Now probe K=17 to see
    if an even smaller K opens new optimization space below the
    nb2171/nb2095 0.4720 post-hoc-blend ceiling.

    nb2631 5-seed K=17 = 0.4692 (per_K_oof_rae cache).  Per cycle-160
    deep-30 rule (nb2060/nb2095 4x under-dispersion), the 5-seed bag is
    optimistically biased; a deep-30 rebuild is the gate-grade evaluation.

PROTOCOL:
    1. Rebuild K=17 with 30 fresh seeds {3001..3030} using the SAME residual-
       LGBM recipe as nb3000 / nb2960 / nb2631 (chemprop_aux anchor + residual
       LGBM on the K=17 feature slice of the 117-col matrix).
    2. K=17 feature indices reconstructed from nb2231 RFE trajectory via
       `reconstruct_K_from_trajectory` (same path as nb2631 / nb3000).
    3. Save deep-30 bag-mean OOF (253,) and te (513,).
    4. Compare deep-30 K=17 RAE vs nb2631 5-seed K=17 RAE (0.4692).
...
Inputs:
    ...
    data/processed/nb2231_summary.json   (K=17 idx reconstruction via traj)
    ...
    data/processed/nb2631_mean_bag_oof_K17.npy   (5-seed reference)

Outputs:
    data/processed/nb3010_summary.json
    data/processed/nb3010_K17_30seed_oof.npy   (253,) float32
    data/processed/te_nb3010_K17.npy           (513,) float32
"""
...
# -- Anchor + residual params (IDENTICAL recipe to nb3000 / nb2960 / nb2631) ---
...
# Reference 5-seed K=17 cache (for comparison)
NB2631_K17_OOF = DATA_PROCESSED / "nb2631_mean_bag_oof_K17.npy"
NB2631_K17_TE = DATA_PROCESSED / "te_nb2631_K17.npy"
...
NB2631_K17_5SEED_REF = 0.4692      # nb2631 5-seed K=17 RAE
CHEMPROP_AUX_REF = 0.6216


# ============================================================================
# helpers (lifted verbatim from nb3000 / nb2960 / nb2631)
# ============================================================================
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEEP-30 fresh-seed K={K_TARGET} RFE pyramid")
    print(f"          fresh seeds = {RESID_SEEDS_DEEP[0]}..{RESID_SEEDS_DEEP[-1]} "
          f"(n={len(RESID_SEEDS_DEEP)})")
    print(f"          5-seed reference RAE = {NB2631_K17_5SEED_REF:.4f} (nb2631)")
    print("=" * 78)
    ...
    with open(NB2231_SUMMARY) as f:
        nb2231 = json.load(f)
    K17_idx = reconstruct_K_from_trajectory(nb2231, K_TARGET)
    K17_idx = np.array(K17_idx, dtype=int)
    if len(K17_idx) != K_TARGET:
        raise ValueError(
            f"K={K_TARGET} idx reconstruction returned {len(K17_idx)} cols"
        )
    print(f"   K={K_TARGET} idx_in_117 (n={len(K17_idx)}): {K17_idx.tolist()}")
    ...
    bag_oof, bag_te, per_seed_rae = build_K_30seed_bag(
        f"K{K_TARGET}", K17_idx, X_te_full, unb_idx, anchor, residual,
    ...
    if NB2631_K17_OOF.exists():
        cached_oof = np.load(NB2631_K17_OOF).astype(np.float64)
        if cached_oof.shape == (n_unb,):
            cached_5seed_rae = float(rae(y_unb, cached_oof))
            delta_30_minus_5 = bag_mean_rae - cached_5seed_rae
            print(f"   5seed_RAE        = {cached_5seed_rae:.4f}  (nb2631 K=17)")
        ...
        print(f"   WARN: 5seed cache missing {NB2631_K17_OOF}")
    print(f"   task spec ref   = {NB2631_K17_5SEED_REF:.4f}  (per task description)")
    delta_vs_task_ref = bag_mean_rae - NB2631_K17_5SEED_REF
    ...
    summary = {
        "tag": TAG,
        "method": "deep30_freshseed_K_rebuild_K17_only",
        "K_target": K_TARGET,
        "anchor_base": "chemprop_aux",
        "anchor_in_rae": rae_anchor,
        "anchor_pre_unblind": True,
        "K17_idx_in_117col": K17_idx.tolist(),
        ...
        "K17_30seed_bag_mean_rae": bag_mean_rae,
        "K17_cached_5seed_rae": cached_5seed_rae,
        "K17_delta_30_minus_5": delta_30_minus_5,
        "task_spec_ref_5seed_rae": NB2631_K17_5SEED_REF,
        "delta_vs_task_spec_ref": delta_vs_task_ref,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "K17_oof_path": str(oof_path),
        "K17_te_path": str(te_path),
    ...
    print(f"   task spec ref (5-seed)        = {NB2631_K17_5SEED_REF:.4f}")
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "K17_30seed_bag_mean_rae",
        "K17_cached_5seed_rae",
        "K17_delta_30_minus_5",
        "per_seed_rae_mean",
        "per_seed_rae_std",
        "per_seed_rae_min",
        "per_seed_rae_max",
