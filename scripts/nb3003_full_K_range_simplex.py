"""nb3003 -- Per-fold SLSQP simplex on full K-range {K18, K19, K20, K21, K22}.

NEW PARADIGM: extended K-range simplex; takes nb2992 (3-anchor {K18, K19, K20}
RAE 0.4479) and adds K=21 + K=22 as 5-seed cached anchors for orthogonal
sensitivity at the upper-K tail. K=19 is now the nb3000 deep-30 build
(NOT nb2631 5-seed cached), giving 3 of 5 anchors at deep-30 depth.
...
PROTOCOL:
    Anchors (5):
        ...
        K19 (deep-30): nb3000
        K20 (deep-30): nb2960
        K21 (5-seed):  nb2631 cached
        K22 (5-seed):  nb2261 cached
    ...
    Deploy:
        ...
        - Apply to (513, 5) stacked te arrays -> te_nb3003

GATE:
    mean_rae < 0.4479  -> "BETTER_THAN_NB2992"
    mean_rae < 0.4518  -> "PROMOTE"
    else               -> "FAIL"
...
References:
    ...
    nb2631 K21 5-seed OOF         (cached)
    nb2261 K22 5-seed OOF         (cached)
    nb2992 per-fold simplex 3K    = 0.4479  <- gate
    nb2982 per-fold simplex 2K    = 0.4505
...
Outputs:
    data/processed/nb3003_summary.json
    data/processed/nb3003_pred_oof.npy   (253,) float32 -- per-fold simplex OOF
    data/processed/te_nb3003.npy         (513,) float32 -- deploy te
    submissions/nb3003_full_K_range_simplex.csv (only if verdict != "FAIL")
"""
...
TAG = "nb3003"
PARENT_TAG = "nb2960+nb3000+nb2631+nb2261"
...
# -- Gates ---------------------------------------------------------------------
GATE_BETTER_THAN_NB2992 = 0.4479
GATE_PROMOTE = 0.4518
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 5 K-anchors {K_LABELS}")
    print(f"          parents: nb2960 (K18,K20 deep-30) + nb3000 (K19 deep-30)")
    print(f"                   + nb2631 (K21 5-seed) + nb2261 (K22 5-seed)")
    print(f"          motivation: extend nb2992 (K18,K19,K20) with K21,K22 tail")
    ...
    print("STEP 1: load K-anchor OOFs and te arrays "
          "(K18,K19,K20 deep-30; K21,K22 5-seed)")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
    ...
    delta_vs_K19 = pooled_rae - REF_K19_DEEP30
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_full_K_range_simplex.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_K20_K21_K22",
        "paradigm": "5_anchor_extend_nb2992_with_K21_K22_tail_5seed",
        ...
        "ref_nb2992_3K": REF_NB2992,
        "ref_nb2982_2K": REF_NB2982,
