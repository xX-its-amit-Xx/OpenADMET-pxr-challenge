"""nb3201 -- Learned per-fold clip on K=18 deep-30 alone (single anchor).

NEW PARADIGM:
    Simplest possible learned-clip. Strip away every blend layer (no pyramid,
    no SLSQP, no rank-stretch) and apply the per-fold (q_low, q_high) grid
    learned-clip primitive directly to K=18 deep-30 OOF -- the single
    cleanest PRE-unblind LGBM-on-SHAP-28 anchor at honest scaffold-CV
    RAE 0.4536.

    Hypothesis: K=18 deep-30 alone has the strongest variance-compressed
    tails (only 30 LGBM seeds, no blend smoothing). A learned-clip should
    therefore extract MORE per-fold tail-taming gain on K=18 alone than on
    blended children (nb3080 wide-bag), because the latter already averages
    out tail outliers. If K=18 + learned-clip beats nb3170 fixed 0.4437 or
    nb3173 learned variants on nb3080, then the clip primitive is anchor-
    invariant and the parent blend is contributing noise, not signal.

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb2960_K18_30seed_oof  (253,)
    Per outer fold:
...
GATE (on 15-seed mean):
    mean < 0.4426 -> "BETTER"   (matches user gate)
    mean < 0.4437 -> "MARGINAL" (matches user gate; would beat nb3170 fixed)
    else          -> "FAIL"

References:
    nb2960 K=18 deep-30 OOF RAE     = 0.4536  <- parent anchor
    nb2991 K=18 wide-seed mean      = 0.4536  (variance check)
...
Outputs:
    data/processed/nb3201_summary.json
    data/processed/nb3201_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3201.npy         (513,) float32 -- deploy te
    submissions/nb3201_learned_clip_K18.csv  (only on BETTER/MARGINAL verdict)
"""
...
# -- Inputs --------------------------------------------------------------------
K_LABEL = "K18"
PRED_OOF_PATH = DATA_PROCESSED / f"nb2960_{K_LABEL}_30seed_oof.npy"
...
# -- Per-fold grid (smaller / simpler than nb3173) -----------------------------
...
def main() -> dict:
    ...
    print(
        ...
        f"{PARENT_TAG} K=18 deep-30 OOF (single anchor)"
    )
    ...
    # -- Load K=18 deep-30 anchor pred_oof + te -------------------------------
    print("\n" + "-" * 78)
    ...
    print(
        f"\n   ref K=18 deep-30 OOF          = {REF_K18_DEEP30:.4f}  "
        f"<- parent (no clip)"
    )
    print(
        f"   delta vs K=18 deep-30 parent  = "
        f"{mean_rae - REF_K18_DEEP30:+.4f}"
    )
    ...
    print(f"   ref nb3080 wide-seed mean    = {REF_NB3080:.4f}")
    print(
        f"   delta vs nb3080               = "
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3201 15-seed mean {mean_rae:.4f} clears "
            f"BETTER gate {GATE_BETTER:.4f}. Per-fold LEARNED clip on K=18 "
            f"deep-30 alone (single anchor, no blend) beats prior post-hoc "
            f"ceilings. Modal pick was (q{ql_mode:.2f}, q{qh_mode:.2f}). "
            f"Confirms clip primitive is anchor-invariant -- the parent "
            f"blend was contributing noise. Re-verify deep-30 before "
            f"PRIMARY-1 swap."
        )
    elif mean_rae < GATE_MARGINAL:
        verdict = "MARGINAL"
        ladder_action = (
            f"MARGINAL. nb3201 15-seed mean {mean_rae:.4f} clears MARGINAL "
            f"gate {GATE_MARGINAL:.4f} but not BETTER {GATE_BETTER:.4f}. "
            f"K=18 + learned-clip ties nb3170 fixed-band on nb3080 wide-bag "
            f"({mean_rae - REF_NB3170_FIXED:+.4f}). Anchor-simplification "
            f"recovers most of the blend value at lower variance. Hold as "
            f"alternate; reproducible with deep-30 may upgrade."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3201 15-seed mean {mean_rae:.4f} fails both gates "
            f"(BETTER {GATE_BETTER:.4f}, MARGINAL {GATE_MARGINAL:.4f}). K=18 "
            f"deep-30 alone + learned-clip does NOT match nb3170 fixed-band "
            f"({mean_rae - REF_NB3170_FIXED:+.4f}). The parent blend (nb3080) "
            f"is contributing real signal that K=18 alone cannot recover. "
            f"Keep current PRIMARY-1; clip primitive is blend-dependent."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_learned_clip_K18.csv"
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_K18_deep30_alone"
        ),
        ...
        "delta_vs_K18_deep30": round(mean_rae - REF_K18_DEEP30, 4),
        "ref_nb2991_wide": REF_NB2991_WIDE,
    ...
    print(f"   95% CI                = [{ci_low:.4f}, {ci_high:.4f}]")
    print(f"   delta vs K=18 parent  = {mean_rae - REF_K18_DEEP30:+.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_K18_deep30", "delta_vs_nb3170_fixed", "delta_vs_nb3080",
        "ql_mode", "qh_mode",
