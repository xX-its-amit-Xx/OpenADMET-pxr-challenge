"""nb1534 -- Best-K Mordred + Best-K AtomPair PRE-unblind 4-way blend.

Extends nb1484's 4-way (AtomPair-30, MACCS-20, Mordred-30, ChempropEmbed-30)
by swapping AtomPair-30 -> AtomPair-K* and Mordred-30 -> Mordred-K* where
K* is the cycle-57 grid winner per family:

    Mordred K*    = nb1523 best_K   (20)
    AtomPair K*   = nb1524 best_K   (25)

Holds MACCS K=20 and ChempropEmbed K=30 unchanged from nb1484.

PROTOCOL
    1. Anchor   = chemprop_aux_te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
    2. ChEMBL pool + Tanimoto kNN-5 (same union as nb1472 / nb1484).
    3. For each family in [AtomPair, MACCS, Mordred, ChempropEmbed]:
         a. Build FULL matrix on 253 = family_full + pred_chembl + sim.
         b. Train one seed-0 shallow LGBM Huber on residual.
         c. Compute SHAP importance (TreeExplainer; fallback gain).
         d. Pick top-K family columns.
         e. PRUNED matrix = top-K + pred_chembl + sim.
         f. 5-seed bag (seeds [0,1,7,42,137]), KFold(5) cross-fit on
            shallow LGBM Huber (depth=3, leaves=7, n_est=80, lr=0.05,
            huber_alpha=1.0, min_child_samples=20).
         g. Mean-bag pred_corr = anchor + mean_bag(residual_oof).
    4. Naive 1/4 mean blend across the 4 corrected OOFs.
    5. 5-fold SLSQP cross-fit (simplex K=4) on the 4 corrected OOFs.
    6. Pool RAE per family + per blend.
    7. Verdict at 0.003 margin vs nb1484 (0.5231).

Outputs:
    scripts/nb1534_best_K_preunblind.py        (this file)
    data/processed/nb1534_summary.json
    data/processed/nb1534_best_oof.npy          (253,) float32 best variant
"""
...
# Default K config (will be overridden by nb1523/nb1524 best_K at runtime
# if those summaries exist).  MACCS and ChempropEmbed inherited from nb1484.
TOP_K_DEFAULT = {
    "AtomPair": 25,        # nb1524 best
    "MACCS": 20,           # nb1484 default
    "Mordred": 20,         # nb1523 best
    "ChempropEmbed": 30,   # nb1484 default
}
...
def _load_best_K_config() -> dict:
    """Pull best_K from nb1523 (Mordred) and nb1524 (AtomPair) summaries.

    Falls back to TOP_K_DEFAULT entries if a summary file is missing.
    """
    cfg = dict(TOP_K_DEFAULT)
    src = {}

    p1523 = DATA_PROCESSED / "nb1523_summary.json"
    if p1523.exists():
        with open(p1523, "r") as f:
            s1523 = json.load(f)
        bK = int(s1523.get("best_K", cfg["Mordred"]))
        cfg["Mordred"] = bK
        src["Mordred"] = {
            "K": bK,
            "rae_mean_bag": float(s1523.get("best_K_rae_mean_bag", -1.0)),
            "source": "nb1523_summary.json",
        }
    else:
        src["Mordred"] = {"K": cfg["Mordred"], "source": "DEFAULT"}

    p1524 = DATA_PROCESSED / "nb1524_summary.json"
    if p1524.exists():
        with open(p1524, "r") as f:
            s1524 = json.load(f)
        bK = int(s1524.get("best_K", cfg["AtomPair"]))
        cfg["AtomPair"] = bK
        src["AtomPair"] = {
            "K": bK,
            "rae_mean_bag": float(s1524.get("best_rae_mean_bag", -1.0)),
            "source": "nb1524_summary.json",
        }
    else:
        src["AtomPair"] = {"K": cfg["AtomPair"], "source": "DEFAULT"}

    src["MACCS"] = {"K": cfg["MACCS"], "source": "nb1484_DEFAULT"}
    src["ChempropEmbed"] = {"K": cfg["ChempropEmbed"], "source": "nb1484_DEFAULT"}
    return cfg, src


def main() -> dict:
    t0 = time.time()
    top_k_cfg, top_k_src = _load_best_K_config()

    print("=" * 78)
    print(f"{TAG} -- Best-K Mordred + Best-K AtomPair PRE-unblind 4-way")
    print(f"          anchor = {ANCHOR}  (PRE-unblind te[unb_idx])")
    print(f"          K config (from nb1523/nb1524 + nb1484 defaults):")
    for fam in FAMILIES:
        info = top_k_src[fam]
        if "rae_mean_bag" in info and info["rae_mean_bag"] >= 0:
            print(f"             {fam:<14s} K={info['K']:>3d}  "
                  f"(src={info['source']}  best_K_rae={info['rae_mean_bag']:.4f})")
        else:
            print(f"             {fam:<14s} K={info['K']:>3d}  "
                  f"(src={info['source']})")
    ...
    # ---- Run 4 families with best-K config ----
    fam_results = []
    ...
    for r in fam_results:
        print(f"   {r['family']:<14s} (K={r['top_k']}) mean_bag RAE = "
              f"{r['rae_mean_bag']:.4f}")
    ...
    print("=" * 78)
    P_4 = per_family_mean_bag.T.astype(np.float64)
    ...
    if beats_nb1484_best:
        verdict = (
            f"BEST_K_4WAY_BEATS_NB1484_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1484_best:
        verdict = f"BEST_K_4WAY_FLAT_VS_NB1484_best_{which_best}"
    elif beats_anchor_best:
        verdict = (
            f"BEST_K_4WAY_BEATS_ANCHOR_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "BEST_K_4WAY_FLAT_VS_ANCHOR"
    else:
        verdict = "BEST_K_4WAY_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "top_k_config": top_k_cfg,
        "top_k_sources": top_k_src,
        "families": [
            {
                "family": r["family"],
