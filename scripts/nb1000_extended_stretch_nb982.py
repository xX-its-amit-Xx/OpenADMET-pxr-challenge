"""nb1000 -- EXTENDED RANK STRETCH on nb982.

Extends nb988's stretch sweep (which found 0.5863 at s=1.25 -- a GRID EDGE)
into the high-stretch regime to locate the true optimum and verify whether
the curve is still monotonically decreasing at s=1.25.

Procedure (per user spec):
  1) Load te_nb982.npy (513 deploy preds, in_RAE=0.6107 at s=1.0).
  2) Grid s in {1.00, 1.10, 1.20, 1.25, 1.30, 1.40, 1.50, 1.60, 1.70, 1.80,
                2.00, 2.25, 2.50}.
  3) For each s: stretched = mu + s * (te_nb982 - mu); in_RAE on 253.
  4) Report whether the curve still monotonically decreases (optimum may
     lie even higher than 2.50).
  5) Plot the curve, save the grid, save the best as a submission CSV.
"""
...
ANCHOR = "nb982"
STRETCH_GRID = [
    1.00, 1.10, 1.20, 1.25, 1.30, 1.40, 1.50, 1.60, 1.70, 1.80,
    2.00, 2.25, 2.50,
]
...
def main() -> dict:
    print("=" * 78)
    print(f"{TAG} -- EXTENDED RANK STRETCH on {ANCHOR}")
    print("=" * 78)
    ...
    print("\n" + "-" * 78)
    print(f"EXTENDED GRID STRETCH IN-SAMPLE ON 253  grid={STRETCH_GRID}")
    print("-" * 78)
    ...
    # =================================================================
    raes = sweep_df["in_rae_253"].values
    diffs = np.diff(raes)
    strictly_decreasing = bool(np.all(diffs < 0))
    monotone_to_best = True
    best_idx = int(np.argmin(raes))
    if best_idx < len(raes) - 1:
        monotone_to_best = bool(np.all(diffs[:best_idx] < 0))
    edge_optimum = bool(best_idx == len(raes) - 1)
    print("\n" + "-" * 78)
    print("MONOTONICITY DIAGNOSTIC")
    print("-" * 78)
    print(f"  strictly decreasing over full grid  = {strictly_decreasing}")
    print(f"  monotone decreasing UP TO best s    = {monotone_to_best}")
    print(f"  best s at grid edge (s={STRETCH_GRID[-1]})        = "
          f"{edge_optimum}")
    if strictly_decreasing or edge_optimum:
        print("  -> curve has not turned; true optimum may lie HIGHER")
    else:
        print(f"  -> curve turns around s={best_s:.2f}  "
              f"(true optimum bracketed)")
    ...
    plt.axhline(rae_anchor_in, color="#7f7f7f", ls=":",
                lw=1, label=f"{ANCHOR} s=1.0  RAE={rae_anchor_in:.4f}")
    plt.xlabel("stretch factor s")
    plt.ylabel("in-sample RAE on 253 unblind")
    plt.title(f"{TAG} -- extended stretch sweep on te_{ANCHOR}")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    ...
    summary = {
        ...
        "monotone_to_best": monotone_to_best,
        "edge_optimum": edge_optimum,
    ...
    print(f"  strictly_decreasing       = {strictly_decreasing}")
    print(f"  monotone_to_best          = {monotone_to_best}")
    print(f"  edge_optimum              = {edge_optimum}")
