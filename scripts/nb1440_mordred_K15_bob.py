"""nb1440 -- Outer-bag VALIDATION of Mordred top-K=15 (SHAP-pruned) + ChEMBL.

Protocol:
    1. Reuse first 15 indices of the nb1364 SHAP-ranked Mordred top list.
    2. For each OUTER seed o in {0, 1, 7, 42, 137}:
         ...
         - For each inner seed: 5-fold cross-fit (KFold random_state=inner)
           of shallow LGBM Huber on the (K=15 Mordred + pred_chembl + sim) 17-col
    ...
    4. Verdict at 0.003 margin vs:
         - nb1412 K=20 standalone (0.5180)
         - nb1423 BoB (Mordred K=20 outer-bag mean, 0.5135)

Outputs:
    scripts/nb1440_mordred_K15_bob.py        (this file)
    data/processed/nb1440_summary.json
    data/processed/nb1440_bob_mean_oof.npy   (253,) float32
    data/processed/nb1440_bob_median_oof.npy (253,) float32
"""
...
# Reference RAEs to score the verdict against.
NB1412_K20_REF = 0.5180     # nb1412 standalone K=20 mean-bag pooled RAE
NB1423_BOB_REF = 0.5135     # nb1423 BoB mean pooled RAE (outer-bag of K=20)
NB1070_REF = 0.5771
DECISION_MARGIN = 0.003

# Top-15 SHAP-ranked Mordred column indices = first 15 of nb1364 ranking.
NB1364_TOP15_IDX = [
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1412 / nb1423 / nb1364."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Outer-bag (Mordred K=15 SHAP-pruned + ChEMBL, anchor={ANCHOR})")
    print(f"          OUTER_SEEDS = {OUTER_SEEDS}")
    print(f"          INNER_BASE  = {INNER_BASE}  (inner = outer*1000 + base)")
    print(f"          REFS: nb1412 K=20 = {NB1412_K20_REF:.4f}   "
          f"nb1423 BoB = {NB1423_BOB_REF:.4f}   margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    top_idx_arr = np.array(NB1364_TOP15_IDX, dtype=int)
    X_mord_unb_top15 = X_mord_te[unb_idx][:, top_idx_arr].astype(np.float32)
    print(f"[feat] top-15 Mordred slice on unblind: {X_mord_unb_top15.shape}")

    # ---- ChEMBL pool + kNN features ----
    print("\n" + "-" * 78)
    print("CHEMBL PXR POOL (local cache; same union as nb1412/nb1423/nb1364)")
    print("-" * 78)
    ...
    # ---- Build 17-col matrix on unblind ----
    ...
    print(f"\n[feat] X_unb shape = {X_unb.shape}  (Mordred-15 + pred_chembl + sim)")
    ...
    for oi, o in enumerate(OUTER_SEEDS):
        ...
        rec = {
            ...
            "delta_vs_nb1412_K20": float(outer_rae - NB1412_K20_REF),
            "delta_vs_nb1423_bob": float(outer_rae - NB1423_BOB_REF),
            "wall_sec": round(dt, 2),
        }
        ...
        print(f"   outer={o:3d}  inner_mean={rec['inner_mean_rae']:.4f}+/-"
              ...
              f"d_vs_K20={rec['delta_vs_nb1412_K20']:+.4f}  "
              f"d_vs_BoB={rec['delta_vs_nb1423_bob']:+.4f}  [{dt:.1f}s]")
    ...
    # ---- Verdict ----
    # Compare BoB mean (preferred row-level estimator) to two refs.
    delta_bob_vs_K20 = rae_bob_mean - NB1412_K20_REF
    delta_bob_vs_bob_ref = rae_bob_mean - NB1423_BOB_REF
    delta_outer_vs_K20 = per_outer_mean - NB1412_K20_REF
    delta_outer_vs_bob_ref = per_outer_mean - NB1423_BOB_REF
    ...
    cls_bob_vs_K20 = _classify(delta_bob_vs_K20)
    cls_bob_vs_bob_ref = _classify(delta_bob_vs_bob_ref)

    if cls_bob_vs_bob_ref == "BEATS":
        verdict = "NB1440_BEATS_NB1423_BOB_AND_NB1412_K20"
    elif cls_bob_vs_bob_ref == "TIES" and cls_bob_vs_K20 in ("TIES", "BEATS"):
        verdict = "NB1440_TIES_NB1423_BOB"
    elif cls_bob_vs_bob_ref == "LOSES" and cls_bob_vs_K20 == "BEATS":
        verdict = "NB1440_BEATS_NB1412_K20_LOSES_NB1423_BOB"
    elif cls_bob_vs_bob_ref == "LOSES" and cls_bob_vs_K20 == "TIES":
        verdict = "NB1440_TIES_NB1412_K20_LOSES_NB1423_BOB"
    else:
        verdict = "NB1440_LOSES_BOTH_REFS"
    ...
    print(f"   nb1412 K=20 reference = {NB1412_K20_REF:.4f}")
    print(f"   nb1423 BoB  reference = {NB1423_BOB_REF:.4f}")
    print(f"   delta(outer_mean - nb1412 K=20) = {delta_outer_vs_K20:+.4f}")
    print(f"   delta(outer_mean - nb1423 BoB ) = {delta_outer_vs_bob_ref:+.4f}")
    print(f"   BoB MEAN   pooled RAE = {rae_bob_mean:.4f}  "
          f"d_K20={delta_bob_vs_K20:+.4f} ({cls_bob_vs_K20})  "
          f"d_BoB={delta_bob_vs_bob_ref:+.4f} ({cls_bob_vs_bob_ref})")
    ...
    summary = {
        ...
        "K_mordred": 15,
        "nb1364_top15_col_idx": NB1364_TOP15_IDX,
        ...
        "nb1412_K20_ref": NB1412_K20_REF,
        "nb1423_bob_ref": NB1423_BOB_REF,
        ...
        "delta_per_outer_mean_vs_nb1412_K20": delta_outer_vs_K20,
        "delta_per_outer_mean_vs_nb1423_bob": delta_outer_vs_bob_ref,
        "delta_bob_mean_vs_nb1412_K20": delta_bob_vs_K20,
        "delta_bob_mean_vs_nb1423_bob": delta_bob_vs_bob_ref,
        "delta_bob_median_vs_nb1412_K20": rae_bob_median - NB1412_K20_REF,
        "delta_bob_median_vs_nb1423_bob": rae_bob_median - NB1423_BOB_REF,
        "class_bob_mean_vs_nb1412_K20": cls_bob_vs_K20,
        "class_bob_mean_vs_nb1423_bob": cls_bob_vs_bob_ref,
        "verdict": verdict,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_bob_mean_vs_nb1412_K20",
        "delta_bob_mean_vs_nb1423_bob",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
