"""nb1332 — PPARgamma (NR1C3) Tox21 agonist qHTS as cross-NR READ-ACROSS feature.

PPARg shares PXR's promiscuous lipophilic pocket; many Tox21 chemicals cross-activate
PXR+PPARg. We DON'T augment training (binding/other-target != PXR activation). Instead:
build a leakage-safe cross-target read-across: train a PPARg pAC50 predictor on 6551
Tox21 PPARg compounds (NEVER sees PXR pEC50), predict PPARg-activity for our 4139 train
+ 513 test -> use that single prediction as ONE extra feature appended to the deployed
4-GBM combined-feature ensemble. Gate: matched scaffold-CV control vs treat, 3 seeds.
...
# ── PPARg data
ded = pickle.load(open("C:/pxr_work/ppar_tox21/ppar_std.pkl", "rb"))
ppar_smi = [r[1] for r in ded]
ppar_y   = np.array([r[4] for r in ded], float)
print(f"PPARg compounds: {len(ppar_smi)} (actives={sum(1 for r in ded if r[3]=='Active')})")
...
# ── coverage: PPARg actives -> our 513 test
act_smi = [r[1] for r in ded if r[3] == "Active"]
...
print(f"Coverage (PPARg actives->test): median Tanimoto={med_tani:.3f}  frac>=0.4={frac_cov:.3f}")

# ── read-across: PPARg predictor (combined feats), leakage-safe re: PXR labels
print("Featurizing PPARg + our train/test ...")
Xppar = impute(combined(ppar_smi)).astype(np.float32)
Xtr   = impute(combined(tr_smi)).astype(np.float32)
Xte   = impute(combined(te_smi)).astype(np.float32)
ppar_model = LGBMRegressor(n_estimators=600, num_leaves=64, learning_rate=0.04, n_jobs=4, verbose=-1)
ppar_model.fit(Xppar, ppar_y)
ra_tr = ppar_model.predict(Xtr).astype(np.float32)
ra_te = ppar_model.predict(Xte).astype(np.float32)
print(f"read-across feat: train range [{ra_tr.min():.2f},{ra_tr.max():.2f}]  "
      f"corr(read-across, PXR pEC50)={np.corrcoef(ra_tr, y)[0,1]:+.3f}")
np.save("C:/pxr_work/ppar_tox21/ra_tr.npy", ra_tr)
np.save("C:/pxr_work/ppar_tox21/ra_te.npy", ra_te)
...
for seed in range(N_SEEDS):
    ...
    if seed == 0:
        np.save("C:/pxr_work/ppar_tox21/oof_treat.npy", treat)
        np.save("C:/pxr_work/ppar_tox21/oof_ctrl.npy", ctrl)
        np.save("C:/pxr_work/ppar_tox21/y.npy", y)
...
corr_err = float(np.corrcoef(ra_tr, np.abs(y - np.load("C:/pxr_work/ppar_tox21/oof_ctrl.npy")))[0,1])
deploy = delta < -0.001 and treat_mean < BEST_RAE

res = {
    "feature": "ppar_gamma_tox21_readacross",
    "n_ppar": len(ppar_smi), "n_actives": int(sum(1 for r in ded if r[3]=="Active")),
...
json.dump(res, open(f"{P}/nb1332_ppar_readacross_gate.json","w"), indent=2)
print("\n=== PPARg READ-ACROSS GATE ===")
