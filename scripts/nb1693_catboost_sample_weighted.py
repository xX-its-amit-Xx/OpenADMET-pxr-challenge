"""nb1693 -- CatBoost on 117-col 5-way K-tuned with sample weighting by
similarity to closest ChEMBL.

HYPOTHESIS:
    Sample weighting by `sim_to_chembl_top1` may help with the
    ChEMBL-distant test compounds.  nb1554 (unweighted 5-way K-tuned
    CatBoost) is the comparison anchor at RAE = 0.5163.  If the
    residual-correction model can be told to emphasize the ChEMBL-near
    unblind rows during fitting (where the kNN-pec50 feature is more
    reliable), the OOF RAE may move below 0.5163.

PROTOCOL:
    1. Build the same 117-col 5-way K-tuned feature matrix as nb1554:
       top-25 AP + top-20 MACCS + top-20 Mord + top-20 chemprop embed
       + top-30 Avalon + pred_chembl_pec50 + mean_sim.
    2. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    3. Sample weight per unblind row:
           sim_top1 = nb1301_sim_chembl_unb.npy  (top-1 Tanimoto to ChEMBL pool)
           w_i      = log(1 + sim_top1_i)
       Pass weight in CatBoost fit(sample_weight=...).
    4. CatBoost(MAE, depth=4, n_est=200, lr=0.05, l2=5) -- identical to
       nb1554; only the per-row sample_weight is new.  5-seed bag, 5-fold
       cross-fit per seed.
    5. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1693_catboost_sample_weighted.py
    data/processed/nb1693_summary.json
    data/processed/nb1693_mean_bag_oof.npy        (253,) float32
"""
...
# Per-row top-1 Tanimoto to ChEMBL pool, computed in nb1301.
SIM_CHEMBL_UNB_PATH = DATA_PROCESSED / "nb1301_sim_chembl_unb.npy"
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on 117-col 5-way "
          f"K-tuned matrix WITH sample weighting by log(1 + sim_to_chembl_top1)")
    print(f"          anchor={ANCHOR}   seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          ref: nb1554 unweighted ({NB1554_REF:.4f})  "
    ...
    # ---- Load top-1 ChEMBL similarity for sample weighting ----
    if not SIM_CHEMBL_UNB_PATH.exists():
        raise FileNotFoundError(
            f"nb1301 sim_chembl_unb cache missing: {SIM_CHEMBL_UNB_PATH}"
        )
    sim_top1_unb = np.load(SIM_CHEMBL_UNB_PATH).astype(np.float64)
    if sim_top1_unb.shape[0] != n_unb:
        raise ValueError(
            f"sim_chembl_unb shape mismatch: {sim_top1_unb.shape} vs n_unb={n_unb}"
        )
    sample_weight = np.log1p(sim_top1_unb).astype(np.float64)
    w_stats = _summarize("sample_weight", sample_weight)
    sim_stats = _summarize("sim_to_chembl_top1", sim_top1_unb)
    print(f"[weight] sim_top1: min={sim_stats['min']:.3f}  "
          f"p50={sim_stats['p50']:.3f}  max={sim_stats['max']:.3f}  "
          f"mean={sim_stats['mean']:.3f}")
    print(f"[weight] log1p(sim_top1): min={w_stats['min']:.4f}  "
          f"p50={w_stats['p50']:.4f}  max={w_stats['max']:.4f}  "
          f"mean={w_stats['mean']:.4f}  std={w_stats['std']:.4f}")
    weight_ratio = float(w_stats["max"] / max(w_stats["min"], 1e-12))
    print(f"[weight] max/min ratio = {weight_ratio:.3f}")
    ...
    # ---- Per-seed CatBoost residual cross-fit (WITH sample_weight) ----
    print("\n" + "-" * 78)
    print(f"PER-SEED WEIGHTED CATBOOST RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        delta_vs_nb1554 = rae_s - NB1554_REF
        per_seed_records.append({
            "seed": int(s),
            "rae_corrected": rae_s,
            "delta_vs_chemprop_aux": rae_s - rae_anchor,
            "delta_vs_nb1554": delta_vs_nb1554,
        ...
        print(f"   seed {s:3d}:  rae_corr = {rae_s:.4f}  "
              ...
              f"d_vs_nb1554 = {delta_vs_nb1554:+.4f})  "
    ...
    if beats_nb1554:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif beats_anchor:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_FLAT_VS_ANCHOR"
    else:
        verdict = "SAMPLE_WEIGHTED_CATBOOST_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "sim_to_chembl_top1_path": str(SIM_CHEMBL_UNB_PATH),
        "sample_weight_formula": "log(1 + sim_to_chembl_top1)",
        ...
        "sim_top1_stats": sim_stats,
        "sample_weight_stats": w_stats,
        "sample_weight_max_min_ratio": weight_ratio,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "sim_top1_stats", "sample_weight_stats",
        "sample_weight_max_min_ratio",
