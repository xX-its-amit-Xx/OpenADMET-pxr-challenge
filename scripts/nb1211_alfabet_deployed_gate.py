"""nb1211 — ALFABET BDE thermochemistry scalars on the DEPLOYED ensemble
config (AIMNet2 + strain + DFT-D4 + DBSTEP, best 0.4242).

11 ALFABET ML-DFT homolytic bond-dissociation-energy scalars (min/max/mean/std/
range/q10 BDE, min/mean BDFE, n_bonds, n_labile, frac_labile). First BOND-
CLEAVAGE THERMOCHEMISTRY axis tried -- distinct from every deployed physics block
(AIMNet2 electronic, strain conformational, D4 dispersion, DBSTEP steric/shape).
Weak mechanistic prior for a non-covalent receptor (BDE ~ metabolic labil.), so
honest gate decides.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + D4 + DBSTEP
  treatment = control + 11 ALFABET BDE scalars
...
DBCOLS = ["vbur_r25", "vbur_r35", "vbur_r45", "vbur_r55", "vbur_r65",
          ...
          "radgyr", "inertial_sf"]
AL = "C:/pxr_work/alfabet/alfabet_features.csv"
ALCOLS = ["bde_min", "bde_max", "bde_mean", "bde_std", "bde_range", "bde_q10",
          "bdfe_min", "bdfe_mean", "n_bonds_scored", "n_labile", "frac_labile"]
...
def main():
    ...
    Xal, al_nan = aligned_block(AL, ALCOLS, tr["name"])
    print(f"imp: AIM={qm_nan} strain={st_nan} d4={d4_nan} dbstep={db_nan} alfabet={al_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        sca = StandardScaler().fit(Xal[trn]); Xal_std = sca.transform(Xal)
        Xctrl = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std])              # deployed 0.4242
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xal_std])   # + ALFABET
        ...
        err = ytr[ho] - ctrl
        almean = Xal_std[ho].mean(1)
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(almean))[0, 1]))
    ...
    print(f"treatment(deployed + ALFABET BDE)          RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           ...
           "alfabet_imputed": al_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1211_alfabet_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1211_alfabet_summary.json -> DEPLOY={deploy}")
