"""nb3293 -- Bayesian Ridge on (y - nb3090) residual using K=20 features.

NEW PARADIGM: Bayesian Ridge (automatic ARD-style regularization via
evidence maximization of alpha/lambda) on the K=20 5-way feature slice
predicting the nb3090 residual. Distinct model class from every prior
residual-on-anchor attempt on this substrate (LGBM trees, NN, CatBoost,
Bayesian Linear, GP, XGB all logged as paradigm-exhausted on the
chemprop_aux residual). nb3090 is a finer-q-cut post-hoc blend of K18/K19
deep-30 (deep-30 mean OOF RAE 0.4472, PRE-clean chain). The question:
does a self-regularizing LINEAR model find residual structure in the
K=20 feature space that the tree-based residual stacks and the q-cut
blend both miss?

RATIONALE (cf. cycle-134 paradigm exhaustion):
    LGBM axis-aligned splits are the inductive bias the prior residual
    stacks used. Bayesian Ridge is a fully different bias: a global
    LINEAR fit with a precision prior on the weights, learning alpha
    (noise precision) and lambda (weight precision) from the data by
    marginal-likelihood maximization -- zero hand-tuned hyperparameters.
    On n=253 with K=20 features the automatic regularization is exactly
    the regime where over-fit is the failure mode, so the evidence-based
    shrinkage is the relevant capacity.

ANCHOR (PRE-clean chain via nb3090):
    nb3090 = finer q-cut blend of K18/K19 deep-30 (deep-30 mean 0.4472)
    nb3090 OOF RAE on 253 (median-seed pred_oof) printed at runtime
    target gate (BETTER) per-fold-mean < 0.4423

PROTOCOL (per kf_seed, 5-fold scaffold split on 253 unblind):
    residual = y_unb - nb3090_oof
    For each fold:
        mdl = BayesianRidge()                       # default automatic reg
        mdl.fit(X_unb_K20[tr_loc], residual[tr_loc])
        oof[va_loc] = nb3090_oof[va_loc] + mdl.predict(X_unb_K20[va_loc])
    per_fold_val_rae = [rae(y[va], oof[va]) for each fold]
    per_fold_mean    = mean(per_fold_val_rae)         <- GATED METRIC
    pooled           = rae(y, oof)                    (reported, not gated)
    Repeat for 15 FRESH kf_seeds {1216..1230}.

DEPLOY:
    Refit BayesianRidge on (X_unb_K20, residual) once (deterministic --
    no random_state); predict on X_te_K20; te_final = te_nb3090 + resid.
    (Bayesian Ridge has no stochastic seed, so a single deploy fit is the
    full mean-bag; no per-seed averaging needed.)
...
Inputs:
    ...
    data/processed/nb3090_pred_oof.npy   data/processed/te_nb3090.npy
...
Outputs:
    data/processed/nb3293_summary.json
    data/processed/nb3293_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3293.npy         (513,) float32 -- deploy te
    submissions/nb3293_bayesian_ridge_residual.csv  (only on BETTER)
"""
...
# -- 117-col feature recipe (identical to nb3262 / nb3163 / nb3141 / nb2960) -
...
# -- References --------------------------------------------------------------
REF_NB3090 = 0.4472     # parent finer-q-cut deep-30 mean
REF_NB3080 = 0.4475
REF_NB2171 = 0.4682     # prior post-hoc-blend anchor swap
REF_NB1191 = 0.4718     # PRE-pyramid wide-seed mean
CHEMPROP_AUX_REF = 0.6216


# ============================================================================
# helpers (lifted from nb3262 117-col feature builder, verbatim)
# ============================================================================
...
def build_117col_feature_matrix(te_smiles, n_test):
    """117-col matrix identical to nb3262 / nb3163 / nb3141 / nb2604 / nb2960."""
...
# ============================================================================
# core honest CV: BayesianRidge on (y - nb3090) residual per kf_seed
# ============================================================================

def _fit_bridge(X_tr, resid_tr):
    """StandardScaler -> BayesianRidge (default automatic alpha/lambda).

    Bayesian Ridge has no random_state -- the fit is deterministic given
    the data, so identical training rows always yield identical weights.
    Standardize features first: ARD-style precision priors assume roughly
    comparable feature scales (Mordred columns span many orders of mag).
    """
...
def _run_one_seed(
    ...
    """One kf_seed honest n-fold scaffold CV; BayesianRidge on residual.

    The model is deterministic (no seed), so per-seed variation comes
    ENTIRELY from the scaffold-fold partition (kf_seed reshuffles folds).
    """
    ...
    for fi, (tr_loc, va_loc) in enumerate(splits):
        scaler, mdl = _fit_bridge(X_unb_K20[tr_loc], residual[tr_loc])
        resid_pred_va = mdl.predict(scaler.transform(X_unb_K20[va_loc]))
        oof[va_loc] = anchor_oof[va_loc] + resid_pred_va
        per_fold_val_rae.append(float(rae(y_unb[va_loc], oof[va_loc])))
        resid_pred_tr = mdl.predict(scaler.transform(X_unb_K20[tr_loc]))
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- BayesianRidge on (y - nb3090) residual, K=20 features")
    print(f"          parent     : {PARENT_TAG} (deep-30 mean {REF_NB3090:.4f})")
    print(f"          features   : 117-col 5-way -> K=20 (nb2280_RFE)")
    print(f"          model      : sklearn BayesianRidge (auto alpha/lambda)")
    ...
    # -- Load nb3090 anchor (PRE-clean chain) --------------------------------
    ...
    print(f"[anchor] nb3090 oof RAE = {rae_anchor:.4f}  (ref {REF_NB3090:.4f}, "
          f"d={rae_anchor - REF_NB3090:+.4f})")
    ...
    print(f"STEP 2: HONEST {N_FOLDS}-fold scaffold CV over {len(KF_SEEDS)}"
          f" kf_seeds (BayesianRidge residual)")
    print("-" * 78)
    seed_records = []
    pooled_raes = []
    per_fold_means = []
    ...
    print(f"\n   ref nb3090 (deep-30 mean) = {REF_NB3090:.4f}")
    print(f"   delta vs nb3090           = {mean_pf - REF_NB3090:+.4f}")
    print(f"   gate (BETTER)             = {GATE_BETTER:.4f}")
    print(f"   delta vs gate             = {mean_pf - GATE_BETTER:+.4f}")
    print(f"   ref nb2171 (anchor-swap)  = {REF_NB2171:.4f}")

    # -- Deploy: single deterministic refit on ALL 253, predict te -----------
    print("\n" + "-" * 78)
    print("STEP 3: deploy refit on all 253 unblind (deterministic Bayesian"
          " Ridge), te = nb3090_te + resid_pred_te")
    print("-" * 78)
    scaler_full, mdl_full = _fit_bridge(X_unb_K20, residual)
    te_resid = mdl_full.predict(scaler_full.transform(X_te_K20)).astype(np.float64)
    te_pred = (anchor_te + te_resid).astype(np.float32)
    te_unb_in_rae = float(rae(y_unb, te_pred[unb_idx]))
    print(f"   te_resid mean={te_resid.mean():+.4f}  std={te_resid.std():.4f}")
    ...
    # Learned hyperparameters of the deploy fit (diagnostic)
    print(f"   BayesianRidge deploy alpha_={mdl_full.alpha_:.4g}  "
          f"lambda_={mdl_full.lambda_:.4g}  n_iter={getattr(mdl_full, 'n_iter_', 'NA')}")
    ...
    if mean_pf < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3293 15-seed per-fold-mean "
            f"{mean_pf:.4f} beats BETTER gate {GATE_BETTER:.4f} "
            f"({mean_pf - GATE_BETTER:+.4f}) and nb3090 deep-30 mean "
            f"{REF_NB3090:.4f} ({mean_pf - REF_NB3090:+.4f}). "
            f"A self-regularizing LINEAR model (BayesianRidge, auto "
            f"alpha/lambda) extracts residual structure on the K=20 5-way "
            f"feature slice beyond the q-cut post-hoc blend -- a model-class "
            f"axis the tree-based residual stacks did not cover. PRE-clean "
            f"anchor chain (nb3090 <- K18/K19 deep-30). Re-verify with "
            f"deep-30 (kf_seeds 30+) before any PRIMARY swap; same "
            f"under-dispersion-risk root as cycle-160."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3293 15-seed per-fold-mean {mean_pf:.4f} fails "
            f"BETTER gate {GATE_BETTER:.4f} ({mean_pf - GATE_BETTER:+.4f}). "
            f"Delta vs nb3090 (deep-30 mean {REF_NB3090:.4f}) = "
            f"{mean_pf - REF_NB3090:+.4f}. A self-regularizing linear "
            f"BayesianRidge on the K=20 5-way feature slice finds no new "
            f"structure on top of the nb3090 q-cut blend; the K=20 matrix is "
            f"already absorbed by the K18/K19 deep-30 anchor chain. "
            f"Residual-on-anchor with a global linear model class closed on "
            f"this anchor; substrate change required (orthogonal anchor or "
            f"off-manifold features), consistent with cycle-134 paradigm "
            f"exhaustion and cycle-169 post-hoc-blend axes closed."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_bayesian_ridge_residual.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": ("bayesian_ridge_on_y_minus_nb3090_residual_with_K20_5way_"
                   "features_standardized_honest_5fold_scaffold_cv_15_fresh_seeds"),
        "model_family": "sklearn.linear_model.BayesianRidge",
        "model_params": "default (auto alpha/lambda via evidence maximization)",
        ...
        "ref_nb3090_deep30_mean": REF_NB3090,
        "ref_nb3080": REF_NB3080,
        "ref_nb2171": REF_NB2171,
        "ref_nb1191": REF_NB1191,
        "ref_chemprop_aux": CHEMPROP_AUX_REF,
        "delta_vs_nb3090_perfold_mean": round(mean_pf - REF_NB3090, 4),
        "delta_vs_gate": round(mean_pf - GATE_BETTER, 4),
        "delta_vs_anchor_in_sample": round(mean_pf - rae_anchor, 4),
        "deploy_bayesianridge_alpha": float(mdl_full.alpha_),
        "deploy_bayesianridge_lambda": float(mdl_full.lambda_),
        "deploy_bayesianridge_n_iter": int(getattr(mdl_full, "n_iter_", 0)),
        "te_resid_mean": float(te_resid.mean()),
        "te_resid_std": float(te_resid.std()),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_nb3090_perfold_mean", "delta_vs_gate",
