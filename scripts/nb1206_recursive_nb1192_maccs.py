"""nb1206 -- Recursive residual on nb1192 anchor (MACCS-166 features).

Hypothesis:
    nb1192 is a 4-way naive-mean blend; its residual variance vs truth is
    shrunk (averaging effect). A very shallow MACCS-based residual on top
    may still extract a sliver because MACCS standalone (~0.5513) accounts
    for most of nb1192's signal, and the residual-after-blend may still
    carry MACCS-correlated structure.

Protocol:
  1. Anchor = nb1192_mean_oof (pooled RAE ~0.5514 on 253 unblind).
  2. residual = y_unb - nb1192_mean_oof
  3. Features: MACCS-166 from data/processed/te_maccs.npy (513,167) sliced
     to unb 253. Bit 0 (always 0) and bit 166 (always 0 in standard
     RDKit MACCS encoding) drop out as zero-variance; LGBM ignores them.
  ...
  6. Verdict at 0.003 margin vs nb1192 (0.5514).

Outputs:
  data/processed/nb1206_per_seed_corrected_oof.npy  (5, 253) float32
  data/processed/nb1206_mean_bag_oof.npy            (253,)   float32
  data/processed/nb1206_summary.json
"""
...
TAG = "nb1206"
ANCHOR = "nb1192"
ANCHOR_FILE = "nb1192_mean_oof.npy"
...
NB1192_MEAN_REF = 0.5514     # the anchor we must beat
MARGIN = 0.003

MACCS_CACHE = DATA_PROCESSED / "te_maccs.npy"   # (513, 167) uint8 -- MACCS-166 plus pad bit 0


def _lgbm_params(seed: int) -> dict:
    """VERY SHALLOW LGBM Huber -- nb1192 residual variance is already shrunk
    by 4-way blending, so cap capacity aggressively to avoid noise fit."""
    return dict(
        objective="huber",
        alpha=1.0,
        learning_rate=0.03,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- RECURSIVE residual LGBM on nb1192 anchor (MACCS-166)")
    print(f"          seeds  = {RESID_SEEDS}")
    print(f"          target = y_unb - nb1192_mean_oof")
    ...
    if not anchor_path.exists():
        raise FileNotFoundError(
            f"{anchor_path} not found; required anchor OOF (run nb1192 first)."
        )
    ...
    print(f"[load] {ANCHOR_FILE} shape={anchor_oof.shape}  "
          f"pooled RAE = {rae_anchor:.4f}  (ref {NB1192_MEAN_REF:.4f})")
    ...
    ratio_nb1192_over_nb1070 = None
    if nb1070_resid_std is not None:
        ratio_nb1192_over_nb1070 = float(residual.std() / nb1070_resid_std)
        print(f"[resid] std_ratio (nb1192 / nb1070) = "
              f"{ratio_nb1192_over_nb1070:.4f}")
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        per_seed_records.append({
            "seed": int(s),
            "rae_corrected": rae_s,
            "delta_vs_nb1192": delta_s,
            "resid_oof_std": float(resid_oof_s.std()),
            "resid_oof_mean": float(resid_oof_s.mean()),
        })
        print(f"   seed {s:3d}:  rae_corr = {rae_s:.4f}  "
              f"(d_vs_nb1192 = {delta_s:+.4f})  "
    ...
    print(f"   pooled RAE(mean_bag) = {rae_mean_bag:.4f}  "
          f"(d_vs_nb1192 = {rae_mean_bag - rae_anchor:+.4f})")
    print(f"   nb1192 anchor        = {NB1192_MEAN_REF:.4f}")

    beats_nb1192 = rae_mean_bag < NB1192_MEAN_REF - MARGIN

    if beats_nb1192:
        verdict = "RECURSIVE_RESIDUAL_NB1192_MACCS_BEATS_ANCHOR_BY_MARGIN"
    elif abs(rae_mean_bag - NB1192_MEAN_REF) < MARGIN:
        verdict = "RECURSIVE_RESIDUAL_NB1192_MACCS_TIES_ANCHOR_NO_SIGNAL_LEFT"
    else:
        verdict = "RECURSIVE_RESIDUAL_NB1192_MACCS_HURTS_ANCHOR_OVERFITS_NOISE"
    ...
    summary = {
        ...
        "rae_anchor_nb1192": rae_anchor,
        "residual_mean": float(residual.mean()),
        "residual_std": float(residual.std()),
        "residual_std_nb1070": nb1070_resid_std,
        "residual_std_ratio_nb1192_over_nb1070": ratio_nb1192_over_nb1070,
        ...
        "delta_mean_bag_vs_nb1192": rae_mean_bag - rae_anchor,
        "margin": MARGIN,
