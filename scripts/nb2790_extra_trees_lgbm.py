"""nb2790 -- LightGBM with extra_trees=True on K=20 chemprop_aux residual.

NEW PARADIGM:
    LightGBM with extra_trees=True samples a RANDOM split threshold per
    candidate feature instead of picking the gradient-optimal split.  This
    fuses Geurts-2006 randomized-threshold trees (cf. nb2731 ExtraTrees)
    with the boosting framework: each round still fits the negative
    gradient of the loss, but the within-tree split selection becomes
    stochastic rather than greedy.

    Distinction from related sweeps already on this anchor:
      - Plain LGBM (nb2103/nb2112/nb2240): greedy best-split GBDT.
      - ExtraTreesRegressor (nb2731): stochastic-split BAGGING, no gradient.
      - nb2790 (this): stochastic-split BOOSTING - the only fully-randomized
        split-threshold gradient ensemble in our zoo on the K=20 substrate.

    Hypothesis: randomized split selection injects variance at the
    per-tree level, which on a 253-row residual target can reduce
    overfitting to noisy SHAP-K20 features while the boosting machinery
    still drives bias down.  Comparable in spirit to RandomForest-vs-
    ExtraTrees but inside the LGBM tree-shape (max_depth=4, num_leaves=15)
    that won the K-sweep ceiling work.
...
PROTOCOL:
    ...
    3. Model: lightgbm.LGBMRegressor(
...
Outputs:
    scripts/nb2790_extra_trees_lgbm.py
    data/processed/nb2790_summary.json
    data/processed/nb2790_pred_oof.npy   (253,) float32
    data/processed/te_nb2790.npy         (513,) float32
"""
...
# --------------------------------------------------------------------------
# LightGBM hyperparameters (per spec)
# --------------------------------------------------------------------------
LGBM_MAX_DEPTH = 4
LGBM_NUM_LEAVES = 15
...
def _new_lgbm(seed: int = LGBM_RANDOM_STATE) -> lgb.LGBMRegressor:
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(extra_trees=True) on K=20 chemprop_aux residual")
    print("=" * 78)
    ...
    print(f"SCAFFOLD 5-FOLD CV  seeds={KF_SEEDS}\n"
          f"LGBM(extra_trees=True): max_depth={LGBM_MAX_DEPTH}  "
          f"num_leaves={LGBM_NUM_LEAVES}  n_estimators={LGBM_N_ESTIMATORS}  "
          f"lr={LGBM_LEARNING_RATE}  seed={LGBM_RANDOM_STATE}")
    print("-" * 78)
    ...
    for kf_seed in KF_SEEDS:
        ...
        for fi, (tr_loc, va_loc) in enumerate(splits):
            mdl = _new_lgbm(seed=LGBM_RANDOM_STATE + kf_seed + fi)
    ...
    print("-" * 78)
    deploy_mdl = _new_lgbm(seed=LGBM_RANDOM_STATE)
    ...
    summary = {
        "tag": TAG,
        "method": (
            "LightGBM with extra_trees=True (randomized-threshold split "
            "selection inside the boosting framework); fit on chemprop_aux "
            "residual over first-K=20 cols of X_117."
        ),
        "paradigm": (
            "stochastic_split_threshold_gradient_boosted_trees_"
            "extra_trees_inside_lgbm_boosting_loop"
        ),
