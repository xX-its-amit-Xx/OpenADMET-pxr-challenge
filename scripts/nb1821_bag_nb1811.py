"""nb1821 -- Outer-bag of nb1811 (LGBM GOSS) for honest validation.

HYPOTHESIS:
    nb1811 reported rae_mean_bag = X.XXXX on the 253 unblind using a 5-inner-seed
    bag of LGBM(boosting='goss', huber, max_depth=4, num_leaves=15, n_est=300,
    lr=0.03, min_child_samples=5, reg_lambda=2) on the 117-col 5-way K-tuned
    matrix with chemprop_aux te[unb_idx] as PRE-unblind anchor.
    ...
    Outer-bag-of-bags (BoB) honest validation:
      ...
      Verdict NB1811_REPRODUCES iff |BoB MEAN - nb1811_rae_mean_bag| < 0.005.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1811/nb1771/nb1554).
    3. For each outer seed s in {0,1,7,42,137}:
         inner_seeds = [s*1000 + j for j in 0..4]
         per-inner: 5-fold cross-fit LGBM(GOSS, huber, ...) on residual
    ...
    5. Verdict vs nb1811 reference.

Outputs:
    scripts/nb1821_bag_nb1811.py
    data/processed/nb1821_summary.json
    data/processed/nb1821_per_outer_oof.npy   (5, 253) float32
"""
...
NB1811_SUMMARY = DATA_PROCESSED / "nb1811_summary.json"
...
NB1554_REF = 0.5163
NB1771_REF = 0.5100
NB1811_REF_FALLBACK = 0.5100  # use nb1811 summary if present, else this
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG of nb1811 (LGBM GOSS) on 5-way K-tuned 117-col "
    ...
    # ---- nb1811 reference ----
    if NB1811_SUMMARY.exists():
        with open(NB1811_SUMMARY) as f:
            nb1811_sum = json.load(f)
        nb1811_ref = float(nb1811_sum.get("rae_mean_bag", NB1811_REF_FALLBACK))
        print(f"[ref] nb1811_summary.rae_mean_bag = {nb1811_ref:.4f}")
    else:
        nb1811_ref = NB1811_REF_FALLBACK
        print(f"[ref] nb1811_summary.json missing -- using fallback "
              f"{nb1811_ref:.4f}")
    ...
    print(f"   nb1811 ref          = {nb1811_ref:.4f}")
    print(f"   d(BoB_MEAN, nb1811) = {bob_mean - nb1811_ref:+.4f}")

    # ---- Verdict ----
    delta_vs_nb1811 = bob_mean - nb1811_ref
    if abs(delta_vs_nb1811) < REPRODUCE_MARGIN:
        verdict = "NB1811_REPRODUCES"
    elif delta_vs_nb1811 < -REPRODUCE_MARGIN:
        verdict = "NB1811_REPRODUCES_BETTER"
    else:
        verdict = "NB1811_DOES_NOT_REPRODUCE"
    ...
    summary = {
        ...
        "delta_bob_mean_vs_nb1811": delta_vs_nb1811,
        "verdict": verdict,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb1554_ref": NB1554_REF,
        "nb1771_ref": NB1771_REF,
        "nb1811_ref": nb1811_ref,
