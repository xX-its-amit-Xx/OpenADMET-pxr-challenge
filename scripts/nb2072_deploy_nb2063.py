"""nb2072 -- Deploy nb2063 (SHAP top-50 LGBM-MSE) to 513-row CSV.

PROTOCOL:
    1. Reuse top-50 SHAP indices from nb2063 (saved as nb2063_top50_idx.npy)
       into the 117-col 5-way K-tuned feature stack.
    2. Build 513-row features using these 50 cols.
    ...
    4. Mean across 5 seeds. te_nb2072 = te_chemprop_aux + mean_residual_513.
    5. Save submissions/nb2072_deploy_nb2063.csv.

ANCHORS:
    nb2063 honest cross-fit RAE = 0.4933 (predicted LB 0.496).
"""
...
# 5-seed bag (matches nb2063 inner seeds)
...
# Reference (honest cross-fit) from nb2063
NB2063_HONEST_REF = 0.4933
NB2063_PREDICTED_LB = 0.496
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- identical to nb2063/nb1861."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb2063 (SHAP top-{TOP_K_SHAP} LGBM-MSE) to 513-row CSV")
    print(f"          bag seeds = {BAG_SEEDS}  total fits = {len(BAG_SEEDS)}")
    print(f"          anchor    = {ANCHOR}  (PRE-unblind te slice)")
    print(f"          ref       = nb2063 honest cross-fit RAE = "
          f"{NB2063_HONEST_REF:.4f}  predicted LB {NB2063_PREDICTED_LB:.3f}")
    print("=" * 78)
    ...
    # ---- Load top-50 SHAP indices from nb2063 ----
    if not NB2063_TOP50_IDX_PATH.exists():
        raise FileNotFoundError(
            f"missing nb2063 top-50 idx: {NB2063_TOP50_IDX_PATH}"
        )
    top50_idx = np.load(NB2063_TOP50_IDX_PATH).astype(np.int32)
    if top50_idx.shape[0] != TOP_K_SHAP:
        raise ValueError(
            f"top50_idx shape {top50_idx.shape} != ({TOP_K_SHAP},)"
        )
    print(f"[load] nb2063 top-{TOP_K_SHAP} SHAP indices: head 10 = "
          f"{top50_idx[:10].tolist()}")

    # ---- Load all K-grid winners (same 117-col stack as nb2063) ----
    ...
    # ---- Restrict to top-50 SHAP cols ----
    if int(top50_idx.max()) >= feat_dim_full:
        raise ValueError(
            f"top50_idx max {int(top50_idx.max())} >= feat_dim_full "
            f"{feat_dim_full}"
        )
    X_te_50 = X_te_117[:, top50_idx].astype(np.float32)
    X_unb_50 = X_unb_117[:, top50_idx].astype(np.float32)
    print(f"\n   SHAP TOP-{TOP_K_SHAP}: X_te_50={X_te_50.shape}  "
          f"X_unb_50={X_unb_50.shape}")
    ...
    for k, s in enumerate(BAG_SEEDS):
        ...
        mdl.fit(X_unb_50, residual_unb)
        resid_513 = mdl.predict(X_te_50)
    ...
    te_nb2072 = te_anchor_513 + mean_resid_513

    # ---- In-sample check on unb_idx ----
    in_pred_unb = te_nb2072[unb_idx]
    rae_in_unb = float(rae(y_unb, in_pred_unb))
    print("\n" + "-" * 78)
    print(f"in-sample RAE on unb_idx = {rae_in_unb:.4f}")
    print(f"anchor in_RAE             = {rae_anchor:.4f}")
    print(f"nb2063 honest cross-fit   = {NB2063_HONEST_REF:.4f}  "
          f"(predicted LB {NB2063_PREDICTED_LB:.3f})")
    print("-" * 78)

    # ---- Stats ----
    te_mean = float(te_nb2072.mean())
    te_std = float(te_nb2072.std())
    te_min = float(te_nb2072.min())
    te_max = float(te_nb2072.max())
    print(f"\nte_nb2072 stats: mean={te_mean:.4f}  std={te_std:.4f}  "
    ...
    df_sub = pd.DataFrame({
        ...
        "pEC50": te_nb2072.astype(np.float32),
    })
    if len(df_sub) != 513:
        raise ValueError(f"submission rows {len(df_sub)} != 513")
    sub_path = SUBMISSIONS_DIR / f"{TAG}_deploy_nb2063.csv"
    df_sub.to_csv(sub_path, index=False)
    print(f"\n[save] submission CSV: {sub_path}  ({len(df_sub)} rows)")

    # ---- Save te artifact ----
    te_path = DATA_PROCESSED / f"te_{TAG}.npy"
    np.save(te_path, te_nb2072.astype(np.float32))
    ...
    summary = {
        "tag": TAG,
        "method": "deploy_nb2063_shap_top50_5seed_bag",
        "anchor": ANCHOR,
        ...
        "feat_dim_top50": int(X_te_50.shape[1]),
        "feat_breakdown_full": {
            "atompair": n_top_ap,
            "maccs": n_top_maccs,
            "mordred": n_top_mord,
        ...
        "te_nb2072_mean": te_mean,
        "te_nb2072_std": te_std,
        "te_nb2072_min": te_min,
        "te_nb2072_max": te_max,
        "in_RAE_unb_idx": rae_in_unb,
        "nb2063_honest_ref": NB2063_HONEST_REF,
        "predicted_LB": NB2063_PREDICTED_LB,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_test", "n_unb", "feat_dim_full", "feat_dim_top50", "n_fits",
        "rae_anchor_chemprop_aux_unb",
        "te_nb2072_mean", "te_nb2072_std",
        "te_nb2072_min", "te_nb2072_max",
        "in_RAE_unb_idx",
        "nb2063_honest_ref", "predicted_LB",
        "submission_csv",
    ):
        print(f"  {k}: {res.get(k)}")
