"""nb1711 -- LightGBM quantile (alpha=0.5) on K-tuned 5-way 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE d4 n200 lr0.05 l2=5) hit RAE 0.5163 on the same
    5-way K-tuned 117-col matrix.  Try one more residual learner family:
    LightGBM with objective='quantile', alpha=0.5 (median regression).
    Same depth/iteration budget so the only thing changing is the loss-
    surface family (LightGBM-quantile vs CatBoost-MAE).  If RAE is within
    0.003 of nb1554, this is a cross-family flat -- evidence that the
    bottleneck is the 5-way K-tuned anchor + features, not the learner.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build the SAME 117-col feature stack from cached top-K rankings:
    ...
    3. 5-seed bag LGBMRegressor(objective='quantile', alpha=0.5,
       max_depth=4, n_estimators=200, learning_rate=0.05, num_leaves=15,
       reg_lambda=1).
...
Outputs:
    scripts/nb1711_lgbm_quantile.py
    data/processed/nb1711_summary.json
    data/processed/nb1711_mean_bag_oof.npy        (253,) float32
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(quantile a=0.5, d4, n200, lr0.05, leaves15, "
          f"l2=1) on 5-way K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    ...
    # ---- Per-seed LightGBM-quantile residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM-QUANTILE(alpha=0.5) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    pearson_vs_nb1550 = _pearson_vs(DATA_PROCESSED / "nb1550_best_oof.npy")
    pearson_vs_nb1553 = _pearson_vs(DATA_PROCESSED / "nb1553_best_oof.npy")
    ...
    if pearson_vs_nb1550 is not None:
        print(f"   Pearson(mean_bag, nb1550_best)   = {pearson_vs_nb1550:.4f}")
    if pearson_vs_nb1553 is not None:
        print(f"   Pearson(mean_bag, nb1553_best)   = {pearson_vs_nb1553:.4f}")
    ...
    if beats_nb1554:
        verdict = "LGBM_QUANTILE_5WAY_K_TUNED_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "LGBM_QUANTILE_5WAY_K_TUNED_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_QUANTILE_5WAY_K_TUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_QUANTILE_5WAY_K_TUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_QUANTILE_5WAY_K_TUNED_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "pearson_vs_nb1550_best": pearson_vs_nb1550,
        "pearson_vs_nb1553_best": pearson_vs_nb1553,
