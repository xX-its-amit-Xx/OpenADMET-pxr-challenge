"""nb3224 -- Learned per-fold clip on nb3020 (K18+K19+K23 deep-30 SLSQP simplex).

NEW PARADIGM (cycle 250+):
    nb3020 produced a 3-anchor SLSQP simplex over {K=18, K=19, K=23} all at
    deep-30. We apply the per-fold (q_low, q_high) learned-clip primitive
    directly on top of the nb3020 OOF (single anchor for this clip layer).

    Hypothesis: the SLSQP simplex already does variance averaging across 3
    K-anchors, but the tails may still be compressible by per-fold quantile
    clipping. If the learned clip extracts additional tail-taming gain over
    the nb3020 parent, the clip primitive is anchor-invariant and applies
    cleanly to simplex-blended substrates (not just single K-anchors as in
    nb3201).

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb3020_pred_oof  (253,) -- K18+K19+K23 deep-30 SLSQP simplex
    Per outer fold:
...
References:
    nb3020 K18+K19+K23 simplex (parent) -- consult nb3020_summary.json
    nb3201 learned-clip on K=18 deep-30 alone (sibling pattern)
...
Inputs:
    ...
    data/processed/nb3020_pred_oof.npy
    data/processed/te_nb3020.npy

Outputs:
    data/processed/nb3224_summary.json
    data/processed/nb3224_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3224.npy         (513,) float32 -- deploy te
    submissions/nb3224_clip_K18_K19_K23_anchor.csv  (only on BETTER verdict)
"""
...
TAG = "nb3224"
PARENT_TAG = "nb3020_K18_K19_K23_simplex_deep30"

# -- Inputs --------------------------------------------------------------------
PRED_OOF_PATH = DATA_PROCESSED / "nb3020_pred_oof.npy"
TE_PATH = DATA_PROCESSED / "te_nb3020.npy"

# -- CV protocol ---------------------------------------------------------------
N_FOLDS = 5
KF_SEEDS = list(range(1216, 1231))  # 15 FRESH seeds {1216..1230}

# -- Per-fold grid (matches nb3201 sibling) ------------------------------------
...
def main() -> dict:
    ...
    print(
        ...
        f"{PARENT_TAG} OOF (single anchor)"
    )
    ...
    # -- Load nb3020 anchor pred_oof + te -------------------------------------
    print("\n" + "-" * 78)
    ...
    print(
        f"\n   ref nb3020 parent OOF         = {full_oof_rae:.4f}  "
        f"<- parent (no clip)"
    )
    print(
        f"   delta vs nb3020 parent (pf)   = "
    ...
    print(f"   ref nb3001 wide-seed mean     = {REF_NB3001:.4f}")
    ...
    if pf_mean < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3224 15-seed per-fold-mean {pf_mean:.4f} "
            f"clears BETTER gate {GATE_BETTER:.4f}. Per-fold LEARNED clip "
            f"applied to nb3020 K18+K19+K23 deep-30 SLSQP simplex extracts "
            f"additional tail-taming gain over the simplex-blended parent "
            f"({pf_mean - full_oof_rae:+.4f}). Modal pick was "
            f"(q{ql_mode:.2f}, q{qh_mode:.2f}). Confirms clip primitive "
            f"applies cleanly to simplex-blended substrates; re-verify with "
            f"wider seed bag before PRIMARY-1 swap."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3224 15-seed per-fold-mean {pf_mean:.4f} fails BETTER "
            f"gate {GATE_BETTER:.4f}. Per-fold learned clip on nb3020 simplex "
            f"does NOT extract additional gain "
            f"({pf_mean - full_oof_rae:+.4f} vs parent). SLSQP simplex over "
            f"3 deep-30 K-anchors already does the variance compression that "
            f"the clip would otherwise capture. Hold current ladder."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_clip_K18_K19_K23_anchor.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_nb3020_K18_K19_K23_simplex"
        ),
        "anchor_pred_oof_path": str(PRED_OOF_PATH),
        "anchor_te_path": str(TE_PATH),
        "anchor_pre_unblind": True,
        "anchor_pool": ["K18", "K19", "K23"],
        "anchor_depth": "deep30_per_K + per_fold_SLSQP_simplex",
        ...
        # references
        "ref_nb3020_parent": round(full_oof_rae, 4),
        "delta_vs_nb3020_parent": round(pf_mean - full_oof_rae, 4),
    ...
    print(f"   95% CI                       = [{ci_low:.4f}, {ci_high:.4f}]")
    print(f"   delta vs nb3020 parent (pf)  = {pf_mean - full_oof_rae:+.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_nb3020_parent", "delta_vs_nb3170_fixed",
        "ql_mode", "qh_mode",
