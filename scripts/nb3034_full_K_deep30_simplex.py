"""nb3034 -- Per-fold SLSQP simplex on FULL 7-anchor deep-30 K-pyramid
              {K17, K18, K19, K20, K21, K22, K23}.

NEW PARADIGM: brute force -- put EVERY deep-30 K-anchor available from
cycles 247-249 into one simplex. Earlier subset pools (nb3002 K18,K19;
nb3011 K17,K18,K19; nb3020 K18,K19,K23; nb3023 K17..K20; etc.) tested
contiguous and partial selections. nb3034 lets SLSQP choose freely
across the full deep-30 K-pyramid spanning K=17 to K=23.

The hypothesis: per-fold SLSQP simplex sparsifies on its own (degenerate
weights are common with 4+ anchors); giving it the full 7-anchor menu
either (a) lets it pick a better sparse subset than any pre-chosen
contiguous slice, or (b) reveals that subset-level performance saturates
and brute-force pooling neither helps nor hurts.

PROTOCOL:
    Anchors (ALL deep-30):
        K17: nb3010 deep-30 fresh-seed OOF + te (bag-mean RAE 0.4680)
        K18: nb2960 deep-30 fresh-seed OOF + te (full-OOF RAE 0.4536)
        K19: nb3000 deep-30 fresh-seed OOF + te (bag-mean RAE 0.4607)
        K20: nb2960 deep-30 fresh-seed OOF + te (full-OOF RAE 0.4625)
        K21: nb3014 deep-30 fresh-seed OOF + te (bag-mean RAE 0.4587)
        K22: nb3014 deep-30 fresh-seed OOF + te (bag-mean RAE 0.4708)
        K23: nb3020 deep-30 fresh-seed OOF + te (bag-mean RAE 0.4750)
    Outer CV: 5-fold scaffold split, 5 fresh kf_seeds {1041..1045}
    Per fold:
    ...
    Deploy:
        ...
        - Apply to (513, 7) stacked te arrays -> te_nb3034

GATE:
    mean_pooled_rae < 0.4511 -> "BETTER_THAN_NB2992"
    else                     -> "FAIL"

References:
    nb3010 K17 deep-30 bag-mean RAE  = 0.4680
    nb2960 K18 deep-30 OOF RAE       = 0.4536
    nb3000 K19 deep-30 bag-mean RAE  = 0.4607
    nb2960 K20 deep-30 OOF RAE       = 0.4625
    nb3014 K21 deep-30 bag-mean RAE  = 0.4587
    nb3014 K22 deep-30 bag-mean RAE  = 0.4708
    nb3020 K23 deep-30 bag-mean RAE  = 0.4750
    nb2992 K18,K19(5sd),K20 deploy   = 0.4479 (gate baseline)
    nb3023 K17..K20 deep-30 5-seed   = (cycle 247 verify)
    nb2171 prior post-hoc-blend ceil = 0.4682
...
Inputs:
    ...
    data/processed/nb3014_K21_30seed_oof.npy
    data/processed/te_nb3014_K21.npy
    data/processed/nb3014_K22_30seed_oof.npy
    data/processed/te_nb3014_K22.npy
    data/processed/nb3020_K23_30seed_oof.npy
    data/processed/te_nb3020_K23.npy

Outputs:
    data/processed/nb3034_summary.json
    data/processed/nb3034_pred_oof.npy   (253,) float32 -- first-seed per-fold OOF blend
    data/processed/te_nb3034.npy         (513,) float32 -- deploy te
    submissions/nb3034_full_K_deep30_simplex.csv  (only if verdict != "FAIL")
"""
...
TAG = "nb3034"
PARENT_TAG = "nb3010+nb2960+nb3000+nb3014+nb3020"

# -- Inputs --------------------------------------------------------------------
K_LABELS = ["K17", "K18", "K19", "K20", "K21", "K22", "K23"]
OOF_PATHS = {
    ...
    "K21": DATA_PROCESSED / "nb3014_K21_30seed_oof.npy",
    "K22": DATA_PROCESSED / "nb3014_K22_30seed_oof.npy",
...
TE_PATHS = {
    ...
    "K21": DATA_PROCESSED / "te_nb3014_K21.npy",
    "K22": DATA_PROCESSED / "te_nb3014_K22.npy",
    "K23": DATA_PROCESSED / "te_nb3020_K23.npy",
}
K_DEPTH = {k: "deep30" for k in K_LABELS}

# -- CV protocol ---------------------------------------------------------------
N_FOLDS = 5
KF_SEEDS = list(range(1041, 1046))  # 5 fresh seeds {1041..1045}
N_STARTS_FOLD = 8
N_STARTS_FULL = 12
DEGEN_MAX_W = 0.85
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on FULL 7-anchor deep-30 K-pyramid")
    print(f"          anchors: {K_LABELS} (all deep-30)")
    print(f"          parents: nb3010 (K17) + nb2960 (K18,K20) + nb3000 (K19) + "
          f"nb3014 (K21,K22) + nb3020 (K23)")
    print(f"          paradigm: brute force -- every deep-30 K-anchor in one simplex")
    ...
    print(f"          gate: <{GATE_BETTER_THAN_NB2992} BETTER_THAN_NB2992 / else FAIL")
    print("=" * 78)
    ...
    print("\n" + "-" * 78)
    print("STEP 1: load K-anchor OOFs and te arrays (all 7 deep-30)")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
    ...
    K = len(K_LABELS)
    P_unb = np.column_stack(oof_cols)  # (253, 7)
    P_te = np.column_stack(te_cols)    # (513, 7)
    ...
    all_w_arr = np.asarray(all_weights, dtype=np.float64)  # (25, 7)
    ...
    print("-" * 78)
    if mean_pooled < GATE_BETTER_THAN_NB2992:
    ...
    delta_vs_K21 = mean_pooled - REF_K21
    delta_vs_K22 = mean_pooled - REF_K22
    delta_vs_K23 = mean_pooled - REF_K23
    ...
    print(f"   delta vs K21  (0.4587)   = {delta_vs_K21:+.4f}")
    print(f"   delta vs K22  (0.4708)   = {delta_vs_K22:+.4f}")
    print(f"   delta vs K23  (0.4750)   = {delta_vs_K23:+.4f}")
    ...
    # pred_oof = first-seed (1041) per-fold OOF blend (single-seed slice).
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_full_K_deep30_simplex.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_full_K17_to_K23_deep30_5seed",
        "paradigm": "brute_force_all_deep30_K_anchors_one_simplex",
        ...
        "ref_K21_deep30": REF_K21,
        "ref_K22_deep30": REF_K22,
        ...
        "delta_vs_K21": delta_vs_K21,
        "delta_vs_K22": delta_vs_K22,
        "delta_vs_K23": delta_vs_K23,
