"""nb983 -- RANK STRETCH on nb972 (te_nb972_long_train).

Copy of the nb562 rank-stretch recipe applied to nb972.

Hypothesis: nb972 has predicted std ~0.633 vs truth 1.032 -- variance
compressed -- so a scalar stretch s>1 should help, similar to how it helped
nb503 in pre-Phase-2 work.

Procedure (per user spec):
  1) Load te_nb972_long_train.npy (513 deploy preds; in_RAE 0.6898 on the 253
     unblind subset).
  2) mu = mean(te_nb972).
  3) Grid s in {1.00, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40}.
  4) For each s: stretched = mu + s * (te_nb972 - mu); compute in_RAE on the
     253 unblind rows.
  5) Pick best s; save submission CSV + te_nb983.npy + summary JSON.
"""
...
TAG = "nb983"
ANCHOR = "nb972"
STRETCH_GRID = [1.00, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40]
...
def main() -> dict:
    print("=" * 78)
    print(f"{TAG} -- RANK STRETCH on {ANCHOR} (te_{ANCHOR}_long_train)")
    print("=" * 78)

    needed = {
        "TEST_BLINDED": DATA_RAW / "pxr-challenge_TEST_BLINDED.csv",
        "UNBLINDED":    DATA_RAW / "pxr-challenge_TEST_PHASE_1_UNBLINDED.csv",
        f"te_{ANCHOR}_long_train.npy":
            DATA_PROCESSED / f"te_{ANCHOR}_long_train.npy",
    }
