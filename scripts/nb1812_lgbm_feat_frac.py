"""nb1812 -- LightGBM (small leaves + feature_fraction=0.7, bagging_fraction=0.8)
on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1771 (LGBM small leaves on the 5-way K-tuned 117-col matrix) hit
    rae_mean_bag 0.5100 and nb1780 5-seed BoB pulled this to 0.5032.
    The base nb1771 params do NOT subsample features per tree.  This script
    adds column subsampling (feature_fraction=0.7) and row subsampling
    (bagging_fraction=0.8, bagging_freq=5) on top of the small-leaf regime
    to decorrelate trees within the bag and probe whether explicit
    per-tree feature drop trims residual variance on novel scaffolds.
        ...
        bagging_freq=5
    Same 5-way K-tuned 117-col feature stack as nb1771:
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(huber, max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=5, reg_lambda=2, feature_fraction=0.7,
...
Outputs:
    scripts/nb1812_lgbm_feat_frac.py
    data/processed/nb1812_summary.json
    data/processed/nb1812_mean_bag_oof.npy        (253,) float32
"""
...
def main() -> dict:
    ...
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=4, num_leaves=15, "
          f"n_est=300, lr=0.03, min_child=5, reg_lambda=2, "
          f"feature_fraction=0.7, bagging_fraction=0.8, bagging_freq=5) "
    ...
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1771 ({NB1771_REF:.4f}), nb1780 BoB ({NB1780_REF:.4f})  "
    ...
    # ---- Per-seed LGBM small-leaf + feat_frac residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (small leaves + feature_fraction=0.7 + "
          f"bagging_fraction=0.8) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1780:
        verdict = "LGBM_FEAT_FRAC_BEATS_NB1780_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1780:
        verdict = "LGBM_FEAT_FRAC_FLAT_VS_NB1780_BOB"
    elif beats_nb1771:
        verdict = "LGBM_FEAT_FRAC_BEATS_NB1771_BUT_WORSE_THAN_NB1780_BOB"
    elif flat_vs_nb1771:
        verdict = "LGBM_FEAT_FRAC_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_FEAT_FRAC_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_FEAT_FRAC_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_FEAT_FRAC_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_FEAT_FRAC_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_FEAT_FRAC_HURTS_ANCHOR"
