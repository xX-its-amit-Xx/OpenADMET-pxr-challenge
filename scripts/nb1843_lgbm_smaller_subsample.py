"""nb1843 -- LightGBM (smaller subsample + min_data_in_leaf=10) on 5-way
K-tuned 117-col features.

HYPOTHESIS:
    nb1812 hit rae_mean_bag 0.5072 with feature_fraction=0.7 and
    bagging_fraction=0.8 on the small-leaf regime, and nb1780 BoB on the
    nb1771 baseline still leads at 0.5032.  This script probes a smaller-
    subsample regime: feature_fraction=0.6 (more aggressive column drop)
    + bagging_fraction=0.7 (more aggressive row drop) + min_child_samples=10
    (require more support per leaf) to further decorrelate trees and
    suppress per-leaf noise on novel-scaffold residuals.
        ...
        bagging_freq=5
    Same 5-way K-tuned 117-col feature stack as nb1771/nb1812:
...
PROTOCOL:
    ...
    2. Build 117-col feature stack (identical to nb1771).
    3. 5-seed bag LightGBM(huber, max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=10, reg_lambda=2, feature_fraction=0.6,
    ...
    5. Verdict at 0.003 margin vs nb1812 (0.5072) and nb1780 BoB (0.5032).

Outputs:
    scripts/nb1843_lgbm_smaller_subsample.py
    data/processed/nb1843_summary.json
    data/processed/nb1843_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM small leaves baseline (no feature_fraction)
NB1780_REF = 0.5032  # 5-seed BoB on nb1771
NB1812_REF = 0.5072  # LGBM feat_frac=0.7 + bagging_frac=0.8
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1492/nb1543/nb1554/nb1771/nb1812."""
    frames = []
...
def main() -> dict:
    ...
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=4, num_leaves=15, "
          f"n_est=300, lr=0.03, min_child=10, reg_lambda=2, "
          f"feature_fraction=0.6, bagging_fraction=0.7, bagging_freq=5) "
    ...
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1812 ({NB1812_REF:.4f}), nb1780 BoB ({NB1780_REF:.4f})  "
    ...
    # ---- Per-seed LGBM smaller-subsample + min_data_in_leaf=10 cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (smaller subsample: feature_fraction=0.6, "
          f"bagging_fraction=0.7, min_child_samples=10) "
          f"RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    pearson_vs_nb1780 = _pearson_vs(DATA_PROCESSED / "nb1780_mean_bag_oof.npy")
    pearson_vs_nb1812 = _pearson_vs(DATA_PROCESSED / "nb1812_mean_bag_oof.npy")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1812 = {rae_mean_bag - NB1812_REF:+.4f}"
          f"  d_vs_nb1780 = {rae_mean_bag - NB1780_REF:+.4f})")
    ...
    if pearson_vs_nb1780 is not None:
        print(f"   Pearson(mean_bag, nb1780_mean)   = {pearson_vs_nb1780:.4f}")
    if pearson_vs_nb1812 is not None:
        print(f"   Pearson(mean_bag, nb1812_mean)   = {pearson_vs_nb1812:.4f}")
    ...
    # ---- Verdict (vs nb1812 and nb1780 BoB) ----
    ...
    beats_nb1780 = rae_mean_bag < NB1780_REF - DECISION_MARGIN
    beats_nb1812 = rae_mean_bag < NB1812_REF - DECISION_MARGIN
    flat_vs_nb1771 = abs(rae_mean_bag - NB1771_REF) < DECISION_MARGIN
    flat_vs_nb1780 = abs(rae_mean_bag - NB1780_REF) < DECISION_MARGIN
    flat_vs_nb1812 = abs(rae_mean_bag - NB1812_REF) < DECISION_MARGIN
    flat_vs_nb1554 = abs(rae_mean_bag - NB1554_REF) < DECISION_MARGIN

    if beats_nb1780:
        verdict = "LGBM_SMALLER_SUBSAMPLE_BEATS_NB1780_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1780:
        verdict = "LGBM_SMALLER_SUBSAMPLE_FLAT_VS_NB1780_BOB"
    elif beats_nb1812:
        verdict = "LGBM_SMALLER_SUBSAMPLE_BEATS_NB1812_BUT_WORSE_THAN_NB1780_BOB"
    elif flat_vs_nb1812:
        verdict = "LGBM_SMALLER_SUBSAMPLE_FLAT_VS_NB1812"
    elif beats_nb1771:
        verdict = "LGBM_SMALLER_SUBSAMPLE_BEATS_NB1771_BUT_WORSE_THAN_NB1812"
    elif flat_vs_nb1771:
        verdict = "LGBM_SMALLER_SUBSAMPLE_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_SMALLER_SUBSAMPLE_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_SMALLER_SUBSAMPLE_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_SMALLER_SUBSAMPLE_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_SMALLER_SUBSAMPLE_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_SMALLER_SUBSAMPLE_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "delta_mean_bag_vs_nb1780": rae_mean_bag - NB1780_REF,
        "delta_mean_bag_vs_nb1812": rae_mean_bag - NB1812_REF,
        "beats_chemprop_aux": bool(beats_anchor),
        "beats_nb1554": bool(beats_nb1554),
        "beats_nb1771": bool(beats_nb1771),
        "beats_nb1780": bool(beats_nb1780),
        "beats_nb1812": bool(beats_nb1812),
        "flat_vs_nb1554": bool(flat_vs_nb1554),
        "flat_vs_nb1771": bool(flat_vs_nb1771),
        "flat_vs_nb1780": bool(flat_vs_nb1780),
        "flat_vs_nb1812": bool(flat_vs_nb1812),
        "pearson_vs_anchor": pearson_vs_anchor,
        "pearson_vs_nb1771_mean_bag": pearson_vs_nb1771,
        "pearson_vs_nb1780_mean_bag": pearson_vs_nb1780,
        "pearson_vs_nb1812_mean_bag": pearson_vs_nb1812,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1780",
        "delta_mean_bag_vs_nb1812",
        "beats_chemprop_aux",
        "beats_nb1554", "flat_vs_nb1554",
        "beats_nb1771", "flat_vs_nb1771",
        "beats_nb1780", "flat_vs_nb1780",
        "beats_nb1812", "flat_vs_nb1812",
        "pearson_vs_anchor",
        "pearson_vs_nb1771_mean_bag",
        "pearson_vs_nb1780_mean_bag",
        "pearson_vs_nb1812_mean_bag",
