"""nb1932 -- Final exploration: LGBM(MSE) on 117-col 5-way stack with smaller
                learning rate (lr=0.02) and proportionally more rounds (n_est=500).

HYPOTHESIS:
    nb1861 ran outer-BoB of nb1852's recipe (LGBM regression/MSE, max_depth=4,
    num_leaves=15, n_est=300, lr=0.03, min_child=5, reg_lambda=2) on the
    117-col 5-way K-tuned stack and landed at BoB MEAN RAE 0.5013.

    nb1932 lowers learning rate to 0.02 and bumps n_estimators to 500
    (effective ~same total work, finer steps). Does slower/finer boosting
    extract additional residual signal vs nb1861 0.5013 at the 0.003 decision
    margin?

PROTOCOL:
    1. Features = 117-col 5-way K-tuned stack (identical to nb1861/nb1852).
       (top-25 AP + top-20 MACCS + top-20 Mord + top-20 Embed + top-30 Avalon
    ...
    3. LGBM(objective='regression' (MSE), max_depth=4, num_leaves=15,
       n_estimators=500, learning_rate=0.02, min_child_samples=5,
       reg_lambda=2.0). 5-seed bag, 5-fold KFold cross-fit per seed.
       Seeds = [0, 1, 7, 42, 137].
    4. Verdict vs nb1861 BoB MEAN ref 0.5013 (margin 0.003):
          BEATS_NB1861        iff  rae_mean_bag < 0.5013 - 0.003
          MATCHES_NB1861      iff  |rae_mean_bag - 0.5013| <= 0.003
          UNDERPERFORMS_NB1861 otherwise

Outputs:
    scripts/nb1932_lgbm_mse_lr02.py
    data/processed/nb1932_summary.json
    data/processed/nb1932_mean_bag_oof.npy        (253,) float32
"""
...
NB1861_REF = 0.5013   # nb1861 BoB MEAN reference
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- smaller lr=0.02, n_est=500 (vs nb1861 lr=0.03/300)."""
    return dict(
        objective="regression",       # standard MSE / L2
        max_depth=4,
        num_leaves=15,
        n_estimators=500,             # bumped from 300
        learning_rate=0.02,           # lowered from 0.03
        min_child_samples=5,
        reg_lambda=2.0,
        random_state=seed,
        n_jobs=2,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE, max_depth=4, num_leaves=15, n_est=500, "
          f"lr=0.02, min_child=5, reg_lambda=2) on 117-col 5-way stack")
    ...
    # ---- Load 5-way K-grid winners + SHAP rankings (identical to nb1861) ----
    ...
    # ---- Build COMBINED 5-way 117-col feature matrix ----
    ...
    print(f"\n   COMBINED 5-WAY 117-col matrix: {X_unb.shape}")

    # ---- 5-seed bag, 5-fold cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (MSE, lr=0.02, n_est=500) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if pearson_vs_nb1852 is not None:
        print(f"   Pearson(mean_bag, nb1852_mean)       = "
              f"{pearson_vs_nb1852:.4f}")
    if pearson_vs_nb1771 is not None:
        print(f"   Pearson(mean_bag, nb1771_mean)       = "
              f"{pearson_vs_nb1771:.4f}")
    if pearson_vs_nb1554 is not None:
        print(f"   Pearson(mean_bag, nb1554_mean)       = "
              f"{pearson_vs_nb1554:.4f}")
    if pearson_vs_nb1861_pooled is not None:
        print(f"   Pearson(mean_bag, nb1861_pooled25)   = "
              f"{pearson_vs_nb1861_pooled:.4f}")

    # ---- Verdict vs nb1861 BoB MEAN 0.5013 ----
    ...
    summary = {
        ...
        "lgbm_reg_lambda": 2.0,
        "delta_vs_nb1861_recipe": "n_est 300->500, lr 0.03->0.02",
        "K_AP_best": K_AP_best,
        ...
        "pearson_vs_nb1861_pooled25": pearson_vs_nb1861_pooled,
        "verdict": verdict,
