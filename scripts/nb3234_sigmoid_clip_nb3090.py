"""nb3234 -- Sigmoid soft-clip on nb3090 (smooth tail compression vs hard clip).

NEW PARADIGM: smooth sigmoid clipping curve instead of hard min/max clip.

    Prior cycle-clip family (nb3173/nb3190/nb3220) used HARD np.clip(pred, lo, hi)
    on quantile-conditional anchor blends. Hard clip discards rank order in the
    clipped tail mass (all clipped rows pinned to lo OR hi). nb3234 substitutes a
    SMOOTH sigmoid (tanh) soft-clip that preserves monotonic rank order through
    the tails while compressing them toward the central mass.
    ...
    Key properties vs hard clip:
      - Monotone in `raw` (no rank ties introduced)
      - Smooth at the boundary (no derivative discontinuity)
      - Asymptotes to mid +- range/2 (acts like a soft lo/hi at q05/q95)
      - Center near `mid` is approximately the identity (slope ~ slope = 1.5)
      - slope=1.5 makes the linearization midpoint shrink by a factor before tanh

    Hypothesis: if hard-clip headroom (nb3173 0.4437, nb3190 0.4422, nb3220 q50
    test) is primarily from variance compression rather than rank disruption,
    soft-clip should EQUAL or BEAT the hard-clip ceiling because it preserves
    rank order. If hard-clip headroom is partly from rank flattening of bad-
    confidence-tail rows, soft-clip will be WORSE (the bad rows still rank).

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb3090_pred_oof  (253,)  -- q35-quantile-conditional blend
    Per outer fold:
        a) Compute (mid, range) from fold-train y ONLY (no truth leak across folds).
           mid   = mean(y[fold_train])
           range = q95(y[fold_train]) - q05(y[fold_train])
           half  = range / 2
        b) Soft-clip fold-val:
           val_pred = mid + half * tanh(SLOPE * (pred_base[fold_val] - mid) / half)
...
References:
    ...
    nb3220 learned-clip on nb3070       = (q50 sensitivity test)
...
Outputs:
    data/processed/nb3234_summary.json
    data/processed/nb3234_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3234.npy         (513,) float32 -- deploy te
    submissions/nb3234_sigmoid_clip_nb3090.csv  (only on BETTER)
"""
...
def _soft_clip(
    ...
    slope > 1 stretches center then asymptotes).
    """
    if half <= 0:
...
def main() -> dict:
    ...
    if pf_mean < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3234 15-seed PER-FOLD-MEAN {pf_mean:.4f} "
            f"clears BETTER gate {GATE_BETTER:.4f} "
            f"({pf_mean - GATE_BETTER:+.4f}). Sigmoid soft-clip "
            f"(slope={SLOPE}, asymp=q05/q95 of fold_train_y) on q35 anchor "
            f"nb3090 ({REF_PARENT_NB3090:.4f}) -> {pf_mean:.4f} = "
            f"{REF_PARENT_NB3090 - pf_mean:.4f} RAE reduction, and either "
            f"matches or beats the hard-clip sibling nb3190 ({REF_NB3190:.4f}) "
            f"by {REF_NB3190 - pf_mean:+.4f}. Confirms variance-compression "
            f"headroom dominates rank-flattening contribution, and a smooth "
            f"monotone transform preserves rank order without sacrificing the "
            f"compression gain. Re-verify with deep-30 before PRIMARY-1 swap. "
            f"anchor_pre_unblind=True (parent nb3090 built on PRE-clean K18/K19 "
            f"deep-30 OOFs)."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3234 15-seed PER-FOLD-MEAN {pf_mean:.4f} fails BETTER "
            f"gate {GATE_BETTER:.4f} ({pf_mean - GATE_BETTER:+.4f}). Delta vs "
            f"parent nb3090 (pf_mean) = {pf_mean - REF_PARENT_NB3090:+.4f}, "
            f"delta vs hard-clip nb3190 = {pf_mean - REF_NB3190:+.4f}. "
            f"Smooth sigmoid (slope={SLOPE}) either over-compresses the central "
            f"mass (slope > 1 stretches center past identity then asymptotes "
            f"early) or under-compresses the tails (smooth tanh shoulder spans "
            f"a wider raw region than a hard cut). If pf_mean is at or below "
            f"the parent ({REF_PARENT_NB3090:.4f}), soft-clip is at least "
            f"rank-preserving non-degrading; tune slope and asymp quantile pair "
            f"before closing axis. If pf_mean > parent, sigmoid family on this "
            f"anchor is closed. Closes the smooth-clip sensitivity axis at "
            f"slope=1.5, q05/q95 asymptote."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_sigmoid_clip_nb3090.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
