"""nb3241 -- Learned per-fold clip on K=19 deep-30 alone (single anchor).

NEW PARADIGM:
    Sibling of nb3201 (K=18 alone). Strip every blend layer (no pyramid,
    no SLSQP, no rank-stretch) and apply the per-fold (q_low, q_high) grid
    learned-clip primitive directly to K=19 deep-30 OOF (nb3000) -- the
    K=19-feature PRE-unblind LGBM-on-SHAP anchor.

    Hypothesis: K=19 deep-30 alone, like K=18 alone, has variance-compressed
    tails. A learned-clip should therefore extract per-fold tail-taming gain
    on the K=19 anchor. Cross-K comparison vs nb3201 (K=18) isolates the
    feature-set axis from the clip primitive itself.

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb3000_K19_30seed_oof  (253,)
    Per outer fold:
...
GATE (on 15-seed PER-FOLD-MEAN):
    mean < 0.4424 -> "BETTER" (user-specified)
    else          -> "FAIL"

References:
    nb3000 K=19 deep-30 OOF RAE     = 0.4607  <- parent anchor
    nb2960 K=18 deep-30 OOF RAE     = 0.4536  (cross-K context)
    nb3201 K=18 + learned-clip      = (see summary; sibling)
...
Outputs:
    data/processed/nb3241_summary.json
    data/processed/nb3241_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3241.npy         (513,) float32 -- deploy te
    submissions/nb3241_clip_K19_alone.csv  (only on BETTER verdict)
"""
...
TAG = "nb3241"
PARENT_TAG = "nb3000_K19"

# -- Inputs --------------------------------------------------------------------
K_LABEL = "K19"
PRED_OOF_PATH = DATA_PROCESSED / "nb3000_K19_30seed_oof.npy"
TE_PATH = DATA_PROCESSED / "te_nb3000_K19.npy"

# -- CV protocol ---------------------------------------------------------------
N_FOLDS = 5
KF_SEEDS = list(range(1216, 1231))  # 15 FRESH seeds {1216..1230}

# -- Per-fold grid (smaller / simpler than nb3173, same as nb3201) -------------
...
def main() -> dict:
    ...
    print(
        ...
        f"{PARENT_TAG} K=19 deep-30 OOF (single anchor)"
    )
    ...
    # -- Load K=19 deep-30 anchor pred_oof + te -------------------------------
    print("\n" + "-" * 78)
    ...
    n_s = len(pfm_arr)
    # Gate is on per-fold-mean per user spec.
    ...
    print("\n" + "-" * 78)
    print(f"AGGREGATE ({n_s} seeds) -- per-fold-mean basis (gate metric)")
    print("-" * 78)
    print(f"   per_fold_mean_RAE    mean = {mean_rae:.4f}")
    ...
    print(
        f"\n   ref K=19 deep-30 OOF          = {REF_K19_DEEP30:.4f}  "
        f"<- parent (no clip)"
    )
    print(
        f"   delta vs K=19 deep-30 parent  = "
        f"{mean_rae - REF_K19_DEEP30:+.4f}"
    )
    print(f"   ref K=18 deep-30 OOF          = {REF_K18_DEEP30:.4f}")
    ...
    # Median-seed OOF for storage (median by per-fold-mean)
    ...
    print(
        ...
        f"pooled={pooled_arr[med_seed_idx]:.4f})"
    )
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3241 15-seed per-fold-mean {mean_rae:.4f} "
            f"clears BETTER gate {GATE_BETTER:.4f}. Per-fold LEARNED clip on "
            f"K=19 deep-30 alone (single anchor, no blend) beats prior post-hoc "
            f"ceilings. Modal pick was (q{ql_mode:.2f}, q{qh_mode:.2f}). "
            f"Confirms clip primitive is anchor-invariant across K=18/K=19 "
            f"single-anchor variants. Re-verify deep-30 before PRIMARY-1 swap."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3241 15-seed per-fold-mean {mean_rae:.4f} fails "
            f"BETTER gate {GATE_BETTER:.4f}. K=19 deep-30 alone + learned-clip "
            f"({mean_rae - REF_NB3170_FIXED:+.4f} vs nb3170 fixed, "
            f"{mean_rae - REF_K19_DEEP30:+.4f} vs K=19 parent). The K=19 "
            f"feature set + clip primitive does not clear the bar; keep "
            f"current PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_clip_K19_alone.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_K19_deep30_alone"
        ),
        ...
        "per_fold_mean_rae_array": [round(float(v), 4) for v in per_fold_mean_raes],
        ...
        "delta_vs_K19_deep30": round(mean_rae - REF_K19_DEEP30, 4),
    ...
    print(f"   delta vs K=19 parent  = {mean_rae - REF_K19_DEEP30:+.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "pooled_mean_rae", "pooled_std_rae",
        "delta_vs_K19_deep30", "delta_vs_K18_deep30",
        "delta_vs_nb3170_fixed", "delta_vs_nb3080",
        "ql_mode", "qh_mode",
