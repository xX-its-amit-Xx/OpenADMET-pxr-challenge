"""nb2270 -- XGBoost (different boosting paradigm vs LGBM) on RFE K=20
...
HYPOTHESIS:
    Cycle 134 nb1051 tried CatBoost (Plain + Ordered) on the K=28 SHAP
    slice and was FLAT vs nb2103 K=28 LGBM (best 0.4737 vs 0.4737 ref).
    The K=28 feature set is Mordred-heavy and was already pruned to
    K=20 in nb2231 (Mordred 4 / ChempropEmbed 8 / AtomPair 4 / MACCS 1 /
    Avalon 2 / ChEMBL_kNN 1) -- a leaner, less-redundant feature set
    where XGBoost's level-wise + L1/L2-regularised histogram-style
    trees may be orthogonal to LGBM's leaf-wise growth on the same
    K=20 features that nb2240 uses.

PROTOCOL:
    1. Load K=20 RFE surviving feature indices from nb2231 (same exact
       set used by nb2240; family counts {Mordred 4, ChempropEmbed 8,
       AtomPair 4, MACCS 1, Avalon 2, ChEMBL_kNN 1}).
    2. Rebuild the 117-col 5-way K-tuned feature matrix on the 513 test
       compounds (identical construction to nb2240 / nb2103: AtomPair /
       MACCS / Mordred / ChempropEmbed / Avalon top-K + ChEMBL kNN
       prediction + mean similarity), slice to K=20 indices, take the
       unb_idx slice for the 253 unblind.
    3. Anchor = chemprop_aux te[unb_idx]; residual = y_unb - anchor.
    4. XGBRegressor (n_estimators=300, max_depth=4, learning_rate=0.03,
       reg_lambda=2, random_state in {0, 1, 7, 42, 137}).
       Use objective='reg:squarederror', tree_method='hist', n_jobs=2,
       verbosity=0.
       For each seed:
    ...
    5. Compare standalone XGB-K20 (mean_bag + median_bag) vs nb2240
       K=20 LGBM anchor mean_bag RAE 0.4630.
       Also compute XGB+LGBM convex blend at alphas {0.0, 0.25, 0.5,
       0.75, 1.0} (alpha=1.0 -> pure XGB, alpha=0.0 -> pure LGBM K=20).
    6. SLSQP-blend XGB-K20 mean_bag into the nb2240 pyramid (drop-in
       replacement of the nb2240_K20 anchor) -- 5-anchor stack:
         0. nb2270_K20    NEW  (XGB mean-bag K=20)
         1. chemprop_aux
         2. nb1191
         3. nb503
         4. nb562
       ...
       Compare pyramid pooled_rae_mean_seeds vs nb2240 pyramid 0.4598.
       Gate: must beat 0.4601 by >= 0.003 -> deploy threshold 0.4571.
    7. If gate beats: deep-30 verify (5 canonical + 25 extra seeds),
       write submissions/nb2270_xgb_k20.csv (deploy refit on all 253).

ANCHORS:
    chemprop_aux in_RAE = 0.6216
    nb2240 K=20 LGBM mean_bag RAE = 0.4630
    nb2240 pyramid pooled (5-seed mean) RAE = 0.4598
    GATE = 0.4598 - 0.003 = 0.4568  (-- safe margin; task wording says
    "0.003 below 0.4601" so threshold = 0.4571; we take the stricter of
    these two = 0.4568, but report both)

Outputs:
    scripts/nb2270_xgb_k20.py
    data/processed/nb2270_summary.json
    data/processed/nb2270_mean_bag_oof_K20.npy        (253,) float32
    data/processed/nb2270_median_bag_oof_K20.npy      (253,) float32
    data/processed/te_nb2270_K20.npy                  (513,) float32
    submissions/nb2270_xgb_k20.csv                    (only if gate pass)
    data/processed/te_nb2270.npy                      (only if deploy)
"""
...
# nb1191 reconstruction parameters (copied from nb2240/nb2171)
...
# ============================================================================
# helpers (copy/specialised from nb2240/nb2103/nb1051)
# ============================================================================
...
def _residual_cross_fit_xgb(X: np.ndarray, residual: np.ndarray, seed: int) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = XGBRegressor(**_xgb_params(seed))
        mdl.fit(X[tr_loc], residual[tr_loc])
        oof[va_loc] = mdl.predict(X[va_loc])
    return oof


def _train_full_then_predict_te_xgb(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- XGBoost on RFE K=20 features as chemprop_aux residual")
    print(f"          XGB(n_est={XGB_N_ESTIMATORS}, depth={XGB_MAX_DEPTH}, "
          f"lr={XGB_LR}, lambda={XGB_REG_LAMBDA}) seeds={RESID_SEEDS} "
          f"folds={RESID_FOLDS}")
    ...
    # ---- XGB residual cross-fit: 5-seed bag, 5-fold CV ----
    print("\n" + "=" * 78)
    print(f"XGB RESIDUAL CROSS-FIT  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print("=" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ts = time.time()
        resid_oof = _residual_cross_fit_xgb(X_unb_K20, residual, s)
        pred_corr = anchor + resid_oof
        per_seed_corrected[i] = pred_corr
        rae_s = float(rae(y_unb, pred_corr))
        te_resid_s = _train_full_then_predict_te_xgb(
    ...
    rae_xgb_mean_bag = float(rae(y_unb, mean_bag_oof_K20))
    rae_xgb_median_bag = float(rae(y_unb, median_bag_oof_K20))
    rae_per_seed_mean = float(np.mean([r["rae_corrected"] for r in per_seed_records]))
    print(f"\n[XGB-K20] mean-bag RAE     = {rae_xgb_mean_bag:.4f}  "
          f"(vs LGBM K=20 {NB2240_K20_MEAN_BAG_REF:.4f}: "
          f"{rae_xgb_mean_bag - NB2240_K20_MEAN_BAG_REF:+.4f})")
    print(f"[XGB-K20] median-bag RAE   = {rae_xgb_median_bag:.4f}  "
          f"(vs LGBM K=20 {NB2240_K20_MEAN_BAG_REF:.4f}: "
          f"{rae_xgb_median_bag - NB2240_K20_MEAN_BAG_REF:+.4f})")
    print(f"[XGB-K20] per-seed mean    = {rae_per_seed_mean:.4f}")
    print(f"[XGB-K20] anchor in_RAE    = {rae_anchor:.4f}")
    ...
    # ---- XGB + LGBM K=20 convex blend ----
    print("\n" + "-" * 78)
    print(f"XGB + LGBM-K20 CONVEX BLEND  alphas={BLEND_ALPHAS}")
    print("-" * 78)
    ...
    # ============================================================================
    # Stage 2: 5-anchor pyramid SLSQP (XGB K=20 replaces nb2240_K20)
    # ============================================================================
    print("\n" + "=" * 78)
    print("STAGE 2: 5-ANCHOR PYRAMID  (nb2270_XGB_K20 swaps in for nb2240_K20)")
    print("=" * 78)
    ...
    anchors_list = [
        ("nb2270_XGB_K20", mean_bag_oof_K20.astype(np.float64), te_K20_513_mean.astype(np.float64)),
    ...
    print("-" * 78)
    print(f"   nb2270 OOF (5-seed mean)  = {pooled_rae_mean_seeds:.4f}")
    ...
    if gate_beat:
        ...
        print(f"\n[save] {te_npy_path}")
        sub_csv_path = SUBMISSIONS / f"{TAG}_xgb_k20.csv"
        df_sub = pd.DataFrame({
            "SMILES": te_smiles,
        ...
        print(f"\n[skip] gate not beat -- no te_nb2270.npy / submission CSV written "
              f"({verdict})")

    # ---- Save summary ----
    summary = {
        "tag": TAG,
        "method": ("xgboost_RFE_K20_residual_5seed_bag_with_nb2240_pyramid_slsqp"),
        "anchor": ANCHOR,
        ...
        # Standalone XGB
        "xgb_per_seed_records": per_seed_records,
        "rae_xgb_mean_bag": rae_xgb_mean_bag,
        "rae_xgb_median_bag": rae_xgb_median_bag,
        "rae_per_seed_mean": rae_per_seed_mean,
        "delta_xgb_mean_bag_vs_lgbm_K20": rae_xgb_mean_bag - NB2240_K20_MEAN_BAG_REF,
        "delta_xgb_median_bag_vs_lgbm_K20": rae_xgb_median_bag - NB2240_K20_MEAN_BAG_REF,
        # Blend
    ...
    print("\n" + "=" * 78)
    print(f"=== {TAG} SUMMARY ===")
    print(f"   XGB K=20 mean_bag RAE          = {rae_xgb_mean_bag:.4f}")
    print(f"   XGB K=20 median_bag RAE        = {rae_xgb_median_bag:.4f}")
    print(f"   XGB K=20 per-seed mean         = {rae_per_seed_mean:.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_xgb_mean_bag",
        "rae_xgb_median_bag",
        "delta_xgb_mean_bag_vs_lgbm_K20",
        "best_standalone_rae",
        "standalone_verdict",
