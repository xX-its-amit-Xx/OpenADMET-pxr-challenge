"""nb1602 -- CatBoost depth=5 on K-tuned 5-way 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE depth=4, n200, lr0.05, l2=5) yielded mean_bag RAE
    = 0.5163 on 253 PRE-unblind, and the nb1561 outer-bag BoB reproduces
    at 0.5155.  Probe one-step-up tree capacity by widening depth (4->5),
    shortening iters (200->150) to control overfit, nudging lr (0.05->0.06),
    and relaxing l2 (5->3).  Same 117-col 5-way K-tuned feature stack,
    same chemprop_aux anchor, same 5-seed bag x 5-fold cross-fit.

PROTOCOL:
    1. Build 117-col 5-way K-tuned feature stack (= nb1554 recipe).
    2. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    3. 5-seed bag CatBoost(loss=MAE, depth=5, n_est=150, lr=0.06, l2=3).
...
Outputs:
    scripts/nb1602_catboost_d5_5way_K.py
    data/processed/nb1602_summary.json
    data/processed/nb1602_mean_bag_oof.npy        (253,) float32
"""
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1554/nb1561 (no leakage on PRE-unblind path)."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d5, n150, lr0.06, l2=3) on 5-way K-tuned "
    ...
    # Avalon = nb1392 top-30 SHAP ranking (K=30 default)
    ...
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST(d5,n150,lr0.06,l2=3) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if pearson_vs_nb1561_bob is not None:
        print(f"   Pearson(mean_bag, nb1561_bobmean)= {pearson_vs_nb1561_bob:.4f}")
    ...
    if beats_nb1561_bob:
        verdict = "CATBOOST_D5_BEATS_NB1561_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1561_bob:
        verdict = "CATBOOST_D5_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = "CATBOOST_D5_BEATS_NB1554_BUT_WORSE_THAN_NB1561_BOB"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_D5_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_D5_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_D5_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_D5_HURTS_ANCHOR"
