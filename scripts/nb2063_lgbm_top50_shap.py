"""nb2063 -- LightGBM(MSE) on SHAP top-50 of the 117-col 5-way K-tuned matrix.

HYPOTHESIS:
    nb1861 (outer-bag-of-bags of nb1852 / LGBM-MSE on the full 117-col stack)
    reported BoB MEAN RAE = 0.5078 and pooled-25bag RAE = 0.5013 on the 253
    unblind subset with chemprop_aux as PRE-unblind anchor.  Many of the 117
    features carry little signal -- the long tail of SHAP importance is noise.
    Restricting to the top-50 most-important features (by global mean |SHAP|
    of a full-fit LGBM-MSE on the residual) may de-noise the LGBM regressor and
    pick up a marginal RAE improvement, OR reproduce nb1861 within margin.

PROTOCOL:
    1. Anchor   = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build the same 117-col 5-way K-tuned feature stack as nb1852/nb1861.
    3. Train ONE LGBM(MSE) with the same nb1852/nb1861 hyperparams on the FULL
       117-col matrix to fit residual (no folding).  Compute SHAP TreeExplainer
       global mean |SHAP value| per feature.  Take the top-50 by importance.
    4. Restrict X to the top-50 columns.  Run a 5-seed bag (seeds 0,1,7,42,137)
       of LGBM(MSE) with KFold(n=5, shuffle=True) cross-fit per seed -- mirrors
       the inner-seed logic of nb1852 and one outer slice of nb1861.
    5. Report:
         per-seed RAE list (5 numbers)
         per-seed mean / median / std / min / max
         mean-bag RAE on the corrected anchor
         median-bag RAE on the corrected anchor
         top-50 feature names + SHAP importance (ranked)
    6. Verdict vs nb1861 BoB MEAN RAE 0.5078 at decision_margin = 0.003.

Outputs:
    scripts/nb2063_lgbm_top50_shap.py
    data/processed/nb2063_summary.json
    data/processed/nb2063_mean_bag_oof.npy   (253,) float32
    data/processed/nb2063_top50_idx.npy      (50,)  int32  (indices into 117)
    data/processed/nb2063_shap_importance_full117.npy  (117,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- identical to nb1852/nb1861."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) on SHAP top-{TOP_K_SHAP} of 117-col 5-way K-tuned "
          f"matrix; anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          ref: nb1861 BoB MEAN RAE = {NB1861_BOB_MEAN_REF:.4f}  "
          f"margin = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- nb1861 reference (if available) ----
    if NB1861_SUMMARY.exists():
        with open(NB1861_SUMMARY) as f:
            nb1861_sum = json.load(f)
        nb1861_bob_mean = float(nb1861_sum.get("bob_mean_rae",
                                                NB1861_BOB_MEAN_REF))
        nb1861_pooled25 = float(nb1861_sum.get("rae_pooled_25bag",
                                                NB1861_POOLED25_REF))
        print(f"[ref] nb1861.bob_mean_rae      = {nb1861_bob_mean:.4f}")
        print(f"[ref] nb1861.rae_pooled_25bag  = {nb1861_pooled25:.4f}")
    else:
        nb1861_bob_mean = NB1861_BOB_MEAN_REF
        nb1861_pooled25 = NB1861_POOLED25_REF
        print(f"[ref] nb1861_summary missing -- using fallbacks "
              f"{nb1861_bob_mean:.4f} / {nb1861_pooled25:.4f}")
    ...
    # ---- Build feature name list (117 entries) ----
    feat_names: list[str] = []
    ...
    assert len(feat_names) == feat_dim, \
        f"name list len {len(feat_names)} != feat_dim {feat_dim}"

    # ---- Step 1: full-fit LGBM(MSE) on residual + SHAP top-K ----
    print("\n" + "-" * 78)
    print(f"STEP 1: full-fit LGBM(MSE) on 117-col residual -> SHAP top-{TOP_K_SHAP}")
    print("-" * 78)
    t_shap = time.time()
    ...
    top50_idx = np.argsort(-shap_imp)[:TOP_K_SHAP].astype(np.int32)
    top50_names = [feat_names[i] for i in top50_idx]
    top50_family = [feat_family[i] for i in top50_idx]
    top50_imp = [float(shap_imp[i]) for i in top50_idx]
    print(f"   full-fit SHAP done   wall = {time.time() - t_shap:.1f}s")
    print(f"   top-{TOP_K_SHAP} indices (head 10): "
          f"{top50_idx[:10].tolist()}")

    # Family breakdown in top-50
    fam_counts: dict[str, int] = {}
    for fam in top50_family:
        fam_counts[fam] = fam_counts.get(fam, 0) + 1
    print(f"   top-{TOP_K_SHAP} family breakdown: {fam_counts}")
    print(f"   top-10 features by SHAP |mean|:")
    for rank, (name, imp, fam) in enumerate(
            zip(top50_names[:10], top50_imp[:10], top50_family[:10]), 1):
        print(f"     {rank:2d}. [{fam:14s}] {name:30s}  |SHAP|={imp:.5f}")

    # ---- Step 2: restrict to top-50, 5-seed bag x 5-fold cross-fit ----
    X_top50 = X_unb[:, top50_idx].astype(np.float32)
    print(f"\n   X_top50 shape = {X_top50.shape}")

    print("\n" + "-" * 78)
    print(f"STEP 2: PER-SEED LGBM(MSE) RESIDUAL CROSS-FIT on top-{TOP_K_SHAP} "
          f"(dim={X_top50.shape[1]})")
    print("-" * 78)
    ...
    # ---- Pearson vs nb1861/nb1852 pooled OOFs ----
    ...
    pearson_vs_nb1852 = _pearson_vs(DATA_PROCESSED / "nb1852_mean_bag_oof.npy")
    pearson_vs_nb1861_pooled = _pearson_vs(
        DATA_PROCESSED / "nb1861_pooled_25bag_oof.npy"
    )
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1861_BoB = {rae_mean_bag - nb1861_bob_mean:+.4f})")
    ...
    if pearson_vs_nb1852 is not None:
        print(f"   Pearson(mean_bag, nb1852_mean_bag)   = {pearson_vs_nb1852:.4f}")
    if pearson_vs_nb1861_pooled is not None:
        print(f"   Pearson(mean_bag, nb1861_pooled_25b) = {pearson_vs_nb1861_pooled:.4f}")
    if pearson_vs_nb1771 is not None:
        print(f"   Pearson(mean_bag, nb1771_mean_bag)   = {pearson_vs_nb1771:.4f}")
    if pearson_vs_nb1554 is not None:
        print(f"   Pearson(mean_bag, nb1554_mean_bag)   = {pearson_vs_nb1554:.4f}")

    # ---- Verdict vs nb1861 BoB MEAN ----
    delta_vs_nb1861_bob = rae_mean_bag - nb1861_bob_mean
    delta_vs_nb1861_pooled = rae_mean_bag - nb1861_pooled25
    beats_anchor = rae_mean_bag < rae_anchor - DECISION_MARGIN
    beats_nb1861_bob = rae_mean_bag < nb1861_bob_mean - DECISION_MARGIN
    flat_vs_nb1861_bob = abs(delta_vs_nb1861_bob) < DECISION_MARGIN

    if beats_nb1861_bob:
        verdict = "TOP50_SHAP_BEATS_NB1861_BoB_MEAN_NEW_PRIMARY_CANDIDATE"
    elif flat_vs_nb1861_bob:
        verdict = "TOP50_SHAP_FLAT_VS_NB1861_BoB_MEAN"
    elif beats_anchor:
        verdict = "TOP50_SHAP_BEATS_ANCHOR_BUT_WORSE_THAN_NB1861_BoB"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "TOP50_SHAP_FLAT_VS_ANCHOR"
    else:
        verdict = "TOP50_SHAP_HURTS_ANCHOR"
    ...
    np.save(DATA_PROCESSED / f"{TAG}_top50_idx.npy", top50_idx)
    print(f"[save] {DATA_PROCESSED / f'{TAG}_top50_idx.npy'}")
    np.save(DATA_PROCESSED / f"{TAG}_shap_importance_full117.npy",
            shap_imp.astype(np.float32))
    print(f"[save] {DATA_PROCESSED / f'{TAG}_shap_importance_full117.npy'}")

    top50_records = [
        {
            "rank": int(rank),
            "feat_idx_in_117": int(idx),
            "feat_name": str(top50_names[rank - 1]),
            "feat_family": str(top50_family[rank - 1]),
            "mean_abs_shap": float(top50_imp[rank - 1]),
        }
        for rank, idx in enumerate(top50_idx.tolist(), 1)
    ]

    summary = {
        "tag": TAG,
        "method": "lgbm_mse_on_top50_shap_of_117col",
        "anchor": ANCHOR,
        ...
        "top50_family_counts": fam_counts,
        "top50_idx_in_117": top50_idx.tolist(),
        "top50_records": top50_records,
        "K_AP_best": K_AP_best,
        ...
        "delta_mean_bag_vs_nb1852": rae_mean_bag - NB1852_REF,
        "delta_mean_bag_vs_nb1861_bob_mean": delta_vs_nb1861_bob,
        "delta_mean_bag_vs_nb1861_pooled_25bag": delta_vs_nb1861_pooled,
        "beats_chemprop_aux": bool(beats_anchor),
        "beats_nb1861_bob_mean": bool(beats_nb1861_bob),
        "flat_vs_nb1861_bob_mean": bool(flat_vs_nb1861_bob),
        "pearson_vs_anchor": pearson_vs_anchor,
        "pearson_vs_nb1852_mean_bag": pearson_vs_nb1852,
        "pearson_vs_nb1861_pooled_25bag": pearson_vs_nb1861_pooled,
        ...
        "nb1861_pooled_25bag_ref": nb1861_pooled25,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1861_pooled_25bag",
        "beats_chemprop_aux",
        "beats_nb1861_bob_mean", "flat_vs_nb1861_bob_mean",
        "pearson_vs_anchor",
        "pearson_vs_nb1861_pooled_25bag",
        "pearson_vs_nb1852_mean_bag",
        "verdict", "pre_unblind_clean",
        "nb1861_bob_mean_ref",
        "nb1861_pooled_25bag_ref",
    ):
        print(f"  {k}: {res.get(k)}")
