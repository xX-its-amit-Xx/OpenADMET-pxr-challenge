"""nb2158 -- LGBM(MSE) on SHAP top-28 features WITH MONOTONIC CONSTRAINTS on top-10.

HYPOTHESIS:
    Each of the top-10 SHAP features has a true monotonic relationship to the
    chemprop_aux residual at the population level. Enforcing the empirical
    sign of that relationship via LightGBM's monotone_constraints regularizes
    the tree splits, preventing spurious non-monotone reversals in low-sample
    leaves and potentially reducing residual-tail variance.

PROTOCOL:
    1. Reuse top-28 SHAP indices from nb2063 SHAP importance vector.
    2. For each of top-10 by |SHAP|, compute Pearson correlation between
       the feature column (on unblind slice) and the chemprop_aux residual.
       Sign of correlation -> monotone_constraint sign (+1/-1).
       Remaining 18 features get 0 (unconstrained).
    3. 5-seed bag (0, 1, 7, 42, 137), 5-fold cross-fit on residual.
    4. LGBM hyperparams: L=15, lr=0.03, mc=5, lambda=2, n_est=300, ff=1.0.
    5. Compare mean-bag and median-bag RAE vs nb2103 (0.4737/0.4698).
    6. Decision margin 0.003.

Outputs:
    scripts/nb2158_monotone_K28.py
    data/processed/nb2158_summary.json
    data/processed/nb2158_mean_bag_oof_K28_monotone.npy
"""
...
K_TOP = 28
N_MONO = 10  # constrain top-10 by |SHAP|
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb2063/nb2103/nb2142/nb2153."""
    frames = []
...
def _lgbm_params(seed: int, monotone_constraints: list[int]) -> dict:
    """LGBM(MSE) with monotone_constraints vector (length=K_TOP)."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) with MONOTONIC CONSTRAINTS on top-{N_MONO} of "
          f"SHAP top-{K_TOP}")
    ...
    # ---- nb2103 K=28 reference (read summary if present) ----
    ...
    # ---- Infer monotone direction for top-10 features ----
    print("\n" + "=" * 78)
    print(f"MONOTONE DIRECTION INFERENCE (top-{N_MONO} by |SHAP|)")
    print("=" * 78)
    monotone_constraints: list[int] = [0] * K_TOP
    mono_records = []
    for j in range(N_MONO):
        col = X_topK[:, j]
        r = _pearson(col, residual)
        if abs(r) < 0.01:
            sgn = 0
        elif r > 0:
            sgn = 1
            ...
            sgn = -1
        monotone_constraints[j] = sgn
        mono_records.append({
            "rank": int(j + 1),
            "feat_idx_in_117": int(topK_idx[j]),
            "feat_name": topK_names[j],
            "feat_family": topK_family[j],
            "pearson_vs_residual": float(r),
            "monotone_sign": int(sgn),
        })
        print(f"   rank {j+1:2d}  idx={int(topK_idx[j]):3d}  "
              f"{topK_names[j]:<24s}  family={topK_family[j]:<14s}  "
              f"corr={r:+.4f}  sign={sgn:+d}")

    print(f"\n   monotone_constraints vector (length {K_TOP}):")
    print(f"     {monotone_constraints}")
    n_pos = sum(1 for s in monotone_constraints if s == 1)
    n_neg = sum(1 for s in monotone_constraints if s == -1)
    n_zero = sum(1 for s in monotone_constraints if s == 0)
    print(f"     (+1 count = {n_pos}  -1 count = {n_neg}  0 count = {n_zero})")

    # ---- 5-seed bag with monotone-constrained LGBM ----
    print("\n" + "=" * 78)
    print("MONOTONE-CONSTRAINED LGBM(MSE) -- 5-seed bag, 5-fold cross-fit")
    print("=" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        resid_oof_s = _residual_cross_fit_one_seed(
            X_topK, residual, s, monotone_constraints
        )
    ...
    if beats_mean and beats_median:
        verdict = f"BEATS_NB2103_K28_ON_BOTH_BY_>={DECISION_MARGIN}"
    elif beats_mean:
        verdict = "BEATS_NB2103_K28_ON_MEAN_ONLY"
    elif beats_median:
        verdict = "BEATS_NB2103_K28_ON_MEDIAN_ONLY"
    elif flat_mean and flat_median:
        verdict = f"FLAT_VS_NB2103_K28_WITHIN_{DECISION_MARGIN}"
    elif beats_anchor:
    ...
    print(f"   mean-bag       RAE = {rae_mean_bag:.4f}  "
          f"(d_vs_nb2103 = {delta_mean_vs_nb2103:+.4f})")
    print(f"   median-bag     RAE = {rae_median_bag:.4f}  "
          f"(d_vs_nb2103 = {delta_median_vs_nb2103:+.4f})")
    print(f"   anchor (chemprop_aux) RAE = {rae_anchor:.4f}")
    print(f"   VERDICT        = {verdict}")

    # Save mean-bag OOF
    out_oof = DATA_PROCESSED / f"{TAG}_mean_bag_oof_K{K_TOP}_monotone.npy"
    np.save(out_oof, mean_bag_oof.astype(np.float32))
    print(f"   [save] {out_oof}")

    summary = {
        "tag": TAG,
        "method": (f"lgbm_mse_monotonic_constraints_top{N_MONO}_of_shap_top{K_TOP}_"
                   "5seed_5fold_cross_fit_117col_5way_K_tuned"),
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        "5-way K-tuned matrix as nb2063/nb2103/nb2142/nb2153"),
        ...
        "monotone_constraints": monotone_constraints,
        "monotone_n_pos": n_pos,
        "monotone_n_neg": n_neg,
        "monotone_n_zero": n_zero,
        "mono_records_top10": mono_records,
        "feat_breakdown_full": {
            "atompair": n_top_ap,
            "maccs": n_top_maccs,
            "mordred": n_top_mord,
