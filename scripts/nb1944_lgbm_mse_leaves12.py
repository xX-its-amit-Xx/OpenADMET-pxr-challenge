"""nb1944 -- Cycle-99 final exploration: LGBM(MSE) with num_leaves=12.

HYPOTHESIS:
    nb1861 (BoB-of-nb1852) reproduced LGBM(MSE, max_depth=4, num_leaves=15,
    n_est=300, lr=0.03) at RAE 0.5013. Final-cycle question: does a tighter
    leaf budget (num_leaves=12 vs 15) + slightly more trees (350 vs 300)
    crack the 0.5013 floor on the 117-col 5-way K-tuned matrix with the
    chemprop_aux PRE-unblind anchor?

PROTOCOL:
    1. Features: 117-col 5-way K-tuned matrix (identical to nb1861/nb1852/
       nb1771/nb1554):
         - AtomPair top-K_AP_best (from nb1524)
         - MACCS  top-K_MACCS         (from nb1352)
         - Mordred top-K_Mord_best    (from nb1523)
         - ChempropEmbed top-K_Embed  (from nb1541)
         - Avalon top-K_Avalon         (from nb1392)
         - ChEMBL PXR pool kNN: pred_chembl_pec50 + mean_sim   (2 cols)
       feat_dim asserted to equal 117.
    2. Anchor: chemprop_aux te[unb_idx] (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    3. Model: LGBMRegressor(
           objective='regression',  # MSE / L2
           max_depth=4,
           num_leaves=12,           # <-- delta vs nb1861 (15)
           n_estimators=350,        # <-- delta vs nb1861 (300)
           learning_rate=0.03,
           min_child_samples=5,
           reg_lambda=2.0,
       ).
       5-seed bag = seeds [0, 1, 7, 42, 137].
       Each seed: 5-fold KFold cross-fit (shuffle=True, random_state=seed)
       on residual. anchor + resid_oof = pred_corr_seed.
       mean_bag_oof = mean across 5 seeds. rae_mean_bag = rae(y_unb, that).
    4. Verdict vs nb1861 (0.5013):
         BETTER : rae_mean_bag < 0.5013 - 0.005
         TIES   : |rae_mean_bag - 0.5013| <= 0.005
         WORSE  : rae_mean_bag > 0.5013 + 0.005

Outputs:
    scripts/nb1944_lgbm_mse_leaves12.py
    data/processed/nb1944_summary.json
    data/processed/nb1944_per_seed_oof.npy     (5, 253) float32
    data/processed/nb1944_mean_bag_oof.npy     (253,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- nb1944 knobs: num_leaves=12, n_est=350."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Cycle-99 LGBM(MSE) num_leaves={LGBM_NUM_LEAVES} "
          f"n_est={LGBM_N_EST} on 117-col 5-way K-tuned matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  resid_folds = {RESID_FOLDS}")
    ...
    if feat_dim != 117:
        print(f"   [warn] feat_dim {feat_dim} != 117 (expected from K-tuned)")
    print(f"\n   COMBINED 5-WAY K-TUNED matrix: {X_unb.shape}")

    # ---- 5-SEED BAG ----
    print("\n" + "-" * 78)
    print(f"5-SEED BAG: seeds={SEEDS} x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})  num_leaves={LGBM_NUM_LEAVES} n_est={LGBM_N_EST}")
    print("-" * 78)
    ...
    print(f"   per-seed MED   RAE  = {seed_median:.4f}")
    print(f"   per-seed std/min/max= {seed_std:.4f} / {seed_min:.4f} / "
          f"{seed_max:.4f}")
    print(f"   MEAN-BAG       RAE  = {rae_mean_bag:.4f}   <== headline")
    ...
    summary = {
        ...
        "rae_per_seed_mean": seed_mean,
        "rae_per_seed_median": seed_median,
        "rae_per_seed_std": seed_std,
        "rae_per_seed_min": seed_min,
        "rae_per_seed_max": seed_max,
...
if __name__ == "__main__":
    ...
    for k in (
        "seeds", "resid_folds",
        "lgbm_num_leaves", "lgbm_n_estimators",
