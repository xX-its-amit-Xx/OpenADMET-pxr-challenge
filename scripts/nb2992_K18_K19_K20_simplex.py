"""nb2992 -- Per-fold SLSQP simplex on 3-anchor pool {K18, K19, K20}.

NEW PARADIGM: extends nb2982 ({K18, K20} 2-anchor pool, RAE 0.4505) by
inserting K=19 between the two endpoints to test whether an odd-K
mid-anchor unlocks additional cross-fold orthogonality. K=19 is taken
from nb2631 (5-seed cached) -- NOT deep-30 -- and used as-is for this
test. If K=19 contributes non-trivial fold mass and the pooled RAE
improves vs nb2982 (0.4505), upgrade K=19 to deep-30 in a follow-up.

PROTOCOL:
    Anchors:
        K18, K20: nb2960 deep-30 fresh-seed OOFs + te arrays
        K19:      nb2631 5-seed cached OOF + te (NOT deep-30)
    ...
    Deploy:
        ...
        - Apply to (513, 3) stacked te arrays -> te_nb2992

GATE:
    mean_rae < 0.4505  -> "BETTER_THAN_NB2982"
    mean_rae < 0.4570  -> "PROMOTE"
    else               -> "FAIL"

References:
    nb2960 K18 deep-30 OOF        = 0.4536
    nb2631 K19 5-seed OOF         = 0.4625
    nb2960 K20 deep-30 OOF        = 0.4625
    nb2982 per-fold simplex K18,K20 (deep-30) = 0.4505
...
Inputs:
    ...
    data/processed/nb2631_mean_bag_oof_K19.npy
    data/processed/te_nb2631_K19.npy

Outputs:
    data/processed/nb2992_summary.json
    data/processed/nb2992_pred_oof.npy   (253,) float32 -- per-fold simplex OOF
    data/processed/te_nb2992.npy         (513,) float32 -- deploy te
    submissions/nb2992_per_fold_simplex_K18_K19_K20.csv  (only if verdict != "FAIL")
"""
...
# -- Gates ---------------------------------------------------------------------
GATE_BETTER_THAN_NB2982 = 0.4505
GATE_PROMOTE = 0.4570
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 3 K-anchors {K_LABELS}")
    print(f"          parents: nb2960 (K18,K20 deep-30) + nb2631 (K19 5-seed)")
    print(f"          motivation: extend nb2982 (K18,K20) with K=19 mid-anchor")
    print(f"          outer CV: {N_FOLDS}-fold scaffold, kf_seed={KF_SEED}")
    print(f"          per fold: SLSQP simplex w (sum=1, w>=0), {N_STARTS_FOLD} starts")
    print(f"          gate: <{GATE_BETTER_THAN_NB2982} BETTER_THAN_NB2982 / "
    ...
    print("\n" + "-" * 78)
    print("STEP 1: load K-anchor OOFs and te arrays (K18,K20 deep-30; K19 5-seed)")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
    ...
    print("STEP 5: GATE")
    print("-" * 78)
    if pooled_rae < GATE_BETTER_THAN_NB2982:
        verdict = "BETTER_THAN_NB2982"
    ...
    print(f"   delta vs K19 (0.4625)    = {delta_vs_K19:+.4f}")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_simplex_K18_K19_K20.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K18_K19_K20",
        "paradigm": "3_anchor_extend_nb2982_with_K19_midpoint",
        ...
        "gate_better_than_nb2982": GATE_BETTER_THAN_NB2982,
