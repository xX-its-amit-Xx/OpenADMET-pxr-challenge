"""nb1178 — MACE-OFF learned-NNP QM scalars on the DEPLOYED ensemble config.

AIMNet2 QM scalars are now DEPLOYED (nb1177, best 0.4297). This gates a SECOND learned NNP
(MACE-OFF medium, a different DFT functional) ON TOP of the deployed config that already contains
the AIMNet2 block — the honest bar is "does MACE add ANY orthogonal QM signal beyond AIMNet2".

  control   = 4-GBM(combined + 9 AIMNet2 scalars) + CheMeleon + TabPFN + sn_oof   (== deployed 0.4297)
  treatment = 4-GBM(combined + 9 AIMNet2 + 7 MACE scalars) + CheMeleon + TabPFN + sn_oof
Matched on the same MTL ho_idx holdouts (nb1177 convention). Deploy only if matched delta < -0.001
AND treatment < deployed best (best_ensemble.json rae).
"""
import os, sys, json
...
MACE = "C:/pxr_work/mace_off/mace_features.csv"
...
MCOLS = ["mace_energy", "mace_epa", "mace_fmax", "mace_frms", "mace_nestd", "mace_nemin", "mace_nemax"]
...
def main():
    ...
    Xmace, na_mace = aligned(MACE, MCOLS, tr["name"])
    print(f"aimnet imputed={na_aim}  mace imputed={na_mace}  (n_tr={n_tr})")
    ...
    for seed in range(N_SEEDS):
        ...
        scm = StandardScaler().fit(Xmace[trn]); Xm = scm.transform(Xmace)
        Xctrl = Xa; Xtreat = np.hstack([Xa, Xm])
    ...
    print(f"treatment(deployed+AIMNet2+MACE)    RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           "c_raes": c_raes, "t_raes": t_raes, "aimnet_imputed": na_aim, "mace_imputed": na_mace,
           "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1178_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1178_summary.json -> DEPLOY={deploy}")
