"""nb3094 -- Per-anchor scalar stretch then quantile blend.

NEW PARADIGM:
    cycle 169+ closed the post-hoc-blend-on-pre-stretched-anchors axis (nb3080
    wide-seed 15-mean 0.4475 = current ceiling on {K18, K19} deep-30 via
    quantile-conditional hard-split blend q_cut=0.4, w_low=(0.9, 0.1),
    w_high=(0.5, 0.5)). All prior stretch work (nb3022 / nb3052 / nb2922 / etc.)
    has been applied to the BLENDED predictor after the blend. Here we INVERT
    the order: per-fold golden-section stretch each anchor INDIVIDUALLY first
    (independent s_K18, s_K19), then run the nb3080 best-combo quantile blend.

    If K18 and K19 are each variance-compressed on the fold-train manifold but
    by DIFFERENT amounts (which is the case -- nb2960 deep-30 truth_std~1.03,
    K18 pred_std ~0.78, K19 pred_std ~0.74), per-anchor decompression before
    blending should be strictly more expressive than a single scalar applied to
    the blended output, because the per-row quantile-conditional weights see
...
PROTOCOL (per kf_seed, 5-fold scaffold split, anchors LOADED no rebuild):
    For each outer fold (tr_loc, va_loc):
        ...
        mu_K19 = mean(K19_pred[tr_loc])
        s_K18 = argmin_{s in [0.95, 1.20]} RAE(y_tr,
                    _stretch(K18_pred[tr_loc], mu_K18, s))
        s_K19 = argmin_{s in [0.95, 1.20]} RAE(y_tr,
                    _stretch(K19_pred[tr_loc], mu_K19, s))
        K18_s_val = _stretch(K18_pred[va_loc], mu_K18, s_K18)
        K19_s_val = _stretch(K19_pred[va_loc], mu_K19, s_K19)
        q_thr = quantile(_stretch(K18_pred[tr_loc], mu_K18, s_K18), q_cut=0.4)
        blend per-row:
            K18_s_val <= q_thr -> 0.9 * K18_s_val + 0.1 * K19_s_val
            K18_s_val >  q_thr -> 0.5 * K18_s_val + 0.5 * K19_s_val
        oof[va_loc] = blend
    pooled_rae = rae(y_unb, oof)
    Repeat for 15 FRESH kf_seeds {1126..1140}; report mean +/- std + 95% CI.

GATE (on 15-seed mean):
    mean < 0.4475 -> "BETTER_THAN_NB3080"  (new PRIMARY-1 candidate)
    else          -> "FAIL"

References:
    nb3080 15-seed wide-mean (CEILING)    = 0.4475 +/- 0.0006
    nb3073 5-seed best combo              = 0.4470 +/- 0.0009
    nb3063 5-seed mean                    = 0.4477
    nb3070 15-seed wide-mean              = 0.4509
    nb3030 15-seed wide-mean              = 0.4509
    nb3002 15-seed wide-mean              = 0.45105
...
Outputs:
    data/processed/nb3094_summary.json
    data/processed/nb3094_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3094.npy         (513,) float32 -- deploy te
    submissions/nb3094_stretch_then_blend.csv  (only on PROMOTE verdict)
"""
...
# -- Gates ---------------------------------------------------------------------
GATE_BETTER_NB3080 = 0.4475  # mean < this -> BETTER_THAN_NB3080
...
def _blend_quantile_conditional(
    ...
    """nb3080 best-combo per-row hard-split blend applied to STRETCHED anchors.
    ...
    out[low_mask] = (
        W_K18_LOW * p_k18_s[low_mask] + W_K19_LOW * p_k19_s[low_mask]
    )
    out[~low_mask] = (
        W_K18_HIGH * p_k18_s[~low_mask] + W_K19_HIGH * p_k19_s[~low_mask]
    )
    return out
...
def _run_one_seed(
    ...
    """One scaffold-CV pass: per-fold per-anchor stretch then quantile blend."""
    ...
    return {
        ...
        "fold_s_K19": [round(s, 4) for s in fold_s_K19],
        "fold_s_K18_mean": float(np.mean(fold_s_K18)),
        "fold_s_K19_mean": float(np.mean(fold_s_K19)),
        "fold_mu_K18_mean": float(np.mean(fold_mu_K18)),
        "fold_mu_K19_mean": float(np.mean(fold_mu_K19)),
...
def main() -> dict:
    ...
    print("=" * 78)
    print(
        f"{TAG} -- per-anchor scalar STRETCH then nb3080 quantile blend on "
        f"{K_LABELS} deep-30"
    )
    print(
        f"          stretch range : s in [{S_LO}, {S_HI}] golden-section per "
        f"anchor per fold"
    )
    print(
    ...
    print(
        f"          gate          : mean < {GATE_BETTER_NB3080:.4f} -> "
        f"BETTER_THAN_NB3080"
    )
    print("=" * 78)
    ...
    for s in KF_SEEDS:
        ...
        all_s_K19.extend(res["fold_s_K19"])
        seed_records.append({
            "kf_seed": res["kf_seed"],
            ...
            "fold_s_K19": res["fold_s_K19"],
            "fold_s_K18_mean": round(res["fold_s_K18_mean"], 4),
            "fold_s_K19_mean": round(res["fold_s_K19_mean"], 4),
        ...
        print(
            ...
            f"s_K19={res['fold_s_K19_mean']:.3f}  "
    ...
    s_K19_arr = np.asarray(all_s_K19, dtype=np.float64)
    s_K18_mean = float(s_K18_arr.mean())
    s_K18_std = float(s_K18_arr.std(ddof=1))
    s_K19_mean = float(s_K19_arr.mean())
    s_K19_std = float(s_K19_arr.std(ddof=1))
    ...
    print(
        f"   s_K19 mean={s_K19_mean:.4f} +/- {s_K19_std:.4f}  "
        f"(n={len(s_K19_arr)})"
    )
    print(
    ...
    # -- Deploy: full-data per-anchor stretch + blend on 513 -----------------
    # Use cross-fold s means (15 seeds x 5 folds = 75 estimates) for deploy.
    mu_K18_full = float(P_unb[:, 0].mean())
    mu_K19_full = float(P_unb[:, 1].mean())
    s_K18_deploy = s_K18_mean
    s_K19_deploy = s_K19_mean

    te_K18_s = _stretch(P_te[:, 0], mu_K18_full, s_K18_deploy)
    te_K19_s = _stretch(P_te[:, 1], mu_K19_full, s_K19_deploy)
    ...
    te_pred = _blend_quantile_conditional(
        te_K18_s, te_K19_s, deploy_q_thr,
    ).astype(np.float32)
    te_pred = np.clip(te_pred, 3.0, 9.0)
    te_unb_in_rae = float(rae(y_unb, te_pred[unb_idx]))
    te_low_share = float(np.mean(te_K18_s <= deploy_q_thr))
    print(f"\n   deploy mu_K18    = {mu_K18_full:.4f}")
    print(f"   deploy mu_K19    = {mu_K19_full:.4f}")
    print(f"   deploy s_K18     = {s_K18_deploy:.4f}")
    print(f"   deploy s_K19     = {s_K19_deploy:.4f}")
    ...
    print("GATE")
    print("-" * 78)
    if mean_rae < GATE_BETTER_NB3080:
        verdict = "BETTER_THAN_NB3080"
        ladder_action = (
            f"PROMOTE. nb3094 wide-seed 15-mean {mean_rae:.4f} beats "
            f"{PARENT_TAG} {REF_NB3080:.4f} by {mean_rae - REF_NB3080:+.4f}. "
            f"Per-anchor stretch BEFORE blend ({K_LABELS[0]} s={s_K18_mean:.3f}, "
            f"{K_LABELS[1]} s={s_K19_mean:.3f}) extracts further gain on top "
            f"of the nb3080 best-combo quantile-conditional blend. "
            "New PRIMARY-1 candidate."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3094 wide-seed 15-mean {mean_rae:.4f} does NOT beat "
            f"{PARENT_TAG} ceiling {REF_NB3080:.4f} "
            f"(delta {mean_rae - REF_NB3080:+.4f}). Per-anchor stretch before "
            f"quantile blend offers no additional decompression beyond the "
            f"current ceiling; the quantile-conditional weights already "
            f"effectively rescale per region. Keep nb3080 PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_stretch_then_blend.csv"
    ...
    summary = {
        ...
        "method": (
            "per_anchor_golden_section_stretch_then_quantile_conditional_blend_"
            "K18_K19_deep30"
        ),
        "paradigm": (
            "per_fold_independent_scalar_stretch_per_anchor_BEFORE_"
        ...
        "s_K19_mean": round(s_K19_mean, 4),
        "s_K19_std": round(s_K19_std, 4),
        ...
        "deploy_mu_K19": round(mu_K19_full, 4),
        "deploy_s_K18": round(s_K18_deploy, 4),
        "deploy_s_K19": round(s_K19_deploy, 4),
        ...
        "gate_better_nb3080": GATE_BETTER_NB3080,
        "verdict": verdict,
    ...
    print(f"   s_K19 mean            = {s_K19_mean:.4f} +/- {s_K19_std:.4f}")
    print(f"   verdict               = {verdict}")
