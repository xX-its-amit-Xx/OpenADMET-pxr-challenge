"""nb2831 -- LGBM warm-start with init_score = nb2240 prediction (per row).

NEW PARADIGM (boosting-continuation axis):
    Standard residual-stack candidates (nb2103/nb2240/nb2700/nb2171 family)
    train a fresh LGBM on `residual = y_unb - anchor` and add the OOF
    prediction back to the anchor.  This is mathematically equivalent to
    LGBM with init_score = 0 fitting (y_unb - anchor), then summing.

    This script flips the paradigm: the **target is full pEC50** (not
    residual) and `init_score = nb2240_pred` per row is passed to LGBM
    via fit(..., init_score=...).  The LGBM trees then learn the
    correction on top of the strong nb2240 prior, exactly as if nb2240
    were a pre-built committee of boosting rounds and we're continuing
    the boosted ensemble with a small additional budget.

    Hypothesis: warm-starting with the strong nb2240 prior (deep-30
    mean-bag RAE 0.4630 on the K=20 substrate) and a SMALL incremental
    budget (n_est=100, lr=0.01) restricts the additional trees to
    fine-grained corrections only -- the prior already explains most
    of the per-row variance, so the new trees focus their capacity on
    the residual tail (rare scaffolds, near-cliff pairs, prediction-
    disagreement regions).  Equivalent to gradient-boosting with a
    pretrained learner as the constant predictor.

    Concretely:
        init_score = nb2240_pred  (per row, fold-train only)
        target     = y_unb        (full pEC50, NOT residual)
        LGBM       = max_depth=4, num_leaves=15, n_est=100, lr=0.01
                     (smaller budget than nb2240's 300 lr=0.03 because
                      the init is already strong)
        substrate  = K=20 surviving cols from nb2240 (same 117 -> K=20
                     index list)

    LightGBM `predict()` returns the boosted raw score on top of
    init_score, so we must add the per-row init back to the predict
    output to recover the original-scale full prediction.

PROTOCOL:
    1. Load nb2240 mean-bag OOF on 253 (init for unb) and te_nb2240_K20
       on 513 (init for test).
    2. Load K=20 feature substrate (X_117 sliced to nb2240
       k20_surviving_idx_in_117 = 20 cols).
    3. For each kf_seed in {1001..1005}: 5-fold scaffold-CV.  For each
       fold:
         init_tr   = nb2240_pred_oof[tr_loc]
         init_va   = nb2240_pred_oof[va_loc]
    ...
    5. Deploy refit on full 253 with init_score = nb2240_pred_oof, then
       predict 513 te with init_score = te_nb2240_K20.
...
OUTPUTS:
    scripts/nb2831_lgbm_warm_nb2240.py
    data/processed/nb2831_summary.json
    data/processed/nb2831_pred_oof.npy   (253,) float32 (mean-bag corrected pred)
    data/processed/te_nb2831.npy         (513,) float32 deploy refit
    submissions/nb2831_lgbm_warm_nb2240.csv
"""
...
TAG = "nb2831"
ANCHOR = "nb2240"
INIT_OOF_PATH = DATA_PROCESSED / "nb2240_mean_bag_oof_K20.npy"
INIT_TE_PATH = DATA_PROCESSED / "te_nb2240_K20.npy"
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM warm-start with init_score = nb2240 pred (per row)")
    print(f"          target = full pEC50  (NOT residual)")
    print(f"          init   = nb2240_mean_bag_oof (unb) / te_nb2240_K20 (te)")
    ...
    # ---- Load nb2240 init (OOF on unb, te on 513) ----
    if not INIT_OOF_PATH.exists():
        raise FileNotFoundError(f"nb2240 OOF init missing: {INIT_OOF_PATH}")
    if not INIT_TE_PATH.exists():
        raise FileNotFoundError(f"nb2240 te init missing: {INIT_TE_PATH}")
    ...
    if init_oof.shape != (n_unb,):
        raise ValueError(
            f"nb2240 OOF init shape {init_oof.shape} expected ({n_unb},)"
        )
    if init_te_513.shape != (n_test,):
        raise ValueError(
            f"nb2240 te init shape {init_te_513.shape} expected ({n_test},)"
        )
    rae_init = float(rae(y_unb, init_oof))
    print(f"[init] nb2240 OOF  RAE on 253 = {rae_init:.4f}  "
          f"(ref nb2240 K=20 mean-bag = {NB2240_K20_REF:.4f})")
    print(f"[init] nb2240 OOF  mean/std   = "
          f"{init_oof.mean():.4f} / {init_oof.std():.4f}")
    print(f"[init] nb2240 te   mean/std   = "
          f"{init_te_513.mean():.4f} / {init_te_513.std():.4f}")

    # ---- Load X_117 substrate + K=20 cols ----
    ...
    print(f"[5-seed] delta vs nb2240 init  = "
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_lgbm_warm_nb2240.csv"
    pd.DataFrame({
        "SMILES": te_smiles,
    ...
    summary = {
        "tag": TAG,
        "method": "lgbm_warm_start_init_score_nb2240_K20_pred_full_pec50_target",
        "rationale": (
            "Boosting-continuation paradigm: target = full pEC50 (not "
            "residual), init_score = nb2240 prediction per row (fold-train "
            "for CV, full-row vector for deploy refit).  LGBM trees learn "
            "the correction on top of the strong nb2240 prior; small "
            "incremental budget (n_est=100, lr=0.01) because init is "
            "already strong (nb2240 K=20 mean-bag RAE 0.4630).  Equivalent "
            "to gradient-boosting with a pretrained learner as the constant "
            "predictor.  predict() returns raw boosted score on top of "
            "init_score, so we add init back manually to recover original "
            "scale."
        ),
        "init_anchor": ANCHOR,
        ...
        "cv_protocol": (
            ...
            "init_score = nb2240_pred_oof per row (fold-train only); "
        ...
        "delta_vs_nb2240_init": mean_rae - rae_init,
