"""nb2080 -- DEEP verify of nb2060 (PRIMARY-1-LB-CLEAN) with 30 fresh kf_seeds.

Cycle 158 nb2060 ran 5 seeds {1001..1005} and reported pooled-RAE mean = 0.4697
+/- 0.00087, passing the GATE OOF<=0.4700 and promoting nb2060 to
PRIMARY-1-LB-CLEAN (6-anchor PRE/POST mix WITHOUT nb730_honest).

This script triples-plus the seed budget for a higher-confidence verdict:
  - kf_seeds {1056..1085} (30 fresh seeds, DISJOINT from any prior nb2060/2075 run)
  - identical anchors / pipeline / stretch grid as nb2060
  - per-seed pooled RAE; mean / std / median / 95% CI (t, df=29)
  - compare against cycle-158 nb2060 claim (0.4697 +/- 0.00087)
  - final PROMOTE verdict: mean <= 0.4720 AND std <= 0.012
  - if PASS: nb2060 stands as PRIMARY-1-LB-CLEAN-DEEP, parallel to nb1191

Pipeline (IDENTICAL to nb2060):
  6 anchors (chemprop_aux, nb1150, nb1158, nb2112, nb503, nb562; NO nb730)
  -> per-fold SLSQP convex blend -> per-fold rank-stretch
...
Output:
  data/processed/nb2080_summary.json
"""
...
# Fresh disjoint seed band:
#   nb2060 cycle-158 used 1001..1005 (5 seeds)
#   nb2075 (parallel nb1191 deep-verify) used 1026..1055 (30 seeds)
#   nb2080 uses 1056..1085 (30 brand-new seeds, disjoint from both prior runs)
KF_SEEDS_DEEP = list(range(1056, 1086))   # 30 seeds: 1056..1085
KF_SEEDS_NB2060 = list(range(1001, 1006)) # 5 seeds (cycle 158 claim)
STRETCH_GRID = [1.000, 1.025, 1.050, 1.075, 1.100, 1.125, 1.150]
LB_W_OOF = 0.51
LB_W_TE = 0.49
...
# nb2060 (cycle 158) claim
NB2060_CLAIM_MEAN = 0.4697460598768995
NB2060_CLAIM_STD = 0.0008676886816327818
NB2060_CLAIM_N = 5

# nb1191 deep-verify (nb2075) reference for parallel-ladder cross-talk
NB1191_CLAIM_MEAN = 0.4703

# Anchors -- IDENTICAL to nb2060 (6 anchors, nb730_honest INTENTIONALLY EXCLUDED)
ANCHORS = [
...
# nb1150 reconstructed OOF (same as nb2060 / nb1191 / nb1162)
NB1150_SLSQP4_OOFS = [
...
def main() -> dict:
    ...
    print(f"{TAG} -- DEEP re-verify of {PARENT_TAG}  "
          f"({len(KF_SEEDS_DEEP)} fresh disjoint seeds {KF_SEEDS_DEEP[0]}-"
          f"{KF_SEEDS_DEEP[-1]})")
    print("=" * 78)
    ...
    print("\n[anchors]  (6-element PRE/POST mix; nb730_honest EXCLUDED)")
    ...
    # ---- Compare against nb2060 (cycle 158) 5-seed claim ----
    print("\n" + "-" * 78)
    print("COMPARISON: nb2060 (5 seeds, cycle 158) vs nb2080 (30 fresh seeds)")
    print("-" * 78)
    mean_shift = pooled_mean - NB2060_CLAIM_MEAN
    std_ratio = pooled_std / NB2060_CLAIM_STD if NB2060_CLAIM_STD > 0 else float("nan")

    # Welch two-sample t test: nb2080 (n=30) vs nb2060 (n=5)
    sem_pool = float(
        np.sqrt(pooled_std**2 / n_seeds + NB2060_CLAIM_STD**2 / NB2060_CLAIM_N)
    )
    if sem_pool > 0:
    ...
    num = (pooled_std**2 / n_seeds + NB2060_CLAIM_STD**2 / NB2060_CLAIM_N) ** 2
    den = (
        (pooled_std**2 / n_seeds) ** 2 / (n_seeds - 1)
        + (NB2060_CLAIM_STD**2 / NB2060_CLAIM_N) ** 2 / (NB2060_CLAIM_N - 1)
    )
    ...
    print(f"   nb2060 mean +/- std = {NB2060_CLAIM_MEAN:.4f} +/- "
          f"{NB2060_CLAIM_STD:.5f}  (n={NB2060_CLAIM_N})")
    print(f"   nb2080 mean +/- std = {pooled_mean:.4f} +/- "
          f"{pooled_std:.5f}  (n={n_seeds})")
    print(f"   shift (2080 - 2060) = {mean_shift:+.4f}")
    print(f"   std ratio (30/5)    = {std_ratio:.2f}x")
    print(f"   Welch t test        = t={welch_t:+.3f}, df~{welch_df:.1f}, "
          f"p~{welch_p:.4f}")

    nb2060_in_deep_ci = ci_low <= NB2060_CLAIM_MEAN <= ci_high
    print(f"   nb2060 mean in nb2080 95%CI?  {nb2060_in_deep_ci}")
    ...
    # parent deploy CSV produced by nb2060 itself
    sub_csv_path = SUBMISSIONS / f"{PARENT_TAG}_deploy_no_nb730.csv"
    sub_csv_exists = sub_csv_path.exists()
    print(f"\n[deploy-file] {sub_csv_path}  exists={sub_csv_exists}")

    if gate_pass and sub_csv_exists:
        ladder_action = "PROMOTE_PRIMARY_1_LB_CLEAN_DEEP_CONFIRMED"
        verdict = "PROMOTE"
        print(f"[ladder] -> CONFIRM {sub_csv_path.name} as "
              f"PRIMARY-1-LB-CLEAN-DEEP  (parallel to nb1191)")
        ...
        ladder_action = "HOLD_DEEP_FAIL"
        verdict = "HOLD"
        print("[ladder] DEEP HOLD; nb2060 NOT confirmed as PRIMARY-1-LB-CLEAN-DEEP")

    summary = {
        "tag": TAG,
        "parent_tag": PARENT_TAG,
        "method": "deep_seed_reverify_no_nb730_pyramid_30seed",
        ...
        "kf_seeds_nb2060": KF_SEEDS_NB2060,
        "n_folds": N_FOLDS,
        ...
        "nb2060_claim_mean": NB2060_CLAIM_MEAN,
        "nb2060_claim_std": NB2060_CLAIM_STD,
        "nb2060_claim_n": NB2060_CLAIM_N,
        "mean_shift_vs_nb2060": float(mean_shift),
        "std_ratio_vs_nb2060": float(std_ratio),
        "welch_t_vs_nb2060": float(welch_t),
        "welch_df_vs_nb2060": float(welch_df) if not np.isnan(welch_df) else None,
        "welch_p_vs_nb2060": float(welch_p) if not np.isnan(welch_p) else None,
        "nb2060_mean_in_deep_ci95": bool(nb2060_in_deep_ci),
    ...
    print(f"   nb2060 claim (n=5)    = {NB2060_CLAIM_MEAN:.4f} +/- "
          f"{NB2060_CLAIM_STD:.5f}")
    print(f"   shift vs nb2060       = {mean_shift:+.4f}  "
