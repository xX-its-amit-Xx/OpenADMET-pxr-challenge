"""nb1963 -- LGBM(MSE) feature-subset ablation: DROP ChempropEmbed family.

HYPOTHESIS:
    nb1861 (BoB of nb1852, RAE 0.5013) used 117-col matrix that included
    ChempropEmbed (40-col K-tuned slice). If ChempropEmbed is collinear with
    the chemprop_aux anchor (it IS the same Chemprop backbone embedding),
    its residual signal may be near-zero and possibly even harmful. Dropping
    it could either:
      (a) match nb1861 (~0.5013) -- ChempropEmbed was inert;
      (b) beat nb1861 (<0.5013) -- ChempropEmbed was *noise-amplifying* the
          anchor; OR
      (c) lose vs nb1861 (>0.5013) -- ChempropEmbed carried real residual
          structure orthogonal to AP/MACCS/Mord/Avalon/ChEMBL.

PROTOCOL:
    1. Features = top-25 AP (nb1524) + top-20 MACCS (nb1352)
                 + top-20 Mordred (nb1523) + top-30 Avalon (nb1392)
                 + ChEMBL kNN pred + ChEMBL mean_sim
                 = 97 cols (NO chemprop embed; task spec listed "77" but
                 the K-grid winners sum to 97 with these allotments).
    ...
    3. LGBM(objective='regression' (MSE), max_depth=4, num_leaves=15,
       ...
       reg_lambda=2). Mirror nb1861 hyperparams exactly.
    4. 5-seed bag {0, 1, 7, 42, 137}, 5-fold KFold cross-fit per seed
       (shuffle=True, random_state=seed). mean_bag_oof = mean of 5 seed OOFs.
    5. Report per-seed RAE, mean-bag RAE, verdict vs nb1861 (0.5013).
       Verdict thresholds:
         BEATS_NB1861        : mean-bag RAE < 0.4963 (>=0.005 below)
         MATCHES_NB1861      : |mean-bag RAE - 0.5013| < 0.005
         LOSES_TO_NB1861     : mean-bag RAE > 0.5063 (>=0.005 above)

Outputs:
    scripts/nb1963_lgbm_mse_no_embed.py
    data/processed/nb1963_summary.json
    data/processed/nb1963_per_seed_corrected_oof.npy  (5, 253) float32
    data/processed/nb1963_mean_bag_oof.npy            (253,)   float32
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) on NO-ChempropEmbed feature stack")
    print(f"          features = AP + MACCS + Mordred + Avalon + ChEMBL kNN")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}  anchor = {ANCHOR}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1861 ({NB1861_REF_FALLBACK:.4f})")
    print(f"          verdict_margin = {VERDICT_MARGIN}")
    print("=" * 78)
    ...
    if NB1861_SUMMARY.exists():
        ...
        print(f"[ref] nb1861_summary.json missing -- fallback {nb1861_ref:.4f}")
    ...
    print(f"[reuse] *** ChempropEmbed: DROPPED (this is the ablation) ***")

    # ---- Feature matrices (NO ChempropEmbed) ----
    ...
    print(f"\n   COMBINED 4-WAY (NO-EMBED) matrix: {X_unb.shape}")
    ...
    for s_i, seed in enumerate(SEEDS):
        t_s = time.time()
        resid_oof_s = _residual_cross_fit_one_seed(X_unb, residual, seed)
        pred_corr_s = anchor + resid_oof_s
        per_seed_corrected[s_i] = pred_corr_s
        ...
        print(f"   seed={seed:4d}  rae_corrected={r_s:.4f}  "
    ...
    print(f"   per-seed std       = {seed_std:.4f}")
    print(f"   mean-bag RAE       = {rae_mean_bag:.4f}")
    print(f"   median-bag RAE     = {rae_median_bag:.4f}")
    print(f"   nb1861 ref         = {nb1861_ref:.4f}")
    print(f"   d(mean-bag, nb1861)= {rae_mean_bag - nb1861_ref:+.4f}")
    print(f"   d(mean-bag, anchor)= {rae_mean_bag - rae_anchor:+.4f}")

    # ---- Verdict ----
    delta_vs_nb1861 = rae_mean_bag - nb1861_ref
    if delta_vs_nb1861 < -VERDICT_MARGIN:
        verdict = "BEATS_NB1861"
    elif delta_vs_nb1861 > VERDICT_MARGIN:
        verdict = "LOSES_TO_NB1861"
    else:
    ...
    summary = {
        ...
        "ablation": "DROP_ChempropEmbed",
        "data_source": ("AtomPair-cache + MACCS-cache + "
                        "Mordred-cached_nb1030 + "
...
if __name__ == "__main__":
    ...
    for k in (
        "seeds", "resid_folds",
        "K_AP_best", "K_Mord_best", "K_Avalon_used", "K_MACCS_fixed",
        "K_Embed_used",
        "n_chembl_pool", "feat_dim",
        "rae_anchor_chemprop_aux", "per_seed_rae", "per_seed_std",
