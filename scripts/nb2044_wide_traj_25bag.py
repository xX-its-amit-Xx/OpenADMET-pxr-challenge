"""nb2044 -- Wide-window boosting-trajectory averaging combined with 25-bag protocol.

HYPOTHESIS:
    nb2034 tested trajectory averaging on a NARROW late window
    [200, 225, 250, 275, 300] -- found neutral vs last-only (mean_bag 0.5083 vs
    0.5072) at a single 5-seed bag, because the late iterations are highly
    correlated (delta_traj_minus_last per-seed is +/- 0.001).

    nb2031 produced the leader at lambda=3, pooled-25-bag = 0.5007.

    nb2044 combines BOTH ideas: per-fold trajectory averaging over a WIDE window
    [100, 140, 180, 220, 260, 300] = 6 snapshots that span the full late
    boosting epoch (not just the last 33% as in nb2034), and runs this inside
    the nb2031 outer-bag(5) x inner-bag(5) protocol so 25 distinct LGBM fits
    each contribute a trajectory-smoothed prediction.

    The expectation: wide-window snapshots are LESS correlated with each other
    than the narrow late-window snapshots of nb2034, so their average has more
    bias-vs-variance trade-off room. If late-only trajectory was neutral
    because the snapshots are functionally identical, a wider window should
    either (a) hurt (early iterations underfit) or (b) help (true SWA-style
    averaging over a longer convergence arc). The 25-bag wrapper absorbs the
    seed-level noise so the comparison vs nb2031 is honest.

    Anchor / feature stack identical to nb1861 / nb2023 / nb2031 / nb2034.
...
PROTOCOL:
    ...
    3. For each outer seed s in {0, 1, 7, 42, 137}:
         ...
         per-inner: 5-fold cross-fit LGBM (regression/MSE, depth=4, leaves=15,
                    300 rounds, lr=0.03, min_child=5, lambda=3.0).
         Per fold, predict at iterations {100, 140, 180, 220, 260, 300} and
         average -> trajectory-smoothed validation prediction.
    ...
    5. Verdict vs nb2031 reference (last-only pooled-25 = 0.5007).

Outputs:
    scripts/nb2044_wide_traj_25bag.py
    data/processed/nb2044_summary.json
    data/processed/nb2044_per_outer_oof.npy        (5, 253) float32
    data/processed/nb2044_pooled_25bag_oof.npy     (253,) float32
"""
...
TRAJ_SNAPSHOTS = [100, 140, 180, 220, 260, 300]   # 6-point WIDE-window SWA
...
NB2031_POOLED25_REF = 0.5007    # 25-bag last-only reference (the target)
NB2034_MEAN_BAG_TRAJ_REF = 0.5083    # narrow-window single 5-seed bag
...
def _residual_cross_fit_trajectory(X: np.ndarray, residual: np.ndarray,
    ...
    per fold predict at TRAJ_SNAPSHOTS and average.
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Wide-window trajectory averaging + 25-bag protocol")
    print(f"          anchor={ANCHOR}  outer_seeds={OUTER_SEEDS}")
    print(f"          inner_per_outer={INNER_PER_OUTER}  folds={RESID_FOLDS}")
    print(f"          lambda={REG_LAMBDA}  snapshots={TRAJ_SNAPSHOTS}")
    print(f"          refs: nb2031 pooled-25 last-only ({NB2031_POOLED25_REF:.4f}),"
          f" nb2034 narrow-window ({NB2034_MEAN_BAG_TRAJ_REF:.4f})")
    ...
    print("\n" + "=" * 78)
    print(f"OUTER {len(OUTER_SEEDS)} x INNER {INNER_PER_OUTER} TRAJECTORY BAG")
    print(f"   snapshots = {TRAJ_SNAPSHOTS} ({len(TRAJ_SNAPSHOTS)} per fold)")
    print(f"   lambda = {REG_LAMBDA}   dim = {feat_dim}")
    print("=" * 78)
    ...
    print(f"   d(BoB MEAN)   vs nb2031     = "
          f"{bob_mean_rae - NB2031_BOB_MEAN_REF:+.4f}")
    print(f"   d(pooled-25)  vs nb2031     = "
          f"{rae_pooled_25 - NB2031_POOLED25_REF:+.4f}")
    print(f"   d(BoB MEAN)   vs nb2034     = "
          f"{bob_mean_rae - NB2034_MEAN_BAG_TRAJ_REF:+.4f}")
    ...
    if delta_pooled < -DECISION_MARGIN:
        verdict_pooled = "WIDE_TRAJECTORY_BEATS_NB2031_POOLED25"
    elif delta_pooled > DECISION_MARGIN:
        verdict_pooled = "WIDE_TRAJECTORY_HURTS_VS_NB2031_POOLED25"
    else:
        verdict_pooled = "WIDE_TRAJECTORY_NEUTRAL_VS_NB2031_POOLED25"
    if delta_bob < -DECISION_MARGIN:
        verdict_bob = "WIDE_TRAJECTORY_BEATS_NB2031_BOB_MEAN"
    elif delta_bob > DECISION_MARGIN:
        verdict_bob = "WIDE_TRAJECTORY_HURTS_VS_NB2031_BOB_MEAN"
    else:
        verdict_bob = "WIDE_TRAJECTORY_NEUTRAL_VS_NB2031_BOB_MEAN"
    ...
    summary = {
        ...
        "trajectory_method": "wide-window-iteration-snapshot-average",
...
if __name__ == "__main__":
    ...
    print(f"  nb2031 pooled-25 ref   : {res['nb2031_pooled25_ref']:.4f}")
    ...
    print(f"  pooled-25 (trajectory) : {res['rae_pooled_25bag']:.4f}")
