"""nb2153 -- LGBM(MSE) on SHAP top-28 features, feature_fraction grid.

HYPOTHESIS:
    nb2103 K=28 (LGBM MSE, top-28 SHAP, feature_fraction default=1.0) achieves
    mean-bag RAE 0.4737 on the 253 unblind. Lowering feature_fraction injects
    stochastic feature sub-sampling at each tree, decorrelating trees in the
    bag and potentially reducing variance on the F2 (greasy-novel-inactive)
    tail. This notebook tests feature_fraction in {0.7, 0.8, 0.9, 1.0} at the
    same K=28 SHAP-pruning level.

PROTOCOL:
    1. Reuse top-28 SHAP indices from nb2063 SHAP importance vector (full117).
    2. For each fraction in {0.7, 0.8, 0.9, 1.0}:
         LGBM(MSE, feature_fraction=X, max_depth=4, num_leaves=15,
              n_estimators=300, learning_rate=0.03, min_child_samples=5,
              reg_lambda=2.0).
       5-seed bag (seeds 0, 1, 7, 42, 137), 5-fold cross-fit on residual.
    3. Report per-fraction per-seed RAE, mean-bag RAE, family breakdown.
    4. Verdict vs nb2103 K=28 (0.4737) at decision_margin = 0.003.

Outputs:
    scripts/nb2153_featfrac_K28.py
    data/processed/nb2153_summary.json
    data/processed/nb2153_mean_bag_oof_K28_ff{ff}.npy   per-fraction
"""
...
# Feature-fraction grid
FEAT_FRAC_GRID = [0.7, 0.8, 0.9, 1.0]
...
NB2103_K28_MSE_REF = 0.4737   # nb2103 K=28 mean-bag RAE (LGBM MSE, ff=1.0)
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb2063/nb2103/nb2142."""
    frames = []
...
def _lgbm_params(seed: int, feat_frac: float) -> dict:
    """LGBM(MSE) with explicit feature_fraction."""
    return dict(
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, feat_frac: float) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = lgb.LGBMRegressor(**_lgbm_params(seed, feat_frac))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) feature_fraction grid on SHAP top-{K_TOP} "
          f"of 117-col 5-way K-tuned matrix")
    print(f"          anchor={ANCHOR}  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          feat_frac grid = {FEAT_FRAC_GRID}")
    print(f"          ref: nb2103 K=28 mean-bag RAE = "
    ...
    # ---- Grid over feature_fraction ----
    print("\n" + "=" * 78)
    print(f"FEATURE_FRACTION GRID  (LGBM MSE, 5-seed bag, 5-fold cross-fit)")
    print("=" * 78)
    per_ff_records = []
    best_ff = None
    best_rae_mean_bag = float("inf")
    for ff in FEAT_FRAC_GRID:
        print(f"\n   feat_frac = {ff}")
        print("   " + "-" * 70)
        ...
        for i, s in enumerate(RESID_SEEDS):
            ts = time.time()
            resid_oof_s = _residual_cross_fit_one_seed(X_topK, residual,
                                                       s, ff)
        ...
        if beats_nb2103:
            verdict_ff = f"BEATS_NB2103_K28_BY_>={DECISION_MARGIN}"
        elif flat_vs_nb2103:
            verdict_ff = f"FLAT_VS_NB2103_K28_WITHIN_{DECISION_MARGIN}"
        elif beats_anchor:
            verdict_ff = "BEATS_ANCHOR_BUT_WORSE_THAN_NB2103_K28"
        elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
            verdict_ff = "FLAT_VS_ANCHOR"
        else:
            verdict_ff = "HURTS_VS_NB2103_K28"
        ...
        print(f"      verdict       = {verdict_ff}")

        # Save mean-bag OOF per fraction
        ff_tag = f"{int(round(ff*100)):03d}"
        out_p = DATA_PROCESSED / f"{TAG}_mean_bag_oof_K{K_TOP}_ff{ff_tag}.npy"
        np.save(out_p, mean_bag_oof.astype(np.float32))
        print(f"      [save] {out_p}")

        per_ff_records.append({
            "feat_frac": float(ff),
            ...
            "verdict": verdict_ff,
            "mean_bag_oof_path": str(out_p),
        })

        if rae_mean_bag < best_rae_mean_bag:
            best_rae_mean_bag = rae_mean_bag
            best_ff = float(ff)

    # ---- Final summary across grid ----
    print("\n" + "=" * 78)
    print("GRID SUMMARY (RAE vs feature_fraction)")
    print("=" * 78)
    print(f"   {'feat_frac':<10} {'mean_bag':<10} {'median_bag':<12} "
          f"{'d_vs_nb2103':<14} {'verdict'}")
    for r in per_ff_records:
        print(f"   {r['feat_frac']:<10.2f} {r['rae_mean_bag']:<10.4f} "
              f"{r['rae_median_bag']:<12.4f} "
              f"{r['delta_mean_bag_vs_nb2103_K28']:<+14.4f} {r['verdict']}")
    print(f"\n   BEST feat_frac = {best_ff}  rae_mean_bag = {best_rae_mean_bag:.4f}  "
          f"d_vs_nb2103_K28 = {best_rae_mean_bag - nb2103_k28_mean_bag:+.4f}")

    summary = {
        "tag": TAG,
        "method": (f"lgbm_mse_feature_fraction_grid_shap_top{K_TOP}_5seed_"
                   "5fold_cross_fit_117col_5way_K_tuned"),
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        "5-way K-tuned matrix as nb2063/nb2103/nb2142 "
        ...
        "per_ff_records": per_ff_records,
        "best_feat_frac": best_ff,
        "best_rae_mean_bag": best_rae_mean_bag,
        "best_delta_vs_nb2103_K28": best_rae_mean_bag - nb2103_k28_mean_bag,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "best_feat_frac",
        "best_rae_mean_bag",
        "best_delta_vs_nb2103_K28",
        "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
    print("\nper_ff_records:")
    for r in res["per_ff_records"]:
        print(f"  ff={r['feat_frac']:.2f}  mean_bag={r['rae_mean_bag']:.4f}  "
              f"median_bag={r['rae_median_bag']:.4f}  "
              f"d_vs_nb2103={r['delta_mean_bag_vs_nb2103_K28']:+.4f}  "
              f"{r['verdict']}")
