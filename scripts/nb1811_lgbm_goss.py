"""nb1811 -- LightGBM GOSS boosting on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1771 (LGBM Huber small-leaves, gbdt) hit rae_mean_bag 0.5100 on the
    same 117-col 5-way K-tuned matrix using min_child_samples=5 and
    max_depth=4/num_leaves=15.  GOSS (Gradient-based One-Side Sampling)
    keeps large-gradient samples (the residual tail) at full weight and
    sub-samples small-gradient samples, which may extract a different
    signal regime on the novel-scaffold tail of the 253 unblind.
...
PROTOCOL:
    ...
    2. Build 117-col feature stack (identical to nb1771/nb1554).
    3. 5-seed bag LightGBM(boosting_type='goss', huber, max_depth=4,
       num_leaves=15, n_est=300, lr=0.03, min_child_samples=5,
       reg_lambda=2).  KFold(n=5, shuffle=True) cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1771 (0.5100).

Outputs:
    scripts/nb1811_lgbm_goss.py
    data/processed/nb1811_summary.json
    data/processed/nb1811_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM Huber small-leaves, gbdt boosting
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(boosting='goss', huber, alpha=1.0, max_depth=4, "
          f"num_leaves=15, n_est=300, lr=0.03, min_child=5, reg_lambda=2) on "
    ...
    # ---- Per-seed LGBM GOSS residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (boosting='goss', huber, small leaves) RESIDUAL "
          f"CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1771:
        verdict = "LGBM_GOSS_BEATS_NB1771_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1771:
        verdict = "LGBM_GOSS_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_GOSS_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_GOSS_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "LGBM_GOSS_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_GOSS_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_GOSS_HURTS_ANCHOR"
