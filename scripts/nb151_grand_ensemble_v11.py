"""nb151 — Grand Ensemble v11 (All models through nb150+).

nb149 LGBM_MAE meta-stack: OOF 0.3069 (new best single model!).
nb145 SLSQP blend: OOF 0.3125 (best ensemble).

This notebook runs an exhaustive search over all non-collapsed models
with the new nb149 as the expected dominant model.

Strategy: same as nb144 but with updated pool including nb149.
Runs after nb149 saves its OOF file.
"""
...
def main():
    print("=== nb151: Grand Ensemble v11 (with nb149 LGBM_MAE) ===\n")
    ...
    np.save(DATA_PROCESSED / "oof_nb151_grand_v11.npy", best_oof)
    np.save(DATA_PROCESSED / "te_nb151_grand_v11.npy",  best_te)
    sub = pd.DataFrame({"Molecule Name": te["name"].values, "pEC50": best_te})
    sub.to_csv(SUBMISSIONS / "151_grand_ensemble_v11.csv", index=False)
    print(f"\nSaved: submissions/151_grand_ensemble_v11.csv")
