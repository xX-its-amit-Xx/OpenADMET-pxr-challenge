"""nb2071 -- Outer-bag VALIDATION of nb2063 (SHAP top-50 LGBM MSE).

PROTOCOL:
    1. Reuse the top-50 SHAP feature indices from nb2063 (saved in
       nb2063_summary.json -- field 'top50_idx_in_117').
    2. For 5 OUTER seeds {0, 1, 7, 42, 137}:
         Rebuild a 5-INNER-seed bag of LGBM(MSE) on top-50 features with
         chemprop_aux anchor.  Inner seeds = [o*1000 + s for s in 0..4]
         where o is the outer seed.
    3. Per-outer pooled RAE = RAE(y_unb, anchor + mean over 5 inner OOFs).
    4. BoB MEAN over the 5 outer RAEs.  BoB MEDIAN over the 5 outer RAEs.
    5. Verdict NB2063_REPRODUCES if per-outer (BoB) MEAN within 0.003 of
       the nb2063 mean_bag reference 0.4933.

Outputs:
    scripts/nb2071_bag_nb2063.py
    data/processed/nb2071_summary.json
    data/processed/nb2071_per_outer_mean_bag_oof.npy  (5, 253) float32
"""
...
N_INNER = 5  # inner seeds per outer
TOP_K_SHAP = 50
...
# Reference (per-outer mean target -- nb2063 mean-bag RAE on top-50)
NB2063_MEAN_BAG_REF = 0.4933
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Outer-bag VALIDATION of nb2063 (SHAP top-{TOP_K_SHAP} "
          f"LGBM-MSE)")
    print(f"          outer seeds = {OUTER_SEEDS}   inner per outer = "
          f"{N_INNER}  folds = {RESID_FOLDS}")
    print(f"          ref: nb2063 mean_bag RAE = {NB2063_MEAN_BAG_REF:.4f}  "
    ...
    nb2063_mean_bag_actual = float(nb2063_sum.get("rae_mean_bag",
                                                   NB2063_MEAN_BAG_REF))
    print(f"[reuse] nb2063 top-{TOP_K_SHAP} indices: "
          f"{top50_idx[:10].tolist()}...")
    print(f"[reuse] nb2063 rae_mean_bag (actual) = "
    ...
    # ---- Outer-bag-of-bags ----
    print("\n" + "-" * 78)
    print(f"OUTER-BAG-OF-BAGS:  {len(OUTER_SEEDS)} outer x {N_INNER} inner "
    ...
    for o_i, o in enumerate(OUTER_SEEDS):
        t_o = time.time()
        inner_seeds = [o * 1000 + s for s in range(N_INNER)]
        ...
        for i_s, s in enumerate(inner_seeds):
            ...
            inner_rae_list.append(float(rae(y_unb, pred_corr_s)))
        mean_bag_o = inner_corrected.mean(axis=0)
        per_outer_mean_bag[o_i] = mean_bag_o
        ...
        per_outer_records.append({
            ...
            "delta_vs_nb2063_ref": rae_o - NB2063_MEAN_BAG_REF,
            "wall_sec": round(time.time() - t_o, 2),
        })
        print(f"   outer {o:3d}:  inner_seeds={inner_seeds}  "
              f"inner_RAE=[{', '.join(f'{r:.4f}' for r in inner_rae_list)}]  "
              f"pooled_mean_bag={rae_o:.4f}  "
              f"d_vs_nb2063={rae_o - NB2063_MEAN_BAG_REF:+.4f}  "
              f"wall={time.time() - t_o:.1f}s")

    per_outer_rae_arr = np.array(per_outer_rae, dtype=np.float64)
    bob_mean = float(per_outer_rae_arr.mean())
    bob_median = float(np.median(per_outer_rae_arr))
    bob_std = float(per_outer_rae_arr.std())
    bob_min = float(per_outer_rae_arr.min())
    bob_max = float(per_outer_rae_arr.max())

    print("\n" + "-" * 78)
    print("BoB AGGREGATIONS")
    print("-" * 78)
    print(f"   per-outer RAE list = "
          f"[{', '.join(f'{r:.4f}' for r in per_outer_rae)}]")
    print(f"   BoB MEAN   RAE     = {bob_mean:.4f}  "
          f"(d_vs_nb2063 = {bob_mean - NB2063_MEAN_BAG_REF:+.4f})")
    print(f"   BoB MEDIAN RAE     = {bob_median:.4f}  "
          f"(d_vs_nb2063 = {bob_median - NB2063_MEAN_BAG_REF:+.4f})")
    print(f"   BoB STD            = {bob_std:.4f}")
    print(f"   BoB min/max        = {bob_min:.4f} / {bob_max:.4f}")

    # ---- Verdict ----
    delta_bob_mean_vs_nb2063 = bob_mean - NB2063_MEAN_BAG_REF
    reproduces = abs(delta_bob_mean_vs_nb2063) <= DECISION_MARGIN

    if reproduces:
        verdict = "NB2063_REPRODUCES"
    elif delta_bob_mean_vs_nb2063 < -DECISION_MARGIN:
        verdict = "NB2071_BEATS_NB2063_OUTSIDE_MARGIN"
    else:
        verdict = "NB2071_WORSE_THAN_NB2063_OUTSIDE_MARGIN"
    print(f"\n   verdict = {verdict}")
    print(f"   |BoB MEAN - nb2063_ref| = "
          f"{abs(delta_bob_mean_vs_nb2063):.4f}  vs margin {DECISION_MARGIN}")

    # ---- Save artifacts ----
    np.save(DATA_PROCESSED / f"{TAG}_per_outer_mean_bag_oof.npy",
            per_outer_mean_bag.astype(np.float32))
    print(f"\n[save] "
          f"{DATA_PROCESSED / f'{TAG}_per_outer_mean_bag_oof.npy'}")

    summary = {
        "tag": TAG,
        "method": "outer_bag_of_bags_validation_of_nb2063",
        "anchor": ANCHOR,
        ...
        "delta_bob_mean_vs_nb2063_ref": delta_bob_mean_vs_nb2063,
        "delta_bob_mean_vs_nb2063_actual":
            bob_mean - nb2063_mean_bag_actual,
        "decision_margin": DECISION_MARGIN,
        "reproduces_nb2063": bool(reproduces),
        "verdict": verdict,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb2063_mean_bag_ref", "nb2063_mean_bag_actual",
        "delta_bob_mean_vs_nb2063_ref",
        "delta_bob_mean_vs_nb2063_actual",
        "decision_margin",
        "reproduces_nb2063",
        "verdict",
