"""nb204 -- Multi-seed div-15/20 QReg: fast version of nb200.

nb200 was 25 seeds × 30 alphas = 3750 QReg fits (~30+ hours). Too slow.

This version: 4 new seeds × 8 alphas × div-15 and div-20 = feasible in 2-3 hours.

Hypothesis: seed_idx=0 (nb197) might not be the optimal starting point for
greedy diversity. A different set of 15 diverse models might give higher ratio
at the same alpha, allowing SLSQP to find a better feasible solution.

Keys alphas chosen to span the nb197 ratio threshold transition:
  [0.001, 0.003, 0.006, 0.008, 0.009, 0.010, 0.015, 0.020]
These are: low (fail), transition zone, reference passing, high passing.

If any seed gives ratio >= 0.58 at alpha=0.006-0.008 with RAE < 0.297639,
that beats nb197.
"""
...
META_STEMS = {
    ...
    "nb203_div25_low_alpha", "nb204_multiseed_fast",
...
def main():
    print("=== nb204: Multi-seed div-15/20 QReg (fast) ===\n", flush=True)
    ...
    # Sparse alpha grid covering key points
    alpha_sparse = [0.001, 0.003, 0.006, 0.008, 0.009, 0.010, 0.015, 0.020]

    # Seeds to test: 0 is nb197, test 1-4 for new diversity selections
    seeds_to_test = [1, 2, 3, 4]  # seed 0 already tested in nb197

    all_cand_oofs, all_cand_tes, all_cand_names = [], [], []
    best_passing_by_seed = {}

    for seed_idx in seeds_to_test:
        print(f"\n--- seed_idx={seed_idx} ---", flush=True)
        for k in [15, 20]:
            ...
            for alpha in alpha_sparse:
                ...
                # Check for new best individually
                nb197_ref = ""
                if alpha >= 0.008:
                    nb197_ref = " [ref]"
                print(f"  div{k} s{seed_idx} a={alpha:.4f}  RAE={r_c:.6f}  ratio={ratio_c:.4f}  [{flag}]{nb197_ref}  ({time.time()-t_a:.0f}s)", flush=True)
                all_cand_oofs.append(oof_c)
                all_cand_tes.append(te_c)
                all_cand_names.append(f"div{k}_s{seed_idx}_a{alpha:.4f}")

                if ratio_c >= COLLAPSE_THRESH:
                    key = (seed_idx, k)
                    if key not in best_passing_by_seed or r_c < best_passing_by_seed[key][0]:
                        best_passing_by_seed[key] = (r_c, ratio_c, alpha)

    print(f"\n=== Candidates summary ===", flush=True)
    print(f"Total: {len(all_cand_oofs)} candidates", flush=True)
    print(f"Elapsed: {time.time()-t0:.0f}s", flush=True)

    # Show best passing per seed
    print("\nBest passing per (seed, k):", flush=True)
    for (s, k), (r, ratio, alpha) in sorted(best_passing_by_seed.items()):
        beat = " ***BETTER THAN nb197***" if r < PREV_BEST else ""
        print(f"  s{s} div{k} a={alpha:.4f}: RAE={r:.6f}  ratio={ratio:.4f}{beat}", flush=True)

    # --- SLSQP: NB188 + all new seeds' candidates ---
    print(f"\n--- SLSQP: NB188 + multi-seed QReg candidates (2000 starts) ---", flush=True)
    ...
    r_best, oof_best, te_best, ratio_best = constrained_slsqp(
        X_tr, X_te, y_tr, n_starts=2000, prev_best=PREV_BEST, tag="multiseed_s1-4")
    ...
    print(f"  nb204 (multi-seed s1-4): RAE={r_best:.6f}  ratio={ratio_best:.4f}  [{flag}]{beat}", flush=True)
    ...
    if oof_best is not None and r_best < PREV_BEST and ratio_best >= COLLAPSE_THRESH:
        ...
        out_stem = "nb204_multiseed_fast_baseline"
        if oof_best is not None:
            np.save(DATA_PROCESSED / f"oof_{out_stem}.npy", oof_best)
            np.save(DATA_PROCESSED / f"te_{out_stem}.npy",  te_best)
            sub_d = pd.DataFrame({"Molecule Name": load_test()["name"].values, "pEC50": te_best})
