"""nb1532 -- Sample-weighted nb1484 4-way.

Identical protocol to nb1484 (PRE-unblind chemprop_aux anchor + 4 SHAP-pruned
family residual learners on ChEMBL kNN + sim) EXCEPT each LGBM fit (both the
shallow seed-0 SHAP-importance fit AND the 5-seed bag cross-fit) is trained
with `sample_weight = sim_to_chembl_top1` per row.

HYPOTHESIS
    Test compounds with high sim_to_chembl_top1 have a more reliable
    pred_chembl signal; weighting LGBM by sim may help the residual learners
    focus on the rows where their inputs are most informative.

PROTOCOL
    1. Load sim_to_chembl_top1 per row on 253 unblind
       (data/processed/nb1301_sim_chembl_unb.npy, shape (253,)).
    2. weight_i = sim_to_chembl_top1_i  (raw similarity, in [0, 1]).
    3. Build nb1484 4-way protocol; pass sample_weight=weight to every
       LGBMRegressor.fit() call.
    4. 5-seed bag shallow LGBM Huber on chemprop_aux + each pruned feature
       family (AtomPair-30, MACCS-20, Mordred-30, ChempropEmbed-30).
    5. 5-fold cross-fit, naive 1/4 mean blend.
    6. Verdict at 0.003 margin vs nb1484 (0.5231).

Outputs:
    scripts/nb1532_sample_weighted_nb1484.py          (this file)
    data/processed/nb1532_summary.json
    data/processed/nb1532_best_oof.npy                (253,) float32
"""
...
# Sample-weight source -- per-row Tanimoto top-1 similarity to ChEMBL pool
# (precomputed by nb1301 on the 253 unblind set).
SIM_WEIGHT_PATH = DATA_PROCESSED / "nb1301_sim_chembl_unb.npy"
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 weight: np.ndarray, seed: int) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = LGBMRegressor(**_lgbm_params(seed))
        mdl.fit(X[tr_loc], residual[tr_loc], sample_weight=weight[tr_loc])
        oof[va_loc] = mdl.predict(X[va_loc])
    return oof


def _compute_shap_importance(X: np.ndarray, residual: np.ndarray,
                             weight: np.ndarray, seed: int = 0):
    mdl = LGBMRegressor(**_lgbm_params(seed))
    mdl.fit(X, residual, sample_weight=weight)
    try:
        import shap
...
def _run_family(family: str, X_fam_unb: np.ndarray,
                ...
                weight: np.ndarray, top_k: int) -> dict:
    print("\n" + "=" * 78)
    ...
    imp_full, imp_src = _compute_shap_importance(X_full, residual, weight,
                                                 seed=0)
    ...
    print(f"\n   PER-SEED RESIDUAL CROSS-FIT (PRUNED, dim={feat_dim_pruned}, "
          f"sample_weight=sim_to_chembl)")
    for i, s in enumerate(RESID_SEEDS):
        resid_oof_s = _residual_cross_fit_one_seed(
            X_pruned, residual, weight, s
        )
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Sample-weighted nb1484 4-way: chemprop_aux anchor + "
          f"AtomPair-30 + MACCS-20 + Mordred-30 + ChempropEmbed-30")
    print(f"          sample_weight = sim_to_chembl_top1 (per 253 unblind row)")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux={CHEMPROP_AUX_REF:.4f}  "
          f"nb1484={NB1484_REF:.4f}")
    ...
    # ---- Sample weight = sim_to_chembl_top1 on 253 unblind ----
    if not SIM_WEIGHT_PATH.exists():
        raise FileNotFoundError(
            f"sim_to_chembl weight source missing: {SIM_WEIGHT_PATH}"
        )
    weight_raw = np.load(SIM_WEIGHT_PATH).astype(np.float64)
    if weight_raw.shape[0] != n_unb:
        raise ValueError(
            f"weight shape mismatch: {weight_raw.shape} vs n_unb={n_unb}"
        )
    # Use raw similarity (already in [0, 1]). LightGBM requires positive
    # weights; floor zeros at a small epsilon so they don't drop rows.
    weight = np.maximum(weight_raw, 1e-6).astype(np.float64)
    # Quantile summary for the report.
    q = np.quantile(weight_raw, [0.0, 0.1, 0.25, 0.5, 0.75, 0.9, 1.0])
    weight_summary = {
        "source_path": str(SIM_WEIGHT_PATH),
        "n": int(weight_raw.shape[0]),
        "mean": float(weight_raw.mean()),
        "std": float(weight_raw.std()),
        "min": float(weight_raw.min()),
        "max": float(weight_raw.max()),
        "q00": float(q[0]),
        "q10": float(q[1]),
        "q25": float(q[2]),
        "q50": float(q[3]),
        ...
        "q90": float(q[5]),
        "q100": float(q[6]),
        "n_zero_floored": int((weight_raw <= 0.0).sum()),
        "weight_transform": "raw_sim_top1_floored_1e-6",
    }
    print(f"[weight] sim_to_chembl_top1 mean={weight_raw.mean():.4f}  "
          f"std={weight_raw.std():.4f}  min={weight_raw.min():.4f}  "
          f"max={weight_raw.max():.4f}")
    print(f"[weight] quantiles q10={q[1]:.4f} q25={q[2]:.4f} q50={q[3]:.4f} "
          f"q75={q[4]:.4f} q90={q[5]:.4f}")
    print(f"[weight] n_zero_floored={weight_summary['n_zero_floored']}  "
          f"transform={weight_summary['weight_transform']}")

    # ---- ChEMBL pool + kNN (identical to nb1484) ----
    print("\n" + "-" * 78)
    ...
    if beats_nb1484_best:
        verdict = f"SAMPLE_WEIGHTED_BEATS_NB1484_NEW_PRIMARY_via_{which_best}"
    elif flat_vs_nb1484_best:
        verdict = f"SAMPLE_WEIGHTED_FLAT_VS_NB1484_best_{which_best}"
    elif beats_anchor_best:
        verdict = (
            f"SAMPLE_WEIGHTED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "SAMPLE_WEIGHTED_FLAT_VS_ANCHOR"
    else:
        verdict = "SAMPLE_WEIGHTED_HURTS_ANCHOR"
