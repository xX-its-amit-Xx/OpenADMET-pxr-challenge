"""nb1931 -- LightGBM with explicit objective='regression_l2' (sklearn API) on
the 117-col 5-way K-tuned matrix; PRE-unblind anchor chemprop_aux.

HYPOTHESIS:
    nb1852 used objective='regression' (the default L2 alias). nb1931 uses the
    explicit alias 'regression_l2' on the same hyperparameter recipe to confirm
    invariance under alias naming and re-anchor the bagged-OOF L2 RAE.

PROTOCOL:
    1. Features = 117-col 5-way K-tuned matrix
       (AtomPair top-K + MACCS top-K + Mordred top-K + ChempropEmbed top-K
        + Avalon top-K + ChEMBL kNN pec50 + mean Tanimoto sim).
    2. Anchor    = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    3. Model     = LGBMRegressor(
           objective='regression_l2', max_depth=4, num_leaves=15,
    ...
    4. Verdict at 0.003 margin vs nb1861 (0.5013, pooled 25-bag pseudo-ref).

Outputs:
    scripts/nb1931_lgbm_l2_sklearn.py
    data/processed/nb1931_summary.json
    data/processed/nb1931_per_seed_oof.npy     (5, 253) float32
    data/processed/nb1931_mean_bag_oof.npy     (253,)   float32
"""
...
NB1861_REF = 0.5013  # pooled 25-bag honest RAE from nb1861
...
def _lgbm_params(seed: int) -> dict:
    """LGBM L2 with EXPLICIT 'regression_l2' objective alias (sklearn API)."""
    return dict(
        objective="regression_l2",   # explicit MSE / L2 alias
        max_depth=4,
        num_leaves=15,
        n_estimators=300,
        learning_rate=0.03,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(objective='regression_l2', sklearn API) on 5-way "
          f"K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          bag seeds = {BAG_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1861 pooled-25bag ({NB1861_REF:.4f})")
    print(f"          decision_margin vs nb1861 = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- nb1861 reference (override default if newer pooled value is present) ----
    ...
    print(f"5-SEED BAG x {RESID_FOLDS}-fold cross-fit  (dim={feat_dim})  "
          f"objective='regression_l2'")
    print("-" * 78)

    per_seed_oof = np.zeros((len(BAG_SEEDS), n_unb), dtype=np.float64)
    per_seed_rae: list[float] = []
    per_seed_records = []

    for s_i, seed in enumerate(BAG_SEEDS):
        t_s = time.time()
    ...
    rae_seed_mean = float(arr.mean())
    rae_seed_median = float(np.median(arr))
    rae_seed_std = float(arr.std())
    rae_seed_min = float(arr.min())
    rae_seed_max = float(arr.max())
    ...
    print(f"   per-seed MEAN  RAE  = {rae_seed_mean:.4f}")
    print(f"   per-seed MED   RAE  = {rae_seed_median:.4f}")
    print(f"   per-seed std/min/max = "
          f"{rae_seed_std:.4f} / {rae_seed_min:.4f} / {rae_seed_max:.4f}")
    ...
    summary = {
        ...
        "rae_seed_mean": rae_seed_mean,
        "rae_seed_median": rae_seed_median,
        "rae_seed_std": rae_seed_std,
        "rae_seed_min": rae_seed_min,
        "rae_seed_max": rae_seed_max,
