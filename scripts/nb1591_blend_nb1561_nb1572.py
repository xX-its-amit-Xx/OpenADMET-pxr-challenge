"""nb1591 — Blend nb1561 CatBoost BoB + nb1572 LGBM Avalon K=20.

HYPOTHESIS: nb1572 is fresh LGBM (not in 5-way K-tuned set). May add row-level
diversity to the CatBoost BoB anchor.

Protocol:
1. Load nb1561 (CatBoost BoB, OOF 0.5155) + nb1572 (LGBM Avalon K=20, OOF 0.5580).
2. Pearson correlation.
3. Grid w in {0.0..1.0 step 0.05} where pred = w * nb1561 + (1 - w) * nb1572.
4. 5-fold cross-fit SLSQP for honest blend weight.
5. Verdict at 0.003 margin vs nb1561 (0.5155).

Outputs:
- data/processed/nb1591_summary.json
- data/processed/nb1591_best_oof.npy
"""
...
def main() -> None:
    ...
    p_lgb = np.load(PROC / "nb1572_best_K_oof.npy").astype(float)
    y = load_truth_253()
    ...
    np.save(PROC / "nb1591_best_oof.npy", blend_cf)

    summary = {
        "n": int(n),
        "rae_nb1561_catboost": rae_cat,
        "rae_nb1572_lgbm_avalon_K20": rae_lgb,
        "pearson_p1561_p1572": pear,
        "grid": grid_rows,
    ...
    out_path = PROC / "nb1591_summary.json"
    out_path.write_text(json.dumps(summary, indent=2))

    # Human-readable report
    print(f"n = {n}")
    print(f"Pearson(nb1561_cat, nb1572_lgb_avalon_K20) = {pear:.4f}")
    print(f"RAE nb1561 (CatBoost BoB)         = {rae_cat:.4f}")
    print(f"RAE nb1572 (LGBM Avalon K=20)     = {rae_lgb:.4f}")
    print("\nGrid sweep (w = weight on nb1561 CatBoost):")
    ...
    print(f"\nSaved: {out_path}")
    print(f"Saved: {PROC / 'nb1591_best_oof.npy'}")
