"""nb1323 -- Isotonic calibration on nb1290 OOF (5-seed bag, 5-fold cross-fit).

Hypothesis:
    nb1290 best_fixed_w blend (w=0.35*nb1190 + 0.65*nb1242, RAE 0.5390) has
    pred_std < truth_std (variance compression). Isotonic regression (monotone
    non-parametric calibration) may recover variance more efficiently than
    per-quantile stretch by fitting an empirical CDF-matching step function.

Protocol:
  1. Load nb1290 best_fixed_w OOF (0.35*nb1190 + 0.65*nb1242).
  2. 5-fold cross-fit isotonic regression per seed:
       - For each fold: IsotonicRegression(y_min=min(truth), y_max=max(truth))
         fit on training-fold (p_oof, y); predict held-out fold.
  3. 5-seed bag (KFold seed varies in {42, 43, 44, 45, 46}).
  4. Pool mean_bag RAE; compare to nb1290 (0.5390) at 0.003 margin.

NO deploy (513) refit -- 253-only honest cross-fit diagnostic.

Outputs:
  data/processed/nb1323_mean_bag_oof.npy   (253,) float32 -- mean across 5 seeds
  data/processed/nb1323_summary.json
"""
...
# nb1290 best_fixed_w reference (0.35 nb1190 + 0.65 nb1242).
NB1290_REF = 0.5390
MARGIN = 0.003
...
def _isotonic_cross_fit(p: np.ndarray, y: np.ndarray,
    ...
    """Per-fold IsotonicRegression(y_min, y_max) fit-and-apply.

    Returns:
        oof: (n,) float64 calibrated predictions
        n_thresholds: list of n_thresholds_ per fold (curve break count)
    """
    n = len(y)
    ...
    for tr_loc, va_loc in kf.split(np.arange(n)):
        ...
        # n_thresholds_ is the number of breakpoints in the fitted curve.
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Isotonic calibration on nb1290 OOF "
          f"(5-fold cross-fit, 5-seed bag)")
    print(f"          verdict margin {MARGIN} vs nb1290 (0.5390)")
    print("=" * 78)
    ...
    if p_oof.shape[0] != n_unb:
        raise ValueError(f"shape mismatch: nb1290_bestw_oof={p_oof.shape}, "
                         f"n_unb={n_unb}")

    nb1290_rae = float(rae(y_unb, p_oof))
    print(f"\n[load] nb1290 best_fixed_w OOF standalone RAE = {nb1290_rae:.4f}  "
          f"(ref {NB1290_REF:.4f})")
    ...
    # ---- Verdict ----
    beats_nb1290 = rae_mean_bag < NB1290_REF - MARGIN
    flat_nb1290 = abs(rae_mean_bag - NB1290_REF) < MARGIN
    delta_vs_nb1290 = rae_mean_bag - NB1290_REF

    if beats_nb1290:
        verdict = (f"ISOTONIC_NB1290_BEATS_NB1290 "
                   f"(mean_bag @ {rae_mean_bag:.4f})")
    elif flat_nb1290:
        verdict = (f"ISOTONIC_NB1290_FLAT_VS_NB1290 "
                   f"(mean_bag @ {rae_mean_bag:.4f})")
    else:
        verdict = (f"ISOTONIC_NB1290_HURTS_VS_NB1290 "
                   f"(mean_bag @ {rae_mean_bag:.4f})")

    print("\n" + "=" * 78)
    print("VERDICT")
    print("=" * 78)
    print(f"   nb1290 standalone (anchor)  : {nb1290_rae:.4f}  "
          f"(ref {NB1290_REF:.4f})")
    ...
    summary = {
        ...
        "n_folds": N_FOLDS,
        "seeds": SEEDS,
        "anchor": "nb1290_bestw_oof",
        "anchor_rae_standalone": nb1290_rae,
