"""nb3011 -- Per-fold SLSQP simplex on 3-anchor pool {K17, K18, K19} (all deep-30).

NEW PARADIGM: extend nb3002 (K18+K19) by adding K17 deep-30 (nb3010).

The cycle-247 pyramid {K18, K19, K20} simplex (nb2992) zeroed K=20 in deploy.
nb3002 then verified that the minimal {K18, K19} pool reaches mean pooled
outer-val RAE 0.4511 (VERIFIED_NEW_PRIMARY1) at deep-30 depth.

This script tests whether adding K=17 -- which lives BELOW the {K18, K19}
RFE plateau and whose deep-30 OOF (nb3010) gave bag-mean RAE 0.4680,
the lowest single-K deep-30 number seen so far -- opens additional
optimization space in the per-fold simplex.

PROTOCOL:
    Anchors:
        K17: nb3010 deep-30 fresh-seed OOFs + te arrays (bag RAE 0.4680)
        K18: nb2960 deep-30 fresh-seed OOFs + te arrays (full-OOF RAE 0.4536)
        K19: nb3000 deep-30 fresh-seed OOFs + te arrays (full-OOF RAE 0.4607)
    Outer CV: 5-fold scaffold split, 15 fresh kf_seeds {1036..1050}
    Per fold:
    ...
    Deploy:
        ...
        - Apply to (513, 3) stacked te arrays -> te_nb3011

GATE:
    mean_pooled_rae < 0.4511  -> "BETTER_THAN_NB3001"  (beats nb3002 0.4511)
    mean_pooled_rae < 0.4518  -> "PROMOTE"             (matches nb3002 gate)
    else                      -> "FAIL"
...
References:
    ...
    nb3001 K18+K19(deep30)+K20 simplex (cached 5-seed K19) = 0.4515 (deep-30 swap)
    nb3002 K18+K19 deep-30 per-fold simplex                = 0.4511
    nb2992 K18+K19(5sd)+K20 deploy                         = 0.4479
...
Outputs:
    data/processed/nb3011_summary.json
    data/processed/nb3011_pred_oof.npy  (253,) float32 -- per-fold simplex OOF
                                         (taken from the first seed = 1036)
    data/processed/te_nb3011.npy        (513,) float32 -- deploy te
    submissions/nb3011_per_fold_simplex_K17_K18_K19_deep30.csv  (only if verdict != "FAIL")
"""
...
K_DEPTH = {"K17": "deep30", "K18": "deep30", "K19": "deep30"}
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 3 deep-30 K-anchors {K_LABELS}")
    print(f"          parents: nb3010 (K17) + nb2960 (K18) + nb3000 (K19), all deep-30")
    print(f"          motivation: extend nb3002 (K18+K19, RAE 0.4511) by adding K17")
    ...
    print("\n" + "-" * 78)
    print(f"STEP 3: outer CV with per-fold SLSQP, 15 fresh kf_seeds")
    print("-" * 78)
    per_seed_pooled = []
    ...
    # pred_oof = first-seed (1036) per-fold OOF blend, representative single-seed slice
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_simplex_K17_K18_K19_deep30.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_K17_K18_K19_deep30_15seed",
        "paradigm": "3_anchor_extend_nb3002_by_adding_K17",
