"""nb1721 -- Super-shallow CatBoost (depth=2) on 117-col 5-way K-tuned features.

HYPOTHESIS:
    nb1554 used CatBoost(MAE, depth=4, n_est=200, lr=0.05, l2=5) on the
    117-col 5-way K-tuned stack and landed at mean_bag RAE 0.5163.  At
    n_unb=253, depth=4 trees have ~16 leaves each which may be over-
    parameterized for the residual signal (residual_std ~ 0.55).
    Test super-shallow stumps (depth=2 = 4 leaves) with more iterations
    (n=400) and lower L2 (3) to see if capacity reduction extracts a
    cleaner anchor correction on small-n OOD residuals.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx] (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack identical to nb1554:
         top-25 AtomPair (nb1524 K=25)
         top-20 MACCS (nb1352)
         top-20 Mordred (nb1523 K=20)
         top-20 ChempropEmbed (nb1541 K=20)
         top-30 Avalon (nb1392 SHAP K=30)
         pred_chembl_pec50 + mean_sim (ChEMBL kNN-5)
    3. 5-seed bag CatBoost(MAE, depth=2, n_est=400, lr=0.05, l2=3).
       KFold(n=5, shuffle=True) cross-fit per seed.
    4. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1721_catboost_depth2.py
    data/processed/nb1721_summary.json
    data/processed/nb1721_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163   # benchmark for verdict (CatBoost depth=4)
...
def _cat_params(seed: int) -> dict:
    """Super-shallow stumps -- depth=2 (4 leaves), n=400, lr=0.05, l2=3."""
    return dict(
        loss_function="MAE",
        depth=2,
        iterations=400,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d2, n400, lr0.05, l2=3) SUPER-SHALLOW on "
          f"5-way K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          benchmark: nb1554 ({NB1554_REF:.4f})  "
    ...
    # ---- Per-seed CatBoost(depth=2) residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST(d=2, n=400, lr=0.05, l2=3) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1554:
        verdict = "DEPTH2_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "DEPTH2_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "DEPTH2_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "DEPTH2_FLAT_VS_ANCHOR"
    else:
        verdict = "DEPTH2_HURTS_ANCHOR"
