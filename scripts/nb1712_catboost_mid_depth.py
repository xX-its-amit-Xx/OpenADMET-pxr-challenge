"""nb1712 -- CatBoost mid-depth (d4, n300, lr0.03, l2=10) on 5-way K-tuned 117-col.

HYPOTHESIS:
    nb1554 (CatBoost MAE d4 n200 lr0.05 l2=5) on the same 5-way K-tuned 117-col
    stack landed at rae_mean_bag = 0.5163 (PRE-unblind anchor chemprop_aux).
    The nb1554 hyperparams are aggressive: lr=0.05 with n=200 may under-fit
    once feature stack widens past 100 cols, and l2=5 may be too permissive
    given residual_std ~ 0.6 on 253 unblind anchors.  This script tests
    "mid-depth, slower learner, stronger regularization":
        depth=4 (same), iterations=300 (+50%), learning_rate=0.03 (-40%),
        l2_leaf_reg=10 (+100%).
    Net effect: total boosting capacity ~ unchanged (n*lr -> 9 vs 10), but
    each tree is shallower in effect (lr=0.03), and the L2 penalty doubles.
    Expected: smoother residual surface, lower seed-variance per fold.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build the same 117-col 5-way K-tuned feature stack as nb1554:
         ...
         top-30 Avalon        (nb1392 SHAP ranking)
         pred_chembl_pec50    (ChEMBL PXR kNN-5)
         mean_sim
    3. 5-seed bag CatBoost(loss=MAE, depth=4, n_est=300, lr=0.03, l2=10).
...
Outputs:
    scripts/nb1712_catboost_mid_depth.py
    data/processed/nb1712_summary.json
    data/processed/nb1712_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # primary benchmark for this run
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d4, n300, lr0.03, l2=10) on 5-way K-tuned "
    ...
    if beats_nb1554:
        verdict = "CATBOOST_MID_DEPTH_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_MID_DEPTH_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_MID_DEPTH_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_MID_DEPTH_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_MID_DEPTH_HURTS_ANCHOR"
