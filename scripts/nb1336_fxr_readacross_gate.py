"""nb1336 — FXR (NR1H4) cross-NR READ-ACROSS feature, honest gate.

FXR is a Type-I activation NR co-expressed + cross-talking with PXR in hepatocyte
xenobiotic/bile-acid handling; shares ligand space (bile-acid mimetics, GW4064-class).
Leakage-safe read-across: train an FXR p-potency predictor on 3855 ChEMBL+Tox21 FXR
compounds (NEVER sees PXR pEC50), predict FXR-activity for our 4139 train + 513 test,
...
P = "data/processed"; OUT = "C:/pxr_work/fxr_tox21"
BEST_RAE = 0.4149
N_SEEDS = 3
...
# ── FXR data
ded = pickle.load(open(f"{OUT}/fxr_std.pkl", "rb"))
fxr_smi = [r[1] for r in ded]
fxr_y   = np.array([r[4] for r in ded], float)
fxr_ik  = set(r[2] for r in ded)
print(f"FXR compounds: {len(fxr_smi)}  p-range {fxr_y.min():.2f}..{fxr_y.max():.2f}")
...
# ── dedup / overlap report (InChIKey FXR vs our train)
tr_ik = set(filter(None, (ikey(s) for s in tr_smi)))
overlap = len(fxr_ik & tr_ik)
print(f"InChIKey overlap FXR-cap-train: {overlap} (FXR predictor never sees PXR labels -> no leakage)")

# ── coverage: FXR actives (p>=6) -> our 513 test
...
print(f"Coverage (test->nearest FXR active): median Tanimoto={med_tani:.3f}  frac>=0.4={frac_cov:.3f}")

# ── read-across: FXR predictor (combined feats), leakage-safe re: PXR labels
print("Featurizing FXR + our train/test ...")
Xfxr = impute(combined(fxr_smi)).astype(np.float32)
Xtr  = impute(combined(tr_smi)).astype(np.float32)
Xte  = impute(combined(te_smi)).astype(np.float32)
fxr_model = LGBMRegressor(n_estimators=600, num_leaves=64, learning_rate=0.04, n_jobs=4, verbose=-1)
fxr_model.fit(Xfxr, fxr_y)
ra_tr = fxr_model.predict(Xtr).astype(np.float32)
ra_te = fxr_model.predict(Xte).astype(np.float32)
...
res = {
    "feature": "fxr_nr1h4_readacross",
    "n_fxr": len(fxr_smi), "ik_overlap_train": overlap,
...
json.dump(res, open(f"{P}/nb1336_fxr_readacross_gate.json","w"), indent=2)
print("\n=== FXR READ-ACROSS GATE ===")
