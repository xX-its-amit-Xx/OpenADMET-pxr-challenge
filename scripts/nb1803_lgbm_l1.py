"""nb1803 -- LightGBM (small leaves + L1 reg_alpha) on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1771 hit rae_mean_bag 0.5100 with small-leaf LGBM(huber, max_depth=4,
    num_leaves=15, min_child_samples=5, reg_lambda=2).  This script adds L1
    regularization (reg_alpha=2) and softens L2 (reg_lambda=1) to test whether
    sparse leaf-level coefficients on the 117-col K-tuned matrix decorrelate
    further from the anchor on novel scaffolds.
        objective='huber', alpha=1.0
        max_depth=4, num_leaves=15
        n_estimators=300, lr=0.03
        min_child_samples=5
        reg_alpha=2.0     (NEW vs nb1771)
        reg_lambda=1.0    (vs 2.0 in nb1771)
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(huber, max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=5, reg_alpha=2, reg_lambda=1).
...
Outputs:
    scripts/nb1803_lgbm_l1.py
    data/processed/nb1803_summary.json
    data/processed/nb1803_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM small leaves + reg_lambda=2 on same 117-col stack
...
def main() -> dict:
    ...
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=4, num_leaves=15, "
          f"n_est=300, lr=0.03, min_child=5, reg_alpha=2, reg_lambda=1) on "
    ...
    # ---- Per-seed LGBM (L1+L2) residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (small leaves, L1=2, L2=1) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1771:
        verdict = "LGBM_L1_BEATS_NB1771_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1771:
        verdict = "LGBM_L1_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_L1_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_L1_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "LGBM_L1_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif flat_vs_nb1501:
        verdict = "LGBM_L1_FLAT_VS_NB1501"
    elif beats_nb1543:
        verdict = "LGBM_L1_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif flat_vs_nb1543:
        verdict = "LGBM_L1_FLAT_VS_NB1543"
    elif beats_anchor:
        verdict = "LGBM_L1_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_L1_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_L1_HURTS_ANCHOR"
