"""nb2603 -- Trim-mean of 30 truly fresh nb2240 K=20 seeds {2001..2030}.

NEW PARADIGM: more carefully than cycle-208 nb2581 (10 seeds, trim top/bot 2).
Trim top-5 and bottom-5 per row, mean of remaining 20 of 30. The wider seed
ensemble (3x more seeds, ~20% trim) gives a much tighter location estimate of
the K=20 anchor pred, suppressing per-seed heavy-tail noise that the K=20
single-seed pyramid can't average out internally.

PROTOCOL:
    1. For each residual seed s in 2001..2030 (30 truly fresh seeds, never used):
    ...
    2. Stack 30 seed K20_oof (253, 30) and K20_te (513, 30).
    3. Trimmed mean: drop top-5 + bottom-5 per row, mean of remaining 20.
       Mean baseline: simple mean of all 30.
    4. For BOTH (trimmed and mean): build 5-anchor pyramid
       {K20_trimmean, chemprop_aux, nb1191, nb503, nb562} and run 5-fold
       scaffold CV with kf_seed=1001 + SLSQP + rank-stretch.
    5. Gate on trimmed pyramid mean RAE.
...
OUTPUTS:
    scripts/nb2603_30_fresh_seed_trim.py
    data/processed/nb2603_summary.json
    data/processed/nb2603_pred_oof.npy   (253,) float32  -- pyramid OOF (trimmed)
    data/processed/te_nb2603.npy         (513,) float32  -- deploy
"""
...
# ============================================================================
N_RESID_SEEDS = 30
RESID_SEEDS = list(range(2001, 2001 + N_RESID_SEEDS))   # truly fresh seeds, never used
RESID_FOLDS = 5
TRIM_TOP = 5
TRIM_BOTTOM = 5
KEEP_PER_ROW = N_RESID_SEEDS - TRIM_TOP - TRIM_BOTTOM   # 20

KF_SEEDS = [1001]   # single kf_seed for the pyramid CV
N_FOLDS = 5
...
# ============================================================================
# helpers (copied from nb2240/nb2560/nb2581)
# ============================================================================
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- trim-mean of {N_RESID_SEEDS} truly fresh nb2240 K=20 seeds")
    print(f"  seeds = {RESID_SEEDS[0]}..{RESID_SEEDS[-1]}  trim top={TRIM_TOP}, bot={TRIM_BOTTOM}, keep {KEEP_PER_ROW}/{N_RESID_SEEDS}")
    print("=" * 78)
    ...
    # ============================================================================
    # STAGE 1: Build 30 single-seed K=20 anchors (corrected OOF + deploy te)
    # ============================================================================
    print("\n" + "=" * 78)
    print(f"STAGE 1: {N_RESID_SEEDS} single-seed K=20 anchors  seeds={RESID_SEEDS[0]}..{RESID_SEEDS[-1]}")
    print("=" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        print(f"  seed={s:4d}  K20_anchor_RAE={per_seed_anchor_rae[i]:.4f}  "
              f"elapsed={time.time() - t_loop:.0f}s")

    # ============================================================================
    # STAGE 2: trimmed mean + mean baseline K=20 anchors
    # ============================================================================
    print("\n" + "=" * 78)
    ...
    # ============================================================================
    # STAGE 3: 5-anchor pyramid CV (kf_seed=1001) -- both variants
    # ============================================================================
    print("\n" + "=" * 78)
    ...
    sub_csv_path = SUBMISSIONS / f"{TAG}_30_fresh_seed_trim.csv"
    ...
    summary = {
        "tag": TAG,
        "method": "trim_mean_30_fresh_seed_K20_pyramid",
        ...
        "k20_n_features": 20,
        "anchors_used": ["nb2603_K20_trim30", "chemprop_aux", "nb1191", "nb503", "nb562"],
