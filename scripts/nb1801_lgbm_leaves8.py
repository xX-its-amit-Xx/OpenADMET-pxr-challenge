"""nb1801 -- LightGBM (num_leaves=8, min_child_samples=3) on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1771 (LGBM small leaves, num_leaves=15, min_child=5) hit rae_mean_bag
    0.5100; nb1780 BoB ensemble pushed to 0.5032.  This script pushes
    LGBM into an even smaller-leaf regime to test whether tighter leaf
    capacity extracts additional novel-scaffold residual signal:
        objective='huber', alpha=1.0
        max_depth=4, num_leaves=8         (vs nb1771 num_leaves=15)
        n_estimators=400, lr=0.025        (vs nb1771 n_est=300, lr=0.03)
        min_child_samples=3               (vs nb1771 min_child=5)
        reg_lambda=2.0
    Same 5-way K-tuned 117-col feature stack as nb1554/nb1771:
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(huber, max_depth=4, num_leaves=8, n_est=400,
       lr=0.025, min_child_samples=3, reg_lambda=2).  KFold(n=5, shuffle=True)
       cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1771 (0.5100) and nb1780 BoB (0.5032).

Outputs:
    scripts/nb1801_lgbm_leaves8.py
    data/processed/nb1801_summary.json
    data/processed/nb1801_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM small leaves (num_leaves=15, min_child=5)
NB1780_BOB_REF = 0.5032  # nb1780 BoB ensemble target
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=4, num_leaves=8, "
          f"n_est=400, lr=0.025, min_child=3, reg_lambda=2) on 5-way K-tuned "
    ...
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          ...
          f"nb1780 BoB ({NB1780_BOB_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- Per-seed LGBM leaves=8 residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (num_leaves=8, min_child=3) RESIDUAL CROSS-FIT "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if beats_nb1780_bob:
        verdict = "LGBM_LEAVES8_BEATS_NB1780_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1780_bob:
        verdict = "LGBM_LEAVES8_FLAT_VS_NB1780_BOB"
    elif beats_nb1771:
        verdict = "LGBM_LEAVES8_BEATS_NB1771_BUT_WORSE_THAN_NB1780_BOB"
    elif flat_vs_nb1771:
        verdict = "LGBM_LEAVES8_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_LEAVES8_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif beats_anchor:
        verdict = "LGBM_LEAVES8_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_LEAVES8_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_LEAVES8_HURTS_ANCHOR"
