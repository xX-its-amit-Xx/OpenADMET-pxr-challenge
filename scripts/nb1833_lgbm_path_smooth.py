"""nb1833 -- LightGBM (gbdt + path_smooth=0.1 + min_split_gain=0.01) on 5-way
K-tuned 117-col features.

HYPOTHESIS:
    nb1771 (LGBM huber d4 num_leaves=15 min_child=5 reg_lambda=2) hit
    rae_mean_bag 0.5100 on chemprop_aux residuals using the 117-col 5-way
    K-tuned stack.  This script tests a different LGBM regularization variant
    on the SAME feature stack to probe whether per-node path smoothing +
    explicit min_split_gain extracts additional residual signal:
        boosting_type='gbdt'
...
PROTOCOL:
    ...
    3. 5-seed bag LightGBM(gbdt, huber, d4, num_leaves=15, n_est=300,
       lr=0.03, min_child=5, reg_lambda=2, min_split_gain=0.01,
       path_smooth=0.1).  KFold(n=5, shuffle=True) cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1771 (0.5100).

Outputs:
    scripts/nb1833_lgbm_path_smooth.py
    data/processed/nb1833_summary.json
    data/processed/nb1833_mean_bag_oof.npy        (253,) float32
"""
...
NB1543_REF = 0.5237
NB1554_REF = 0.5163
NB1771_REF = 0.5100  # LGBM small leaves (huber, d4, num_leaves=15, mc=5, l2=2)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(gbdt, huber, alpha=1.0, max_depth=4, "
          f"num_leaves=15, n_est=300, lr=0.03, min_child=5, reg_lambda=2, "
          f"min_split_gain=0.01, path_smooth=0.1) on 5-way K-tuned 117-col "
          f"matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1771 ({NB1771_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- Per-seed LGBM (gbdt + path_smooth + min_split_gain) cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (gbdt, path_smooth=0.1, min_split_gain=0.01) "
    ...
    if beats_nb1771:
        verdict = "LGBM_PATH_SMOOTH_BEATS_NB1771_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1771:
        verdict = "LGBM_PATH_SMOOTH_FLAT_VS_NB1771"
    elif beats_nb1554:
        verdict = "LGBM_PATH_SMOOTH_BEATS_NB1554_BUT_WORSE_THAN_NB1771"
    elif flat_vs_nb1554:
        verdict = "LGBM_PATH_SMOOTH_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "LGBM_PATH_SMOOTH_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif beats_nb1543:
        verdict = "LGBM_PATH_SMOOTH_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif beats_anchor:
        verdict = "LGBM_PATH_SMOOTH_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_PATH_SMOOTH_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_PATH_SMOOTH_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "lgbm_reg_lambda": 2.0,
        "lgbm_min_split_gain": 0.01,
        "lgbm_path_smooth": 0.1,
        "K_AP_best": K_AP_best,
