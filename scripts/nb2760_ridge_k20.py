"""nb2760 -- Plain Ridge regression on K=20 chemprop_aux residual (alpha sweep).

NEW PARADIGM:
    Simplest possible LINEAR baseline: StandardScaler -> Ridge(alpha=X) on the
    raw K=20 feature slice, no polynomial expansion, no interaction terms,
    no kernel.  This isolates the "what does a plain linear model on K=20 buy
    us over chemprop_aux?" question and provides the cleanest possible
    bias-variance trade-off knob (alpha) decoupled from feature expansion.

    Cycle-167+ ceiling work (nb2171 deep-30 0.4682) sits on K=20 via LGBM /
    polynomial-Ridge / SVR-RBF / ElasticNet -- all of which add representation
    capacity.  This script asks: at the OTHER extreme, is the floor (plain
    linear) good enough to enter the ladder?  Negative result is informative
    for the substrate-vs-operator debate.
...
PROTOCOL:
    ...
    5. Alpha sweep {0.01, 0.1, 1.0, 10.0, 100.0} -- pick best by mean pooled
       RAE across seeds.
...
Outputs:
    data/processed/nb2760_summary.json
    data/processed/nb2760_pred_oof.npy   (253,) float32 -- best-alpha mean OOF
    data/processed/te_nb2760.npy         (513,) float32 -- best-alpha deploy
"""
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Plain Ridge alpha sweep on K=20 chemprop_aux residual")
    print("=" * 78)
    ...
    # ---- Alpha sweep ----
    print("\n" + "-" * 78)
    print(f"ALPHA SWEEP  alphas={ALPHA_GRID}  seeds={KF_SEEDS}\n"
    ...
    summary = {
        "tag": TAG,
        "method": (
            "Plain Ridge regression (no polynomial expansion, no kernel) on "
            "chemprop_aux residual over first-K=20 cols of X_117. Per-fold "
            "StandardScaler -> Ridge(alpha) over alpha sweep "
            f"{ALPHA_GRID}. 20 raw features, n=253 unblind."
        ),
        "paradigm": "linear_floor_baseline_plain_ridge_k20_no_expansion",
