"""nb2102 -- Deploy nb2092 POOLED-25 SHAP top-30 BoB MEDIAN to 513-row CSV.

PROTOCOL:
    1. Reuse top-30 SHAP indices from nb2081_summary.json (same as nb2092).
    ...
    4. te_nb2102 = te_chemprop_aux + median_residual_513.
    5. Save submissions/nb2102_deploy_nb2092_median.csv.

ANCHORS:
    nb2092 POOLED-25 MEDIAN honest cross-fit RAE = 0.4743.
    Honest LB target = 0.4743.
"""
...
# Identical 5 outer x 5 inner = 25 seeds to nb2092
...
NB2092_SUMMARY = DATA_PROCESSED / "nb2092_summary.json"
...
# nb2092 anchor: POOLED-25 MEDIAN honest cross-fit RAE
NB2092_POOLED25_MEDIAN_REF = 0.4743
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- identical to nb2092 / nb2081 / nb2063."""
    return dict(
...
def main() -> dict:
    ...
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb2092 POOLED-25 SHAP top-{TOP_K_SHAP} "
    ...
    print(f"          ref           = nb2092 POOLED-25 MEDIAN honest "
          f"cross-fit RAE = {NB2092_POOLED25_MEDIAN_REF:.4f}")
    print("=" * 78)

    # ---- Load nb2081 top-30 SHAP indices (same as nb2092) ----
    ...
    print(f"[reuse] nb2081 top-{TOP_K_SHAP} indices (head 10): "
          f"{top30_idx[:10].tolist()}")

    # Sanity-check against nb2092 (should be identical)
    nb2092_pooled25_mean_ref = float("nan")
    nb2092_pooled25_median_ref = float("nan")
    if NB2092_SUMMARY.exists():
        with open(NB2092_SUMMARY) as f:
            nb2092_sum = json.load(f)
        nb2092_pooled25_mean_ref = float(
            nb2092_sum.get("pooled25_rae", float("nan"))
        )
        nb2092_pooled25_median_ref = float(
            nb2092_sum.get("pooled25_median_rae", float("nan"))
        )
        nb2092_top30 = np.array(
            nb2092_sum.get("top30_idx_in_117_from_nb2081", []), dtype=int
        )
        same = (nb2092_top30.shape[0] == TOP_K_SHAP
                and np.array_equal(nb2092_top30, top30_idx))
        print(f"[check] nb2092 top30_idx match = {same}")
        print(f"[check] nb2092 POOLED-25 MEAN   = "
              f"{nb2092_pooled25_mean_ref:.6f}")
        print(f"[check] nb2092 POOLED-25 MEDIAN = "
              f"{nb2092_pooled25_median_ref:.6f}")
    ...
    te_nb2102 = te_anchor_513 + median_resid_513
    te_nb2102_mean_variant = te_anchor_513 + mean_resid_513  # diagnostic

    # ---- In-sample check on unb_idx ----
    in_pred_unb_median = te_nb2102[unb_idx]
    rae_in_unb_median = float(rae(y_unb, in_pred_unb_median))
    in_pred_unb_mean = te_nb2102_mean_variant[unb_idx]
    ...
    print(f"nb2092 POOLED-25 MEDIAN honest cross-fit = "
          f"{NB2092_POOLED25_MEDIAN_REF:.4f}  (honest LB target)")
    print("-" * 78)

    # ---- Stats ----
    te_mean = float(te_nb2102.mean())
    te_std = float(te_nb2102.std())
    te_min = float(te_nb2102.min())
    te_max = float(te_nb2102.max())
    print(f"\nte_nb2102 stats: mean={te_mean:.4f}  std={te_std:.4f}  "
    ...
    summary = {
        "tag": TAG,
        "method": "deploy_nb2092_pooled25_shap_top30_median_lgbm_mse",
        "anchor": ANCHOR,
        ...
        "te_nb2102_mean": te_mean,
        "te_nb2102_std": te_std,
        "te_nb2102_min": te_min,
        "te_nb2102_max": te_max,
        "in_RAE_unb_idx_median_deploy": rae_in_unb_median,
        "in_RAE_unb_idx_mean_diag": rae_in_unb_mean,
        "nb2092_pooled25_mean_ref": nb2092_pooled25_mean_ref,
        "nb2092_pooled25_median_ref_used_as_anchor": (
            NB2092_POOLED25_MEDIAN_REF
        ),
        "nb2092_pooled25_median_ref_summary": (
            nb2092_pooled25_median_ref
        ),
        "honest_lb_target": NB2092_POOLED25_MEDIAN_REF,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "n_test", "n_unb", "feat_dim_full", "feat_dim_top30", "n_total_fits",
        "rae_anchor_chemprop_aux_unb",
        "te_nb2102_mean", "te_nb2102_std",
        "te_nb2102_min", "te_nb2102_max",
