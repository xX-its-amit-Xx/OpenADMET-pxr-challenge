"""nb3230 -- Learned per-fold clip on full {K18, K19, K20, K23, K24, K28}
equal-weight deep-30 mean (NEW PARADIGM: clip the broader K-mean).

NEW PARADIGM (cycle 250+):
    Prior clip-on-simplex work (nb3201 K18 alone, nb3224 K18+K19+K23 SLSQP)
    operated on either a single K-anchor or a 3-anchor SLSQP simplex. This
    script widens the substrate to the FULL six-K equal-weight mean over
    {K=18, K=19, K=20, K=23, K=24, K=28} deep-30 OOFs and applies the
    per-fold (q_low, q_high) learned-clip primitive on top.

    Hypothesis: the broader 6-K equal-weight mean is a more variance-
    averaged anchor than the 3-K SLSQP simplex (more independent noise
    sources averaged out). If the learned clip extracts additional tail-
    taming gain over the 6-K parent (as it did over nb3020 in nb3224), the
    clip primitive is anchor-invariant AND the wider K-pool delivers a
    cleaner substrate. Sweep complements nb3224 (clip on simplex) and
    nb3201 (clip on K=18 alone).

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = mean(K18, K19, K20, K23, K24, K28)  (253,) -- equal weights
    Per outer fold:
...
References:
    nb3224 clip on K18+K19+K23 SLSQP simplex (sibling, 3-anchor parent)
    nb3201 clip on K=18 deep30 alone (sibling, 1-anchor parent)
    nb3020 K18+K19+K23 simplex (3-anchor SLSQP)
...
Inputs (per-K deep-30 mean-bag OOF + te files):
    data/processed/_audit_unblind_idx.npy
    data/processed/_audit_unblind_y.npy
    data/processed/nb2960_K18_30seed_oof.npy + te
    data/processed/nb3000_K19_30seed_oof.npy + te (te_nb3000_K19.npy)
    data/processed/nb2960_K20_30seed_oof.npy + te
    data/processed/nb3020_K23_30seed_oof.npy + te (te_nb3020_K23.npy)
    data/processed/nb2960_K24_30seed_oof.npy + te
    data/processed/nb2960_K28_30seed_oof.npy + te

Outputs:
    data/processed/nb3230_summary.json
    data/processed/nb3230_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3230.npy         (513,) float32 -- deploy te
    submissions/nb3230_clip_on_full_K_pyramid.csv  (only on BETTER verdict)
"""
...
TAG = "nb3230"
PARENT_TAG = "full_K_pyramid_equal_mean_K18_K19_K20_K23_K24_K28_deep30"

# -- Inputs: per-K deep-30 OOF + te (six anchors, all PRE-unblind) ------------
K_ANCHOR_FILES = {
    ...
    23: ("nb3020_K23_30seed_oof.npy", "te_nb3020_K23.npy"),
...
# -- Per-fold grid (matches nb3201/nb3224 sibling) -----------------------------
...
def main() -> dict:
    ...
    # -- Build equal-weight mean ---------------------------------------------
    print("\n" + "-" * 78)
    print("STEP 2: equal-weight mean over 6 K-anchors")
    print("-" * 78)
    ...
    pred_base = oof_stack6.mean(axis=1)  # equal weights
    te_base = te_stack6.mean(axis=1)
    full_oof_rae = float(rae(y_unb, pred_base))
    print(
        f"   pred_base (6K mean): oof_RAE={full_oof_rae:.4f}  "
        f"mean={pred_base.mean():.3f}  std={pred_base.std():.3f}  "
        f"min={pred_base.min():.3f}  max={pred_base.max():.3f}"
    )
    print(
        f"   te_base (6K mean):   mean={te_base.mean():.3f}  "
    ...
    print(
        f"\n   ref 6K-mean parent OOF        = {full_oof_rae:.4f}  "
        f"<- parent (no clip)"
    )
    print(
        f"   delta vs 6K-mean parent (pf)  = "
    ...
    if pf_mean < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3230 15-seed per-fold-mean {pf_mean:.4f} "
            f"clears BETTER gate {GATE_BETTER:.4f}. Per-fold LEARNED clip "
            f"applied to full 6-K equal-weight mean "
            f"{sorted(K_ANCHOR_FILES.keys())} deep-30 extracts additional "
            f"tail-taming gain over the 6K-mean parent "
            f"({pf_mean - full_oof_rae:+.4f}). Modal pick was "
            f"(q{ql_mode:.2f}, q{qh_mode:.2f}). Confirms broader K-pool "
            f"substrate complements (or beats) the 3-K SLSQP simplex parent "
            f"used in nb3224; re-verify with wider seed bag before "
            f"PRIMARY-1 swap."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3230 15-seed per-fold-mean {pf_mean:.4f} fails BETTER "
            f"gate {GATE_BETTER:.4f}. Per-fold learned clip on full 6-K "
            f"equal-weight mean does NOT extract additional gain "
            f"({pf_mean - full_oof_rae:+.4f} vs parent). Broader K-pool "
            f"averaging already does the variance compression that the "
            f"clip would otherwise capture. Hold current ladder."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_clip_on_full_K_pyramid.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_6K_equal_weight_mean_"
            "K18_K19_K20_K23_K24_K28_deep30"
        ),
        "anchor_files": {
        ...
        # references
        "ref_6K_parent": round(full_oof_rae, 4),
        "delta_vs_6K_parent": round(pf_mean - full_oof_rae, 4),
    ...
    print(f"   95% CI                       = [{ci_low:.4f}, {ci_high:.4f}]")
    print(f"   delta vs 6K-mean parent (pf) = {pf_mean - full_oof_rae:+.4f}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_6K_parent", "delta_vs_nb3170_fixed",
        "ql_mode", "qh_mode",
