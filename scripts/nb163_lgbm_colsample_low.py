"""nb163 — LGBM_MAE with Very Low Column Sampling.

Problem: base-only OOF (110 models) → LGBM_MAE overfits feature combinations
→ test collapse (ratio=0.57).

Key insight from nb149/nb152 comparison:
- nb149 (mixed pool): colsample=1.0, ratio=0.58 PASS
- nb154-A (base-only): colsample=1.0, ratio=0.57 FAIL

Hypothesis: with 110 correlated OOF features, using all features per tree
(colsample=1.0) lets LGBM always pick the same "best" OOF combinations.
Setting colsample=0.2-0.3 forces each tree to use a random 22-33 OOF features,
creating more diversity and potentially better generalization.

Tests on base-only pool (110 OOFs + 5 assay = 115 features):
  A: colsample=0.30 (33 features/tree)
  B: colsample=0.25 (29 features/tree)
  C: colsample=0.20 (23 features/tree)
  D: colsample=0.15 (17 features/tree)
  E: colsample=0.10 (12 features/tree) — extreme
  F: Best colsample, multi-seed ensemble (5 seeds)
"""
...
BASE_CONFIG = dict(
    n_estimators=1000, num_leaves=63, learning_rate=0.025, min_child_samples=5,
    subsample=0.8, reg_alpha=0.05, objective="regression_l1", verbose=-1, n_jobs=4, random_state=42
)
...
def main():
    print("=== nb163: LGBM_MAE with Low Column Sampling ===\n")
    ...
    for col_frac in [0.30, 0.25, 0.20, 0.15, 0.10]:
        label = f"colsample{int(col_frac*100):02d}"
        print(f"\n--- colsample={col_frac:.2f} ---")
        cfg = {**BASE_CONFIG, "colsample_bytree": col_frac}
        r, oof, te_pred, ratio = cv_lgbm_mae(cfg, X_tr_all, y_tr, X_te_all, splits, label)
        results[label] = (r, oof, te_pred, ratio)
        if ratio >= COLLAPSE_THRESH:
            print(f"  --> PASSES threshold at colsample={col_frac:.2f}!")
            break  # Found the threshold — run multi-seed next
    ...
    if valid:
        ...
        best_col = float(best_single.replace("colsample", "")) / 100.0
        print(f"\nBest passing: {best_single} RAE={valid[best_single][0]:.4f}")

        # Multi-seed ensemble on best passing config
        print(f"\n--- Multi-seed (5x) on best passing colsample={best_col:.2f} ---")
        best_cfg = {**BASE_CONFIG, "colsample_bytree": best_col}
        seed_oofs = [valid[best_single][1]]; seed_tes = [valid[best_single][2]]
        for seed in [123, 456, 789, 1234]:
            ...
            m_f = lgb.train(cfg_s, lgb.Dataset(X_tr_all, label=y_tr), callbacks=[lgb.log_evaluation(-1)])
        ...
        print(f"  [multi-seed {best_col:.2f}] RAE={r_ms:.4f}  ratio={ratio_ms:.2f}  [{flag}]")
        results[f"multiseed_{best_single}"] = (r_ms, oof_ms, te_ms, ratio_ms)
    ...
    best_label = min(all_valid, key=lambda k: all_valid[k][0])
    best_r, best_oof, best_te, best_ratio = all_valid[best_label]
    ...
    np.save(DATA_PROCESSED / "oof_nb163_lgbm_colsample.npy", best_oof)
    np.save(DATA_PROCESSED / "te_nb163_lgbm_colsample.npy",  best_te_out)
    sub = pd.DataFrame({"Molecule Name": te["name"].values, "pEC50": best_te_out})
    sub.to_csv(SUBMISSIONS / "163_lgbm_colsample.csv", index=False)
    print(f"\nSaved: submissions/163_lgbm_colsample.csv  OOF RAE={best_r:.4f}")
