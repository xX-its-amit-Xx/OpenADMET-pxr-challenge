"""nb3043 -- 15-seed wide-seed per-fold SLSQP simplex on 3 K-anchors
            {K18, K19, K22} all at deep-30.

CONTEXT (cycle 250+):
    K22 deep-30 OOF RAE = 0.4708 (from nb3014 rebuild) is STANDALONE WORSE
    than K23 deep-30 OOF RAE = 0.4750 -- wait, K22 is actually 0.4708 better.
    Re-state: K22 deep-30 0.4708 vs K23 deep-30 0.4750 -- K22 is slightly
    better standalone. But this run is to test the pair-correlation
    hypothesis: even if K22 standalone is not the strongest, its CORRELATION
    structure with {K18, K19} may differ from K20/K21/K23, opening a new
    blend axis.

    Same structural protocol as nb3030 (which verified nb3020 = {K18, K19,
    K23} at deep-30 with kf_seeds 1051..1065 -> mean 0.4509 BETTER_THAN_NB3001).
    nb3043 swaps K23 -> K22 to test if K22 carries an orthogonal residual
    pair-correlation signal.

    All 3 anchors are PRE-unblind (chemprop_aux anchor + residual LGBM on
    K-feature slices of nb2231 117-col matrix). No POST-unblind chain risk.
...
PROTOCOL:
    Anchors LOADED from caches (NO rebuild):
        ...
        K22: data/processed/nb3014_K22_30seed_oof.npy / te_nb3014_K22.npy

    For each (kf_seed, fold):
        - 5-fold scaffold split
        - SLSQP simplex on fold-train slice (multi-start = 8)
        - apply to held-out fold-val
    Outer CV per seed: pooled RAE on the 5 outer-val folds.
    kf_seeds = {1066..1080} (15 fresh, EXCLUDES 1001 and 1051..1065 used
    by prior wide-seed verifies).

    Deploy:
        - SLSQP refit on FULL 253 -> single weight vector
        - apply to (513, 3) stacked te arrays -> te_nb3043

GATE:
    mean_pooled_rae < 0.4509 -> "BETTER_THAN_NB3030"
        (nb3043 beats current wide-seed-verified PRIMARY-1 nb3030)
    else                     -> "FAIL"

References:
    nb2960 K18 deep-30 OOF                 = 0.4536
    nb3000 K19 deep-30 OOF                 = 0.4607
    nb3014 K22 deep-30 OOF                 = 0.4708 (rebuild from nb3014)
    ...
    nb2992 per-fold simplex 3K (linear)    = 0.4479
...
Outputs:
    data/processed/nb3043_summary.json
    data/processed/nb3043_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3043.npy         (513,) float32 -- full-pool deploy te
    submissions/nb3043_K18_K19_K22_deep30_simplex.csv  (only if BETTER_THAN_NB3030)
"""
...
TAG = "nb3043"
PARENT_TAG = "nb2960+nb3000+nb3014(K22)"

# -- Inputs --------------------------------------------------------------------
K_LABELS = ["K18", "K19", "K22"]
OOF_PATHS = {
...
K_DEPTH = {"K18": "deep30", "K19": "deep30", "K22": "deep30"}

# -- CV protocol ---------------------------------------------------------------
N_FOLDS = 5
KF_SEEDS = list(range(1066, 1081))   # 15 fresh seeds {1066..1080}
                                     # EXCLUDES 1001 and 1051..1065
N_STARTS_FOLD = 8
N_STARTS_FULL = 12
DEGEN_MAX_W = 0.85
...
def _run_one_seed(P_unb: np.ndarray, y_unb: np.ndarray,
    ...
    """Run nb3043 per-fold SLSQP simplex pipeline at a single kf_seed."""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 15-seed per-fold SLSQP simplex on {K_LABELS} (all deep-30)")
    print(f"          NEW PARADIGM: K22 worse standalone (0.4708) than K23 (0.4750)")
    print(f"          but maybe pair-correlation with K18/K19 different")
    print(f"          all 3 anchors PRE-unblind (chemprop_aux + residual LGBM)")
    print(f"          kf_seeds = {KF_SEEDS[0]}..{KF_SEEDS[-1]} (15 fresh, NEW)")
    ...
    # Pair-wise correlations (the heart of this hypothesis)
    ...
    if mean_rae < GATE_BETTER_THAN_NB3030:
        ...
        ladder_action = (
            f"PROMOTE nb3043 to PRIMARY-1. Wide-seed mean {mean_rae:.4f} "
            f"beats nb3030 ceiling {REF_NB3030:.4f}. "
            f"Pair-correlation hypothesis CONFIRMED: K22 swap-in for K23 "
            f"opens new blend axis despite weaker standalone."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT nb3043. Wide-seed mean {mean_rae:.4f} >= nb3030 "
            f"ceiling {REF_NB3030:.4f}. Keep nb3030 as PRIMARY-1. "
            f"K22 pair-correlation hypothesis NOT confirmed."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_K18_K19_K22_deep30_simplex.csv"
    ...
    summary = {
        ...
        "method": "wide_seed_15_per_fold_slsqp_K18_K19_K22_all_deep30",
        "paradigm": "K22_swap_for_K23_pair_correlation_hypothesis",
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_nb3030", "delta_vs_nb3001", "delta_vs_nb3003",
        "verdict", "ladder_action",
    ):
        print(f"  {k}: {res.get(k)}")
    print(f"  per_K_full_oof_rae: {res.get('per_K_full_oof_rae')}")
    print(f"  oof_corr_matrix: {res.get('oof_corr_matrix')}")
