"""nb1892 -- LightGBM MSE 5-seed bag on 117-col 5-way K-tuned (alt seed family).

HYPOTHESIS:
    nb1881's MSE-only 5-seed bag at seeds {60, 70, 80, 90, 100} achieves an
    in-sample 253-unblind RAE of 0.4999. The seed selection on a 5-bag is a
    luck variable. This notebook probes whether the in-sample 0.4999 is
    structural (objective + features + anchor) or partly seed-luck by re-
    running an identical 5-bag with a disjoint seed family {3, 17, 99, 271,
    1000}. If the mean-bag RAE stays within ~0.002 of 0.4999 the MSE result
    is structural; if it drifts > 0.005 the original seed family was lucky.
...
PROTOCOL:
    ...
    3. 5 MSE seeds = [3, 17, 99, 271, 1000].
    ...
    4. Aggregations:
         - per-seed RAE list
         - mean-bag RAE = rae(y_unb, mean of 5 corrected OOFs)
    5. Verdict at DECISION_MARGIN = 0.003 vs nb1881 MSE bag (0.4999):
         BEAT_NB1881_MSE       if mean-bag RAE < 0.4999 - 0.003
         TIES_NB1881_MSE       if |mean-bag RAE - 0.4999| <= 0.003
         WORSE_THAN_NB1881_MSE if mean-bag RAE > 0.4999 + 0.003

Outputs:
    scripts/nb1892_lgbm_mse_alt_seeds.py
    data/processed/nb1892_summary.json
    data/processed/nb1892_per_seed_oof.npy   (5, 253) float32
    data/processed/nb1892_mean_bag_oof.npy   (253,)   float32
"""
...
NB1881_MSE_BAG_REF = 0.4999   # MSE-only 5-seed bag RAE from nb1881
...
def main() -> dict:
    ...
    # ---- 5-seed MSE bag, alt seed family ----
    print("\n" + "-" * 78)
    print(f"5-SEED MSE BAG (alt seeds): {MSE_SEEDS}  "
          f"(cross-fit {RESID_FOLDS}-fold per seed; dim={feat_dim})")
    print("-" * 78)
    ...
    for s in MSE_SEEDS:
        ...
        per_seed_seeds.append(int(s))
        per_seed_rae.append(r)
        per_seed_oof_list.append(corr)
        print(f"   mse    seed={s:4d}  rae={r:.4f}  wall={time.time() - t_s:.1f}s")

    all_arr = np.vstack(per_seed_oof_list)   # (5, n_unb)
    mean_bag_oof = all_arr.mean(axis=0)
    rae_mean_bag = float(rae(y_unb, mean_bag_oof))
    rae_per_seed_mean = float(np.mean(per_seed_rae))

    # --- Verdict at 0.003 margin vs nb1881 MSE bag (0.4999) ---
    delta_vs_nb1881_mse = rae_mean_bag - nb1881_mse_bag_ref
    if delta_vs_nb1881_mse < -DECISION_MARGIN:
        verdict = "BEAT_NB1881_MSE"
    elif abs(delta_vs_nb1881_mse) <= DECISION_MARGIN:
        verdict = "TIES_NB1881_MSE"
    else:
        verdict = "WORSE_THAN_NB1881_MSE"
    ...
    print(f"   MEAN-BAG (5 seeds) RAE = {rae_mean_bag:.4f}")
    print(f"   nb1881 MSE bag ref     = {nb1881_mse_bag_ref:.4f}")
    print(f"   delta mean-bag vs ref  = {delta_vs_nb1881_mse:+.4f}")
    ...
    summary = {
        ...
        "per_seed_seeds": per_seed_seeds,
        "per_seed_rae": per_seed_rae,
        "rae_per_seed_mean": rae_per_seed_mean,
        "rae_mean_bag": rae_mean_bag,
        "nb1881_mse_bag_ref": nb1881_mse_bag_ref,
        "delta_mean_bag_vs_nb1881_mse": delta_vs_nb1881_mse,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "mse_seeds", "resid_folds",
        "K_AP_best", "K_Mord_best", "K_Embed_best", "K_Avalon_used",
        "n_chembl_pool", "feat_dim",
        "rae_anchor_chemprop_aux",
        "per_seed_seeds", "per_seed_rae",
        "rae_per_seed_mean", "rae_mean_bag",
        "nb1881_mse_bag_ref", "delta_mean_bag_vs_nb1881_mse",
