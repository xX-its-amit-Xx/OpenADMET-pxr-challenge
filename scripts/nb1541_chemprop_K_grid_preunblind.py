"""nb1541 -- SHAP K-grid for chemprop_embed on PRE-unblind chemprop_aux anchor.

Hypothesis:
    nb1484 fixed the ChempropEmbed slot at K=30 (mean_bag pooled RAE 0.5401)
    by analogy with the AtomPair / Mordred K=30 recipes.  The optimal K for
    chemprop_embed against a PRE-unblind chemprop_aux anchor is not pinned --
    could be smaller (more pruning at n=253 -> less residual capacity, less
    overfit) or larger (chemprop_embed has dense, low-bit-correlation features
    so more dims may carry signal).  Sweep K in {10, 15, 20, 30, 50, 75, 100}
    on the SAME SHAP ranking used by nb1484 (TreeExplainer, seed=0, full-residual
    LGBM on the 302-col matrix).  Keep ChEMBL pred + sim cols identical.  Same
    5-seed bag, same 5-fold cross-fit, same LGBM hyperparams.

Protocol:
    1.  Anchor = chemprop_aux_te[unb_idx]  (PRE-unblind te slice, in_RAE 0.6216).
        residual = y_unb - anchor.
    2.  Build ChEMBL PXR pool (same union as nb1484).  Compute kNN-5 Tanimoto on
        cached Morgan-2048 over std test SMILES; pull pred_chembl_pec50 +
        mean_sim for all test rows, slice to unblind.
    3.  Load te_chemprop_embed_300.npy, slice to 253 unblind.  Build full 302-col
        feature matrix = chemprop_embed-300 + pred_chembl + sim.
    4.  Train ONE seed-0 shallow LGBM Huber on residual; SHAP TreeExplainer
        (fallback LGBM gain) -> embed-only importance vector.
    5.  For each K in K_GRID, slice top-K embed dims by SHAP importance.  Build
        (K + 2)-col feature matrix on 253 unblind rows.
    6.  5-seed bag (seeds [0, 1, 7, 42, 137]), 5-fold cross-fit per K.  Mean-bag
        pooled RAE per K.
    7.  Verdict at 0.003 margin vs nb1484 ChempropEmbed slot (0.5401 mean-bag).

Outputs:
    scripts/nb1541_chemprop_K_grid_preunblind.py    (this file)
    data/processed/nb1541_summary.json
    data/processed/nb1541_best_K_oof.npy            (253,) float32  best K mean-bag
"""
...
NB1484_CHEMPROP_EMBED_SLOT_REF = 0.5401   # ChempropEmbed K=30 mean_bag in nb1484
DECISION_MARGIN = 0.003

K_GRID = [10, 15, 20, 30, 50, 75, 100]
...
def _evaluate_K(K: int, X_embed_unb: np.ndarray, top_dim_order: np.ndarray,
    ...
    return {
        ...
        "delta_mean_bag_vs_nb1484_slot": rae_mean_bag - NB1484_CHEMPROP_EMBED_SLOT_REF,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP K-grid for chemprop_embed  (anchor={ANCHOR}, PRE-unblind)")
    print(f"          K_GRID = {K_GRID}")
    print(f"          nb1484 ChempropEmbed slot ref = "
          f"{NB1484_CHEMPROP_EMBED_SLOT_REF:.4f}  (K=30 mean_bag)")
    ...
    # ---- ChEMBL pool ----
    print("\n" + "-" * 78)
    print("CHEMBL PXR POOL (local cache; same union as nb1484)")
    print("-" * 78)
    ...
    if X_embed_te.shape[0] != n_test:
        raise ValueError(f"chemprop embed cache shape mismatch: {X_embed_te.shape}")
    n_embed = int(X_embed_te.shape[1])
    print(f"   chemprop embed cache shape = {X_embed_te.shape}  (n_dims={n_embed})")
    ...
    print(f"\n   full feature matrix: {X_unb_full.shape}  "
          f"(chemprop_embed-{n_embed} + pred_chembl + sim)")
    ...
    print(f"   chemprop embed dims with nonzero importance: "
          f"{n_nonzero_imp}/{n_embed}")

    # ---- Sanity-check ranking vs nb1484 stored top-30 ----
    nb1484_summary_p = DATA_PROCESSED / "nb1484_summary.json"
    rank_match_top30 = None
    set_overlap_top30 = None
    if nb1484_summary_p.exists():
        with open(nb1484_summary_p, "r") as f:
            nb1484_summary = json.load(f)
        nb1484_chemprop = None
        for fam in nb1484_summary.get("families", []):
            if fam.get("family") == "ChempropEmbed":
                nb1484_chemprop = fam
                break
        if nb1484_chemprop is not None:
            nb1484_top30 = nb1484_chemprop.get("top_idx_ranked", [])
            local_top30 = top_dim_order[:30].tolist()
            rank_match_top30 = (local_top30 == nb1484_top30)
            set_overlap_top30 = len(set(local_top30) & set(nb1484_top30))
            print(f"   nb1484 ChempropEmbed top-30 ranking match: "
                  f"{rank_match_top30}  (set overlap = {set_overlap_top30}/30)")
    ...
    for K in K_GRID:
        ...
        print(f"   K={K:3d}  feat_dim={rec['feat_dim']:3d}  "
              ...
              f"d_vs_nb1484_slot={rec['delta_mean_bag_vs_nb1484_slot']:+.4f}  "
              f"[{dt:.1f}s]")
    ...
    beats_nb1484_slot = best_rae < NB1484_CHEMPROP_EMBED_SLOT_REF - DECISION_MARGIN
    ties_nb1484_slot = abs(best_rae - NB1484_CHEMPROP_EMBED_SLOT_REF) <= DECISION_MARGIN
    beats_anchor = best_rae < rae_anchor - DECISION_MARGIN

    if beats_nb1484_slot:
        verdict = (
            f"CHEMPROP_K_GRID_BEATS_NB1484_SLOT_AT_K={best_K}_"
            f"NEW_PREUNBLIND_CHEMPROP_EMBED_SLOT"
        )
    elif ties_nb1484_slot:
        verdict = f"CHEMPROP_K_GRID_FLAT_VS_NB1484_SLOT_AT_K={best_K}"
    elif beats_anchor:
        verdict = (
            f"CHEMPROP_K_GRID_LOSES_TO_NB1484_SLOT_BUT_BEATS_ANCHOR_"
            f"BEST_K={best_K}"
        )
    else:
        verdict = f"CHEMPROP_K_GRID_HURTS_ANCHOR_BEST_K={best_K}"

    print("\n" + "-" * 78)
    print("K-GRID SUMMARY")
    print("-" * 78)
    print(f"   {'K':>4}  {'feat_dim':>8}  {'per_seed_mean':>13}  "
          f"{'mean_bag':>9}  {'median_bag':>10}  {'d_vs_slot':>10}  "
          f"{'d_vs_anchor':>11}")
    # reference row
    print(f"   {'30':>4}  {32:>8}  {'(ref)':>13}  "
          f"{NB1484_CHEMPROP_EMBED_SLOT_REF:>9.4f}  {'(ref)':>10}  "
          f"{0.0:>+10.4f}  {NB1484_CHEMPROP_EMBED_SLOT_REF - rae_anchor:>+11.4f}"
          f"   <-- nb1484 ChempropEmbed slot ref")
    for r in sorted(per_K_records, key=lambda x: x["K"]):
        print(f"   {r['K']:>4}  {r['feat_dim']:>8}  "
              ...
              f"{r['delta_mean_bag_vs_nb1484_slot']:>+10.4f}  "
              f"{r['delta_mean_bag_vs_anchor']:>+11.4f}")
    print(f"\n   best K (sweep only)        = {best_K}  RAE = {best_rae:.4f}")
    print(f"   nb1484 ChempropEmbed slot  = {NB1484_CHEMPROP_EMBED_SLOT_REF:.4f}")
    print(f"   delta(best - nb1484_slot)  = "
          f"{best_rae - NB1484_CHEMPROP_EMBED_SLOT_REF:+.4f}  "
          f"(margin = {DECISION_MARGIN})")
    ...
    summary = {
        ...
        "data_source": "te_chemprop_embed_300_nb1312_cache + local_chembl_caches_union",
        ...
        "nb1484_chemprop_top30_rank_match": rank_match_top30,
        "nb1484_chemprop_top30_set_overlap": set_overlap_top30,
        ...
        "nb1484_chemprop_embed_slot_ref": NB1484_CHEMPROP_EMBED_SLOT_REF,
        "decision_margin": DECISION_MARGIN,
        "per_K_records": per_K_records,
        "rae_by_K": {str(k): v for k, v in rae_by_K.items()},
        "best_K": int(best_K),
        "best_K_rae_mean_bag": float(best_rae),
        "delta_best_K_vs_nb1484_slot": float(
            best_rae - NB1484_CHEMPROP_EMBED_SLOT_REF
        ),
        "delta_best_K_vs_anchor": float(best_rae - rae_anchor),
        "beats_nb1484_slot": bool(beats_nb1484_slot),
        "ties_nb1484_slot": bool(ties_nb1484_slot),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb1484_chemprop_top30_rank_match",
        "nb1484_chemprop_top30_set_overlap",
        "embed_nonzero_imp_dims",
        "rae_anchor_chemprop_aux",
        "K_grid", "rae_by_K",
        "best_K", "best_K_rae_mean_bag",
        "delta_best_K_vs_nb1484_slot", "delta_best_K_vs_anchor",
        "beats_nb1484_slot", "ties_nb1484_slot", "beats_anchor",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
