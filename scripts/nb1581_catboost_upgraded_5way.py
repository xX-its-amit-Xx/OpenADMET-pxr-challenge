"""nb1581 -- CatBoost on upgraded 5-way K-tuned (Avalon K=20) 107-col features.

HYPOTHESIS:
    nb1554 (117-col, Avalon K=30) reported mean_bag RAE = 0.5163.
    nb1561 BoB-validated to 0.5155.  Avalon was the ONLY family kept at
    its default K=30 -- every other family was K-tuned.  This script
    drops Avalon to K=20 (matching the other K-tuned families) to test
    whether tighter Avalon truncation removes noise SHAP bits and lifts
    the mean_bag RAE past nb1561's BoB ceiling.

PROTOCOL
    1. Build 107-col feature stack:
         top-25 AtomPair      (nb1524 best_K)
         top-20 MACCS         (nb1352 fixed)
         top-20 Mordred       (nb1523 best_K)
         top-20 ChempropEmbed (nb1541 best_K)
         top-20 Avalon        (nb1392 SHAP top-30 sliced to 20)
         pred_chembl_pec50    (ChEMBL kNN-5)
         mean_sim
       Total dim = 25 + 20 + 20 + 20 + 20 + 2 = 107.
    ...
    5. Verdict at 0.003 margin vs:
         - nb1554 (0.5163, in-sample mean_bag)
         - nb1561 BoB (0.5155, outer-bag mean)

Outputs:
    scripts/nb1581_catboost_upgraded_5way.py
    data/processed/nb1581_summary.json
    data/processed/nb1581_mean_bag_oof.npy        (253,) float32
"""
...
# Avalon K-tune target (was K=30 in nb1554)
K_AVALON_TARGET = 20
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163       # CatBoost 5-way K-tuned, Avalon K=30 (in-sample mean_bag)
NB1561_REF = 0.5155       # nb1554 BoB outer-bag mean
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on UPGRADED 5-way "
          f"K-tuned 107-col matrix (Avalon K={K_AVALON_TARGET}); "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1554 ({NB1554_REF:.4f}), nb1561_BoB ({NB1561_REF:.4f})  "
    ...
    # Avalon = nb1392 SHAP top-30 ranking, sliced to top-20 (UPGRADE vs nb1554)
    avalon_full_ranked = np.array(
        sum_1392["top_avalon_bit_indices_ranked"], dtype=int
    )
    if len(avalon_full_ranked) < K_AVALON_TARGET:
        raise ValueError(
            f"Avalon ranking only has {len(avalon_full_ranked)} bits; "
            f"need >= {K_AVALON_TARGET}"
        )
    top_avalon_bit_idx = avalon_full_ranked[:K_AVALON_TARGET]
    K_Avalon_used = int(K_AVALON_TARGET)
    ...
    print(f"[reuse] top-{n_top_avalon}     Avalon bits (nb1392 SHAP top-30 -> top-{K_AVALON_TARGET})")
    ...
    # ---- Build COMBINED 5-way UPGRADED feature matrix (107 cols) ----
    ...
    if feat_dim != 107:
        raise ValueError(
            f"feat_dim {feat_dim} != 107 (target); check K-grid sources"
        )
    print(f"\n   UPGRADED 5-WAY matrix: {X_unb.shape}  "
    ...
    pearson_vs_nb1561_mean = _pearson_vs(DATA_PROCESSED / "nb1561_bob_mean_oof.npy")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1561 = {rae_mean_bag - NB1561_REF:+.4f})")
    ...
    if pearson_vs_nb1561_mean is not None:
        print(f"   Pearson(mean_bag, nb1561_bob_mn) = {pearson_vs_nb1561_mean:.4f}")
    ...
    beats_nb1561 = rae_mean_bag < NB1561_REF - DECISION_MARGIN
    flat_vs_nb1554 = abs(rae_mean_bag - NB1554_REF) < DECISION_MARGIN
    flat_vs_nb1561 = abs(rae_mean_bag - NB1561_REF) < DECISION_MARGIN

    if beats_nb1561:
        verdict = "UPGRADED_5WAY_AVALON_K20_BEATS_NB1561_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1561:
        verdict = "UPGRADED_5WAY_AVALON_K20_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = "UPGRADED_5WAY_AVALON_K20_BEATS_NB1554_BUT_WORSE_THAN_NB1561"
    elif flat_vs_nb1554:
        verdict = "UPGRADED_5WAY_AVALON_K20_FLAT_VS_NB1554"
    elif beats_nb1543:
        verdict = "UPGRADED_5WAY_AVALON_K20_BEATS_NB1543_BUT_WORSE_THAN_NB1554"
    elif beats_anchor:
        verdict = "UPGRADED_5WAY_AVALON_K20_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "UPGRADED_5WAY_AVALON_K20_FLAT_VS_ANCHOR"
    else:
        verdict = "UPGRADED_5WAY_AVALON_K20_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "K_source": {
            ...
            "Avalon": f"nb1392 SHAP top-30 sliced to top-{K_AVALON_TARGET} (UPGRADE vs nb1554 K=30)",
        },
        ...
        "delta_mean_bag_vs_nb1561": rae_mean_bag - NB1561_REF,
        ...
        "flat_vs_nb1561": bool(flat_vs_nb1561),
        "pearson_vs_anchor": pearson_vs_anchor,
        "pearson_vs_nb1554_mean_bag": pearson_vs_nb1554,
        "pearson_vs_nb1561_bob_mean": pearson_vs_nb1561_mean,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1561",
        "beats_chemprop_aux",
        "beats_nb1554", "flat_vs_nb1554",
        "beats_nb1561", "flat_vs_nb1561",
        "pearson_vs_anchor",
