"""nb1513 -- PRE-unblind 6-way SHAP-pruned residual blend.

Extends nb1484's 4-way (AtomPair-30, MACCS-20, Mordred-30, ChempropEmbed-30)
with TWO additional residual learners:
    5. Morgan-2048 top-30 (SHAP-pruned via nb1382 ranking) + ChEMBL kNN (32 cols)
    6. AtomPair-2048 top-25 (uses nb1373 top-25, the first 25 of its top-30
       ranking) + ChEMBL kNN (27 cols)

All anchored to chemprop_aux (PRE-unblind te file on 513).

HYPOTHESIS
    More feature axes may help if not already redundant.  Morgan and
    AtomPair-K=25 are both untested in the PRE-unblind path.  Morgan was the
    OG fingerprint family for this project; AtomPair-K=25 is a feature-budget
    contraction of an already-helpful learner.

PROTOCOL
    1. Anchor = chemprop_aux_te[unb_idx]  (PRE-unblind).
    2. ChEMBL PXR kNN-5 (Tanimoto on Morgan-2048): same union as
       nb1472/nb1484/nb1492.
    3. For each of the FIRST FOUR families (AtomPair, MACCS, Mordred,
       ChempropEmbed): identical to nb1484/nb1492 -- SHAP-prune top-K family
       columns + (pred_chembl, sim).
    4. For Morgan-30: re-derive Morgan-2048 via morgan_fp_batch (no persistent
       te_morgan cache); SHAP-prune on residual; take top-30 bits.
    5. For AtomPair-K25: use the cached te_atompair.npy, but take the
       precomputed top-25 indices from nb1373's top-30 SHAP-rank (first 25).
       This re-uses an established SHAP ranking; we don't recompute SHAP.
    6. Each learner: 5-seed bag (seeds [0, 1, 7, 42, 137]), KFold(n=5)
       cross-fit on shallow LGBM Huber (depth=3, num_leaves=7, n_est=80,
       lr=0.05, huber_alpha=1.0, min_child_samples=20).
    7. Per-learner mean-bag corrected OOF = anchor + mean_bag(residual_oof).
    8. Naive 1/6 mean blend + 5-fold SLSQP cross-fit (simplex K=6).
    9. Verdict at 0.003 margin vs nb1484 (0.5231) and nb1492 (0.5260).

Outputs:
    scripts/nb1513_preunblind_6way.py             (this file)
    data/processed/nb1513_summary.json
    data/processed/nb1513_best_oof.npy             (253,) float32 best variant
"""
...
# Pre-computed SHAP rankings (re-used; we don't recompute for these).
NB1382_SUMMARY_PATH = DATA_PROCESSED / "nb1382_summary.json"
...
# Six learners.  AtomPair-K30 + Morgan-K30 + AtomPairK25 all draw from
# te_atompair.npy / morgan_fp_batch; the difference is the *index subset*.
TOP_K = {
    "AtomPair": 30,
    "MACCS": 20,
    "Mordred": 30,
...
# Families for which we RECOMPUTE SHAP in-script (nb1484 protocol).
SHAP_RECOMPUTE_FAMILIES = {
    "AtomPair", "MACCS", "Mordred", "ChempropEmbed", "Morgan",
}
# Families for which we REUSE a precomputed ranking (no SHAP recompute).
PRECOMPUTED_RANKING_FAMILIES = {"AtomPairK25"}
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1472/nb1484/nb1492."""
    frames = []
...
def _build_morgan_te(std_test_smiles: list[str], n_test: int) -> np.ndarray:
    """Rebuild Morgan-2048 for the 513 test compounds (no persistent cache)."""
    X = morgan_fp_batch(
        std_test_smiles,
    ...
    if X.shape[0] != n_test:
        raise ValueError(f"Morgan rebuild shape mismatch: {X.shape}")
    return X


def _load_family_te(family: str, n_test: int,
                    std_test_smiles: list[str] | None) -> np.ndarray:
    if family == "AtomPair" or family == "AtomPairK25":
        p = ATOMPAIR_TE_PATH
        if not p.exists():
    ...
    if family == "ChempropEmbed":
        ...
        return X
    if family == "Morgan":
        if std_test_smiles is None:
            raise ValueError("std_test_smiles required for Morgan rebuild")
        return _build_morgan_te(std_test_smiles, n_test)
    raise ValueError(f"unknown family: {family}")


def _load_precomputed_top_idx(family: str, top_k: int):
    """Load a precomputed SHAP top-K ranking for families we don't recompute."""
    if family == "AtomPairK25":
        ...
        with open(NB1373_SUMMARY_PATH) as f:
            d = json.load(f)
        ranked = d["top_atompair_bit_indices_ranked"]
        imps = d.get("top_atompair_bit_importance_ranked")
        if len(ranked) < top_k:
            raise ValueError(
                f"nb1373 ranking has {len(ranked)} indices, < {top_k} requested"
            )
        idx = np.array(ranked[:top_k], dtype=int)
        imp = (
            np.array(imps[:top_k], dtype=float)
            if imps is not None else np.zeros(top_k, dtype=float)
        )
        return idx, imp, "nb1373_top30_first25"
    raise ValueError(
        f"no precomputed ranking handler for family={family}"
    )


def _run_family_shap_recompute(family: str, X_fam_unb: np.ndarray,
                               pred_chembl_unb: np.ndarray,
                               mean_sim_unb: np.ndarray,
                               anchor: np.ndarray, y_unb: np.ndarray,
                               residual: np.ndarray, top_k: int) -> dict:
    """nb1484/nb1492-style: build FULL matrix, SHAP-prune in-script."""
    print("\n" + "=" * 78)
    print(f"FAMILY = {family}   X_fam_unb shape = {X_fam_unb.shape}   "
          f"top_k = {top_k}   mode = shap_recompute")
    print("=" * 78)
    ...
    pred_chembl_imp_val = float(imp_full[n_fam])
    sim_imp_val = float(imp_full[n_fam + 1])

    return _bag_with_top_idx(
        family=family,
        X_fam_unb=X_fam_unb,
        ...
        top_imp=top_imp,
        n_fam=n_fam,
        feat_dim_full=feat_dim_full,
        n_nonzero_imp=n_nonzero_imp,
        pred_chembl_importance=pred_chembl_imp_val,
        sim_importance=sim_imp_val,
        shap_source=imp_src,
        mode="shap_recompute",
    )


def _run_family_precomputed(family: str, X_fam_unb: np.ndarray,
                            pred_chembl_unb: np.ndarray,
                            mean_sim_unb: np.ndarray,
                            anchor: np.ndarray, y_unb: np.ndarray,
                            residual: np.ndarray, top_k: int) -> dict:
    """Use a precomputed SHAP ranking; no FULL-matrix SHAP step."""
    print("\n" + "=" * 78)
    print(f"FAMILY = {family}   X_fam_unb shape = {X_fam_unb.shape}   "
          f"top_k = {top_k}   mode = precomputed_ranking")
    print("=" * 78)
    n_fam = int(X_fam_unb.shape[1])
    top_idx, top_imp, src = _load_precomputed_top_idx(family, top_k)
    top_k_eff = len(top_idx)
    top_idx_sorted = np.sort(top_idx)
    print(f"   ranking source = {src}")
    print(f"   top-{top_k_eff} {family} indices (sorted asc): "
          f"{top_idx_sorted.tolist()}")
    feat_dim_full = n_fam + 2  # +pred_chembl, +sim  (not built here)
    ...
    return _bag_with_top_idx(
        ...
        n_fam=n_fam,
        feat_dim_full=feat_dim_full,
        n_nonzero_imp=int((top_imp > 0).sum()),
        pred_chembl_importance=float("nan"),
        sim_importance=float("nan"),
        shap_source=src,
        mode="precomputed_ranking",
    )


def _bag_with_top_idx(family: str, X_fam_unb: np.ndarray,
                      pred_chembl_unb: np.ndarray, mean_sim_unb: np.ndarray,
                      anchor: np.ndarray, y_unb: np.ndarray,
                      residual: np.ndarray, top_idx: np.ndarray,
                      top_imp: np.ndarray, n_fam: int, feat_dim_full: int,
                      n_nonzero_imp: int, pred_chembl_importance: float,
                      sim_importance: float, shap_source: str,
                      mode: str) -> dict:
    """Shared bagging path once top_idx is known."""
    ...
    return {
        ...
        "pred_chembl_importance": pred_chembl_importance,
        "sim_importance": sim_importance,
        "n_nonzero_imp": n_nonzero_imp,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- PRE-unblind 6-way: chemprop_aux anchor + "
          f"AtomPair-30 + MACCS-20 + Mordred-30 + ChempropEmbed-30 + "
          f"Morgan-30 + AtomPair-K25")
    ...
    # ---- Standardize test SMILES once (used for kNN + Morgan rebuild) ----
    ...
    for family in FAMILIES:
        X_fam_te = _load_family_te(
            family, n_test,
            std_test_smiles if family == "Morgan" else None,
        )
        X_fam_unb = X_fam_te[unb_idx].astype(np.float32)
        if family in SHAP_RECOMPUTE_FAMILIES:
            result = _run_family_shap_recompute(
                family=family,
                X_fam_unb=X_fam_unb,
                pred_chembl_unb=pred_chembl_unb,
                mean_sim_unb=mean_sim_unb,
        ...
        elif family in PRECOMPUTED_RANKING_FAMILIES:
            result = _run_family_precomputed(
                family=family,
                X_fam_unb=X_fam_unb,
                pred_chembl_unb=pred_chembl_unb,
                mean_sim_unb=mean_sim_unb,
            ...
            raise ValueError(f"family {family} has no run mode")
    ...
    print(f"   nb1472 ref                   = {NB1472_REF:.4f}")
    print(f"   nb1484 ref                   = {NB1484_REF:.4f}")
    print(f"   nb1492 ref                   = {NB1492_REF:.4f}")
    print(f"   naive 1/6 mean blend RAE     = {rae_mean:.4f}")
    print(f"   5-fold SLSQP cross-fit RAE   = {rae_slsqp:.4f}")
    ...
    delta_naive_vs_nb1484 = rae_mean - NB1484_REF
    delta_slsqp_vs_nb1484 = rae_slsqp - NB1484_REF
    delta_best_vs_nb1484 = rae_best - NB1484_REF
    delta_naive_vs_nb1492 = rae_mean - NB1492_REF
    delta_slsqp_vs_nb1492 = rae_slsqp - NB1492_REF
    delta_best_vs_nb1492 = rae_best - NB1492_REF
    delta_naive_vs_nb1472 = rae_mean - NB1472_REF
    delta_slsqp_vs_nb1472 = rae_slsqp - NB1472_REF
    delta_best_vs_nb1472 = rae_best - NB1472_REF
    ...
    beats_nb1484_naive = rae_mean < NB1484_REF - DECISION_MARGIN
    beats_nb1484_slsqp = rae_slsqp < NB1484_REF - DECISION_MARGIN
    beats_nb1484_best = rae_best < NB1484_REF - DECISION_MARGIN
    flat_vs_nb1484_best = abs(rae_best - NB1484_REF) < DECISION_MARGIN
    beats_nb1492_naive = rae_mean < NB1492_REF - DECISION_MARGIN
    beats_nb1492_slsqp = rae_slsqp < NB1492_REF - DECISION_MARGIN
    beats_nb1492_best = rae_best < NB1492_REF - DECISION_MARGIN
    beats_nb1472_naive = rae_mean < NB1472_REF - DECISION_MARGIN
    beats_nb1472_slsqp = rae_slsqp < NB1472_REF - DECISION_MARGIN
    beats_nb1472_best = rae_best < NB1472_REF - DECISION_MARGIN
    beats_anchor_best = rae_best < rae_anchor - DECISION_MARGIN

    if beats_nb1484_best:
        verdict = (
            f"PRE_UNBLIND_6WAY_BEATS_NB1484_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1484_best:
        verdict = f"PRE_UNBLIND_6WAY_FLAT_VS_NB1484_best_{which_best}"
    elif beats_nb1492_best:
        verdict = (
            f"PRE_UNBLIND_6WAY_BEATS_NB1492_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif beats_nb1472_best:
        verdict = (
            f"PRE_UNBLIND_6WAY_BEATS_NB1472_BUT_WORSE_THAN_NB1492"
            f"_best_{which_best}"
        )
    elif beats_anchor_best:
        verdict = (
            f"PRE_UNBLIND_6WAY_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "PRE_UNBLIND_6WAY_FLAT_VS_ANCHOR"
    else:
        verdict = "PRE_UNBLIND_6WAY_HURTS_ANCHOR"

    print(f"   d_naive_vs_nb1484   = {delta_naive_vs_nb1484:+.4f}")
    print(f"   d_slsqp_vs_nb1484   = {delta_slsqp_vs_nb1484:+.4f}")
    print(f"   d_best_vs_nb1484    = {delta_best_vs_nb1484:+.4f}   "
          f"(best = {which_best})")
    print(f"   d_naive_vs_nb1492   = {delta_naive_vs_nb1492:+.4f}")
    print(f"   d_slsqp_vs_nb1492   = {delta_slsqp_vs_nb1492:+.4f}")
    print(f"   d_best_vs_nb1492    = {delta_best_vs_nb1492:+.4f}")
    ...
    print(f"   beats_nb1492_best   = {beats_nb1492_best}")
    print(f"   beats_nb1472_best   = {beats_nb1472_best}")
    ...
    summary = {
        ...
        "delta_naive_vs_nb1472": delta_naive_vs_nb1472,
        "delta_slsqp_vs_nb1472": delta_slsqp_vs_nb1472,
        "delta_best_vs_nb1472": delta_best_vs_nb1472,
        "delta_naive_vs_nb1484": delta_naive_vs_nb1484,
        "delta_slsqp_vs_nb1484": delta_slsqp_vs_nb1484,
        "delta_best_vs_nb1484": delta_best_vs_nb1484,
        "delta_naive_vs_nb1492": delta_naive_vs_nb1492,
        "delta_slsqp_vs_nb1492": delta_slsqp_vs_nb1492,
        "delta_best_vs_nb1492": delta_best_vs_nb1492,
        "beats_nb1472_naive": bool(beats_nb1472_naive),
        "beats_nb1472_slsqp": bool(beats_nb1472_slsqp),
        "beats_nb1472_best": bool(beats_nb1472_best),
        ...
        "beats_nb1492_naive": bool(beats_nb1492_naive),
        "beats_nb1492_slsqp": bool(beats_nb1492_slsqp),
        "beats_nb1492_best": bool(beats_nb1492_best),
        "beats_anchor_best": bool(beats_anchor_best),
        "verdict": verdict,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb1472_ref": NB1472_REF,
        "nb1484_ref": NB1484_REF,
        "nb1492_ref": NB1492_REF,
