"""nb1822 -- Outer-bag VALIDATION of nb1812 (LGBM small leaves + feature_fraction).
...
PROTOCOL:
    For each outer seed o in {0, 1, 7, 42, 137}:
        ...
        For each inner seed:
            ...
            on 117-col 5-way K-tuned matrix; anchor = chemprop_aux te[unb_idx].
        Pool the 5 inner-seed corrected anchors -> per-outer mean-bag.
        Record per-outer pooled RAE.

    Aggregate BoB (Bag-of-Bags):
        bob_mean_oof   = mean across the 5 per-outer mean-bag oofs
        bob_median_oof = median across the 5 per-outer mean-bag oofs

    Verdict NB1812_REPRODUCES if BoB per-outer mean RAE within 0.003 of
    nb1812 reference 0.5072 (rae_mean_bag from nb1812_summary.json).

Outputs:
    scripts/nb1822_bag_nb1812.py
    data/processed/nb1822_summary.json
    data/processed/nb1822_bob_mean_oof.npy        (253,) float32
    data/processed/nb1822_bob_median_oof.npy      (253,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """Identical to nb1812: small leaves + feature_fraction + bagging."""
    return dict(
        objective="huber",
        alpha=1.0,
        max_depth=4,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG VALIDATION of nb1812 "
          f"(LGBM small leaves + feature_fraction=0.7 + bagging_fraction=0.8)")
    ...
    print(f"          NB1812_REF  = {NB1812_REF}  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- Outer-bag validation ----
    print("\n" + "-" * 78)
    print("OUTER-BAG VALIDATION (5 outers x 5 inners x 5 folds)")
    print("-" * 78)
    ...
    print(f"   nb1812_ref            = {NB1812_REF:.4f}  "
          f"(delta per-outer-mean vs ref = "
          f"{rae_per_outer_mean - NB1812_REF:+.4f})")

    # ---- Verdict (per spec): per-outer mean within 0.003 of 0.5072 ----
    delta_vs_ref = rae_per_outer_mean - NB1812_REF
    if abs(delta_vs_ref) <= DECISION_MARGIN:
        verdict = "NB1812_REPRODUCES"
    elif rae_per_outer_mean < NB1812_REF - DECISION_MARGIN:
        verdict = "NB1812_OPTIMISTIC_ON_VALIDATION"
    else:
        verdict = "NB1812_PESSIMISTIC_ON_VALIDATION"
    ...
    summary = {
        ...
        "delta_per_outer_mean_vs_ref": delta_vs_ref,
        "delta_bob_mean_vs_ref": rae_bob_mean - NB1812_REF,
        "delta_bob_median_vs_ref": rae_bob_median - NB1812_REF,
        "verdict": verdict,
