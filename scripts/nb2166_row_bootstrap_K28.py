"""nb2166 -- Row-bootstrap bagging at SHAP top-K=28 of 117-col matrix.

HYPOTHESIS:
    nb2159 noted that the residual problem has *low* tree-to-tree variance
    under pure seed-only perturbation -- so seed-only mean-bag (nb2103,
    0.4737) saturates close to the single-seed median.  The remaining
    decorrelation budget must come from ROW-BOOTSTRAP (Breiman bagging on
    the sample axis), not from seed perturbation.  This notebook isolates
    that lever: hold features fixed at SHAP top-28, hold the LGBM(MSE)
    hyperparameters fixed at the nb2103-K=28 recipe, and turn on LGBM's
    native row-bootstrap (`subsample` with `bagging_freq=1`).  Sweep the
    bootstrap fraction (subsample) in {0.7, 0.8, 0.9, 1.0}.  At
    subsample=1.0 the resample is full-N without replacement, equivalent
    to seed-only -- a clean ablation anchor.

    K=28 (target_K=28 from nb2159) corresponds to per-seed row-bootstrap
    bags of n=253 within each fold's TRAIN partition; OOB rows accumulate
    their leaf-mean predictions through LGBM's standard bagged-boosting
    machinery.  Mean-bag and median-bag are taken across the 5 outer
    seeds (each its own KFold split + bootstrap stream).

PROTOCOL:
    1. Load SHAP top-28 features (nb2063 cached SHAP importance,
       re-ranked on the 117-col 5-way K-tuned matrix to match nb2103 K=28).
    2. Load chemprop_aux te[unb_idx] anchor and form residual = y - anchor.
    3. For each subsample in {0.7, 0.8, 0.9, 1.0}: run 5-seed bag x
       5-fold cross-fit of LGBM(MSE, subsample=ss, bagging_freq=1) on
       chemprop_aux residual.  bagging_seed is bound to the outer seed
       so each of the 5 seeds gets a distinct bootstrap stream.
    4. Report per-subsample: per-seed RAE list, per-seed mean/median/std,
       mean-bag RAE, median-bag RAE.
    5. Verdict vs nb2103 K=28 (0.4737 mean-bag, 0.4698 median-bag) at
       decision_margin = 0.003.

Outputs:
    scripts/nb2166_row_bootstrap_K28.py
    data/processed/nb2166_summary.json
    data/processed/nb2166_mean_bag_oof_ss{ss}.npy   (253,) float32
    data/processed/nb2166_median_bag_oof_ss{ss}.npy (253,) float32
"""
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb2063/nb2081/nb2091/nb2103/nb2160."""
    frames = []
...
def _lgbm_bootstrap_params(seed: int, subsample: float) -> dict:
    """LGBM(MSE) -- same core hyperparams as nb2103 K=28 but with row-bootstrap.

    Notes:
      - subsample = bagging_fraction; row-fraction sampled WITHOUT replacement
        per iteration (LGBM does not expose true with-replacement bagging,
        but `subsample` < 1.0 with `bagging_freq=1` is the canonical
        row-bootstrap analog and is what the nb2159 prescription targets).
      - bagging_freq=1: re-sample rows every iteration (so trees see
        different row supports -> tree-to-tree decorrelation).
      - bagging_seed bound to outer seed -> distinct bootstrap stream
        per outer seed.
      - At subsample=1.0 the resample is full-N (no decorrelation); this is
        the seed-only ablation anchor and should match nb2103 K=28 closely.
      - feature_fraction = 1.0 (ff=1.0) to isolate row-bootstrap's effect.
    """
    return dict(
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, subsample: float) -> np.ndarray:
    """5-fold cross-fit; inside each fold, fit LGBM with row-bootstrap on
    the TRAIN partition and predict the held-out fold rows.

    Per the nb2159 prescription, the bootstrap sample is realized inside
    the LGBM booster (subsample + bagging_freq=1).  OOB rows accumulate
    predictions implicitly through LGBM's bagged boosting machinery: each
    tree's leaf statistics are computed on its bootstrap sample, but every
    train row is still scored by the full ensemble at predict-time.
    """
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = lgb.LGBMRegressor(**_lgbm_bootstrap_params(seed, subsample))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- ROW-BOOTSTRAP bagging at SHAP K={K_FIXED}, subsample sweep {SUBSAMPLES}")
    ...
    # ---- nb2103 K=28 reference (current best on this anchor + matrix) ----
    ...
    # ---- Load all K-grid winners (same as nb2103/nb2160) ----
    ...
    # ---- ChEMBL kNN feature (same as nb2103/nb2160) ----
    print("\n" + "-" * 78)
    ...
    # ---- subsample (row-bootstrap fraction) sweep ----
    print("\n" + "-" * 78)
    print(f"ROW-BOOTSTRAP SUBSAMPLE SWEEP {SUBSAMPLES} "
          f"(bagging_freq=1, bagging_seed=outer_seed)")
    print("-" * 78)
    per_ss_results: list[dict] = []
    for subsample in SUBSAMPLES:
        ss_tag = f"{int(round(subsample * 100)):03d}"
        ...
        for i, s in enumerate(RESID_SEEDS):
            ...
            print(f"   ss={subsample:.2f} seed={s:3d}:  rae_corr = {rae_s:.4f}  "
        ...
        print(f"   ss={subsample:.2f} per-seed RAE   = "
              f"[{', '.join(f'{r:.4f}' for r in per_seed_rae)}]")
        print(f"   ss={subsample:.2f} per-seed mean  = "
              f"{rae_per_seed_mean:.4f}  std = {rae_per_seed_std:.4f}")
        print(f"   ss={subsample:.2f} pooled mean    = {rae_mean_bag:.4f}  "
              f"(d_vs_nb2103 K=28 = {delta_mean_vs_nb2103:+.4f})")
        print(f"   ss={subsample:.2f} pooled median  = {rae_median_bag:.4f}  "
        ...
        print(f"   ss={subsample:.2f} verdict        = {verdict_d}")

        out_mean = DATA_PROCESSED / f"{TAG}_mean_bag_oof_ss{ss_tag}.npy"
        np.save(out_mean, mean_bag_oof.astype(np.float32))
        print(f"   [save] {out_mean}")
        out_median = DATA_PROCESSED / f"{TAG}_median_bag_oof_ss{ss_tag}.npy"
        np.save(out_median, median_bag_oof.astype(np.float32))
        print(f"   [save] {out_median}")

        per_ss_results.append({
    ...
    # ---- Summary table ----
    print("\n" + "=" * 78)
    print("ROW-BOOTSTRAP SUBSAMPLE SUMMARY TABLE")
    print("=" * 78)
    print(f"   {'ss':>5s}  {'mean_bag':>10s}  {'median_bag':>10s}  "
          f"{'per_seed_mean':>13s}  {'per_seed_std':>12s}  "
          f"{'d_mean_vs_K28':>13s}  {'d_med_vs_K28':>12s}  verdict")
    print(f"   {'NoBS':>5s}  {nb2103_k28_mean_bag:>10.4f}  "
          f"{nb2103_k28_median_bag:>10.4f}  {'N/A':>13s}  {'N/A':>12s}  "
          f"{0.0:>+13.4f}  {0.0:>+12.4f}  BASELINE(nb2103_K28)")
    for r in per_ss_results:
        print(f"   {r['subsample']:>5.2f}  {r['rae_mean_bag']:>10.4f}  "
    ...
    # best subsample by mean_bag and by median_bag (within sweep)
    sweep_mean = [r["rae_mean_bag"] for r in per_ss_results]
    sweep_median = [r["rae_median_bag"] for r in per_ss_results]
    best_mean_i = int(np.argmin(sweep_mean))
    best_median_i = int(np.argmin(sweep_median))
    best_ss_mean = float(per_ss_results[best_mean_i]["subsample"])
    best_ss_median = float(per_ss_results[best_median_i]["subsample"])
    ...
    all_label = ["NoBS_GBDT"] + [
        f"BS_ss{r['subsample']:.2f}" for r in per_ss_results
    ]
    ...
    print(f"\n   best BS (mean-bag) = ss={best_ss_mean:.2f}  "
          f"RAE {best_mean_bag:.4f}  "
          f"(d_vs_K28 = {best_mean_bag - nb2103_k28_mean_bag:+.4f})")
    print(f"   best BS (median-bag) = ss={best_ss_median:.2f}  "
    ...
    # ---- Global verdict ----
    if (best_overall_mean_label != "NoBS_GBDT"
            and best_overall_mean_rae < nb2103_k28_mean_bag - DECISION_MARGIN):
        global_verdict = (
            f"ROW_BOOTSTRAP_BEATS_NB2103_K28_MEAN_AT_{best_overall_mean_label}"
        )
    elif (best_overall_median_label != "NoBS_GBDT"
            and best_overall_median_rae < nb2103_k28_median_bag - DECISION_MARGIN):
        global_verdict = (
            f"ROW_BOOTSTRAP_BEATS_NB2103_K28_MEDIAN_ONLY_AT_"
        ...
        global_verdict = "ROW_BOOTSTRAP_FLAT_VS_NB2103_K28"
    else:
        global_verdict = (
            "ROW_BOOTSTRAP_DOES_NOT_BEAT_NB2103_K28_GBDT_REMAINS_OPTIMAL"
        )
    ...
    summary = {
        "tag": TAG,
        "method": ("lgbm_mse_row_bootstrap_subsample_sweep_at_K28_on_117col"),
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        "5-way K-tuned matrix as nb2063/nb2081/nb2091/"
                        "nb2103/nb2160"),
        "model_family": "LightGBM_GBDT_row_bootstrap",
        ...
        "per_subsample_records": per_ss_results,
        "best_subsample_mean_bag": best_ss_mean,
        "best_subsample_mean_bag_rae": best_mean_bag,
        "best_subsample_median_bag": best_ss_median,
        "best_subsample_median_bag_rae": best_median_bag,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "best_subsample_mean_bag", "best_subsample_mean_bag_rae",
        "best_subsample_median_bag", "best_subsample_median_bag_rae",
        ...
        print(f"  {k}: {res.get(k)}")
    print("\n==== PER-SUBSAMPLE TABLE ====")
    for r in res["per_subsample_records"]:
        print(f"  ss={r['subsample']:.2f}  "
