"""nb1912 -- CatBoost RMSE on 5-way K-tuned 117-col matrix vs nb1861 (0.5013).

HYPOTHESIS:
    Replace nb1861's LGBM(MSE) residual learner with CatBoost(RMSE),
    a symmetric oblivious-tree booster with built-in ordered-boosting
    bias reduction. On the same 117-col 5-way K-tuned matrix and the
    same chemprop_aux PRE-unblind anchor, CatBoost may extract a
    different residual signal than LGBM and beat nb1861's bagged
    mean RAE of 0.5013 by the 0.003 decision margin.

PROTOCOL:
    1. Features: 117-col 5-way K-tuned (AtomPair K=25 + MACCS K=20 +
       Mordred K=20 + ChempropEmbed K=20 + Avalon K=30 + pred_chembl_pec50
       + mean_sim) -- identical stack to nb1861/nb1902.
    ...
    4. 5-seed bag: seeds = [0, 1, 7, 42, 137]
    ...
    5. mean_bag = mean of 5 corrected OOFs -> rae_mean_bag.
    6. Verdict at 0.003 margin vs nb1861 (rae 0.5013):
         delta < -0.003  -> CATBOOST_WINS_vs_NB1861
         |delta| <= 0.003 -> TIES_NB1861
         delta > +0.003  -> CATBOOST_LOSES_vs_NB1861

Outputs:
    scripts/nb1912_catboost_rmse.py
    data/processed/nb1912_summary.json
    data/processed/nb1912_per_seed_oof.npy   (5, 253) float32
    data/processed/nb1912_mean_bag_oof.npy   (253,)   float32
"""
...
def _catboost_params(seed: int) -> dict:
    """CatBoostRegressor with RMSE loss; user-specified knobs."""
    return dict(
        loss_function="RMSE",
        depth=4,
        n_estimators=200,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(RMSE) 5-seed bag x 5-fold cross-fit on 5-way "
          f"K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          CatBoost: loss=RMSE depth=4 n_est=200 lr=0.05 l2_leaf_reg=5")
    ...
    # ---- nb1861 reference if present (use user-stated 0.5013 as comparator) ----
    ...
    print("\n" + "-" * 78)
    print(f"CATBOOST (RMSE) 5-seed bag x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "CATBOOST_WINS_vs_NB1861"
    elif abs(delta_vs_nb1861) <= DECISION_MARGIN:
        verdict = "TIES_NB1861"
    else:
        verdict = "CATBOOST_LOSES_vs_NB1861"
