"""nb3282 -- Gaussian Process posterior mean on the nb3090 residual.

NEW PARADIGM (vs nb3273 scalar stretch / nb3163 LGBM residual bit):

    Every prior residual-correction on the K18/K20 anchor family has used a
    TREE model (LGBM axis-aligned splits) or a 1-parameter scalar stretch.
    This script swaps the residual model class for a kernel method:
        ...
        correction = GP.posterior_mean(X_K20[va])
        final[va]  = nb3090[va] + correction

    Why a GP is orthogonal to the LGBM residual paradigm (cycle-134 thesis):
      - LGBM partitions feature space into axis-aligned boxes; an RBF GP
        models the residual as a smooth function of EUCLIDEAN distance in
        the K=20 feature space. The inductive bias is global smoothness +
        a learned noise floor (WhiteKernel) rather than greedy split gain.
      - normalize_y centers/scales the residual target so the GP prior mean
        is 0 in standardized space; the WhiteKernel(0.5) regularizes toward
        the prior (shrinks the correction where the residual is unexplained
        by smooth feature distance), which is the natural Bayesian analogue
        of LGBM's reg_lambda / min_child_samples shrinkage.
      - At n=253 the GP posterior mean is a smoothed kernel-ridge fit; if
        the nb3090 residual has any smooth structure in the K=20 features,
        the posterior mean extracts it; if not, normalize_y + WhiteKernel
        collapse the correction toward 0 and we recover nb3090 (safe).

PROTOCOL:
    1. 117-col 5-way feature matrix identical to nb3163 / nb3123 / nb3141
    ...
    3. Anchor = nb3090_pred_oof (253) / te_nb3090 (513). residual = y - nb3090.
    4. HONEST 5-fold scaffold CV over 15 fresh kf_seeds {1216..1230}:
         ...
         per-fold-mean = mean over the 15 kf_seeds of the pooled RAE.
    5. DEPLOY: scaler+GP refit on ALL 253; te = te_nb3090 + GP.predict(te_K20).

GATE:
    per-fold-mean < 0.4423 -> "BETTER"   (must beat nb3090 anchor 0.447 with
                                          margin; same band as nb3273 family)
    else                   -> "FAIL"
...
Outputs:
    data/processed/nb3282_summary.json
    data/processed/nb3282_pred_oof.npy   (253,) float32 -- best-seed (min RAE) OOF
    data/processed/te_nb3282.npy         (513,) float32 -- deploy te
    submissions/nb3282_gp_posterior_residual.csv (BETTER only)
"""
...
# -- Feature cache paths (identical to nb3163) --------------------------------
...
# -- GP kernel hyperparams (per task spec) ------------------------------------
...
# -- ChEMBL kNN params (identical to nb3163 / nb3123 / nb2604) -----------------
...
NB3090_BEST_COMBO_REF = 0.4472  # nb3090 best-combo 15-seed mean
...
# ============================================================================
# feature-matrix helpers (lifted verbatim from nb3163)
# ============================================================================
...
def build_117col_feature_matrix(te_smiles, n_test):
    """117-col matrix identical to nb3163 / nb3123 / nb3141 / nb2604 / nb2960."""
...
def fold_honest_one_kf_seed(
    ...
    """One kf_seed honest 5-fold scaffold CV pass.
    ...
    return pooled, oof, per_fold_rae, corr_std_per_fold
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Gaussian Process posterior mean on {PARENT_TAG} residual")
    print(f"          anchor   : {PARENT_TAG} (OOF RAE {NB3090_ANCHOR_REF})")
    print(f"          GP kernel: RBF(ls={RBF_LENGTH_SCALE}) + "
    ...
    print(f"STEP 2: HONEST 5-fold scaffold CV over {len(KF_SEEDS)} kf_seeds "
          f"(GP posterior mean on nb3090 residual; fold-honest StandardScaler)")
    print("-" * 78)
    per_kf_pooled = []
    per_kf_fold_rae = []
    per_kf_corr_std = []
    ...
    for kf_seed in KF_SEEDS:
        ts = time.time()
        pooled, oof_s, fold_rae, corr_std = fold_honest_one_kf_seed(
    ...
    print("\n" + "-" * 78)
    print("STEP 4: deploy GP refit on ALL 253 -> te_nb3282.npy")
    print("-" * 78)
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_gp_posterior_residual.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "gaussian_process_posterior_mean_on_nb3090_residual",
        "paradigm": "RBF_GP_kernel_residual_correction_K20_features",
        ...
        "ref_nb3090_best_combo": NB3090_BEST_COMBO_REF,
        "ref_nb3173": NB3173_REF,
        "ref_nb2960_K18": NB2960_K18_REF,
        "ref_nb2960_K20": NB2960_K20_REF,
    ...
    print(f"   GP kernel                  = RBF({RBF_LENGTH_SCALE}) + "
