"""nb1213 — morfeus SASA + dispersion-interaction on the DEPLOYED ensemble
(AIMNet2 + MMFF strain + DFT-D4 + DBSTEP steric/shape, best 0.4242).

7 morfeus scalars: SASA area+vol (mf_sasa_area, mf_sasa_vol) and dispersion
interaction index area/vol/pint/pmin/pmax (mf_disp_area, mf_disp_vol,
mf_disp_pint, mf_disp_pmin, mf_disp_pmax -- 5 cols; actually 7 total: sasa×2 +
disp×5). Solvent-accessible surface and London-dispersion surface are distinct
from deployed blocks (electronic charges/AIMNet2, conformer strain, D4 C6/α
polarizability, DBSTEP steric bulk/Sterimol).

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIM + strain + D4 + DBSTEP
  treatment = control + 7 morfeus SASA/disp scalars
...
DBCOLS = ["vbur_r25", "vbur_r35", "vbur_r45", "vbur_r55", "vbur_r65",
          ...
          "radgyr", "inertial_sf"]
MF = "C:/pxr_work/morfeus/morfeus_features.csv"
MFCOLS = ["mf_sasa_area", "mf_sasa_vol", "mf_disp_area", "mf_disp_vol",
          "mf_disp_pint", "mf_disp_pmin", "mf_disp_pmax"]
...
def main():
    ...
    Xmf, mf_nan = aligned_block(MF, MFCOLS, tr["name"])
    print(f"imp: AIM={qm_nan} strain={st_nan} d4={d4_nan} dbstep={db_nan} morfeus={mf_nan}")

    topK = topK_configs(("lgbm", "xgb", "cat", "histgb"))
    print(f"deployed: {[r['arch'] for r in topK]} + CheMeleon + TabPFN + sn_oof + AIM + strain + D4 + DBSTEP")
    ...
    for seed in range(N_SEEDS):
        ...
        scm = StandardScaler().fit(Xmf[trn]); Xmf_std = scm.transform(Xmf)
        Xctrl  = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std])           # deployed best 0.4242
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xmf_std])  # + morfeus SASA/disp
        ...
        err = ytr[ho] - ctrl
        mfmean = Xmf_std[ho].mean(1)
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(mfmean))[0, 1]))
    ...
    n_neg = sum(1 for d in [t-c for t, c in zip(t_raes, c_raes)] if d < 0)
    print(f"\ncontrol  (deployed AIM+strain+D4+DBSTEP)   RAE = {cm:.4f} +/- {np.std(c_raes):.4f}")
    print(f"treatment(deployed + morfeus SASA/disp)    RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           "c_raes": c_raes, "t_raes": t_raes, "n_neg": n_neg, "n_seeds": N_SEEDS,
           "resid_corr_mean": float(np.mean(resid_corrs)),
           "morfeus_imputed": mf_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1213_morfeus_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1213_morfeus_summary.json -> DEPLOY={deploy}")
