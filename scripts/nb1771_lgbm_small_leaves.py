"""nb1771 -- LightGBM (small leaves, min_child_samples=5) on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost MAE d4 on the same 117-col 5-way K-tuned matrix) hit
    rae_mean_bag 0.5163 on chemprop_aux residuals.  The LGBM Huber siblings
    (nb1492 K=30, nb1543 K-tuned) plateaued at 0.5237--0.5260 with default
    LGBM regularization (num_leaves=31, min_child_samples=20).  This script
    pushes LGBM into a much smaller-leaf regime that mirrors CatBoost's
    shallow-tree behavior, isolating fine-grain residual splits on novel
    scaffolds that the default leaf size washes out:
        objective='huber', alpha=1.0
        max_depth=4, num_leaves=15
        n_estimators=300, lr=0.03
        min_child_samples=5  (vs default 20)
        reg_lambda=2.0
    Same 5-way K-tuned 117-col feature stack as nb1554:
...
PROTOCOL:
    ...
    2. Build 117-col feature stack (identical to nb1554).
    3. 5-seed bag LightGBM(huber, max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=5, reg_lambda=2).  KFold(n=5, shuffle=True)
       cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1554 (0.5163).

Outputs:
    scripts/nb1771_lgbm_small_leaves.py
    data/processed/nb1771_summary.json
    data/processed/nb1771_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # CatBoost(MAE, d4, n200, lr0.05, l2=5) on same 117-col stack
...
def main() -> dict:
    ...
    print(f"{TAG} -- LightGBM(huber, alpha=1.0, max_depth=4, num_leaves=15, "
          f"n_est=300, lr=0.03, min_child=5, reg_lambda=2) on 5-way K-tuned "
    ...
    if beats_nb1554:
        verdict = "LGBM_SMALL_LEAVES_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "LGBM_SMALL_LEAVES_FLAT_VS_NB1554"
    elif beats_nb1501:
        verdict = "LGBM_SMALL_LEAVES_BEATS_NB1501_BUT_WORSE_THAN_NB1554"
    elif flat_vs_nb1501:
        verdict = "LGBM_SMALL_LEAVES_FLAT_VS_NB1501"
    elif beats_nb1543:
        verdict = "LGBM_SMALL_LEAVES_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif flat_vs_nb1543:
        verdict = "LGBM_SMALL_LEAVES_FLAT_VS_NB1543"
    elif beats_anchor:
        verdict = "LGBM_SMALL_LEAVES_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_SMALL_LEAVES_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_SMALL_LEAVES_HURTS_ANCHOR"
