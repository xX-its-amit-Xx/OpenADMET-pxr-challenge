"""nb1554 -- CatBoost on K-tuned 5-way 117-col features.

HYPOTHESIS:
    The 4-way K-tuned recipe (nb1543: top-25 AP + top-20 MACCS + top-20 Mord
    + top-30 ChempropEmbed = 97 cols) yielded mean_bag RAE = 0.5237 -- flat
    vs nb1501 baseline (0.5223).  Two missing K-tuning wins:
        - nb1541 chemprop embed best_K = 20  (rae_mean_bag 0.5352, beats
          K=30 default 0.5400 by 0.0049)
        - Avalon-30 family entry adds biologically orthogonal capacity
          (nb1492 5-way LGBM-Huber 0.5260 at K=30 default)
    This script combines five K-tuned families into a 117-col CatBoost(MAE,
    d4, n200, lr0.05, l2=5) feature stack:
        ...
        top-30 Avalon        (nb1392 SHAP ranking, K=30 default)
        pred_chembl_pec50    (ChEMBL PXR kNN-5)
        mean_sim
    Total dim = 25 + 20 + 20 + 20 + 30 + 2 = 117.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack from cached top-K rankings.
    ...
    5. Verdict at 0.003 margin vs nb1550 (5-way K-tuned LGBM-Huber);
       fallback refs nb1543 (0.5237) and nb1501 (0.5223).

Outputs:
    scripts/nb1554_catboost_5way_K_tuned.py
    data/processed/nb1554_summary.json
    data/processed/nb1554_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
# nb1550 is the 5-way K-tuned LGBM-Huber sibling -- predicted ~0.5210 based
# on K-tuning Avalon + chemprop K=20 lift over nb1492 (0.5260).  Use a soft
# estimate; cross-model verdict is anchored to nb1543/nb1501 explicitly.
NB1550_REF_ESTIMATE = 0.5210
...
def main() -> dict:
    ...
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on 5-way K-tuned "
          f"117-col matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1543 ({NB1543_REF:.4f}), nb1550_est ({NB1550_REF_ESTIMATE:.4f})  "
    ...
    # Avalon = nb1392 top-30 SHAP ranking (no K-grid winner anchored on
    # chemprop_aux; reuse the K=30 ranking which is what nb1492 used)
    ...
    pearson_vs_nb1492 = _pearson_vs(DATA_PROCESSED / "nb1492_best_oof.npy")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1543 = {rae_mean_bag - NB1543_REF:+.4f}"
    ...
    if pearson_vs_nb1492 is not None:
        print(f"   Pearson(mean_bag, nb1492_best)   = {pearson_vs_nb1492:.4f}")
    ...
    beats_nb1550_est = rae_mean_bag < NB1550_REF_ESTIMATE - DECISION_MARGIN
    flat_vs_nb1550_est = abs(rae_mean_bag - NB1550_REF_ESTIMATE) < DECISION_MARGIN
    flat_vs_nb1501 = abs(rae_mean_bag - NB1501_REF) < DECISION_MARGIN
    flat_vs_nb1543 = abs(rae_mean_bag - NB1543_REF) < DECISION_MARGIN

    if beats_nb1550_est:
        verdict = "CATBOOST_5WAY_K_TUNED_BEATS_NB1550_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1550_est:
        verdict = "CATBOOST_5WAY_K_TUNED_FLAT_VS_NB1550_EST"
    elif beats_nb1501:
        verdict = "CATBOOST_5WAY_K_TUNED_BEATS_NB1501_BUT_WORSE_THAN_NB1550_EST"
    elif flat_vs_nb1501:
        verdict = "CATBOOST_5WAY_K_TUNED_FLAT_VS_NB1501"
    elif beats_nb1543:
        verdict = "CATBOOST_5WAY_K_TUNED_BEATS_NB1543_BUT_WORSE_THAN_NB1501"
    elif flat_vs_nb1543:
        verdict = "CATBOOST_5WAY_K_TUNED_FLAT_VS_NB1543"
    elif beats_anchor:
        verdict = "CATBOOST_5WAY_K_TUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_5WAY_K_TUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_5WAY_K_TUNED_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "K_source": {
            ...
            "Avalon": "nb1392 SHAP top-30 (K=30 default; no K-grid anchored to chemprop_aux)",
        },
        ...
        "delta_mean_bag_vs_nb1550_est": rae_mean_bag - NB1550_REF_ESTIMATE,
        ...
        "beats_nb1550_est": bool(beats_nb1550_est),
        "flat_vs_nb1501": bool(flat_vs_nb1501),
        "flat_vs_nb1543": bool(flat_vs_nb1543),
        "flat_vs_nb1550_est": bool(flat_vs_nb1550_est),
        ...
        "pearson_vs_nb1492_best": pearson_vs_nb1492,
        ...
        "nb1550_ref_estimate": NB1550_REF_ESTIMATE,
        "nb1550_ref_note": (
            "nb1550 (5-way K-tuned LGBM-Huber) not yet built; estimate "
            "extrapolated from nb1492 K=30 (0.5260) minus expected lift "
            "from chemprop K=20 (~0.0049) and Avalon K-tuning headroom."
        ),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1543",
        "delta_mean_bag_vs_nb1550_est",
        "beats_chemprop_aux",
        "beats_nb1501", "flat_vs_nb1501",
        "beats_nb1543", "flat_vs_nb1543",
        "beats_nb1550_est", "flat_vs_nb1550_est",
        "pearson_vs_anchor",
        "pearson_vs_nb1543_mean_bag",
