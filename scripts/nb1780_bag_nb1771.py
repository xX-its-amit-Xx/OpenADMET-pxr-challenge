"""nb1780 -- Outer-bag VALIDATION of nb1771 (LGBM small leaves).
...
PROTOCOL:
    ...
    Verdict NB1771_REPRODUCES if BoB mean RAE within 0.003 of nb1771 reference
    0.5100 (rae_mean_bag from nb1771_summary.json was 0.50998 ~ 0.5100).

Outputs:
    scripts/nb1780_bag_nb1771.py
    data/processed/nb1780_summary.json
    data/processed/nb1780_bob_mean_oof.npy        (253,) float32
    data/processed/nb1780_bob_median_oof.npy      (253,) float32
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG VALIDATION of nb1771 (LGBM small leaves)")
    ...
    print(f"          NB1771_REF  = {NB1771_REF}  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    print(f"   nb1771_ref            = {NB1771_REF:.4f}  "
          f"(delta per-outer-mean vs ref = "
          f"{rae_per_outer_mean - NB1771_REF:+.4f})")

    # ---- Verdict (per spec): per-outer mean within 0.003 of 0.5100 ----
    delta_vs_ref = rae_per_outer_mean - NB1771_REF
    if abs(delta_vs_ref) <= DECISION_MARGIN:
        verdict = "NB1771_REPRODUCES"
    elif rae_per_outer_mean < NB1771_REF - DECISION_MARGIN:
        verdict = "NB1771_OPTIMISTIC_ON_VALIDATION"
    else:
        verdict = "NB1771_PESSIMISTIC_ON_VALIDATION"
    ...
    summary = {
        ...
        "delta_bob_mean_vs_ref": rae_bob_mean - NB1771_REF,
        "delta_bob_median_vs_ref": rae_bob_median - NB1771_REF,
        "verdict": verdict,
