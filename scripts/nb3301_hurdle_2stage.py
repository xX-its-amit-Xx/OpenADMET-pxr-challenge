"""nb3301 -- HURDLE 2-STAGE: classify active/inactive, then regress within.

NEW PARADIGM (vs the post-hoc-blend / residual-LGBM cycle):
    Every prior move on this anchor refined a *regression* surface (clip,
    stretch, residual-LGBM, SLSQP blend).  This script instead splits the
    problem the way the biology does -- most test compounds are NON-binders
    (flat baseline) and a minority are genuine PXR activators.  A two-stage
    HURDLE model:

      STAGE 1 (classify):  LGBM binary classifier on the full 117-col 5-way
        feature matrix, trained on the 4139 CRC train rows for the label
        (pec50 >= 4.5)  ("above-baseline / active").  Train and the 253
        unblind test rows are InChIKey-disjoint (overlap=0), so the
        classifier probability P_active on the 253 is an HONEST out-of-fold
        signal -- it never saw an unblind label.

      STAGE 2 (regress within):  soft-blend the strong nb3200 regression
        anchor against the train-median (the "inactive" prediction) using
        the stage-1 probability as the gate:

            pred_i = w_i * nb3200_i + (1 - w_i) * train_median
            w_i    = clip(P_active_i, p_floor, 1) ** T

        The sharpening exponent T (and an optional probability floor) is
        chosen PER TRAINING FOLD (fold-honest) from a small grid to
        minimise train-fold RAE.  As T -> large, w_i -> 1 for confidently
        active rows and the operator collapses to pure nb3200 (so the hurdle
        cannot do materially worse than the anchor); as T shrinks, genuinely
        low-P inactive rows are pulled toward the train-median, decompressing
        the over-predicted greasy-novel-inactive tail (the F2 failure mode).

WHY THIS COULD BREAK THE 0.4416 ANCHOR:
    nb3200's residual tail is dominated by novel-scaffold inactives that the
    regression surface over-predicts toward the active band.  A classifier
    trained on 4139 train labels carries an orthogonal "is this even a
    binder" signal that the regression OOF does not, and routing those rows
    to the train-median is a different correction axis than clip/stretch.

HONEST EVAL (matches nb3141 / cycle-160 protocol):
    For each kf_seed in {1216..1230} (15 FRESH seeds):
        ...
        for (tr_loc, va_loc) in splits:
            ...
            oof[va_loc] = blend(P_active[va_loc], nb3200[va_loc], T*, p_floor*)
        pooled_rae[kf_seed] = rae(y_unb, oof)
    PER-FOLD-MEAN reported as mean +/- std over the 15 kf_seeds.

    The stage-1 classifier is fit ONCE on the disjoint 4139 train (bagged
    over 5 seeds); only the cheap stage-2 (T, p_floor) selection is
    fold-honest on the 253.  No unblind label leaks into stage 1.
...
Inputs:
    ...
    data/processed/tr_atompair.npy  (4139,2048)  + te_atompair.npy (513,2048)
    data/processed/tr_maccs.npy     (4139,167)   + te_maccs.npy    (513,167)
    data/processed/tr_avalon512.npy (4139,512)   + te_avalon512.npy(513,512)
    data/processed/tr_chemprop_embed_300.npy (4139,300) + te_chemprop_embed_300.npy
    C:/pxr_artifacts/nb1030/X_mordred_train.npy (4139,1533) + X_mordred_test.npy
...
Outputs:
    data/processed/nb3301_summary.json
    data/processed/nb3301_pred_oof.npy   (253,) float32  honest mean-over-seeds OOF
    data/processed/te_nb3301.npy         (513,) float32  deploy te
    submissions/nb3301_hurdle_2stage.csv (only on BETTER)
"""
...
# -- Anchor (regression surface, stage-2 "active" branch) --------------------
...
# -- ChEMBL PXR kNN feature pair (identical to nb3123/nb3141 117-col build) --
...
# ============================================================================
# ChEMBL kNN helpers (lifted verbatim from nb3141 117-col build)
# ============================================================================
...
def fit_stage1_proba(X_tr_117, y_active, X_unb_117, X_te_117):
    ...
    253 unblind and 513 test. Also returns 5-fold internal train CV AUC and a
    train-OOF P_active (for threshold/calibration diagnostics only)."""
    n_tr = X_tr_117.shape[0]
    p_unb = np.zeros(X_unb_117.shape[0], dtype=np.float64)
    p_te = np.zeros(X_te_117.shape[0], dtype=np.float64)
    # internal 5-fold OOF on train (diagnostic AUC; bagged over seeds)
    from sklearn.model_selection import StratifiedKFold
    oof_p = np.zeros(n_tr, dtype=np.float64)
    for s in CLF_SEEDS:
        # full-train fit -> unblind + test probabilities
...
# ============================================================================
# Stage-2 hurdle blend
# ============================================================================
def _blend(p_active, anchor_pred, train_median, T, p_floor):
    """w = clip(P, p_floor, 1)**T ; pred = w*anchor + (1-w)*train_median."""
    w = np.clip(p_active, p_floor, 1.0) ** T
    return w * anchor_pred + (1.0 - w) * train_median


def _pick_blend(p_tr, anchor_tr, y_tr, train_median):
    """Fold-honest grid search for (T, p_floor) minimising train-fold RAE."""
    best = (np.inf, T_GRID[-1], PFLOOR_GRID[0])
    for T in T_GRID:
        for pf in PFLOOR_GRID:
            pred = _blend(p_tr, anchor_tr, train_median, T, pf)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- HURDLE 2-STAGE (classify active>={ACTIVE_THR}, regress within)")
    print(f"          anchor (active branch) = {ANCHOR_TAG}  (ref OOF {NB3200_ANCHOR_REF:.4f})")
    print(f"          stage-1 clf seeds = {CLF_SEEDS}")
    ...
    print(f"[load] train active rate (y>={ACTIVE_THR}) = {y_active.mean():.3f} "
          f"({int(y_active.sum())}/{n_train})   train_median={train_median:.4f}")
    ...
    # honest unblind classification check vs truth-derived label
    ...
    print(f"   unblind frac P<0.3 = {(p_unb < 0.3).mean():.3f}  "
          f"(candidate inactives routed toward median)")
    ...
    for kf_seed in KF_SEEDS:
        ...
        for tr_loc, va_loc in splits:
            T, pf, _ = _pick_blend(
                p_unb[tr_loc], anchor_oof[tr_loc], y_unb[tr_loc], train_median,
            )
            picked_T.append(T)
            picked_pf.append(pf)
            oof[va_loc] = _blend(
                p_unb[va_loc], anchor_oof[va_loc], train_median, T, pf,
            )
    ...
    # PER-FOLD-MEAN aggregate (the gate statistic, per task spec)
    ...
    # OOF artifact = mean over kf_seeds (each kf covers every row once)
    ...
    print("-" * 78)
    dep_T, dep_pf, dep_rae_tr = _pick_blend(p_unb, anchor_oof, y_unb, train_median)
    te_pred = _blend(p_te, anchor_te, train_median, dep_T, dep_pf).astype(np.float32)
    ...
    if pfm_mean < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3301 hurdle 2-stage per-fold-mean {pfm_mean:.4f} "
            f"< gate {GATE_BETTER:.4f} and beats nb3200 anchor {rae_anchor:.4f} by "
            f"{rae_anchor - pfm_mean:+.4f}. Stage-1 classifier (train AUC {train_auc:.3f}) "
            f"routes low-P inactives to train-median; modal blend (T={T_mode}, "
            f"p_floor={pf_mode}). Verify with deep-30 before PRIMARY promotion."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3301 hurdle 2-stage per-fold-mean {pfm_mean:.4f} >= gate "
            f"{GATE_BETTER:.4f} (delta vs anchor {pfm_mean - rae_anchor:+.4f}). "
            f"Hurdle routing toward train-median (const RAE ~1.0) cannot beat the "
            f"regression surface on a 68%-active unblind; stage-1 P_active not "
            f"sharp enough to isolate the F2 inactive tail without compressing "
            f"actives. Keep nb3200 anchor; no ladder change."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_hurdle_2stage.csv"
    if verdict == "BETTER":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "hurdle_2stage_clf_active_then_softblend_anchor_vs_train_median",
        ...
        "anchor_note": (
            ...
            "stage-1 classifier is PRE-unblind (4139 train, InChIKey-disjoint)."
        ),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "pooled_mean", "rae_anchor_nb3200", "delta_vs_anchor",
