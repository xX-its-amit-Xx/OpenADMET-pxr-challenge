"""nb3172 -- Looser quantile clip (q10/q90) on nb3080 predictions.

NEW PARADIGM:
    Less aggressive tail-cap than nb3161's (q05, q95). Tighten the clip band to
    fold-train (q10, q90), removing more extreme-tail predictions while keeping
    interior preds untouched. No model fit, no per-row blending -- just a
    1-parameter-per-fold floor/ceiling at a looser quantile pair.

    Hypothesis: nb3161 (q05, q95) showed range-clip neutrality/mild gain; if
    nb3080 over-extrapolates in the outer ~10% on each tail, narrower clipping
    at (q10, q90) may yield a slightly larger -RAE move. If the broader (q05,
    q95) move was real, (q10, q90) tightens the cap further and may compound
    the gain (or over-compress and hurt).

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb3080_pred_oof  (253,) -- prior wide-seed blend
    Per outer fold:
        a) lo = quantile(y[fold_train], 0.10)
           hi = quantile(y[fold_train], 0.90)
...
GATE (on 15-seed mean):
    mean < 0.4437 -> "BETTER"
    mean < 0.4475 -> "MARGINAL"  (no worse than nb3080 wide-seed mean)
    else          -> "FAIL"

References:
    nb3080 15-seed wide-mean        = 0.4475 +/- 0.0006  <- parent
    nb3161 (q05, q95) clip          = (run for delta context)
...
Outputs:
    data/processed/nb3172_summary.json
    data/processed/nb3172_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3172.npy         (513,) float32 -- deploy te
    submissions/nb3172_looser_clip.csv   (only on BETTER or MARGINAL verdict)
"""
...
# -- Clip quantiles (FIXED, looser than nb3161 0.05/0.95) ----------------------
Q_LOW = 0.10
Q_HIGH = 0.90

# -- Gates ---------------------------------------------------------------------
GATE_BETTER = 0.4437   # mean < this -> BETTER
GATE_MARGINAL = 0.4475  # mean < this -> MARGINAL (no worse than nb3080)
...
def main() -> dict:
    ...
    print("=" * 78)
    print(
        f"{TAG} -- LOOSER quantile clip per-fold (q{Q_LOW:.2f}, q{Q_HIGH:.2f}) "
        f"on {PARENT_TAG} pred_oof"
    )
    print(
    ...
    # -- Deploy: clip te to (q10, q90) of FULL 253 y --------------------------
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3172 15-seed mean {mean_rae:.4f} beats "
            f"BETTER gate {GATE_BETTER:.4f} and nb3080 parent "
            f"{REF_NB3080:.4f} ({mean_rae - REF_NB3080:+.4f}). Per-fold "
            f"(q{Q_LOW:.2f}, q{Q_HIGH:.2f}) looser y-range clip yields real "
            f"-{abs(mean_rae - REF_NB3080):.4f} RAE on top of nb3080. Consider "
            f"as candidate; re-verify with deep-30 before PRIMARY-1 swap."
        )
    elif mean_rae < GATE_MARGINAL:
        verdict = "MARGINAL"
        ladder_action = (
            f"MARGINAL. nb3172 15-seed mean {mean_rae:.4f} is below MARGINAL "
            f"gate {GATE_MARGINAL:.4f} but does NOT beat BETTER gate "
            f"{GATE_BETTER:.4f}. Move vs nb3080 parent "
            f"{REF_NB3080:.4f} = {mean_rae - REF_NB3080:+.4f}. Looser "
            f"(q{Q_LOW:.2f}, q{Q_HIGH:.2f}) clip is small/no harm; consider "
            f"as a tied-alternate; do not swap PRIMARY-1 without deep-30."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3172 15-seed mean {mean_rae:.4f} exceeds MARGINAL "
            f"gate {GATE_MARGINAL:.4f} and does NOT beat nb3080 parent "
            f"{REF_NB3080:.4f} ({mean_rae - REF_NB3080:+.4f}). Per-fold "
            f"y-range clip at (q{Q_LOW:.2f}, q{Q_HIGH:.2f}) HURTS on this "
            f"anchor (tail-cap over-compresses interior). Keep nb3080 / prior "
            f"PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_looser_clip.csv"
    ...
    summary = {
        ...
        "method": (
            "per_fold_y_range_clip_nb3080_pred_oof_q10_q90"
        ),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_parent", "delta_vs_nb3030",
        "deploy_lo", "deploy_hi",
