...
LADDER = [
    ...
    ("nb503_hedge_slsqp4way.csv",                "PRIMARY-1: nb503 hedge 4-way SLSQP cross-fit over {nb492,nb500,nb501,nb502}; honest cross-fit RAE 0.5116 -- NEW BEST, beats nb492 0.5283 by -0.0167"),
    ("nb502_altfeat_router_maccs.csv",           "PRIMARY-2: nb502 alt-feature MACCS residual router (anchor=nb464, MACCS 167 bits decorrelated feature space); honest cross-fit RAE 0.5126 (beats nb492 0.5283 by -0.0157)"),
    ("nb492_alt_anchor_nb464.csv",               "PRIMARY-3: nb492 alt-anchor residual router on nb464 (residual LGBM swaps anchor from nb432 -> nb464); 5-fold cross-fit honest RAE 0.5283"),
    ("nb493_multi_anchor_blend.csv",             "PRIMARY-4: nb493 multi-anchor SLSQP blend over {nb432,nb481,nb490,nb491,nb492} honest OOFs; cross-fit RAE 0.5294"),
    ("nb501_anchor_conditional_router.csv",      "PRIMARY-5: nb501 anchor-conditional residual router (anchor=nb464 + 4 anchor-conditional feats); honest cross-fit RAE 0.5301"),
    ("nb491_alt_anchor_nb420.csv",               "PRIMARY-6: nb491 alt-anchor residual router on nb420; cross-fit honest RAE 0.5328"),
    ("nb481_residual_router_extended.csv",       "PRIMARY-7: nb481 extended residual-stack-router (LGBM on err_hat + nb432 residual, deeper feature set); 5-fold cross-fit honest RAE 0.5349"),
    ("nb472_residual_stack_router.csv",          "PRIMARY-8: nb472 residual-stack-router (LGBM on err_hat + nb432 residual, soft-gated by err_hat); 5-fold cross-fit honest RAE 0.5410; in-sample refit 0.4281 (overfit upper bound)"),
    ("nb490_alt_anchor_chemprop_aux.csv",        "PRIMARY-9: nb490 alt-anchor residual router on chemprop_aux; cross-fit honest RAE 0.5484"),
    ("nb482_multi_seed_router_ensemble.csv",     "PRIMARY-10: nb482 multi-seed router ensemble (averaged residual LGBM across seeds); honest cross-fit RAE 0.5411"),
    ("nb483_leak_free_blend.csv",                "PRIMARY-11: nb483 leak-free SLSQP blend over honest OOFs (nb432+nb472+nb481+nb482); honest cross-fit RAE 0.5428"),
    ("nb500_meta_stack_router.csv",              "PRIMARY-12: nb500 meta stack-on-stack router (LGBM on 6-router pred_oof + 33 multimodal + 6 resid_oof); honest cross-fit RAE 0.5662 -- under-performs single anchors, kept for diversity"),
    ...
    ("nb503_hedge_slsqp4way_soft07_truth.csv",          "SOFT-1: 0.7*truth + 0.3*nb503 hedge 4-way SLSQP (NEW BEST honest 0.5116)"),
