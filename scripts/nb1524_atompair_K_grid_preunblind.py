"""nb1524 -- Fresh SHAP K-grid on PRE-unblind path.

Anchor: chemprop_aux (PRE-unblind, te[unb_idx] in_RAE ~ 0.6216).
Residual learner: shallow LGBM Huber on top-K SHAP-ranked AtomPair bits
+ pred_chembl + mean_sim.  Sweep K in {10, 20, 25, 30, 35, 40, 50}, with
5-seed bag (seeds [0, 1, 7, 42, 137]) and KFold(5) cross-fit per K, per seed.

Verdict at 0.003 margin vs nb1460 K=30 mean-bag RAE 0.5550.

Outputs:
    scripts/nb1524_atompair_K_grid_preunblind.py
    data/processed/nb1524_summary.json
    data/processed/nb1524_best_K_oof.npy        (253,) float32 mean-bag at best K
"""
...
NB1460_K30_REF = 0.5550
DECISION_MARGIN = 0.003

K_GRID = [10, 20, 25, 30, 35, 40, 50]
...
def main() -> dict:
    ...
    for K in K_GRID:
        ...
        beats_nb1460 = rae_mean_bag < NB1460_K30_REF - DECISION_MARGIN
        ...
        k_records.append({
            ...
            "delta_mean_bag_vs_nb1460_K30": rae_mean_bag - NB1460_K30_REF,
            "beats_anchor": bool(beats_anchor),
            "beats_nb1460_K30": bool(beats_nb1460),
        })
        ...
        print(f"   K={K:3d}  feat={feat_dim_pruned:3d}  "
              ...
              f"d_vs_nb1460={rae_mean_bag - NB1460_K30_REF:+.4f}")
    ...
    beats_nb1460 = best_rae < NB1460_K30_REF - DECISION_MARGIN

    if beats_nb1460:
        verdict = f"K{best_K}_BEATS_NB1460_K30_NEW_PRE_UNBLIND_CANDIDATE"
    elif beats_anchor and abs(best_rae - NB1460_K30_REF) < DECISION_MARGIN:
        verdict = f"K{best_K}_FLAT_VS_NB1460_K30_TIE_AT_MARGIN"
    elif beats_anchor:
        verdict = f"K{best_K}_BEATS_ANCHOR_BUT_WORSE_THAN_NB1460_K30"
    ...
    print(f"   nb1460 K=30 ref        = {NB1460_K30_REF:.4f}")
    print(f"   d_vs_anchor            = {best_rae - rae_anchor:+.4f}")
    print(f"   d_vs_nb1460_K30        = {best_rae - NB1460_K30_REF:+.4f}")
    print(f"   beats_anchor           = {beats_anchor}")
    print(f"   beats_nb1460_K30       = {beats_nb1460}")
    ...
    summary = {
        ...
        "best_delta_vs_nb1460_K30": best_rae - NB1460_K30_REF,
        "best_beats_anchor": bool(beats_anchor),
        "best_beats_nb1460_K30": bool(beats_nb1460),
        "verdict": verdict,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb1460_K30_ref": NB1460_K30_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "best_delta_vs_anchor", "best_delta_vs_nb1460_K30",
        "best_beats_anchor", "best_beats_nb1460_K30",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
    print("\n  K-grid table (K, mean_bag, median_bag, d_vs_nb1460):")
    for rec in res["k_records"]:
        print(f"    K={rec['K']:3d}  mean_bag={rec['rae_mean_bag']:.4f}  "
              f"median_bag={rec['rae_median_bag']:.4f}  "
              f"d_vs_nb1460={rec['delta_mean_bag_vs_nb1460_K30']:+.4f}")
