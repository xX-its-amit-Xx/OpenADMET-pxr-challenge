"""nb1633 -- CatBoost on 6-way K-tuned 137-col features (5-way + ChemBERTa K=20).

HYPOTHESIS:
    nb1620 used ChemBERTa top-50 dims appended to the 5-way K-tuned 117-col
    stack (167 cols total).  This script tests the SMALLER ChemBERTa dose
    K=20 (the nb1611 standalone best_K), giving a tighter 137-col matrix.
    The motivation: depth-4 CatBoost with l2_leaf_reg=5 may benefit from
    fewer noisy ChemBERTa dims, since nb1611 found best_K=20 standalone
    suggests ChemBERTa carries useful signal only in the top-20 SHAP
    ranking.  A leaner 137-col matrix may avoid spurious-split risk and
    extract orthogonal capacity more cleanly than the 167-col 6-way add.

PROTOCOL:
    1. Build features (137 cols):
         ...
         top-20 ChemBERTa     (nb1611 best_K)
         pred_chembl_pec50    (ChEMBL PXR kNN-5)
         mean_sim
    2. Anchor = chemprop_aux te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    3. 5-seed bag CatBoost(MAE, d4, n200, lr0.05, l2=5).
       KFold(n=5, shuffle=True, random_state=seed) cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1554 (0.5163) and nb1561 BoB mean (0.5155).

Outputs:
    scripts/nb1633_catboost_6way_chemberta_K20.py (this file)
    data/processed/nb1633_summary.json
    data/processed/nb1633_mean_bag_oof.npy        (253,) float32
"""
...
# ChemBERTa K (fixed per protocol -- top 20 by nb1611 SHAP ranking)
K_CHEMBERTA = 20
...
NB1620_REF = None  # filled at runtime if available
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1492/nb1543/nb1554/nb1620."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE,d4,n200,lr0.05,l2=5) on 6-way K-tuned "
          f"137-col matrix (5-way + ChemBERTa top-{K_CHEMBERTA})")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1554 ({NB1554_REF:.4f}), nb1561_BoB ({NB1561_BOB_REF:.4f})  "
    ...
    # ChemBERTa = nb1611 top-20 dims by SHAP ranking (smaller dose vs nb1620's 50)
    ...
    # ---- Build COMBINED 6-way K-tuned feature matrix (137 cols) ----
    ...
    pearson_vs_nb1561_bob = _pearson_vs(
        DATA_PROCESSED / "nb1561_bob_mean_oof.npy"
    )
    ...
    pearson_vs_nb1620 = _pearson_vs(
        DATA_PROCESSED / "nb1620_mean_bag_oof.npy"
    )

    # Optional reference: nb1620 mean_bag RAE if summary available
    nb1620_ref = None
    nb1620_summary_path = DATA_PROCESSED / "nb1620_summary.json"
    if nb1620_summary_path.exists():
        try:
            with open(nb1620_summary_path) as f:
                _nb1620_sum = json.load(f)
            nb1620_ref = float(_nb1620_sum.get("rae_mean_bag", None))
        except Exception:
            nb1620_ref = None
    ...
    if nb1620_ref is not None:
        print(f"   d_vs_nb1620            = {rae_mean_bag - nb1620_ref:+.4f}  "
              f"(nb1620 ref = {nb1620_ref:.4f})")
    ...
    if pearson_vs_nb1561_bob is not None:
        print(f"   Pearson(mean_bag, nb1561_bob_mean)   = "
              f"{pearson_vs_nb1561_bob:.4f}")
    if pearson_vs_nb1543 is not None:
        print(f"   Pearson(mean_bag, nb1543_mean_bag)   = "
              f"{pearson_vs_nb1543:.4f}")
    ...
    if beats_nb1561_bob:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_K20_BEATS_NB1561_BOB_"
                   "NEW_PRE_UNBLIND_PRIMARY")
    elif flat_vs_nb1561_bob:
        verdict = "CATBOOST_6WAY_CHEMBERTA_K20_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_K20_BEATS_NB1554_BUT_"
                   "WORSE_THAN_NB1561_BOB")
    elif flat_vs_nb1554:
        verdict = "CATBOOST_6WAY_CHEMBERTA_K20_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_K20_BEATS_ANCHOR_BUT_"
                   "WORSE_THAN_NB1554")
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_6WAY_CHEMBERTA_K20_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_6WAY_CHEMBERTA_K20_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "K_source": {
            ...
            "ChemBERTa": ("nb1611 SHAP top-20 (smaller dose vs nb1620's "
                          "top-50; matches nb1611 standalone best_K)"),
        },
        ...
        "delta_mean_bag_vs_nb1620": (
            (rae_mean_bag - nb1620_ref) if nb1620_ref is not None else None
        ),
        "nb1620_ref": nb1620_ref,
        ...
        "pearson_vs_nb1561_bob_mean": pearson_vs_nb1561_bob,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "pearson_vs_nb1561_bob_mean",
        "pearson_vs_nb1611_best_K",
