"""nb1894 -- LGBM regression (MSE) with HEAVIER regularization on the 117-col
5-way K-tuned matrix; chemprop_aux PRE-unblind anchor; 5-seed bag, 5-fold
cross-fit. Continues the nb1852/nb1861 regularization-axis exploration.

HYPOTHESIS:
    nb1861 (reg_lambda=2.0 only) pooled-25-bag RAE = 0.5013 on the 253-unblind
    set. Push regularization further:
      * reg_lambda=5.0       (5x L2 vs nb1861)
      * reg_alpha=1.0        (NEW: L1 was 0 in nb1861)
      * min_split_gain=0.05  (NEW: gain floor; previously default 0.0)
    If the model is over-fitting per-tree splits, these knobs should tighten
    OOF; if it's under-fitting, RAE goes up.

PROTOCOL:
    1. Features = 117-col 5-way K-tuned (AtomPair + MACCS + Mordred +
       ChempropEmbed + Avalon + ChEMBL kNN pred + ChEMBL mean sim)
       identical pipeline as nb1861.
    2. Anchor = chemprop_aux te[unb_idx] (PRE-unblind clean).
       residual = y_unb - anchor.
    3. LGBM(objective='regression', max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=5, reg_lambda=5, reg_alpha=1,
       min_split_gain=0.05).
...
Outputs:
    scripts/nb1894_lgbm_mse_heavy_reg.py
    data/processed/nb1894_summary.json
    data/processed/nb1894_per_seed_oof.npy   (5, 253) float32
    data/processed/nb1894_mean_bag_oof.npy   (253,)   float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- HEAVY regularization variant.

    Knobs differing from nb1861:
      reg_lambda  : 2.0 -> 5.0
      reg_alpha   : 0.0 -> 1.0
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) HEAVY-REG on 117-col 5-way K-tuned matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          knobs : reg_lambda=5  reg_alpha=1  min_split_gain=0.05")
