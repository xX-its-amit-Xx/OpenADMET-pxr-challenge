"""nb1503 -- Deploy nb1484 SLSQP-weighted variant (cross-fit RAE 0.5231) to a
513-row submission CSV.

Anchor    : te_chemprop_aux.npy (513,) -- PRE-unblind.
Residuals : four SHAP-pruned families (top-K indices loaded from nb1484):
              A AtomPair-30     + ChEMBL  (32 cols)
              B MACCS-20        + ChEMBL  (22 cols)
              C Mordred-30      + ChEMBL  (32 cols)
              D ChempropEmbed-30 + ChEMBL (32 cols)
            Each: 5-seed bag shallow LGBM Huber trained on ALL 253 unblind
            with target = y_unb - chemprop_aux[unb_idx]; predict 513.
Blend     : te_nb1503 = chemprop_aux
                + (w_AP*r_AP + w_MACCS*r_MACCS + w_Mord*r_Mord + w_embed*r_embed)
            with w = mean of 5 SLSQP fold-weights from nb1484:
                [0.2301, ~0, 0.3330, 0.4370]  (sums to 1.0)

Honest LB anchor: nb1484 slsqp 5-fold cross-fit RAE = 0.5231
Predicted LB    : 0.5231 + 0.003 = 0.526
"""
...
SUB_OUT = SUB_DIR / f"{TAG}_deploy_nb1484_slsqp.csv"
...
def _load_family_te(family: str, n_test: int) -> np.ndarray:
    if family == "AtomPair":
        ...
        # NaN-impute via column median (mirror nb1484)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Deploy nb1484 SLSQP-weighted variant to 513 CSV")
    ...
    # ---- nb1484 SHAP-pruned indices + SLSQP weights per family ----
    with open(NB1484_SUMMARY) as f:
        nb1484 = json.load(f)
    fam_top_idx = {}
    fam_crossfit_rae = {}
    for fam_entry in nb1484["families"]:
        fam_top_idx[fam_entry["family"]] = np.asarray(
            fam_entry["top_idx_ranked"], dtype=int
        )
        fam_crossfit_rae[fam_entry["family"]] = float(fam_entry["rae_mean_bag"])
    ...
    # SLSQP mean-over-folds weights ordered as nb1484 families_order
    families_order = nb1484["families_order"]  # ["AtomPair","MACCS","Mordred","ChempropEmbed"]
    w_mean = np.asarray(nb1484["slsqp_w_mean_over_folds"], dtype=np.float64)
    w_map = {f: float(w_mean[i]) for i, f in enumerate(families_order)}
    w_sum = float(w_mean.sum())
    print(f"[slsqp] families_order = {families_order}")
    print(f"[slsqp] w_mean_over_folds = {[round(x,4) for x in w_mean.tolist()]} "
          f"(sum={w_sum:.4f})")
    for f in FAMILIES:
        print(f"[slsqp] w[{f}] = {w_map[f]:.4f}")
    ...
    for family in FAMILIES:
        ...
        residual_deploys[family] = mean_bag_resid_513
        ...
        per_family_diag[family] = {
            ...
            "slsqp_w": w_map[family],
        }
        print(f"[diag][{family}] te_family in_RAE on unblind = "
              f"{per_family_diag[family]['in_RAE_unb_deploy_in_sample']:.4f}")

    # ---- SLSQP-weighted blend (sum w_i * r_i) ----
    print("\n" + "=" * 78)
    print("SLSQP-WEIGHTED BLEND ACROSS 4 FAMILY RESIDUALS")
    print("  weights (mean-over-5-folds):  "
          f"{[f'{w_map[f]:.4f}' for f in FAMILIES]}")
    print("=" * 78)
    mean_resid_513 = np.zeros(n_test, dtype=np.float64)
    for family in FAMILIES:
        mean_resid_513 += w_map[family] * residual_deploys[family]
    print(f"[blend] sum_w = {sum(w_map[f] for f in FAMILIES):.4f}")
    print(f"[blend] weighted_resid_513: mean={mean_resid_513.mean():+.4f}  "
          f"std={mean_resid_513.std():.4f}")

    te_nb1503 = te_anchor_513 + mean_resid_513
    print(f"[te]  te_{TAG}: mean={te_nb1503.mean():.4f}  "
          f"std={te_nb1503.std():.4f}  "
          f"min={te_nb1503.min():.4f}  max={te_nb1503.max():.4f}")

    in_RAE_unb = float(rae(y_unb, te_nb1503[unb_idx]))
    naive_crossfit_RAE = float(nb1484["rae_naive_mean_blend"])
    slsqp_crossfit_RAE = float(nb1484["rae_slsqp_crossfit"])
    print(f"[in]  in_RAE on unblind (deploy-on-all, in-sample) = "
          f"{in_RAE_unb:.4f}   "
          f"(cross-fit naive 1/4 = {naive_crossfit_RAE:.4f}, "
          f"slsqp = {slsqp_crossfit_RAE:.4f})")

    # ---- Save te + submission CSV ----
    np.save(TE_OUT, te_nb1503.astype(np.float32))
    print(f"[save] {TE_OUT}")
    ...
    out = pd.DataFrame({
        ...
        "pEC50": te_nb1503.astype(np.float64),
    })
    if len(out) != n_test:
    ...
    summary = {
        ...
        "deploy_of": "nb1484",
        "blend_variant": "slsqp_5fold_mean_weights",
        "seeds": SEEDS,
        "n_test": n_test,
        "n_unb": n_unb,
        "rae_anchor_chemprop_aux_in_RAE_unb": rae_anchor,
        "per_family_top_k": {
            f: int(len(fam_top_idx[f])) for f in FAMILIES
        },
        "slsqp_w_mean_over_folds": {f: w_map[f] for f in FAMILIES},
        "slsqp_w_sum": float(sum(w_map[f] for f in FAMILIES)),
        "per_family_diag": per_family_diag,
        "per_family_crossfit_RAE_from_nb1484": fam_crossfit_rae,
        "te_mean": float(te_nb1503.mean()),
        "te_std": float(te_nb1503.std()),
        "te_min": float(te_nb1503.min()),
        "te_max": float(te_nb1503.max()),
        "in_RAE_unb_deploy_in_sample": in_RAE_unb,
        "honest_crossfit_RAE_nb1484_naive": naive_crossfit_RAE,
        "honest_crossfit_RAE_nb1484_slsqp": slsqp_crossfit_RAE,
        "predicted_LB": slsqp_crossfit_RAE + 0.003,
        "te_path": str(TE_OUT),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "slsqp_w_sum",
        "per_family_diag",
        "per_family_crossfit_RAE_from_nb1484",
