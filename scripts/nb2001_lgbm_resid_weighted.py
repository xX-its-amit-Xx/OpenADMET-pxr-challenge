"""nb2001 -- LGBM(MSE) with class_weight-balanced-style sample weights on residual learner.

HYPOTHESIS:
    nb1861 (LGBM MSE, 5-seed bag x 5-fold cross-fit) achieved BoB MEAN
    RAE ~0.5013 on the residual atop chemprop_aux (PRE-unblind anchor,
    in_RAE 0.6216).  All knobs fixed identical to nb1852/nb1771.

    nb2001 probes a 'class_weight=balanced' analog for regression:
    sample_weight = 1 / |residual|, clipped to [0.1, 10].  This
    up-weights small-residual rows (easy cases the anchor already nails)
    and down-weights large-residual rows (hard outliers).  It is the
    OPPOSITE of huber's bias toward outliers and a 'cushion' against the
    LGBM(MSE) tendency to over-fit a handful of large-residual rows.

    Configuration (same shape as nb1852 except sample_weight):
        objective='regression'        (MSE)
        max_depth=4, num_leaves=15
        n_estimators=300, lr=0.03
        min_child_samples=5
        reg_lambda=2.0
        sample_weight = clip(1 / |residual|, 0.1, 10)
    Same 5-way K-tuned 117-col feature stack:
...
PROTOCOL:
    ...
    2. Build 117-col feature stack (identical to nb1852/nb1861).
    3. sample_weight = clip(1 / max(|residual|, 1e-3), 0.1, 10).
    4. 5-seed bag LightGBM(regression/MSE, max_depth=4, num_leaves=15,
       n_est=300, lr=0.03, min_child_samples=5, reg_lambda=2),
       fit with sample_weight on training fold only.
...
Outputs:
    scripts/nb2001_lgbm_resid_weighted.py
    data/processed/nb2001_summary.json
    data/processed/nb2001_mean_bag_oof.npy        (253,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- identical to nb1852/nb1861."""
    return dict(
...
def _sample_weights(residual: np.ndarray) -> np.ndarray:
    """class_weight=balanced analog for regression:
       ...
       Small-residual rows (already correct) get HIGH weight (cushion);
       large-residual rows (outliers) get LOW weight (down-weight)."""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM(MSE) + sample_weight=1/|resid| clipped to "
          f"[{SW_CLIP_LO}, {SW_CLIP_HI}] on 117-col 5-way K-tuned matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1852 MSE ({NB1852_REF:.4f}), nb1861 BoB ({NB1861_REF:.4f})  "
    ...
    # ---- Per-seed LGBM MSE residual cross-fit (with sample_weight) ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM (MSE, sample_weight=1/|resid| clipped to "
          f"[{SW_CLIP_LO},{SW_CLIP_HI}]) RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        resid_oof_s = _residual_cross_fit_one_seed(
            X_unb, residual, sample_weight, s
        )
    ...
    pearson_vs_nb1861 = _pearson_vs(DATA_PROCESSED / "nb1861_pooled_25bag_oof.npy")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1852 = {rae_mean_bag - NB1852_REF:+.4f}"
          f"  d_vs_nb1861 = {rae_mean_bag - NB1861_REF:+.4f})")
    ...
    if pearson_vs_nb1852 is not None:
        print(f"   Pearson(mean_bag, nb1852_mean)   = {pearson_vs_nb1852:.4f}")
    if pearson_vs_nb1861 is not None:
        print(f"   Pearson(mean_bag, nb1861_pooled) = {pearson_vs_nb1861:.4f}")
    if pearson_vs_nb1771 is not None:
        print(f"   Pearson(mean_bag, nb1771_mean)   = {pearson_vs_nb1771:.4f}")
    if pearson_vs_nb1554 is not None:
        print(f"   Pearson(mean_bag, nb1554_mean)   = {pearson_vs_nb1554:.4f}")

    # ---- Verdict (vs nb1861 BoB mean -- direct ablation target) ----
    beats_anchor = rae_mean_bag < rae_anchor - DECISION_MARGIN
    beats_nb1554 = rae_mean_bag < NB1554_REF - DECISION_MARGIN
    beats_nb1852 = rae_mean_bag < NB1852_REF - DECISION_MARGIN
    beats_nb1861 = rae_mean_bag < NB1861_REF - DECISION_MARGIN
    flat_vs_nb1861 = abs(rae_mean_bag - NB1861_REF) < DECISION_MARGIN
    flat_vs_nb1852 = abs(rae_mean_bag - NB1852_REF) < DECISION_MARGIN

    if beats_nb1861:
        verdict = "WEIGHTED_LGBM_BEATS_NB1861_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1861:
        verdict = "WEIGHTED_LGBM_FLAT_VS_NB1861"
    elif beats_nb1852:
        verdict = "WEIGHTED_LGBM_BEATS_NB1852_BUT_WORSE_THAN_NB1861"
    elif flat_vs_nb1852:
        verdict = "WEIGHTED_LGBM_FLAT_VS_NB1852_WORSE_THAN_NB1861"
    elif beats_nb1554:
        verdict = "WEIGHTED_LGBM_BEATS_NB1554_BUT_WORSE_THAN_NB1852"
    elif beats_anchor:
        verdict = "WEIGHTED_LGBM_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "WEIGHTED_LGBM_FLAT_VS_ANCHOR"
    else:
        verdict = "WEIGHTED_LGBM_HURTS_ANCHOR"
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "beats_chemprop_aux",
        "beats_nb1554", "beats_nb1852",
        "beats_nb1861", "flat_vs_nb1861",
        "pearson_vs_anchor",
        "pearson_vs_nb1861_pooled",
