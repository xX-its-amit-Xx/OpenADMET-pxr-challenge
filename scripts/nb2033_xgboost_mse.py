"""nb2033 -- XGBoost MSE on 117-col 5-way K-tuned matrix.

HYPOTHESIS:
    nb1861 / nb1852 used LightGBM MSE on the 117-col K-tuned feature stack
    against chemprop_aux anchor. This notebook swaps the model to XGBRegressor
    (objective='reg:squarederror', max_depth=4, n_estimators=300,
    learning_rate=0.03, reg_lambda=2) with a 5-seed bag and 5-fold cross-fit
    on the unblind residual.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1852/nb1861).
    3. 5-seed bag: seeds {0, 1, 7, 42, 137}; for each seed do KFold(n_splits=5,
       shuffle=True, random_state=seed) residual cross-fit using
       XGBRegressor(objective='reg:squarederror', max_depth=4, n_estimators=300,
                    learning_rate=0.03, reg_lambda=2.0).
       mean_bag_oof = mean across 5 seeds; mean_bag_pred = anchor + mean_bag_oof.
       rae_mean_bag = rae(y_unb, mean_bag_pred).
    4. Per-seed RAE + mean-bag RAE reported.
    5. Verdict vs nb1861.

Outputs:
    scripts/nb2033_xgboost_mse.py
    data/processed/nb2033_summary.json
    data/processed/nb2033_per_seed_oof.npy   (5, 253) float32
    data/processed/nb2033_mean_bag_oof.npy   (253,)   float32
"""
...
NB1852_REF_FALLBACK = 0.5062
NB1861_REF_FALLBACK = 0.5062
...
def _xgb_params(seed: int) -> dict:
    """XGBRegressor(reg:squarederror) -- per nb2033 spec."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- XGBoost(reg:squarederror) on 5-way K-tuned 117-col matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          bag seeds = {BAG_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          xgb: max_depth=4, n_estimators=300, lr=0.03, reg_lambda=2")
    print("=" * 78)

    # ---- nb1861 / nb1852 reference ----
    ...
    if NB1852_SUMMARY.exists():
        with open(NB1852_SUMMARY) as f:
            nb1852_sum = json.load(f)
        nb1852_ref = float(nb1852_sum.get("rae_mean_bag", NB1852_REF_FALLBACK))
        print(f"[ref] nb1852_summary.rae_mean_bag = {nb1852_ref:.4f}")
    else:
        nb1852_ref = NB1852_REF_FALLBACK
        print(f"[ref] nb1852_summary.json missing -- using fallback "
              f"{nb1852_ref:.4f}")
    ...
    # ---- 5-SEED BAG LOOP ----
    print("\n" + "-" * 78)
    print(f"5-SEED BAG: seeds={BAG_SEEDS} x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})  model=XGBRegressor(reg:squarederror)")
    print("-" * 78)
    ...
    for s_i, seed in enumerate(BAG_SEEDS):
        t_s = time.time()
        resid_oof_i = _residual_cross_fit_one_seed(X_unb, residual, seed)
        pred_corr_i = anchor + resid_oof_i
        per_seed_oof[s_i] = pred_corr_i
        ...
        print(f"   seed={seed:4d}  rae={r_i:.4f}  wall={time.time() - t_s:.1f}s")
    ...
    if abs(delta_vs_nb1861) < DECISION_MARGIN:
        verdict = "XGBOOST_TIES_LGBM_nb1861"
    elif delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "XGBOOST_BEATS_LGBM_nb1861"
    else:
        verdict = "XGBOOST_WORSE_THAN_LGBM_nb1861"
