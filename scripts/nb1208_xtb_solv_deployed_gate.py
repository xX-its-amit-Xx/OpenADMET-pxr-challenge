"""nb1208 — GFN2-xTB ALPB-water SOLVATION descriptors on the DEPLOYED
ensemble config (AIMNet2 + MMFF strain + DFT-D4 + DBSTEP, best 0.4242).

8 solvation-polarization scalars (gas->ALPB-water single-point deltas):
  solv_dG_kcal      implicit-solvation free energy (ALPB)
  solv_dE_per_heavy dGsolv normalized per heavy atom
  solv_ddip         solvent-induced dipole polarization
  solv_dgap         solvent-induced HOMO-LUMO gap shift
  solv_dqabs        solvent-induced atomic-charge polarization
  dip_solv          solvated dipole
  gap_solv          solvated gap
  qabs_solv         solvated mean |q|
SOLVATION / hydrophobic-DESOLVATION is the genuinely-untested observable:
every deployed physics block is GAS-PHASE (AIMNet2 charges/forces, strain
energetics, D4 dispersion-polarizability, DBSTEP steric/shape) -- none encodes
the cost of stripping water on transfer into PXR's large HYDROPHOBIC pocket,
which is mechanistically central to xenobiotic activation.

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + D4 + DBSTEP
  treatment = control + 8 xTB-ALPB solvation scalars
...
DBCOLS = ["vbur_r25", "vbur_r35", "vbur_r45", "vbur_r55", "vbur_r65",
          ...
          "radgyr", "inertial_sf"]
SV = "C:/pxr_work/xtb/solv_features_all.csv"
SVCOLS = ["solv_dG_kcal", "solv_dE_per_heavy", "solv_ddip", "solv_dgap",
          "solv_dqabs", "dip_solv", "gap_solv", "qabs_solv"]
...
def aligned_block(csv_path, cols, tr_names, clip=False):
    ...
    if clip:  # winsorize extreme tails (xTB has occasional charged-species blowups)
        lo = np.nanpercentile(X, 0.5, axis=0); hi = np.nanpercentile(X, 99.5, axis=0)
        X = np.clip(X, lo, hi)
    return X, nan_rows
...
def main():
    ...
    Xsv, sv_nan = aligned_block(SV, SVCOLS, tr["name"], clip=True)
    print(f"AIMNet2 imp={qm_nan} strain imp={st_nan} d4 imp={d4_nan} dbstep imp={db_nan} solv imp={sv_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        scv = StandardScaler().fit(Xsv[trn]); Xsv_std = scv.transform(Xsv)
        Xctrl = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std])              # deployed 0.4242
        Xtreat = np.hstack([Xqm_std, Xst_std, Xd4_std, Xdb_std, Xsv_std])   # + solvation
        ...
        err = ytr[ho] - ctrl
        svmean = Xsv_std[ho].mean(1)
        resid_corrs.append(float(np.corrcoef(np.abs(err), np.abs(svmean))[0, 1]))
    ...
    print(f"treatment(deployed + xTB-ALPB solvation)      RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    print(f"MATCHED delta = {delta:+.4f}   deployed best = {prev}")
    n_neg = int(sum(1 for cc, tt in zip(c_raes, t_raes) if tt < cc))
    deploy = bool(delta < -0.001 and tm < prev)
    out = {"control_rae": cm, "treatment_rae": tm, "matched_delta": delta,
           "c_raes": c_raes, "t_raes": t_raes, "n_neg_seeds": n_neg,
           "resid_corr_mean": float(np.mean(resid_corrs)),
           "solv_imputed": sv_nan, "deployed_best": prev, "deploy": deploy}
    json.dump(out, open(f"{P}/nb1208_xtb_solv_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1208_xtb_solv_summary.json -> DEPLOY={deploy} ({n_neg}/{N_SEEDS} neg seeds)")
