"""nb1902 -- Extra Trees MSE on 5-way K-tuned 117-col matrix.

HYPOTHESIS:
    Replace nb1852/nb1861's LGBM(MSE) residual learner with
    ExtraTreesRegressor(MSE) -- a randomized-split, fully-grown ensemble that
    decorrelates trees more aggressively than LGBM boosting. On the 117-col
    5-way K-tuned matrix with the chemprop_aux PRE-unblind anchor, ExtraTrees
    may pick up a different residual signal than LGBM.

PROTOCOL:
    1. Features: 117-col 5-way K-tuned (AtomPair K=25 + MACCS K=20 +
       Mordred K=20 + ChempropEmbed K=20 + Avalon K=30 + pred_chembl_pec50 +
       mean_sim) -- identical stack to nb1861.
    ...
    6. Verdict at 0.003 margin vs nb1861 (rae 0.5013):
         delta < -0.003  -> ET_WINS_vs_NB1861
         |delta| <= 0.003 -> TIES_NB1861
         delta > +0.003  -> ET_LOSES_vs_NB1861

Outputs:
    scripts/nb1902_extra_trees_mse.py
    data/processed/nb1902_summary.json
    data/processed/nb1902_per_seed_oof.npy   (5, 253) float32
    data/processed/nb1902_mean_bag_oof.npy   (253,)   float32
"""
...
def _et_params(seed: int) -> dict:
    """ExtraTreesRegressor -- MSE / squared_error criterion."""
    return dict(
        n_estimators=300,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- ExtraTrees(MSE) 5-seed bag x 5-fold cross-fit on 5-way "
          f"K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          ET: n_est=300 max_depth=6 max_features=0.5 "
          f"min_samples_leaf=5 n_jobs=-1")
    ...
    print("\n" + "-" * 78)
    print(f"EXTRA TREES (MSE) 5-seed bag x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "ET_WINS_vs_NB1861"
    elif abs(delta_vs_nb1861) <= DECISION_MARGIN:
        verdict = "TIES_NB1861"
    else:
        verdict = "ET_LOSES_vs_NB1861"
