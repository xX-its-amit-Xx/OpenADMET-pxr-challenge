"""nb1168_gate — honest ensemble gate for pooled sister-NR 3-head GNN vs the DEPLOYED octant-main-head GNN.
Mirrors nb1167_gate; treatment = sn_oof (deployed octant main-head + sisterNR aux). Deploy: sn < deployed best - 0.001 AND < control.
"""
import os, sys, json
import numpy as np
...
def run_config(archs, label, Xtr, ytr, se, n_tr, chem, tab):
    ...
    arms = {"control": "octant_oof", "sn": "sn_oof"}
    res = {k: [] for k in arms}
...
def main():
    ...
    out = {"5arch": {"means": m5, "std": s5, "sn_minus_control": round(d5, 4)},
           "4gbm_deployed": {"means": m4, "std": s4, "sn_minus_control": round(d4, 4)}}
    json.dump(out, open(f"{P}/nb1168_summary.json", "w"), indent=2)

    prev = json.load(open(BEST))["rae"] if os.path.exists(BEST) else 0.4386
    sn4, ctl4 = m4["sn"], m4["control"]
    if d4 < -0.001 and sn4 < ctl4:
        print(f"\nDEPLOY-WORTHY: 4-GBM matched sn-control {d4:+.4f} (<-0.001). Build deploy submission next.")
    else:
