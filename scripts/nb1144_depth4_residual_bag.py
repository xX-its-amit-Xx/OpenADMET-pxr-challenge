"""nb1144 -- Depth-4 residual-LGBM bag across 5 KFold seeds.

Hypothesis: nb1130 used max_depth=3 / num_leaves=7 / n_est=80 / lr=0.05; the
residual signal might have unexploited capacity (depth=3 too shallow). Depth=5
likely overfits the 253-row residual target. Depth=4 sits in the plausible
optimum: 2x leaves (12 vs 7), slightly higher n_est (120) and lower lr (0.04)
to compensate for the deeper splits and re-balance shrinkage vs capacity.

Protocol per seed s in {0, 1, 7, 42, 137} (identical to nb1130):
  1. Anchor = nb1070 median-bag cross-fit OOF (constant across seeds).
  2. residual = y_unb - nb1070_oof
  3. KFold(n=5, shuffle=True, random_state=s) on 253 unblind.
  4. LGBM Huber (max_depth=4, num_leaves=12, n_est=120, lr=0.04,
...
Compare to nb1130 mean_bag and nb1070 anchor.

Outputs:
  data/processed/nb1144_per_seed_corrected_oof.npy  (5, 253) float32
  data/processed/nb1144_mean_bag_oof.npy            (253,) float32
  data/processed/nb1144_median_bag_oof.npy          (253,) float32
...
NB1130_MEAN_BAG_REF = 0.5704  # nb1130 mean-bag reference (depth=3 bag)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPTH-4 residual-LGBM bag, "
    ...
    print(f"          LGBM: max_depth=4, num_leaves=12, n_est=120, lr=0.04, "
    ...
    if not nb1070_oof_path.exists():
        raise FileNotFoundError(
            f"{nb1070_oof_path} not found; required anchor OOF."
        )
    ...
    print("\n" + "-" * 78)
    print("PER-SEED RESIDUAL CROSS-FIT (depth=4)")
    print("-" * 78)
    ...
    if beats_nb1130:
        verdict = "DEPTH4_BEATS_DEPTH3_BAG"
    elif beats_nb1070:
        verdict = "DEPTH4_BEATS_ANCHOR_BUT_NOT_DEPTH3"
    else:
        verdict = "DEPTH4_NO_GAIN_OVER_DEPTH3"
