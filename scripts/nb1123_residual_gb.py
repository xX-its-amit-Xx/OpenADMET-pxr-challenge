"""nb1123 -- Shallow LGBM Huber on the SIGNED residual of nb1070.

Hypothesis: nb1070 (median-bag of per-quantile stretch on nb1014) is the
current honest-cross-fit champion at pooled RAE ~0.5790. Any remaining
structure in (truth - nb1070_oof) is either (a) feature-correlated signal
the stretch protocol couldn't recover (low Tanimoto rare-scaffold tail,
PEOE_VSA4-dominated F2 inactives) or (b) pure analog-noise noise. A SHALLOW
LGBM (max_depth=3, n_est=80, lr=0.05) fit on combined Morgan+RDKit (2265
features) is the minimal-capacity probe: enough depth to catch 3-way
charge x scaffold-bucket x logP interactions, too little to memorize the
n=253 unblind rows.

This is the residual-correction analogue of nb472's residual stack
router (which used predictor-disagreement features); here we use the
RAW chemistry feature matrix instead -- the residual is signed, so the
sign of correction matters.

Risk: at this honest cross-fit level the residual is mostly noise.
Expected outcome: pred_corrected RAE ~= nb1070 RAE +- 0.005. If it
beats nb1070 by >= 0.003 it's a real win; if Spearman(residual_oof,
truth) is < 0.10 it confirms noise dominance.

Procedure:
  1. Regenerate nb1070_pred_oof on 253 unblind via the same protocol
     used in nb1070 (5-seed median bag of per-quantile-bin stretch on
     nb1014 anchor).
  2. target = y_unb - pred_oof   (signed residual)
  3. X = combined(Morgan+RDKit, 2265) on the 253 unblind SMILES.
  4. Honest 5-fold cross-fit shallow LGBM Huber to produce
     residual_oof (n=253).
  5. pred_corrected = nb1070_oof + residual_oof; pooled RAE.

Outputs:
  data/processed/nb1070_pred_oof.npy       (regen)
  data/processed/nb1123_residual_oof.npy   (residual cross-fit OOF)
  data/processed/nb1123_pred_oof.npy       (corrected prediction OOF)
  data/processed/nb1123_summary.json
"""
...
# Residual model: shallow LGBM Huber.
RESID_SEED = 42
RESID_FOLDS = 5
LGBM_PARAMS = dict(
    objective="huber",
...
# ---------- nb1070 cross-fit OOF regeneration (verbatim) ----------
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- shallow LGBM Huber on (truth - nb1070_oof) residual")
    ...
    # ---- Honest 5-fold cross-fit shallow LGBM on residual ----
    print("\n" + "-" * 78)
...
if __name__ == "__main__":
    res = main()
    print("\n==== SUMMARY ====")
    for k in ("pooled_rae_base", "pooled_rae_corrected", "delta_vs_nb1070",
