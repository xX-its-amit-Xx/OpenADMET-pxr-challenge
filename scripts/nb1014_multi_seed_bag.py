"""nb1014 -- Multi-seed bag of the nb1001 honest cross-fit protocol.

The nb1001 protocol produced a single pooled 5-fold cross-fit RAE of
0.5994 using KFold(seed=42). A single seed is a noisy estimator of the
true LB-honest RAE -- the 5-fold partition itself is a random variable.

Hypothesis: averaging the pooled cross-fit RAE across multiple KFold
seeds reduces the seed-noise of the 5-fold estimate, giving a more
reliable LB-honest number, and averaging (w0, s) across seeds gives a
deploy point that is more robust than any single seed's choice.

Procedure (repeat the nb1001 protocol for SEEDS = {0, 1, 7, 42, 137}):
  Pool = [chemprop_aux, nb972_long_train] on the 253 unblind.
  For each seed in SEEDS:
    5-fold KFold(shuffle=True, random_state=seed) on the 253:
      For each fold f:
        a. SLSQP for w0_f (chemprop weight, K=2 -> w1 = 1 - w0)
           on the 4 train folds.
  ...
  Mean of (w0_f, s_f) across all 5*5 = 25 folds = deploy point.
...
Outputs:
  ...
  data/processed/nb1014_summary.json
  submissions/nb1014_multi_seed_bag.csv
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- multi-seed bag of nb1001 honest cross-fit "
          f"(seeds={SEEDS})")
    print("=" * 78)
    ...
    plain = SUBMISSIONS / f"{TAG}_multi_seed_bag.csv"
    pd.DataFrame({
    ...
    if delta_vs_nb1001 < -0.005:
        verdict = "BEATS_NB1001"
    elif abs(delta_vs_nb1001) <= 0.005:
        verdict = "TIES_NB1001"
    else:
        verdict = "WORSE_THAN_NB1001"
    print(f"\n[verdict] bag vs nb1001 (seed=42 0.5994): "
          f"delta={delta_vs_nb1001:+.4f}  -> {verdict}")
    ...
    print(f"   delta                      = {delta_vs_nb1001:+.4f}")
