"""nb2081 -- SHAP K-grid: top-{30, 40, 60, 75} feature ablation around nb2063 K=50.

HYPOTHESIS:
    nb2063 reported mean-bag RAE 0.4933 on the 253 unblind subset using LGBM(MSE)
    fit to the residual (y_unb - chemprop_aux) on the top-50 SHAP features out
    of the 117-col 5-way K-tuned stack.  K=50 was a hand-pick.  Sweeping K in
    {30, 40, 60, 75} tests whether the SHAP ranking is at its sweet spot at 50
    or whether more / fewer features matter.  Same LGBM(MSE) hyperparams as
    nb2063 (max_depth=4, num_leaves=15, n_est=300, lr=0.03, min_child_samples=5,
    reg_lambda=2.0), 5-seed bag, 5-fold cross-fit per seed.

PROTOCOL:
    1. Reuse SHAP importance ranking from nb2063
       (data/processed/nb2063_shap_importance_full117.npy and
        data/processed/nb2063_top50_idx.npy).  Re-rank the full 117-col SHAP
       importance vector to produce top-K orderings for K in {30, 40, 60, 75}.
       (top-50 is exactly the nb2063 cached selection.)
    2. Rebuild the same 117-col 5-way K-tuned feature matrix as nb2063, using
       the same K-grid winners (nb1352/1392/1484/1523/1524/1541) and ChEMBL kNN.
    3. For each K in {30, 40, 60, 75}: slice X_unb to top-K cols, run 5-seed bag
       (seeds 0, 1, 7, 42, 137) of LGBM(MSE) with KFold(n=5, shuffle=True)
       cross-fit per seed -- mirrors nb2063 / nb1852 inner-seed logic.
    4. Report per-K: per-seed RAE list, per-seed mean/median/std, mean-bag RAE,
       median-bag RAE, family breakdown.
    5. Verdict vs nb2063 K=50 (0.4933) at decision_margin = 0.003.

Outputs:
    scripts/nb2081_shap_K_grid.py
    data/processed/nb2081_summary.json
    data/processed/nb2081_mean_bag_oof_K{K}.npy   (253,) float32 per K
"""
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1852/nb1861/nb2063."""
    frames = []
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- identical to nb1852/nb1861/nb2063."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP K-grid {K_GRID} on 117-col 5-way K-tuned matrix")
    print(f"          anchor={ANCHOR}  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          ref: nb2063 K=50 mean-bag RAE = {NB2063_K50_REF:.4f}  "
    ...
    # Sanity check: argsort -shap_imp_full117 [:50] should equal cached
    ...
    # ---- Load all K-grid winners (same as nb2063) ----
    ...
    # ---- ChEMBL kNN feature (same as nb2063) ----
    print("\n" + "-" * 78)
    ...
    for K in K_GRID:
        ...
        flat_vs_nb2063_k50 = abs(delta_vs_nb2063_k50) < DECISION_MARGIN
        ...
        if beats_nb2063_k50:
            verdict_K = "BEATS_NB2063_K50"
        elif flat_vs_nb2063_k50:
            verdict_K = "FLAT_VS_NB2063_K50"
        elif beats_anchor:
            verdict_K = "BEATS_ANCHOR_BUT_WORSE_THAN_NB2063_K50"
        ...
        per_K_results.append({
            ...
            "flat_vs_nb2063_K50": bool(flat_vs_nb2063_k50),
            "verdict": verdict_K,
        })
    ...
    print(f"   {'K':>4s}  {'mean_bag':>10s}  {'median_bag':>10s}  "
          ...
          f"{'d_vs_anchor':>11s}  {'d_vs_nb2063_K50':>15s}  verdict")
    rows = [(NB2063_K50_REF, "K=50_REF", nb2063_k50_mean_bag,
             nb2063_k50_median_bag, None, None,
             nb2063_k50_mean_bag - rae_anchor, 0.0, "BASELINE")]
    print(f"   {50:>4d}  {nb2063_k50_mean_bag:>10.4f}  "
          f"{nb2063_k50_median_bag:>10.4f}  {'N/A':>13s}  {'N/A':>12s}  "
          f"{nb2063_k50_mean_bag - rae_anchor:>+11.4f}  {0.0:>+15.4f}  "
          f"BASELINE")
    for r in per_K_results:
        print(f"   {r['K']:>4d}  {r['rae_mean_bag']:>10.4f}  "
              ...
              f"{r['delta_mean_bag_vs_nb2063_K50']:>+15.4f}  "
              f"{r['verdict']}")

    # best K by mean-bag RAE among the SWEEP (excluding K=50 ref)
    ...
    # best K overall (including 50)
    all_K = [50] + [r["K"] for r in per_K_results]
    all_rae = [nb2063_k50_mean_bag] + sweep_rae
    ...
    print(f"   delta best vs K=50 = "
          f"{best_K_overall_rae - nb2063_k50_mean_bag:+.4f}")

    # ---- Overall verdict ----
    if best_K_overall == 50:
        global_verdict = "K=50_IS_OPTIMAL_NO_GRID_GAIN"
    elif best_K_overall_rae < nb2063_k50_mean_bag - DECISION_MARGIN:
        global_verdict = (
            f"K_GRID_BEATS_NB2063_K50_AT_K={best_K_overall}"
        )
    elif abs(best_K_overall_rae - nb2063_k50_mean_bag) < DECISION_MARGIN:
        global_verdict = (
            f"K_GRID_FLAT_VS_NB2063_K50_BEST_K={best_K_overall}"
        )
    else:
        global_verdict = "K_GRID_DOES_NOT_BEAT_NB2063_K50"
    ...
    summary = {
        "tag": TAG,
        "method": "lgbm_mse_shap_K_grid_30_40_60_75_on_117col",
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        "5-way K-tuned matrix as nb2063 (AtomPair / "
        ...
        "delta_best_K_overall_vs_K50": (
            best_K_overall_rae - nb2063_k50_mean_bag
        ),
...
if __name__ == "__main__":
    ...
    for r in res["per_K_records"]:
        print(f"  K={r['K']:>3d}  mean_bag={r['rae_mean_bag']:.4f}  "
              ...
              f"d_vs_K50={r['delta_mean_bag_vs_nb2063_K50']:+.4f}  "
              f"{r['verdict']}")
