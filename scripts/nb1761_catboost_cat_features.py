"""nb1761 -- CatBoost with one-hot binary fingerprint bits as cat_features.

HYPOTHESIS:
    nb1554 (CatBoost, MAE, d4, n200, lr0.05, l2=5) on the 117-col 5-way
    K-tuned matrix yields mean_bag RAE = 0.5163.  All 117 cols are treated
    as numerical in nb1554.  But 115 of those cols are actually binary
    fingerprint bits (25 AtomPair + 20 MACCS + 20 ChempropEmbed + 30
    Avalon + 20 Mordred?) -- in fact only 75 are truly binary one-hot bits
    (AP + MACCS + Avalon = 75 raw bits 0/1); Mordred and ChempropEmbed are
    continuous, and the last 2 (pred_chembl_pec50, mean_sim) are
    continuous.  CatBoost's `cat_features` mechanism uses target-statistics
    encoding (ordered TS) for binary categoricals instead of axis-aligned
    splits -- this can extract richer signal from rare bits than the
    default depth-4 axis splits on `0/1` numeric values.

PROTOCOL:
    1. Build the same 117-col 5-way K-tuned feature matrix as nb1554.
       Layout (column indices):
            [  0 :  25)  AtomPair top-25         BINARY  cat
            [ 25 :  45)  MACCS top-20            BINARY  cat
            [ 45 :  65)  Mordred top-20          continuous
            [ 65 :  85)  ChempropEmbed top-20    continuous
            [ 85 : 115)  Avalon top-30           BINARY  cat
            [115 : 116)  pred_chembl_pec50       continuous
            [116 : 117)  mean_sim                continuous
       cat_features = [0..25) U [25..45) U [85..115)  -- 75 cols total.
    ...
    3. 5-seed bag CatBoost(loss=MAE, depth=4, n_est=200, lr=0.05, l2=5,
       cat_features=binary_bit_indices).
       NOTE: CatBoost requires cat-feature columns to be integer-typed in
       the input matrix (or string).  We cast the 75 binary cols to int32
       and the rest stay float32.  Pool-API takes a mixed-dtype DataFrame
       to preserve per-column dtype.
    4. KFold(n=5, shuffle=True) cross-fit per seed.
    5. Pool mean_bag and median_bag corrected anchors.
    6. Verdict at 0.003 margin vs nb1554 (0.5163, prior PRIMARY).

Outputs:
    scripts/nb1761_catboost_cat_features.py
    data/processed/nb1761_summary.json
    data/processed/nb1761_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163       # prior PRE-unblind PRIMARY (CatBoost-numeric on same 117 cols)
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1492/nb1543/nb1554."""
    frames = []
...
def _build_mixed_dtype_df(X: np.ndarray, cat_idx: list[int]) -> pd.DataFrame:
    """Build a DataFrame where cat_idx columns are int32 (binary bits)
    and all other columns are float32.  CatBoost reads cat_features by
    integer position, but needs them to be int (or str) -- it errors on
    float categoricals."""
    df = pd.DataFrame()
    cat_set = set(cat_idx)
    for j in range(X.shape[1]):
        col = X[:, j]
        if j in cat_set:
            # binary 0/1 bit -- clip + cast to int32
            col_i = np.rint(np.clip(col, 0.0, 1.0)).astype(np.int32)
            df[f"c_{j:03d}"] = col_i
        else:
            df[f"x_{j:03d}"] = col.astype(np.float32)
    return df


def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, cat_idx: list[int]) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    # Build mixed-dtype DataFrame ONCE (slice by fold indices below).
    X_df = _build_mixed_dtype_df(X, cat_idx)
    cat_col_names = [c for c in X_df.columns if c.startswith("c_")]
    for tr_loc, va_loc in kf.split(np.arange(n)):
        X_tr = X_df.iloc[tr_loc].reset_index(drop=True)
        X_va = X_df.iloc[va_loc].reset_index(drop=True)
        y_tr = residual[tr_loc]
        mdl = CatBoostRegressor(**_cat_params(seed), cat_features=cat_col_names)
        mdl.fit(X_tr, y_tr)
...
def main() -> dict:
    ...
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on 5-way K-tuned "
          f"117-col matrix WITH cat_features=binary_bits (AP+MACCS+Avalon=75)")
    print(f"          PRE-unblind anchor={ANCHOR}")
    ...
    print(f"[reuse] top-{n_top_ap}     AtomPair bits (nb1524 K={K_AP_best})  -- CAT")
    print(f"[reuse] top-{n_top_maccs}     MACCS bits  (nb1352)             -- CAT")
    print(f"[reuse] top-{n_top_mord}     Mordred cols (nb1523 K={K_Mord_best})  -- num")
    print(f"[reuse] top-{n_top_embed}     ChempropEmbed dims (nb1541 K={K_Embed_best})  -- num")
    print(f"[reuse] top-{n_top_avalon}     Avalon bits (nb1392 SHAP K=30)  -- CAT")
    ...
    # Column layout (must match the cat_idx list below):
    #   [0 : K_AP)                    AtomPair top-K_AP_best         BINARY
    #   [K_AP : K_AP+K_MACCS)         MACCS top-20                   BINARY
    #   [K_AP+K_MACCS : ... +K_Mord)  Mordred top-K_Mord_best        cont
    #   [... : ... + K_Embed)         ChempropEmbed top-K_Embed_best cont
    #   [... : ... + K_Avalon)        Avalon top-30                  BINARY
    #   [... : ... + 1)               pred_chembl_pec50              cont
    #   [... : ... + 1)               mean_sim                       cont
    ...
    # ---- Build cat_features index list (75 binary bits) ----
    ap_lo, ap_hi = 0, n_top_ap
    mc_lo, mc_hi = ap_hi, ap_hi + n_top_maccs
    mo_lo, mo_hi = mc_hi, mc_hi + n_top_mord
    em_lo, em_hi = mo_hi, mo_hi + n_top_embed
    av_lo, av_hi = em_hi, em_hi + n_top_avalon
    cont_lo, cont_hi = av_hi, av_hi + 2
    assert cont_hi == feat_dim

    cat_idx = (list(range(ap_lo, ap_hi))
               + list(range(mc_lo, mc_hi))
               + list(range(av_lo, av_hi)))
    n_cat = len(cat_idx)
    n_cont = feat_dim - n_cat
    ...
    print(f"   cat_features count = {n_cat}  (AP {ap_lo}..{ap_hi}  "
          f"MACCS {mc_lo}..{mc_hi}  Avalon {av_lo}..{av_hi})")
    print(f"   continuous count   = {n_cont}  (Mord {mo_lo}..{mo_hi}  "
          f"Embed {em_lo}..{em_hi}  cont {cont_lo}..{cont_hi})")

    # ---- Quick sanity: binary cols are actually {0,1} ----
    cat_block = X_unb[:, cat_idx]
    cat_min = float(cat_block.min())
    cat_max = float(cat_block.max())
    cat_uniques = int(len(np.unique(np.rint(np.clip(cat_block, 0, 1)).astype(np.int32))))
    print(f"   cat_block range = [{cat_min:.2f}, {cat_max:.2f}]  "
          f"unique-after-cast = {cat_uniques}")
    ...
    for i, s in enumerate(RESID_SEEDS):
        ts = time.time()
        resid_oof_s = _residual_cross_fit_one_seed(X_unb, residual, s, cat_idx)
    ...
    # ---- Verdict (anchored on nb1554 = 0.5163) ----
    ...
    if beats_nb1554:
        verdict = "CATBOOST_CAT_FEATURES_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_CAT_FEATURES_FLAT_VS_NB1554"
    elif beats_nb1543:
        verdict = "CATBOOST_CAT_FEATURES_BEATS_NB1543_BUT_WORSE_THAN_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_CAT_FEATURES_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_CAT_FEATURES_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_CAT_FEATURES_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "cat_features_active": True,
        "n_cat_features": n_cat,
        "cat_features_breakdown": {
            "atompair_bits": n_top_ap,
            "maccs_bits": n_top_maccs,
            "avalon_bits": n_top_avalon,
        },
        "cat_features_column_ranges": {
            "atompair": [int(ap_lo), int(ap_hi)],
            "maccs":    [int(mc_lo), int(mc_hi)],
            "avalon":   [int(av_lo), int(av_hi)],
        },
        "K_AP_best": K_AP_best,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "n_cat_features", "cat_features_breakdown",
