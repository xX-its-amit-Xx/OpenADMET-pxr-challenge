"""nb2103 -- Even finer SHAP K-grid {26, 27, 28, 29, 31, 32, 33, 34, 35}.

HYPOTHESIS:
    nb2081 found K=30 mean-bag RAE = 0.4788 as the optimum SHAP-pruning level
    on the 117-col 5-way K-tuned feature matrix.  nb2091 swept K in
    {10, 15, 20, 25} and confirmed K=30 remains optimal (K=25 was flat at
    0.4790, K=20 worse at 0.4903).  This notebook zooms in to a 9-point grid
    one step at a time around the K=30 winner: K in
    {26, 27, 28, 29, 31, 32, 33, 34, 35}.

    Same LGBM(MSE) hyperparams as nb2063/nb2081/nb2091 (max_depth=4,
    num_leaves=15, n_estimators=300, lr=0.03, min_child_samples=5,
    reg_lambda=2.0), 5-seed bag (seeds 0, 1, 7, 42, 137), 5-fold cross-fit
    per seed.

PROTOCOL:
    1. Reuse SHAP importance ranking from nb2063
       (data/processed/nb2063_shap_importance_full117.npy).  Re-rank the full
       117-col SHAP importance vector and slice top-K for each K in the grid.
    2. Rebuild the same 117-col 5-way K-tuned feature matrix as
       nb2063/nb2081/nb2091, using the same K-grid winners
       (nb1352/1392/1484/1523/1524/1541) and ChEMBL kNN.
    3. For each K in {26, 27, 28, 29, 31, 32, 33, 34, 35}: slice X_unb to
       top-K cols, run 5-seed bag (seeds 0, 1, 7, 42, 137) of LGBM(MSE)
       with KFold(n=5, shuffle=True) cross-fit per seed.
    4. Report per-K: per-seed RAE list, per-seed mean/median/std, mean-bag
       RAE, median-bag RAE, family breakdown.
    5. Verdict vs nb2081 K=30 (0.4788) at decision_margin = 0.003.

Outputs:
    scripts/nb2103_shap_K_fine.py
    data/processed/nb2103_summary.json
    data/processed/nb2103_mean_bag_oof_K{K}.npy   (253,) float32 per K
"""
...
K_GRID = [26, 27, 28, 29, 31, 32, 33, 34, 35]
...
NB2091_SUMMARY = DATA_PROCESSED / "nb2091_summary.json"
...
NB2081_K30_REF = 0.4788   # nb2081 mean-bag RAE (top-30 SHAP) -- beat target
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1852/nb1861/nb2063/nb2081/nb2091."""
    frames = []
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- identical to nb1852/nb1861/nb2063/nb2081/nb2091."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP K-grid FINE {K_GRID} on 117-col 5-way K-tuned matrix")
    ...
    # ---- nb2081 K=30 reference (beat target) ----
    ...
    # ---- nb2091 K=25 reference (closest to fine grid bottom) ----
    nb2091_k25_mean_bag = None
    nb2091_k25_median_bag = None
    if NB2091_SUMMARY.exists():
        with open(NB2091_SUMMARY) as f:
            nb2091_sum = json.load(f)
        for r in nb2091_sum.get("per_K_records", []):
            if int(r.get("K", -1)) == 25:
                nb2091_k25_mean_bag = float(r["rae_mean_bag"])
                nb2091_k25_median_bag = float(r["rae_median_bag"])
                break
    if nb2091_k25_mean_bag is not None:
        print(f"[ref] nb2091.K=25 mean_bag_rae   = {nb2091_k25_mean_bag:.4f}")
    ...
    # ---- Load all K-grid winners (same as nb2063/nb2081/nb2091) ----
    ...
    # ---- ChEMBL kNN feature (same as nb2063/nb2081/nb2091) ----
    print("\n" + "-" * 78)
    ...
    # ---- Feature names (same as nb2063/nb2081/nb2091) ----
    feat_names: list[str] = []
    ...
    # ---- K-grid sweep (FINE: one-step grid around K=30) ----
    print("\n" + "-" * 78)
    print(f"K-GRID SWEEP (FINE): {K_GRID}")
    print("-" * 78)
    ...
    # ---- Select best K ----
    print("\n" + "=" * 78)
    print("K-GRID SUMMARY TABLE (FINE)")
    print("=" * 78)
    ...
    if best_K_overall == 30:
        global_verdict = "NB2081_K30_REMAINS_OPTIMAL_FINE_K_GRID_FLAT_OR_WORSE"
    elif best_K_overall_rae < nb2081_k30_mean_bag - DECISION_MARGIN:
        global_verdict = (
            f"FINE_K_GRID_BEATS_NB2081_K30_AT_K={best_K_overall}"
        )
    elif abs(best_K_overall_rae - nb2081_k30_mean_bag) < DECISION_MARGIN:
        global_verdict = (
            f"FINE_K_GRID_FLAT_VS_NB2081_K30_BEST_K={best_K_overall}"
        )
    else:
        global_verdict = "FINE_K_GRID_DOES_NOT_BEAT_NB2081_K30"
    ...
    summary = {
        "tag": TAG,
        "method": ("lgbm_mse_shap_K_grid_FINE_26_27_28_29_31_32_33_34_35"
                   "_on_117col"),
        "anchor": ANCHOR,
        ...
        "nb2091_K25_mean_bag_ref": nb2091_k25_mean_bag,
        "nb2091_K25_median_bag_ref": nb2091_k25_median_bag,
