"""nb2532 -- SVR with RBF kernel on K=20 features.

NEW PARADIGM (vs LGBM trees): kernel SVM for non-axis-aligned decision
surfaces.  Tree splits are axis-aligned (rectangles in feature space);
the RBF kernel implicitly maps to an infinite-dimensional space where
decision surfaces are radially-symmetric Gaussian bumps, capturing
local interactions LGBM cannot represent without explicit cross terms.

PROTOCOL:
    1. Load K=20 RFE-surviving features from nb2231_summary.json
       and rebuild the 117-col 5-way feature matrix (AtomPair / MACCS /
       Mordred / ChempropEmbed / Avalon + ChEMBL kNN), slice to the
       20 surviving indices on the 513 test compounds.
    2. StandardScaler per fold (fit on train fold only, transform val).
    3. SVR(kernel='rbf', gamma='scale', epsilon=0.1) sweeping
       C in {0.1, 1.0, 10.0}.
    4. 5-fold scaffold CV on 253 unblind, 5 kf_seeds in {1001..1005}.
    5. Gate: best C mean_rae < 0.4570 -> PROMOTE
                                < 0.4601 -> MARGINAL_BEAT
                                else      -> FAIL
    6. Save nb2532_summary.json, pred_oof, te (using best C).

OUTPUTS:
    scripts/nb2532_svm_rbf_k20.py
    data/processed/nb2532_summary.json
    data/processed/nb2532_pred_oof.npy        (253,) float32  (best C)
    data/processed/te_nb2532.npy              (513,) float32  (best C)
"""
...
# -------- SVR hyperparameters --------
C_GRID = [0.1, 1.0, 10.0]
SVR_KWARGS = dict(kernel="rbf", gamma="scale", epsilon=0.1)

# -------- nb2240 reference --------
NB2240_REF_OOF_K20 = 0.4682  # nb2171 5-anchor pyramid pooled RAE (K20 sub-ensemble proxy)

# -------- feature build paths (copied from nb2240/nb2502) --------
...
# =============================================================================
# Helpers (copied from nb2240/nb2502 -- feature build + ChEMBL pool)
# =============================================================================
...
def cv_run_for_C(X_unb, y_unb, unb_scaffolds, C_value):
    """Run 5-seed scaffold CV for a single C value.

    Per fold:
      * Fit StandardScaler on train slice (so val slice is held out from scaler stats).
      * Fit SVR(C=C_value, **SVR_KWARGS) on scaled train slice.
      * Predict val slice -> populate OOF.

    Returns dict with per-seed pooled RAE, oof arrays, and mean.
    """
    n_unb = X_unb.shape[0]
    per_seed = []
    all_oofs = []
    for kf_seed in KF_SEEDS:
        ...
        for tr_loc, va_loc in splits:
            ...
            mdl = SVR(C=C_value, **SVR_KWARGS)
            mdl.fit(X_tr, y_unb[tr_loc])
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SVR(RBF) on K=20 features (non-axis-aligned kernel paradigm)")
    print("=" * 78)
    ...
    # =========================================================================
    # Sweep C across {0.1, 1.0, 10.0}, 5-fold scaffold CV, 5 kf_seeds
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"SVR(RBF) C-sweep  C_grid={C_GRID}  kf_seeds={KF_SEEDS}  folds={N_FOLDS}")
    print(f"   {SVR_KWARGS} + StandardScaler per fold")
    print("-" * 78)
    ...
    for C in C_GRID:
        tsw = time.time()
        res = cv_run_for_C(X_unb_K20, y_unb, unb_scaffolds, C)
        wall = time.time() - tsw
        per_seed_str = ", ".join(f"{r['pooled_rae']:.4f}" for r in res["per_seed"])
        print(f"   C={C:>5g}  mean_rae={res['mean_rae']:.4f} (+/- {res['std_rae']:.4f})  "
              f"per_seed=[{per_seed_str}]  wall={wall:.1f}s")
        ...
        if (best is None) or (res["mean_rae"] < best["mean_rae"]):
            best = res

    print(f"\n[best] C={best['C']:>5g}  mean_rae={best['mean_rae']:.4f}  "
          f"(rae_of_mean_oof={best['rae_of_mean_oof']:.4f})")

    # --- gate vs PROMOTE/MARGINAL thresholds ---
    ...
    if best_mean_rae < GATE_PROMOTE:
        ...
        verdict = "FAIL"
    delta_nb2240 = best_mean_rae - NB2240_REF_OOF_K20
    print(f"\n[gate] best_C={best['C']}  mean_rae={best_mean_rae:.4f}  "
    ...
    # =========================================================================
    # Deploy at best C: refit on all 253 (single StandardScaler), predict 513
    # =========================================================================
    print("\n" + "-" * 78)
    print(f"DEPLOY  (refit SVR(C={best['C']}) on all 253; predict 513)")
    print("-" * 78)
    scaler_full = StandardScaler()
    X_unb_scaled = scaler_full.fit_transform(X_unb_K20)
    X_te_scaled = scaler_full.transform(X_te_K20)
    mdl_full = SVR(C=best["C"], **SVR_KWARGS)
    mdl_full.fit(X_unb_scaled, y_unb)
    deploy_te = mdl_full.predict(X_te_scaled).astype(np.float32)
    ...
    np.save(pred_oof_path, best["mean_oof"].astype(np.float32))
    ...
    summary = {
        "tag": TAG,
        "method": "SVR_RBF_K20_chemprop_aux_anchor_free",
        "paradigm": "kernel_SVM_non_axis_aligned_decision_surfaces",
        ...
        "best_rae_of_mean_oof": best["rae_of_mean_oof"],
