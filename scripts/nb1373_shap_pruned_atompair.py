"""nb1373 -- SHAP-pruned AtomPair top-30 + ChEMBL residual learner.

Hypothesis:
    nb1172 used full AtomPair-2048 as residual features over the nb1070 anchor
    (mean-bag RAE 0.5659).  nb1352 demonstrated that SHAP-pruning MACCS from
    167 -> 20 bits (plus pred_chembl + sim) cut residual capacity at n=253
    and improved RAE to 0.5323.  By analogy, SHAP-pruning AtomPair from 2048
    -> 30 bits (plus pred_chembl + sim) may yield a similar overfit-shrink
    win over the full-AtomPair baseline, possibly competitive with or
    beating nb1352 thanks to the orthogonal pair-distance channel.

Protocol:
    1.  Anchor = nb1070_pred_oof on 253 unblind rows.
        residual = y_unb - anchor.
    2.  Build ChEMBL PXR pool (same union as nb1242/nb1352).  Compute kNN-5
        Tanimoto on cached Morgan-2048 over std test SMILES; pull
        pred_chembl_pec50 + mean_sim for all test rows, slice to unblind.
    3.  Build full 2050-col feature matrix on 253:
            AtomPair-2048 (cached, sliced) + pred_chembl_pec50 + mean_sim
        Train ONE seed-0 shallow LGBM Huber on residual to get SHAP
        importance via shap.TreeExplainer; fall back to LGBM gain on
        failure.  Take mean(|SHAP|) per feature, slice AtomPair-only,
    ...
    6.  Verdict at 0.003 margin vs:
            nb1352  (SHAP-pruned MACCS top-20 baseline, 0.5323 mean-bag)
            nb1172  (full AtomPair-2048 standalone, 0.5659 mean-bag)
            nb1070  (anchor, ~0.5771-0.5790 pooled)

Outputs:
    scripts/nb1373_shap_pruned_atompair.py        (this file)
    data/processed/nb1373_summary.json
    data/processed/nb1373_mean_bag_oof.npy        (253,) float32
    data/processed/nb1373_median_bag_oof.npy      (253,) float32
    data/processed/nb1373_per_seed_corrected_oof.npy (5, 253) float32
"""
...
NB1070_REF = 0.5771
NB1172_REF = 0.5659      # full AtomPair-2048 standalone residual bag
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP-pruned AtomPair top-{TOP_K_ATOMPAIR} + ChEMBL residual; "
          f"anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          baselines: nb1352 ({NB1352_REF:.4f}), "
          f"nb1172 ({NB1172_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1172 = {rae_mean_bag - NB1172_REF:+.4f}"
    ...
    print(f"   pooled RAE(median_bag) = {rae_median_bag:.4f}  "
          ...
          f"  d_vs_nb1172 = {rae_median_bag - NB1172_REF:+.4f}"
          f"  d_vs_nb1352 = {rae_median_bag - NB1352_REF:+.4f})")
    print(f"   nb1070 ref            = {NB1070_REF:.4f}")
    print(f"   nb1172 ref            = {NB1172_REF:.4f}  (full AtomPair-2048)")
    print(f"   nb1352 ref            = {NB1352_REF:.4f}  (SHAP-pruned MACCS top-20)")

    beats_nb1070 = rae_mean_bag < rae_anchor - DECISION_MARGIN
    beats_nb1172 = rae_mean_bag < NB1172_REF - DECISION_MARGIN
    beats_nb1352 = rae_mean_bag < NB1352_REF - DECISION_MARGIN

    if beats_nb1352:
        verdict = "SHAP_PRUNED_ATOMPAIR_BEATS_NB1352_NEW_PRIMARY_CANDIDATE"
    elif abs(rae_mean_bag - NB1352_REF) < DECISION_MARGIN:
        verdict = "SHAP_PRUNED_ATOMPAIR_FLAT_VS_NB1352"
    elif beats_nb1172:
        verdict = "SHAP_PRUNED_ATOMPAIR_BEATS_NB1172_BUT_WORSE_THAN_NB1352"
    elif beats_nb1070:
        verdict = "SHAP_PRUNED_ATOMPAIR_HELPS_NB1070_BUT_WORSE_THAN_NB1172"
    else:
        verdict = "SHAP_PRUNED_ATOMPAIR_HURTS_NB1070"
    ...
    summary = {
        ...
        "delta_mean_bag_vs_nb1172": rae_mean_bag - NB1172_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "beats_nb1070", "beats_nb1172", "beats_nb1352",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
