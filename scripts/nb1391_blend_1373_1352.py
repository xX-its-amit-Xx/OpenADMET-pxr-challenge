"""nb1391 -- Blend nb1373 (AtomPair-pruned) + nb1352 (MACCS-pruned).

HYPOTHESIS
    nb1373 uses the top-30 AtomPair bits (2048-bit hashed substructure family)
    while nb1352 uses the top-20 MACCS bits (167-bit curated dictionary). Only
    one bit index (1145) overlaps between the two SHAP top-K sets. The two
    families are bit-orthogonal, so a linear blend may pick up gain that neither
    family captures alone.

PROTOCOL
    1. Load nb1373_mean_bag_oof.npy (RAE 0.5095) and nb1352_mean_bag_oof.npy
       (RAE 0.5323).
    2. Also load nb1373_median_bag_oof.npy (RAE 0.5128) and
       nb1352_median_bag_oof.npy (RAE 0.5315).
    3. Pearson on raw preds AND residuals-from-truth for all 4 candidates.
    4. Grid-search w in {0.0..1.0 step 0.05} over w*nb1373 + (1-w)*nb1352 for
       both (mean,mean) and (median,median) pairings -- report top-3 variants.
    5. 5-fold SLSQP cross-fit on (mean,mean) and (median,median) pairings.
    6. Verdict at 0.003 margin vs nb1373 mean (0.5095).

OUTPUTS
    data/processed/nb1391_summary.json
    data/processed/nb1391_best_oof.npy   (253,) float32  best variant predictor
"""
...
NB1352_MEAN_REF = 0.5323   # MACCS-pruned    mean-bag
NB1352_MEDIAN_REF = 0.5315 # MACCS-pruned    median-bag

ANCHOR_REF = NB1373_MEAN_REF   # gate against the best component
MARGIN = 0.003
...
def _grid_2way(p_a: np.ndarray, p_b: np.ndarray, y: np.ndarray,
    ...
    # find best record (and capture its blend for later if needed).
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 2-way blend  nb1373 (AtomPair) + nb1352 (MACCS)")
    print(f"         grid sweep (step 0.05) + 5-fold SLSQP cross-fit")
    print(f"         gate vs nb1373_mean {ANCHOR_REF:.4f}  margin {MARGIN:.3f}")
    print("=" * 78)
    ...
    p2m = np.load(paths["nb1352_mean"]).astype(np.float64)
    p2d = np.load(paths["nb1352_median"]).astype(np.float64)

    for tag, arr in (
        ("nb1373_mean", p1m), ("nb1373_median", p1d),
        ("nb1352_mean", p2m), ("nb1352_median", p2d),
    ):
    ...
    print(f"[load] nb1352 mean   (MACCS-20)   : RAE = {rae_p2m:.4f}  "
          f"(ref {NB1352_MEAN_REF:.4f})")
    print(f"[load] nb1352 median (MACCS-20)   : RAE = {rae_p2d:.4f}  "
          f"(ref {NB1352_MEDIAN_REF:.4f})")
    ...
    print("\n" + "-" * 78)
    print("(A) Grid sweep  w*nb1373 + (1-w)*nb1352   (step 0.05)")
    print("-" * 78)
    ...
    def _print_grid(name: str, records: list[dict]) -> list[dict]:
    ...
    top3_mm = _print_grid("(mean ,mean )", grid_mm)
    top3_dd = _print_grid("(median,median)", grid_dd)
    top3_md = _print_grid("(mean ,median)", grid_md)
    top3_dm = _print_grid("(median,mean )", grid_dm)
    ...
    print(f"   (mm) mean fold weights   w1373={mean_w_mm[0]:.4f}  "
          f"w1352={mean_w_mm[1]:.4f}  (std {std_w_mm[0]:.4f}/{std_w_mm[1]:.4f})")
    print(f"   (dd) mean fold weights   w1373={mean_w_dd[0]:.4f}  "
          f"w1352={mean_w_dd[1]:.4f}  (std {std_w_dd[0]:.4f}/{std_w_dd[1]:.4f})")
    ...
    beats_nb1373 = best_rae < ANCHOR_REF - MARGIN
    flat_vs_nb1373 = abs(best_rae - ANCHOR_REF) < MARGIN

    if beats_nb1373:
        verdict = f"NB1391_BEATS_NB1373_BY_MARGIN  (best={best_name})"
    elif flat_vs_nb1373:
        verdict = f"NB1391_FLAT_VS_NB1373  (best={best_name})"
    else:
        verdict = f"NB1391_HURTS_VS_NB1373  (best={best_name})"
    ...
    print(f"   nb1352 mean   standalone : {rae_p2m:.4f}")
    print(f"   nb1352 median standalone : {rae_p2d:.4f}")
    ...
    print(f"   anchor nb1373 mean       : {ANCHOR_REF:.4f}")
    print(f"   best variant             : {best_name}  ({best_rae:.4f})")
    print(f"   margin vs nb1373 mean    : "
    ...
    summary = {
        ...
        "components": [
            ...
            "nb1352_mean_bag_oof",
            "nb1352_median_bag_oof",
        ],
        "rae_nb1373_mean_standalone": rae_p1m,
        "rae_nb1373_median_standalone": rae_p1d,
        "rae_nb1352_mean_standalone": rae_p2m,
        "rae_nb1352_median_standalone": rae_p2d,
        ...
        "delta_best_vs_nb1373_mean": best_rae - ANCHOR_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_nb1352_mean_standalone",
        "rae_nb1352_median_standalone",
        "pred_corr_pearson",
