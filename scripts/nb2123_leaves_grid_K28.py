"""nb2123 -- LGBM(MSE) leaves grid {12, 14, 16, 18} on top-28 SHAP.

HYPOTHESIS:
    nb2103 K-fine sweep confirmed K=28 (top-28 SHAP-pruned cols of the 117-col
    5-way K-tuned feature matrix) as the new optimum at mean-bag RAE = 0.4737,
    edging out the previous K=30 winner (0.4788) and the K=25/35 neighbours
    (0.4790 / 0.4793). Holding K=28 fixed, this notebook sweeps num_leaves
    one step at a time on either side of the nb2103 default (15):
    num_leaves in {12, 14, 16, 18}. Looser trees (more leaves) may capture
    additional non-linear residual structure on top of chemprop_aux, but the
    SHAP-pruning to K=28 may already absorb most of that signal; tighter trees
    (fewer leaves) may help if the K=28 column set already overfits at 15
    leaves on n_unb=253.

PROTOCOL:
    1. Reuse SHAP importance ranking from nb2063
       (data/processed/nb2063_shap_importance_full117.npy) -> top-28 cols
       (matches nb2103 K=28 exactly).
    2. Rebuild the same 117-col 5-way K-tuned feature matrix as
       nb2063/nb2081/nb2091/nb2103, using the same K-grid winners
       (nb1352/1392/1484/1523/1524/1541) and ChEMBL kNN.
    3. For each num_leaves L in {12, 14, 16, 18}: 5-seed bag LGBM(MSE,
       max_depth=4, num_leaves=L, n_estimators=300, lr=0.03, min_child=5,
       reg_lambda=2.0) with KFold(n=5, shuffle=True) cross-fit per seed
       (seeds 0, 1, 7, 42, 137).
    4. Report per-L: per-seed RAE list, per-seed mean/median/std, mean-bag
       RAE, median-bag RAE.
    5. Verdict vs nb2103 K=28 leaves=15 (0.4737) at decision_margin = 0.003.

Outputs:
    scripts/nb2123_leaves_grid_K28.py
    data/processed/nb2123_summary.json
    data/processed/nb2123_mean_bag_oof_L{L}.npy   (253,) float32 per L
"""
...
NB2103_K28_REF = 0.4737   # nb2103 mean-bag RAE at K=28 leaves=15 (current best)
...
def _lgbm_params(seed: int, num_leaves: int) -> dict:
    """LGBM(MSE) -- nb2103 hyperparams with num_leaves swept."""
    return dict(
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
                                 seed: int, num_leaves: int) -> np.ndarray:
    n = len(residual)
    kf = KFold(n_splits=RESID_FOLDS, shuffle=True, random_state=seed)
    oof = np.full(n, np.nan, dtype=np.float64)
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = lgb.LGBMRegressor(**_lgbm_params(seed, num_leaves))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) num_leaves grid {LEAVES_GRID} at K={K_FIXED}")
    print(f"          anchor={ANCHOR}  seeds={RESID_SEEDS}  folds={RESID_FOLDS}")
    print(f"          ref: nb2103 K={K_FIXED} leaves=15 mean-bag RAE = "
          f"{NB2103_K28_REF:.4f}  margin = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- nb2103 K=28 reference (current best) ----
    nb2103_k28_mean_bag = NB2103_K28_REF
    nb2103_k28_median_bag = NB2103_K28_REF
    ...
    if not np.array_equal(full_rank_order[:50], nb2063_top50_idx_cached):
        print("   [warn] full-rank top-50 != cached nb2063 top-50; "
              "will use full re-rank for top-28")
    else:
    ...
    # ---- ChEMBL kNN feature (same as nb2063/nb2081/nb2091/nb2103) ----
    print("\n" + "-" * 78)
    ...
    # ---- Feature names (same as nb2063/nb2081/nb2091/nb2103) ----
    feat_names: list[str] = []
    ...
    # ---- Slice to top-28 SHAP (same as nb2103 K=28) ----
    ...
    print(f"\n   top-{K_FIXED} family breakdown: {fam_counts}")
    X_topK = X_unb[:, topK_idx].astype(np.float32)
    print(f"   X_topK shape = {X_topK.shape}")

    # ---- num_leaves sweep ----
    print("\n" + "-" * 78)
    print(f"NUM_LEAVES SWEEP at K={K_FIXED}: {LEAVES_GRID}")
    print("-" * 78)
    per_L_results: list[dict] = []
    for L in LEAVES_GRID:
        ...
        for i, s in enumerate(RESID_SEEDS):
            ts = time.time()
            resid_oof_s = _residual_cross_fit_one_seed(X_topK, residual, s, L)
            ...
            print(f"   L={L:2d} seed={s:3d}:  rae_corr = {rae_s:.4f}  "
        ...
        print(f"   L={L:2d} per-seed RAE   = "
              f"[{', '.join(f'{r:.4f}' for r in per_seed_rae)}]")
        print(f"   L={L:2d} per-seed mean  = {rae_per_seed_mean:.4f}  "
              f"std = {rae_per_seed_std:.4f}")
        print(f"   L={L:2d} pooled mean    = {rae_mean_bag:.4f}  "
              f"(d_vs_anchor = {rae_mean_bag - rae_anchor:+.4f}  "
              f"d_vs_nb2103_K28L15 = {delta_vs_nb2103_k28:+.4f})")
        print(f"   L={L:2d} pooled median  = {rae_median_bag:.4f}")
        if beats_nb2103_k28:
            verdict_L = "BEATS_NB2103_K28_L15"
        elif flat_vs_nb2103_k28:
            verdict_L = "FLAT_VS_NB2103_K28_L15"
        elif beats_anchor:
            verdict_L = "BEATS_ANCHOR_BUT_WORSE_THAN_NB2103_K28_L15"
        elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
            verdict_L = "FLAT_VS_ANCHOR"
        else:
            verdict_L = "HURTS_ANCHOR"
        print(f"   L={L:2d} verdict        = {verdict_L}")

        # Save mean-bag OOF per L
        out_p = DATA_PROCESSED / f"{TAG}_mean_bag_oof_L{L}.npy"
        ...
        per_L_results.append({
            ...
            "delta_mean_bag_vs_nb2103_K28_L15": delta_vs_nb2103_k28,
            "beats_chemprop_aux": bool(beats_anchor),
            "beats_nb2103_K28_L15": bool(beats_nb2103_k28),
            "flat_vs_nb2103_K28_L15": bool(flat_vs_nb2103_k28),
            "verdict": verdict_L,
        })

    # ---- Select best L ----
    print("\n" + "=" * 78)
    print("NUM_LEAVES SWEEP SUMMARY TABLE")
    print("=" * 78)
    print(f"   {'L':>4s}  {'mean_bag':>10s}  {'median_bag':>10s}  "
          f"{'per_seed_mean':>13s}  {'per_seed_std':>12s}  "
          f"{'d_vs_anchor':>11s}  {'d_vs_nb2103_K28L15':>18s}  verdict")
    print(f"   {15:>4d}  {nb2103_k28_mean_bag:>10.4f}  "
          f"{nb2103_k28_median_bag:>10.4f}  {'N/A':>13s}  {'N/A':>12s}  "
          f"{nb2103_k28_mean_bag - rae_anchor:>+11.4f}  {0.0:>+18.4f}  "
          f"BASELINE(nb2103_K28_L15)")
    for r in per_L_results:
        print(f"   {r['num_leaves']:>4d}  {r['rae_mean_bag']:>10.4f}  "
              f"{r['rae_median_bag']:>10.4f}  {r['rae_per_seed_mean']:>13.4f}  "
              f"{r['rae_per_seed_std']:>12.4f}  "
              f"{r['delta_mean_bag_vs_chemprop_aux']:>+11.4f}  "
              f"{r['delta_mean_bag_vs_nb2103_K28_L15']:>+18.4f}  "
              f"{r['verdict']}")

    # best L by mean-bag RAE among the SWEEP (excluding L=15 ref)
    sweep_rae = [r["rae_mean_bag"] for r in per_L_results]
    best_sweep_i = int(np.argmin(sweep_rae))
    best_L_sweep = int(per_L_results[best_sweep_i]["num_leaves"])
    best_L_sweep_rae = float(per_L_results[best_sweep_i]["rae_mean_bag"])

    # best L overall (including L=15 nb2103 ref)
    all_L = [15] + [r["num_leaves"] for r in per_L_results]
    all_rae = [nb2103_k28_mean_bag] + sweep_rae
    best_overall_i = int(np.argmin(all_rae))
    best_L_overall = int(all_L[best_overall_i])
    best_L_overall_rae = float(all_rae[best_overall_i])

    print(f"\n   best L in sweep  = {best_L_sweep}  "
          f"(mean_bag RAE {best_L_sweep_rae:.4f})")
    print(f"   best L overall    = {best_L_overall}  "
          f"(mean_bag RAE {best_L_overall_rae:.4f})")
    print(f"   delta best vs nb2103 K=28 L=15 = "
          f"{best_L_overall_rae - nb2103_k28_mean_bag:+.4f}")

    # ---- Overall verdict ----
    if best_L_overall == 15:
        global_verdict = "NB2103_K28_L15_REMAINS_OPTIMAL_LEAVES_GRID_FLAT_OR_WORSE"
    elif best_L_overall_rae < nb2103_k28_mean_bag - DECISION_MARGIN:
        global_verdict = (
            f"LEAVES_GRID_BEATS_NB2103_K28_L15_AT_L={best_L_overall}"
        )
    elif abs(best_L_overall_rae - nb2103_k28_mean_bag) < DECISION_MARGIN:
        global_verdict = (
            f"LEAVES_GRID_FLAT_VS_NB2103_K28_L15_BEST_L={best_L_overall}"
        )
    else:
        global_verdict = "LEAVES_GRID_DOES_NOT_BEAT_NB2103_K28_L15"
    ...
    summary = {
        "tag": TAG,
        "method": (f"lgbm_mse_num_leaves_grid_{'_'.join(str(l) for l in LEAVES_GRID)}"
                   f"_at_K{K_FIXED}_top_shap_117col"),
        "anchor": ANCHOR,
        ...
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        ...
                        "Avalon + ChEMBL kNN), sliced to top-28 SHAP"),
        "model_family": "LightGBM",
        "lgbm_objective": "regression",
        "lgbm_max_depth": 4,
        "lgbm_num_leaves_grid": LEAVES_GRID,
        ...
        "nb2103_K28_L15_mean_bag_ref": nb2103_k28_mean_bag,
        "nb2103_K28_L15_median_bag_ref": nb2103_k28_median_bag,
        "per_L_records": per_L_results,
        "best_L_sweep": best_L_sweep,
        "best_L_sweep_rae_mean_bag": best_L_sweep_rae,
        "best_L_overall": best_L_overall,
        "best_L_overall_rae_mean_bag": best_L_overall_rae,
        "delta_best_L_overall_vs_nb2103_K28_L15": (
            best_L_overall_rae - nb2103_k28_mean_bag
        ),
        "verdict": global_verdict,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb2103_K28_L15_ref": NB2103_K28_REF,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "lgbm_num_leaves_grid",
        "K_fixed",
        "feat_dim_full",
        "n_chembl_pool",
        "rae_anchor_chemprop_aux",
        "nb2103_K28_L15_mean_bag_ref",
        "best_L_sweep", "best_L_sweep_rae_mean_bag",
        "best_L_overall", "best_L_overall_rae_mean_bag",
        "delta_best_L_overall_vs_nb2103_K28_L15",
        "verdict", "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
    print("\n==== PER-L TABLE ====")
    for r in res["per_L_records"]:
        print(f"  L={r['num_leaves']:>3d}  mean_bag={r['rae_mean_bag']:.4f}  "
              f"median_bag={r['rae_median_bag']:.4f}  "
              f"per_seed_mean={r['rae_per_seed_mean']:.4f}  "
              f"std={r['rae_per_seed_std']:.4f}  "
              f"d_vs_K28L15={r['delta_mean_bag_vs_nb2103_K28_L15']:+.4f}  "
              f"{r['verdict']}")
