"""nb1831 -- Deploy nb1821 pooled 25-bag (LGBM GOSS) to 513-row CSV.

PROTOCOL:
    Same 117-col feature stack as nb1821, but deploy on full 513 test set.
    1. For 5 outer x 5 inner = 25 deploy fits:
         LGBM(boosting='goss', huber, max_depth=4, num_leaves=15, n_est=300,
    ...
    4. te_nb1831 = te_chemprop_aux + mean_residual_513.
    5. Save submissions/nb1831_deploy_nb1821.csv (513 rows).

ANCHORS:
    pooled 25-bag honest cross-fit RAE = 0.5025 (predicted LB 0.506).
"""
...
NB1821_SUMMARY = DATA_PROCESSED / "nb1821_summary.json"
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1821 pooled 25-bag (LGBM GOSS) to 513-row CSV")
    ...
    print("\n" + "-" * 78)
    print(f"DEPLOY: 25 LGBM(GOSS) fits on all {n_unb} unblind, predict {n_test}")
    print("-" * 78)
    ...
    te_nb1831 = te_anchor_513 + mean_resid_513

    # ---- In-sample check on unb_idx ----
    in_pred_unb = te_nb1831[unb_idx]
    rae_in_unb = float(rae(y_unb, in_pred_unb))
    print("\n" + "-" * 78)
    print(f"in-sample RAE on unb_idx = {rae_in_unb:.4f}")
    print(f"anchor in_RAE             = {rae_anchor:.4f}")
    print(f"nb1821 pooled 25-bag      = 0.5025 (honest cross-fit)")
    print("-" * 78)

    # ---- Stats ----
    te_mean = float(te_nb1831.mean())
    te_std = float(te_nb1831.std())
    te_min = float(te_nb1831.min())
    te_max = float(te_nb1831.max())
    print(f"\nte_nb1831 stats: mean={te_mean:.4f}  std={te_std:.4f}  "
    ...
    df_sub = pd.DataFrame({
        ...
        "pEC50": te_nb1831.astype(np.float32),
    })
    if len(df_sub) != 513:
        raise ValueError(f"submission rows {len(df_sub)} != 513")
    sub_path = SUBMISSIONS_DIR / f"{TAG}_deploy_nb1821.csv"
    df_sub.to_csv(sub_path, index=False)
    print(f"\n[save] submission CSV: {sub_path}  ({len(df_sub)} rows)")

    # ---- Save te artifact ----
    te_path = DATA_PROCESSED / f"te_{TAG}.npy"
    np.save(te_path, te_nb1831.astype(np.float32))
    ...
    summary = {
        ...
        "te_nb1831_mean": te_mean,
        "te_nb1831_std": te_std,
        "te_nb1831_min": te_min,
        "te_nb1831_max": te_max,
        "in_RAE_unb_idx": rae_in_unb,
        "nb1821_pooled_25bag_honest": 0.5025,
        "predicted_LB": 0.506,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "te_nb1831_mean", "te_nb1831_std",
        "te_nb1831_min", "te_nb1831_max",
        "in_RAE_unb_idx",
        "nb1821_pooled_25bag_honest", "predicted_LB",
        "submission_csv",
    ):
        print(f"  {k}: {res.get(k)}")
