"""nb2111 -- Pooled-25 SHAP top-28 (5 outer x 5 inner).

HYPOTHESIS:
    nb2103 fine-grid SHAP K-sweep produced K=28 mean_bag = 0.4737 /
    median_bag = 0.4698 using a 5-seed bag (seeds 0, 1, 7, 42, 137) of
    LGBM(MSE) with KFold(n=5, shuffle=True) cross-fit per seed.

    This notebook expands the bag to a pooled-25 design:
        5 OUTER seeds (0, 1, 7, 42, 137)
        x 5 INNER seeds (per outer o: inner = [o*1000 + s for s in {0,1,7,42,137}])
        = 25 total fits.
    Each fit is one full 5-fold cross-fit on the residual y_unb - anchor;
    the 25 OOF residual vectors are added back to the anchor and POOLED
    (mean or median over the 25-dim axis) to produce a single 253-vec
    corrected prediction.

PROTOCOL:
    1. Reuse top-28 SHAP indices from nb2103 (= full_rank_order[:28] on
       data/processed/nb2063_shap_importance_full117.npy).
    2. Rebuild the same 117-col 5-way K-tuned feature matrix as
       nb2063/nb2081/nb2091/nb2103, slice top-28.
    3. 5 outer seeds, each with 5 inner seeds = 25 fits of LGBM(MSE) with
       KFold(n=5, shuffle=True, random_state=inner_seed) and
       LGBMRegressor(random_state=inner_seed). Same hyperparams as
       nb2063/nb2081/nb2091/nb2103.
    4. Pool the 25 corrected OOF vectors via MEAN and via MEDIAN; report
       BoB (bag-of-bags) mean and BoB median plus the pooled-25 RAE.
    5. Verdict vs nb2103 K=28 (mean_bag 0.4737 / median_bag 0.4698) at
       decision_margin = 0.003.

Outputs:
    scripts/nb2111_pooled_25bag_shap28.py
    data/processed/nb2111_summary.json
    data/processed/nb2111_pooled25_mean_oof.npy   (253,) float32
    data/processed/nb2111_pooled25_median_oof.npy (253,) float32
"""
...
K_SHAP = 28  # nb2103 fine-grid result
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Pooled-25 SHAP top-{K_SHAP} on 117-col 5-way K-tuned matrix")
    ...
    if NB2103_SUMMARY.exists():
        ...
        for r in nb2103_sum.get("per_K_records", []):
            if int(r.get("K", -1)) == K_SHAP:
                ...
                # cross-check top-28 indices vs nb2103
    ...
    print("\n" + "-" * 78)
    print(f"POOLED-25: outer={OUTER_SEEDS}  inner_offsets={INNER_OFFSETS}")
    print("-" * 78)
    ...
    # Outer-level (BoB) buckets: each outer aggregates 5 inner OOFs
    ...
    for oi, o in enumerate(OUTER_SEEDS):
        ...
        # Outer-level aggregation: mean / median over 5 inner OOFs
    ...
    # ---- Pooled-25 RAE: pool all 25 OOFs ----
    ...
    # ---- BoB: bag-of-bags (mean over outer-level aggregates) ----
    ...
    print(f"   nb2103 K=28 mean_bag ref              = "
          f"{nb2103_k28_mean_bag:.4f}")
    print(f"   nb2103 K=28 median_bag ref            = "
