"""nb2872 -- LGBM K=20 NATIVE POISSON objective on shifted chemprop_aux residual.

NEW PARADIGM (vs default L2-regression on chemprop_aux residual):
    Every prior K=20 LGBM substrate (nb2240, nb2860, nb2700, etc.) has used
    objective='regression' (L2/MSE) on the chemprop_aux residual. That loss
    is symmetric and assumes Gaussian noise around the residual. The known
    F2 failure mode (greasy-novel-inactive over-prediction; -0.11 RAE prize
    per the Phase-2 pm06 framing) is fundamentally asymmetric: the residual
    distribution has a heavy LEFT tail (under-prediction of true inactives
    where the anchor over-predicts) and the cost of getting that tail right
    matters more than the symmetric-MSE loss can express.

    The Poisson NLL objective has an asymmetric variance-mean coupling
    (var = mean) and a log-link inside LightGBM's internal optimization,
    which means leaf splits that reduce the residual on the high-magnitude
    tail get larger gradient updates than the corresponding mid-magnitude
    splits an L2 loss would prefer. This biases the residual fit toward
    the tail rows -- exactly the F2 failure cluster -- without adding any
    new features or anchors.

    The wrinkle: LightGBM's Poisson objective requires y >= 0. Residuals on
    a 4.5-9 pEC50 scale span roughly [-2.5, +3.0]. The simple shift
    y_shift = y_resid - min(y_resid) + 0.1 maps residuals into [0.1, ~5.5]
    while preserving the rank order and the relative tail geometry. After
    training, the Poisson-prediction in shifted space is mapped back to
    residual space via pred_resid = pred - 0.1 + min(y_resid), and the final
    pEC50 prediction is anchor + pred_resid. The shift is computed ONCE on
    the unblind 253 residual support and applied identically at train and
    deploy (no leakage -- min is a fold-invariant offset).

PROTOCOL:
    1. Load X_117 substrate -> slice K=20 surviving columns from nb2240
       summary (identical substrate as nb2700/nb2860).
    2. residual = y_unb - chemprop_aux_te[unb_idx]  (only PRE-clean anchor).
    3. Compute y_min = min(residual); shift = -y_min + 0.1.
       y_shift = residual + shift  (>= 0.1 by construction).
    4. LGBM(objective='poisson', max_depth=4, num_leaves=15, n_est=300,
            learning_rate=0.03, min_child_samples=5, reg_lambda=2.0).
    5. 5-fold scaffold CV (`scaffold_kfold_indices`) per seed; predict on
       shifted residual space, then unshift back to residual space.
    6. 5 kf_seeds {1001..1005} -> mean-bag aggregate.
    7. Deploy refit on full 253 per seed -> predict 513.
...
OUTPUTS:
    scripts/nb2872_lgbm_poisson.py
    data/processed/nb2872_summary.json
    data/processed/nb2872_pred_oof.npy       (253,) float32 mean-bag CORRECTED
    data/processed/te_nb2872.npy             (513,) float32 deploy
    submissions/nb2872_lgbm_poisson.csv
"""
...
# Shift epsilon for Poisson positivity (small, well below typical |residual|)
SHIFT_EPS = 0.1
...
NB2240_K20_REF = 0.4630  # K=20 LGBM baseline (L2 regression at reg_lambda=2.0)


def _lgbm_params(seed: int) -> dict:
    """LGBM hyperparams -- native Poisson objective, all else fixed to nb2240."""
    return dict(
        objective="poisson",
        max_depth=4,
        num_leaves=15,
...
def main() -> dict:
    ...
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM K=20 native Poisson on shifted chemprop_aux residual")
    ...
    # ---- Shift residual into positive support for Poisson ----
    ...
    # ---- Per-seed scaffold-CV in shifted space ----
    print("\n" + "-" * 78)
    print(f"POISSON LGBM  seeds={KF_SEEDS}  folds={N_FOLDS}  shift={shift:+.4f}")
    print("-" * 78)
    ...
    for i, seed in enumerate(KF_SEEDS):
        ts = time.time()
        # OOF in shifted space
        oof_shift = _scaffold_cv_one_seed(X_unb, y_shift, unb_scaffolds, seed)
        per_seed_oof_shift[i] = oof_shift
        # Deploy refit in shifted space
        te_shift = _deploy_te_one_seed(X_unb, y_shift, X_te, seed)
        per_seed_te_shift[i] = te_shift
        # Unshift -> residual space, then add anchor for corrected pEC50
    ...
    # Mean-bag in shifted space, then unshift
    ...
    mean_bag_te_shift = per_seed_te_shift.mean(axis=0)
    mean_bag_te_resid = mean_bag_te_shift - shift
    ...
    print(f"\n   per_seed_mean = {per_seed_mean:.4f}  std = {per_seed_std:.4f}")
    ...
    print(f"   te[unb_idx] in-sample = {te_unb_in_sample_rae:.4f}  "
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_lgbm_poisson.csv"
    pd.DataFrame({
        "SMILES": test_smiles,
    ...
    print("=" * 78)
    print(f"   mean_bag_rae        = {mean_rae:.4f}")
    ...
    summary = {
        "tag": TAG,
        "method": "lgbm_K20_NATIVE_POISSON_on_shifted_chemprop_aux_residual",
        "rationale": (
            "Native Poisson objective at the K=20 substrate, with shifted "
            "residual y_shift = residual - min(residual) + 0.1 to satisfy "
            "LightGBM's y >= 0 constraint. Poisson NLL has asymmetric "
            "variance-mean coupling and a log-link that biases leaf splits "
            "toward the high-magnitude residual tail -- aligned with the F2 "
            "greasy-novel-inactive over-prediction failure mode where the "
            "left tail of the residual distribution dominates LB cost."
        ),
        "anchor": ANCHOR,
        ...
        "mean_rae": mean_rae,  # alias for gate consumers
