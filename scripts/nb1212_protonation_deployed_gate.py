"""nb1212 — Protonation/titration-microstate scalars on the FULL DEPLOYED config
(AIMNet2 + MMFF strain + DFT-D4, best 0.4252).

9 protonation scalars (dimorphite-dl 2.x titration enumeration): pH-7.4 protomer
net/abs charge, microstate ambiguity at physio pH, titratable richness + net-charge
span over pH 4..10, charge-shift vs the naive neutral SMILES, and fraction-ionized.
Distinct thermodynamic axis: the deployed QM blocks are electronic (AIMNet2),
conformational (strain), and dispersion/polarizability (D4) — none see the pH-7.4
*ionization state*, which the neutral-SMILES ECFP in `combined` is also blind to.
Most PXR analogs are ionizable. Features at C:/pxr_work/protonation/prot_features.csv
(4652 rows, 0 NaN).

  control   = 4-GBM(combined) + CheMeleon + TabPFN + sn_oof + AIMNet2 + strain + DFT-D4
  treatment = control + 9 protonation scalars
...
PCOLS = ["prot_q74", "prot_qabs74", "prot_nstate_74", "prot_nstate_rng",
...
def main():
    ...
    Xpr, pr_nan = aligned_block(PR, PCOLS, tr["name"])
    print(f"AIMNet2 imp={qm_nan} strain imp={st_nan} d4 imp={d4_nan} prot imp={pr_nan}")
    ...
    for seed in range(N_SEEDS):
        ...
        scq = StandardScaler().fit(Xqm[trn]); Xqm_s = scq.transform(Xqm)
        scs = StandardScaler().fit(Xst[trn]); Xst_s = scs.transform(Xst)
        scd = StandardScaler().fit(Xd4[trn]); Xd4_s = scd.transform(Xd4)
        scp = StandardScaler().fit(Xpr[trn]); Xpr_s = scp.transform(Xpr)
        Xctrl = np.hstack([Xqm_s, Xst_s, Xd4_s])             # deployed best (0.4252)
        Xtreat = np.hstack([Xqm_s, Xst_s, Xd4_s, Xpr_s])     # + protonation
    ...
    print(f"treatment(deployed+protonation)           RAE = {tm:.4f} +/- {np.std(t_raes):.4f}")
    ...
    json.dump(out, open(f"{P}/nb1212_summary.json", "w"), indent=2)
    print(f"\nsaved {P}/nb1212_summary.json -> DEPLOY={deploy}")
