"""nb1153 -- Shallow residual-LGBM bag on nb1070 anchor, MORDRED features only.

Hypothesis:
    The depth=3 residual-LGBM bag in nb1130 (over nb1070) used combined
    Morgan(2048) + RDKit-desc(217) = 2265 features and gained ~-0.012 RAE.
    Mordred 1533 descriptors encode topology / autocorrelation / Burden /
    information-content channels that are NOT in RDKit-desc.  nb1093 showed
    Mordred-as-feature long-train hits Pearson(nb972) ~0.93 -- i.e. ~14%
    residual variance is orthogonal to the Morgan+RDKit manifold.  If that
    orthogonality survives at the *residual* layer, a shallow LGBM Huber on
    Mordred-only of (truth - nb1070_oof) extracts a different chunk of
    signal than nb1130 did.  Mean-bag cross-fit RAE < nb1070 anchor by a
    measurable margin implies blending nb1130 + nb1153 should compound.

Protocol per seed s in {0, 1, 7, 42, 137} (identical to nb1130 / nb1152):
  ...
  4. Shallow LGBM Huber (max_depth=3, num_leaves=7, n_est=80, lr=0.05,
     min_child_samples=20) on cached Mordred(1533) sliced to unblind rows;
...
Compare to nb1070 anchor (pooled RAE on 253) and to nb1130 mean-bag ref.

NO deploy (513) refit -- 253-only honest cross-fit diagnostic.

Outputs:
  data/processed/nb1153_per_seed_corrected_oof.npy  (5, 253) float32
  data/processed/nb1153_mean_bag_oof.npy            (253,) float32
  data/processed/nb1153_median_bag_oof.npy          (253,) float32
...
NB1130_MEAN_BAG_REF = 0.5673   # depth=3 Morgan+RDKit residual bag on nb1070
...
def _load_mordred_unblind(n_test_expected: int, unb_idx: np.ndarray) -> np.ndarray:
    ...
    # Median-impute residual NaNs / Infs (Mordred can emit them).
...
def main() -> dict:
    ...
    print(f"{TAG} -- SHALLOW residual-LGBM bag on top of nb1070, "
          f"MORDRED-only features, {len(RESID_SEEDS)} KFold seeds")
    ...
    print(f"[feat] X_unb shape = {X_unb.shape}  (Mordred only, no Morgan/RDKit)")

    print("\n" + "-" * 78)
    print("PER-SEED RESIDUAL CROSS-FIT (depth=3 shallow, Mordred 1533)")
    print("-" * 78)
    ...
    if beats_nb1130:
        verdict = "MORDRED_RESIDUAL_BEATS_NB1130_ORTHOGONAL_SIGNAL"
    elif beats_nb1070:
        verdict = "MORDRED_RESIDUAL_HELPS_NB1070_BUT_NOT_NB1130"
    elif abs(rae_mean_bag - rae_anchor) < 0.003:
        verdict = "MORDRED_RESIDUAL_FLAT_NO_NEW_SIGNAL"
    else:
        verdict = "MORDRED_RESIDUAL_HURTS_NB1070"
...
if __name__ == "__main__":
    ...
    for k in ("rae_anchor_nb1070", "per_seed_rae",
              ...
              "beats_nb1070", "beats_nb1130", "verdict"):
        print(f"  {k}: {res.get(k)}")
