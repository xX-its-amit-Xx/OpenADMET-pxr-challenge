"""nb3173 -- Learned per-fold optimal clip percentiles via grid search.

NEW PARADIGM:
    Generalizes nb3161/nb3170 fixed (q05, q95) clip by LEARNING per-fold
    (q_low, q_high) on fold-train, then applying to fold-val. This gives
    each fold flexibility to widen or tighten the clip band based on its
    own train-distribution shape.

    Hypothesis: nb3170 fixed q05/q95 hits 0.4437 across 15 seeds. Some
    folds may have heavier tails and benefit from q02/q98; others may have
    tighter cores and benefit from q10/q90. Per-fold learning should
    outperform the one-size-fits-all fixed band by ~0.001-0.005 RAE.
...
GATE (on 15-seed mean):
    mean < 0.4437 -> "BETTER"   (beats nb3170 fixed q05/q95)
    mean < 0.4475 -> "MARGINAL" (beats nb3080 parent but not nb3170)
    else          -> "FAIL"

References:
    nb3170 15-seed (fixed q05/q95)  = 0.4437 +/- 0.0009  <- target to beat
...
Outputs:
    data/processed/nb3173_summary.json
    data/processed/nb3173_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3173.npy         (513,) float32 -- deploy te
    submissions/nb3173_learned_clip.csv  (only on BETTER/MARGINAL verdict)
"""
...
# -- Gates ---------------------------------------------------------------------
GATE_BETTER = 0.4437   # mean < this -> BETTER  (beats nb3170 fixed)
GATE_MARGINAL = 0.4475  # mean < this -> MARGINAL (beats nb3080 parent)
...
def main() -> dict:
    ...
    # This is the analog of the per-fold pick but on the full unblind y as
    # the "train" set for the deploy refit.
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3173 15-seed mean {mean_rae:.4f} beats "
            f"nb3170 fixed-band {REF_NB3170_FIXED:.4f} "
            f"({mean_rae - REF_NB3170_FIXED:+.4f}). Per-fold LEARNED "
            f"(q_low, q_high) grid search adds real -"
            f"{abs(mean_rae - REF_NB3170_FIXED):.4f} on top of nb3170 fixed "
            f"q05/q95. Modal pick was (q{ql_mode:.2f}, q{qh_mode:.2f}). "
            f"Re-verify with deep-30 before PRIMARY-1 swap."
        )
    elif mean_rae < GATE_MARGINAL:
        verdict = "MARGINAL"
        ladder_action = (
            f"MARGINAL. nb3173 15-seed mean {mean_rae:.4f} beats parent "
            f"nb3080 {REF_NB3080:.4f} but does NOT beat nb3170 fixed-band "
            f"{REF_NB3170_FIXED:.4f} ({mean_rae - REF_NB3170_FIXED:+.4f}). "
            f"Grid-learning the per-fold band adds noise but no signal. "
            f"Keep nb3170 fixed q05/q95 as the clip primitive; do not "
            f"promote learned-clip variant."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3173 15-seed mean {mean_rae:.4f} fails to beat "
            f"parent nb3080 {REF_NB3080:.4f} ({mean_rae - REF_NB3080:+.4f}) "
            f"and nb3170 fixed-band {REF_NB3170_FIXED:.4f}. Per-fold grid "
            f"learning of (q_low, q_high) overfits the small inner training "
            f"set. Keep nb3170 / prior PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_learned_clip.csv"
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_nb3080_pred_oof"
        ),
