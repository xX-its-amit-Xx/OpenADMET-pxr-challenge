"""nb1181 deploy — ADD 8 MMFF conformational-strain scalars to the deployed config (honest gate WIN).

nb1181_strain_gate (deployed 4-GBM + CheMeleon + TabPFN + sn_oof + AIMNet2 control):
  control 0.4291  treatment(+strain) 0.4268  matched delta -0.0023 (2/3 seeds neg)
  treatment < deployed best 0.4297 -> DEPLOY.

Strain is MARGINAL-OVER-DEPLOYED, so the 4 GBMs train on combined + 9 AIMNet2 QM scalars
+ 8 strain scalars (both per-train StandardScaler) and predict the 513 with the same blocks.
GNN/CheMeleon/TabPFN members UNCHANGED.
"""
import os, sys, json
...
def main():
    ...
    Xex_tr = np.hstack([Xqm_tr, Xst_tr]); Xex_te = np.hstack([Xqm_te, Xst_te])
    print(f"train {len(tr)} | test {len(te)} | extra cols {Xex_tr.shape[1]} (AIMNet2 {len(ACOLS)} + strain {len(SCOLS)})", flush=True)
    ...
    out = "submissions/nb1181_strain_ensemble.csv"
    ...
    json.dump({"rae": 0.4268,
               "via": "+MMFF strain scalars on top of AIMNet2 in 4-GBM (matched -0.0023 vs deployed, nb1181_gate)",
               "submission": out,
               "members": [c["arch"] for c in topK] + ["sisterNR_gnn", "chemeleon", "tabpfn"],
               "qm_feats": ACOLS, "strain_feats": SCOLS},
              open(f"{SD}/best_ensemble.json", "w"), indent=2)
    print("updated best_ensemble.json -> 0.4268 (+strain)", flush=True)
