"""nb1941 -- 10-outer Bag-of-Bags of nb1861 (LGBM MSE) for tighter estimate.

HYPOTHESIS:
    nb1861 reported BoB MEAN/MEDIAN over 5 outer seeds for the
    LGBM(regression/MSE, max_depth=4, num_leaves=15, n_est=300, lr=0.03,
    min_child_samples=5, reg_lambda=2) on the 117-col 5-way K-tuned matrix
    with chemprop_aux te[unb_idx] PRE-unblind anchor.

    Tighter estimate: rerun the protocol with 10 outer seeds
    {0,1,2,7,42,99,137,250,500,750}; each outer seed runs a 5-inner-seed
    bag with 5-fold KFold cross-fit. Aggregate 10-outer BoB MEAN and
    MEDIAN, compare to nb1861's 5-outer 0.5013 reference.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1861/nb1852/nb1771/nb1554).
    ...
    5. Verdict vs nb1861 5-outer reference.

Outputs:
    scripts/nb1941_bag_nb1861_10outer.py
    data/processed/nb1941_summary.json
    data/processed/nb1941_per_outer_oof.npy   (10, 253) float32
    data/processed/nb1941_pooled_50bag_oof.npy (253,) float32
"""
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- mirrors nb1861/nb1852 exactly; all knobs fixed."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 10-OUTER Bag-of-Bags of nb1861 (LGBM regression/MSE) on "
    ...
    print(f"   nb1861 5-outer MEAN ref   = {nb1861_ref:.4f}")
    print(f"   nb1861 5-outer MEDIAN ref = {nb1861_median_ref:.4f}")
    print(f"   d(BoB_MEAN_10, nb1861_MEAN_5)     = "
          f"{bob_mean - nb1861_ref:+.4f}")
    print(f"   d(BoB_MEDIAN_10, nb1861_MEDIAN_5) = "
          f"{bob_median - nb1861_median_ref:+.4f}")

    # ---- Verdict ----
    delta_vs_nb1861 = bob_mean - nb1861_ref
    if abs(delta_vs_nb1861) < REPRODUCE_MARGIN:
        verdict = "NB1861_REPRODUCES"
    elif delta_vs_nb1861 < -REPRODUCE_MARGIN:
        verdict = "NB1861_REPRODUCES_BETTER"
    else:
        verdict = "NB1861_DOES_NOT_REPRODUCE"
    ...
    summary = {
        ...
        "delta_bob_median_vs_nb1861": bob_median - nb1861_median_ref,
        "verdict": verdict,
