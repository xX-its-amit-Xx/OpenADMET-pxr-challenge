"""nb1772 -- LightGBM dart boosting on K-tuned 5-way 117-col features.

HYPOTHESIS:
    Dropout-additive-regression-trees (DART) boosting can reduce overfit on
    the residual signal relative to GBDT.  The 5-way K-tuned 117-col stack
    (top-25 AP + top-20 MACCS + top-20 Mord + top-20 ChempropEmbed + top-30
    Avalon + 2 ChEMBL kNN cols) has been the strongest residual-feature
    container we have (nb1554 CatBoost rae_mean_bag = 0.5163 vs anchor
    0.6216).  We swap in LightGBM(boosting_type='dart', objective='huber',
    alpha=1.0) on the same 117-col matrix and check whether DART's stochastic
    tree dropout regularization shifts residual-OOF below nb1554.

PROTOCOL:
    1. Features  = 117-col 5-way K-tuned matrix
                   (top-25 AP + top-20 MACCS + top-20 Mord
                    + top-20 ChempropEmbed + top-30 Avalon
                    + pred_chembl_pec50 + mean_sim).
    2. Anchor    = chemprop_aux te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
                   residual = y_unb - anchor.
    3. Model     = LGBM(boosting_type='dart', objective='huber', alpha=1.0,
                        max_depth=4, num_leaves=15, n_estimators=200,
                        learning_rate=0.05, drop_rate=0.1).
                   5-seed bag, 5-fold cross-fit per seed.
    4. Verdict   = 0.003 margin vs nb1554 (0.5163); also report vs
                   anchor / nb1543 / nb1501.

Outputs:
    scripts/nb1772_lgbm_dart.py
    data/processed/nb1772_summary.json
    data/processed/nb1772_mean_bag_oof.npy        (253,) float32
"""
...
NB1501_REF = 0.5223
NB1543_REF = 0.5237
NB1554_REF = 0.5163  # CatBoost(MAE, d4, n200, lr0.05, l2=5) on same 117-col
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LightGBM DART (huber, alpha=1.0, d4, lvs15, n200, "
          f"lr0.05, drop=0.1) on 5-way K-tuned 117-col matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1543 ({NB1543_REF:.4f}), nb1554 ({NB1554_REF:.4f})  "
    ...
    # ---- Per-seed LGBM-DART residual cross-fit ----
    print("\n" + "-" * 78)
    print(f"PER-SEED LGBM-DART RESIDUAL CROSS-FIT (dim={feat_dim})")
    print("-" * 78)
    ...
    # ---- Verdict at 0.003 margin vs nb1554 ----
    ...
    if beats_nb1554:
        verdict = "LGBM_DART_5WAY_K_TUNED_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "LGBM_DART_5WAY_K_TUNED_FLAT_VS_NB1554"
    elif beats_nb1543:
        verdict = "LGBM_DART_5WAY_K_TUNED_BEATS_NB1543_BUT_WORSE_THAN_NB1554"
    elif flat_vs_nb1543:
        verdict = "LGBM_DART_5WAY_K_TUNED_FLAT_VS_NB1543"
    elif beats_nb1501:
        verdict = "LGBM_DART_5WAY_K_TUNED_BEATS_NB1501_BUT_WORSE_THAN_NB1543"
    elif flat_vs_nb1501:
        verdict = "LGBM_DART_5WAY_K_TUNED_FLAT_VS_NB1501"
    elif beats_anchor:
        verdict = "LGBM_DART_5WAY_K_TUNED_BEATS_ANCHOR_BUT_WORSE_THAN_NB1501"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "LGBM_DART_5WAY_K_TUNED_FLAT_VS_ANCHOR"
    else:
        verdict = "LGBM_DART_5WAY_K_TUNED_HURTS_ANCHOR"
