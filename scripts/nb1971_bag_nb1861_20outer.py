"""nb1971 -- 20-outer Bag-of-Bags of nb1861 (LGBM MSE) for tighter estimate.

HYPOTHESIS:
    nb1941 reported BoB MEAN/MEDIAN over 10 outer seeds (pooled 50-bag
    = 0.4990) for the LGBM(regression/MSE, max_depth=4, num_leaves=15,
    n_est=300, lr=0.03, min_child_samples=5, reg_lambda=2) on the 117-col
    5-way K-tuned matrix with chemprop_aux te[unb_idx] PRE-unblind anchor.

    Tighter estimate: rerun the protocol with 20 outer seeds
    {0,1,2,3,7,11,42,99,137,250,500,750,1000,1234,2026,5,17,99,888,999};
    each outer seed runs a 5-inner-seed bag with 5-fold KFold cross-fit.
    Aggregate 20-outer BoB MEAN and MEDIAN; compare to nb1941's 10-outer
    reference.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1861/nb1941).
    3. For each outer seed s in OUTER_SEEDS:
         inner_seeds = [s*1000 + j for j in 0..4]
         per-inner: 5-fold cross-fit LGBM(regression/MSE, ...) on residual
         mean_bag_outer = mean of 5 inner OOFs, added back to anchor
         rae_outer = rae(y_unb, mean_bag_outer)
    4. Report per-outer RAE list, BoB MEAN, BoB MEDIAN.
    5. Verdict vs nb1941 10-outer reference.

Outputs:
    scripts/nb1971_bag_nb1861_20outer.py
    data/processed/nb1971_summary.json
    data/processed/nb1971_per_outer_oof.npy   (20, 253) float32
    data/processed/nb1971_pooled_100bag_oof.npy (253,) float32
"""
...
# Exactly as specified by the user (note: 99 appears twice in the spec).
OUTER_SEEDS = [0, 1, 2, 3, 7, 11, 42, 99, 137, 250,
               500, 750, 1000, 1234, 2026, 5, 17, 99, 888, 999]
INNER_PER_OUTER = 5
RESID_FOLDS = 5
...
NB1941_SUMMARY = DATA_PROCESSED / "nb1941_summary.json"
...
NB1554_REF = 0.5163
NB1771_REF = 0.5100
NB1861_REF_FALLBACK = 0.5013   # nb1861 5-outer BoB MEAN
NB1941_REF_FALLBACK = 0.4990   # nb1941 pooled 50-bag
REPRODUCE_MARGIN = 0.005
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- mirrors nb1861/nb1941 exactly; all knobs fixed."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- 20-OUTER Bag-of-Bags of nb1861 (LGBM regression/MSE) on "
          f"5-way K-tuned 117-col matrix; PRE-unblind anchor={ANCHOR}")
    ...
    print(f"          reproduce_margin = {REPRODUCE_MARGIN}")
    print("=" * 78)

    # ---- nb1861 / nb1941 reference ----
    if NB1861_SUMMARY.exists():
        with open(NB1861_SUMMARY) as f:
            nb1861_sum = json.load(f)
        nb1861_ref = float(nb1861_sum.get("bob_mean_rae", NB1861_REF_FALLBACK))
        nb1861_median_ref = float(
            nb1861_sum.get("bob_median_rae", NB1861_REF_FALLBACK)
        )
        print(f"[ref] nb1861_summary.bob_mean_rae   = {nb1861_ref:.4f}")
        print(f"[ref] nb1861_summary.bob_median_rae = {nb1861_median_ref:.4f}")
    else:
        nb1861_ref = NB1861_REF_FALLBACK
        nb1861_median_ref = NB1861_REF_FALLBACK
        print(f"[ref] nb1861_summary.json missing -- using fallback "
              f"{nb1861_ref:.4f}")

    if NB1941_SUMMARY.exists():
        with open(NB1941_SUMMARY) as f:
            nb1941_sum = json.load(f)
        nb1941_mean_ref = float(nb1941_sum.get("bob_mean_rae", NB1941_REF_FALLBACK))
        nb1941_median_ref = float(
            nb1941_sum.get("bob_median_rae", NB1941_REF_FALLBACK)
        )
        nb1941_pooled_ref = float(
            nb1941_sum.get("rae_pooled_50bag", NB1941_REF_FALLBACK)
        )
        print(f"[ref] nb1941_summary.bob_mean_rae       = {nb1941_mean_ref:.4f}")
        print(f"[ref] nb1941_summary.bob_median_rae     = {nb1941_median_ref:.4f}")
        print(f"[ref] nb1941_summary.rae_pooled_50bag   = {nb1941_pooled_ref:.4f}")
    else:
        nb1941_mean_ref = NB1941_REF_FALLBACK
        nb1941_median_ref = NB1941_REF_FALLBACK
        nb1941_pooled_ref = NB1941_REF_FALLBACK
        print(f"[ref] nb1941_summary.json missing -- using fallback "
              f"{nb1941_pooled_ref:.4f}")
    ...
    for o_i, o_seed in enumerate(OUTER_SEEDS):
        ...
        print(f"   ==> outer_pos={o_i:2d}  outer={o_seed:4d}  "
              f"rae_outer_mean_bag={rae_o:.4f}  "
    ...
    # Pooled corrected oof across all 20 outer x 5 inner = 100 cross-fits
    n_total_bag = len(OUTER_SEEDS) * INNER_PER_OUTER
    pooled_oof = per_outer_oof.mean(axis=0)
    rae_pooled = float(rae(y_unb, pooled_oof))

    print("\n" + "-" * 78)
    print(f"BAG-OF-BAGS AGGREGATIONS ({len(OUTER_SEEDS)} outer)")
    print("-" * 78)
    ...
    print(f"   BoB std/min/max     = {bob_std:.4f} / {bob_min:.4f} / "
          f"{bob_max:.4f}")
    print(f"   pooled {n_total_bag}-bag RAE  = {rae_pooled:.4f}")
    print(f"   nb1941 10-outer MEAN      = {nb1941_mean_ref:.4f}")
    print(f"   nb1941 10-outer MEDIAN    = {nb1941_median_ref:.4f}")
    print(f"   nb1941 pooled 50-bag      = {nb1941_pooled_ref:.4f}")
    print(f"   d(BoB_MEAN_20, nb1941_MEAN_10)     = "
          f"{bob_mean - nb1941_mean_ref:+.4f}")
    print(f"   d(BoB_MEDIAN_20, nb1941_MEDIAN_10) = "
          f"{bob_median - nb1941_median_ref:+.4f}")
    print(f"   d(pooled_100, nb1941_pooled_50)    = "
          f"{rae_pooled - nb1941_pooled_ref:+.4f}")

    # ---- Verdict ----
    delta_vs_nb1941_mean = bob_mean - nb1941_mean_ref
    delta_vs_nb1941_pooled = rae_pooled - nb1941_pooled_ref
    if abs(delta_vs_nb1941_mean) < REPRODUCE_MARGIN:
        verdict = "NB1941_REPRODUCES"
    elif delta_vs_nb1941_mean < -REPRODUCE_MARGIN:
        verdict = "NB1941_TIGHTENS_BETTER"
    else:
        verdict = "NB1941_TIGHTENS_WORSE"
    ...
    np.save(DATA_PROCESSED / f"{TAG}_pooled_{n_total_bag}bag_oof.npy",
            pooled_oof.astype(np.float32))
    print(f"[save] {DATA_PROCESSED / f'{TAG}_pooled_{n_total_bag}bag_oof.npy'}")
    ...
    summary = {
        ...
        "delta_bob_mean_vs_nb1861": bob_mean - nb1861_ref,
        "delta_bob_mean_vs_nb1941": delta_vs_nb1941_mean,
        "delta_bob_median_vs_nb1941": bob_median - nb1941_median_ref,
        "delta_pooled_vs_nb1941_pooled": delta_vs_nb1941_pooled,
        "verdict": verdict,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb1554_ref": NB1554_REF,
        "nb1771_ref": NB1771_REF,
        "nb1861_ref_mean": nb1861_ref,
        "nb1861_ref_median": nb1861_median_ref,
        "nb1941_ref_mean": nb1941_mean_ref,
        "nb1941_ref_median": nb1941_median_ref,
        "nb1941_ref_pooled": nb1941_pooled_ref,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "outer_seeds", "n_outer", "inner_per_outer", "resid_folds",
        "n_total_bag",
        ...
        "delta_bob_mean_vs_nb1554",
        "delta_bob_mean_vs_nb1771",
        "delta_bob_mean_vs_nb1861",
        "delta_bob_mean_vs_nb1941",
        "delta_bob_median_vs_nb1941",
        "delta_pooled_vs_nb1941_pooled",
        "nb1861_ref_mean", "nb1861_ref_median",
        "nb1941_ref_mean", "nb1941_ref_median", "nb1941_ref_pooled",
