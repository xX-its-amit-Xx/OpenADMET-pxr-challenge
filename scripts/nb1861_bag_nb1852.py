"""nb1861 -- Outer-bag of nb1852 (LGBM MSE / regression) for honest validation.

HYPOTHESIS:
    nb1852 reported rae_mean_bag using a 5-inner-seed bag of LGBM(
    objective='regression' (MSE), max_depth=4, num_leaves=15, n_est=300, lr=0.03,
    min_child_samples=5, reg_lambda=2) on the 117-col 5-way K-tuned matrix with
    chemprop_aux te[unb_idx] as PRE-unblind anchor.

    Outer-bag-of-bags (BoB) honest validation:
      For each of 5 outer seeds {0,1,7,42,137}:
          run a 5-inner-seed bag = {seed_outer * 1000 + j for j in 0..4}
          5-fold KFold cross-fit per inner seed (shuffle=True, random_state=inner)
          mean_bag_oof_outer = mean across 5 inner seeds (corrected anchor)
          rae_outer = rae(y_unb, mean_bag_oof_outer)
      Aggregate:
          BoB MEAN   RAE = mean(rae_outer over 5)
          BoB MEDIAN RAE = median(rae_outer over 5)
      Verdict NB1852_REPRODUCES iff |BoB MEAN - nb1852_rae_mean_bag| < 0.005.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 117-col feature stack (identical to nb1852/nb1771/nb1554).
    ...
    5. Verdict vs nb1852 reference.

Outputs:
    scripts/nb1861_bag_nb1852.py
    data/processed/nb1861_summary.json
    data/processed/nb1861_per_outer_oof.npy   (5, 253) float32
    data/processed/nb1861_pooled_25bag_oof.npy (253,) float32
"""
...
NB1554_REF = 0.5163
NB1771_REF = 0.5100
NB1852_REF_FALLBACK = 0.5100  # use nb1852 summary if present, else this
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- mirrors nb1852 exactly; all knobs fixed."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- OUTER-BAG of nb1852 (LGBM regression/MSE) on 5-way K-tuned "
    ...
    print(f"   d(BoB_MEAN, nb1852) = {bob_mean - nb1852_ref:+.4f}")

    # ---- Verdict ----
    delta_vs_nb1852 = bob_mean - nb1852_ref
    if abs(delta_vs_nb1852) < REPRODUCE_MARGIN:
        verdict = "NB1852_REPRODUCES"
    elif delta_vs_nb1852 < -REPRODUCE_MARGIN:
        verdict = "NB1852_REPRODUCES_BETTER"
    else:
        verdict = "NB1852_DOES_NOT_REPRODUCE"
    ...
    summary = {
        ...
        "delta_bob_mean_vs_nb1852": delta_vs_nb1852,
        "verdict": verdict,
