"""nb3082 -- Sigmoid-smooth quantile blend on 2-anchor pool {K18, K19}.

NEW PARADIGM (vs nb3070 hard-split + nb3071 linear-rank):
    Replace both the binary hard split at q50 (nb3070) and the linear-rank
    schedule (nb3071) with a *sigmoid-smooth* schedule on the K18 prediction
    VALUE (not rank). Hypothesis: a smooth, value-anchored transition tracks
    the bias gradient K18 -> K19 more accurately than rank-based interpolation
    because the bias structure is in pEC50 space, not rank space.

    Per-fold schedule (q50, iqr computed on the fold-TRAIN K18 predictions):
        z = (K18_pred - q50) / iqr
        w_K18 = 0.9 - 0.4 * sigmoid(10 * z)
        # K18_pred << q50 (z->-inf): w_K18 -> 0.90, w_K19 -> 0.10
        # K18_pred == q50 (z=0):     w_K18 = 0.70, w_K19 = 0.30
        # K18_pred >> q50 (z->+inf): w_K18 -> 0.50, w_K19 -> 0.50

PROTOCOL (per kf_seed, 5-fold scaffold split):
    1. In fold-train: compute q50, iqr of K18 predictions.
    2. In fold-val: compute z = (K18_val - q50) / iqr.
    3. w_K18 = 0.9 - 0.4 * sigmoid(10 * z).
    ...
    Deploy:
        - Compute q50, iqr from K18 te predictions (full 513).
        - z = (K18_te - q50) / iqr, w_K18 = 0.9 - 0.4 * sigmoid(10 * z).
        - Blend, clip [3, 9].
...
References:
    ...
    nb3070 hard-split q50         = 0.4477 <- CEILING TO BEAT
    nb3071 soft (linear-rank)     = ?
...
Outputs:
    data/processed/nb3082_summary.json
    data/processed/nb3082_pred_oof.npy  (253,) float32 -- median-seed OOF
    data/processed/te_nb3082.npy        (513,) float32 -- deploy te
    submissions/nb3082_sigmoid_smooth_blend.csv (only on PROMOTE verdict)
"""
...
# z = (K18_pred - q50) / iqr  (fold-train computed q50, iqr)
W_BASE_HI = 0.90    # asymptote at z -> -inf (low K18, K18 trusted more)
W_SPREAD = 0.40     # range of variation, so asymptote at z -> +inf is 0.50
SIGMOID_GAIN = 10.0 # steepness factor

# -- Gates ---------------------------------------------------------------------
GATE_BETTER_THAN_NB3070 = 0.4477  # mean < this -> BETTER_THAN_NB3070
...
def _blend_sigmoid_smooth(
    p_k18: np.ndarray,
    p_k19: np.ndarray,
    q50: float,
    iqr: float,
) -> tuple[np.ndarray, np.ndarray]:
    """Per-row sigmoid-smooth blend keyed on K18 prediction value.

    w_K18 = W_BASE_HI - W_SPREAD * sigmoid(SIGMOID_GAIN * (p_k18 - q50) / iqr)
...
def _run_one_seed(
    ...
    """Run sigmoid-smooth blend pipeline at a single kf_seed."""
    ...
    for fold_i, (tr_loc, va_loc) in enumerate(splits):
        ...
        val_pred, val_w = _blend_sigmoid_smooth(val_p_k18, val_p_k19, q50, iqr)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Sigmoid-smooth quantile blend on {K_LABELS} deep-30")
    ...
    print(f"          asymptotes: z->-inf w_K18={W_BASE_HI:.2f}; "
          f"z=0 w_K18={W_BASE_HI - 0.5*W_SPREAD:.2f}; "
    ...
    print(f"\n   full-pool K18 q50 = {full_q50:.4f}  iqr = {full_iqr:.4f}")
    ...
    for s in KF_SEEDS:
        ...
        print(f"   kf={s}: pooled_RAE={res['pooled_rae']:.4f}  "
              ...
              f"q50={res['fold_q50_mean']:.3f} iqr={res['fold_iqr_mean']:.3f}  "
    ...
    print(f"\n   ref nb3070 hard-split ceiling = {REF_NB3070:.4f}")
    print(f"   delta vs nb3070               = {mean_rae - REF_NB3070:+.4f}")

    # -- Deploy: use full-513 K18 q50/iqr for sigmoid centering --------------
    te_k18 = P_te[:, 0]
    te_k19 = P_te[:, 1]
    ...
    te_pred_raw, te_w_k18 = _blend_sigmoid_smooth(te_k18, te_k19, te_q50, te_iqr)
    te_pred = np.clip(te_pred_raw.astype(np.float32), 3.0, 9.0)
    te_unb_in_rae = float(rae(y_unb, te_pred[unb_idx]))
    print(f"\n   deploy q50={te_q50:.3f}  iqr={te_iqr:.3f}")
    ...
    if mean_rae < GATE_BETTER_THAN_NB3070:
        ...
        ladder_action = (
            f"PROMOTE candidate. nb3082 sigmoid-smooth blend mean "
            f"{mean_rae:.4f} beats nb3070 ceiling {REF_NB3070:.4f} "
            f"({mean_rae - REF_NB3070:+.4f}). "
            "Value-anchored sigmoid schedule extracts gain hard-split missed; "
            "re-verify with wider seed sweep before deploy."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3082 sigmoid-smooth mean {mean_rae:.4f} "
            f"not better than nb3070 ceiling {REF_NB3070:.4f} "
            f"({mean_rae - REF_NB3070:+.4f}). "
            "Value-anchored sigmoid schedule does not break hard-split "
            "ceiling on this anchor pair. Keep nb3070 PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_sigmoid_smooth_blend.csv"
    ...
    summary = {
        ...
        "method": "sigmoid_smooth_value_quantile_blend_K18_K19_deep30",
        ...
        "schedule": {
            ...
            "w_K18_at_q50": round(W_BASE_HI - 0.5 * W_SPREAD, 4),
