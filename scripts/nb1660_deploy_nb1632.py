"""nb1660 -- DEPLOY nb1632 outer-bag-of-bags (BoB) blend to 513-row CSV.

HANDOFF FROM nb1632 (PRE-unblind cross-fit verdict):
    BoB MEAN     pooled RAE = 0.5107
    BoB MEDIAN   pooled RAE = 0.5118
    per-outer    blend_o    = [0.5139, 0.5170, 0.5118, 0.5108, 0.5137]
    blend_o      = 0.55 * nb1561_o + 0.45 * nb1612_o
    nb1612_o     = per-outer best-of(naive 1/6 mean, SLSQP-5fold cross-fit)
                   weights/variant taken from nb1632_summary.json (locked)
...
PROTOCOL
    For each outer seed o in {0,1,7,42,137}:
        ...
        nb1612_o deploy (6-family residual blend, ChemBERTa K=50):
            ...
            Pull per-outer locked recipe from nb1632_summary.json:
    ...
    Row-level BoB MEAN  + MEDIAN across outers.

    te_nb1660_mean   = bob_mean_513
    te_nb1660_median = bob_median_513
    (Both already include the anchor since each per-outer blend does.)

OUTPUTS
    scripts/nb1660_deploy_nb1632.py
    data/processed/te_nb1660_mean.npy            (513,) float32
    data/processed/te_nb1660_median.npy          (513,) float32
    data/processed/nb1660_summary.json
    submissions/nb1660_deploy_nb1632_mean.csv    SMILES, Molecule Name, pEC50
    submissions/nb1660_deploy_nb1632_median.csv  SMILES, Molecule Name, pEC50

Honest LB anchors: 0.5107 mean / 0.5118 median (PRE-unblind, predicted LB ~0.514).
"""
...
# Outer/inner schedule -- locked to nb1632.
OUTER_SEEDS = [0, 1, 7, 42, 137]
INNER_OFFSETS = [0, 1, 7, 42, 137]

# Blend weights -- locked to nb1622/nb1632 best grid w.
W_NB1561 = 0.55
W_NB1612 = 1.0 - W_NB1561

# nb1632 BoB cross-fit anchors.
HONEST_LB_ANCHOR_MEAN = 0.5107
HONEST_LB_ANCHOR_MEDIAN = 0.5118

# nb1612 K-tuning (matches nb1612 / nb1623 / nb1632 recipe).
TOP_K_NB1612 = {
    "AtomPair":      25,
    "MACCS":         20,
    "Mordred":       20,
...
# nb1561 5-way K-tuned indices (same summaries used by nb1561/nb1570/nb1632).
...
# nb1632 per-outer locked recipe (variant + SLSQP weights).
NB1632_SUMMARY = DATA_PROCESSED / "nb1632_summary.json"
...
def _family_resid_513_5seed_bag(
    ...
    """SHAP-prune per outer (same recipe as nb1632._run_family_nb1612), then
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1632 BoB blend (5 outer x [nb1561_o + nb1612_o]) -> 513")
    ...
    # ---- nb1632 per-outer locked recipe ----
    if not NB1632_SUMMARY.exists():
        raise FileNotFoundError(f"nb1632 summary missing: {NB1632_SUMMARY}")
    with open(NB1632_SUMMARY) as f:
        sum_1632 = json.load(f)
    per_outer_lock = {
        int(rec["outer_seed"]): {
        ...
        for rec in sum_1632["per_outer_records"]
    }
    print("\n[nb1632-lock] per-outer nb1612 variant + SLSQP-mean-over-folds w:")
    for o in OUTER_SEEDS:
        rec = per_outer_lock[int(o)]
        print(f"   outer {o:3d}  variant={rec['best_variant']:<16s}  "
    ...
    # ---- Outer-bag deploy ----
    print("\n" + "=" * 78)
    print("OUTER-BAG DEPLOY x 5  [nb1561_o = 5-inner CatBoost bag (513-resid)]")
    ...
    # ---- BoB MEAN + MEDIAN over 5 outer blend_o vectors (513,) ----
    bob_mean_513 = outer_blend_513.mean(axis=0)
    bob_median_513 = np.median(outer_blend_513, axis=0)
    te_nb1660_mean = bob_mean_513
    te_nb1660_median = bob_median_513

    in_rae_mean = float(rae(y_unb, te_nb1660_mean[unb_idx]))
    in_rae_median = float(rae(y_unb, te_nb1660_median[unb_idx]))
    ...
    print("\n" + "=" * 78)
    print("BoB DEPLOY SUMMARY  (5 outer-seed blend stack -> row-level MEAN/MEDIAN)")
    print("=" * 78)
    ...
    print()
    print(f"   te_nb1660_mean    mean={te_nb1660_mean.mean():.4f}  "
          f"std={te_nb1660_mean.std():.4f}  "
          f"min={te_nb1660_mean.min():.4f}  "
          f"max={te_nb1660_mean.max():.4f}")
    print(f"   te_nb1660_median  mean={te_nb1660_median.mean():.4f}  "
          f"std={te_nb1660_median.std():.4f}  "
          f"min={te_nb1660_median.min():.4f}  "
          f"max={te_nb1660_median.max():.4f}")
    ...
    print(f"   d_median_vs_anchor = {in_rae_median - rae_anchor:+.4f}")
    ...
    np.save(te_mean_path, te_nb1660_mean.astype(np.float32))
    np.save(te_median_path, te_nb1660_median.astype(np.float32))
    print(f"\n[save] {te_mean_path}")
    print(f"[save] {te_median_path}")

    # ---- Save CSVs ----
    SUBMISSIONS.mkdir(parents=True, exist_ok=True)
    csv_mean_path = SUBMISSIONS / f"{TAG}_deploy_nb1632_mean.csv"
    csv_median_path = SUBMISSIONS / f"{TAG}_deploy_nb1632_median.csv"
    df_mean = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1660_mean.astype(np.float64),
    })
    df_median = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1660_median.astype(np.float64),
    })
    ...
    summary = {
        ...
        "nb1632_per_outer_lock_source": str(NB1632_SUMMARY),
        ...
        "te_nb1660_mean_stats": {
            "mean": float(te_nb1660_mean.mean()),
            "std": float(te_nb1660_mean.std()),
            "min": float(te_nb1660_mean.min()),
            "max": float(te_nb1660_mean.max()),
        },
        "te_nb1660_median_stats": {
            "mean": float(te_nb1660_median.mean()),
            "std": float(te_nb1660_median.std()),
            "min": float(te_nb1660_median.min()),
            "max": float(te_nb1660_median.max()),
        },
        ...
        "honest_lb_anchor_mean_nb1632": HONEST_LB_ANCHOR_MEAN,
        "honest_lb_anchor_median_nb1632": HONEST_LB_ANCHOR_MEDIAN,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "te_nb1660_mean_stats", "te_nb1660_median_stats",
        ...
        "honest_lb_anchor_mean_nb1632",
        "honest_lb_anchor_median_nb1632",
