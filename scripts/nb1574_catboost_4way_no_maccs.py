"""nb1574 -- CatBoost on K-tuned 4-way 97-col features (MACCS DROPPED).

HYPOTHESIS:
    Per nb1553/nb1554 results, MACCS gets 0% SLSQP weight.  Drop MACCS to
    reduce overfit on the 253 unblind.  Test if a leaner 4-way recipe
    (top-25 AP + top-20 Mord + top-20 ChempropEmbed + top-30 Avalon + 2
    ChEMBL kNN features = 97 cols) outperforms the 5-way 117-col CatBoost
    (nb1554 mean-bag RAE 0.5163) and the nb1561 BoB sibling (0.5155).

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx] (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Build 97-col feature stack (NO MACCS):
    ...
    5. Verdict at 0.003 margin vs nb1554 (0.5163) and nb1561 BoB (0.5155).

Outputs:
    scripts/nb1574_catboost_4way_no_maccs.py
    data/processed/nb1574_summary.json
    data/processed/nb1574_mean_bag_oof.npy        (253,) float32
"""
...
NB1561_BOB_REF = 0.5155  # nb1554 BoB (Best-of-Bag) sibling
...
def main() -> dict:
    ...
    print(f"{TAG} -- CatBoost(MAE, d4, n200, lr0.05, l2=5) on 4-way K-tuned "
          f"97-col matrix (NO MACCS); PRE-unblind anchor={ANCHOR}")
    ...
    for p in (NB1392_SUMMARY, NB1484_SUMMARY,
    ...
    # ---- Build COMBINED 4-way K-tuned feature matrix (NO MACCS) ----
    ...
    print(f"\n   COMBINED 4-WAY K-TUNED matrix (NO MACCS): {X_unb.shape}  "
    ...
    if beats_nb1561_bob:
        verdict = "CATBOOST_4WAY_NO_MACCS_BEATS_NB1561_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1561_bob:
        verdict = "CATBOOST_4WAY_NO_MACCS_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = "CATBOOST_4WAY_NO_MACCS_BEATS_NB1554_BUT_WORSE_THAN_NB1561_BOB"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_4WAY_NO_MACCS_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_4WAY_NO_MACCS_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_4WAY_NO_MACCS_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_4WAY_NO_MACCS_HURTS_ANCHOR"
