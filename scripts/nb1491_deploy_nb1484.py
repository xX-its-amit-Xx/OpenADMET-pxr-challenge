"""nb1491 -- Deploy nb1484 (PRE-unblind 4-way SHAP-pruned residual blend) to a
...
Blend     : te_nb1491 = chemprop_aux
                + (resid_AP + resid_MACCS + resid_Mordred + resid_Embed) / 4

Honest LB anchor: nb1484 naive 1/4 cross-fit RAE = 0.5257
                  nb1484 best (slsqp 5-fold) cross-fit RAE = 0.5231
"""
...
SUB_OUT = SUB_DIR / f"{TAG}_deploy_nb1484.csv"
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Deploy nb1484 PRE-unblind 4-way naive 1/4 mean to 513 CSV")
    ...
    # ---- nb1484 SHAP-pruned indices per family ----
    ...
    print("\n" + "=" * 78)
    print("NAIVE 1/4 MEAN BLEND ACROSS 4 FAMILY RESIDUALS")
    print("=" * 78)
    resid_stack = np.stack(residual_deploys, axis=0)  # (4, 513)
    ...
    te_nb1491 = te_anchor_513 + mean_resid_513
    print(f"[te]  te_{TAG}: mean={te_nb1491.mean():.4f}  "
          f"std={te_nb1491.std():.4f}  "
          f"min={te_nb1491.min():.4f}  max={te_nb1491.max():.4f}")

    in_RAE_unb = float(rae(y_unb, te_nb1491[unb_idx]))
    ...
    np.save(TE_OUT, te_nb1491.astype(np.float32))
    print(f"[save] {TE_OUT}")
    ...
    out = pd.DataFrame({
        ...
        "pEC50": te_nb1491.astype(np.float64),
    })
    if len(out) != n_test:
    ...
    summary = {
        ...
        "te_mean": float(te_nb1491.mean()),
        "te_std": float(te_nb1491.std()),
        "te_min": float(te_nb1491.min()),
        "te_max": float(te_nb1491.max()),
        ...
        "predicted_LB": naive_crossfit_RAE + 0.003,
        "te_path": str(TE_OUT),
