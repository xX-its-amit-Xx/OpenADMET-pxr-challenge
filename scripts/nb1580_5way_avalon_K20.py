"""nb1580 -- PRE-unblind 5-way K-tuned with Avalon UPGRADED to K=20.

Upgrade from nb1553: drop Avalon K=30 -> Avalon K=20.
    ...
    Avalon        K=20   (UPGRADED from K=30 in nb1553)
...
PROTOCOL
    ...
    6. Pool RAE for both.
    7. Verdict at 0.003 margin vs nb1553 ref (0.5225 naive).

Outputs:
    scripts/nb1580_5way_avalon_K20.py            (this file)
    data/processed/nb1580_summary.json
    data/processed/nb1580_best_oof.npy            (253,) float32 best variant
"""
...
NB1484_REF = 0.5231
NB1492_REF = 0.5260
NB1553_REF = 0.5225  # prior K-tuned 5-way naive 1/5 (this run's primary ref)
DECISION_MARGIN = 0.003

# All K values tuned per-family from K-grid runs (Avalon UPGRADED to 20)
TOP_K = {
    "AtomPair": 25,       # nb1524 best_K = 25
    "MACCS": 20,          # nb1424 (user override) = 20
    "Mordred": 20,        # nb1523 best_K = 20
    "ChempropEmbed": 20,  # nb1541 best_K = 20
    "Avalon": 20,         # UPGRADED from K=30 in nb1553
}
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1553/nb1492/nb1484/nb1472."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- PRE-unblind 5-way K-TUNED (Avalon UPGRADED K=20): "
          f"chemprop_aux anchor + "
          f"AtomPair-25 + MACCS-20 + Mordred-20 + ChempropEmbed-20 + Avalon-20")
    ...
    print(f"          refs: chemprop_aux={CHEMPROP_AUX_REF:.4f}  "
          ...
          f"nb1492={NB1492_REF:.4f}  nb1553={NB1553_REF:.4f}")
    ...
    print(f"   nb1553 ref (primary)         = {NB1553_REF:.4f}")
    print(f"   naive 1/5 mean blend RAE     = {rae_mean:.4f}")
    print(f"   5-fold SLSQP cross-fit RAE   = {rae_slsqp:.4f}")
    ...
    beats_nb1553_naive = rae_mean < NB1553_REF - DECISION_MARGIN
    beats_nb1553_slsqp = rae_slsqp < NB1553_REF - DECISION_MARGIN
    beats_nb1553_best = rae_best < NB1553_REF - DECISION_MARGIN
    flat_vs_nb1553_best = abs(rae_best - NB1553_REF) < DECISION_MARGIN
    ...
    if beats_nb1553_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_BEATS_NB1553_NEW_PRIMARY_via_{which_best}"
        )
    elif flat_vs_nb1553_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_FLAT_VS_NB1553_best_{which_best}"
        )
    elif beats_nb1492_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_BEATS_NB1492_BUT_WORSE_THAN_NB1553"
            f"_best_{which_best}"
        )
    elif beats_nb1484_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_BEATS_NB1484_BUT_WORSE_THAN_NB1492"
            f"_best_{which_best}"
        )
    elif beats_nb1472_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_BEATS_NB1472_BUT_WORSE_THAN_NB1484"
            f"_best_{which_best}"
        )
    elif beats_anchor_best:
        verdict = (
            f"PRE_UNBLIND_5WAY_AVALON_K20_BEATS_ANCHOR_BUT_WORSE_THAN_NB1472"
            f"_best_{which_best}"
        )
    elif abs(rae_best - rae_anchor) < DECISION_MARGIN:
        verdict = "PRE_UNBLIND_5WAY_AVALON_K20_FLAT_VS_ANCHOR"
    else:
        verdict = "PRE_UNBLIND_5WAY_AVALON_K20_HURTS_ANCHOR"

    print(f"   d_naive_vs_nb1553   = {delta_naive_vs_nb1553:+.4f}")
    print(f"   d_slsqp_vs_nb1553   = {delta_slsqp_vs_nb1553:+.4f}")
    print(f"   d_best_vs_nb1553    = {delta_best_vs_nb1553:+.4f}   "
          f"(best = {which_best})")
    print(f"   beats_nb1553_naive  = {beats_nb1553_naive}")
    print(f"   beats_nb1553_slsqp  = {beats_nb1553_slsqp}")
    print(f"   beats_nb1553_best   = {beats_nb1553_best}")
    ...
    summary = {
        ...
        "beats_nb1553_naive": bool(beats_nb1553_naive),
        "beats_nb1553_slsqp": bool(beats_nb1553_slsqp),
        "beats_nb1553_best": bool(beats_nb1553_best),
        "flat_vs_nb1553_best": bool(flat_vs_nb1553_best),
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_naive_vs_nb1553",
        "delta_slsqp_vs_nb1553",
        "delta_best_vs_nb1553",
        "beats_nb1553_naive",
        "beats_nb1553_slsqp",
        "beats_nb1553_best",
        "flat_vs_nb1553_best",
