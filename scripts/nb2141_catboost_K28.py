"""nb2141 -- CatBoost(RMSE) on SHAP top-28 features.

HYPOTHESIS:
    nb2103 found K=28 mean-bag RAE = 0.4737 as the SHAP-pruning optimum on the
    117-col 5-way K-tuned feature matrix using LGBM(MSE) (max_depth=4,
    num_leaves=15, n_estimators=300, lr=0.03, min_child=5, reg_lambda=2.0,
    5-seed bag, 5-fold cross-fit).  This notebook swaps the LGBM(MSE) residual
    learner for CatBoost(RMSE) at the same top-28 SHAP-pruned matrix to test
    whether a different tree booster family (ordered boosting + symmetric trees)
    extracts new signal vs the LGBM baseline.

PROTOCOL:
    1. Reuse the same 117-col 5-way K-tuned matrix and the nb2063 cached SHAP
       importance vector.  Slice top-28 indices.
    2. CatBoost(loss_function='RMSE', depth=4, n_estimators=200,
       learning_rate=0.05, l2_leaf_reg=5), 5-seed bag (seeds 0, 1, 7, 42, 137),
       5-fold KFold cross-fit per seed.
    3. Report per-seed RAE, mean-bag RAE, median-bag RAE, family breakdown.
    4. Verdict vs nb2103 K=28 lr=0.03 (0.4737) at decision_margin=0.003.

Outputs:
    scripts/nb2141_catboost_K28.py
    data/processed/nb2141_summary.json
    data/processed/nb2141_mean_bag_oof.npy   (n_unb,) float32
"""
...
# CatBoost hyperparameters (per protocol)
CB_DEPTH = 4
CB_N_EST = 200
CB_LR = 0.05
CB_L2 = 5
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb2063/nb2081/nb2091/nb2103/nb2133."""
    frames = []
...
def _catboost_params(seed: int) -> dict:
    """CatBoost(RMSE) -- depth=4, n_est=200, lr=0.05, l2=5."""
    return dict(
        loss_function="RMSE",
        depth=CB_DEPTH,
        n_estimators=CB_N_EST,
        learning_rate=CB_LR,
        l2_leaf_reg=CB_L2,
        random_seed=seed,
        thread_count=2,
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(RMSE) K={K_FIXED} depth={CB_DEPTH} "
          f"n_est={CB_N_EST} lr={CB_LR} l2={CB_L2}")
    ...
    # ---- Load all K-grid winners (same as nb2063/nb2103/nb2133) ----
    ...
    # ---- CatBoost(RMSE) 5-seed bag ----
    print("\n" + "-" * 78)
    print(f"CATBOOST(RMSE) K={K_FIXED}  depth={CB_DEPTH}  n_est={CB_N_EST}  "
          f"lr={CB_LR}  l2={CB_L2}  seeds={RESID_SEEDS}")
    print("-" * 78)
    ...
    print("\n" + "=" * 78)
    print("CATBOOST(RMSE) K=28 SUMMARY")
    print("=" * 78)
    print(f"   per-seed RAE   = "
          f"[{', '.join(f'{r:.4f}' for r in per_seed_rae)}]")
    print(f"   per-seed mean  = {rae_per_seed_mean:.4f}  "
          f"median = {rae_per_seed_median:.4f}  "
    ...
    if beats_nb2103_k28:
        verdict = "CATBOOST_BEATS_NB2103_K28_LGBM"
    elif flat_vs_nb2103_k28:
        verdict = "CATBOOST_FLAT_VS_NB2103_K28_LGBM"
    elif beats_anchor:
        verdict = "CATBOOST_BEATS_ANCHOR_BUT_WORSE_THAN_NB2103_K28_LGBM"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_HURTS_ANCHOR"
    print(f"   verdict        = {verdict}")

    # ---- Save mean-bag OOF ----
    out_p = DATA_PROCESSED / f"{TAG}_mean_bag_oof.npy"
    ...
    summary = {
        "tag": TAG,
        "method": (
            f"catboost_rmse_K{K_FIXED}_depth{CB_DEPTH}_n{CB_N_EST}"
            f"_lr{CB_LR}_l2{CB_L2}_5seed_5fold_on_117col"
        ),
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "data_source": ("nb2063 cached SHAP importance + same 117-col "
                        "5-way K-tuned matrix as nb2103/nb2133 (AtomPair / "
                        "MACCS / Mordred / ChempropEmbed / Avalon + ChEMBL kNN)"),
        "model_family": "CatBoost",
        "loss_function": "RMSE",
        "cb_depth": CB_DEPTH,
        "cb_n_estimators": CB_N_EST,
        "cb_learning_rate": CB_LR,
        "cb_l2_leaf_reg": CB_L2,
        "K_fixed": K_FIXED,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_mean_bag",
        "rae_median_bag",
        "delta_mean_bag_vs_nb2103_K28_lr03",
        "verdict",
        "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
