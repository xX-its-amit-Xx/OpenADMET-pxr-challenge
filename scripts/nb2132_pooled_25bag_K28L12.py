"""nb2132 -- Pooled-25 SHAP top-28 with num_leaves=12 (5 outer x 5 inner).

HYPOTHESIS:
    nb2123 num_leaves grid {12, 14, 16, 18} at K=28 found num_leaves=12 as
    the winner with mean-bag RAE = 0.4726, edging out the previous nb2103
    K=28 num_leaves=15 default (0.4737). This notebook reproduces the
    pooled-25 design from nb2111 but uses num_leaves=12 to test whether the
    tighter trees retain their advantage at the larger 25-fit pool.

PROTOCOL:
    1. Same as nb2131 (pooled-25 SHAP top-28) but with num_leaves=12.
    2. Reuse top-28 SHAP indices from nb2103 (= full_rank_order[:28] on
       data/processed/nb2063_shap_importance_full117.npy).
    3. Rebuild the same 117-col 5-way K-tuned feature matrix as
       nb2063/nb2081/nb2091/nb2103/nb2111, slice top-28.
    4. 5 outer seeds (0, 1, 7, 42, 137), each with 5 inner seeds
       (inner = outer*1000 + offset, offsets {0, 1, 7, 42, 137}) = 25 fits
       of LGBM(MSE, max_depth=4, num_leaves=12, n_estimators=300, lr=0.03,
       min_child=5, reg_lambda=2.0) with KFold(n=5, shuffle=True,
       random_state=inner_seed).
    5. Pool the 25 corrected OOF vectors via MEAN and via MEDIAN; report
       BoB (bag-of-bags) mean and BoB median plus the pooled-25 RAE.
    6. Verdict vs nb2111 pooled-25 L=15 and vs nb2123 K=28 L=12 5-bag mean
       (0.4726) at decision_margin = 0.003.

Outputs:
    scripts/nb2132_pooled_25bag_K28L12.py
    data/processed/nb2132_summary.json
    data/processed/nb2132_pooled25_mean_oof.npy   (253,) float32
    data/processed/nb2132_pooled25_median_oof.npy (253,) float32
"""
...
K_SHAP = 28
NUM_LEAVES = 12  # nb2123 winner at K=28
...
NB2123_K28_L12_MEAN_BAG_REF = 0.4726  # nb2123 5-bag at L=12
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(MSE) -- nb2103/nb2111 hyperparams with num_leaves=12 (nb2123 winner)."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Pooled-25 SHAP top-{K_SHAP} L={NUM_LEAVES} "
    ...
    print(f"          ref: nb2103 K=28 L=15 mean_bag {NB2103_K28_MEAN_BAG_REF:.4f} / "
          f"median_bag {NB2103_K28_MEDIAN_BAG_REF:.4f}")
    print(f"          ref: nb2123 K=28 L=12 5-bag mean {NB2123_K28_L12_MEAN_BAG_REF:.4f}")
    print(f"          margin = {DECISION_MARGIN}")
    print("=" * 78)

    # ---- nb2063 SHAP importance + nb2103/2111/2123 references ----
    ...
    nb2123_k28_l12_mean_bag = NB2123_K28_L12_MEAN_BAG_REF
    nb2123_k28_l12_median_bag = None
    if NB2123_SUMMARY.exists():
        with open(NB2123_SUMMARY) as f:
            nb2123_sum = json.load(f)
        for r in nb2123_sum.get("per_L_records", []):
            if int(r.get("num_leaves", -1)) == NUM_LEAVES:
                nb2123_k28_l12_mean_bag = float(r["rae_mean_bag"])
                nb2123_k28_l12_median_bag = float(r["rae_median_bag"])
                break
    print(f"[ref] nb2123.K=28 L={NUM_LEAVES} mean_bag_rae   = "
          f"{nb2123_k28_l12_mean_bag:.4f}")
    if nb2123_k28_l12_median_bag is not None:
        print(f"[ref] nb2123.K=28 L={NUM_LEAVES} median_bag_rae = "
              f"{nb2123_k28_l12_median_bag:.4f}")

    nb2111_pooled25_mean = None
    nb2111_pooled25_median = None
    if NB2111_SUMMARY.exists():
        with open(NB2111_SUMMARY) as f:
            nb2111_sum = json.load(f)
        nb2111_pooled25_mean = float(nb2111_sum.get("rae_pooled25_mean",
                                                    nb2103_k28_mean_bag))
        nb2111_pooled25_median = float(nb2111_sum.get("rae_pooled25_median",
                                                      nb2103_k28_median_bag))
        print(f"[ref] nb2111 pooled25 L=15 mean   = "
              f"{nb2111_pooled25_mean:.4f}")
        print(f"[ref] nb2111 pooled25 L=15 median = "
              f"{nb2111_pooled25_median:.4f}")
    ...
    # ---- Pooled-25 cross-fit: 5 outer x 5 inner ----
    print("\n" + "-" * 78)
    ...
    delta_pooled25_mean_vs_nb2123_l12 = (
        rae_pooled25_mean - nb2123_k28_l12_mean_bag
    )
    delta_pooled25_mean_vs_nb2111 = None
    if nb2111_pooled25_mean is not None:
        delta_pooled25_mean_vs_nb2111 = (
            rae_pooled25_mean - nb2111_pooled25_mean
        )
    ...
    v_pooled25_vs_nb2123_l12 = _verdict(
        rae_pooled25_mean, nb2123_k28_l12_mean_bag
    )
    ...
    v_pooled25_vs_nb2111 = None
    if nb2111_pooled25_mean is not None:
        v_pooled25_vs_nb2111 = _verdict(
            rae_pooled25_mean, nb2111_pooled25_mean
        )
    ...
    print(f"\n   POOLED-25 mean   RAE = {rae_pooled25_mean:.4f}  "
          f"(d_vs_nb2103_K28_L15_mean   = "
          f"{delta_pooled25_mean_vs_nb2103:+.4f})  "
          f"{v_pooled25_mean}")
    print(f"   POOLED-25 median RAE = {rae_pooled25_median:.4f}  "
          f"(d_vs_nb2103_K28_L15_median = "
          f"{delta_pooled25_median_vs_nb2103:+.4f})  "
          f"{v_pooled25_median}")
    print(f"   POOLED-25 mean vs nb2123 K=28 L={NUM_LEAVES} 5-bag "
          f"({nb2123_k28_l12_mean_bag:.4f}) = "
          f"{delta_pooled25_mean_vs_nb2123_l12:+.4f}  "
          f"{v_pooled25_vs_nb2123_l12}")
    if nb2111_pooled25_mean is not None:
        print(f"   POOLED-25 mean vs nb2111 pooled-25 L=15 "
              f"({nb2111_pooled25_mean:.4f}) = "
              f"{delta_pooled25_mean_vs_nb2111:+.4f}  "
              f"{v_pooled25_vs_nb2111}")
    ...
    print(f"   nb2123 K=28 L={NUM_LEAVES} 5-bag mean ref       = "
          f"{nb2123_k28_l12_mean_bag:.4f}")
    if nb2111_pooled25_mean is not None:
        print(f"   nb2111 pooled-25 L=15 mean ref        = "
              f"{nb2111_pooled25_mean:.4f}")
    ...
    summary = {
        "tag": TAG,
        "method": (f"lgbm_mse_pooled_25bag_shap_top28_L{NUM_LEAVES}"
                   "_on_117col_outer5_inner5"),
        "anchor": ANCHOR,
        ...
        "nb2123_K28_L12_mean_bag_ref": nb2123_k28_l12_mean_bag,
        "nb2123_K28_L12_median_bag_ref": nb2123_k28_l12_median_bag,
        "nb2111_pooled25_L15_mean_ref": nb2111_pooled25_mean,
        "nb2111_pooled25_L15_median_ref": nb2111_pooled25_median,
        ...
        "delta_pooled25_mean_vs_nb2103_K28_L15_mean": (
            delta_pooled25_mean_vs_nb2103
        ),
        "delta_pooled25_median_vs_nb2103_K28_L15_median": (
            delta_pooled25_median_vs_nb2103
        ),
        "delta_pooled25_mean_vs_nb2123_K28_L12": (
            delta_pooled25_mean_vs_nb2123_l12
        ),
        "delta_pooled25_mean_vs_nb2111_pooled25_L15": (
            delta_pooled25_mean_vs_nb2111
        ),
        "verdict_pooled25_mean_vs_nb2103_L15": v_pooled25_mean,
        "verdict_pooled25_median_vs_nb2103_L15": v_pooled25_median,
        "verdict_pooled25_mean_vs_nb2123_K28_L12": v_pooled25_vs_nb2123_l12,
        "verdict_pooled25_mean_vs_nb2111_pooled25_L15": v_pooled25_vs_nb2111,
        "verdict_bob_mean_of_mean_vs_nb2103_L15": v_bob_mean_of_mean,
        "verdict_bob_median_of_mean_vs_nb2103_L15": v_bob_median_of_mean,
        "pre_unblind_clean": True,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb2103_K28_mean_bag_REF_const": NB2103_K28_MEAN_BAG_REF,
        "nb2103_K28_median_bag_REF_const": NB2103_K28_MEDIAN_BAG_REF,
        "nb2123_K28_L12_mean_bag_REF_const": NB2123_K28_L12_MEAN_BAG_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb2103_K28_L15_median_bag_ref",
        "nb2123_K28_L12_mean_bag_ref",
        "nb2111_pooled25_L15_mean_ref",
        "rae_per_inner_mean",
        "rae_per_inner_std",
        "rae_pooled25_mean",
        "rae_pooled25_median",
        ...
        "delta_pooled25_mean_vs_nb2123_K28_L12",
        "delta_pooled25_mean_vs_nb2111_pooled25_L15",
        "verdict_pooled25_mean_vs_nb2103_L15",
        "verdict_pooled25_median_vs_nb2103_L15",
        "verdict_pooled25_mean_vs_nb2123_K28_L12",
        "verdict_pooled25_mean_vs_nb2111_pooled25_L15",
        "pre_unblind_clean",
    ):
        print(f"  {k}: {res.get(k)}")
