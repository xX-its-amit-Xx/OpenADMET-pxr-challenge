"""nb1731 -- CatBoost with binary feature subset weighting.

HYPOTHESIS:
    nb1554's 117-col 5-way K-tuned matrix includes MACCS bits and other
    binary substructure flags.  Some of these bits are nearly constant
    across the 253 unblind rows (very low variance) -- they carry little
    signal but CatBoost still splits on them and can over-weight noisy
    near-constant bits relative to high-variance informative columns.
    This script down-weights near-constant columns (var < 0.01) via
    CatBoost feature_weights=0.5 and up-weights variable columns to 2.0.

PROTOCOL:
    1. Build the same 117-col feature stack as nb1554 (top-25 AP +
       top-20 MACCS + top-20 Mord + top-20 ChempropEmbed + top-30 Avalon
       + pred_chembl_pec50 + mean_sim = 117 dims).
    2. Compute per-column variance across the 253 unblind rows.
       Columns with var < 0.01 -> feature_weight = 0.5 (down-weighted).
       Columns with var >= 0.01 -> feature_weight = 2.0 (up-weighted).
    3. Anchor = chemprop_aux te[unb_idx]; residual = y_unb - anchor.
    4. 5-seed bag CatBoost(MAE, d4, n200, lr0.05, l2=5,
       feature_weights=array), KFold(n=5, shuffle=True) cross-fit per seed.
    5. Pool mean_bag corrected anchor.
    6. Verdict at 0.003 margin vs nb1554 (rae_mean_bag = 0.5163).

Outputs:
    scripts/nb1731_catboost_feature_weights.py
    data/processed/nb1731_summary.json
    data/processed/nb1731_mean_bag_oof.npy        (253,) float32
"""
...
# Variance threshold for the binary subset weighting rule.
VAR_THRESHOLD = 0.01
WEIGHT_LOW = 0.5    # near-constant columns (var < threshold)
WEIGHT_HIGH = 2.0   # variable columns (var >= threshold)
...
NB1554_REF = 0.5163  # 5-way K-tuned CatBoost (rae_mean_bag from summary)
...
def _cat_params(seed: int, feature_weights: np.ndarray) -> dict:
    return dict(
        loss_function="MAE",
        depth=4,
        iterations=200,
...
def _residual_cross_fit_one_seed(X: np.ndarray, residual: np.ndarray,
    ...
    for tr_loc, va_loc in kf.split(np.arange(n)):
        mdl = CatBoostRegressor(**_cat_params(seed, feature_weights))
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost feature_weights (var<{VAR_THRESHOLD}->{WEIGHT_LOW}, "
          f"var>={VAR_THRESHOLD}->{WEIGHT_HIGH}) on 5-way K-tuned 117-col matrix; "
          f"anchor={ANCHOR}")
    ...
    # ---- Per-column variance + feature_weights ----
    col_var = X_unb.var(axis=0).astype(np.float64)
    is_low = col_var < VAR_THRESHOLD
    feature_weights = np.where(is_low, WEIGHT_LOW, WEIGHT_HIGH).astype(np.float64)
    n_low = int(is_low.sum())
    n_high = int((~is_low).sum())

    # Family slab boundaries (in same order as concat):
    # [0:n_ap] AP, [n_ap:n_ap+n_maccs] MACCS, ..., last 2 are kNN scalars
    family_boundaries = [
        ("AtomPair", 0, n_top_ap),
        ("MACCS", n_top_ap, n_top_ap + n_top_maccs),
        ("Mordred", n_top_ap + n_top_maccs,
         n_top_ap + n_top_maccs + n_top_mord),
        ("ChempropEmbed", n_top_ap + n_top_maccs + n_top_mord,
         n_top_ap + n_top_maccs + n_top_mord + n_top_embed),
        ("Avalon", n_top_ap + n_top_maccs + n_top_mord + n_top_embed,
         n_top_ap + n_top_maccs + n_top_mord + n_top_embed + n_top_avalon),
        ("pred_chembl", n_top_ap + n_top_maccs + n_top_mord
         + n_top_embed + n_top_avalon,
         n_top_ap + n_top_maccs + n_top_mord + n_top_embed + n_top_avalon + 1),
        ("mean_sim", n_top_ap + n_top_maccs + n_top_mord + n_top_embed
         + n_top_avalon + 1,
         n_top_ap + n_top_maccs + n_top_mord + n_top_embed + n_top_avalon + 2),
    ]
    weight_per_family = {}
    var_stats_per_family = {}
    for fam_name, a, b in family_boundaries:
        slab_var = col_var[a:b]
        slab_is_low = is_low[a:b]
        weight_per_family[fam_name] = {
            "n_total": int(b - a),
            "n_low_weight": int(slab_is_low.sum()),
            "n_high_weight": int((~slab_is_low).sum()),
            "frac_low_weight": float(slab_is_low.mean()) if (b - a) > 0 else 0.0,
        }
        var_stats_per_family[fam_name] = {
            "var_min": float(slab_var.min()) if (b - a) > 0 else None,
            "var_median": float(np.median(slab_var)) if (b - a) > 0 else None,
            "var_max": float(slab_var.max()) if (b - a) > 0 else None,
            "var_mean": float(slab_var.mean()) if (b - a) > 0 else None,
        }

    print("\n" + "-" * 78)
    print(f"FEATURE WEIGHT DISTRIBUTION  (var_thr={VAR_THRESHOLD})")
    print("-" * 78)
    print(f"   total cols          = {feat_dim}")
    print(f"   n_low_weight ({WEIGHT_LOW})  = {n_low}  "
          f"({100.0 * n_low / feat_dim:.1f}%)")
    print(f"   n_high_weight ({WEIGHT_HIGH}) = {n_high}  "
          f"({100.0 * n_high / feat_dim:.1f}%)")
    print(f"   col_var range       = [{col_var.min():.4g}, {col_var.max():.4g}]")
    print(f"   col_var median      = {float(np.median(col_var)):.4g}")
    print("   per-family breakdown:")
    for fam_name, _, _ in family_boundaries:
        d_w = weight_per_family[fam_name]
        d_v = var_stats_per_family[fam_name]
        v_min = d_v["var_min"]
        v_med = d_v["var_median"]
        v_max = d_v["var_max"]
        v_min_s = f"{v_min:.4g}" if v_min is not None else "n/a"
        v_med_s = f"{v_med:.4g}" if v_med is not None else "n/a"
        v_max_s = f"{v_max:.4g}" if v_max is not None else "n/a"
        print(f"      {fam_name:15s} n={d_w['n_total']:3d}  "
              f"low={d_w['n_low_weight']:3d}  high={d_w['n_high_weight']:3d}  "
              f"var[min/med/max]=[{v_min_s}, {v_med_s}, {v_max_s}]")

    # ---- Per-seed CatBoost residual cross-fit with feature_weights ----
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST RESIDUAL CROSS-FIT WITH feature_weights "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        resid_oof_s = _residual_cross_fit_one_seed(
            X_unb, residual, s, feature_weights
        )
    ...
    if beats_nb1554:
        verdict = "CATBOOST_FEATURE_WEIGHTS_BEATS_NB1554_NEW_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_FEATURE_WEIGHTS_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_FEATURE_WEIGHTS_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_FEATURE_WEIGHTS_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_FEATURE_WEIGHTS_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "model_family": "CatBoost_feature_weights",
        "catboost_loss": "MAE",
        "catboost_depth": 4,
        "catboost_iterations": 200,
        "catboost_learning_rate": 0.05,
        "catboost_l2_leaf_reg": 5.0,
        "feature_weight_var_threshold": VAR_THRESHOLD,
        "feature_weight_low": WEIGHT_LOW,
        "feature_weight_high": WEIGHT_HIGH,
        "n_low_weight_cols": n_low,
        "n_high_weight_cols": n_high,
        "frac_low_weight": float(n_low) / float(feat_dim),
        "col_var_min": float(col_var.min()),
        "col_var_max": float(col_var.max()),
        "col_var_median": float(np.median(col_var)),
        "weight_per_family": weight_per_family,
        "var_stats_per_family": var_stats_per_family,
        "K_AP_best": K_AP_best,
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "feature_weight_var_threshold", "feature_weight_low",
        "feature_weight_high",
        "n_low_weight_cols", "n_high_weight_cols", "frac_low_weight",
