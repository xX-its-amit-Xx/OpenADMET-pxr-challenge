"""nb2854 -- Hard binary classifier-routed 2-stage on 117-col K-tuned features.

NEW PARADIGM (vs nb2554 sigmoid weights, vs nb2522 partition):
    Hard binary classifier gates which regressor head is called.

    Stage 1: LGBMClassifier on (X_117, y >= 5.5) with class_weight='balanced'
             (LightGBM is_unbalance=True), trained on full 4139.

    Stage 2a (hit regressor):  LGBMRegressor trained on ALL 4139 rows; the
             target is the original y if y >= 5.5, else clipped to 5.5.
             So this head is specialised for predicting the hit-magnitude
             surface while keeping the non-hit floor as a soft prior.

    Stage 2b (nonhit regressor): LGBMRegressor on ALL 4139 with target =
             original y if y < 5.5, else clipped to 5.5.  Specialised for
             non-hit landscape with hits collapsed to the ceiling.

    Inference (per row):
        if P(hit) > 0.5:  pred = stage2a(row)
        else:             pred = stage2b(row)
    (Hard routing -- no soft blend, no sigmoid.)

    Cross-fit: 5-fold scaffold CV over 253 UNBLIND with 5 kf_seeds
    {1001..1005}.  Classifier + both regressors are 4139-trained deploy
    pipelines (not refit per fold); fold partition only chunks the unblind
    for pooled RAE scoring (matches nb2522 protocol -- same df, fair gate).
...
Outputs:
    scripts/nb2854_classifier_regressor_2stage.py
    data/processed/nb2854_summary.json
    data/processed/nb2854_pred_oof.npy       (253,) float32
    data/processed/te_nb2854.npy             (513,) float32  deploy
"""
...
# Threshold for hit class label (per task: 5.5 == matches nb2522)
HIT_THRESHOLD = 5.5
ROUTE_THRESHOLD = 0.5     # P_hit > 0.5 -> stage2a; else stage2b
...
# -------- feature build paths (same as nb1983/nb2522 -> 117 cols) ------------
...
# -------- helpers (lifted verbatim from nb2522) ------------------------------
...
def _clf_params(seed):
    """LGBMClassifier (binary) -- mild capacity, is_unbalance handles 9% hits."""
    return dict(
        objective="binary",
        max_depth=4,
        num_leaves=15,
        ...
        is_unbalance=True,           # class_weight='balanced' equivalent
        reg_lambda=1.0,
        random_state=seed,
        n_jobs=2,
        verbosity=-1,
...
def _reg_params(seed):
    """LGBMRegressor for stage 2a/2b -- same shape as nb2522."""
    return dict(
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Hard binary classifier-routed 2-stage "
          "(clf P(hit) -> hard if/else to 2 reg heads)")
    print(f"   HIT_THRESHOLD = {HIT_THRESHOLD}   ROUTE_THRESHOLD = {ROUTE_THRESHOLD}")
    print(f"   SEEDS = {SEEDS}   KF_SEEDS = {KF_SEEDS}   N_FOLDS = {N_FOLDS}")
    print("=" * 78)
    ...
    # ---- Construct stage 2a/2b TARGETS on full 4139 with clipping ----
    # stage2a: y_train if hit; clipped to HIT_THRESHOLD (5.5) if non-hit
    # stage2b: y_train if non-hit; clipped to HIT_THRESHOLD (5.5) if hit
    y_stage2a = np.where(y_train >= HIT_THRESHOLD, y_train, HIT_THRESHOLD).astype(np.float64)
    y_stage2b = np.where(y_train < HIT_THRESHOLD, y_train, HIT_THRESHOLD).astype(np.float64)
    print(f"[stage2 targets] y_stage2a: mean={y_stage2a.mean():.3f}  "
          f"std={y_stage2a.std():.3f}  range "
          f"[{y_stage2a.min():.3f},{y_stage2a.max():.3f}]")
    print(f"[stage2 targets] y_stage2b: mean={y_stage2b.mean():.3f}  "
          f"std={y_stage2b.std():.3f}  range "
          f"[{y_stage2b.min():.3f},{y_stage2b.max():.3f}]")

    # ---- STAGE 1 sanity: classifier scaffold-CV on 4139 ----
    print("\n" + "-" * 78)
    ...
    route_mask_te = (p_hit_te > ROUTE_THRESHOLD)
    n_route_hit = int(route_mask_te.sum())
    n_route_nh = int((~route_mask_te).sum())
    print(f"   p_hit_te mean = {p_hit_te.mean():.4f}  "
          f"std = {p_hit_te.std():.4f}  range "
          f"[{p_hit_te.min():.4f},{p_hit_te.max():.4f}]")
    print(f"   hard-route counts on 513: "
          f"P>{ROUTE_THRESHOLD} -> stage2a: {n_route_hit}  "
          f"P<={ROUTE_THRESHOLD} -> stage2b: {n_route_nh}")
    route_mask_unb = route_mask_te[unb_idx]
    print(f"   hard-route counts on 253 unblind: "
          f"stage2a={int(route_mask_unb.sum())}  "
          f"stage2b={int((~route_mask_unb).sum())}  "
          f"(true unblind hit count={(y_unb >= HIT_THRESHOLD).sum()})")

    # ---- STAGE 2a/2b DEPLOY: regressors on FULL 4139 with clipped targets ----
    print("\n[stage2 deploy] BOTH regressors trained on ALL 4139 (clipped targets)")
    pred2a_te = _bag_fit_predict_reg(X_tr, y_stage2a, X_te)
    pred2b_te = _bag_fit_predict_reg(X_tr, y_stage2b, X_te)
    ...
    # ---- HARD ROUTING on 513 ----
    deploy_te = np.where(route_mask_te, pred2a_te, pred2b_te).astype(np.float32)
    pred_unb_deploy = deploy_te[unb_idx]
    in_sample_unb_rae = float(rae(y_unb, pred_unb_deploy))
    print(f"\n[deploy 513] hard-routed pred mean = {deploy_te.mean():.3f}  "
    ...
    # NB: stages 1/2a/2b are all 4139-trained deploy pipelines; the per-kf-seed
    # scaffold CV over 253 chunks the unblind for pooled scoring (mirrors
    # nb2522 protocol for fair gate comparability).
    print("\n" + "-" * 78)
    ...
    summary = {
        "tag": TAG,
        "method": ("hard binary classifier-routed 2-stage "
                   "(LGBM clf + 2 LGBM reg heads on FULL 4139 w/ clipped targets, "
                   "hard if/else routing at P_hit > 0.5)"),
        ...
        "n_test": n_test,
        "n_unb": n_unb,
        "n_hits_train": int(y_bin_train.sum()),
        "n_nonhits_train": int(n_train - y_bin_train.sum()),
        ...
        "n_route_hit_te": n_route_hit,
        "n_route_nh_te": n_route_nh,
        "n_route_hit_unb": int(route_mask_unb.sum()),
        "n_route_nh_unb": int((~route_mask_unb).sum()),
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "hit_threshold", "route_threshold",
        "n_hits_train", "hits_frac_train", "hits_frac_unblind",
        "stage1_clf_auc_train_oof", "stage1_clf_brier_train_oof",
        "p_hit_te_mean", "p_hit_unb_mean",
        "n_route_hit_te", "n_route_hit_unb",
