"""nb1974 -- LGBM regression (MSE) with extra_trees=True on the 117-col 5-way
K-tuned matrix; chemprop_aux PRE-unblind anchor; 5-seed bag, 5-fold cross-fit.
Continues marginal LGBM exploration past nb1861 / nb1894 / nb1852.

HYPOTHESIS:
    nb1861 (reg_lambda=2.0, default split selection) pooled-25-bag RAE = 0.5013
    on the 253-unblind set; nb1852 mean-bag ~0.5100. Knob to test here:
      * extra_trees=True  (ExtraTrees-style random split selection in LGBM;
                           reduces overfit to per-feature greedy splits)
    All other knobs IDENTICAL to nb1861 (objective='regression', max_depth=4,
    num_leaves=15, n_est=300, lr=0.03, min_child_samples=5, reg_lambda=2).
    If extra_trees randomization helps generalize at small n (203 train per
    fold), mean-bag RAE drops below nb1861 0.5013; if it hurts (too random for
    253-row residual signal), RAE goes up.

PROTOCOL:
    1. Features = 117-col 5-way K-tuned (AtomPair + MACCS + Mordred +
       ChempropEmbed + Avalon + ChEMBL kNN pred + ChEMBL mean sim)
       identical pipeline as nb1861 / nb1894.
    2. Anchor = chemprop_aux te[unb_idx] (PRE-unblind clean).
       residual = y_unb - anchor.
    3. LGBM(objective='regression', max_depth=4, num_leaves=15, n_est=300,
       lr=0.03, min_child_samples=5, reg_lambda=2, extra_trees=True).
    4. 5-seed bag {0,1,7,42,137}, each seed = 5-fold KFold cross-fit
       (shuffle=True, random_state=seed) on residual.
    5. Per-seed RAE = rae(y_unb, anchor + resid_oof_seed).
       Mean-bag RAE  = rae(y_unb, anchor + mean_resid_oof_across_5_seeds).
    6. Verdict at 0.003 margin vs nb1861 (0.5013).

Outputs:
    scripts/nb1974_lgbm_extra_trees.py
    data/processed/nb1974_summary.json
    data/processed/nb1974_per_seed_oof.npy   (5, 253) float32
    data/processed/nb1974_mean_bag_oof.npy   (253,)   float32
"""
...
NB1771_REF = 0.5100
NB1852_REF = 0.5100
NB1861_REF_FALLBACK = 0.5013  # pooled 25-bag from nb1861
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression/MSE) -- extra_trees=True variant.

    Knobs IDENTICAL to nb1861 except:
      extra_trees : False -> True   (random split point selection)
    All else (objective='regression', max_depth=4, num_leaves=15,
    n_estimators=300, learning_rate=0.03, min_child_samples=5,
    reg_lambda=2.0) unchanged.
    """
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM(MSE) extra_trees=True on 117-col 5-way K-tuned matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {SEEDS}  folds = {RESID_FOLDS}")
    print(f"          knob  : extra_trees=True (all other LGBM knobs match nb1861)")
    print(f"          refs  : chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1554 ({NB1554_REF:.4f}), nb1771 ({NB1771_REF:.4f}), "
          f"nb1852 ({NB1852_REF:.4f})")
    print(f"          decision margin vs nb1861 = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    if NB1861_SUMMARY.exists():
        ...
        nb1861_ref = float(nb1861_sum.get("rae_pooled_25bag",
                                          NB1861_REF_FALLBACK))
    ...
    # ---- BAG LOOP ----
    print("\n" + "-" * 78)
    print(f"BAG: {len(SEEDS)} seeds x {RESID_FOLDS}-fold cross-fit  "
          f"(dim={feat_dim})")
    print("-" * 78)
    ...
    for s_i, s_seed in enumerate(SEEDS):
        t_s = time.time()
        resid_oof_s = _residual_cross_fit_one_seed(X_unb, residual, s_seed)
        ...
        per_seed_records.append({
            "seed": int(s_seed),
            "rae_seed": r_s,
            "delta_seed_vs_anchor": r_s - rae_anchor,
            "wall_sec": round(time.time() - t_s, 2),
        })
        print(f"   seed={s_seed:4d}   rae={r_s:.4f}   "
    ...
    print(f"   per-seed std           = {per_seed_std:.4f}")
    print(f"   mean-bag (avg OOF) RAE = {rae_mean_bag:.4f}")
    print(f"   nb1861 ref (25-bag)    = {nb1861_ref:.4f}")
    ...
    if delta_vs_nb1861 < -DECISION_MARGIN:
        verdict = "WIN_VS_NB1861"
    elif delta_vs_nb1861 > DECISION_MARGIN:
        verdict = "LOSE_VS_NB1861"
    else:
        verdict = "TIE_VS_NB1861"

    print(f"   verdict (margin {DECISION_MARGIN}) = {verdict}")
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "per_seed_rae",
        "per_seed_mean_rae", "per_seed_median_rae", "per_seed_std_rae",
        "rae_mean_bag",
