"""nb1299 deploy — ADD 13 OrbMol (Orbital-Materials Orb-v3 NNP) scalars to deployed config.
(honest gate WIN, nb1299_orbmol_gate).

  control (AIMNet2+strain+D4+DBSTEP)  0.4242  treatment (+OrbMol) 0.4231
  matched delta -0.00109  (3/3 seeds neg)  treat < 0.4242 -> DEPLOY.

OrbMol scalars are MARGINAL-OVER-DEPLOYED, appended to the same 4-GBM feature vector
as combined + 9 AIMNet2 + 8 strain + 15 DFT-D4 + 16 DBSTEP + 13 OrbMol.
...
def main():
    ...
    ddf = pd.read_csv(D4);  bdf = pd.read_csv(DB)
    odf = pd.read_csv(ORB)
    ...
    print(f"train {len(tr)} | test {len(te)} | extra cols {Xex_tr.shape[1]} "
          f"(AIMNet2 {len(ACOLS)} + strain {len(SCOLS)} + D4 {len(DCOLS)} "
          f"+ DBSTEP {len(DBCOLS)} + OrbMol {len(ORBCOLS)})", flush=True)
    ...
    out = "submissions/nb1299_orbmol_ensemble.csv"
    ...
    best = {
        "rae": 0.4231,
        "via": "+13 OrbMol (Orb-v3 NNP) scalars on top of AIMNet2+strain+D4+DBSTEP in 4-GBM "
               "(matched -0.00109, 3/3 seeds neg, nb1299_gate)",
        "submission": out,
    ...
    print("updated best_ensemble.json -> 0.4231 (+OrbMol)", flush=True)
