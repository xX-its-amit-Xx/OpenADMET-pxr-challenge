"""nb1494 -- Outer-bag VALIDATION of nb1484 (PRE-unblind 4-way SHAP-pruned
residual blend: AtomPair-30 + MACCS-20 + Mordred-30 + ChempropEmbed-30,
anchored to chemprop_aux PRE-unblind te[unb_idx]).

Same 4-family SHAP-pruned recipe as nb1484 (with per-family top-K columns
PINNED to nb1484's published top-idx so SHAP rerank noise does not pollute
the outer-seed signal) but repeats the inner 5-seed shallow LGBM Huber
residual cross-fit bag across five OUTER seeds {0, 1, 7, 42, 137} with
inner seeds reparameterized per outer as
    inner_seeds(o) = [o*1000 + s for s in {0, 1, 7, 42, 137}].

For each outer seed o, for each FAMILY in [AtomPair, MACCS, Mordred,
ChempropEmbed], we evaluate a 5-inner-seed mean-bag of shallow LGBM Huber
residual learners on the PRUNED feature matrix
    [top-K(family) + pred_chembl + sim]
and produce a corrected OOF
    fam_corrected_o = anchor + mean_bag_inner(residual_oof)
Then nb1484_o = (1/4) * sum(fam_corrected_o for fam in 4 families).

Aggregates:
    per_outer_rae   = rae(y_unb, nb1484_o)         for o in OUTER_SEEDS
    bob_mean_oof    = row-mean   of 5 nb1484_o     -> pooled RAE
    bob_median_oof  = row-median of 5 nb1484_o     -> pooled RAE

Verdict NB1484_REPRODUCES iff |per_outer_mean - nb1484_ref| < 0.003,
where nb1484_ref = naive 1/4 mean blend RAE from nb1484 cross-fit
(0.52567...; see data/processed/nb1484_summary.json :: rae_naive_mean_blend).

Outputs:
    scripts/nb1494_bag_nb1484.py                  (this file)
    data/processed/nb1494_summary.json
    data/processed/nb1494_bob_mean_oof.npy        (253,) float32
    data/processed/nb1494_bob_median_oof.npy      (253,) float32
"""
...
NB1472_REF = 0.5330
DECISION_MARGIN_PRIMARY = 0.003     # margin for nb1484 vs nb1472 PROCEED gate
REPRODUCE_MARGIN = 0.003            # margin for outer-bag REPRODUCE verdict
...
def _build_family_pruned_unblind(
    ...
    the SHAP top-idx pinned from nb1484."""
...
def main() -> dict:
    ...
    print(f"{TAG} -- OUTER-BAG VALIDATION of nb1484 "
          f"(PRE-unblind 4-way SHAP-pruned residual blend on chemprop_aux)")
    ...
    # ---- Step 1: Read nb1484 result and gate ----
    ...
    nb1472_ref = float(sum_1484.get("nb1472_ref", NB1472_REF))
    delta_best_vs_nb1472 = nb1484_best - nb1472_ref

    print(f"\n[gate] nb1484 RAE best                = {nb1484_best:.6f}  "
          f"(variant = {nb1484_best_variant})")
    print(f"[gate] nb1484 RAE naive 1/4 mean blend = {nb1484_naive:.6f}")
    print(f"[gate] nb1484 RAE 5-fold SLSQP         = {nb1484_slsqp:.6f}")
    print(f"[gate] nb1472 ref                      = {nb1472_ref:.6f}")
    print(f"[gate] delta_best_vs_nb1472            = {delta_best_vs_nb1472:+.6f}")
    print(f"[gate] required PROCEED margin         = -{DECISION_MARGIN_PRIMARY}")

    if delta_best_vs_nb1472 > -DECISION_MARGIN_PRIMARY:
        decision = "SKIP"
        print(f"[gate] DECISION = SKIP (nb1484 does NOT beat nb1472 by "
              f">= {DECISION_MARGIN_PRIMARY})")
        summary = {
            "tag": TAG,
            "decision": decision,
            ...
            "decision_margin": DECISION_MARGIN_PRIMARY,
            "verdict": "SKIPPED_nb1484_does_not_beat_nb1472",
    ...
    decision = "PROCEED"
    print(f"[gate] DECISION = PROCEED (nb1484 beats nb1472 by "
          f"{abs(delta_best_vs_nb1472):.4f} >= {DECISION_MARGIN_PRIMARY})")

    # ---- Reference for verdict: nb1484 cross-fit naive 1/4 mean (matches the
    #      structure of nb1484_o = mean of 4 family-corrected vectors) ----
    nb1484_ref = nb1484_naive
    print(f"[verdict-ref] nb1484_ref (naive 1/4 mean) = {nb1484_ref:.6f}")

    # ---- Pull pinned top-idx per family from nb1484 ----
    ...
    # outer_blend[oi] = (1/4) * sum over families of mean_bag_inner(corrected)
    ...
    summary = {
        ...
        "nb1484_rae_naive_mean_blend": nb1484_naive,
        "nb1484_rae_slsqp_crossfit": nb1484_slsqp,
        "nb1484_ref_for_verdict": nb1484_ref,
        ...
        "delta_per_outer_mean_vs_nb1484": delta_per_outer,
        "delta_outer0_vs_nb1484": delta_outer0,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "nb1484_rae_best", "nb1484_best_variant",
        ...
        "delta_per_outer_mean_vs_nb1484",
        "delta_outer0_vs_nb1484",
