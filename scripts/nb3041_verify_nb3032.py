"""nb3041 -- 15-seed wide-seed verification of nb3032 per-fold SLSQP simplex
            on 4 K-anchors {K14, K16, K18, K19} all at deep-30.

CONTEXT:
    nb3032 reported pooled outer-val RAE 0.4493 on single kf_seed=1001
    ("BETTER_THAN_NB2992", -0.0018 vs nb3002 wide-seed-confirmed 0.4511
    ceiling). Per cycle-160/245 wide-seed protocol, must verify with >=15
    fresh seeds before promote.

    nb3032 paradigm: 4-anchor simplex pool {K14, K16, K18, K19} all at
    deep-30 (chemprop_aux anchor + residual LGBM on K-feature slices of
    nb2231 117-col matrix). All 4 anchors are PRE-unblind; no POST-unblind
    contamination chain risk.

PROTOCOL:
    Re-run nb3032 per-fold SLSQP simplex step at 15 FRESH kf_seeds
    {1066..1080} (NEW seeds, not used in any prior wide-seed verify).
    Per kf_seed: 5-fold scaffold split, SLSQP simplex per fold on
    {K14, K16, K18, K19} deep-30 cached OOFs.
    Report mean +/- std + 95% CI across the 15 fresh seeds.

    Anchors LOADED from caches (NO rebuild):
        K14: data/processed/nb3032_K14_30seed_oof.npy / te_nb3032_K14.npy
        K16: data/processed/nb3032_K16_30seed_oof.npy / te_nb3032_K16.npy
        K18: data/processed/nb2960_K18_30seed_oof.npy / nb2960_K18_30seed_te.npy
        K19: data/processed/nb3000_K19_30seed_oof.npy / te_nb3000_K19.npy

GATE (on 15-seed mean, same paradigm as nb3030/nb3040):
    mean < 0.4511 -> "VERIFIED_NEW_PRIMARY1"
        (nb3032 beats nb3001/nb3002 wide-seed-verified ceiling 0.4511)
    mean < 0.4518 -> "VERIFIED_MARGINAL"
        (still inside marginal band but not new PRIMARY-1)
    shift > +0.005 vs single-kf -> "LUCKY_SEED_TRAP"
        (nb3032 single-kf=1001 was a fortunate seed; reject promotion)
    else -> "FAIL_OR_REPORT"

References:
    nb3032 single-kf=1001 pooled RAE   = 0.4493   <- parent
    nb3002 deep-30 simplex (K18,K19)   = 0.4511   <- prior ceiling
    nb3001 15-seed wide-mean            = 0.4511
    nb3003 single-seed reference        = 0.4518
    nb3020 single-kf=1001               = 0.4501
    nb2960 K18 deep-30 OOF              = 0.4536
    nb3000 K19 deep-30 OOF              = 0.4607
    nb3032 K14 deep-30 bag-mean         = 0.4845
    nb3032 K16 deep-30 bag-mean         = 0.4734
    nb2171 ceiling deep-30              = 0.4682

Inputs:
    data/processed/_audit_unblind_idx.npy
    data/processed/_audit_unblind_y.npy
    data/processed/nb3032_K14_30seed_oof.npy
    data/processed/te_nb3032_K14.npy
    data/processed/nb3032_K16_30seed_oof.npy
    data/processed/te_nb3032_K16.npy
...
Outputs:
    data/processed/nb3041_summary.json
    data/processed/nb3041_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3041.npy         (513,) float32 -- full-pool deploy te
    submissions/nb3041_wide_seed_verify_nb3032.csv  (only on promote verdicts)
"""
...
OOF_PATHS = {
    "K14": DATA_PROCESSED / "nb3032_K14_30seed_oof.npy",
    "K16": DATA_PROCESSED / "nb3032_K16_30seed_oof.npy",
    "K18": DATA_PROCESSED / "nb2960_K18_30seed_oof.npy",
    "K19": DATA_PROCESSED / "nb3000_K19_30seed_oof.npy",
}
TE_PATHS = {
    "K14": DATA_PROCESSED / "te_nb3032_K14.npy",
    "K16": DATA_PROCESSED / "te_nb3032_K16.npy",
...
# -- References ----------------------------------------------------------------
REF_NB3032_SINGLE_KF = 0.4493
REF_NB3002_DEEP30 = 0.4511
...
def _run_one_seed(P_unb: np.ndarray, y_unb: np.ndarray,
    ...
    """Run nb3032 per-fold SLSQP simplex pipeline at a single kf_seed."""
...
def main() -> dict:
    ...
    print(f"{TAG} -- 15-seed wide-seed verify of {PARENT_TAG} per-fold SLSQP "
          f"{K_LABELS}")
    print(f"          all 4 anchors deep-30 (PRE-unblind chemprop_aux + resid)")
    print(f"          kf_seeds = {KF_SEEDS}")
    print(f"          (fresh, EXCLUDES 1001 from nb3032 single-fit and "
          f"all prior wide-seed seeds 1006-1065)")
    ...
    print("\n" + "-" * 78)
    print("STEP 1: load 4 K-anchor deep-30 OOFs and te arrays")
    print("-" * 78)
    oof_cols, te_cols = [], []
    per_K_full_rae = {}
    for k in K_LABELS:
        if not OOF_PATHS[k].exists():
            raise FileNotFoundError(f"missing OOF cache: {OOF_PATHS[k]}")
        if not TE_PATHS[k].exists():
            raise FileNotFoundError(f"missing te cache: {TE_PATHS[k]}")
    ...
    shift_vs_single = mean_rae - REF_NB3032_SINGLE_KF
    print(f"\n   nb3032 single-kf=1001     = {REF_NB3032_SINGLE_KF:.4f}")
    print(f"   shift (mean - single)     = {shift_vs_single:+.4f}")
    print(f"   nb3002 deep-30 ceiling    = {REF_NB3002_DEEP30:.4f}")
    ...
    if shift_vs_single > LUCKY_SHIFT_THRESHOLD:
        ...
        ladder_action = (
            f"REJECT nb3032 promotion. nb3032 single-kf=1001 "
            f"{REF_NB3032_SINGLE_KF:.4f} was a fortunate seed; wide-seed "
            f"mean {mean_rae:.4f} shift {shift_vs_single:+.4f} exceeds "
            f"+{LUCKY_SHIFT_THRESHOLD:.3f} tolerance. Keep prior PRIMARY-1."
        )
    elif mean_rae < GATE_NEW_PRIMARY1:
        verdict = "VERIFIED_NEW_PRIMARY1"
        ladder_action = (
            f"PROMOTE nb3032 to PRIMARY-1 (wide-seed {mean_rae:.4f} beats "
            f"nb3001/nb3002 ceiling {REF_NB3001_WIDE_MEAN:.4f}). "
            "Demote prior PRIMARY-1 to PRIMARY-2."
        )
    elif mean_rae < GATE_MARGINAL:
        verdict = "VERIFIED_MARGINAL"
        ladder_action = (
            f"KEEP prior PRIMARY-1. nb3032 wide-seed {mean_rae:.4f} is "
            f"inside PROMOTE gate {GATE_MARGINAL} but not strictly better "
            f"than nb3001 ceiling {REF_NB3001_WIDE_MEAN}; tag as alternate."
        )
    else:
        verdict = "FAIL_OR_REPORT"
        ladder_action = (
            f"REJECT nb3032 promotion. Wide-seed mean {mean_rae:.4f} above "
    ...
    summary = {
        ...
        "method": "wide_seed_15_verify_per_fold_slsqp_K14_K16_K18_K19_all_deep30",
        ...
        "ref_nb3032_single_kf": REF_NB3032_SINGLE_KF,
        "ref_nb3002_deep30": REF_NB3002_DEEP30,
        ...
        "ref_K14_bag": REF_K14_BAG,
        "ref_K16_bag": REF_K16_BAG,
    ...
    print(f"   95% CI                = [{ci_low:.4f}, {ci_high:.4f}]")
    print(f"   shift vs nb3032=1001  = {shift_vs_single:+.4f}")
