"""nb1462 -- SHAP-pruned chemprop-encoder embedding top-30 + ChEMBL residual.

Hypothesis:
    nb1312 used full chemprop-encoder 300-d embedding (concatenated with MACCS-167)
    as a residual feature over nb1070 anchor (mean-bag RAE 0.5647).  nb1352 / nb1373
    demonstrated that SHAP-pruning bit-vector features down to ~20-30 dims plus
    pred_chembl + sim consistently shrinks residual capacity at n=253 and reduces
    overfit (nb1373 atompair-pruned 0.5095 vs nb1172 full-atompair 0.5659).
    By analogy, SHAP-pruning the chemprop 300-d embedding to top-30 dims plus
    pred_chembl + sim should similarly reduce residual capacity and may translate
    to a cross-fit gain over both nb1312 (full embed, 0.5647) and the current
    SHAP-pruned champion nb1373 (0.5095).

Protocol:
    1.  Anchor = nb1070_pred_oof on 253 unblind rows.
        residual = y_unb - anchor.
    2.  Build ChEMBL PXR pool (same union as nb1242/nb1352/nb1373).  Compute
        kNN-5 Tanimoto on cached Morgan-2048 over std test SMILES; pull
        pred_chembl_pec50 + mean_sim for all test rows, slice to unblind.
    3.  Load te_chemprop_embed_300.npy (cached from nb1312), slice to 253 unblind.
        Build full 302-col feature matrix on 253:
            chemprop_embed-300 + pred_chembl_pec50 + mean_sim
        Train ONE seed-0 shallow LGBM Huber on residual to get SHAP importance
        via shap.TreeExplainer; fall back to LGBM gain on failure.  Take
        mean(|SHAP|) per feature, slice embed-only, pick top-30 embed dims.
    4.  Build PRUNED 32-col feature matrix = top-30 embed dims + pred_chembl
        + mean_sim.
    ...
    6.  Verdict at 0.003 margin vs:
            ...
            nb1312  (full chemprop embed-300 + MACCS standalone, 0.5647 mean-bag)
            nb1070  (anchor, ~0.5771 pooled)

Outputs:
    scripts/nb1462_shap_pruned_chemprop_embed.py  (this file)
    data/processed/nb1462_summary.json
    data/processed/nb1462_mean_bag_oof.npy        (253,) float32
    data/processed/nb1462_median_bag_oof.npy      (253,) float32
    data/processed/nb1462_per_seed_corrected_oof.npy (5, 253) float32
"""
...
CHEMPROP_EMBED_TE_PATH = DATA_PROCESSED / "te_chemprop_embed_300.npy"  # (513, 300) float32
...
NB1070_REF = 0.5771
NB1312_REF = 0.5647      # full chemprop embed-300 + MACCS standalone residual bag
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP-pruned chemprop-embed top-{TOP_K_EMBED} + ChEMBL residual; "
          f"anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          baselines: nb1373 ({NB1373_REF:.4f}), "
          f"nb1312 ({NB1312_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    # ---- chemprop embedding (unblind slice) ----
    ...
    # ---- Build FULL feature matrix (embed + pred_chembl + sim) ----
    ...
    # Embed-only importance slice
    embed_imp = imp_full[:n_embed]
    top_k = min(TOP_K_EMBED, n_embed)
    top_dim_order = np.argsort(-embed_imp)
    top_dim_idx = top_dim_order[:top_k].astype(int)
    top_dim_idx_sorted = np.sort(top_dim_idx)
    top_dim_imp = embed_imp[top_dim_idx]
    print(f"   top-{top_k} chemprop embed dim indices (ranked by importance):")
    for rank, (dim, val) in enumerate(zip(top_dim_idx.tolist(),
                                          top_dim_imp.tolist())):
        print(f"      rank {rank+1:2d}:  dim {dim:5d}   imp = {val:.5f}")
    print(f"   top-{top_k} dim indices (sorted asc): {top_dim_idx_sorted.tolist()}")

    n_nonzero_imp = int((embed_imp > 0).sum())
    print(f"   chemprop embed dims with nonzero importance: {n_nonzero_imp}/{n_embed}")

    # ---- Build PRUNED 32-col feature matrix ----
    X_embed_unb_pruned = X_embed_unb[:, top_dim_idx]
    ...
    print(f"\n   PRUNED feature matrix: {X_unb_pruned.shape}  "
          f"(top-{top_k} chemprop embed dims + pred_chembl + sim)")
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1312 = {rae_mean_bag - NB1312_REF:+.4f}"
    ...
    print(f"   pooled RAE(median_bag) = {rae_median_bag:.4f}  "
          ...
          f"  d_vs_nb1312 = {rae_median_bag - NB1312_REF:+.4f}"
          f"  d_vs_nb1373 = {rae_median_bag - NB1373_REF:+.4f})")
    print(f"   nb1070 ref            = {NB1070_REF:.4f}")
    print(f"   nb1312 ref            = {NB1312_REF:.4f}  (full chemprop embed + MACCS)")
    print(f"   nb1373 ref            = {NB1373_REF:.4f}  (SHAP-pruned AtomPair top-30)")

    beats_nb1070 = rae_mean_bag < rae_anchor - DECISION_MARGIN
    beats_nb1312 = rae_mean_bag < NB1312_REF - DECISION_MARGIN
    beats_nb1373 = rae_mean_bag < NB1373_REF - DECISION_MARGIN

    if beats_nb1373:
        verdict = "SHAP_PRUNED_CHEMPROP_EMBED_BEATS_NB1373_NEW_PRIMARY_CANDIDATE"
    elif abs(rae_mean_bag - NB1373_REF) < DECISION_MARGIN:
        verdict = "SHAP_PRUNED_CHEMPROP_EMBED_FLAT_VS_NB1373"
    elif beats_nb1312:
        verdict = "SHAP_PRUNED_CHEMPROP_EMBED_BEATS_NB1312_BUT_WORSE_THAN_NB1373"
    elif beats_nb1070:
        verdict = "SHAP_PRUNED_CHEMPROP_EMBED_HELPS_NB1070_BUT_WORSE_THAN_NB1312"
    else:
        verdict = "SHAP_PRUNED_CHEMPROP_EMBED_HURTS_NB1070"
    ...
    summary = {
        ...
        "feature_source": "te_chemprop_embed_300_nb1312_cache",
        ...
        "top_k_embed": int(top_k),
        "top_embed_dim_indices_ranked": [int(b) for b in top_dim_idx.tolist()],
        "top_embed_dim_importance_ranked": [float(v) for v in top_dim_imp.tolist()],
        "top_embed_dim_indices_sorted_asc": [int(b) for b in top_dim_idx_sorted.tolist()],
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "top_embed_dim_importance_ranked",
        ...
        "delta_mean_bag_vs_nb1312",
        "delta_mean_bag_vs_nb1373",
        "beats_nb1070", "beats_nb1312", "beats_nb1373",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
