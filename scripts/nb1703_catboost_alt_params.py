"""nb1703 -- Outer-bag CatBoost depth=4 ALT-PARAMS on 117-col 5-way K-tuned.

HYPOTHESIS:
    nb1554 (CatBoost MAE depth=4, n_est=200, lr=0.05, l2=5) yielded mean_bag
    RAE = 0.5163 on 253 PRE-unblind, and the nb1561 outer-bag BoB reproduces
    at ~0.5155.  This is the "alt-grid" CatBoost probe (the nb1701 sibling)
    using a slower, longer schedule:
        depth        = 4   (unchanged)
        learning_rate= 0.04 (slower; was 0.05)
        iterations   = 250  (longer; was 200)
        l2_leaf_reg  = 4    (slightly relaxed; was 5)
    Slowing lr while lengthening iterations is the canonical CatBoost shrink-
    trade probe: same effective gradient capacity but smoother per-tree step
    and more averaging.  Pairs with the same 117-col 5-way K-tuned feature
    stack, chemprop_aux anchor, 5-seed bag x 5-fold cross-fit.

PROTOCOL:
    1. Build 117-col 5-way K-tuned feature stack (= nb1554 recipe):
    ...
    3. 5-seed bag CatBoost(loss=MAE, depth=4, n_est=250, lr=0.04, l2=4).
...
Outputs:
    scripts/nb1703_catboost_alt_params.py
    data/processed/nb1703_summary.json
    data/processed/nb1703_mean_bag_oof.npy        (253,) float32
"""
...
# Alt-params (nb1701 alt-grid CatBoost)
CB_DEPTH = 4
CB_ITERATIONS = 250
CB_LR = 0.04
CB_L2 = 4.0
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1554 (no leakage on PRE-unblind path)."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d{CB_DEPTH}, n{CB_ITERATIONS}, "
          f"lr{CB_LR}, l2={CB_L2}) on 5-way K-tuned 117-col matrix")
    print(f"        anchor          = {ANCHOR}  (PRE-unblind)")
    ...
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST RESIDUAL CROSS-FIT  "
          f"(dim={feat_dim}; lr={CB_LR}, n_est={CB_ITERATIONS}, "
          f"depth={CB_DEPTH}, l2={CB_L2})")
    print("-" * 78)
    ...
    # ---- Verdict (0.003 margin vs nb1554) ----
    ...
    if beats_nb1554:
        verdict = "NB1703_ALT_PARAMS_BEATS_NB1554_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1554:
        verdict = "NB1703_ALT_PARAMS_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = "NB1703_ALT_PARAMS_BEATS_ANCHOR_BUT_WORSE_THAN_NB1554"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "NB1703_ALT_PARAMS_FLAT_VS_ANCHOR"
    else:
        verdict = "NB1703_ALT_PARAMS_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "alt_grid_source": "nb1701_alt_grid",
        "catboost_loss": "MAE",
        "catboost_depth": CB_DEPTH,
        "catboost_iterations": CB_ITERATIONS,
        "catboost_learning_rate": CB_LR,
        "catboost_l2_leaf_reg": CB_L2,
        "K_AP_best": K_AP_best,
