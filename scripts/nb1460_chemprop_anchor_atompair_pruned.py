"""nb1460 -- PRE-unblind path: SHAP-pruned AtomPair top-30 + ChEMBL
residual learner anchored to chemprop_aux (instead of nb1070).

Hypothesis:
    nb1373 anchored to nb1070 (POST-unblind, inner-fit on 4139+253) hit
    mean_bag RAE 0.5095 on the 253 cross-fit but is POST-unblind so
    cross-fit RAE no longer transfers cleanly to LB (per the two-regime
    calibration: PRE-unblind in_RAE ~ LB + 0.003; POST-unblind unreliable,
    typically LB 0.7-0.9).

    chemprop_aux is PRE-unblind (trained on 4139 only), giving an in_RAE
    on 253 of 0.6216 -> predicted LB ~0.6246 (current best honest LB
    candidate).  By swapping the anchor from nb1070 to chemprop_aux but
    keeping the SHAP-pruned AtomPair-30 + ChEMBL residual frame from
    nb1373, we get a PRE-unblind ladder candidate whose 253 cross-fit
    number may translate to LB with the +0.003 PRE-unblind delta.

Protocol:
    1.  Anchor = chemprop_aux te[unb_idx] (PRE-unblind).
        residual = y_unb - anchor.
    2.  ChEMBL PXR pool: identical union as nb1373 / nb1352 / nb1242.
        kNN-5 Tanimoto on cached Morgan-2048 over standardized test
        SMILES; produce pred_chembl_pec50 + mean_sim for all 513 rows,
        slice to unblind.
    3.  Build FULL 2050-col matrix (AtomPair-2048 + pred_chembl + sim)
        on 253; train ONE seed-0 shallow LGBM Huber on residual; pull
        SHAP importance via shap.TreeExplainer with LGBM-gain fallback.
        Slice AtomPair-only importance, pick top-30 bit indices.
    ...
    5.  5-seed bag (seeds [0, 1, 7, 42, 137]), KFold(n=5) cross-fit per
        ...
        lr=0.05, huber_alpha=1.0, min_child_samples=20).  Mean-bag and
        median-bag pooled RAE.
    6.  Verdict at 0.003 margin vs:
            chemprop_aux  standalone (anchor)   0.6216
            nb1373 nb1070-anchor mean-bag       0.5095

Outputs:
    scripts/nb1460_chemprop_anchor_atompair_pruned.py
    data/processed/nb1460_summary.json
    data/processed/nb1460_mean_bag_oof.npy        (253,) float32
    data/processed/nb1460_median_bag_oof.npy      (253,) float32
    data/processed/nb1460_per_seed_corrected_oof.npy (5, 253) float32
"""
...
TAG = "nb1460"
ANCHOR = "chemprop_aux"           # PRE-unblind anchor (te file on 513)
...
CHEMPROP_AUX_REF = 0.6216         # chemprop_aux te[unb_idx] in_RAE (PRE-unblind)
NB1373_NB1070_REF = 0.5095        # nb1373 (nb1070 anchor) mean-bag RAE
...
def main() -> dict:
    ...
    # ---- Build FULL feature matrix (AtomPair + pred_chembl + sim) ----
    ...
    # AtomPair-only importance slice
    ...
    print(f"   pooled RAE(mean_bag)   = {rae_mean_bag:.4f}  "
          ...
          f"  d_vs_nb1373_nb1070 = {rae_mean_bag - NB1373_NB1070_REF:+.4f})")
    print(f"   pooled RAE(median_bag) = {rae_median_bag:.4f}  "
          f"(d_vs_anchor = {rae_median_bag - rae_anchor:+.4f}"
          f"  d_vs_nb1373_nb1070 = {rae_median_bag - NB1373_NB1070_REF:+.4f})")
    print(f"   anchor (chemprop_aux)  = {rae_anchor:.4f}")
    print(f"   nb1373/nb1070 ref     = {NB1373_NB1070_REF:.4f}")

    beats_anchor = rae_mean_bag < rae_anchor - DECISION_MARGIN
    beats_nb1373 = rae_mean_bag < NB1373_NB1070_REF - DECISION_MARGIN

    if beats_nb1373:
        verdict = (
            "CHEMPROP_AUX_ANCHOR_BEATS_NB1373_NB1070_NEW_PRE_UNBLIND_PRIMARY"
        )
    elif beats_anchor and abs(rae_mean_bag - NB1373_NB1070_REF) < DECISION_MARGIN:
        verdict = "CHEMPROP_AUX_ANCHOR_FLAT_VS_NB1373_NB1070_PRE_UNBLIND_CANDIDATE"
    elif beats_anchor:
        verdict = (
            "CHEMPROP_AUX_ANCHOR_BEATS_STANDALONE_BUT_WORSE_THAN_NB1373_NB1070"
        )
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CHEMPROP_AUX_ANCHOR_FLAT_VS_STANDALONE_NO_RESIDUAL_LIFT"
    else:
        verdict = "CHEMPROP_AUX_ANCHOR_HURTS_STANDALONE"
    ...
    summary = {
        ...
        "delta_mean_bag_vs_nb1373_nb1070": rae_mean_bag - NB1373_NB1070_REF,
        "beats_chemprop_aux": bool(beats_anchor),
        "beats_nb1373_nb1070": bool(beats_nb1373),
        "verdict": verdict,
        "chemprop_aux_ref": CHEMPROP_AUX_REF,
        "nb1373_nb1070_ref": NB1373_NB1070_REF,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_mean_bag_vs_nb1373_nb1070",
        "beats_chemprop_aux", "beats_nb1373_nb1070",
        "verdict",
    ):
        print(f"  {k}: {res.get(k)}")
