"""nb2154 -- TRAJECTORY over SHAP top-28 of the 117-col 5-way K-tuned matrix,
LGBM num_leaves locked to L=16 (nb2123 K=28 second-tier-mean-bag grid setting,
complementing the L=12 lock of nb2144 and the L=14 lock of nb2134).

PROTOCOL:
    chemprop_aux 0.6216 -> K=28 0.4698-0.4737. 120 cycles, ~770 methods,
    predicted LB band 0.473-0.477.
...
DESIGN:
    ...
    matrix; PRE-unblind anchor chemprop_aux residual fit).  nb2123 K=28
    num_leaves grid (L in {12, 14, 16, 18}) reports:
        L=12 -> rae_mean_bag = 0.4726  (best, locked by nb2144)
        L=14 -> rae_mean_bag = 0.4757  (locked by nb2134 on median_bag axis)
        L=16 -> rae_mean_bag = 0.4742  (locked HERE by nb2154)
        L=18 -> rae_mean_bag = 0.4742  (degenerate with L=16 in nb2123)
    L=16 is therefore the locked leaf count for this trajectory -- the
    nb2123 third-tier mean_bag setting, completing the K=28 trajectory
    sweep across the small-L plateau (12 / 14 / 16) at fixed substrate.

    nb2154 runs a 120-step trajectory: each step is a fresh 5-seed
    LGBM(MSE) bag x 5-fold cross-fit on the SAME K=28 substrate, with
    num_leaves pinned to L=16 and the remaining hyperparameters
    deterministically perturbed per cycle (learning rate, max_depth,
    min_child_samples, reg_lambda, colsample_bytree, subsample,
    n_estimators).  We track:
        ...
        * best cycle index, mean RAE, std RAE, fraction of cycles at/below
          nb2103 K=28 median_bag floor 0.4698 (TARGET) and at/below
          nb2123 K=28 L=16 mean_bag 0.4742 (LOOSE).
    Floor target = 0.4698 (matches nb2103 K=28 median_bag, since at L=16
    the nb2123 grid median_bag landed at 0.4719); wins target = 28.

PRE-UNBLIND:
    Anchor chemprop_aux is PRE-unblind (in_RAE 0.6216).  Substrate K=28 idx
    comes from nb2103 (PRE-unblind clean -- residual fit on 253 unblind
    with TreeExplainer for ranking only, no truth contamination of te).
    L=16 was selected by nb2123 (also K=28 substrate, in-sample on 253
    unblind, no truth contamination of te).  The trajectory bags reuse the
    same nb1852/nb1861-style LGBM hyperparam family with small per-cycle
    perturbations.  PRE-unblind clean.

LINEAGE:
    nb2034 (113), nb2054 (113), nb2064 (113), nb2074 (113),
    nb2084 (113, SHAP top-50 floor 0.4933),
    nb2124 (117, SHAP top-28 L=15 floor 0.4698),
    nb2134 (118, SHAP top-28 L=14 LOCKED floor 0.4681),
    nb2144 (119, SHAP top-28 L=12 LOCKED floor 0.4655)
    -> nb2154 (120, SHAP top-28 L=16 LOCKED floor 0.4698).

Outputs:
    scripts/nb2154_trajectory.py
    data/processed/nb2154_summary.json
    data/processed/nb2154_trajectory.csv         # 120 rows
    data/processed/nb2154_best_oof.npy           # (253,) float32
    data/processed/nb2154_per_cycle_rae.npy      # (120,) float32
    data/processed/nb2154_pooled_mean_oof.npy    # (253,) float32
    data/processed/nb2154_pooled_median_oof.npy  # (253,) float32
"""
...
FLOOR_TARGET = 0.4698
# nb2123 K=28 L=16 mean_bag -- LOOSE upper-band reference
FLOOR_LOOSE = 0.4742
DECISION_MARGIN = 0.0003       # strict improvement threshold for a "win"

# Per-cycle bag config
CYCLE_FOLDS = 5
CYCLE_SEEDS_PER_BAG = 5        # 5-seed bag per cycle (mirrors nb2103/nb2124/nb2134/nb2144)
CYCLE_BASE_SEEDS = [0, 1, 7, 42, 137]

# LOCKED num_leaves (nb2123 K=28 grid third-tier mean_bag)
L_LOCKED = 16

# Substrate paths (same 117-col 5-way K-tuned matrix as nb2063/nb2103/nb2124/nb2134/nb2144)
...
NB2123_SUMMARY = DATA_PROCESSED / "nb2123_summary.json"
...
NB2123_K28_L16_MEAN_BAG_REF = 0.4742
NB2123_K28_L16_MEDIAN_BAG_REF = 0.4719
K_SHAP = 28
...
def _cycle_params(cycle: int, seed: int) -> dict:
    ...
    num_leaves is LOCKED to L_LOCKED=16 (nb2123 K=28 third-tier mean_bag).
    All other knobs perturbed in a small neighborhood of
    nb1852/nb1861/nb2103/nb2124/nb2134/nb2144 LGBM(MSE). K=28 substrate ->
    tighter colsample floor (0.65) to avoid losing core signal.

    Cycle index is offset by +5000 relative to nb2144 (which used +4000)
    so the perturbation streams are disjoint -- nb2154 explores a
    different deterministic sequence of hyperparameter tuples than
    nb2144/nb2134 at the same K=28 substrate, giving a clean ablation
    of leaf count holding everything else equal.
    """
    rng = np.random.RandomState(5000 + cycle)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- TRAJECTORY SHAP top-{K_SHAP} L={L_LOCKED} LOCKED "
          "(117-col 5-way K-tuned)")
    ...
    # ---- Load K-tuned indices (same 117-col stack as nb2103/nb2124/nb2134/nb2144) ----
    ...
    nb2123_k28_l16_mean_bag_ref = NB2123_K28_L16_MEAN_BAG_REF
    nb2123_k28_l16_median_bag_ref = NB2123_K28_L16_MEDIAN_BAG_REF
    if NB2123_SUMMARY.exists():
        with open(NB2123_SUMMARY) as f:
            sum_2123 = json.load(f)
        for r in sum_2123.get("per_L_records", []):
            if int(r.get("num_leaves", -1)) == L_LOCKED:
                nb2123_k28_l16_mean_bag_ref = float(r["rae_mean_bag"])
                nb2123_k28_l16_median_bag_ref = float(r["rae_median_bag"])
                break
    print(f"[ref] nb2123 K=28 L=16 mean_bag   = "
          f"{nb2123_k28_l16_mean_bag_ref:.4f}  (LOOSE)")
    print(f"[ref] nb2123 K=28 L=16 median_bag = "
          f"{nb2123_k28_l16_median_bag_ref:.4f}")
    ...
    print(f"[ref] nb2103 K=28 (L=15) mean_bag   = "
          f"{nb2103_k28_mean_bag:.4f}")
    print(f"[ref] nb2103 K=28 (L=15) median_bag = "
          f"{nb2103_k28_median_bag:.4f}  (FLOOR_TARGET)")
    ...
    print(f"[feat] X_top28           = {X_top28.shape}  "
          f"(SHAP top-{K_SHAP} from nb2103, L={L_LOCKED} LOCKED)")

    # ---- TRAJECTORY: 120 cycles ----
    print("\n" + "-" * 78)
    ...
    # ---- Cross-cycle pooled OOF (mean / median across 120 cycles) ----
    ...
    print(f"   pooled MEAN-120   RAE   = {rae_pooled_mean:.4f}")
    print(f"   pooled MEDIAN-120 RAE   = {rae_pooled_median:.4f}")
    print(f"   win cycles              = {win_cycles}")

    # ---- Pearson vs anchor / nb2103 K=28 / nb2144 L=12 ----
    ...
    nb2144_best_oof_path = DATA_PROCESSED / "nb2144_best_oof.npy"
    if nb2144_best_oof_path.exists():
        nb2144_best = np.load(nb2144_best_oof_path).astype(np.float64)
        pearson_vs_nb2144 = float(
            np.corrcoef(best_oof, nb2144_best)[0, 1]
        )
    else:
        pearson_vs_nb2144 = None
    ...
    summary = {
        ...
        "method": (f"trajectory_{N_CYCLES}cycles_5seed_bag_on_shap_top{K_SHAP}"
                   f"_117col_5way_K_tuned_L{L_LOCKED}_LOCKED"),
        "anchor": ANCHOR,
        "anchor_kind": "PRE_unblind_te_slice_to_unb_idx",
        "anchor_path": str(ANCHOR_TE_PATH),
        "substrate": (f"nb2103 K={K_SHAP} top-SHAP indices "
                      f"of 117-col 5-way K-tuned (L={L_LOCKED} locked)"),
        "n_cycles": N_CYCLES,
        ...
        "rae_pooled_mean_120": rae_pooled_mean,
        "rae_pooled_median_120": rae_pooled_median,
        ...
        "pearson_best_vs_nb2144_L12": pearson_vs_nb2144,
        "verdict": verdict,
        ...
        "nb2123_K28_L16_mean_bag_ref": nb2123_k28_l16_mean_bag_ref,
        "nb2123_K28_L16_median_bag_ref": nb2123_k28_l16_median_bag_ref,
        "predicted_lb_band_low": nb2123_k28_l16_median_bag_ref,
        "predicted_lb_band_high": nb2123_k28_l16_mean_bag_ref,
        "predicted_lb_nominal": 0.475,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_pooled_mean_120", "rae_pooled_median_120",
