"""nb1620 -- CatBoost on 6-way K-tuned 167-col features (5-way + ChemBERTa K=50).

HYPOTHESIS:
    nb1554 (5-way K-tuned CatBoost MAE,d4,n200,lr0.05,l2=5 on 117 cols) hit
    pooled mean_bag RAE = 0.5163 on the 253 PRE-unblind set anchored on
    chemprop_aux (in_RAE 0.6216).  nb1611 swept SHAP K on ChemBERTa-77M-MTR
    embeddings (384-d) as a standalone residual family and reported best_K=20
    with mean_bag RAE = 0.5528 -- WORSE than nb1554 standalone, but the
    importance ranking surfaces a sub-symbolic ChemBERTa subspace that may
    add orthogonal capacity when *concatenated* with the 5-way K-tuned stack.

    This script tests the 6-way add: take the nb1554 117-col stack and
    splice top-50 ChemBERTa dims (broader than nb1611's best_K=20, to let
    CatBoost prune via depth-4 + l2_leaf_reg=5).  Total dim = 167.

PROTOCOL:
    1. Build features (167 cols):
...
Outputs:
    scripts/nb1620_catboost_6way_chemberta.py     (this file)
    data/processed/nb1620_summary.json
    data/processed/nb1620_mean_bag_oof.npy        (253,) float32
"""
...
def main() -> dict:
    ...
    print(f"{TAG} -- CatBoost(MAE,d4,n200,lr0.05,l2=5) on 6-way K-tuned "
          f"167-col matrix (5-way + ChemBERTa top-{K_CHEMBERTA})")
    ...
    if beats_nb1561_bob:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_BEATS_NB1561_BOB_"
                   "NEW_PRE_UNBLIND_PRIMARY")
    elif flat_vs_nb1561_bob:
        verdict = "CATBOOST_6WAY_CHEMBERTA_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_BEATS_NB1554_BUT_"
                   "WORSE_THAN_NB1561_BOB")
    elif flat_vs_nb1554:
        verdict = "CATBOOST_6WAY_CHEMBERTA_FLAT_VS_NB1554"
    elif beats_anchor:
        verdict = ("CATBOOST_6WAY_CHEMBERTA_BEATS_ANCHOR_BUT_"
                   "WORSE_THAN_NB1554")
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_6WAY_CHEMBERTA_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_6WAY_CHEMBERTA_HURTS_ANCHOR"
    ...
    summary = {
        ...
        "K_source": {
            ...
            "ChemBERTa": ("nb1611 SHAP top-50 (broader than nb1611 best_K=20 "
                          "to let CatBoost depth-4 + l2=5 prune)"),
        },
