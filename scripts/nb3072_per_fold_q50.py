"""nb3072 -- Per-fold q50 from fold-TRAIN (not fold-val) for nb3063-style split.

NEW PARADIGM:
    Quantile-conditional hard-split blend on {K18, K19} deep-30, but the q50
    cutoff is computed EXCLUSIVELY from the fold-train K18 OOF predictions
    (never from fold-val). This is the cleanest cross-fit form -- the split
    threshold carries no validation-fold information.

PROTOCOL (per kf_seed, 5-fold scaffold split):
    1. Inside each outer fold: q50 = median(K18_pred[fold_train])
       (the split threshold is a function of fold-TRAIN ONLY).
    2. Apply to fold-VAL rows:
         row_pred_K18 <= q50  -> w_K18=0.8, w_K19=0.2  (low-half / inactive tail)
         row_pred_K18 >  q50  -> w_K18=0.5, w_K19=0.5  (high-half / active tail)
    3. Stitch the 5 fold-val predictions into oof_blend (253,);
       pooled_rae across folds.
    Repeat for 15 fresh kf_seeds {1096..1110}.

    Deploy:
        - Compute q50 = median of K18 OOF on the FULL 253 (deploy proxy).
        - For te (513): rows where te_K18 <= q50 -> (0.8, 0.2);
          else (0.5, 0.5). Apply, clip [3, 9], save te_nb3072.
...
References:
    ...
    nb3030 wide-seed 15-mean ceiling      = 0.4509   <- gate
    nb3063 nb3072-precursor 5-seed mean   = 0.4477  (kf 1051..1055)
    nb3070 wide-seed verify 15-mean       = 0.4477  (kf 1096..1110)
...
Outputs:
    data/processed/nb3072_summary.json
    data/processed/nb3072_pred_oof.npy  (253,) float32 -- median-seed OOF
    data/processed/te_nb3072.npy        (513,) float32 -- deploy te
    submissions/nb3072_per_fold_q50.csv (only on BETTER_THAN_NB3030)
"""
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- Per-fold q50 from fold-TRAIN (no val leakage) on "
          f"{K_LABELS} deep-30")
    print(f"          low (K18<=q50): (K18={W_LOW_K18}, K19={W_LOW_K19})")
    print(f"          high (K18 >q50): (K18={W_HIGH_K18}, K19={W_HIGH_K19})")
    ...
    print(f"   ref nb3063 5-seed precursor  = {REF_NB3063:.4f}")
    ...
    if mean_rae < GATE_BETTER_THAN_NB3030:
        ...
        ladder_action = (
            f"PROMOTE candidate. nb3072 fold-TRAIN q50 quantile blend 15-mean "
            f"{mean_rae:.4f} beats nb3030 ceiling {REF_NB3030:.4f} "
            f"({mean_rae - REF_NB3030:+.4f}). Cleaner cross-fit -- q50 derived "
            "purely from fold-train; no val info leakage."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3072 fold-TRAIN q50 15-mean {mean_rae:.4f} "
            f"not better than nb3030 ceiling {REF_NB3030:.4f} "
            f"({mean_rae - REF_NB3030:+.4f}). Keep nb3030 / prior PRIMARY-1."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_q50.csv"
    ...
    summary = {
        ...
        "method": "per_fold_TRAIN_q50_quantile_conditional_blend_K18_K19_deep30",
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "delta_vs_nb3030", "delta_vs_nb3070",
