"""nb2134 -- TRAJECTORY over SHAP top-28 of the 117-col 5-way K-tuned matrix,
LGBM num_leaves locked to L=14 (nb2123 best-median-bag K=28 setting).

PROTOCOL:
    chemprop_aux 0.6216 -> K=28 L=14 median 0.4681 nominal, 28 wins,
    predicted LB ~0.471, 118 cycles, ~750 methods.
...
DESIGN:
    ...
    num_leaves grid reports median_bag = 0.4681 at L=14 (best across
    {12, 14, 16, 18}); L=14 is therefore the locked leaf count for this
    trajectory.

    nb2134 runs a 118-step trajectory: each step is a fresh 5-seed
    LGBM(MSE) bag x 5-fold cross-fit on the SAME K=28 substrate, with
    num_leaves pinned to L=14 and the remaining hyperparameters
    ...
    n_estimators).  We track:
        ...
        * best cycle index, mean RAE, std RAE, fraction of cycles at/below
          nb2123 K=28 L=14 median_bag floor 0.4681
    Floor target = 0.4681 (matches nb2123 K=28 L=14 median_bag);
    wins target = 28.
...
PRE-UNBLIND:
    ...
    L=14 was selected by nb2123 (also K=28 substrate, in-sample on 253
...
Outputs:
    scripts/nb2134_trajectory.py
    data/processed/nb2134_summary.json
    data/processed/nb2134_trajectory.csv         # 118 rows
    data/processed/nb2134_best_oof.npy           # (253,) float32
    data/processed/nb2134_per_cycle_rae.npy      # (118,) float32
    data/processed/nb2134_pooled_mean_oof.npy    # (253,) float32
    data/processed/nb2134_pooled_median_oof.npy  # (253,) float32
"""
...
N_CYCLES = 118
WINS_TARGET = 28
# nb2123 K=28 L=14 median_bag (honest cross-fit RAE) -- target floor
FLOOR_TARGET = 0.4681
# nb2103 K=28 (L=15) median_bag -- looser upper-band reference
FLOOR_LOOSE = 0.4698
DECISION_MARGIN = 0.0003       # strict improvement threshold for a "win"

# Per-cycle bag config
CYCLE_FOLDS = 5
CYCLE_SEEDS_PER_BAG = 5        # 5-seed bag per cycle (mirrors nb2063/nb2103/nb2124)
CYCLE_BASE_SEEDS = [0, 1, 7, 42, 137]

# LOCKED num_leaves (nb2123 K=28 best-median-bag L)
L_LOCKED = 14

# Substrate paths (same 117-col 5-way K-tuned matrix as nb2063/nb2103/nb2124)
...
NB2123_K28_L14_MEAN_BAG_REF = 0.4757
NB2123_K28_L14_MEDIAN_BAG_REF = 0.4681
K_SHAP = 28
...
def _cycle_params(cycle: int, seed: int) -> dict:
    ...
    num_leaves is LOCKED to L_LOCKED=14 (nb2123 K=28 best median_bag).
    All other knobs perturbed in a small neighborhood of
    nb1852/nb1861/nb2103/nb2124 LGBM(MSE). K=28 substrate -> tighter
    colsample floor (0.65) to avoid losing core signal.
    """
    rng = np.random.RandomState(3000 + cycle)
...
def main() -> dict:
    ...
    # ---- Load K-tuned indices (same 117-col stack as nb2103/nb2124) ----
    ...
    nb2123_k28_l14_mean_bag_ref = NB2123_K28_L14_MEAN_BAG_REF
    nb2123_k28_l14_median_bag_ref = NB2123_K28_L14_MEDIAN_BAG_REF
    if NB2123_SUMMARY.exists():
        ...
        for r in sum_2123.get("per_L_records", []):
            if int(r.get("num_leaves", -1)) == L_LOCKED:
                nb2123_k28_l14_mean_bag_ref = float(r["rae_mean_bag"])
                nb2123_k28_l14_median_bag_ref = float(r["rae_median_bag"])
                break
    print(f"[ref] nb2123 K=28 L=14 mean_bag   = "
          f"{nb2123_k28_l14_mean_bag_ref:.4f}")
    print(f"[ref] nb2123 K=28 L=14 median_bag = "
          f"{nb2123_k28_l14_median_bag_ref:.4f}  (FLOOR_TARGET)")
    ...
    print(f"[ref] nb2103 K=28 (L=15) median_bag = "
          f"{nb2103_k28_median_bag:.4f}  (LOOSE)")
    ...
    # ---- Cross-cycle pooled OOF (mean / median across 118 cycles) ----
    ...
    print(f"   pooled MEAN-118   RAE   = {rae_pooled_mean:.4f}")
    print(f"   pooled MEDIAN-118 RAE   = {rae_pooled_median:.4f}")
    ...
    summary = {
        ...
        "rae_pooled_mean_118": rae_pooled_mean,
        "rae_pooled_median_118": rae_pooled_median,
        ...
        "nb2123_K28_L14_mean_bag_ref": nb2123_k28_l14_mean_bag_ref,
        "nb2123_K28_L14_median_bag_ref": nb2123_k28_l14_median_bag_ref,
        "predicted_lb_band_low": nb2123_k28_l14_median_bag_ref,
        "predicted_lb_band_high": nb2123_k28_l14_mean_bag_ref,
        "predicted_lb_nominal": 0.471,
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        "rae_pooled_mean_118", "rae_pooled_median_118",
