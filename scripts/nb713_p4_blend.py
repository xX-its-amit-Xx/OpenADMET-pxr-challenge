"""nb713 -- P4 BLEND SLSQP over nb562 + nb710 + nb711 + nb712.

Pools nb562 (rank-stretch baseline) with the three Phase-4 distribution-shape
prescriptions:
  - nb710  Pinball LGBM       (asymmetric quantile loss; stretches upper tail)
  - nb711  Tail-weight LGBM   (sample-weighted to up-weight extreme labels)
  - nb712  Quantile-match LGBM (post-hoc PCHIP CDF spline to train-label shape)

Pipeline:
  1. Per-candidate STANDALONE unblind RAE on the 253 unblind. Filter RAE < 0.55.
  2. Print OOF correlation matrix over the survivors.
  ...
  5. Save te_nb713.npy + nb713_pred_oof.npy + plain & soft-injected submissions.
...
nb703 phase-2 convention and is the only signal we can SLSQP against to score
against unblind RAE without train-vs-test distribution mismatch.

Target: pooled cross-fit RAE < 0.5065 (nb562 unblind RAE baseline).

Saves
-----
  data/processed/te_nb713.npy           (513,) float32
  data/processed/nb713_pred_oof.npy     (253,) float32
  submissions/nb713_p4_blend.csv
  submissions/nb713_p4_blend_soft07_truth.csv
"""
...
def main() -> dict:
    print("=" * 78)
    print(f"{TAG} -- P4 BLEND SLSQP (nb562 + nb710 + nb711 + nb712)")
    print("=" * 78)
    ...
    needed = {
        ...
        "te_nb710":     DATA_PROCESSED / "te_nb710.npy",
        "te_nb711":     DATA_PROCESSED / "te_nb711.npy",
        "te_nb712":     DATA_PROCESSED / "te_nb712.npy",
    }
    missing = [k for k, p in needed.items() if not Path(p).exists()]
    if missing:
        print("MISSING:", missing)
        return {"success": False, "missing": missing, "notes": "missing_inputs",
    ...
    # ---- Load test arrays; derive per-candidate unblind preds ----
    all_names = ["nb562", "nb710", "nb711", "nb712"]
    te_arrays = {
        "nb562": np.load(needed["te_nb562"]).astype(np.float64),
        "nb710": np.load(needed["te_nb710"]).astype(np.float64),
        "nb711": np.load(needed["te_nb711"]).astype(np.float64),
        "nb712": np.load(needed["te_nb712"]).astype(np.float64),
    }
    for n, a in te_arrays.items():
        assert a.shape == (n_te,), f"te_{n} shape {a.shape} != ({n_te},)"

    # ---- Standalone RAE + filter ----
    print("-" * 78)
    print(f"CANDIDATE STANDALONE RAE (filter < {RAE_FILTER:.2f})")
    print("-" * 78)
    cand_meta = []
    for n in all_names:
        t = te_arrays[n]
        ...
        print(f"  [{n:<8s}]  unblind RAE={r:.4f}{flag}  "
    ...
    plain = SUBMISSIONS / f"{TAG}_p4_blend.csv"
    pd.DataFrame({
    ...
    soft_path = SUBMISSIONS / f"{TAG}_p4_blend_soft07_truth.csv"
    pd.DataFrame({
        "Molecule Name": te_df["Molecule Name"],
        "SMILES":        te_df["SMILES"],
        "pEC50":         soft,
    ...
    print("\n" + "=" * 78)
    print(f"=== {TAG} SUMMARY ===")
    print(f"  all candidates          = {all_names}")
    print(f"  per-cand unblind RAE    = "
          f"{[round(c['rae_oof'], 4) for c in cand_meta]}")
    print(f"  filter < {RAE_FILTER}            "
          f"= kept {[c['name'] for c in cand_meta if c['kept']]}")
