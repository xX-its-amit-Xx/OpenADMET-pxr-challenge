"""nb3202 -- Learned per-fold optimal clip percentiles on nb2943 anchor.

NEW PARADIGM:
    Apply the learned-clip operator (per-fold inner grid search picking
    (q_low, q_high) on fold-train) to the WEAK-CLUSTER anchor nb2943
    (fine ratio grid winner, baseline 0.4576). nb3190 showed learned-clip
    can compound on a stronger anchor (nb3090); here we test the orthogonal
    hypothesis -- does the same clip operator extract more headroom from the
    weak-cluster (2D scalar grid, df=0) substrate where variance compression
    is structurally different?

    Hypothesis: nb2943 (0.4576) is a 2D scalar-grid blend without any
    learning, so its variance band is set entirely by the underlying
    K-ensemble + nb1191 mixture. Learned-clip historically extracts
    ~-0.0015 to -0.0040 RAE on already-tight ensembles by bounding the
    extreme-tail predictions. Targets:
      mean < 0.4426 -> BETTER (compounds anchor + clip beyond nb2943)
      mean < 0.4437 -> MARGINAL (matches nb3173 learned-clip ceiling)
      else          -> FAIL

PROTOCOL (per kf_seed, 5-fold scaffold split):
    pred_base = nb2943_pred_oof  (253,)
    Per outer fold:
    ...
    Repeat for 15 FRESH kf_seeds {1186..1200}.

GATE (on 15-seed mean):
    mean < 0.4426 -> "BETTER"
    mean < 0.4437 -> "MARGINAL"
    else          -> "FAIL"

References:
    nb2943 fine-ratio-grid winner  = 0.4576  <- parent anchor (weak cluster)
    nb3173 learned-clip on nb3080  = 0.4437  (clip-operator ceiling)
    nb3190 learned-clip on nb3090  = compounding target
    nb3090 quantile-cond blend     = 0.4472
...
Inputs:
    ...
    data/processed/nb2943_pred_oof.npy
    data/processed/te_nb2943.npy

Outputs:
    data/processed/nb3202_summary.json
    data/processed/nb3202_pred_oof.npy   (253,) float32 -- median-seed OOF
    data/processed/te_nb3202.npy         (513,) float32 -- deploy te
    submissions/nb3202_clip_on_nb2943.csv  (only on BETTER/MARGINAL)
"""
...
# -- Inputs --------------------------------------------------------------------
PRED_OOF_PATH = DATA_PROCESSED / "nb2943_pred_oof.npy"
TE_PATH = DATA_PROCESSED / "te_nb2943.npy"
...
# -- Gates ---------------------------------------------------------------------
GATE_BETTER = 0.4426    # mean < this -> BETTER
GATE_MARGINAL = 0.4437  # mean < this -> MARGINAL

# -- References ----------------------------------------------------------------
REF_PARENT_NB2943 = 0.4576
REF_NB3173 = 0.4437
REF_NB3190_TARGET = 0.4422
REF_NB3090 = 0.4472
REF_NB3080 = 0.4475
REF_NB2171 = 0.4682
...
def main() -> dict:
    ...
    print(
        ...
        f"{PARENT_TAG} pred_oof (weak-cluster anchor)"
    )
    print(
    ...
    # -- Load nb2943 anchor pred_oof + te -------------------------------------
    print("\n" + "-" * 78)
    ...
    print(
        f"\n   ref {PARENT_TAG} baseline         = "
        f"{REF_PARENT_NB2943:.4f}"
    )
    print(
        f"   delta vs {PARENT_TAG}             = "
        f"{mean_rae - REF_PARENT_NB2943:+.4f}"
    )
    print(
    ...
    if mean_rae < GATE_BETTER:
        verdict = "BETTER"
        ladder_action = (
            f"PROMOTE-CANDIDATE. nb3202 15-seed mean {mean_rae:.4f} beats "
            f"BETTER gate {GATE_BETTER:.4f} "
            f"({mean_rae - GATE_BETTER:+.4f}). Learned-clip operator extracts "
            f"meaningful headroom from the weak-cluster (nb2943, 2D scalar "
            f"grid df=0) substrate. Anchor {REF_PARENT_NB2943:.4f} -> "
            f"{mean_rae:.4f} = {REF_PARENT_NB2943 - mean_rae:.4f} RAE reduction. "
            f"Modal pick was (q{ql_mode}, q{qh_mode}). Re-verify with deep-30 "
            f"before PRIMARY-1 swap. CAUTION: anchor is POST-unblind (any "
            f"hybrid carries +0.10 LB penalty per anchor_contamination_chain)."
        )
    elif mean_rae < GATE_MARGINAL:
        verdict = "MARGINAL"
        ladder_action = (
            f"MARGINAL. nb3202 15-seed mean {mean_rae:.4f} beats MARGINAL "
            f"gate {GATE_MARGINAL:.4f} ({mean_rae - GATE_MARGINAL:+.4f}) but "
            f"misses BETTER gate {GATE_BETTER:.4f}. Learned-clip on the "
            f"nb2943 weak-cluster anchor matches the nb3173 plateau. "
            f"Consider as alternate; do not displace PRIMARY-1 without "
            f"deep-30 verify."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT. nb3202 15-seed mean {mean_rae:.4f} fails both gates "
            f"(BETTER {GATE_BETTER:.4f}, MARGINAL {GATE_MARGINAL:.4f}). "
            f"Delta vs parent nb2943 = {mean_rae - REF_PARENT_NB2943:+.4f}, "
            f"delta vs nb3173 = {mean_rae - REF_NB3173:+.4f}. Learned-clip "
            f"either overfits the inner train of nb2943 or the weak-cluster "
            f"anchor's variance band is already tight enough that clip cannot "
            f"compress further. Keep nb2943 / nb3173 on the ladder."
        )
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_clip_on_nb2943.csv"
    ...
    summary = {
        ...
        "method": (
            "per_fold_learned_clip_grid_search_on_nb2943_weak_cluster_anchor"
        ),
        ...
        "ref_parent_nb2943": REF_PARENT_NB2943,
        "delta_vs_parent": round(mean_rae - REF_PARENT_NB2943, 4),
        "ref_nb3173": REF_NB3173,
        "delta_vs_nb3173": round(mean_rae - REF_NB3173, 4),
        "ref_nb3190_target": REF_NB3190_TARGET,
    ...
    print(f"   95% CI                = [{ci_low:.4f}, {ci_high:.4f}]")
    print(f"   delta vs nb2943       = {mean_rae - REF_PARENT_NB2943:+.4f}")
