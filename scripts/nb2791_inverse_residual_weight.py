"""nb2791 -- Per-row inverse-residual sample weighting (vs nb2784 entropy weighting).

NEW PARADIGM (cycle 175+ prescription):
    OPPOSITE of nb2784: instead of DOWN-weighting high-disagreement rows,
    UP-weight rows where the chemprop_aux anchor is WRONG. The intuition:
    hard rows (large |residual|) carry the marginal signal that K=20 LGBM
    can still exploit, so a residual-learner should focus there. Easy rows
    (small residual) are already covered by the anchor.

WEIGHT RECIPE:
    Per row i (on 253 unblind):
        w_i_raw = 1 / (|y_i - chemprop_aux_pred_oof_i| + 0.5)
    Then normalize so mean(w) == 1.0 (LGBM sample_weight convention).
    eps=0.5 keeps the dynamic range bounded (max factor ~2x vs typical
    residual std 0.5 log-units), preventing single rows from dominating.

ANCHOR:
    chemprop_aux (nb1133_chemprop_aux_pred_oof.npy) -- the only verified-clean
    PRE-unblind anchor (cf. feedback_kaggle_chemprop_dead_end). Residual
    target = y_unb - chemprop_aux_oof.

LGBM:
    K=20 RFE features sliced from the 117-col 5-way matrix
    (AtomPair / MACCS / Mordred / ChempropEmbed / Avalon + ChEMBL kNN).
    Hyperparams identical to nb2240 / nb2784 K=20:
        max_depth=4, num_leaves=15, n_est=300, lr=0.03,
        min_child_samples=5, reg_lambda=2.0.
    K=20 LGBM seeds per fold (vs 5 in nb2784) -- larger bag because we
    expect the inverse-resid weights to add variance.

CV:
    5-fold scaffold CV on 253, 5 kf_seeds, mean over 5 LGBM seeds within
...
OUTPUTS:
    data/processed/nb2791_summary.json
    data/processed/nb2791_pred_oof.npy   (253,) float32
    data/processed/te_nb2791.npy         (513,) float32
"""
...
# ---- Hyperparameters ----
EPS = 0.5                         # bounded inverse-residual epsilon
N_FOLDS = 5
KF_SEEDS = [1001, 1002, 1003, 1004, 1005]
# K=20 LGBM seeds (bigger bag than nb2784's 5 because inverse-resid weights
...
# ============================================================================
# helpers (identical to nb2784 feature pipeline)
# ============================================================================
...
# ---------------------------------------------------------------------------
# inverse-residual weights
# ---------------------------------------------------------------------------

def inverse_residual_weights(y_train, anchor_train, eps=EPS):
    """
    Per-row UP-weighting of hard rows (where anchor is wrong).

    w_i_raw = 1 / (|y_i - anchor_i| + eps)
    Normalize: w = w_raw / mean(w_raw) so mean(w) == 1.0.

    eps=0.5 caps the dynamic range. With residuals typically ~U[0, 1.5],
    w_raw ranges roughly 0.5 to 2.0, w (post-normalize) roughly 0.5x-2x.
    """
    resid = np.asarray(y_train, dtype=np.float64) - np.asarray(anchor_train, dtype=np.float64)
    w_raw = 1.0 / (np.abs(resid) + float(eps))
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- inverse-residual sample weighting (UP-weight hard rows)")
    print("=" * 78)
    ...
    w = inverse_residual_weights(y_unb, anchor_oof, eps=EPS)
    ...
    diag = {
        ...
        "n_top_decile_resid": int((abs_resid >= np.percentile(abs_resid, 90)).sum()),
        "weight_top_decile_resid_mean": float(
            w[abs_resid >= np.percentile(abs_resid, 90)].mean()
        ),
        "weight_bottom_decile_resid_mean": float(
            w[abs_resid <= np.percentile(abs_resid, 10)].mean()
        ),
    }
    print(
        f"[weights] eps={EPS}  |resid|[mean/median/max]="
    ...
    print(
        f"[sanity] expected UP-weight on hard rows: "
        f"top_decile_resid_w_mean ({diag['weight_top_decile_resid_mean']:.3f}) "
        f"{'<' if diag['weight_top_decile_resid_mean'] < diag['weight_bottom_decile_resid_mean'] else '>'} "
    ...
    summary = {
        "tag": TAG,
        "method": "per_row_inverse_residual_reweight",
        "paradigm_note": (
            "UP-weight hard rows where anchor is wrong (vs nb2784 entropy "
            "which DOWN-weighted high-disagreement rows)."
        ),
        "anchor": "chemprop_aux",
        ...
        "eps": EPS,
        "weight_recipe": "w_raw = 1 / (|y - anchor_pred| + eps); w = w_raw / mean(w_raw)",
        "weight_stats": diag,
        "kf_seeds": KF_SEEDS,
