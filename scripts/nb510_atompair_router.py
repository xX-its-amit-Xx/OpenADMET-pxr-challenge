"""nb510 -- ATOM-PAIR RESIDUAL ROUTER.

Hypothesis: AtomPair fingerprints encode topological distance pairs (a 3rd
distinct feature space relative to MACCS substructure keys (nb502) and the
multimodal 33-col matrix (nb492)). The shallow LGBM router on top of nb464
should learn a residual signal that is decorrelated from both nb492 and nb502,
giving an even better blend candidate.

Pipeline mirrors nb502 exactly except for the feature matrix:
  1. Anchor: nb464 (te_nb464.npy).
  2. Features: AtomPair fingerprints, 2048 bits
     (rdkit.Chem.rdFingerprintGenerator.GetAtomPairGenerator(fpSize=2048)).
  ...
  6. Honest unblind RAE via cross-fit resid_oof (target: < 0.5283 nb492,
     ideally near 0.5126 nb502).
  7. Refit on all 253 -> deploy resid_hat (513,).
  8. Save te_nb510.npy + pred_oof + resid_oof; emit plain + soft07 CSVs.
  9. Cache fingerprints at data/processed/{tr,te}_atompair.npy for reuse.
...
def _atompair_gen():
    return rdFingerprintGenerator.GetAtomPairGenerator(fpSize=FP_DIM)


def atompair_bits(smi: str, gen) -> np.ndarray | None:
    if not smi:
        return None
...
def build_atompair_matrix(smiles: list[str], tag: str) -> np.ndarray:
    cache = DATA_PROCESSED / f"{tag}_atompair.npy"
    if cache.exists():
        arr = np.load(cache)
        if arr.shape == (len(smiles), FP_DIM):
            print(f"  loaded cached AtomPair: {cache.name} shape={arr.shape}")
            return arr
    print(f"  computing AtomPair for {len(smiles)} {tag} molecules ...")
    gen = _atompair_gen()
    out = np.zeros((len(smiles), FP_DIM), dtype=np.uint8)
    n_fail = 0
    for i, smi in enumerate(smiles):
        bits = atompair_bits(smi, gen)
        if bits is None:
            n_fail += 1
            continue
        out[i] = bits
...
def main() -> dict:
    print("=" * 78)
    print(f"{TAG} -- ATOM-PAIR RESIDUAL ROUTER (anchor={ANCHOR_NAME})")
    print("=" * 78)
    ...
    # ---- AtomPair feature matrices ----
    print("\nBuilding AtomPair feature matrices:")
    X_tr = build_atompair_matrix(tr_smis, "tr").astype(np.float32)  # cache reuse
    X = build_atompair_matrix(te_smis, "te").astype(np.float32)
    ...
    # ---- Correlation against nb502 and nb492 residual_oof ----
    ...
    plain = SUBMISSIONS / f"{TAG}_atompair_router.csv"
    pd.DataFrame({
    ...
    soft_path = SUBMISSIONS / f"{TAG}_atompair_router_soft07_truth.csv"
    pd.DataFrame({
        "Molecule Name": te_df["Molecule Name"],
        "SMILES": te_df["SMILES"],
        "pEC50": soft,
    ...
    print(f"  feature space                         = AtomPair (2048 bits)")
