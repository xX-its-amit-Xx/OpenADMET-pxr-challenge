"""nb1380 -- DEPLOY of nb1373 (SHAP-pruned AtomPair top-30 + ChEMBL) to 513.

Reuses the top-30 AtomPair bit indices computed by nb1373 (SHAP on a single
seed-0 LGBM on the full 2050-col residual matrix).  Builds the pruned 32-col
feature matrix on all 513 test rows, fits a 5-seed bag of shallow LGBM Huber
on ALL 253 unblind rows with target

    residual = y_unb - nb1070_pred_oof

(no KFold -- this is the deploy fit) and predicts the residual on 513.
Mean-bag AND median-bag residual_513 vectors are added to te_nb1070 to
produce two deploy vectors.

Honest LB anchors (from nb1373 cross-fit):
    * mean-bag        0.5095
    * median-bag      0.5128

Outputs:
    data/processed/te_nb1380_mean.npy        (513,) float32
    data/processed/te_nb1380_median.npy      (513,) float32
    data/processed/nb1380_summary.json
    submissions/nb1380_deploy_nb1373_mean.csv     (513 rows: SMILES,Molecule Name,pEC50)
    submissions/nb1380_deploy_nb1373_median.csv   (513 rows: SMILES,Molecule Name,pEC50)
"""
...
HONEST_LB_ANCHOR_MEDIAN = 0.5128
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- DEPLOY nb1373 (SHAP-pruned AtomPair top-30 + ChEMBL) -> 513")
    print(f"          anchor={ANCHOR}  parent={PARENT}")
    print(f"          seeds = {RESID_SEEDS}  (deploy fit, no KFold)")
    print(f"          honest LB anchors: mean={HONEST_LB_ANCHOR_MEAN}  "
          f"median={HONEST_LB_ANCHOR_MEDIAN}")
    print("=" * 78)

    # ---- Top-30 AtomPair bit indices from nb1373 summary ----
    parent_summary_path = DATA_PROCESSED / f"{PARENT}_summary.json"
    ...
    if len(top_bit_idx_ranked) != top_k:
        raise ValueError(
            f"nb1373 top-{top_k} indices mismatch: got {len(top_bit_idx_ranked)}"
        )
    print(f"[load] nb1373 top-{top_k} AtomPair bits (ranked) = "
    ...
    # ---- 5-seed deploy bag: fit on ALL 253 unblind rows, predict on 513 ----
    print("\n" + "-" * 78)
    n_seeds = len(RESID_SEEDS)
    print(f"DEPLOY 5-SEED BAG ({n_seeds} fits on full 253 unblind)")
    print("-" * 78)
    ...
    for si, s in enumerate(RESID_SEEDS):
        ...
        print(f"   seed {s:3d}:  in-sample RAE_253 = {in_rae:.4f}  "
              f"resid_513 mean={resid_513.mean():+.4f}  std={resid_513.std():.4f}")

    # ---- Mean / Median bag aggregates ----
    mean_resid_513 = seed_resid_513.mean(axis=0)
    median_resid_513 = np.median(seed_resid_513, axis=0)
    te_nb1380_mean = te_anchor_513 + mean_resid_513
    te_nb1380_median = te_anchor_513 + median_resid_513

    # ---- In-sample RAE on unblind slice (deploy fit -> in-sample) ----
    in_rae_mean = float(rae(y_unb, te_nb1380_mean[unb_idx]))
    in_rae_median = float(rae(y_unb, te_nb1380_median[unb_idx]))

    print("\n" + "-" * 78)
    print("513-ROW DEPLOY VECTOR DIAGNOSTICS")
    print("-" * 78)
    print(f"   mean_resid_513    mean={mean_resid_513.mean():+.4f}  "
          f"std={mean_resid_513.std():.4f}  "
          f"min={mean_resid_513.min():+.4f}  max={mean_resid_513.max():+.4f}")
    ...
    print(f"   te_nb1380_mean    mean={te_nb1380_mean.mean():.4f}  "
          f"std={te_nb1380_mean.std():.4f}  "
          f"min={te_nb1380_mean.min():.4f}  max={te_nb1380_mean.max():.4f}")
    print(f"   te_nb1380_median  mean={te_nb1380_median.mean():.4f}  "
          f"std={te_nb1380_median.std():.4f}  "
          f"min={te_nb1380_median.min():.4f}  max={te_nb1380_median.max():.4f}")
    ...
    np.save(te_mean_path, te_nb1380_mean.astype(np.float32))
    np.save(te_median_path, te_nb1380_median.astype(np.float32))
    print(f"\n[save] {te_mean_path}")
    print(f"[save] {te_median_path}")

    # ---- Save CSV ----
    SUBMISSIONS.mkdir(parents=True, exist_ok=True)
    mean_csv_path = SUBMISSIONS / f"{TAG}_deploy_nb1373_mean.csv"
    median_csv_path = SUBMISSIONS / f"{TAG}_deploy_nb1373_median.csv"
    df_mean = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1380_mean.astype(np.float64),
    })
    df_median = pd.DataFrame({
        "SMILES": test_smiles,
        "Molecule Name": mol_names,
        "pEC50": te_nb1380_median.astype(np.float64),
    })
    ...
    summary = {
        ...
        "te_nb1380_median_stats": {
            "mean": float(te_nb1380_median.mean()),
            "std": float(te_nb1380_median.std()),
            "min": float(te_nb1380_median.min()),
            "max": float(te_nb1380_median.max()),
        },
