"""nb2981 -- Per-fold SLSQP simplex on 5 anchors: K=18, K=20, K=24, K=28, nb1191.

NEW PARADIGM:
    Extend nb2973 (4 K-anchors per-fold simplex, mean_rae 0.4539) with the
    nb1191 PRE-unblind wide-seed pyramid OOF as a 5th anchor.  Hypothesis:
    nb1191 sits on a different residual axis (its substrate is the post-hoc
    blend route, not the deep-30 K-pyramid route), so SLSQP simplex on
    5 anchors should give the optimizer access to a partially decorrelated
    direction and beat the 4-K ceiling.

PROTOCOL:
    Anchors: nb2960 deep-30 OOFs+te for K=18,20,24,28, plus nb1191_pred_oof
             (253,) + te_nb1191 (513,).
    ...
    Deploy:
        ...
        - Apply to (513, 5) stacked te arrays -> te_nb2981
...
References:
    nb2973 (4-K per-fold simplex)  = 0.4539
    nb2960 K18 deep-30 OOF         = 0.4536  (best single K)
    nb2960 K20 deep-30 OOF         = 0.4625
    nb2960 K24 deep-30 OOF         = 0.4687
    nb2960 K28 deep-30 OOF         = 0.4740
    nb1191 PRE-pyramid wide-seed   = 0.4718
...
Inputs:
    ...
    data/processed/nb1191_pred_oof.npy
    data/processed/te_nb1191.npy

Outputs:
    data/processed/nb2981_summary.json
    data/processed/nb2981_pred_oof.npy   (253,) float32 -- per-fold simplex OOF
    data/processed/te_nb2981.npy         (513,) float32 -- deploy te
    submissions/nb2981_per_fold_simplex_5K_nb1191.csv  (only if verdict != "FAIL")
"""
...
# -- Inputs --------------------------------------------------------------------
ANCHOR_LABELS = ["K18", "K20", "K24", "K28", "nb1191"]
OOF_PATHS = {
...
TE_PATHS = {
    ...
    "K24": DATA_PROCESSED / "nb2960_K24_30seed_te.npy",
    "K28": DATA_PROCESSED / "nb2960_K28_30seed_te.npy",
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- per-fold SLSQP simplex on 5 anchors {ANCHOR_LABELS}")
    print(f"          parent={PARENT_TAG} (4-K per-fold simplex, mean_rae 0.4539)")
    print(f"          new anchor: nb1191 PRE-pyramid wide-seed (RAE 0.4718)")
    ...
    for k in ANCHOR_LABELS:
        ...
        per_anchor_full_rae[k] = round(r, 4)
        print(f"   {k:>8s}: oof_RAE = {r:.4f}  te_mean={te_arr.mean():.3f}  "
    ...
    # Leak sanity
    leak_flags = {}
    for i, k in enumerate(ANCHOR_LABELS):
    ...
    print(f"          {'  '.join([f'{k:>7s}' for k in ANCHOR_LABELS])}")
    for i, ki in enumerate(ANCHOR_LABELS):
        row = "  ".join([f"{corr_mat[i, j]:7.3f}" for j in range(K)])
        print(f"   {ki:>8s}  {row}")
    ...
    for fold_i, (tr_loc, va_loc) in enumerate(splits):
        ...
        fold_records.append({
            ...
            "weights": {ANCHOR_LABELS[k]: round(float(w[k]), 4) for k in range(K)},
            "train_rae": round(float(r_train), 4),
            "val_rae": round(r_val, 4),
            "max_w": round(float(w.max()), 4),
            "degenerate": degen,
        })
        wstr = "  ".join(f"{ANCHOR_LABELS[k]}={w[k]:.3f}" for k in range(K))
    ...
    w_stack = np.stack(fold_w_list, axis=0)  # (5, 5)
    w_mean = w_stack.mean(axis=0)
    w_mean = w_mean / w_mean.sum()
    print(f"   mean-of-fold weights: "
          + ", ".join(f"{ANCHOR_LABELS[k]}={w_mean[k]:.4f}" for k in range(K)))
    ...
    full_pool_weights = {ANCHOR_LABELS[k]: round(float(w_full[k]), 4) for k in range(K)}
    ...
    for k in range(K):
        ...
        print(f"     w[{ANCHOR_LABELS[k]:8s}] = {w_full[k]:+.4f}{flag}")
    ...
    delta_vs_nb1191 = pooled_rae - REF_NB1191
    ...
    print(f"   delta vs nb1191 (0.4718)  = {delta_vs_nb1191:+.4f}")
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_per_fold_simplex_5K_nb1191.csv"
    if verdict != "FAIL":
        pd.DataFrame({
            "SMILES": te_smiles,
    ...
    summary = {
        ...
        "method": "per_fold_slsqp_simplex_5anchors_4K_plus_nb1191",
        "paradigm": "extend_nb2973_with_nb1191_pre_pyramid",
        "anchor_pool": ANCHOR_LABELS,
        "anchor_pre_unblind": True,
        "per_anchor_full_oof_rae": per_anchor_full_rae,
        "anchor_leak_eq_truth_frac": leak_flags,
        "oof_corr_matrix": corr_mat.tolist(),
        "oof_corr_labels": ANCHOR_LABELS,
        "n_folds": N_FOLDS,
        "kf_seed": KF_SEED,
        ...
        "mean_w_across_folds": {ANCHOR_LABELS[k]: round(float(w_mean[k]), 4)
                                for k in range(K)},
    ...
    print("\n" + "=" * 78)
    print(f"=== {TAG} SUMMARY ===")
    print(f"   per-anchor full-OOF RAE   = "
          + ", ".join([f"{k}={v:.4f}" for k, v in per_anchor_full_rae.items()]))
    print(f"   pooled outer-val RAE      = {pooled_rae:.4f}")
    print(f"   mean-of-fold weights      = "
          + ", ".join([f"{ANCHOR_LABELS[k]}={w_mean[k]:.3f}" for k in range(K)]))
...
if __name__ == "__main__":
    ...
    for k in (
        ...
        print(f"  {k}: {res.get(k)}")
    print(f"  per_anchor_full_oof_rae: {res.get('per_anchor_full_oof_rae')}")
