"""nb1173_gate — honest ensemble gate for the AhR 4-head GNN vs the DEPLOYED sisterNR 3-head GNN.
Mirrors nb1169_gate; control = sn_oof (deployed), treatment = ahr_oof (deployed + AhR aux).
Deploy: ahr < deployed best - 0.001 AND ahr < control on the matched 4-GBM deployed config.
"""
import os, sys, json
import numpy as np
...
def run_config(archs, label, Xtr, ytr, se, n_tr, chem, tab):
    ...
    arms = {"control": "sn_oof", "ahr": "ahr_oof"}
    res = {k: [] for k in arms}
...
def main():
    ...
    out = {"5arch": {"means": m5, "std": s5, "ahr_minus_control": round(d5, 4)},
           "4gbm_deployed": {"means": m4, "std": s4, "ahr_minus_control": round(d4, 4)}}
    json.dump(out, open(f"{P}/nb1173_summary.json", "w"), indent=2)

    prev = json.load(open(BEST))["rae"] if os.path.exists(BEST) else 0.4367
    ahr4, ctl4 = m4["ahr"], m4["control"]
    if d4 < -0.001 and ahr4 < ctl4:
        print(f"\nDEPLOY-WORTHY: 4-GBM matched ahr-control {d4:+.4f} (<-0.001). Build deploy submission next.")
    else:
