"""nb1384 -- SHAP K-grid for AtomPair: top-K AtomPair bits + ChEMBL.

Hypothesis:
    nb1373 used K=30 (mean-bag RAE 0.5095).  This sweep tests whether
    K in {10, 20, 50, 75, 100, 150} captures more (or less) signal than
    K=30, holding everything else constant.

Protocol:
    1.  Reuse SHAP importance ranking for AtomPair-2048 -- compute via one
        global LGBM Huber (seed=0) on full 2050-col feature matrix on the
        253 unblind rows, exactly as nb1373.
    2.  For each K in {10, 20, 50, 75, 100, 150}:
            features = top-K AtomPair bits (by SHAP) + pred_chembl + sim
                     = (K + 2) columns
    3.  5-seed bag (seeds [0, 1, 7, 42, 137]), 5-fold cross-fit per seed.
    4.  Compare mean-bag RAE vs K; identify best-K.
    5.  Verdict at 0.003 margin vs nb1373 K=30 (0.5095).

Outputs:
    scripts/nb1384_atompair_K_grid.py        (this file)
    data/processed/nb1384_summary.json
    data/processed/nb1384_best_K_oof.npy     mean-bag OOF for best K (253,)
    data/processed/nb1384_per_K_mean_bag_oof.npy  (6, 253) all K mean-bags
"""
...
def _bag_for_K(X_unb_pruned: np.ndarray, residual: np.ndarray,
    ...
    """5-seed residual cross-fit; return (per_seed_rae, mean_bag_oof, rae_mean_bag,
    rae_median_bag, median_bag_oof, per_seed_corrected, resid_oof_stats)."""
    n_unb = len(y_unb)
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- SHAP K-grid AtomPair (top-K) + ChEMBL residual; "
          f"anchor={ANCHOR}")
    print(f"          K_grid = {K_GRID}")
    print(f"          seeds  = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          baseline: nb1373 K=30 mean_bag = {NB1373_REF:.4f}  "
    ...
    # ---- Report table ----
    print("\n" + "=" * 78)
    print("K-GRID RAE TABLE")
    print("=" * 78)
    print(f"  {'K':>5}  {'feat_dim':>9}  {'mean_bag':>10}  {'median_bag':>11}  "
          f"{'per_seed_std':>13}  {'d_vs_nb1373':>12}")
    print("  " + "-" * 70)
    for r in per_K_records:
        ...
        print(f"  {r['K']:>5d}  {r['feat_dim_pruned']:>9d}  "
              ...
              f"{r['delta_vs_nb1373']:>+12.4f}{flag}")
    print(f"  nb1373 K=30 reference                       {NB1373_REF:.4f}  "
          f"(margin = {DECISION_MARGIN})")
    print(f"  best K = {best_K}  (mean_bag = {best_rae_mean_bag:.4f})  "
          f"d_vs_nb1373 = {delta_best_vs_nb1373:+.4f}")

    if beats_nb1373:
        verdict = f"K_GRID_BEST_K{best_K}_BEATS_NB1373_NEW_PRIMARY_CANDIDATE"
    elif ties_nb1373:
        verdict = f"K_GRID_BEST_K{best_K}_FLAT_VS_NB1373"
    else:
        verdict = f"K_GRID_BEST_K{best_K}_WORSE_THAN_NB1373"
