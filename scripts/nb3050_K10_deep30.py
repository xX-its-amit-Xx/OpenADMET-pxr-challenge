"""nb3050 -- Build K=10 deep-30, then 15-seed wide-seed per-fold SLSQP simplex
            on 3 K-anchors {K10, K18, K19} all at deep-30.

NEW PARADIGM (cycle 251+):
    Explore very small K (K=10) -- FAR from the K=18/K=19 region in feature
    space. The hypothesis is that aggressively-pruned feature slices may
    carry a different residual correlation structure than the standard
    K=17..K=23 cluster, opening a new blend axis.
    ...
    LGBM on K-feature slices of nb2231 117-col matrix):
        ...
        nb3043 {K18, K19, K22} = ?         (under test)
    The nb3050 K=10 swap pushes WAY below the prior K-grid (K17..K23) to
    test whether sparse feature-set diversity translates to genuine
    orthogonality after deep-30 averaging removes optimism.
...
PROTOCOL:
    STEP A  -- Build K=10 deep-30 (seeds 3001..3030) with identical recipe
               to nb3010 / nb3000 / nb2960 / nb3020 (chemprop_aux anchor +
               residual LGBM on K=10 feature slice of nb2231 117-col matrix).
               K=10 reconstructed from nb2231 RFE trajectory.

    STEP B  -- Load K18/K19 deep-30 caches; stack with new K10 deep-30.
               15-seed wide-seed per-fold SLSQP simplex on {K10, K18, K19}.
               kf_seeds = {1081..1095} (FRESH, NOT used by any prior wide-
               seed verify -- 1001 used by single-fits, 1006..1080 used by
               nb2990/nb3001/nb3030/nb3043 etc).

    STEP C  -- Deploy: SLSQP refit on FULL 253 -> single weight vector ->
               apply to (513, 3) stacked te arrays -> te_nb3050.

GATE (on 15-seed mean pooled RAE):
    mean < 0.4509 -> "BETTER_THAN_NB3030"
        (nb3050 beats current wide-seed-verified PRIMARY-1 nb3030 ceiling;
         very-small-K paradigm CONFIRMED.)
    else          -> "FAIL"
        (K=10 substrate change does not break ceiling.)
...
Inputs:
    ...
    data/processed/nb2231_summary.json   (K=10 idx via RFE trajectory)
...
Outputs:
    data/processed/nb3050_summary.json
    data/processed/nb3050_K10_30seed_oof.npy   (253,) float32 -- K10 deep-30 OOF
    data/processed/te_nb3050_K10.npy           (513,) float32 -- K10 deep-30 te
    data/processed/nb3050_pred_oof.npy         (253,) float32 -- median-seed OOF
    data/processed/te_nb3050.npy               (513,) float32 -- full-pool deploy te
    submissions/nb3050_K10_deep30_simplex.csv  (only if BETTER_THAN_NB3030)
"""
...
TAG = "nb3050"
PARENT_TAG = "nb2960+nb3000+nb3050(K10 deep30)"

# -- K-pyramid target ---------------------------------------------------------
K_TARGET_REBUILD = 10
K_LABELS = ["K10", "K18", "K19"]
K_DEPTH = {"K10": "deep30", "K18": "deep30", "K19": "deep30"}

# -- Anchor + residual params (IDENTICAL recipe to nb3000 / nb2960 / nb3020) --
...
N_FOLDS = 5
KF_SEEDS = list(range(1081, 1096))   # 15 fresh kf_seeds {1081..1095}
                                     # EXCLUDES 1001 (single-fits) and
                                     # 1006..1080 (prior wide-seed verifies)
N_STARTS_FOLD = 8
N_STARTS_FULL = 12
DEGEN_MAX_W = 0.85
...
# ============================================================================
# helpers (lifted verbatim from nb3020 / nb3010 / nb3000)
# ============================================================================
...
def main():
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- build K=10 deep-30, then 15-seed wide-seed per-fold SLSQP "
    ...
    print(f"          fresh kf_seeds    = {KF_SEEDS[0]}..{KF_SEEDS[-1]} "
          f"(n={len(KF_SEEDS)})")
    print(f"          NEW PARADIGM: K=10 explores very-small-K region, FAR from "
          f"K=18/K=19 in feature space")
    ...
    # -- Reconstruct K=10 indices --------------------------------------------
    print("\n" + "-" * 78)
    print(f"STEP 1: reconstruct K=10 idx from nb2231 RFE trajectory")
    print("-" * 78)
    if not NB2231_SUMMARY.exists():
        raise FileNotFoundError(f"missing {NB2231_SUMMARY}")
    with open(NB2231_SUMMARY) as f:
        nb2231 = json.load(f)
    K10_idx = np.array(reconstruct_K_from_trajectory(nb2231, K_TARGET_REBUILD), dtype=int)
    if len(K10_idx) != K_TARGET_REBUILD:
        raise ValueError(f"K=10 idx reconstruction returned {len(K10_idx)} cols")
    print(f"   K=10 idx (n={len(K10_idx)}): {K10_idx.tolist()}")
    ...
    # -- Build K=10 deep-30 --------------------------------------------------
    print("\n" + "-" * 78)
    print(f"STEP 3: K=10 residual-LGBM with {len(RESID_SEEDS_DEEP)} fresh seeds")
    print("-" * 78)
    bag_oof_K10, bag_te_K10, per_seed_rae_K10 = build_K_30seed_bag(
        "K10", K10_idx, X_te_full, unb_idx, anchor, residual,
        te_anchor_513, n_test, n_unb, RESID_SEEDS_DEEP,
    )
    bag_mean_rae_K10 = float(rae(y_unb, bag_oof_K10))
    per_seed_arr_K10 = np.array(per_seed_rae_K10, dtype=np.float64)
    print(f"\n   [K10] per-seed RAE mean = {per_seed_arr_K10.mean():.4f}  "
          f"std = {per_seed_arr_K10.std(ddof=1):.4f}")
    print(f"   [K10] 30-seed BAG-MEAN RAE = {bag_mean_rae_K10:.4f}")
    K10_oof_path = DATA_PROCESSED / f"{TAG}_K10_30seed_oof.npy"
    K10_te_path = DATA_PROCESSED / f"te_{TAG}_K10.npy"
    np.save(K10_oof_path, bag_oof_K10.astype(np.float32))
    np.save(K10_te_path, bag_te_K10.astype(np.float32))
    print(f"   [save] {K10_oof_path}")
    print(f"   [save] {K10_te_path}")
    ...
    if mean_rae < GATE_BETTER_THAN_NB3030:
        ...
        ladder_action = (
            f"PROMOTE nb3050 to PRIMARY-1. Wide-seed mean {mean_rae:.4f} "
            f"beats nb3030 ceiling {REF_NB3030:.4f}. Very-small-K (K=10) "
            f"paradigm CONFIRMED: aggressively-pruned feature slice carries "
            f"residual orthogonality the K=17..K=23 cluster lacks."
        )
    else:
        verdict = "FAIL"
        ladder_action = (
            f"REJECT nb3050. Wide-seed mean {mean_rae:.4f} >= nb3030 "
            f"ceiling {REF_NB3030:.4f}. Very-small-K paradigm NOT confirmed; "
    ...
    sub_csv = SUBMISSIONS / f"{TAG}_K10_deep30_simplex.csv"
    ...
    summary = {
        ...
        "method": "K10_deep30_then_15seed_wide_seed_perfold_slsqp_K10_K18_K19",
        "paradigm": "very_small_K_substrate_change_K10_vs_K18_K19_cluster",
        ...
        "K10_idx_in_117": K10_idx.tolist(),
        # K10 rebuild
        "K10_per_seed_rae": per_seed_rae_K10,
        "K10_per_seed_rae_mean": float(per_seed_arr_K10.mean()),
        "K10_per_seed_rae_std": float(per_seed_arr_K10.std(ddof=1)),
        "K10_per_seed_rae_min": float(per_seed_arr_K10.min()),
        "K10_per_seed_rae_max": float(per_seed_arr_K10.max()),
        "K10_30seed_bag_mean_rae": bag_mean_rae_K10,
        "K10_oof_path": str(K10_oof_path),
        "K10_te_path": str(K10_te_path),
        "K10_te_mean": float(bag_te_K10.mean()),
        "K10_te_std": float(bag_te_K10.std()),
        # SLSQP simplex
    ...
    print("\n" + "=" * 78)
    print(f"=== {TAG} SUMMARY ===")
    print(f"   K10 deep-30 RAE          = {bag_mean_rae_K10:.4f}")
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "K10_30seed_bag_mean_rae",
        "K10_per_seed_rae_mean",
        "K10_per_seed_rae_std",
        "per_K_full_oof_rae",
        "mean_rae",
        "std_rae",
        "ci95_low",
