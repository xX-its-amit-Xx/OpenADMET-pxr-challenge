"""nb1911 -- LGBM MSE on EXPANDED 6-way K-tuned matrix (adds Morgan-30).

HYPOTHESIS:
    nb1861 reported rae_pooled_25bag = 0.5013 using a 5-outer x 5-inner bag of
    LGBM(objective='regression' / MSE, max_depth=4, num_leaves=15, n_est=300,
    lr=0.03, min_child_samples=5, reg_lambda=2) on the 117-col 5-way K-tuned
    matrix (AtomPair K=25 + MACCS 20 + Mordred K=20 + ChempropEmbed K=20 +
    Avalon 30 + ChEMBL pred + sim) with chemprop_aux te[unb_idx] as anchor.

    Question: does augmenting the matrix with the top-30 SHAP-pruned
    Morgan-2048 bits (from nb1382, ECFP4 r=2 N=2048) -- the OG fingerprint
    family for this project -- give the residual learner additional capacity
    over the 5 chemistry families already present?  Morgan-30 carries ~30
    extra Boolean cols, growing the matrix to 147; if the 6th family is
    redundant the verdict will be flat (no improvement at 0.003 margin).

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]  (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Feature stack (147 cols):
         top-K_AP=25 AtomPair bits (nb1524) +
         top-20      MACCS bits  (nb1352) +
         top-K_M=20  Mordred cols (nb1523) +
         top-K_E=20  Chemprop embed dims (nb1541) +
         top-30      Avalon bits (nb1392) +
         top-30      Morgan-2048 bits (nb1382 SHAP, ECFP4 r=2) +
         pred_chembl_pec50 +
         mean_sim
    3. Single 5-seed bag, seeds = [0, 1, 7, 42, 137].
       Each seed: 5-fold KFold cross-fit LGBM(MSE) on residual; OOF added back
       to anchor.  rae_seed = rae(y_unb, anchor + oof_seed).
    4. mean_bag = mean of 5 seed OOFs added to anchor.
       Verdict at 0.003 margin vs nb1861 (0.5013).
    5. LGBM hyperparams = identical to nb1861/nb1852 (objective='regression',
       max_depth=4, num_leaves=15, n_estimators=300, lr=0.03,
       min_child_samples=5, reg_lambda=2).

Outputs:
    scripts/nb1911_lgbm_mse_6way_morgan.py    (this file)
    data/processed/nb1911_summary.json
    data/processed/nb1911_per_seed_corrected_oof.npy   (5, 253) float32
    data/processed/nb1911_mean_bag_oof.npy             (253,) float32
"""
...
NB1382_SUMMARY = DATA_PROCESSED / "nb1382_summary.json"
...
NB1861_REF = 0.5013          # rae_pooled_25bag reference for nb1861
...
def _lgbm_params(seed: int) -> dict:
    """LGBM(regression / MSE) -- mirrors nb1861/nb1852 exactly."""
    return dict(
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- LGBM MSE 5-seed bag on EXPANDED 6-way K-tuned matrix "
          f"(adds Morgan-30); PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1861 ({NB1861_REF:.4f})  margin = {DECISION_MARGIN}")
    print("=" * 78)
    ...
    if NB1861_SUMMARY.exists():
        ...
        nb1861_ref = float(nb1861_sum.get("rae_pooled_25bag", NB1861_REF))
        print(f"[ref] nb1861_summary.rae_pooled_25bag = {nb1861_ref:.4f}")
    else:
    ...
    for p in (NB1352_SUMMARY, NB1382_SUMMARY, NB1392_SUMMARY,
              NB1484_SUMMARY, NB1523_SUMMARY, NB1524_SUMMARY, NB1541_SUMMARY):
        if not p.exists():
            raise FileNotFoundError(f"missing {p}")
    with open(NB1352_SUMMARY) as f:
        sum_1352 = json.load(f)
    with open(NB1382_SUMMARY) as f:
        sum_1382 = json.load(f)
    ...
    top_morgan_bit_idx = np.array(
        sum_1382["top_morgan_bit_indices_ranked"], dtype=int
    )
    K_Morgan_used = int(len(top_morgan_bit_idx))
    ...
    n_top_morgan = int(K_Morgan_used)
    ...
    print(f"[reuse] top-{n_top_morgan}     Morgan-{MORGAN_NBITS} bits (nb1382 SHAP K={n_top_morgan})")
    ...
    # ---- Morgan-2048 (rebuild via morgan_fp_batch with explicit r=2 N=2048) ----
    t_morgan = time.time()
    fp_test_morgan = morgan_fp_batch(
        std_test_smiles,
        radius=MORGAN_RADIUS, n_bits=MORGAN_NBITS
    )
    print(f"   morgan FP test (r={MORGAN_RADIUS}, n_bits={MORGAN_NBITS}): "
          f"shape={fp_test_morgan.shape}  "
          f"density={fp_test_morgan.mean():.4f}  "
          f"(wall {time.time() - t_morgan:.1f}s)")
    X_morgan_te = fp_test_morgan.astype(np.float32)
    if X_morgan_te.shape[1] != MORGAN_NBITS:
        raise ValueError(
            f"Morgan test width mismatch: {X_morgan_te.shape[1]} vs {MORGAN_NBITS}"
        )
    X_morgan_unb = X_morgan_te[unb_idx]
    X_morgan_unb_top = X_morgan_unb[:, top_morgan_bit_idx].astype(np.float32)
    print(f"[feat] X_morgan_unb_top  = {X_morgan_unb_top.shape}")

    # ---- Assemble 6-way 147-col matrix ----
    X_unb = np.concatenate(
        [
            ...
            X_emb_unb_top,           # 20
            X_av_unb_top,            # 30
            X_morgan_unb_top,        # 30  <-- new family
    ...
    expected_dim = (n_top_ap + n_top_maccs + n_top_mord
                    + n_top_embed + n_top_avalon + n_top_morgan + 2)
    if feat_dim != expected_dim:
        raise ValueError(f"feat_dim {feat_dim} != expected {expected_dim}")
    print(f"\n   COMBINED 6-WAY K-TUNED matrix: {X_unb.shape}  "
          f"(was 117 in nb1861; +{n_top_morgan} Morgan)")

    # ---- 5-SEED BAG LOOP (single bag, no outer loop) ----
    print("\n" + "-" * 78)
    print(f"5-SEED BAG: {len(RESID_SEEDS)} seeds x {RESID_FOLDS}-fold cross-fit"
          f"  (dim={feat_dim})")
    print("-" * 78)
    ...
    for i, s in enumerate(RESID_SEEDS):
        ...
        print(f"   seed {s:4d}:  rae_corr = {rae_s:.4f}  "
    ...
    print(f"   pooled RAE(median_bag) = {rae_median_bag:.4f}  "
          ...
          f"  d_vs_nb1861 = {rae_median_bag - nb1861_ref:+.4f})")
    print(f"   nb1861 ref            = {nb1861_ref:.4f}")

    delta_vs_nb1861 = rae_mean_bag - nb1861_ref
    beats_nb1861 = delta_vs_nb1861 < -DECISION_MARGIN
    flat_vs_nb1861 = abs(delta_vs_nb1861) < DECISION_MARGIN

    if beats_nb1861:
        verdict = "NB1911_BEATS_NB1861_MORGAN_FAMILY_HELPS"
    elif flat_vs_nb1861:
        verdict = "NB1911_FLAT_VS_NB1861_MORGAN_FAMILY_REDUNDANT"
    else:
        verdict = "NB1911_WORSE_THAN_NB1861_MORGAN_FAMILY_HURTS"
    ...
    summary = {
        ...
        "data_source": ("AtomPair-cache + MACCS-cache + "
                        ...
                        "Avalon-cache + Morgan-2048-rebuild + "
...
if __name__ == "__main__":
    ...
    print("\n==== SUMMARY ====")
    for k in (
        "resid_seeds", "resid_folds",
        "K_AP_best", "K_Mord_best", "K_Embed_best",
        "K_Avalon_used", "K_MACCS_fixed", "K_Morgan_used",
        "n_chembl_pool", "feat_dim", "feat_breakdown",
        "rae_anchor_chemprop_aux",
        "per_seed_rae",
        "rae_per_seed_mean", "rae_per_seed_median",
        "rae_per_seed_std", "rae_per_seed_min", "rae_per_seed_max",
