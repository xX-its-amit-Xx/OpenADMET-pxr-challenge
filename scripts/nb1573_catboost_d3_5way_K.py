"""nb1573 -- CatBoost depth-3 on 5-way K-tuned 117-col features.

HYPOTHESIS:
    nb1554 (CatBoost depth=4, n_est=200, lr=0.05, l2=5) on the 117-col
    5-way K-tuned matrix anchored on chemprop_aux yielded mean_bag
    RAE = 0.5163.  nb1561 (BoB across CatBoost configs) sits at 0.5155.
    Hypothesis: a shallower tree (depth=3) with a slightly longer
    schedule (n_est=300) and a softer learning rate (lr=0.04) plus
    stronger L2 regularization (l2=8) reduces residual-cross-fit
    variance on a 253-row in-sample evaluation -- the residual
    distribution on chemprop_aux has std ~0.5 and the n=253 sample
    is small, so over-flexible trees over-shrink the tail.

PROTOCOL:
    1. Anchor = chemprop_aux te[unb_idx]   (PRE-unblind, in_RAE 0.6216).
       residual = y_unb - anchor.
    2. Reuse identical 117-col 5-way K-tuned feature stack from nb1554
       (top-25 AP + top-20 MACCS + top-20 Mord + top-20 Embed
        + top-30 Avalon + pred_chembl_pec50 + mean_sim).
    3. 5-seed bag CatBoost(loss=MAE, depth=3, n_est=300, lr=0.04,
       l2=8).  KFold(n=5, shuffle=True) cross-fit per seed.
    4. Pool mean_bag and median_bag corrected anchors.
    5. Verdict at 0.003 margin vs nb1554 (0.5163) and nb1561 BoB (0.5155).

Outputs:
    scripts/nb1573_catboost_d3_5way_K.py
    data/processed/nb1573_summary.json
    data/processed/nb1573_mean_bag_oof.npy        (253,) float32
"""
...
# CatBoost hyperparams (depth=3 variant)
CAT_DEPTH = 3
CAT_ITERS = 300
CAT_LR = 0.04
CAT_L2 = 8.0
...
def _load_chembl_pool() -> pd.DataFrame:
    """Same union as nb1554/nb1543/nb1492."""
    frames = []
...
def main() -> dict:
    t0 = time.time()
    print("=" * 78)
    print(f"{TAG} -- CatBoost(MAE, d{CAT_DEPTH}, n{CAT_ITERS}, lr{CAT_LR}, "
          f"l2={CAT_L2}) on 5-way K-tuned 117-col matrix; "
          f"PRE-unblind anchor={ANCHOR}")
    print(f"          seeds = {RESID_SEEDS}  folds = {RESID_FOLDS}")
    print(f"          refs: chemprop_aux ({CHEMPROP_AUX_REF:.4f}), "
          f"nb1554 ({NB1554_REF:.4f}), nb1561 BoB ({NB1561_BOB_REF:.4f})  "
    ...
    print("\n" + "-" * 78)
    print(f"PER-SEED CATBOOST(d={CAT_DEPTH}, n={CAT_ITERS}, lr={CAT_LR}, "
    ...
    if beats_nb1561_bob:
        verdict = "CATBOOST_D3_5WAY_K_BEATS_NB1561_BOB_NEW_PRE_UNBLIND_PRIMARY"
    elif flat_vs_nb1561_bob:
        verdict = "CATBOOST_D3_5WAY_K_FLAT_VS_NB1561_BOB"
    elif beats_nb1554:
        verdict = "CATBOOST_D3_5WAY_K_BEATS_NB1554_BUT_WORSE_THAN_NB1561_BOB"
    elif flat_vs_nb1554:
        verdict = "CATBOOST_D3_5WAY_K_FLAT_VS_NB1554"
    elif beats_nb1543:
        verdict = "CATBOOST_D3_5WAY_K_BEATS_NB1543_BUT_WORSE_THAN_NB1554"
    elif beats_anchor:
        verdict = "CATBOOST_D3_5WAY_K_BEATS_ANCHOR_BUT_WORSE_THAN_NB1543"
    elif abs(rae_mean_bag - rae_anchor) < DECISION_MARGIN:
        verdict = "CATBOOST_D3_5WAY_K_FLAT_VS_ANCHOR"
    else:
        verdict = "CATBOOST_D3_5WAY_K_HURTS_ANCHOR"
