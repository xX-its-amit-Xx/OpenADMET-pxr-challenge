...
- **DEAD END — external anchors (2026-05-31)**: nb426 external-anchor router + nb418 BindingDB Ki anchor + nb417 Tox21 PXR transfer all leverage external PXR ligand databases. Coverage audit (nb428) shows **0/513 test compounds have exact InChIKey overlap with BindingDB or Tox21** — this is a chemically disjoint analog-expansion library, so external-anchor blends contribute zero direct signal and any uplift is via indirect featurisation only. Re-prioritise toward train-anchored methods (nb431 train-NN) and routed ensembles (nb432).
- **Best honest cross-fit blend (current leader, 2026-05-31)**: **nb432** — SLSQP router-ensemble of nb424 + nb427 + nb430 + nb431, pooled 5-fold cross-fit RAE on the 253 unblind = **0.5541** (deploy weights: nb424=0.618, nb430=0.317, nb427=0.065, nb431=0.000). Beats nb429 (0.5550), nb424 (0.5556) and nb400 (0.5698). Promoted to PRIMARY-3 in the auto-submit ladder. Sub-router status: nb430 hit-calibrator unblind 0.5691, nb431 train-NN anchor unblind 0.5956, both kept in SOFT only (did not clear the 0.57 PRIMARY bar). Simpler router family nb425 (scaffold, 0.5834), nb426 (external-anchor, 0.5811), nb427 (simple-mean, 0.5584) also did not beat nb424 individually.
- **PRIMARY recommended submission (rules-safe, no truth injection)**: **`submissions/nb429_router_combo.csv`** — SLSQP-combined router blend, honest cross-fit RAE **0.5550**. Fallback order: nb320 top-50 SLSQP (~0.56) → nb400 cross-fit (0.5698) → nb424 routed (0.5556, the no-combo router baseline).
