...
- **2026-06-01 STRUCTURE TRACK — v2 STAGED (14 low-conf re-docks), v1 + v2 BOTH AWAITING USER AUTHORIZATION**:
  - **v1 = `submissions/structure_baseline_v1.zip`** — 184 Boltz-2 cofolded PDBs, mean ligand pLDDT **0.84**, flat layout `<id>-1.pdb`, 36.3 MB uncompressed / 7.7 MB zipped, 184/184 IDs match `pxr-challenge_structure_TEST_BLINDED.csv`, LIG residue present in every file, ligand connectivity matches expected SMILES via RDKit `AssignBondOrdersFromTemplate`, 0 validator errors. Zero-shot Boltz-2 single-seed pose; expected LDDT-PLI band 0.4–0.6.
  - **v2 = `submissions/structure_baseline_v2.zip`** — 184 PDBs total (**170 original Boltz-2 + 14 re-docked low-confidence cases**, the 14 swapped poses are timestamped 2026-06-01 08:45 in the archive; remaining 170 retain the 2026-05-08 14:06–14:07 Boltz-2 cofold timestamps). Same flat layout `<id>-1.pdb`, 34.4 MB uncompressed / 7.7 MB zipped, validated against the structure-track ID list with 0 missing / 0 extra and full LIG-residue + connectivity pass.
  - **BOTH AWAIT USER AUTHORIZATION before sending to the Gradio structure-track endpoint** (separate rate-limit bucket from the activity-track API, so will not collide with the 4h activity cron `5f1285ff`). **Activity track**: nb562 0.5065 LOCKED; auto-cron `5f1285ff` firing the activity ladder (nb562 → nb503 → …) every 4h independently. Memory note `feedback_structure_track_redock.md` captures the v2 re-dock subset and the v1↔v2 decision criteria.
